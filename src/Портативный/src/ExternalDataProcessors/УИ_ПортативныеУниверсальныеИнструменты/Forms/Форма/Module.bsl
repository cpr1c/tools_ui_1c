...
Процедура ПерераспределитьРазделыПоКолонкам(ОбщееКоличествоКомандИнструментов)
	...
	Для Каждого ТекРаздел Из ГруппыРазделов Цикл
		...
		Если (Не ЗначениеЗаполнено(ОстатокРаздела) Или КоличествоЭлементовВРазделе / ОстатокРаздела > 2
			Или ОстатокРаздела < 0) И ТекущийНомерРаздела + 1 <> КоличествоКолонок Тогда
