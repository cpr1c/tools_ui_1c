...
Процедура ОсобенностиОтладкиВПортативныхИнструментах(Команда)
         UT_CommonClient.ОткрытьСтраницуОсобенностейОтладкиПортативныхИнструметов();
КонецПроцедуры
...
Процедура ПриОткрытииЗапускОбработчиковЗапускаИнструментов()
	...
	UT_CommonClient.ПриНачалеРаботыСистемы();
...
Функция ВерсияПлатформыНеМладше(ВерсияДляСравнения) Экспорт
	ВерсияБезСборки=ConfigurationVersionWithoutBuildNumber(CurrentAppVersion());

	Возврат CompareVersionsWithoutBuildNumber(ВерсияБезСборки, ВерсияДляСравнения)>=0;
КонецФункции

&НаКлиенте
Функция ConfigurationVersionWithoutBuildNumber(Знач Версия) Экспорт
...
&НаКлиенте
Функция CurrentAppVersion() Экспорт
...
//
&НаКлиенте
Функция CompareVersionsWithoutBuildNumber(Знач СтрокаВерсии1, Знач СтрокаВерсии2) Экспорт
