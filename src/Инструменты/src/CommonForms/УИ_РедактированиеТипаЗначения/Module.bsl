...
Процедура СформироватьНаборТипов(Параметры)
	...
	Иначе
		Если ТипЗнч(Параметры.НаборТипов) = Тип("Строка") Тогда
			ВременныйМассивТипов = СтрРазделить(Параметры.НаборТипов, ",");
			Для Каждого ТекНабор Из ВременныйМассивТипов Цикл
				ИмяНабора = СокрЛП(ВРег(ТекНабор));
				НаборТипов.Добавить(ИмяНабора);
			КонецЦикла;
		ИначеЕсли ТипЗнч(Параметры.НаборТипов) = Тип("СписокЗначений") Тогда
			Для Каждого ТекНабор Из Параметры.НаборТипов Цикл
				ИмяНабора = СокрЛП(ВРег(ТекНабор.Значение));
				НаборТипов.Добавить(ИмяНабора);
			КонецЦикла;

		ИначеЕсли ТипЗнч(Параметры.НаборТипов) = Тип("Массив") Тогда
			Для Каждого ТекНабор Из Параметры.НаборТипов Цикл
				ИмяНабора = СокрЛП(ВРег(ТекНабор));
				НаборТипов.Добавить(ИмяНабора);
			КонецЦикла;
...
Функция СформироватьНачальныйТипДанных(Параметры)
	...
	ИначеЕсли Параметры.ХранилищеКонтейнераЗначенияТипа <> Неопределено Тогда 
		...
		ХранилищеКонтейнера = Параметры.ХранилищеКонтейнераЗначенияТипа; 
...
Функция СформироватьОграничениеТипа(Параметры)
	...
	Если Параметры.ОграничениеТипа <> Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(ОграничениеТипаПараметр) = Тип("Структура") Тогда
			ЗначениеКонтейнера = УИ_ОбщегоНазначения.ЗначениеИзКонтейнераФормы(ОграничениеТипаПараметр);//ОписаниеТипов
