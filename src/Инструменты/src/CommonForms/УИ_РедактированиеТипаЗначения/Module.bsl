...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СформироватьНаборТипов(Параметры);
	
	НачальныйТипДанных = СформироватьНачальныйТипДанных(Параметры);
	ЗаполнитьСоставНачальногоОписанияТипов(НачальныйТипДанных);
	ОграничениеТипа = СформироватьОграничениеТипа(Параметры);
	...
	ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения = 
		Параметры.ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения;
	ИспользоватьВыборФормыВыбораСсылочногоЗначения = (Параметры.ИспользоватьВыборФормыВыбораСсылочногоЗначения
													  И РежимВыбора
													  И Не ВозвратХранилищаДляКонтейнераЗначения);
	Элементы.ГруппаВыбораФормыВыбораСсылочногоЗначения.Видимость = ИспользоватьВыборФормыВыбораСсылочногоЗначения;
...
Процедура ДеревоТиповВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если РежимВыбора И Не СоставнойТип И ТекСтрока.Групповой Тогда
		Возврат;
	КонецЕсли;
...
// Формирует набор типов значений для редактирования в зависимости от режима запуска или пользовательских настроек
//
// Параметры:
//   Параметры - Структура - параметры формирования набора типов:
//               * РежимЗапуска - Число - режим запуска см. УИ_ОбщегоНазначенияКлиентСервер.РежимыЗапускаРедактораТипа
//               * НаборТипов - Строка - имена наборов типов через запятую
//                            - СписокЗначений из Строка - список имен наборов типов
//                            - Массив из Строка - массив имен наборов типов
&НаСервере
Процедура СформироватьНаборТипов(Параметры)
	...
	Если Параметры.РежимЗапуска = РежимыЗапуска.ТипыХраненияДанных Тогда
	...
	ИначеЕсли Параметры.РежимЗапуска = РежимыЗапуска.ТипыДляПараметровЗапроса Тогда 
		НаборТипов.Добавить(ДоступныеНаборыТипов.Ссылки);
		НаборТипов.Добавить(ДоступныеНаборыТипов.СоставныеСсылки);
		НаборТипов.Добавить(ДоступныеНаборыТипов.Примитивные);
		НаборТипов.Добавить(ДоступныеНаборыТипов.ХранилищеЗначения);
	...
	ИначеЕсли Параметры.РежимЗапуска = РежимыЗапуска.ТипыДляПолейСКД Тогда 
		НаборТипов.Добавить(ДоступныеНаборыТипов.Ссылки);
		НаборТипов.Добавить(ДоступныеНаборыТипов.СоставныеСсылки);
		НаборТипов.Добавить(ДоступныеНаборыТипов.Примитивные);
		НаборТипов.Добавить(ДоступныеНаборыТипов.ХранилищеЗначения);
		НаборТипов.Добавить(ДоступныеНаборыТипов.УникальныйИдентификатор);
		НаборТипов.Добавить(ДоступныеНаборыТипов.Null);
	ИначеЕсли Параметры.РежимЗапуска = РежимыЗапуска.ТипыДляПараметровСКД Тогда 
		НаборТипов.Добавить(ДоступныеНаборыТипов.Ссылки);
		НаборТипов.Добавить(ДоступныеНаборыТипов.СоставныеСсылки);
		НаборТипов.Добавить(ДоступныеНаборыТипов.Примитивные);
		НаборТипов.Добавить(ДоступныеНаборыТипов.ХранилищеЗначения);
	...
	Иначе
		ЗаполнитьНаборТиповПоПараметру(НаборТипов, Параметры.НаборТипов);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьНаборТиповПоПараметру(НаборТипов, ПараметрНаборТипов)
	
	Если ТипЗнч(ПараметрНаборТипов) = Тип("Строка") Тогда
		ВременныйМассивТипов = СтрРазделить(ПараметрНаборТипов, ",");
		Для Каждого ТекНабор Из ВременныйМассивТипов Цикл
			ИмяНабора = СокрЛП(ВРег(ТекНабор));
			НаборТипов.Добавить(ИмяНабора);
		КонецЦикла;
	ИначеЕсли ТипЗнч(ПараметрНаборТипов) = Тип("СписокЗначений") Тогда
		Для Каждого ТекНабор Из ПараметрНаборТипов Цикл
			ИмяНабора = СокрЛП(ВРег(ТекНабор.Значение));
			НаборТипов.Добавить(ИмяНабора);
		КонецЦикла;

	ИначеЕсли ТипЗнч(ПараметрНаборТипов) = Тип("Массив") Тогда
		Для Каждого ТекНабор Из ПараметрНаборТипов Цикл
			ИмяНабора = СокрЛП(ВРег(ТекНабор));
			НаборТипов.Добавить(ИмяНабора);
		КонецЦикла;
	Иначе
		ВызватьИсключение НСтр("ru = 'Передан непредусмотренный тип для параметра ""НаборТипов"".'");
	КонецЕсли;

КонецПроцедуры

// Формирует начальное описание типов на основе переданных параметров.
//
// Параметры:
//   Параметры - Структура:
// 		* ТипДанных - ОписаниеТипов, Строка - описание типов или его XML-представление
// 		* ХранилищеКонтейнераЗначенияТипа - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипа
// 		* ХранилищеКонтейнераЗначенияОписанияТипа - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипа
//
// Возвращаемое значение:
//   ОписаниеТипов - сформированное описание типов
&НаСервере
Функция СформироватьНачальныйТипДанных(Параметры)
	
	ТипОписаниеТипов = Тип("ОписаниеТипов");
	ТипСтрока = Тип("Строка");
	
	Если Параметры.ТипДанных <> Неопределено Тогда
		Если ТипЗнч(Параметры.ТипДанных) = ТипОписаниеТипов Тогда
			НачальныйТипДанных = Параметры.ТипДанных;
		ИначеЕсли ТипЗнч(Параметры.ТипДанных) = ТипСтрока Тогда 
			Попытка
				НачальныйТипДанных = УИ_ОбщегоНазначения.ЗначениеИзСтрокиXML(Параметры.ТипДанных, ТипОписаниеТипов);
			Исключение
				ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось преобразовать переданный тип ""%1"".'"),
										Параметры.ТипДанных);
				УИ_ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
			КонецПопытки;
		Иначе
			ВызватьИсключение НСтр("ru = 'Передан непредусмотренный тип для параметра ""ТипДанных"".'");
		КонецЕсли;
	ИначеЕсли Параметры.ХранилищеКонтейнераЗначенияТипа <> Неопределено Тогда
		НачальныйТипДанных = Новый ОписаниеТипов;
		ОбработатьПараметрХранилищеКонтейнераЗначенияТипа(Параметры.ХранилищеКонтейнераЗначенияТипа,
			НачальныйТипДанных);
	ИначеЕсли Параметры.ХранилищеКонтейнераЗначенияОписанияТипа <> Неопределено Тогда 
		НачальныйТипДанных = Новый ОписаниеТипов;
		ОбработатьПараметрХранилищеКонтейнераЗначенияОписанияТипа(Параметры.ХранилищеКонтейнераЗначенияОписанияТипа,
			НачальныйТипДанных);
	Иначе
		НачальныйТипДанных = Новый ОписаниеТипов;
	КонецЕсли;
	Возврат НачальныйТипДанных;
	
КонецФункции

&НаСервере
Процедура ОбработатьПараметрХранилищеКонтейнераЗначенияТипа(ХранилищеКонтейнераЗначенияТипа, НачальныйТипДанных)
	...
	// Описание параметра см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипа
	ХранилищеКонтейнера = ХранилищеКонтейнераЗначенияТипа; 
	Если ХранилищеКонтейнера <> Неопределено Тогда
		...
		Исключение
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось прочитать тип из контейнера.'"));
			УИ_ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
		КонецПопытки;
	КонецЕсли;
...
&НаСервере
Процедура ОбработатьПараметрХранилищеКонтейнераЗначенияОписанияТипа(ХранилищеКонтейнераЗначенияОписанияТипа,
	НачальныйТипДанных)
	...
	ТипКонтейнераЗначения = УИ_ОбщегоНазначенияКлиентСервер.ТипыКонтейнеровЗначения().ОписаниеТипов;
		// Описание параметра см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипа		
	ХранилищеКонтейнера = Параметры.ХранилищеКонтейнераЗначенияОписанияТипа;
	Если ХранилищеКонтейнера <> Неопределено Тогда
		Попытка
			НачальныйТипДанных = УИ_ОбщегоНазначения.ЗначениеИзСтрокиXML(ХранилищеКонтейнера.Значение, ТипОписаниеТипов);
		Исключение
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось прочитать описание типов из контейнера.'"));
			УИ_ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
		КонецПопытки;
	КонецЕсли;
...
// Формирует строковое представление ограничения типа для использования в форме
//
// Параметры:
//   Параметры - Структура:
//               * ОграничениеТипа - ОписаниеТипов - готовое описание типов
//                                 - Строка - строка, готовая для использования
//                                 - Структура - контейнер формы с описанием типов
//
// Возвращаемое значение:
//   Строка - строковое представление ограничения типа в формате XML или пустая строка
&НаСервереБезКонтекста
Функция СформироватьОграничениеТипа(Параметры)
	
	ОграничениеТипаСтрокой = "";
	Если Параметры.ОграничениеТипа <> Неопределено Тогда
		ОграничениеТипаПараметр = Параметры.ОграничениеТипа;
		Если ТипЗнч(ОграничениеТипаПараметр) = Тип("ОписаниеТипов") Тогда
			ОграничениеТипаСтрокой = УИ_ОбщегоНазначения.ЗначениеВСтрокуXML(ОграничениеТипаПараметр);
		ИначеЕсли ТипЗнч(ОграничениеТипаПараметр) = Тип("Строка") Тогда
			ОграничениеТипаСтрокой = ОграничениеТипаПараметр;
		ИначеЕсли ТипЗнч(ОграничениеТипаПараметр) = Тип("Структура") Тогда
			ЗначениеКонтейнера = УИ_ОбщегоНазначения.ЗначениеИзХранилищаКонтейнераОписанияТипов(ОграничениеТипаПараметр);
			ОграничениеТипаСтрокой = УИ_ОбщегоНазначения.ЗначениеВСтрокуXML(ЗначениеКонтейнера);
		Иначе
			ВызватьИсключение НСтр("ru = 'Передан непредусмотренный тип для параметра ""ОграничениеТипа"".'");
		КонецЕсли;
	КонецЕсли;
	Возврат ОграничениеТипаСтрокой;
...
Функция ВыбранноеОписаниеТипов(Форма, МассивТипов)
	...
	Описание = СформироватьОписаниеТипа(ТипыОписания, ТипыТипом, ТипыСтрокой, КвалификаторЧисла, КвалификаторСтроки,
		КвалификаторДаты);
	ИзменитьОписаниеТипаПоОграничению(Форма, Описание);
...
&НаКлиентеНаСервереБезКонтекста
Функция СформироватьОписаниеТипа(ТипыОписания, ТипыТипом, ТипыСтрокой, КвалификаторЧисла, КвалификаторСтроки,
	КвалификаторДаты)
...
&НаКлиентеНаСервереБезКонтекста
Процедура ИзменитьОписаниеТипаПоОграничению(Форма, Описание)
	
	Если ЗначениеЗаполнено(Форма.ОграничениеТипа) Тогда
		ОписаниеОграниченияТипа = ОписаниеОграниченияТипа(Форма.ОграничениеТипа);
		Если ОписаниеОграниченияТипа <> Неопределено Тогда
			ПроверенныеТипы = Новый Массив;
			Для Каждого ТипОписания Из Описание.Типы() Цикл
				Если ОписаниеОграниченияТипа.СодержитТип(ТипОписания) Тогда
					ПроверенныеТипы.Добавить(ТипОписания);
				КонецЕсли;
			КонецЦикла;
			Описание = Новый ОписаниеТипов(ПроверенныеТипы);
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция ВозвращаемоеХранилищеДляКонтейнера(Описание)
	...
	Если ТипКонтейнераЗначения = ТипыКонтейнеров.Тип Тогда
		ХранилищеТипа = УИ_ОбщегоНазначения.ЗначениеХранилищаКонтейнераТипа(Описание);
	Иначе
		ХранилищеТипа = УИ_ОбщегоНазначения.ЗначениеХранилищаКонтейнераОписаниеТипов(Описание);
	КонецЕсли;
	
	Возврат ХранилищеТипа;
КонецФункции

// Описание содержит тип которые не отображается в тонком клиенте.
// 
// Параметры:
...
//  Булево
&НаСервере
Функция ОписаниеСодержитТипыКоторыеНеПередаютсяСКлиентаНаСервер(ОписаниеТипов)
	Если ОписаниеТипов.Типы().Количество() = 0 Тогда
		Возврат Ложь;
	КонецЕсли;	
		
	ТипыБезКлиентСервернойПередачи = УИ_ОбщегоНазначения.ТипыБезКлиентСервернойПередачи();
	
	Для Каждого ТекТип Из ТипыБезКлиентСервернойПередачи Цикл
		Если ОписаниеТипов.СодержитТип(ТекТип) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция ВозвращаемоеЗначениеФормы() 
	...
	Если ВозвратХранилищаДляКонтейнераЗначения  Тогда
		ВозвращаемоеЗначение = ВозвращаемоеХранилищеДляКонтейнера(Описание);
	Иначе

		ВозвращаемоеЗначение = Описание;
		Если РежимВыбора Тогда
			ВозвращаемоеЗначение = Новый Структура;
			Если ОписаниеСодержитТипыКоторыеНеПередаютсяСКлиентаНаСервер(Описание) Тогда
				ВозвращаемоеЗначение.Вставить("Описание", ВозвращаемоеХранилищеДляКонтейнера(Описание));
			Иначе
...
// Возвращает описание ограничения типа.
// 
// Параметры:
//  ОграничениеТипаСтрокой - Строка - Ограничение типа строкой
// 
// Возвращаемое значение:
//  Неопределено, ОписаниеТипов - Описание ограничения типа
&НаСервереБезКонтекста
Функция ОписаниеОграниченияТипа(ОграничениеТипаСтрокой)
	ОписаниеОграниченияТипа = Неопределено;
	Попытка
		ОписаниеОграниченияТипа = УИ_ОбщегоНазначения.ЗначениеИзСтрокиXML(ОграничениеТипаСтрокой, Тип("ОписаниеТипов"));
	Исключение
		УИ_ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось преобразовать ограничение типа.'"));
	КонецПопытки;
	Если ОписаниеОграниченияТипа = Новый ОписаниеТипов Тогда
		ОписаниеОграниченияТипа = Неопределено;
	КонецЕсли;
	Возврат ОписаниеОграниченияТипа;
КонецФункции

&НаСервере
Процедура ЗаполнитьДеревоТипов(Знач НачальныйТипДанных = Неопределено)
	
	ОписаниеОграниченияТипа = Неопределено;
	Если ЗначениеЗаполнено(ОграничениеТипа) Тогда
		ОписаниеОграниченияТипа = ОписаниеОграниченияТипа(ОграничениеТипа);
	КонецЕсли;
