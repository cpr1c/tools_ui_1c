...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ХранилищеКонтейнераЗначения") Тогда
		...
		ХранилищеКонтейнераЗначения = Параметры.ХранилищеКонтейнераЗначения;//см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаТабличныйДокумент
		Если ХранилищеКонтейнераЗначения <> Неопределено Тогда
			ТабличныйДокумент = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеИзХранилищаКонтейнераТабличногоДокумента(ХранилищеКонтейнераЗначения);
		КонецЕсли;
		
	ИначеЕсли Параметры.ТабличныйДокумент = Неопределено Тогда
	...
	Если Параметры.Свойство("ВозвращатьХранилищеТаблицыЗначений") Тогда
		//@skip-check unknown-form-parameter-access
		Если Параметры.ВозвращатьХранилищеТаблицыЗначений Тогда
			ВозвращатьХранилищеТаблицыЗначений = Истина;
		КонецЕсли;	
	КонецЕсли;
	...
	Элементы.РедактироватьВоВнешнейПрограмме.Видимость = Ложь;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = УИ_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сохранить изменения в %1?'"), ИмяДокумента);
	УИ_ОбщегоНазначенияКлиент.ПоказатьВопросПользователю(ОписаниеОповещения, ТекстВопроса , РежимДиалогаВопрос.ДаНет);
...
Процедура Применить(Команда)
	Если ВозвращатьХранилищеТаблицыЗначений Тогда
		Закрыть(ДанныеТабличногоДокументаВХранилищеКонтейнераТаблицыЗначений());
	ИначеЕсли ВозвратХранилищаДляКонтейнераЗначения Тогда
		Закрыть(УИ_ОбщегоНазначенияКлиентСервер.ЗначениеХранилищеКонтейнераТабличногоДокумента(ТабличныйДокумент));
	Иначе
...
Процедура Перевести(Команда)
	...
	ТекстВопроса = УИ_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура НачатьДиалогСохраненияФайла(Знач ОбработчикЗавершения)
	...
	УИ_ОбщегоНазначенияКлиент.ПоказатьДиалогВыбора(ОписаниеОповещения, ДиалогСохраненияФайла);
...
Функция ВернутьПараметрыВТекст(Знач Текст, ОбработанныеПараметры)
	...
	Возврат УИ_СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтрокуИзМассива(Текст, ОбработанныеПараметры);
...
// Данные табличного документа в хранилище контейнера таблицы значений.
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТаблицыЗначений 
&НаСервере
Функция ДанныеТабличногоДокументаВХранилищеКонтейнераТаблицыЗначений()
	ТаблицаЗначений = ДанныеТабличногоДокументаВВидеТаблицыЗначений();
	
	Возврат УИ_ОбщегоНазначения.ЗначениеХранилищаКонтейнераТаблицыЗначений(ТаблицаЗначений);
КонецФункции

// Данные табличного документа в виде таблицы значений.
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Данные табличного документа в виде таблицы значений
&НаСервере
Функция ДанныеТабличногоДокументаВВидеТаблицыЗначений()

	Попытка
		Построитель = Новый ПостроительЗапроса;
		Построитель.ИсточникДанных = Новый ОписаниеИсточникаДанных(ТабличныйДокумент.Область());

		ТабличныеДанные = Построитель.Результат.Выгрузить();
