...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы <> Истина И Модифицированность Тогда
		Отказ = Истина;
		ПодтвердитьЗакрытие(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект));
	КонецЕсли;
...
Процедура ДеревоОтчетовПриАктивизацииСтроки(Элемент)
	...
	Если Не ИдетАктивацияСтроки И ТекущаяСтрока <> Элемент.ТекущаяСтрока Тогда
		...
		Исключение
			...
			УИ_ОбщегоНазначенияКлиент.СообщитьПользователю(ОписаниеОшибки);
...
Процедура РезультатТабличныйДокументОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ОбработкаРасшифровки.ПоказатьВыборДействия(
		Новый ОписаниеОповещения("РезультатТабличныйДокументОбработкаРасшифровкиЗавершение", ЭтотОбъект,
...
Процедура ВнешниеНаборыДанныхПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	СтрокиИмени = ТекСтрокаДерева.ВнешниеНаборыДанных.НайтиСтроки(Новый Структура("Имя", ТекДанные.Имя));
	Если СтрокиИмени.Количество() > 1 Тогда
		ПоказатьПредупреждение( , "Набор данных с таким именем уже есть! Введите другое имя.", , Заголовок);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
КонецПроцедуры
...
#КонецОбласти
#Область ОбработчикиСобытийЭлементовТаблицыФормыДеревоОтчетовВременныеТаблицы
&НаКлиенте
Процедура ДеревоОтчетовВременныеТаблицыПредставлениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка=Ложь;
	ТекДанные=Элементы.ДеревоОтчетовВременныеТаблицы.ТекущиеДанные;
	Если ТекДанные=Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	УИ_ОбщегоНазначенияКлиент.РедактироватьТаблицуЗначений(ТекДанные.Значение,
														   ЭтотОбъект,
														   Новый ОписаниеОповещения("ДеревоОтчетовВременныеТаблицыПредставлениеНачалоВыбораЗавершение",
		ЭтотОбъект, Новый Структура("ТекСтрока", Элементы.ДеревоОтчетовВременныеТаблицы.ТекущаяСтрока)));
КонецПроцедуры

&НаКлиенте
Процедура ДеревоОтчетовВременныеТаблицыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	Если ОтменаРедактирования Тогда
		Возврат;
	КонецЕсли;	
	...
	СтрокиИмени = ТекСтрокаДерева.ВременныеТаблицы.НайтиСтроки(Новый Структура("Имя", ТекДанные.Имя));
	Если СтрокиИмени.Количество() > 1 Тогда
		ПоказатьПредупреждение( , "Временная таблица с таким именем уже есть! Введите другое имя.", , Заголовок);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОткрытьФайлОтчетов(Команда)
	...
	ПодтвердитьЗакрытие(Новый ОписаниеОповещения("ОткрытьФайлОтчетовЗавершение", ЭтотОбъект));
...
Процедура НовыйФайлОтчетов(Команда)
	...
	ПодтвердитьЗакрытие(Новый ОписаниеОповещения("НовыйФайлОтчетовЗавершение", ЭтотОбъект));
...
Процедура ЗагрузитьСхемуИзФайла(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ЗагрузитьСхемуИзФайлаПослеПодключенияРасширения", ЭтотОбъект,
...
Процедура ЗагрузитьНастройкиИзВариантаОтчетаНаСервере(ВариантДляЗагрузки) 
	...
	ТекСтрокаДерева.НастройкиСКД = УИ_ОбщегоНазначения.НастройкиКомпоновкиДанныхВСтроку(ВариантНастроекСКД.Настройки);
	
	ЗагрузитьНастройкиВКомпоновщикНастроек(ВариантНастроекСКД.Настройки);
...
Процедура СохранитьСхемуВФайлПослеПодключенияРасширения(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДополнительныеПараметрыОповещения = Новый Структура("ВыборФайла", ВыборФайла);
		Оповещение = Новый ОписаниеОповещения("СохранитьСхемуВФайлПослеВыбораФайла", ЭтотОбъект,
...
Процедура СохранитьСхемуВФайлПослеВыбораФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	НачатьПолучениеФайлов(Новый ОписаниеОповещения("СохранитьСхемуВФайлЗавершение", ЭтотОбъект), ПолучаемыеФайлы, "",
		Ложь);
...
Процедура ЗагрузитьСхемуИзФайлаПослеПодключенияРасширения(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		НачатьПомещениеФайлов(Новый ОписаниеОповещения("ЗагрузитьСхемуИзФайлаПослеПомещенияФайлов", ЭтотОбъект), ,
			ВыборФайла);

	Иначе

		ДополнительныеПараметрыОповещения = Новый Структура("Адрес", Адрес);
		Оповещение = Новый ОписаниеОповещения("ЗагрузитьСхемуИзФайлаПослеПомещенияФайла", ЭтотОбъект,
			ДополнительныеПараметрыОповещения);
		НачатьПомещениеФайла(Оповещение, Адрес, , Истина);
...
Процедура ЗагрузитьФайлКонсоли(ЗагружаемоеИмяФайла)
	...
	НачатьПодключениеРасширенияРаботыСФайлами(
		Новый ОписаниеОповещения("ЗагрузитьФайлКонсолиПослеПодключенияРасширения", ЭтотОбъект,
...
Процедура ЗагрузитьФайлКонсолиПослеПодключенияРасширения(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		Иначе
			...
			НачатьПомещениеФайлов(
				Новый ОписаниеОповещения("ЗагрузитьФайлКонсолиПослеПодключенияРасширенияПослеПомещенияФайлов",
				ЭтотОбъект), ПомещаемыеФайлы, , Ложь);
	...
	Иначе
		...
		НачатьПомещениеФайла(
			Новый ОписаниеОповещения("ЗагрузитьФайлКонсолиПослеПодключенияРасширенияПослеПомещенияФайла", ЭтотОбъект), ,
...
Процедура ЗагрузитьФайлКонсолиНаСервере(Адрес)
	...
	ПараметрыЧтения = НовыйПараметрыЧтенияФайлаВДеревоОтчетов();
	ПараметрыЧтения.ЕстьВнешниеНаборыДанных = ДеревоФайла.Колонки.Найти("ВнешниеНаборыДанных") <> Неопределено;
	ПараметрыЧтения.ЕстьВременныеТаблицы = ДеревоФайла.Колонки.Найти("ВременныеТаблицы") <> Неопределено;
	Если ПараметрыЧтения.ЕстьВнешниеНаборыДанных Тогда
		ПараметрыЧтения.ПропускаемыеРеквизитыВЗаполнитьЗначения = "ВнешниеНаборыДанных";
	КонецЕсли;
	Если ПараметрыЧтения.ЕстьВременныеТаблицы Тогда
		ПараметрыЧтения.ПропускаемыеРеквизитыВЗаполнитьЗначения = ПараметрыЧтения.ПропускаемыеРеквизитыВЗаполнитьЗначения
																  + ",ВременныеТаблицы";
	КонецЕсли;
	
	ПрочитатьСтрокуДереваФайлаВДеревоОтчетов(ДеревоФайла, ДеревоОтчетов, ПараметрыЧтения);
...
Функция НовыйПараметрыЧтенияФайлаВДеревоОтчетов()
	Данные = Новый Структура;
	Данные.Вставить("ЕстьВнешниеНаборыДанных", Ложь);
	Данные.Вставить("ЕстьВременныеТаблицы", Ложь);
	Данные.Вставить("ПропускаемыеРеквизитыВЗаполнитьЗначения", "");
...
//  ПараметрыЧтения - см. НовыйПараметрыЧтенияФайлаВДеревоОтчетов
&НаСервере
Процедура ПрочитатьСтрокуДереваФайлаВДеревоОтчетов(СтрокаДереваФайла, СтрокаДереваОтчетов, ПараметрыЧтения)
	...
	Для Каждого ТекСтр Из СтрокаДереваФайла.Строки Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтр, , ПараметрыЧтения.ПропускаемыеРеквизитыВЗаполнитьЗначения);
		...
		КонецЕсли;
		Если ПараметрыЧтения.ЕстьВременныеТаблицы Тогда
			Для Каждого ТекСтрокаВнешнихДанных Из ТекСтр.ВременныеТаблицы Цикл
				НоваяСтрокаВременныхТаблиц = НоваяСтрока.ВременныеТаблицы.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаВременныхТаблиц, ТекСтрокаВнешнихДанных);
			КонецЦикла;
		КонецЕсли;

		ПрочитатьСтрокуДереваФайлаВДеревоОтчетов(ТекСтр, НоваяСтрока, ПараметрыЧтения);
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗагрузитьСхемуКомпоновкиДанныхВКомпоновщикНастроек(ЭлементДерева, ЗагружатьНастройкиПоУмолчанию)
	...
	Если ЗагружатьНастройкиПоУмолчанию И ЗначениеЗаполнено(ЭлементДерева.НастройкиСКД) Тогда
		Настройки = УИ_ОбщегоНазначения.НастройкиКомпоновкиДанныхИзСтроки(ЭлементДерева.НастройкиСКД);

		ЗагрузитьНастройкиВКомпоновщикНастроек(Настройки);
	КонецЕсли;
...
// 
// Параметры:
//  Настройки - НастройкиКомпоновкиДанных - Настроки
&НаСервере
Процедура ЗагрузитьНастройкиВКомпоновщикНастроек(Настройки)
...
Процедура ЗагрузитьНастройкиВаниантаВТекущуюСтроку(ЭлементДерева)
	...
	Если ЭлементДерева.Данные <> "" Тогда
		...
		Настройки = УИ_ОбщегоНазначения.НастройкиКомпоновкиДанныхИзСтроки(ЭлементДерева.Данные);
...
Функция СохранитьДанныеТекущейСтрокиНаСервере()
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		...
		// Вариант отчета.
			НовыеДанные = УИ_ОбщегоНазначения.НастройкиКомпоновкиДанныхВСтроку(Отчет.КомпоновщикНастроек.Настройки);
...
Процедура ВывестиМакетКомпоновкиДанныхВТабличныйДокумент(МакетКомпоновкиДанных, ДанныеРасшифровкиОбъект)
	...
	Если УИ_ОбщегоНазначенияКлиентСервер.ВерсияПлатформыНеМладше("8.3.17") Тогда

		//@skip-check bsl-legacy-check-dynamic-feature-access
		ПроцессорКомпоновкиДанных.Инициализировать(МакетКомпоновкиДанных,
												   СтруктураВнешнихНаборовДанных(),
												   ДанныеРасшифровкиОбъект,
												   Истина,
												   ,
												   МенеджерВременныхТаблицОтчета());
	Иначе
		ПроцессорКомпоновкиДанных.Инициализировать(МакетКомпоновкиДанных,
												   СтруктураВнешнихНаборовДанных(),
												   ДанныеРасшифровкиОбъект,
...
Функция ПолучитьТекстРезультатаКомпоновкиДанных(МакетКомпоновкиДанных, ДанныеРасшифровкиОбъект)
	...
	Если ДанныеРасшифровкиОбъект <> Неопределено Тогда
		...
		Стр = УИ_ОбщегоНазначения.ДанныеРасшифровкиКомпоновкиДанныхВСтроку(ДанныеРасшифровкиОбъект);
...
Процедура ВывестиМакетКомпоновкиДанныхВТекст(МакетКомпоновкиДанных)
	...
	ТекстМакетаКомпоновкиДанных = УИ_ОбщегоНазначения.МакетКомпоновкиДанныхВСтроку(МакетКомпоновкиДанных);
...
Процедура ВывестиМакетКомпоновкиДанныхВТекстДляКоллекции(МакетКомпоновкиДанных)
	...
	ТекстМакетаКомпоновкиДанныхДляКоллекции = УИ_ОбщегоНазначения.МакетКомпоновкиДанныхВСтроку(МакетКомпоновкиДанных);
...
Функция СформироватьНаСервереВИсполняемыеНастройкиКомпоновкиДанныхXML()
	...
	Если Элементы.ДеревоОтчетов.ТекущаяСтрока <> Неопределено Тогда
		...
		// Отчет.
			СхемаКомпоновкиДанных = ПолучитьСхемуКомпоновкиДанныхСервер();
			ИсполняемыеНастройкиXML = УИ_ОбщегоНазначения.НастройкиКомпоновкиДанныхВСтроку(СхемаКомпоновкиДанных.НастройкиПоУмолчанию);
			ОтобразитьПанельРезультатов();

		ИначеЕсли Элемент.ТипСтроки = 1 Тогда
		// Вариант отчета.

			ИсполняемыеНастройкиXML = УИ_ОбщегоНазначения.НастройкиКомпоновкиДанныхВСтроку(Отчет.КомпоновщикНастроек.Настройки);
			ОтобразитьПанельРезультатов();

		ИначеЕсли Элемент.ТипСтроки = 2 Тогда
		// Настройки отчета.
			ИсполняемыеНастройкиXML = УИ_ОбщегоНазначения.НастройкиКомпоновкиДанныхВСтроку(Отчет.КомпоновщикНастроек.ПолучитьНастройки());
			ОтобразитьПанельРезультатов();
...
Функция ПолучитьСхемуКомпоновкиДанных(ТекстСхемы)
	...
	Если ТекстСхемы <> "" Тогда
		...
		Возврат УИ_ОбщегоНазначения.СхемаКомпоновкиДанныхИзСтроки(ТекстСхемы);
...
Процедура УстановитьСхемуКомпоновкиДанныхКлиент(Схема, ИдентификаторСтроки = Неопределено)
	...
	Если Отчет.КомпоновщикНастроек.Настройки.Структура.Количество() > 0 Тогда
		...
		ДополнительныеПараметры = Новый Структура("Схема,ИдентификаторСтроки", Схема, ИдентификаторСтроки);
		Оповещение = Новый ОписаниеОповещения("УстановитьСхемуКомпоновкиДанныхКлиентЗавершение", ЭтотОбъект,
			ДополнительныеПараметры);
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Загрузить настройки по умолчанию?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура УстановитьСхемуКомпоновкиДанныхКлиентЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	УстановитьСхемуКомпоновкиДанных(ДополнительныеПараметры.Схема,
...
Процедура УстановитьСхемуКомпоновкиДанных(
	...
	ИначеЕсли ТипЗнч(Схема) = Тип("Строка") Тогда
		СхемаДляУстановки = УИ_ОбщегоНазначения.СхемаКомпоновкиДанныхИзСтроки(Схема);
		Если СхемаДляУстановки = Неопределено Тогда
			ТекстОшибки = НСтр("ru = 'Не удалось получить схему компоновки данных.'");
	...
	ТекСтрокаДерева.Данные = УИ_ОбщегоНазначения.СхемаКомпоновкиДанныхВСтроку(СхемаДляУстановки);
	
	Если Не ЗначениеЗаполнено(ТекСтрокаДерева.НастройкиСКД) Или ЗагружатьНастройкиПоУмолчанию Тогда
		ТекСтрокаДерева.НастройкиСКД = УИ_ОбщегоНазначения.НастройкиКомпоновкиДанныхВСтроку(СхемаДляУстановки.НастройкиПоУмолчанию);
	КонецЕсли;
...
#Если ТолстыйКлиентОбычноеПриложение Или ТолстыйКлиентУправляемоеПриложение Тогда
	СхемаКомпоновкиДанных = ПолучитьСхемуКомпоновкиДанныхКлиент();
...
#Иначе
	...
	Если ТекДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	СхемаКомпоновкиДанных = ТекДанные.Данные;
	ДополнительныеПараметры = Новый Структура("ИдентификаторСтроки", ТекДанные.ПолучитьИдентификатор());
	Оповещение = Новый ОписаниеОповещения("ОткрытьКонструкторСхемыКомпоновкиДанныхЗавершение", ЭтотОбъект,
		ДополнительныеПараметры);
#КонецЕсли	

	УИ_ОбщегоНазначенияКлиент.ОткрытьКонструкторСхемыКомпоновкиДанных(СхемаКомпоновкиДанных, ЭтотОбъект, Оповещение);
...
Процедура Сохранить(Как, ОбработкаОповещения)
	...
	ДополнительныеПараметры = Новый Структура("Как, ОбработкаОповещения", Как, ОбработкаОповещения);
	Оповещение = Новый ОписаниеОповещения("СохранитьЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	НачатьПодключениеРасширенияРаботыСФайлами(Оповещение);
...
Процедура СохранитьЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДополнительныеПараметрыОповещения = Новый Структура("ОбработкаОповещения", ОбработкаОповещения);
		Оповещение = Новый ОписаниеОповещения("СохранитьЗавершениеПослеПолученияФайлов", ЭтотОбъект,
			ДополнительныеПараметрыОповещения);
		НачатьПолучениеФайлов(Оповещение, ПолучаемыеФайлы, ВыборФайла, Ложь);
...
Процедура ПодтвердитьЗакрытие(ОбработкаОповещения)
	...
	Если Модифицированность Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПодтвердитьЗакрытиеЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВопрос(Оповещение,
					   НСтр("ru = 'Отчеты модифицированы. Сохранить изменения?'"),
...
// 
// Возвращаемое значение:
//  СписокЗначений из Строка - Текущий узел дерева отчетов.
&НаСервере
...
Процедура СохранитьНастройкиКонсоли()
	...
	НастройкиСохранения = Новый Структура("ИмяФайла,ТекущийУзел", ИмяФайла, ПолучитьТекущийУзел());
	ХранилищеОбщихНастроек.Сохранить("НастройкиКонсолиСистемыОтчетности5", , НастройкиСохранения);
...
Процедура ВыполнитьНаСервереИзМакетаКомпоновкиДанных()
	...
	МакетКомпоновкиДанных = УИ_ОбщегоНазначения.МакетКомпоновкиДанныхИзСтроки(ТекстМакетаКомпоновкиДанных);
...
Процедура ВыполнитьНаСервереИзМакетаКомпоновкиДанныхВКоллекцию()
	...
	МакетКомпоновкиДанных = УИ_ОбщегоНазначения.МакетКомпоновкиДанныхИзСтроки(ТекстМакетаКомпоновкиДанныхДляКоллекции);
	ВывестиМакетКомпоновкиДанныхВКоллекцию(МакетКомпоновкиДанных);
	ОтобразитьПанельРезультатов();
...
Функция ИмяФайлаЭталонаТабличногоДокумента()
	...
	Результат = НСтр("ru = 'Эталон табличного документа.mxl'");

	Если Результат = "" Тогда

		Результат = "Эталон табличного документа.xml";

	КонецЕсли;

	Возврат КаталогВременныхФайлов() + Результат;
...
Функция ИмяФайлаТабличногоДокумента()
	...
	Результат = НСтр("ru = 'Табличный документ.xml'");

	Если Результат = "" Тогда

		Результат = "Табличный документ.xml";
...
Функция ИмяФайлаЭталонаМакета()
	...
	Результат = НСтр("ru = 'Эталон макета.xml'");

	Если Результат = "" Тогда

		Результат = "Эталон макета.xml";
...
Функция ИмяФайлаЭталонаИсполняемыхНастроек()
	...
	Результат = НСтр("ru = 'Эталон исполняемых настроек.xml'");

	Если Результат = "" Тогда

		Результат = "Эталон исполняемых настроек.xml";
...
Функция ИмяФайлаИсполняемыхНастроек()
	...
	Результат = НСтр("ru = 'Исполняемые настройки.xml'");

	Если Результат = "" Тогда

		Результат = "Исполняемые настройки.xml";
...
Функция ИмяФайлаЭталонаРезультатаXML()
	...
	Результат = НСтр("ru = 'Эталон результата.xml'");

	Если Результат = "" Тогда

		Результат = "Эталон результата.xml";
...
Функция ИмяФайлаРезультатаXML()
	...
	Результат = НСтр("ru = 'Результат.xml'");
...
Функция ИмяФайлаЭталонаМакетаДляКоллекции()
	...
	Результат = НСтр("ru = 'Эталон макета для коллекции.xml'");

	Если Результат = "" Тогда

		Результат = "Эталон макета для коллекции.xml";
...
Функция ИмяФайлаМакетаДляКоллекции()
	...
	Результат = НСтр("ru = 'Макет для коллекции.xml'");

	Если Результат = "" Тогда

		Результат = "Макет для коллекции.xml";
...
Функция ИмяФайлаЭталонаРезультатаXMLДляКоллекции()
	...
	Результат = НСтр("ru = 'Эталон результата для коллекции.xml'");

	Если Результат = "" Тогда

		Результат = "Эталон результата для коллекции.xml";
...
Функция ИмяФайлаРезультатаXMLДляКоллекции()
	...
	Результат = НСтр("ru = 'Результат для коллекции.xml'");

	Если Результат = "" Тогда

		Результат = "Результат для коллекции.xml";
...
Процедура ЗаполнитьСКДДляОтладки(АдресДанныхОтладки)
	ДанныеДляОтладки = ПолучитьИзВременногоХранилища(АдресДанныхОтладки);//см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСериализованнаяДляОтладкиСхемаКомпоновкиДанных
	...
	ШаблонПредставления = "Строк: %1 Колонок: %2";
	Если ДанныеДляОтладки.Свойство("ВнешниеНаборыДанных") Тогда
		Для Каждого КлючЗначение ИЗ ДанныеДляОтладки.ВнешниеНаборыДанных Цикл
			...
			НС.Представление = СтрШаблон(ШаблонПредставления, ТЗ.Количество(), ТЗ.Колонки.Количество());
		КонецЦикла;
	КонецЕсли;

	Если ДанныеДляОтладки.Свойство("ВременныеТаблицы") Тогда
		Для Каждого КлючЗначение Из ДанныеДляОтладки.ВременныеТаблицы Цикл
			Если ТипЗнч(КлючЗначение.Значение) <> Тип("ТаблицаЗначений") Тогда
				Продолжить;
			КонецЕсли;

			НС = Элемент.ВременныеТаблицы.Добавить();
			НС.Имя = КлючЗначение.Ключ;
			НС.Значение = ЗначениеВСтрокуВнутр(КлючЗначение.Значение);
			НС.Представление = СтрШаблон(ШаблонПредставления,
									   КлючЗначение.Значение.Количество(),
									   КлючЗначение.Значение.Колонки.Количество());
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура ДеревоОтчетовВременныеТаблицыПредставлениеНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекСтрокаДерева = ДеревоОтчетов.НайтиПоИдентификатору(ТекущаяСтрока);
	Если ТекСтрокаДерева = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекДанныеСтроки = ТекСтрокаДерева.ВременныеТаблицы.НайтиПоИдентификатору(ДополнительныеПараметры.ТекСтрока);
	ТекДанныеСтроки.Значение=Результат.Значение;
	ТекДанныеСтроки.Представление=Результат.Представление;
КонецПроцедуры
...
&НаСервере
Функция МенеджерВременныхТаблицОтчета() 
	...
	Если ТекСтрокаДерева.ВременныеТаблицы.Количество() = 0 Тогда
		Возврат МВТ;
	КонецЕсли;

	Для Каждого СтрокаВременнаяТаблица Из ТекСтрокаДерева.ВременныеТаблицы Цикл
		Если ЗначениеЗаполнено(СтрокаВременнаяТаблица.Значение) Тогда
			Попытка
				ТЗ = ЗначениеИзСтрокиВнутр(СтрокаВременнаяТаблица.Значение);
			Исключение
				Продолжить;
			КонецПопытки;
		Иначе
		...
		УИ_ОбщегоНазначения.ДобавитьТаблицуЗначенийВМенеджерВременныхТаблиц(МВТ, СтрокаВременнаяТаблица.Имя, ТЗ);
...
Функция ПолучитьЗапросИзСКД(МакетКомпоновкиДанных)
	...
	Если ТипЗнч(МакетКомпоновкиДанных) = Тип("МакетКомпоновкиДанных") Тогда
		...
		ШаблонНабораДанных = "НАБОР ДАННЫХ ""%1"": 
							 |%2";

		РазделительНаборовДанных = "
								   |
								   |///////////////////////////////////////////////////////////
								   |
								   |";
		...
		Для Каждого ТекЗапрос Из МассивЗапросов Цикл
			ТекстЗапроса = СтрШаблон(ШаблонНабораДанных, ТекЗапрос.НаборДанных, ТекЗапрос.Запрос);
			ОписаниеЗапросов.Добавить(ТекстЗапроса);
		КонецЦикла;
		ТекстыЗапросов = СтрСоединить(ОписаниеЗапросов, РазделительНаборовДанных);
...
Функция ПодготовитьДанныеЗапросовДляКонсоли()
	...
	Если Элементы.ДеревоОтчетов.ТекущаяСтрока <> Неопределено Тогда
		...
		Если Элемент.ТипСтроки = 0 Тогда//Отчет
			...
			МассивЗапросовИзСКД = Новый Массив;//Массив Из см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСериализованныйДляОтладкиЗапрос
			...
			СтруктураТаблицМенеджераВременныхТаблиц = Новый Структура;
			Для Каждого ТекСтрокаВременныхТаблиц Из Элемент.ВременныеТаблицы Цикл
				СтруктураТаблицМенеджераВременныхТаблиц.Вставить(ТекСтрокаВременныхТаблиц.Имя,
																 ЗначениеИзСтрокиВнутр(ТекСтрокаВременныхТаблиц.Значение));
			КонецЦикла;
			
			
			Для Каждого ТекЗапрос Из МассивЗапросов Цикл
				ДанныеЗапроса = УИ_ОбщегоНазначенияКлиентСервер.НовыйСериализованныйДляОтладкиЗапрос();
				ДанныеЗапроса.Имя = ТекЗапрос.НаборДанных;
				ДанныеЗапроса.Текст = ТекЗапрос.Запрос;
				ДанныеЗапроса.Параметры = ПараметрыЗапросов;
				ДанныеЗапроса.ВременныеТаблицы = СтруктураТаблицМенеджераВременныхТаблиц;

				МассивЗапросовИзСКД.Добавить(ДанныеЗапроса);
			КонецЦикла;
