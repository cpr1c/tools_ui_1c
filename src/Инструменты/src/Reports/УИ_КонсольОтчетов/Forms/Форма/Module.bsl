...
Функция ПолучитьЗапросИзСКД(МакетКомпоновкиДанных)
	Если ТипЗнч(МакетКомпоновкиДанных) = Тип("МакетКомпоновкиДанных") Тогда
		...
		ПолучитьЗапросыИзНаборов(МакетКомпоновкиДанных.НаборыДанных, ТекстыЗапросов);
	КонецЕсли;
...
&НаСервере
Процедура ПолучитьЗапросыИзНаборов(НаборыДанных, ТекстыЗапросов)
		...
		Для Каждого НаборДанных Из НаборыДанных Цикл
			Если ТипЗнч(НаборДанных) = Тип("НаборДанныхОбъединениеМакетаКомпоновкиДанных") Тогда
				Если НаборДанных.Элементы.Количество() Тогда
					ПолучитьЗапросыИзНаборов(НаборДанных.Элементы, ТекстыЗапросов);
				КонецЕсли;
			КонецЕсли;
			
			Если ТипЗнч(НаборДанных) = Тип("НаборДанныхЗапросМакетаКомпоновкиДанных") Тогда
				ПолучитьЗапросИзНабораДанныхЗапрос(НаборДанных, ТекстыЗапросов);
			КонецЕсли;
		КонецЦикла;
...
&НаСервере
Процедура ПолучитьЗапросИзНабораДанныхЗапрос(НаборДанных, ТекстыЗапросов)
	Если Не ПустаяСтрока(НаборДанных.Запрос) Тогда
		ТекстыЗапросов = ТекстыЗапросов + СтрШаблон("НАБОР ДАННЫХ ""%1"": %2%3", НаборДанных.Имя, Символы.ПС, НаборДанных.Запрос);
