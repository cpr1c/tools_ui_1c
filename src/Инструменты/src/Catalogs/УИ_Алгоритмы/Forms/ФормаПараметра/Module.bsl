...
Процедура УправлениеВидимостьюКоллекции(П = "Массив")
	...
	КонецЕсли;
	Если П = "Массив" Тогда
		Справочники.УИ_Алгоритмы.ДобавитьКолонкуНС(ЭтотОбъект, "Значение", ОписаниеТипа, "ПараметрКоллекция");
	ИначеЕсли П = "Структура" Тогда
		...
		Справочники.УИ_Алгоритмы.ДобавитьКолонкуНС(ЭтотОбъект, "Ключ", ОТ, "ПараметрКоллекция");
		Справочники.УИ_Алгоритмы.ДобавитьКолонкуНС(ЭтотОбъект, "Значение", ОписаниеТипа, "ПараметрКоллекция");
	ИначеЕсли П = "Соответствие" Тогда
		Элементы.ОписаниеТипа.Видимость=Ложь;
		Элементы.ДобавитьКолонку.Видимость=Ложь;
		Справочники.УИ_Алгоритмы.ДобавитьКолонкуНС(ЭтотОбъект, "Ключ", ОписаниеТипа, "ПараметрКоллекция");
		Справочники.УИ_Алгоритмы.ДобавитьКолонкуНС(ЭтотОбъект, "Значение", ОписаниеТипа, "ПараметрКоллекция");
	Иначе
...
Процедура ДействияПриИзменениеПараметра()
	...
	ИначеЕсли ТипПараметра = "Коллекция" Тогда
		...
		Если Параметры.ТипПараметра = "Массив" Тогда
			Справочники.УИ_Алгоритмы.ДобавитьКолонкуНС(ЭтотОбъект, "Значение", ОписаниеТипа, "ПараметрКоллекция");
			Т = Справочники.УИ_Алгоритмы.КоллекцияВТЗ(Параметр);
		...
		Иначе
			...
			Для Каждого Колонка Из Параметр.Колонки Цикл
				Справочники.УИ_Алгоритмы.ДобавитьКолонкуНС(ЭтотОбъект, Колонка.Имя, Колонка.ТипЗначения, "ПараметрКоллекция");
			КонецЦикла;
...
Процедура ДобавитьКолонкуЗавершение(Значение, ДополнительныеПараметры) Экспорт
	...
	Если Не ПустаяСтрока(КолонкаИмя) Тогда
		ДобавитьКолонкуНСНаСервере(ЭтотОбъект, СокрЛП(КолонкаИмя), ОписаниеТипа, "ПараметрКоллекция");
		ОписаниеТипа="";
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьКолонкуНСНаСервере(пФорма, КолонкаИмя, ОписаниеТипа, ИмяТаблицы)
	Справочники.УИ_Алгоритмы.ДобавитьКолонкуНС(пФорма, КолонкаИмя, ОписаниеТипа, ИмяТаблицы);
КонецПроцедуры

&НаСервереБезКонтекста
Процедура УдалитьКолонкуНСНаСервере(пФорма, КолонкаИмя, ИмяТаблицы)
	Справочники.УИ_Алгоритмы.УдалитьКолонкуНС(пФорма, КолонкаИмя, ИмяТаблицы);
КонецПроцедуры
...
Процедура УдалитьКолонкуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		УдалитьКолонкуНСНаСервере(ЭтотОбъект, ИмяКолонки, "ПараметрКоллекция");
	КонецЕсли;
...
Процедура ИзменитьПараметр()
	...
	Если Не НовоеЗначение = Неопределено Тогда
		ВыбОбъект=РеквизитФормыВЗначение("Объект");
		ВыбОбъект.ИзменитьПараметр(НаименованиеПараметра, НовоеЗначение);
	КонецЕсли;
КонецПроцедуры
