...
Процедура ОтобразитьЗначениеПараметраСервер(ИмяПараметра, ТипПараметраСтроки)
	...
	ИначеЕсли ТипПараметра = "Коллекция" Тогда
		...
		Для каждого ИмяЭлемента Из УдаляемыеЭлементы Цикл
            Справочники.УИ_Алгоритмы.УдалитьКолонкуНС(ЭтотОбъект, ИмяЭлемента, "ПараметрКоллекция");
		КонецЦикла;
...
Процедура ДобавитьКолонкуНС(Знач КолонкаИмя, ОписаниеТипаКолонки, ТаблицаФормы)  Экспорт
