...
Процедура _ОткрытьФормуСпискаДоп(Команда)
	...
	Если СтрДЗ <> Неопределено и не ПустаяСтрока(_ИмяФормыСписка) Тогда
		...
		УИ_ОбщегоНазначенияКлиент.ОткрытьДинамическийСписок(СтрукПараметры.ПолноеИмя);
		Возврат;
...
Процедура _ОткрытьОбъект(Команда)
	...
	// Замена выполнения обработки методом УИ
	УИ_ОбщегоНазначенияКлиент.РедактироватьОбъект(пСтрук.мОбъектСсылка, пСтрук.АдресаХранилищ);
	Возврат;
