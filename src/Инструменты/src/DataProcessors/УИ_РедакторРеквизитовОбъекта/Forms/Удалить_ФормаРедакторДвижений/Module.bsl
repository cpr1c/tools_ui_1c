...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	ПоказатьВводЗначения(Новый ОписаниеОповещения("вОбработатьВыборДокументаДляЗагрузкиДвижений", ЭтотОбъект),
		ВыбранноеЗначение, НСтр("ru = 'Документ для загрузки движений'"));
КонецПроцедуры
...
Процедура вПоказатьВопрос(ИмяПроцедуры, ТекстВопроса, ДопПараметры = Неопределено)
	ПоказатьВопрос(Новый ОписаниеОповещения(ИмяПроцедуры, ЭтотОбъект, ДопПараметры), ТекстВопроса,
...
Процедура вСоздатьРеквизитыНаборовЗаписей(НадоСоздаватьРеквизиты = Истина)
	...
	Для Каждого Элем Из СоотвДанные Цикл
		...
		Элем = Элементы.Добавить("_" + ИмяРеквизита + "_ПереключитьАктивностьЗаписей", Тип("КнопкаФормы"),
			ЭлемТЗ.КоманднаяПанель);
		Элем.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
		Элем.ИмяКоманды = "_ПереключитьАктивностьЗаписей";

		Элем = Элементы.Добавить("_" + ИмяРеквизита + "_ОткрытьОбъект", Тип("КнопкаФормы"),
			ЭлемТЗ.КоманднаяПанель);
		Элем.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
		Элем.ИмяКоманды = "_ОткрытьОбъект";

		Элем = Элементы.Добавить("_" + ИмяРеквизита + "_ОбновитьНабор", Тип("КнопкаФормы"),
			ЭлемТЗ.КоманднаяПанель);
		Элем.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
		Элем.ИмяКоманды = "_ОбновитьНабор";

		Элем = Элементы.Добавить("_" + ИмяРеквизита + "_ЗаписатьНабор", Тип("КнопкаФормы"),
			ЭлемТЗ.КоманднаяПанель);
		Элем.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
...
Функция вЗагрузитьДвиженияИзДокументаНаСервере(ДокСсылка)
	...
	Для Каждого ЭлеметМД Из ДокСсылка.Метаданные().Движения Цикл
		Если ДвиженияПриемник.Содержит(ЭлеметМД) Тогда
			...
			Если ТабДанные.Количество() <> 0 Тогда
				СтрукОсновное = Новый Структура("Период, Регистратор",
					вПолучитьПериодРегистрации(мОбъектСсылка, _ПолноеИмяДокумента), мОбъектСсылка);
