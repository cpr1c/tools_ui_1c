...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	вДобавитьКомандуОчисткиТаблицы(Элементы._НаборЗаписей, "_НаборЗаписей");
КонецПроцедуры
...
&НаСервере
Процедура вДобавитьКомандуОчисткиТаблицы(Таблица, ПутьКДанным)
	...
	ИмяГруппыКнопок = "ГруппаОчистка_" + ПутьКДанным;
	ГруппаКнопок = Элементы.Найти(ИмяГруппыКнопок);
	Если ГруппаКнопок = Неопределено Тогда
		ГруппаКнопок = Элементы.Добавить(ИмяГруппыКнопок, Тип("ГруппаФормы"), Таблица.КонтекстноеМеню);
		ГруппаКнопок.Вид = ВидГруппыФормы.ГруппаКнопок;
	КонецЕсли;

	Кнопка = Элементы.Добавить("ОчиститьТаблицу_" + ПутьКДанным, Тип("КнопкаФормы"), ГруппаКнопок);
...
Процедура Подключаемый_ОчиститьТаблицу(Команда)
	...
	вОчиститьТаблицуНаСервере(ПутьКДанным);
КонецПроцедуры

&НаСервере
Процедура вОчиститьТаблицуНаСервере(ПутьКДанным)
	...
	Если НадоСоздаватьРеквизиты Тогда
		...
		Если МассивКСозданию.Количество() <> 0 Тогда
			...
			Для Каждого ЭлемТЧ Из ОбъектМД.ТабличныеЧасти Цикл
				...
				вДобавитьКомандуОчисткиТаблицы(НоваяТаблица, ИмяТаб);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;	
