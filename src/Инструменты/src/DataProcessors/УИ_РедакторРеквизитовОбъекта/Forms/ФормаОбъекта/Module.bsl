...
Процедура РеквизитыОбъектаЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Значение = ТекДанные["Значение"];	
	Если Не УИ_ОбщегоНазначенияВызовСервера.ЭтоПеречислениеПоТипу(ТипЗнч(Значение)) Тогда
		Возврат;
	КонецЕсли;

	СтандартнаяОбработка = Ложь;
	УИ_ОбщегоНазначенияКлиент.НачатьВыборЗначенияПеречисления(Значение, , Элементы.РеквизитыОбъектаЗначение);
КонецПроцедуры
