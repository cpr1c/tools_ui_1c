...
Процедура мОбъектСсылкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	УИ_ОбщегоНазначенияКлиент.ПолеФормыНачалоВыбораЗначения(ЭтотОбъект, Элемент, мОбъектСсылка, СтандартнаяОбработка,
		Новый ОписаниеОповещения("мОбъектСсылкаНачалоВыбораЗавершениеВыбораПустогоЗначения", ЭтотОбъект), "Ссылки");
КонецПроцедуры

&НаКлиенте
Процедура мОбъектСсылкаНачалоВыбораЗавершениеВыбораПустогоЗначения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НадоСоздаватьРеквизиты Тогда
		...
		Если МассивКСозданию.Количество() <> 0 Тогда
			...
			Для Каждого ЭлемТЧ Из ОбъектМД.ТабличныеЧасти Цикл
				...
				НоваяТаблица.УстановитьДействие("ПриОкончанииРедактирования", "ТабличнаяЧастьПриОкончанииРедактирования");
				НоваяТаблица.УстановитьДействие("ПослеУдаления", "ТабличнаяЧастьПослеУдаления");
...
&НаКлиенте
Процедура вПоказатьЗначенияПеречисления(Значение)
	ПараметрыФормы = Новый Структура("ЗначениеПеречисления", Значение);
	ОткрытьФорму("ОбщаяФорма.УИ_ФормаПеречисления", ПараметрыФормы, Элементы.РеквизитыОбъектаЗначение);
КонецПроцедуры  
...
Процедура РеквизитыОбъектаЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекДанные <> Неопределено Тогда
		Значение = ТекДанные["Значение"];
		Если ЭтоПеречисление(ТипЗнч(Значение)) Тогда
			СтандартнаяОбработка = Ложь;
			вПоказатьЗначенияПеречисления(Значение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры