...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УИ_ОбщегоНазначения.УстановитьУсловноеОформлениеТаблицыЗначенийПросмотраЗначений(ЭтотОбъект,
																					 Элементы.ЗначенияПеременныхПослеВыполнения,
																					 "ЗначенияПеременныхПослеВыполнения");
...
Процедура АлгоритмыПриАктивизацииСтроки(Элемент)
	...
	// Проверка на активацию новой строки
	Если ТекДанные.Имя = "" Тогда
		Возврат;
	КонецЕсли;

	СобытияПриАктивацииСтрокиАлгоритма(ТекДанные);
КонецПроцедуры
...
Процедура АлгоритмыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		ИдентификаторАлгоритма = Формат(ТекДанные.ПолучитьИдентификатор(), "ЧГ=0;");
		ТекДанные.Имя = СтрШаблон("Алгоритм%1", ИдентификаторАлгоритма);
		СобытияПриАктивацииСтрокиАлгоритма(ТекДанные);
	КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыЗначенияПеременныхПослеВыполнения

&НаКлиенте
Процедура ЗначенияПеременныхПослеВыполненияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	СтрокаПеременной = ЗначенияПеременныхПослеВыполнения.НайтиПоИдентификатору(ВыбраннаяСтрока);
	Если СтрокаПеременной = Неопределено Тогда
		Возврат;
	КонецЕсли;

	Если Поле.Имя = "ЗначенияПеременныхПослеВыполненияЗначениеЕстьОшибка" И СтрокаПеременной.ЗначениеЕстьОшибка Тогда
		ЗаголовокФормы = "Ошибка при обработке свойства " + СтрокаПеременной.Имя;
...
Процедура ЗакрытьКонсоль(Команда)
	Оповещение = Новый ОписаниеОповещения("ЗакрытьКонсольЗавершение", ЭтотОбъект);
	ПоказатьВопрос(Оповещение, "Выйти из консоли кода?", РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура ВыполнитьКод(Команда)
	...
	Если СохранятьПередВыполнением И ЗначениеЗаполнено(ИмяФайлаАлгоритма) Тогда
		Оповещение = Новый ОписаниеОповещения("ВыполнитьКодЗавершениеСохраненияФайлаАлгоритмов", ЭтотОбъект,
			ДопИнформация);
		СохранитьФайлНаДиск( , Оповещение);
	Иначе
...
Процедура ПросмотретьЗначениеПеременной(Команда)
	СтрокаПеременной = Элементы.ЗначенияПеременныхПослеВыполнения.ТекущиеДанные;
	Если СтрокаПеременной = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ОткрытьПросмотрЗначенияСтрокиПеременнойНаКлиенте(СтрокаПеременной);
...
&НаКлиенте
Процедура СобытияПриАктивацииСтрокиАлгоритма(Знач ТекДанные)
	УИ_ИдентификаторТекущейСтрокиАлгоритмов = ТекДанные.ПолучитьИдентификатор();

	ИдентификаторРедактора = "Код";
	УИ_РедакторКодаКлиент.УстановитьТекстРедактора(ЭтотОбъект, ИдентификаторРедактора, ТекДанные.Текст);
	УИ_РедакторКодаКлиент.УстановитьОригинальныйТекстРедактора(ЭтотОбъект, ИдентификаторРедактора, ТекДанные.ОригинальныйТекст);
...
Процедура ОткрытьПросмотрЗначенияСтрокиПеременнойНаКлиенте(СтрокаПеременной)
	...
	ВариантФормы = УИ_ОбщегоНазначенияКлиент.ВариантыФормыПросмотраЗначения().Значение;
...
Процедура ВыполнитьКодЗавершениеСохраненияФайлаАлгоритмов(Результат, ДополнительныеПараметры) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ВыполнитьКодЗавершениеСборкиОбработок", ЭтотОбъект, ПараметрыОповещения);
...
Процедура ВыполнитьКодАлгоритма(Форма, Алгоритмы, ИдентификаторСтрокиАлгоритмов)
	...
	Если ПараметрыВыполненияАлгоритма.ВозвращатьЗначенияПеременных Тогда
		...
		Форма.ЗначенияПеременныхПослеВыполнения.Очистить();

		Для Каждого КлючЗначение Из РезультатВыполнения.ЗначенияПеременных Цикл
			НС = Форма.ЗначенияПеременныхПослеВыполнения.Добавить();
...
Процедура СохранитьФайлНаДиск(СохранитьКак = Ложь, ОписаниеОповещенияОЗаверешении = Неопределено)
	...
	Оповещение = Новый ОписаниеОповещения("СохранитьФайлЗавершение", ЭтотОбъект, ДопПараметрыОповещения);
	УИ_ОбщегоНазначенияКлиент.СохранитьДанныеКонсолиВФайл("КонсольКода",
														  СохранитьКак,
														  СтруктураОписанияСохраняемогоФайла(),
														  ПолучитьСтрокуСохранения(),
														  Оповещение);
КонецПроцедуры
...
Функция КонтекстВыполненияАлгоритма(Переменные)
	...
	Для Каждого СтрокаТЧ Из Переменные Цикл
		...
		ЗначениеПоля = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеПоляСКонтейнеромЗначения(СтрокаТЧ, СтруктураХраненияПоля);
		КонтекстВыполнения.Вставить(СтрокаТЧ.Имя, ЗначениеПоля);
	КонецЦикла;
