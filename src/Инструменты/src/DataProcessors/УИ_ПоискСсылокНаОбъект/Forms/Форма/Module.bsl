...
&НаКлиенте
Процедура ИсходныйОбъектНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ПоказыватьПеречисления", 	Истина);
	ПараметрыФормы.Вставить("ПоказатьВыборФормыСписка", Истина);
	Оповещение = Новый ОписаниеОповещения("ИсходныйОбъектНачалоВыбора_Продолжение", ЭтотОбъект);
	ОткрытьФорму("ОбщаяФорма.УИ_ФормаВыбораТипаОбъекта", ПараметрыФормы, ЭтотОбъект, , , , Оповещение);
КонецПроцедуры
...
&НаКлиенте
Процедура ИсходныйОбъектНачалоВыбора_Продолжение(Результат, ДопПараметры) Экспорт
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Значение = Результат.Значение;
	ВидФормыСписка = Результат.ВидФормыСписка;
	МетаданныеОбъекта = ОписаниеМетаданных(Значение);
	ИмяОбъектаМетаданных = МетаданныеОбъекта.ВидОбъекта + "." + МетаданныеОбъекта.Имя;

	ПараметрыФормыВыбора = Новый Структура;
	ПараметрыФормыВыбора.Вставить("РежимВыбора", 			Истина);
	ПараметрыФормыВыбора.Вставить("МножественныйВыбор", 	Ложь);
	ПараметрыФормыВыбора.Вставить("ИмяОбъектаМетаданных",	ИмяОбъектаМетаданных);

	Оповещение = Новый ОписаниеОповещения("ИсходныйОбъектНачалоВыбора_Завершение", ЭтотОбъект);
	Если ВидФормыСписка = 0 Тогда
		ИмяФормыВыбора = ИмяОбъектаМетаданных + ".ФормаВыбора";
		ОткрытьФорму(ИмяФормыВыбора, ПараметрыФормыВыбора, ЭтотОбъект, , , , Оповещение);
	ИначеЕсли ВидФормыСписка = 1 Тогда			
		ОткрытьФорму("Обработка.УИ_ДинамическийСписок.Форма.Форма", ПараметрыФормыВыбора, ЭтотОбъект, , , , Оповещение); 
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ИсходныйОбъектНачалоВыбора_Завершение(Результат, ДопПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Объект.ИсходныйОбъект = Результат;
	ИсходныйОбъектПриИзменении(Неопределено);
		
КонецПроцедуры

&НаСервереБезКонтекста
Функция ОписаниеМетаданных(Значение)

	МетаданныеОбъекта = Новый Структура;
	МетаданныеОбъекта.Вставить("Имя", 		 Значение.Метаданные().Имя);
	МетаданныеОбъекта.Вставить("ВидОбъекта", УИ_ОбщегоНазначения.ВидОбъектаПоТипу(ТипЗнч(Значение)));
