...
Функция СтруктураХраненияБазыДанных() Экспорт
	...
	Для Каждого ТекСтрока Из ТаблицаХранения Цикл
		...
		ИначеЕсли ВРег(ТекСтрока.Назначение) = "ОСНОВНАЯ"
		      ИЛИ ВРег(ТекСтрока.Назначение) = "MAIN"Тогда
			ОсновнойОбъект = ТекСтрока.ИмяТаблицы;
			ИмяТаблицы = ТекСтрока.ИмяТаблицы;
		ИначеЕсли ВРег(ТекСтрока.Назначение) = "РЕГИСТРАЦИЯИЗМЕНЕНИЙ"
		      ИЛИ ВРег(ТекСтрока.Назначение) = "CHANGERECORD"Тогда
			ОсновнойОбъект = ТекСтрока.Метаданные;
			ИмяТаблицы = ТекСтрока.ИмяТаблицы;
		ИначеЕсли ВРег(ТекСтрока.Назначение) = "РЕГИСТРАЦИЯИЗМЕНЕНИЙКОНСТАНТЫ"
		      ИЛИ ВРег(ТекСтрока.Назначение) = "CONSTANTSCHANGERECORD"Тогда
			ОсновнойОбъект = ТекСтрока.Метаданные;
			ИмяТаблицы = СтрШаблонВнутр("%1.Изменения", ТекСтрока.Метаданные);
		ИначеЕсли ВРег(ТекСтрока.Назначение) = "ИТОГИ"
		      ИЛИ ВРег(ТекСтрока.Назначение) = "TOTALS"
		      ИЛИ ВРег(ТекСтрока.Назначение) = "ОБОРОТЫ"
		      ИЛИ ВРег(ТекСтрока.Назначение) = "TURNOVERS"Тогда
			ОсновнойОбъект = ТекСтрока.Метаданные;
...
Функция ЭтоЗначениеСсылочногоТипа(Значение) Экспорт
	...
	СсылочныеКоллекции.Вставить("БизнесПроцесс"         , Истина);
...
Функция ПолучитьИмяПредопределенныхДанных(Значение) Экспорт
	...
	Совместимость8_3_6 = Совместимость8_3_6();

	ИмяКоллекцииМетаданных = Неопределено;
	
	Если Метаданные.Справочники.Содержит(МетаданныеЗначения) Тогда
		ИмяКоллекцииМетаданных = "Справочники";
	ИначеЕсли Метаданные.ПланыВидовХарактеристик.Содержит(МетаданныеЗначения) Тогда
		ИмяКоллекцииМетаданных = "ПланыВидовХарактеристик";
	ИначеЕсли Метаданные.ПланыСчетов.Содержит(МетаданныеЗначения) Тогда
		ИмяКоллекцииМетаданных = "ПланыСчетов";
	ИначеЕсли Метаданные.ПланыВидовРасчета.Содержит(МетаданныеЗначения) Тогда
		ИмяКоллекцииМетаданных = "ПланыВидовРасчета";
	ИначеЕсли Метаданные.Перечисления.Содержит(МетаданныеЗначения) Тогда
		ИндексЗначения = Перечисления[МетаданныеЗначения.Имя].Индекс(Значение);
		Возврат МетаданныеЗначения.ЗначенияПеречисления[ИндексЗначения].Имя;
	...
	Иначе
		ИмяПредопределенныхДанных = Вычислить(СтрШаблонВнутр("%1[МетаданныеЗначения.Имя].ПолучитьИмяПредопределенного(Значение)", ИмяКоллекцииМетаданных));
	КонецЕсли;
...
//  СписокСвойств       - Строка, Массив из Строка  - Список допустимых свойств (если пустой, то допустимы все свойства)
//  ИсключаяСвойства    - Строка, Массив из Строка  - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
// Возвращаемое значение:
...
Функция СтруктураТипЗнч(СтруктураЗначения)
	...
	ИначеЕсли ТипЗнч(СтруктураЗначения) = Тип("Структура") Тогда
		...
		ИначеЕсли СтруктураЗначения.Свойство("Типы")
...
//                                                       будет возвращена битая ссылка;
//                                                       Ложь - будет возвращена пустая ссылка
//  СписокСвойств                  - Строка,           - Список допустимых свойств (если пустой, то допустимы все свойства)
//                                   Массив(Строка)
//  ИсключаяСвойства               - Строка, Массив(Строка)        - Список исключаемых свойств (если пустой, то допустимы все свойства)
//                                   Массив(Строка)
//
...
Функция СсылкаВСтруктуру(Значение) Экспорт
	...
	Если СтрокаНачинаетсяСВнутр(МетаданныеЗначения.ПолноеИмя(), "Перечисление.") Тогда
		ИмяПредопределенныхДанных = "";
...
Функция ТипДополненияПериодаКомпоновкиДанныхВСтроку(Значение)
	...
	Если СтрокаНачинаетсяСВнутр(СИ.ВерсияПриложения, "8.3.12") Тогда
...
Функция Совместимость8_3_6()
	...
	Если Число(ЧастиВерсии[2]) < 6 Тогда
		Возврат Ложь;
	КонецЕсли;
