...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УИ_ПриСозданииНаСервереПосле(Параметры);
...
Функция ПолучитьПомеченныеНаУдаление()
	...
	Если ЭтотОбъект["УИ_УдаляемыеОбъекты"].Количество() > 0 Тогда
		МассивПомеченные = ЭтотОбъект["УИ_УдаляемыеОбъекты"].ВыгрузитьЗначения();
		ЭтотОбъект["УИ_УдаляемыеОбъекты"].Очистить(); //Чтобы у пользователя была возможность удалить все помеченные объекты
	Иначе
...
Процедура УИ_ПриСозданииНаСервереПосле(Параметры)
	...
	РеквизитФормы = Новый РеквизитФормы(
		"УИ_УдаляемыеОбъекты",  //Имя реквизита формы
		ТипРеквизита,     //Тип
		"",               //Путь  (Пусто, "Объект", ИмяТЧ)
						  // например "Объект.Товары"	
		"Объекты на удаление");//Заголовок
	...
	//Загрузим переданный массив в ревизит формы
	ЭтаФорма["УИ_УдаляемыеОбъекты"] = Новый СписокЗначений;
	Если (Параметры.Свойство("УИ_УдаляемыеОбъекты")) Тогда
		Если ТипЗнч(Параметры.УИ_УдаляемыеОбъекты) = Тип("Массив") Тогда
			ЭтаФорма["УИ_УдаляемыеОбъекты"].ЗагрузитьЗначения(Параметры.УИ_УдаляемыеОбъекты);
...
&НаКлиенте
Процедура УИ_ПриОткрытииПосле()

	Если ЭтаФорма["УИ_УдаляемыеОбъекты"].Количество() > 0 Тогда
		ВыполнитьДалее();
	КонецЕсли;
