...
Процедура ОбъектыЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОповещения.Вставить("ИдентификаторСтроки", Элементы.Объекты.ТекущаяСтрока);

	УИ_ОбщегоНазначенияКлиент.ПолеФормыНачалоВыбораЗначения(ЭтотОбъект, Элемент, ТекДанные.Значение,
		СтандартнаяОбработка, Новый ОписаниеОповещения("ОбъектыЗначениеНачалоВыбораЗавершениеВыбораПустогоЗначения",
		ЭтотОбъект, ПараметрыОповещения), "Ссылки");
КонецПроцедуры
...
&НаКлиенте
Процедура ОбъектыЗначениеНачалоВыбораЗавершениеВыбораПустогоЗначения(Результат, ДополнительныеПараметры) Экспорт
	СтрокаОбъектов = Объекты.НайтиПоИдентификатору(ДополнительныеПараметры.ИдентификаторСтроки);
	Если СтрокаОбъектов = Неопределено Тогда
		Возврат;
	КонецЕсли;
	СтрокаОбъектов.Значение = Результат;
КонецПроцедуры
