...
Процедура ДеревоОбъектовПередРазворачиванием(Элемент, Строка, Отказ)
	...
	Если СтрокиДЗ.Количество() = 1 И ПустаяСтрока(СтрокиДЗ[0].ВидУзла) Тогда
		...
		ИначеЕсли УзелДЗ.ВидУзла = "ГруппаРазделовМД" Тогда
			ПереченьРазделов = "Подсистемы, ОбщиеМодули, ПараметрыСеанса, Роли, ОбщиеРеквизиты, ПланыОбмена,
				|ПодпискиНаСобытия, РегламентныеЗадания, ФункциональныеОпции, ПараметрыФункциональныхОпций,
				|ОпределяемыеТипы, ХранилищаНастроек, ОбщиеКоманды, ГруппыКоманд, ПрочиеКоманды, ОбщиеФормы,
				|ОбщиеМакеты, ПакетыXDTO, WebСервисы, HTTPСервисы";
...
Функция вСформироватьСтруктуруУзлаДЗ(ВидУзла = "", Имя = "", ПолноеИмя = "", Синоним = "", ЕстьДети = Ложь,
	...
	Струк.Вставить("ВидУзла", ВидУзла);
	Струк.Вставить("Имя", Имя);
	Струк.Вставить("ПолноеИмя", ПолноеИмя);
	Струк.Вставить("Синоним", Синоним);
	Струк.Вставить("ПредставлениеОбъекта", ПредставлениеОбъекта);
	Струк.Вставить("ЕстьДети", ЕстьДети);
	Струк.Вставить("ОсновнаяТаблицаSQL", "");
	Струк.Вставить("ЕстьИзмененияРасширениями", Ложь);
	Возврат Струк;
КонецФункции
...
Функция вПолучитьСоставРазделаМД(Знач ИмяРаздела)
	...
	Таблица.Колонки.Добавить("ЕстьИзмененияРасширениями", ТипБулево);
	Таблица.Колонки.Добавить("ЕстьДети", ТипБулево);
			...
			Для Каждого ОбъектХХХ Из Метаданные[Элем.Ключ] Цикл
				...
				Если вПроверитьНаличиеСвойства(ОбъектХХХ, "Команды") Тогда
					Для Каждого Элем Из ОбъектХХХ.Команды Цикл
						...
						Стр.ЕстьИзмененияРасширениями = Элем.ЕстьИзмененияРасширениямиКонфигурации();
						Стр.ВидУзла = "ОбъектМД";
					КонецЦикла;
				КонецЕсли;
			КонецЦикла;
