...
Функция ПрочитатьДанныеИПолучитьТЗ(ИдентификаторИсточникаДанных, ТекстОшибки = "", Подключение = Неопределено) Экспорт
	
	//Текущая или внешняя база 1С 8
	Если ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 0 Или ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 1 Тогда
		 ТЗ = ВыполнитьЗапрос1С8ИПолучитьТЗ(ИдентификаторИсточникаДанных, ТекстОшибки, Подключение);
	//SQL
	ИначеЕсли ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 2 Тогда		
		ТЗ = ВыполнитьЗапросSQLИПолучитьТЗ(ИдентификаторИсточникаДанных, ТекстОшибки);
	//Файл
	ИначеЕсли ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 3 Тогда
		ТЗ = ПрочитатьДанныеИзФайлаИПолучитьТЗ(ИдентификаторИсточникаДанных, ТекстОшибки);
	//Таблица
	ИначеЕсли ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 4 Тогда		
		ТЗ = ПолучитьДанныеИзТабличногоДокумента(ИдентификаторИсточникаДанных, ТекстОшибки);
	//Внешняя база 1С 7.7
	ИначеЕсли ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 5 Тогда		
		ТЗ = ВыполнитьЗапрос1С77ИПолучитьТЗ(ИдентификаторИсточникаДанных, ТекстОшибки);
	//JSON
	ИначеЕсли ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 6 Тогда		
		ТЗ = ПрочитатьДанныеИзJSONИПолучитьТЗ(ИдентификаторИсточникаДанных, ТекстОшибки);
	Иначе
		ТекстОшибки = "Тип базы " + ИдентификаторИсточникаДанных + " '" + ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] + "' не предусмотрен";
		Сообщить(Формат(ТекущаяДата(),"ДЛФ=DT") + ": " + ТекстОшибки);
		ТЗ = Неопределено;
	КонецЕсли;
	
	Если ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] >= 3 И ЭтотОбъект["СворачиватьТаблицу" + ИдентификаторИсточникаДанных] Тогда
		СтолбцыКлюча = "Ключ1";
		Если ЧислоСтолбцовВКлюче > 1 Тогда
...
Функция ВыполнитьЗапрос1С8ИПолучитьТЗ(ИдентификаторИсточникаДанных, ТекстОшибок = "", Подключение = Неопределено)
	
	//Текущая
	Если ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 0 Тогда
		
		Запрос = Новый Запрос;
		Запрос.Текст = ЭтотОбъект["ТекстЗапроса" + ИдентификаторИсточникаДанных];
		
		УстановитьПараметры(Запрос, ИдентификаторИсточникаДанных); 
		
	//Внешняя
	ИначеЕсли ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 1 Тогда
		
		Если ЭтотОбъект["ВариантРаботыВнешнейБазы" + ИдентификаторИсточникаДанных] = 0 Тогда
			ПараметрСоединения = 
				"File=""" + ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "ПутьКБазе"]
				+ """;Usr=""" + ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Логин"]
				+ """;Pwd=""" + ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Пароль"] + """;";	
		Иначе
			ПараметрСоединения = 
				"Srvr=""" + ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Сервер"]
				+ """;Ref=""" + ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "ПутьКБазе"] 
				+ """;Usr=""" + ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Логин"] 
				+ """;Pwd=""" + ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Пароль"] + """;";
		КонецЕсли;
				
		Попытка
			COMConnector = Новый COMОбъект(ЭтотОбъект["ВерсияПлатформыВнешнейБазы" + ИдентификаторИсточникаДанных] + ".COMConnector");
	...
	Если ТЗ <> Неопределено Тогда
		...
		Если ЧислоСтолбцовВКлюче > ЧислоКолонокВТЗ Тогда
			ТекстОшибки = "Выборка из источника " + ИдентификаторИсточникаДанных + " содержит " + ЧислоКолонокВТЗ + " колонок, проверьте корректность заданного числа столбцов в ключе";
		...
		Для СчетчикРеквизитов = 1 По Мин(ЧислоКолонокВТЗ - ЧислоСтолбцовВКлюче, ЧислоРеквизитов) Цикл //
			...
			Если ЭтотОбъект["НастройкиФайла" + ИдентификаторИсточникаДанных].Количество() >= СчетчикРеквизитов Тогда
				ЗаголовокРеквизитаИзНастроек = ЭтотОбъект["НастройкиФайла" + ИдентификаторИсточникаДанных][СчетчикРеквизитов - 1].ЗаголовокРеквизитаДляПользователя;
			Иначе
				ЗаголовокРеквизитаИзНастроек = "";
			КонецЕсли;
			
			ИмяРеквизита = Строка(ИдентификаторИсточникаДанных) + СчетчикРеквизитов;
		...
		Если (ЭтотОбъект["ИспользоватьВКачествеКлючаУникальныйИдентификатор" + ИдентификаторИсточникаДанных]
			Или ЭтотОбъект["ПриводитьКлючКВерхнемуРегистру" + ИдентификаторИсточникаДанных] 
			Или ЭтотОбъект["УдалятьИзКлючаФигурныеСкобки" + ИдентификаторИсточникаДанных] 
			Или ЭтотОбъект["ПриводитьКлючКСтроке" + ИдентификаторИсточникаДанных]
			Или ЭтотОбъект["ВыполнятьПроизвольныйКодКлюча1" + ИдентификаторИсточникаДанных])
			Или ЧислоСтолбцовВКлюче > 1 И 
			(ЭтотОбъект["ИспользоватьВКачествеКлюча2УникальныйИдентификатор" + ИдентификаторИсточникаДанных] 
			Или ЭтотОбъект["ПриводитьКлюч2КВерхнемуРегистру" + ИдентификаторИсточникаДанных] 
			Или ЭтотОбъект["УдалятьИзКлюча2ФигурныеСкобки" + ИдентификаторИсточникаДанных] 
			Или ЭтотОбъект["ПриводитьКлюч2КСтроке" + ИдентификаторИсточникаДанных]
			Или ЭтотОбъект["ВыполнятьПроизвольныйКодКлюча2" + ИдентификаторИсточникаДанных])
			Или ЧислоСтолбцовВКлюче > 2 И 
			(ЭтотОбъект["ИспользоватьВКачествеКлюча3УникальныйИдентификатор" + ИдентификаторИсточникаДанных] 
			Или ЭтотОбъект["ПриводитьКлюч3КВерхнемуРегистру" + ИдентификаторИсточникаДанных] 
			Или ЭтотОбъект["УдалятьИзКлюча3ФигурныеСкобки" + ИдентификаторИсточникаДанных] 
			Или ЭтотОбъект["ПриводитьКлюч3КСтроке" + ИдентификаторИсточникаДанных]
			Или ЭтотОбъект["ВыполнятьПроизвольныйКодКлюча3" + ИдентификаторИсточникаДанных])Тогда
			...
			Если ЭтотОбъект["ИспользоватьВКачествеКлючаУникальныйИдентификатор" + ИдентификаторИсточникаДанных]
				Или ЭтотОбъект["ПриводитьКлючКСтроке" + ИдентификаторИсточникаДанных] 
				Или ЭтотОбъект["ПриводитьКлючКВерхнемуРегистру" + ИдентификаторИсточникаДанных] 
				Или ЭтотОбъект["УдалятьИзКлючаФигурныеСкобки" + ИдентификаторИсточникаДанных] Тогда
			...
			Если ЧислоСтолбцовВКлюче > 1 Тогда
				...
				Если ЭтотОбъект["ИспользоватьВКачествеКлюча2УникальныйИдентификатор" + ИдентификаторИсточникаДанных]
					Или ЭтотОбъект["ПриводитьКлюч2КСтроке" + ИдентификаторИсточникаДанных] 
					Или ЭтотОбъект["ПриводитьКлюч2КВерхнемуРегистру" + ИдентификаторИсточникаДанных] 
					Или ЭтотОбъект["УдалятьИзКлюча2ФигурныеСкобки" + ИдентификаторИсточникаДанных] Тогда
			...
			Если ЧислоСтолбцовВКлюче > 2 Тогда
				...
				Если ЭтотОбъект["ИспользоватьВКачествеКлюча3УникальныйИдентификатор" + ИдентификаторИсточникаДанных]
					Или ЭтотОбъект["ПриводитьКлюч3КСтроке" + ИдентификаторИсточникаДанных] 
					Или ЭтотОбъект["ПриводитьКлюч3КВерхнемуРегистру" + ИдентификаторИсточникаДанных] 
					Или ЭтотОбъект["УдалятьИзКлюча3ФигурныеСкобки" + ИдентификаторИсточникаДанных] Тогда
			...
			Для Каждого СтрокаТЗ Из ТЗ Цикл
				...
				Попытка
					...
					Если ЭтотОбъект["ИспользоватьВКачествеКлючаУникальныйИдентификатор" + ИдентификаторИсточникаДанных] Тогда
						Если ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 0 Тогда
						...
						КонецЕсли;
					ИначеЕсли ЭтотОбъект["ПриводитьКлючКСтроке" + ИдентификаторИсточникаДанных]
						Или ЭтотОбъект["ПриводитьКлючКВерхнемуРегистру" + ИдентификаторИсточникаДанных]
						Или ЭтотОбъект["УдалятьИзКлючаФигурныеСкобки" + ИдентификаторИсточникаДанных] Тогда
						СтрокаТЗ[ИмяКолонкиКлюч1] = СокрЛП(Строка(СтрокаТЗ[ИмяПервойКолонки]));
					КонецЕсли;
					
					Если ЭтотОбъект["ДлинаКлючаПриПриведенииКСтроке" + ИдентификаторИсточникаДанных] > 0 Тогда
						СтрокаТЗ[ИмяКолонкиКлюч1] = СокрЛП(Лев(СтрокаТЗ[ИмяКолонкиКлюч1], ЭтотОбъект["ДлинаКлючаПриПриведенииКСтроке" + ИдентификаторИсточникаДанных]));
					КонецЕсли;
						
					Если ЭтотОбъект["ПриводитьКлючКВерхнемуРегистру" + ИдентификаторИсточникаДанных] Тогда
						СтрокаТЗ[ИмяКолонкиКлюч1] = СокрЛП(ВРег(Строка(СтрокаТЗ[ИмяКолонкиКлюч1])));
					КонецЕсли;
					
					Если ЭтотОбъект["УдалятьИзКлючаФигурныеСкобки" + ИдентификаторИсточникаДанных] Тогда
						СтрокаТЗ[ИмяКолонкиКлюч1] = СокрЛП(СтрЗаменить(СтрЗаменить(Строка(СтрокаТЗ[ИмяКолонкиКлюч1]), "{", ""), "}", ""));
					КонецЕсли;
					
				Исключение
					
					ТекстОшибки = "Ошибка при обработке ключа в строке " + СчетчикСтрок + " выборки из базы " + ИдентификаторИсточникаДанных + ": " + ОписаниеОшибки();
					ТекстОшибок = ТекстОшибок + Символы.ПС + ТекстОшибки;
				...
				Если ЧислоСтолбцовВКлюче > 1 Тогда
					...
					Попытка
						...
						Если ЭтотОбъект["ИспользоватьВКачествеКлюча2УникальныйИдентификатор" + ИдентификаторИсточникаДанных] Тогда
							Если ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 0 Тогда
							...
							КонецЕсли;
						ИначеЕсли ЭтотОбъект["ПриводитьКлюч2КСтроке" + ИдентификаторИсточникаДанных]
							Или ЭтотОбъект["ПриводитьКлюч2КВерхнемуРегистру" + ИдентификаторИсточникаДанных]
							Или ЭтотОбъект["УдалятьИзКлюча2ФигурныеСкобки" + ИдентификаторИсточникаДанных] Тогда
						...
						Если ЭтотОбъект["ДлинаКлюча2ПриПриведенииКСтроке" + ИдентификаторИсточникаДанных] > 0 Тогда
							СтрокаТЗ[ИмяКолонкиКлюч2] = СокрЛП(Лев(СтрокаТЗ[ИмяКолонкиКлюч2], ЭтотОбъект["ДлинаКлючаПриПриведенииКСтроке" + ИдентификаторИсточникаДанных]));
						КонецЕсли;
						
						Если ЭтотОбъект["ПриводитьКлюч2КВерхнемуРегистру" + ИдентификаторИсточникаДанных] Тогда
							СтрокаТЗ[ИмяКолонкиКлюч2] = СокрЛП(ВРег(Строка(СтрокаТЗ[ИмяКолонкиКлюч2])));
						КонецЕсли;
						
						Если ЭтотОбъект["УдалятьИзКлюча2ФигурныеСкобки" + ИдентификаторИсточникаДанных] Тогда
							СтрокаТЗ[ИмяКолонкиКлюч2] = СокрЛП(СтрЗаменить(СтрЗаменить(Строка(СтрокаТЗ[ИмяКолонкиКлюч2]), "{", ""), "}", ""));
						КонецЕсли;
						
					Исключение
						
						ТекстОшибки = "Ошибка при обработке столбца 2 ключа в строке " + СчетчикСтрок + " выборки из базы " + ИдентификаторИсточникаДанных + ": " + ОписаниеОшибки();
						ТекстОшибок = ТекстОшибок + Символы.ПС + ТекстОшибки;
				...
				Если ЧислоСтолбцовВКлюче > 2 Тогда
					...
					Попытка
						...
						Если ЭтотОбъект["ИспользоватьВКачествеКлюча3УникальныйИдентификатор" + ИдентификаторИсточникаДанных] Тогда
							Если ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 0 Тогда
							...
							КонецЕсли;
						ИначеЕсли ЭтотОбъект["ПриводитьКлюч3КСтроке" + ИдентификаторИсточникаДанных]
							Или ЭтотОбъект["ПриводитьКлюч3КВерхнемуРегистру" + ИдентификаторИсточникаДанных]
							Или ЭтотОбъект["УдалятьИзКлюча3ФигурныеСкобки" + ИдентификаторИсточникаДанных] Тогда
							СтрокаТЗ[ИмяКолонкиКлюч3] = СокрЛП(Строка(СтрокаТЗ[ИмяТретьейКолонки]));
						КонецЕсли;
						
						Если ЭтотОбъект["ДлинаКлюча3ПриПриведенииКСтроке" + ИдентификаторИсточникаДанных] > 0 Тогда
							СтрокаТЗ[ИмяКолонкиКлюч3] = СокрЛП(Лев(СтрокаТЗ[ИмяКолонкиКлюч3], ЭтотОбъект["ДлинаКлючаПриПриведенииКСтроке" + ИдентификаторИсточникаДанных]));
						КонецЕсли;
						
						Если ЭтотОбъект["ПриводитьКлюч3КВерхнемуРегистру" + ИдентификаторИсточникаДанных] Тогда
							СтрокаТЗ[ИмяКолонкиКлюч3] = СокрЛП(ВРег(Строка(СтрокаТЗ[ИмяКолонкиКлюч3])));
						КонецЕсли;
						
						Если ЭтотОбъект["УдалятьИзКлюча3ФигурныеСкобки" + ИдентификаторИсточникаДанных] Тогда
							СтрокаТЗ[ИмяКолонкиКлюч3] = СокрЛП(СтрЗаменить(СтрЗаменить(Строка(СтрокаТЗ[ИмяКолонкиКлюч3]), "{", ""), "}", ""));
						КонецЕсли;
						
					Исключение
						
						ТекстОшибки = "Ошибка при обработке столбца 3 ключа в строке " + СчетчикСтрок + " выборки из базы " + ИдентификаторИсточникаДанных + ": " + ОписаниеОшибки();
						ТекстОшибок = ТекстОшибок + Символы.ПС + ТекстОшибки;
...
#Область Произвольный_код_обработки_ключа
				...
				Если ЭтотОбъект["ВыполнятьПроизвольныйКодКлюча1" + ИдентификаторИсточникаДанных] Тогда
					Попытка
					    Выполнить ЭтотОбъект["ПроизвольныйКодКлюча1" + ИдентификаторИсточникаДанных];
					Исключение
						ТекстОшибки = "Ошибка при выполнении произвольного кода (ключ 1: """ + КлючТек + """) источника " + ИдентификаторИсточникаДанных + ": " + ОписаниеОшибки();
				...
				Если ЧислоСтолбцовВКлюче > 1 Тогда
					...
					Если ЭтотОбъект["ВыполнятьПроизвольныйКодКлюча2" + ИдентификаторИсточникаДанных] Тогда
						Попытка
						    Выполнить ЭтотОбъект["ПроизвольныйКодКлюча2" + ИдентификаторИсточникаДанных];
						Исключение
							ТекстОшибки = "Ошибка при выполнении произвольного кода (ключ 2: """ + КлючТек + """) источника " + ИдентификаторИсточникаДанных + ": " + ОписаниеОшибки();
							Сообщить(ТекстОшибки);
						КонецПопытки;
					КонецЕсли;
				...
				Если ЧислоСтолбцовВКлюче > 2 Тогда
					...
					Если ЭтотОбъект["ВыполнятьПроизвольныйКодКлюча3" + ИдентификаторИсточникаДанных] Тогда
						Попытка
						    Выполнить ЭтотОбъект["ПроизвольныйКодКлюча3" + ИдентификаторИсточникаДанных];
						Исключение
							ТекстОшибки = "Ошибка при выполнении произвольного кода (ключ 3: """ + КлючТек + """) источника " + ИдентификаторИсточникаДанных + ": " + ОписаниеОшибки();
							Сообщить(ТекстОшибки);
						КонецПопытки;
					КонецЕсли;
...
Функция ВыполнитьЗапрос1С77ИПолучитьТЗ(ИдентификаторИсточникаДанных, ТекстОшибок = "", Подключение = Неопределено)
	
	ПутьКБазе = ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "ПутьКБазе"];
	Пользователь = ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Логин"];
	Пароль = ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Пароль"];
	...
	ТекстЗапроса = ЭтотОбъект["ТекстЗапроса" + ИдентификаторИсточникаДанных];
	...
	Для СчетчикСтрок = 1 По КоличествоСтрок Цикл
		...
		Попытка
			...
			Если ЭтотОбъект["ПриводитьКлючКСтроке" + ИдентификаторИсточникаДанных] Тогда
		...
		Если ЧислоСтолбцовВКлюче > 1 Тогда
			...
			Попытка
				...
				Если ЭтотОбъект["ПриводитьКлюч2КСтроке" + ИдентификаторИсточникаДанных] Тогда
		...
		Если ЧислоСтолбцовВКлюче > 2 Тогда
			...
			Если ЭтотОбъект["ПриводитьКлюч3КСтроке" + ИдентификаторИсточникаДанных] Тогда
...
#Область Произвольный_код_обработки_ключа
			...
			Если ЭтотОбъект["ВыполнятьПроизвольныйКодКлюча1" + ИдентификаторИсточникаДанных] Тогда
				Попытка
				    Выполнить ЭтотОбъект["ПроизвольныйКодКлюча1" + ИдентификаторИсточникаДанных];
				Исключение
					ТекстОшибки = "Ошибка при выполнении произвольного кода (ключ 1: """ + Ключ1 + """) источника " + ИдентификаторИсточникаДанных + ": " + ОписаниеОшибки();
					Сообщить(ТекстОшибки);
				КонецПопытки;
			КонецЕсли;
			...
			Если ЧислоСтолбцовВКлюче > 1 Тогда
				...
				Если ЭтотОбъект["ВыполнятьПроизвольныйКодКлюча2" + ИдентификаторИсточникаДанных] Тогда
					Попытка
					    Выполнить ЭтотОбъект["ПроизвольныйКодКлюча2" + ИдентификаторИсточникаДанных];
					Исключение
						ТекстОшибки = "Ошибка при выполнении произвольного кода (ключ 2: """ + Ключ2 + """) источника " + ИдентификаторИсточникаДанных + ": " + ОписаниеОшибки();
						Сообщить(ТекстОшибки);
					КонецПопытки;
				КонецЕсли;
			...
			Если ЧислоСтолбцовВКлюче > 2 Тогда
				...
				Если ЭтотОбъект["ВыполнятьПроизвольныйКодКлюча3" + ИдентификаторИсточникаДанных] Тогда
					Попытка
					    Выполнить ЭтотОбъект["ПроизвольныйКодКлюча3" + ИдентификаторИсточникаДанных];
					Исключение
						ТекстОшибки = "Ошибка при выполнении произвольного кода (ключ 3: """ + Ключ3 + """) источника " + ИдентификаторИсточникаДанных + ": " + ОписаниеОшибки();
						Сообщить(ТекстОшибки);
					КонецПопытки;
				КонецЕсли;
...
#Область Заполнение_ТЗ
	...
	Если ТЗ <> Неопределено Тогда
		...
		Если (ЭтотОбъект["ПриводитьКлючКВерхнемуРегистру" + ИдентификаторИсточникаДанных] 
		Или ЭтотОбъект["УдалятьИзКлючаФигурныеСкобки" + ИдентификаторИсточникаДанных] 
		Или ЭтотОбъект["ПриводитьКлючКСтроке" + ИдентификаторИсточникаДанных]
		Или ЭтотОбъект["ВыполнятьПроизвольныйКодКлюча1" + ИдентификаторИсточникаДанных])
		Или ЧислоСтолбцовВКлюче > 1 И 
		(ЭтотОбъект["ПриводитьКлюч2КВерхнемуРегистру" + ИдентификаторИсточникаДанных] 
		Или ЭтотОбъект["УдалятьИзКлюча2ФигурныеСкобки" + ИдентификаторИсточникаДанных] 
		Или ЭтотОбъект["ПриводитьКлюч2КСтроке" + ИдентификаторИсточникаДанных]
		Или ЭтотОбъект["ВыполнятьПроизвольныйКодКлюча2" + ИдентификаторИсточникаДанных])
		Или ЧислоСтолбцовВКлюче > 2 И 
		(ЭтотОбъект["ПриводитьКлюч3КВерхнемуРегистру" + ИдентификаторИсточникаДанных] 
		Или ЭтотОбъект["УдалятьИзКлюча3ФигурныеСкобки" + ИдентификаторИсточникаДанных] 
		Или ЭтотОбъект["ПриводитьКлюч3КСтроке" + ИдентификаторИсточникаДанных]
		Или ЭтотОбъект["ВыполнятьПроизвольныйКодКлюча3" + ИдентификаторИсточникаДанных])Тогда
			...
			Если ЭтотОбъект["ПриводитьКлючКСтроке" + ИдентификаторИсточникаДанных] 
				Или ЭтотОбъект["ПриводитьКлючКВерхнемуРегистру" + ИдентификаторИсточникаДанных] 
				Или ЭтотОбъект["УдалятьИзКлючаФигурныеСкобки" + ИдентификаторИсточникаДанных] Тогда
			...
			Если ЧислоСтолбцовВКлюче > 1 Тогда
				...
				Если ЭтотОбъект["ПриводитьКлюч2КСтроке" + ИдентификаторИсточникаДанных] 
					Или ЭтотОбъект["ПриводитьКлюч2КВерхнемуРегистру" + ИдентификаторИсточникаДанных] 
					Или ЭтотОбъект["УдалятьИзКлюча2ФигурныеСкобки" + ИдентификаторИсточникаДанных] Тогда
			...
			Если ЧислоСтолбцовВКлюче > 2 Тогда
				...
				Если ЭтотОбъект["ПриводитьКлюч3КСтроке" + ИдентификаторИсточникаДанных] 
					Или ЭтотОбъект["ПриводитьКлюч3КВерхнемуРегистру" + ИдентификаторИсточникаДанных] 
					Или ЭтотОбъект["УдалятьИзКлюча3ФигурныеСкобки" + ИдентификаторИсточникаДанных] Тогда
...
#КонецОбласти
		...
		Для СчетчикРеквизитов = 1 По ЭтотОбъект["НастройкиФайла" + ИдентификаторИсточникаДанных].Количество() Цикл
			
			ИмяРеквизита = Строка(ИдентификаторИсточникаДанных) + СчетчикРеквизитов;
			ЗаголовокРеквизитаИзНастроек = ЭтотОбъект["НастройкиФайла" + ИдентификаторИсточникаДанных][СчетчикРеквизитов - 1].ЗаголовокРеквизитаДляПользователя;
			
			ПредставленияЗаголовковРеквизитов[ИмяРеквизита] = ?(ПустаяСтрока(ЗаголовокРеквизитаИзНастроек), "Реквизит " + ИдентификаторИсточникаДанных + СчетчикРеквизитов, ИмяРеквизита + ": " + ЗаголовокРеквизитаИзНастроек);
...
Функция ВыполнитьЗапросSQLИПолучитьТЗ(ИдентификаторИсточникаДанных, ТекстОшибок = "")
	
	ServerName =  ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Сервер"];
	DSN 	= ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "ПутьКБазе"];                                                                                                           
	UID 	= ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Логин"];
	PWD 	= ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Пароль"];
	Driver 	= ЭтотОбъект["ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "ДрайверSQL"];
	...
	Попытка
		...
		Command.CommandText = ЭтотОбъект["ТекстЗапроса" + ИдентификаторИсточникаДанных];
		...
		Если ЧислоСтолбцовВКлюче > ЧислоКолонокВТЗ Тогда
			...
			ТекстОшибки = "Выборка из источника " + ИдентификаторИсточникаДанных + " содержит " + ЧислоКолонокВТЗ + " кол., проверьте корректность заданного числа столбцов в ключе";
			СообщениеПользователю = Новый СообщениеПользователю;
			СообщениеПользователю.Текст = ТекстОшибки;
			СообщениеПользователю.Поле = "Объект.ЧислоСтолбцовВКлюче";
			СообщениеПользователю.Сообщить();
...
Функция ПрочитатьДанныеИзФайлаИПолучитьТЗ(ИдентификаторИсточникаДанных, ТекстОшибок = "")
	...
	ПутьКФайлу 			= ЭтотОбъект["ПодключениеКВнешнейБазе"		+ ИдентификаторИсточникаДанных + "ПутьКФайлу"];
	ФорматФайла 		= ЭтотОбъект["ПодключениеКВнешнейБазе" 		+ ИдентификаторИсточникаДанных + "ФорматФайла"];
	НомерПервойСтроки 	= ЭтотОбъект["НомерПервойСтрокиФайла" 		+ ИдентификаторИсточникаДанных];
	НастройкиФайла 		= ЭтотОбъект["НастройкиФайла" 				+ ИдентификаторИсточникаДанных];
	НомерТаблицы		= ЭтотОбъект["ПодключениеКВнешнейБазе"		+ ИдентификаторИсточникаДанных + "НомерТаблицыВФайле"];
	
	НомерСтолбцаСКлючом = ЭтотОбъект["НомерСтолбцаСКлючомИзФайла" 	+ ИдентификаторИсточникаДанных];
	ИмяСтолбцаСКлючом 	= ЭтотОбъект["ИмяСтолбцаСКлючомИзФайла" 	+ ИдентификаторИсточникаДанных];	
	Если ЧислоСтолбцовВКлюче > 1 Тогда
		НомерСтолбцаСКлючом2 	= ЭтотОбъект["НомерСтолбцаСКлючом2ИзФайла" + ИдентификаторИсточникаДанных];
		ИмяСтолбцаСКлючом2 		= ЭтотОбъект["ИмяСтолбцаСКлючом2ИзФайла" + ИдентификаторИсточникаДанных];
	КонецЕсли;	
	Если ЧислоСтолбцовВКлюче > 2 Тогда
		НомерСтолбцаСКлючом3 	= ЭтотОбъект["НомерСтолбцаСКлючом3ИзФайла" + ИдентификаторИсточникаДанных];
		ИмяСтолбцаСКлючом3 		= ЭтотОбъект["ИмяСтолбцаСКлючом3ИзФайла" + ИдентификаторИсточникаДанных];
	КонецЕсли;
...
#Область XML
	...
	Если ФорматФайла = "XML" Тогда
		...
		ИмяРодительскогоУзла = ЭтотОбъект["ИмяРодительскогоУзлаФайла" + ИдентификаторИсточникаДанных];
		ИмяЭлементаСДаннымиФайла = ЭтотОбъект["ИмяЭлементаСДаннымиФайла" + ИдентификаторИсточникаДанных];
...
#Область XML_СпособХраненияДанныхВXMLФайле_В_атрибутах
					...
					Если ЭтотОбъект["СпособХраненияДанныхВXMLФайле" + ИдентификаторИсточникаДанных] = "В атрибутах" Тогда
...
#Область XML_СпособХраненияДанныхВXMLФайле_В_элементах
					...
					ИначеЕсли ЭтотОбъект["СпособХраненияДанныхВXMLФайле" + ИдентификаторИсточникаДанных] = "В элементах" Тогда
...
#КонецОбласти 
					...
					Иначе 
						ВызватьИсключение "Не задан способ хранения данных в XML-файле базы " + ИдентификаторИсточникаДанных;				
					КонецЕсли;
					...
					Для Каждого СтрокаНастроекФайла Из НастройкиФайла Цикл
						...
						Если ЭтотОбъект["СворачиватьТаблицу" + ИдентификаторИсточникаДанных] Тогда
							Попытка
...
#Область XLS
			...
			Если ФорматФайла = "XLS" Тогда
				...
				Если НомерСтолбцаСКлючом > НомерПоследнейКолонки Тогда
					ТекстОшибки = "Файл " + ИдентификаторИсточникаДанных + " содержит " + НомерПоследнейКолонки + " кол., проверьте настройки столбцов ключа";
					СообщениеПользователю = Новый СообщениеПользователю;
					СообщениеПользователю.Текст = ТекстОшибки;
					СообщениеПользователю.Поле = "Объект.НомерСтолбцаСКлючомИзФайла " + ИдентификаторИсточникаДанных;
					СообщениеПользователю.Сообщить();
					ТекстОшибок = ТекстОшибок + Символы.ПС + ТекстОшибки;
				...
				Если ЧислоСтолбцовВКлюче > 1 И НомерСтолбцаСКлючом2 > НомерПоследнейКолонки Тогда
					ТекстОшибки = "Файл " + ИдентификаторИсточникаДанных + " содержит " + НомерПоследнейКолонки + " кол., проверьте настройки столбца 2 ключа";
					СообщениеПользователю = Новый СообщениеПользователю;
					СообщениеПользователю.Текст = ТекстОшибки;
					СообщениеПользователю.Поле = "Объект.НомерСтолбцаСКлючом2ИзФайла " + ИдентификаторИсточникаДанных;
					СообщениеПользователю.Сообщить();
					ТекстОшибок = ТекстОшибок + Символы.ПС + ТекстОшибки;
					ЧислоКолонокВФайлеМеньшеТребуемого = Истина;
				КонецЕсли;
				...
				Если ЧислоСтолбцовВКлюче > 2 И НомерСтолбцаСКлючом3 > НомерПоследнейКолонки Тогда
					ТекстОшибки = "Файл " + ИдентификаторИсточникаДанных + " содержит " + НомерПоследнейКолонки + " кол., проверьте настройки столбца 3 ключа";
					СообщениеПользователю = Новый СообщениеПользователю;
					СообщениеПользователю.Текст = ТекстОшибки;
					СообщениеПользователю.Поле = "Объект.НомерСтолбцаСКлючом3ИзФайла " + ИдентификаторИсточникаДанных;
					СообщениеПользователю.Сообщить();
					ТекстОшибок = ТекстОшибок + Символы.ПС + ТекстОшибки;
					ЧислоКолонокВФайлеМеньшеТребуемого = Истина;
				КонецЕсли;
...
#КонецОбласти
				...
				Для Каждого СтрокаНастроекФайла Из НастройкиФайла Цикл
					...
					Если СтрокаНастроекФайла.НомерКолонки > НомерПоследнейКолонки Тогда
						ТекстОшибки = "Файл " + ИдентификаторИсточникаДанных + " содержит " + НомерПоследнейКолонки + " кол., проверьте настройки колонок реквизитов";
						СообщениеПользователю = Новый СообщениеПользователю;
						СообщениеПользователю.Текст = ТекстОшибки;
						СообщениеПользователю.Поле = "Объект.НастройкиФайла" + ИдентификаторИсточникаДанных;
						СообщениеПользователю.Сообщить();
						ТекстОшибок = ТекстОшибок + Символы.ПС + ТекстОшибки;
						Возврат ТЗ;
					КонецЕсли;
...
Функция ПрочитатьДанныеИзJSONИПолучитьТЗ(ИдентификаторИсточникаДанных, ТекстОшибок = "")
	...
	ЧтениеJSON.УстановитьСтроку(ЭтотОбъект["ТекстЗапроса" + ИдентификаторИсточникаДанных]);
...
Функция ПолучитьДанныеИзТабличногоДокумента(ИдентификаторИсточникаДанных, ТекстОшибок = "")
	...
	Пока Истина Цикл
		...
		Ключ1 = ЭтотОбъект["Таблица" + ИдентификаторИсточникаДанных].Область(НомерТекущейСтроки,НомерСтолбцаСКлючом,НомерТекущейСтроки,НомерСтолбцаСКлючом).Текст;
		...
		Если ЧислоСтолбцовВКлюче > 1 Тогда
			...
			Ключ2 = ЭтотОбъект["Таблица" + ИдентификаторИсточникаДанных].Область(НомерТекущейСтроки,НомерСтолбцаСКлючом2,НомерТекущейСтроки,НомерСтолбцаСКлючом2).Текст;
		...
		Если ЧислоСтолбцовВКлюче > 2 Тогда
			...
			Ключ3 = ЭтотОбъект["Таблица" + ИдентификаторИсточникаДанных].Область(НомерТекущейСтроки,НомерСтолбцаСКлючом3,НомерТекущейСтроки,НомерСтолбцаСКлючом3).Текст;
...
#КонецОбласти 
		...
		Для Каждого СтрокаНастроекФайла Из НастройкиФайла Цикл
			...
			ИмяРеквизита = "Реквизит" + СтрокаНастроекФайла.НомерСтроки;
			СтрокаПриемник[ИмяРеквизита] = СокрЛП(ЭтотОбъект["Таблица" + ИдентификаторИсточникаДанных].Область(НомерТекущейСтроки,СтрокаНастроекФайла.НомерКолонки,НомерТекущейСтроки,СтрокаНастроекФайла.НомерКолонки).Текст);
...
Функция ПроверитьВозможностьПодключенияКИсточнику(ИдентификаторИсточникаДанных, ТекстОшибок) Экспорт
	
	//Внешняя база 1С 8
	Если ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 1 Тогда
		 ПодключениеВыполненоУспешно = ПроверитьВозможностьПодключенияКВнешейБазе1С8(ИдентификаторИсточникаДанных, ТекстОшибок);
	//SQL
	ИначеЕсли ЭтотОбъект["ТипБазы" + ИдентификаторИсточникаДанных] = 2 Тогда
		ПодключениеВыполненоУспешно = ПроверитьВозможностьПодключенияКБазеДанныхSQL(ИдентификаторИсточникаДанных, ТекстОшибок);
	Иначе
...
Функция ПолучитьДанныеВВидеСтруктурыНаСервере(СохранятьТабличныеДокументы = Ложь) Экспорт
	...
	ДанныеСтруктура.Вставить("ВидимостьЧислоЗаписейА",									ВидимостьЧислоЗаписейА);
	ДанныеСтруктура.Вставить("ВидимостьЧислоЗаписейБ",									ВидимостьЧислоЗаписейБ);
	ДанныеСтруктура.Вставить("ВидимостьКлюч1",											ВидимостьКлюч1);
	ДанныеСтруктура.Вставить("ВидимостьКлюч2",											ВидимостьКлюч2);
	ДанныеСтруктура.Вставить("ВидимостьКлюч3",											ВидимостьКлюч3);
...
//Функция ПолучитьМассивОбщихРеквизитов() Экспорт
//	
//	РеквизитыМассив = Новый Массив;
//	
//	РеквизитыМассив.Добавить("КодДляВыводаСтрок");
//	РеквизитыМассив.Добавить("КодДляЗапретаВыводаСтрок");
//	РеквизитыМассив.Добавить("ЛогическийОператорДляУсловийВыводаСтрок");
//	РеквизитыМассив.Добавить("ЛогическийОператорДляУсловийЗапретаВыводаСтрок");
//	РеквизитыМассив.Добавить("КодДляВыводаСтрокРедактируетсяВручную");
//	РеквизитыМассив.Добавить("КодДляЗапретаВыводаСтрокРедактируетсяВручную");
//	РеквизитыМассив.Добавить("УсловияВыводаСтрокОтключены");
//	РеквизитыМассив.Добавить("УсловияЗапретаВыводаСтрокОтключены");
//	РеквизитыМассив.Добавить("РеляционнаяОперация");
//	РеквизитыМассив.Добавить("ТипПериодаАбсолютный");
//	РеквизитыМассив.Добавить("ТипПериода");
//	РеквизитыМассив.Добавить("ЗначениеАбсолютногоПериода");
//	РеквизитыМассив.Добавить("ЗначениеОтносительногоПериода");
//	РеквизитыМассив.Добавить("ЗначениеПодчиненногоОтносительногоПериода");
//	РеквизитыМассив.Добавить("ДискретностьОтносительногоПериода");
//	РеквизитыМассив.Добавить("ДискретностьПодчиненногоОтносительногоПериода");
//	РеквизитыМассив.Добавить("СоставнойКлюч");
//	РеквизитыМассив.Добавить("ЧислоСтолбцовВКлюче");
//	РеквизитыМассив.Добавить("ЧислоСтрокСПустымиКлючамиДляПрерыванияЧтения");
//	РеквизитыМассив.Добавить("ОтображатьТипыСтолбцовКлюча");
//	РеквизитыМассив.Добавить("ПутьКФайлуВыгрузки");
//	РеквизитыМассив.Добавить("ФорматФайлаВыгрузки");
//	РеквизитыМассив.Добавить("СортироватьТаблицуРасхождений");
//	РеквизитыМассив.Добавить("ПорядокСортировкиТаблицыРасхождений");
//	
//	Возврат РеквизитыМассив;
//	
//КонецФункции

//Функция ПолучитьМассивОбщихТЧ(ИдентификаторИсточникаДанных) Экспорт
//	
//	МассивТЧ = Новый Массив;
//	
//	МассивТЧ.Добавить("УсловияВыводаСтрок");
//	МассивТЧ.Добавить("УсловияЗапретаВыводаСтрок");

//	Возврат МассивТЧ;
//	
//КонецФункции

//Функция ПолучитьМассивРеквизитовИсточника(ИдентификаторИсточникаДанных) Экспорт
//	
//	РеквизитыМассив = Новый Массив;
//	
//	РеквизитыМассив.Добавить("ТекстЗапроса" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ТипБазы" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "ПутьКБазе");
//	РеквизитыМассив.Добавить("ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Логин");
//	РеквизитыМассив.Добавить("ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Пароль");
//	РеквизитыМассив.Добавить("ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "Сервер");
//	РеквизитыМассив.Добавить("ВариантРаботыВнешнейБазы" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ВерсияПлатформыВнешнейБазыА");
//	РеквизитыМассив.Добавить("ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "ДрайверSQL");
//	РеквизитыМассив.Добавить("ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "ФорматФайла");
//	РеквизитыМассив.Добавить("ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "ПутьКФайлу");
//	РеквизитыМассив.Добавить("ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "УстройствоХраненияФайла");
//	РеквизитыМассив.Добавить("ПодключениеКВнешнейБазе" + ИдентификаторИсточникаДанных + "НомерТаблицыВФайле");
//	РеквизитыМассив.Добавить("НомерПервойСтрокиФайла" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("НомерСтолбцаСКлючомИзФайла" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("НомерСтолбцаСКлючом2ИзФайла" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("НомерСтолбцаСКлючом3ИзФайла" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ИмяСтолбцаСКлючомИзФайла" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ИмяСтолбцаСКлючом2ИзФайла" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ИмяСтолбцаСКлючом3ИзФайла" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ИмяЭлементаСДаннымиФайла" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ИмяРодительскогоУзлаФайла" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("СпособХраненияДанныхВXMLФайле" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ПриводитьКлючКСтроке" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ПриводитьКлюч2КСтроке" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ПриводитьКлюч3КСтроке" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ДлинаКлючаПриПриведенииКСтроке" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ДлинаКлюча2ПриПриведенииКСтроке" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ДлинаКлюча3ПриПриведенииКСтроке" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ИспользоватьВКачествеКлючаУникальныйИдентификатор" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ИспользоватьВКачествеКлюча2УникальныйИдентификатор" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ИспользоватьВКачествеКлюча3УникальныйИдентификатор" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ПриводитьКлючКВерхнемуРегистру" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ПриводитьКлюч2КВерхнемуРегистру" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ПриводитьКлюч3КВерхнемуРегистру" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("УдалятьИзКлючаФигурныеСкобки" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("УдалятьИзКлюча2ФигурныеСкобки" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("УдалятьИзКлюча3ФигурныеСкобки" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ПроизвольныйКодКлюча1" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ПроизвольныйКодКлюча2" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ПроизвольныйКодКлюча3" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ВыполнятьПроизвольныйКодКлюча1" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ВыполнятьПроизвольныйКодКлюча2" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ВыполнятьПроизвольныйКодКлюча3А" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("ВидимостьРеквизит" + ИдентификаторИсточникаДанных + "1");
//	РеквизитыМассив.Добавить("ВидимостьРеквизит" + ИдентификаторИсточникаДанных + "2");
//	РеквизитыМассив.Добавить("ВидимостьРеквизит" + ИдентификаторИсточникаДанных + "3");
//	РеквизитыМассив.Добавить("ВидимостьРеквизит" + ИдентификаторИсточникаДанных + "4");
//	РеквизитыМассив.Добавить("ВидимостьРеквизит" + ИдентификаторИсточникаДанных + "5");
//	РеквизитыМассив.Добавить("СворачиватьТаблицу" + ИдентификаторИсточникаДанных);
//	РеквизитыМассив.Добавить("Таблица" + ИдентификаторИсточникаДанных);
//	
//	Возврат РеквизитыМассив;
//	
//КонецФункции

Функция ПолучитьМассивТЧИсточника(ИдентификаторИсточникаДанных) Экспорт
	
	МассивТЧ = Новый Массив;
	
	МассивТЧ.Добавить("НастройкиФайла" + ИдентификаторИсточникаДанных);
	МассивТЧ.Добавить("СписокПараметров" + ИдентификаторИсточникаДанных);
		
	Возврат МассивТЧ;
	
КонецФункции

Функция ПоменятьМестамиИсточникиНаСервере() Экспорт
	
	Для Каждого РеквизитОбработки Из ЭтотОбъект.Метаданные().Реквизиты Цикл
	
		Сообщить(РеквизитОбработки.Имя);
	
	КонецЦикла;
	
	Для Каждого ТЧОбработки Из ЭтотОбъект.Метаданные().ТабличныеЧасти Цикл
		
		Сообщить(ТЧОбработки.Имя);
		
		Для Каждого РеквизитТЧОбработки Из ТЧОбработки.Реквизиты Цикл
	
			Сообщить("	" + РеквизитТЧОбработки.Имя);
...
Процедура УстановитьПараметры(Запрос, ИдентификаторИсточникаДанных)
	
	Для Каждого Параметр Из ЭтотОбъект["СписокПараметров" + ИдентификаторИсточникаДанных] Цикл
...
Функция ВыгрузитьРезультатВФайлНаСервере(ДляКлиента = Ложь, РасширениеФорматФайла = Неопределено) Экспорт

	ЕстьОшибкиЗаполнения = Ложь;
	
	Если ДляКлиента = Ложь Тогда
		
		РасширениеФорматФайла = ФорматФайлаВыгрузки;
	
	КонецЕсли;
	
	РасширениеФорматФайла = ВРег(РасширениеФорматФайла);
	
	Если ПустаяСтрока(РасширениеФорматФайла) Тогда
		
		ТекстОшибки = "Не указан формат файла выгрузки (на сервере)";
		СообщениеПользователю = Новый СообщениеПользователю;
		СообщениеПользователю.Текст = ТекстОшибки;
		Если ДляКлиента = Ложь  Тогда
	...
	Если ДляКлиента Тогда
		...
		ПутьКВременномуФайлу = ПолучитьИмяВременногоФайла(РасширениеФорматФайла);
	...
	Сообщить(Формат(ТекущаяДата(), "ДЛФ=DT") + " Выгрузка в файл """ + ПутьКВременномуФайлу + """ формата """ + РасширениеФорматФайла + """ начата");
	
	Если РасширениеФорматФайла = "CSV" Тогда
	...
	ИначеЕсли РасширениеФорматФайла = "XLS" Или
		РасширениеФорматФайла = "DOCX" Или
		РасширениеФорматФайла = "HTML" Или
		РасширениеФорматФайла = "MXL" Или
		РасширениеФорматФайла = "ODS" Или
		РасширениеФорматФайла = "PDF" Или
		РасширениеФорматФайла = "TXT" Или
		РасширениеФорматФайла = "XLSX" Тогда
		...
		ТабличныйДокумент.Записать(ПутьКВременномуФайлу, ТипФайлаТабличногоДокумента[РасширениеФорматФайла]);
		
	Иначе
		
		ТекстОшибки = "Формат файла выгрузки """ + РасширениеФорматФайла + """ не предусмотрен";
		СообщениеПользователю = Новый СообщениеПользователю;
		СообщениеПользователю.Текст = ТекстОшибки;
		Если ДляКлиента = Ложь Тогда
			СообщениеПользователю.Поле = "Объект.ФорматФайлаВыгрузки";
...
Функция ПроверитьВозможностьПодключенияКВнешейБазе1С8(ИдентификаторИсточникаДанных, ТекстОшибок)
...
Функция ПроверитьВозможностьПодключенияКБазеДанныхSQL(ИдентификаторИсточникаДанных, ТекстОшибок)
