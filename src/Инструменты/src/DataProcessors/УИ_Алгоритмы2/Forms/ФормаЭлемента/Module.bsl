...
Процедура ТаблицаПараметровЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОповещения.Вставить("ИдентификаторСтроки", Элементы.ТаблицаПараметров.ТекущаяСтрока);

	УИ_ОбщегоНазначенияКлиент.ПолеФормыНачалоВыбораЗначения(ЭтотОбъект,
															Элемент,
															ТекДанные.Значение,
															СтандартнаяОбработка,
															Новый ОписаниеОповещения("ТаблицаПараметровЗначениеНачалоВыбораЗавершениеВыбораПустогоЗначения",
		ЭтотОбъект, ПараметрыОповещения),
															УИ_ОбщегоНазначенияКлиентСервер.ВсеНаборыТиповДляРедактирования(),
															Новый ОписаниеОповещения("ТаблицаПараметровЗначениеНачалоВыбораЗавершениеВыбораТипа",
		ЭтотОбъект, ПараметрыОповещения));
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаПараметровЗначениеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	ТекДанные = Элементы.ТаблицаПараметров.ТекущиеДанные;
	Если ТекДанные = Неопределено Тогда
		ВозвраТ;
	КонецЕсли;
...
Процедура ТаблицаПараметровЗначениеОчистка(Элемент, СтандартнаяОбработка)
	...
	ТекДанные.ТипЗначенияПредставление = "";
...
&НаКлиенте
Процедура ТаблицаПараметровЗначениеНачалоВыбораЗавершениеВыбораПустогоЗначения(Результат, ДополнительныеПараметры) Экспорт
	СтрокаПеременной = ТаблицаПараметров.НайтиПоИдентификатору(ДополнительныеПараметры.ИдентификаторСтроки);
...
&НаКлиенте
Процедура ТаблицаПараметровЗначениеНачалоВыбораЗавершениеВыбораТипа(Результат, ДополнительныеПараметры) Экспорт
	СтрокаПеременной = ТаблицаПараметров.НайтиПоИдентификатору(ДополнительныеПараметры.ИдентификаторСтроки);
	Если СтрокаПеременной = Неопределено Тогда
		Возврат;
	КонецЕсли;
	СтрокаПеременной.ТипЗначенияПредставление = Строка(Результат);
	
	Если Результат = Новый ОписаниеТипов("ТаблицаЗначений") Тогда
		СтрокаПеременной.ТипЗначенияСтрокой = УИ_ОбщегоНазначенияКлиентСервер.ТипСтрокойТаблицаЗначений();
		СтрокаПеременной.ТипЗначения = Новый ОписаниеТипов("Строка");
	Иначе
