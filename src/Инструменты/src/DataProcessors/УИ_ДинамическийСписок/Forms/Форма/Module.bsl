...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ИмяОбъектаМетаданных) Тогда
		Элементы.ФормаПоказатьДеревоМетаданных.Видимость=Ложь;
		УстановитьВидимостьДереваМетаданных(ЭтотОбъект, Ложь);
		УстановитьПараметрыДинамическогоСпискаНаСервере(ИдентификаторСтрокиОбъектаМетаданных);
		УИ_ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
																		"ДинамическийСписок",
																		"ТекущаяСтрока",
...
Процедура УдалитьВыделенныеОбъекты(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("УдалитьВыделенныеОбъектыЗавершение", ЭтотОбъект, ДополнительныеПараметры), ТекстВопроса,
		РежимДиалогаВопрос.ДаНет, 20, , "ВНИМАНИЕ");
КонецПроцедуры

&НаКлиенте
Процедура УдалитьВыделенныеОбъектыСКонтролемСсылок(Команда)
	Если Элементы.ГруппаСтраницыВыбораДинамическогоСписка.ТекущаяСтраница <> Элементы.ГруппаДинамическийСписок Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыФормы.Вставить("УИ_УдаляемыеОбъекты", МассивСсылок);
	ОткрытьФорму("Обработка.УИ_УдалениеПомеченныхОбъектов.Форма.ОсновнаяФорма", ПараметрыФормы);
КонецПроцедуры

&НаКлиенте
Процедура ПоказатьДеревоМетаданных(Команда)

	УстановитьВидимостьДереваМетаданных(ЭтотОбъект);
...
Процедура УдалитьДинамическийСписок(ИмяРеквизита)
	...
	ДинамичныйСписок = Элементы.Найти(ИмяРеквизита);
...
Процедура УстановитьПараметрыДинамическогоСпискаНаСервере(ВыбраннаяСтрока)
	...
	Если ТипЗнч(ВыборГруппИЭлементов) = Тип("ИспользованиеГруппИЭлементов") Тогда
		ЭлементФормы.ВыборГруппИЭлементов = ВыборГруппИЭлементов;
	Иначе
	...
	Для Каждого ТекРеквизит Из ТекДанные.Реквизиты Цикл
		...
		Попытка
			...
			Колонка.Вид = ?(УИ_ОбщегоНазначения.ОписаниеТипаСостоитИзТипа(ТекРеквизит.ТипЗначения, Тип("Булево")),
		...
		Исключение
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось создать колонку списка для реквизита %1'"), ТекРеквизит.Имя);
	...
	ОписаниеКнопки=УИ_РаботаСФормами.НовыйОписаниеКомандыКнопки();
	ОписаниеКнопки.Имя=ИмяРеквизитаФормы + "УдалитьВыделенныеОбъектыСКонтролемСсылок";
	ОписаниеКнопки.РодительЭлемента = ГруппаКоманднаяПанель;
	ОписаниеКнопки.СоздаватьКнопку=Истина;
	ОписаниеКнопки.ИмяКоманды="УдалитьВыделенныеОбъектыСКонтролемСсылок";
	УИ_РаботаСФормами.СоздатьКнопкуПоОписанию(ЭтотОбъект, ОписаниеКнопки);
...
Процедура ДобавитьВДеревоВидОбъекта(Картинка, ИмяТипаМетаданных, ИмяТипаОбъектов, Представление)
	...
	Для Каждого МетаданныеОбъекта Из Метаданные[ИмяТипаМетаданных] Цикл
		СинонимОбъектаМетаданных = ?(ПустаяСтрока(МетаданныеОбъекта.Синоним), "", " ("
																				  + МетаданныеОбъекта.Синоним
																				  + ")");

		Если ЗначениеЗаполнено(СтрокаПоиска) И СтрНайти(ВРег(СинонимОбъектаМетаданных), ВРег(СтрокаПоиска)) = 0 Тогда
			Продолжить;
		КонецЕсли;

		ЭтоЖурналДокументов=УИ_ОбщегоНазначения.ЭтоЖурналДокументов(МетаданныеОбъекта);

		СтрокаВида = ЭлементыСтрокиДерева.Добавить();
		СтрокаВида.Картинка = Картинка;
		СтрокаВида.Представление = МетаданныеОбъекта.Имя + СинонимОбъектаМетаданных;
		СтрокаВида.ТипОбъектов = ИмяТипаОбъектов;
		СтрокаВида.ВидОбъекта = МетаданныеОбъекта.Имя;
		СтрокаВида.Комментарий = МетаданныеОбъекта.Комментарий;
		СтрокаВида.ПолноеИмя = МетаданныеОбъекта.ПолноеИмя();
		...
		МассивТаблицРеквизитов = МассивТаблицРеквизитовДляОбъектаМетаданных(МетаданныеОбъекта);
...
Процедура УдалитьВыделенныеОбъектыНаСервере(МассивСсылок)
	Для Каждого Ссылка Из МассивСсылок Цикл
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка при удалении объекта:
										 |%1'"), ОписаниеОшибки);
			УИ_ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
		КонецПопытки;
	КонецЦикла;
...
Процедура УстановитьВидимостьДереваМетаданных(Форма, Знач ВидимостьДерева = Неопределено)

	Элементы = Форма.Элементы;
	Если ВидимостьДерева = Неопределено Тогда
		ВидимостьДерева = НЕ Элементы.ФормаПоказатьДеревоМетаданных.Пометка;
	КонецЕсли;

	Элементы.ФормаПоказатьДеревоМетаданных.Пометка = ВидимостьДерева;
	Элементы.ГруппаМетаданные.Видимость = Элементы.ФормаПоказатьДеревоМетаданных.Пометка;
