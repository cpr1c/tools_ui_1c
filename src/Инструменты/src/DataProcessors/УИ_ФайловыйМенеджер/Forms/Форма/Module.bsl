...
&НаКлиенте
Асинх Функция СкопироватьНаКлиент()
...
&НаКлиенте
Асинх Функция СкопироватьНаСервер()
	...
	Для Каждого ИдентификаторСтроки Из ЭлементТаблицы.ВыделенныеСтроки Цикл
		...
		Иначе
			...
			СтрокаСтруктура.Вставить("АдресВХранилище", АдресВХранилище);
		КонецЕсли;
	КонецЦикла;
...
Асинх Процедура ПереместитьССервераНаКлиент()
	Ждать СкопироватьНаКлиент();
	УдалитьНаСервере();
КонецПроцедуры

&НаКлиенте
Асинх Процедура ПереместитьСКлиентаНаСервер()
	Ждать СкопироватьНаСервер();
	УдалитьНаКлиенте();
КонецПроцедуры
...
Функция СоздатьКаталогНаСервере(Знач ПолноеИмя)
...
Процедура СоздатьКаталогЗавершениеСозданияКаталога(ИмяКаталога, ДополнительныеПараметры) Экспорт
	...
	Разделитель = ?(ЭтоЛеваяПанель, РазделительПутиКлиент, РазделительПутиСервер);

	НуженРазделительВКонце = Не СтрЗаканчиваетсяНа(ИмяКаталога, Разделитель);
	Если НуженРазделительВКонце Тогда
		ИмяКаталога = ИмяКаталога + Разделитель;
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура РазвернутьФайлНаСервере(Знач АдресВХранилище, Знач КонечноеИмяФайла)
...
Асинх Процедура ОбновитьДеревоФайлов(НаКлиенте = Истина)
	Если НаКлиенте = Ложь Тогда
		...
		ТекущийКаталог = ТекущийКаталогСервер;
		ФайлыТекущегоКаталога = СодержимоеКаталогаНаСервере(ТекущийКаталог, РазделительПутиСервер, WindowsСервер);
	...
	Иначе
		...
		ТекущийКаталог = ТекущийКаталогКлиент;
		ФайлыТекущегоКаталога = Ждать СодержимоеКаталогаНаКлиенте(ТекущийКаталог);
...
Функция ПолноеИмя(Файл, ЭтоКаталог, РазделительПути, ЭтоWindows)
	КоличествоНачальныхСимволов = 2;
	ПолноеИмяФайла = Файл.ПолноеИмя + ?(ЭтоКаталог, РазделительПути, "");
	Если Не ЭтоWindows И Лев(Файл.ПолноеИмя, КоличествоНачальныхСимволов) = "//" Тогда
		ПолноеИмяФайла = Сред(Файл.ПолноеИмя, КоличествоНачальныхСимволов);
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоТекущийИлиРодительскийКаталог(ПолноеИмяФайла)
	Возврат ПолноеИмяФайла = "/./"
		Или ПолноеИмяФайла = "/../"
		Или ПолноеИмяФайла = "/."
		Или ПолноеИмяФайла = "/..";
...
&НаСервереБезКонтекста
Функция РазмерФайлаНаСервере(Файл)
	Попытка
...
&НаКлиенте
Асинх Функция РазмерФайлаНаКлиенте(Файл)
	Попытка
		РазмерФайла = Файл.РазмерАсинх();
	Исключение
		РазмерФайла = 0;
	КонецПопытки;
...
&НаКлиентеНаСервереБезКонтекста
Функция Килобайты(Байты)
	КБайт = 1000;
	Возврат Байты / КБайт;
КонецФункции

&НаСервереБезКонтекста
Функция ЭтоКаталогНаСервере(Файл)
	Возврат Файл.Существует() И Файл.ЭтоКаталог();
КонецФункции

&НаКлиенте
Асинх Функция ЭтоКаталогНаКлиенте(Файл)
	Возврат Ждать Файл.СуществуетАсинх() И Ждать Файл.ЭтоКаталогАсинх();
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИмяФайла(Файл, ПолноеИмяФайла, ЭтоКаталог, РазделительПути)
	...
	Если Не ЗначениеЗаполнено(ИмяФайла) Тогда
		...
		Если Не ЭтоКаталог И СтрНачинаетсяС(ИмяФайла, РазделительПути) Тогда
			КоличествоНачальныхСимволов = 2;
			ИмяФайла = Сред(ИмяФайла, КоличествоНачальныхСимволов);
		КонецЕсли;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ДатаИзмененияНаСервере(Файл)
	Попытка
...
&НаКлиенте
Асинх Функция ДатаИзмененияНаКлиенте(Файл)
	Попытка
		ДатаИзменения = Ждать Файл.ПолучитьВремяИзмененияАсинх();
	Исключение
		ДатаИзменения = '00010101';
	КонецПопытки;
...
&НаКлиентеНаСервереБезКонтекста
Функция СвойстваФайла(Файл, ПолноеИмяФайла, ЭтоКаталог, ДатаИзменения, РазмерФайлаВКБ, РазделительПути)
	ИмяФайла = ИмяФайла(Файл, ПолноеИмяФайла, ЭтоКаталог, РазделительПути);
	Расширение = ?(ЭтоКаталог, "", Файл.Расширение);
	ИндексКартинки = ИндексКартинки(Расширение, ЭтоКаталог);
	Представление = Формат(ДатаИзменения, "ДФ='yyyy-MM-dd HH:MM:ss'");

	СвойстваФайла = Новый Структура;
	СвойстваФайла.Вставить("ЭтоКаталог", ЭтоКаталог);
	СвойстваФайла.Вставить("ПолноеИмя", ПолноеИмяФайла);
	СвойстваФайла.Вставить("Имя", ИмяФайла);
	СвойстваФайла.Вставить("Расширение", Расширение);
	СвойстваФайла.Вставить("ИндексКартинки", ИндексКартинки);
	СвойстваФайла.Вставить("ДатаИзменения", ДатаИзменения);
	СвойстваФайла.Вставить("Представление", Представление);
	СвойстваФайла.Вставить("Размер", РазмерФайлаВКБ);

	Возврат СвойстваФайла;
КонецФункции

&НаСервереБезКонтекста
Функция СодержимоеКаталогаНаСервере(Знач Каталог, Знач РазделительПути, Знач ЭтоWindows)
	Результат = Новый Массив;

	Файлы = НайтиФайлы(Каталог, "*", Ложь);
	Для Каждого Файл Из Файлы Цикл
		ЭтоКаталог = ЭтоКаталогНаСервере(Файл);

		ПолноеИмяФайла = ПолноеИмя(Файл, ЭтоКаталог, РазделительПути, ЭтоWindows);
		Если ЭтоТекущийИлиРодительскийКаталог(ПолноеИмяФайла) Тогда
			Продолжить;
		КонецЕсли;

		ДатаИзменения = ДатаИзмененияНаСервере(Файл);
		РазмерФайлаВКБ = ?(ЭтоКаталог, Неопределено, Килобайты(РазмерФайлаНаСервере(Файл)));

		СвойстваФайла = СвойстваФайла(Файл, ПолноеИмяФайла, ЭтоКаталог, ДатаИзменения,
			РазмерФайлаВКБ, РазделительПути);

		Результат.Добавить(СвойстваФайла);
	КонецЦикла;
...
&НаКлиенте
Асинх Функция СодержимоеКаталогаНаКлиенте(Каталог)
	Результат = Новый Массив;

	Файлы = Ждать НайтиФайлыАсинх(Каталог, "*", Ложь);
	Для Каждого Файл Из Файлы Цикл
		ЭтоКаталог = Ждать ЭтоКаталогНаКлиенте(Файл);

		ПолноеИмяФайла = ПолноеИмя(Файл, ЭтоКаталог, РазделительПутиКлиент, WindowsКлиент);
		Если ЭтоТекущийИлиРодительскийКаталог(ПолноеИмяФайла) Тогда
			Продолжить;
		КонецЕсли;

		ДатаИзменения = Ждать ДатаИзмененияНаКлиенте(Файл);
		РазмерФайлаВКБ = ?(ЭтоКаталог, Неопределено, Килобайты(Ждать РазмерФайлаНаКлиенте(Файл)));

		СвойстваФайла = СвойстваФайла(Файл, ПолноеИмяФайла, ЭтоКаталог, ДатаИзменения,
			РазмерФайлаВКБ, РазделительПутиКлиент);
...
Функция ПолучитьСписокДисковWindowsНаСервере(Знач РазделительПути)
...
&НаСервереБезКонтекста
Функция ИмяКаталогаНаСервере(Знач ИмяКаталога)
...
&НаСервереБезКонтекста
Функция НайтиВсеФайлыНаСервере(Знач ТекущийКаталог, Знач РазделительПути, Знач УникальныйИдентификаторФормы)
	Результат = Новый Массив;
...
Процедура ПереименоватьФайлНаСервере(ИмяФайлаИсточника, ИмяФайлаПриемника, РазделительПути)
	...
	Иначе
		ПереименоватьКаталог(ИмяФайлаИсточника, ИмяФайлаПриемника, РазделительПути);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиентеНаСервереБезКонтекста
Процедура ПереименоватьКаталог(ИмяФайлаИсточника, ИмяФайлаПриемника, РазделительПути)
	ЭтоWindows = УИ_ОбщегоНазначенияКлиентСервер.ЭтоWindows();
	Если ЭтоWindows Тогда
		ПереименоватьКаталогНаWindows(ИмяФайлаИсточника, ИмяФайлаПриемника, РазделительПути);
	Иначе
		УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Переименование каталогов не реализовано.");
	КонецЕсли;
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ПереименоватьКаталогНаWindows(ИмяФайлаИсточника, ИмяФайлаПриемника, РазделительПути)
...
Процедура ПроверитьВозможностьЗапуска()
	Если Не УИ_ОбщегоНазначенияКлиентСервер.ВерсияПлатформыПоддерживаетАсинхронныеФункции() Тогда
		СообщениеОбОшибке = СтрШаблон("Для запуска ""%1"" требуется версия платформа не ниже %2.",
			ИмяОбработки(),
			УИ_ОбщегоНазначенияКлиентСервер.МинимальнаяВерсияПлатформыСПоддержкойАсинхронныхФункций()); 
...
Функция ИмяОбработки()
	Возврат РеквизитФормыВЗначение("Объект").Метаданные().Синоним;
КонецФункции
