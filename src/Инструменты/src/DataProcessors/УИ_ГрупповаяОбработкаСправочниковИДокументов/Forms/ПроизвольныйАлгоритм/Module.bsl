...
Функция ВыполнитьОбработку() Экспорт
	Если Открыта() Тогда
		ТекстПроизвольногоАлгоритма = УИ_РедакторКодаКлиент.ТекстКодаРедактора(ЭтотОбъект, "Редактор");
	КонецЕсли;
	...
	Для Индекс = 0 По НайденныеОбъекты.Количество() - 1 Цикл
		...
		Если СтрокаНайденныхОбъектов.Выбрать Тогда//
			...
			ОбработатьОбъект(СтрокаНайденныхОбъектов.Объект, Индекс, ТекстПроизвольногоАлгоритма);
		КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УИ_РедакторКодаСервер.СоздатьЭлементыРедактораКода(ЭтотОбъект, "Редактор", Элементы.ПолеПроизвольногоАлгоритма);
...
Процедура Подключаемый_РедакторКодаЗавершениеИнициализации() Экспорт
	УИ_РедакторКодаКлиент.УстановитьТекстРедактора(ЭтотОбъект, "Редактор", ТекстПроизвольногоАлгоритма, Истина);
	
	ДобавляемыйКонтекст = Новый Структура;
	Если НайденныеОбъекты.Количество()>0 Тогда
		ДобавляемыйКонтекст.Вставить("Объект", ТипЗнч(НайденныеОбъекты[0].Объект));
	Иначе
