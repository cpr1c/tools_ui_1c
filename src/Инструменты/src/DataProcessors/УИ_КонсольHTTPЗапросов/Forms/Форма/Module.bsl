...
Процедура ИмпортироватьДанныеCurl(СодержимоеИмпорта)
	...
	Для Каждого Куки Из РезультатОбработки.Куки Цикл
		ЗначенияКуки = Новый Массив;
		ЗначенияКуки.Добавить(Куки.Ключ);
		Если Куки.Значение <> Неопределено Тогда
			ЗначенияКуки.Добавить(Куки.Значение);
		КонецЕсли;
...
#КонецЕсли
	...
	УстановитьЗаголовокКукиВЗапросHTTP(Форма, СтруктураURL, Заголовки);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьЗаголовокКукиВЗапросHTTP(Форма, СтруктураURL, ЗаголовкиЗапроса)

	ШаблонКуки = "%1=%2";
	СписокКуки = Новый Массив;
	КукиЗапроса = КукиДляТекущегоЗапроса(Форма, СтруктураURL);
...
&НаКлиентеНаСервереБезКонтекста
Функция КукиДляТекущегоЗапроса(Форма, СтруктураURL)
...
Функция ИспользоватьКукиДляЗапросаHTTP(ОписаниеКуки, ДоменЗапроса, ПутьЗапроса, ТекущаяДата, ЭтоЗащищенноеСоединение)
	...
	ОграниченияКукиНарушены = ОписаниеКуки.ДействуетДо < ТекущаяДата
		Или Не ОписаниеКуки.ТолькоДляДомена И Не СтрЗаканчиваетсяНа(ДоменЗапроса, ДоменКуки) 
...
Процедура СгенерироватьКодИнициализацииЗаголовков(СтрокиДляОбъединения, СтрокаДереваЗапросов, СтруктураURL)
	...
	ШаблонКуки = "%1=%2";
	КукиЗапроса = КукиДляТекущегоЗапроса(ЭтотОбъект, СтруктураURL);
	СписокКуки = Новый Массив;
	Для Каждого ОписаниеКуки Из КукиЗапроса Цикл
		КлючКуки = ОписаниеКуки.Ключ;
		ЗначениеКуки = ОписаниеКуки.Значение;
