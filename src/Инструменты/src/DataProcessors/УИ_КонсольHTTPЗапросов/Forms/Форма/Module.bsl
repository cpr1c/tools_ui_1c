...
&НаКлиенте
Процедура ИспользоватьBOMПриИзменении(Элемент)
	ЗаполнитьСтруктуруJSONВДеревеЗапросов();
КонецПроцедуры
...
Процедура ДеревоЗапросовПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если Не ЗначениеЗаполнено(ТекДанные.Имя) Тогда
			ТекДанные.Имя = "Запрос"+Формат(ТекДанные.ПолучитьИдентификатор(), "ЧГ=0;");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьДоступностьТелаЗапроса(Форма)
	Форма.Элементы.ГруппаСтраницаЗапросаТело.Доступность = Не ЗапросБезТелаЗапроса(Форма.ЗапросHTTP);
КонецПроцедуры
...
Функция ПодготовленныйЗапросHTTP(СтруктураURL)
	...
	СтрокаПараметров = "";
	Для Каждого КлючЗначение Из СтруктураURL.ПараметрыURL Цикл
...
&НаКлиенте
Процедура ЗаполнитьСтруктуруJSONВДеревеЗапросов()
...
Процедура ПараметрыЗапросаПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	Если НоваяСтрока Тогда
		Элементы.ПараметрыURL.ТекущиеДанные.Использовать = Истина;
	КонецЕсли;
КонецПроцедуры
	...
	Если ПоложениеПараметров > 0 Тогда
		...
		Для Каждого стрПарам Из мПараметры Цикл
			...
			Если МассивИмяЗнач.Количество() = 2 Тогда
				НоваяСтрока = ПараметрыURL.Добавить();
...
//	СтруктураJSON.Вставить("ТаблицаЗаголовковЗапроса", СериализаторJSON.ЗначениеВСтруктуру(ТЗТаблицаЗаголовковЗапроса));
	...
	ТЗПараметры = ПараметрыURL.Выгрузить();
	СтруктураJSON.Вставить("ПараметрыURL", СериализаторJSON.ЗначениеВСтруктуру(ТЗПараметры));
...
Процедура ИзвлечьЗапросИзСтруктурыJSON(JSONСтрока)
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, СтруктураJSON, , "ПараметрыURL");
	
	ПараметрыURL.Очистить();
	
	Для Каждого стрПараметр Из СтруктураJSON.ПараметрыURL Цикл
		НоваяСтрока = ПараметрыURL.Добавить();
