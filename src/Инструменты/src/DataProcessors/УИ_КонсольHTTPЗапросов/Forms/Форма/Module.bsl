...
Процедура ТаблицаЗаголовковЗапросаКлючАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание,
	...
	Для Каждого СтрокаЗаголова Из ЗаголовкиЗапросаДляПоиска Цикл
		Для Каждого ЗначениеЗаголовка Из СтрокаЗаголова.ДанныеПоиска Цикл
			ЧастьЗаголовка = ЗначениеЗаголовка.Значение;
			Если СтрНачинаетсяС(ЧастьЗаголовка, ТекстПоиска) Тогда
				ДанныеВыбора.Добавить(СтрокаЗаголова.Имя);
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Процедура ЗаполнитьЗаголовкиЗапросаДляПоиска()
	...
	Для НомерСтроки = НачалоДанных По МакетЗаголовков.ВысотаТаблицы Цикл
		...
		Если ЗначениеЗаполнено(ИмяЗаголовка) Тогда
			НовыйЗаголовок = ЗаголовкиЗапросаДляПоиска.Добавить();
			НовыйЗаголовок.Имя = ИмяЗаголовка;
			НовыйЗаголовок.ИмяПоиска = НРег(ИмяЗаголовка);
			СписокДанных = Новый СписокЗначений;
			ЗначенияДанных = СтрРазделить(НовыйЗаголовок.ИмяПоиска, "-");
			СписокДанных.ЗагрузитьЗначения(ЗначенияДанных);
			НовыйЗаголовок.ДанныеПоиска = СписокДанных;
		КонецЕсли;
	КонецЦикла;
