...
Процедура ТаблицаЗаголовковЗапросаКлючАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание,
	...
	Иначе
		...
		Для Сч = 0 По ПоследнийЭлемент Цикл
			ОсновнаяСтрока = СтрокиПроверки[Сч].Значение;
			Если СтрокиУдаления[ОсновнаяСтрока] <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Для СчетчикУдаления = Сч По ПоследнийЭлемент Цикл
				СтрокаПроверки = СтрокиПроверки[СчетчикУдаления].Значение;
		...
		Для Каждого Элемент Из СтрокиПроверки Цикл
			Если СтрокиУдаления[Элемент.Значение] <> Неопределено Тогда
				СтрокиПоиска.Добавить(Элемент.Значение);
			КонецЕсли;
		КонецЦикла;
...
Процедура ДобавитьЗаголовокВПоиск(Форма, ИмяЗаголовка)
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		...
		Форма.Элементы.ТаблицаЗаголовковЗапросаКлюч.СписокВыбора.Добавить(ИмяЗаголовка);
	КонецЕсли;
...
Процедура ПоискЗаголовковПоЧастям(ДанныеВыбора, СтрокиПоиска, ЖирныйШрифт, ЗеленыйЦвет)
	...
	Для Каждого СтрокаЗаголова Из ЗаголовкиЗапросаДляПоиска Цикл
		СписокЧастей = Новый Массив;
		ПерваяСтрока = Истина;
		ЕстьСовпадение = Ложь;
		Для Сч = 0 По СтрокаЗаголова.ДанныеПоиска.Количество() - 1 Цикл
			ЧастьЗаголовка = СтрокаЗаголова.ДанныеПредставления[Сч].Значение;
			ЧастьЗаголовкаПоиска = СтрокаЗаголова.ДанныеПоиска[Сч].Значение;
			...
			Для Каждого ТекстПоиска Из СтрокиПоиска Цикл
		...
		Если ЕстьСовпадение Тогда
			...
			ДанныеВыбора.Добавить(СтрокаЗаголова.Имя, ПредставлениеЗаголовка);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПоискЗаголовковПоСтроке(ДанныеВыбора, СтрокаПоиска, ЖирныйШрифт, ЗеленыйЦвет)
	...
	Для Каждого СтрокаЗаголова Из ЗаголовкиЗапросаДляПоиска Цикл
		...
		СписокЧастей = Новый Массив();
		ПозицияПоиска = 1;
		НайденнаяПозиция = СтрНайти(СтрокаЗаголова.ИмяПоиска, СтрокаПоиска, , ПозицияПоиска);
		Пока НайденнаяПозиция > 0 Цикл
			ЕстьСовпадение = Истина;
			СтрокаБезВыделения = Сред(СтрокаЗаголова.Имя, ПозицияПоиска, НайденнаяПозиция - ПозицияПоиска);
			Если СтрокаБезВыделения <> "" Тогда
				СписокЧастей.Добавить(СтрокаБезВыделения);	
			КонецЕсли;
			ВыделенныйТекст = Сред(СтрокаЗаголова.Имя, НайденнаяПозиция, ДлинаСтрокиПоиска);
			ФорматированнаяСтрока = Новый ФорматированнаяСтрока(ВыделенныйТекст, ЖирныйШрифт, ЗеленыйЦвет);
			СписокЧастей.Добавить(ФорматированнаяСтрока);
			...
			НайденнаяПозиция = СтрНайти(СтрокаЗаголова.ИмяПоиска, СтрокаПоиска, , ПозицияПоиска);
		КонецЦикла;
		СтрокаБезВыделения = Сред(СтрокаЗаголова.Имя, ПозицияПоиска);
		Если СтрокаБезВыделения <> "" Тогда
			СписокЧастей.Добавить(СтрокаБезВыделения);	
		КонецЕсли;
		Если ЕстьСовпадение Тогда
