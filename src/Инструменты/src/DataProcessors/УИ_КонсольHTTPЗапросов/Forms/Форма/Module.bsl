...
Процедура ДеревоЗапросовПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		ИзвлечьЗапросИзСтруктурыJSON(ТекущиеДанные.СтруктураJSON);
	КонецЕсли;
КонецПроцедуры
...
//	КонецЕсли;
	...
	ТекущиеДанные = Элементы.ДеревоЗапросов.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда
		ТекущиеДанные.СтруктураJSON = УпаковатьЗапросВСтруктуруJSON();
	КонецЕсли;
КонецПроцедуры
...
Процедура ОткрытьФайлЗапросов(Команда)
	Если ИсторияЗапросов.Количество() = 0 И ДеревоЗапросов.ПолучитьЭлементы().Количество() = 0 Тогда
		ЗагрузитьФайлКонсоли();
	Иначе
		ПоказатьВопрос(Новый ОписаниеОповещения("ОткрытьФайлОтчетовЗавершение", ЭтотОбъект),
			"История или дерево запросов непустые. Продолжить?", РежимДиалогаВопрос.ДаНет, 15, КодВозвратаДиалога.Нет);
	КонецЕсли;
КонецПроцедуры
...
Процедура СохранитьДеревоЗапросов(Команда)
	УИ_ОбщегоНазначенияКлиент.СохранитьДанныеКонсолиВФайл("КонсольHTTPЗапросов", Ложь,
		СтруктураОписанияСохраняемогоФайла(), ПоместитьДанныеДереваЗапросовВоВременноеХранилище(),
		Новый ОписаниеОповещения("СохранениеВФайлЗавершение", ЭтотОбъект));
КонецПроцедуры
...
Процедура ОткрытьДеревоЗапросов(Команда)
	УИ_ОбщегоНазначенияКлиент.ПрочитатьДанныеКонсолиИзФайла("КонсольHTTPЗапросов",
		СтруктураОписанияСохраняемогоФайла(), Новый ОписаниеОповещения("ЗагрузитьФайлДереваЗапросовПослеПомещенияФайла",
		ЭтотОбъект));
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьФайлДереваЗапросовПослеПомещенияФайла(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбработкиЗагрузкиДереваЗапросовНаСервере(Адрес, ИмяФайлаЗапросов);

КонецПроцедуры

&НаСервере
Процедура ОбработкиЗагрузкиДереваЗапросовНаСервере(Адрес, ИмяФайлаЗапросов)
	ДеревоЗапросовЗначение = Обработки.УИ_КонсольHTTPЗапросов.ДанныеСохраненияИзСериализованнойСтроки(Адрес, ИмяФайлаЗапросов);
...
&НаСервере
Функция ПоместитьДанныеДереваЗапросовВоВременноеХранилище()
...
//	ТаблицаЗначенийИстории.Колонки.Удалить("АдресТелаОтветаСтрокой");
	...
	Результат = ПоместитьВоВременноеХранилище(ДеревоЗапросовЗначение, УникальныйИдентификатор);
	Возврат Результат;
...
&НаСервере
Функция УпаковатьЗапросВСтруктуруJSON()
	
	СериализаторJSON=Обработки.УИ_ПреобразованиеДанныхJSON.Создать();
	
	СтруктураJSON = Новый Структура;
	СтруктураJSON.Вставить("ЗапросHTTP", ЗапросHTTP);
	СтруктураJSON.Вставить("URLЗапроса", URLЗапроса);
	СтруктураJSON.Вставить("Таймаут", Таймаут);
	СтруктураJSON.Вставить("ЗаголовкиСтрока", ЗаголовкиСтрока);
	СтруктураJSON.Вставить("Таймаут", Таймаут);
		
//	ТЗТаблицаЗаголовковЗапроса = ТаблицаЗаголовковЗапроса.Выгрузить();
//	СтруктураJSON.Вставить("ТаблицаЗаголовковЗапроса", СериализаторJSON.ЗначениеВСтруктуру(ТЗТаблицаЗаголовковЗапроса));
	
	ТЗПараметры = ПараметрыЗапроса.Выгрузить();
	СтруктураJSON.Вставить("ПараметрыЗапроса", СериализаторJSON.ЗначениеВСтруктуру(ТЗПараметры));
	
	СтруктураJSON.Вставить("ТелоЗапроса", ТелоЗапроса);
	СтруктураJSON.Вставить("ТелоЗапросаДвоичныеДанныеСтрокой", ТелоЗапросаДвоичныеДанныеСтрокой);
	СтруктураJSON.Вставить("ИспользоватьBOM", ИспользоватьBOM);
	СтруктураJSON.Вставить("КодировкаТелаЗапроса", КодировкаТелаЗапроса);
	СтруктураJSON.Вставить("ВидТелаЗапроса", ВидТелаЗапроса);
	СтруктураJSON.Вставить("ИспользоватьПрокси", ИспользоватьПрокси);
	СтруктураJSON.Вставить("ПроксиПользователь", ПроксиПользователь);
	СтруктураJSON.Вставить("ПроксиПароль", ПроксиПароль);
	СтруктураJSON.Вставить("ПроксиПорт", ПроксиПорт);
	СтруктураJSON.Вставить("ПроксиАутентификацияОС", ПроксиАутентификацияОС);
	
	JSONСтрока = СериализаторJSON.ЗаписатьОписаниеОбъектаВJSON(СтруктураJSON);
...
&НаСервере
Процедура ИзвлечьЗапросИзСтруктурыJSON(JSONСтрока)
	...
	СтруктураJSONПред = СериализаторJSON.ПрочитатьОписаниеОбъектаИзJSON(JSONСтрока);
	СтруктураJSON = СериализаторJSON.ЗначениеИзСтруктуры(СтруктураJSONПред, Истина);
	
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, СтруктураJSON, , "ПараметрыЗапроса");
	
	ПараметрыЗапроса.Очистить();
	
	Для Каждого стрПараметр Из СтруктураJSON.ПараметрыЗапроса Цикл
		НоваяСтрока = ПараметрыЗапроса.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, стрПараметр);
	КонецЦикла;
	
//	ТаблицаЗаголовковЗапроса.Очистить();
//	Для Каждого стрПараметр Из СтруктураJSON.ТаблицаЗаголовковЗапроса Цикл
