...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УИ_ОбщегоНазначения.ФормаИнструментаПриСозданииНаСервере(ЭтотОбъект,
															 ...
															 КоманднаяПанель);
	ЗаполнитьЗаголовкиЗапросаДляПоиска();
...
Процедура ЗаголовкиСтрокаПриИзменении(Элемент)
	...
	ОбновитьЗаголовокВкладкиЗаголовкиЗапроса();
КонецПроцедуры
...
&НаКлиенте
Процедура ДеревоЗапросовАутентификацияИспользоватьПриИзменении(Элемент)
	ОбновитьЗаголовокВкладкиАутентификации();
КонецПроцедуры

&НаКлиенте
Процедура ДеревоЗапросовИспользоватьКукиПриИзменении(Элемент)
	ПриИзмененииИспользованияКукиЗапроса();
КонецПроцедуры
...
Процедура ДеревоЗапросовПриАктивизацииСтроки(Элемент)
	...
	Если ОтключитьАктивациюСтроки Тогда
		ОтключитьАктивациюСтроки = Ложь;
		Возврат;
	КонецЕсли;
...
Процедура ТаблицаЗаголовковЗапросаКлючАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание,
	...
	СтрокиПоиска = СтрокиПоискаЗаголовковЗапроса(Текст);
	ДанныеВыбора = ДанныеВыбораЗаголовковЗапроса(СтрокиПоиска);
	
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаЗаголовковЗапросаПослеУдаления(Элемент)
	ОбновитьЗаголовокВкладкиЗаголовкиЗапроса();
	ОтключитьАктивациюСтроки = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаЗаголовковЗапросаПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	Если ОтменаРедактирования Тогда
		Возврат;
	КонецЕсли;
	ОбновитьЗаголовокВкладкиЗаголовкиЗапроса();
...
&НаКлиенте
Процедура ПараметрыURLПослеУдаления(Элемент)
	ОбновитьЗаголовокВкладкиПараметрыURL();
КонецПроцедуры

&НаКлиенте
Процедура ПараметрыURLПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыДеревоЗапросовИсторияЗапросов

&НаКлиенте
Процедура ДеревоЗапросовИсторияЗапросовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	ОткрытьПодробнуюИнформациюПоТекущейИстории();
	СтандартнаяОбработка = Ложь;
КонецПроцедуры
...
&НаКлиенте
Процедура УправлениеКуки(Команда)

	СписокКуки = Новый Массив;
	Для Каждого СтрокаКуки Из Куки Цикл
		ОписаниеКуки = ОписаниеКуки();
		ЗаполнитьЗначенияСвойств(ОписаниеКуки, СтрокаКуки);
		СписокКуки.Добавить(ОписаниеКуки);
	КонецЦикла;
	ПараметрыОткрытия = Новый Структура("Куки", СписокКуки);
	Оповещение = Новый ОписаниеОповещения("УправлениеКукиЗавершение", ЭтотОбъект);
...
Функция ПараметрыВыбораНастроекИмпорта()
	...
	НовыйФормат.Представление = НСтр("ru = 'Insomnia (v4)'");
	...
	НовыйФормат.Представление = НСтр("ru = 'Postman. Коллекции (v2.1)'");
	НовыйФормат.ВариантСодержимого = ВариантыСодержимого.Файл;
	НовыйФормат.ФильтрФайловДляДиалога = "Файл JSON|*.json";
	ПараметрыИмпорта.Форматы.Добавить(НовыйФормат);
	...
	НовыйФормат.Представление = НСтр("ru = 'Postman. Environments'");
	НовыйФормат.ВариантСодержимого = ВариантыСодержимого.Файл;
	НовыйФормат.ФильтрФайловДляДиалога = "Файл JSON|*.json";
	ПараметрыИмпорта.Форматы.Добавить(НовыйФормат);
	...
	НовыйФормат.Представление = НСтр("ru = 'cURL'");
...
Процедура ИмпортироватьДанныеИмпортаНаСервере(ДанныеИмпорта)
	...
	ВариантыСодержимого = УИ_ОбщегоНазначенияКлиентСервер.ВариантыСодержимогоДляИмпорта();
	Если ДанныеИмпорта.Формат.ВариантСодержимого = ВариантыСодержимого.Файл Тогда
		СодержимоеИмпорта = СодержимоеФайлаИмпорта(ДанныеИмпорта);
		Если СодержимоеИмпорта = Неопределено Тогда
			Возврат;
		КонецЕсли;
	ИначеЕсли ДанныеИмпорта.Формат.ВариантСодержимого = ВариантыСодержимого.Строка Тогда
		СодержимоеИмпорта = ДанныеИмпорта.Содержимое;
	    Если ПустаяСтрока(СодержимоеИмпорта) Тогда
			Возврат;
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ru = 'Передан неподдерживаемый вариант содержимого для импорта.'");
	...
	Если ДанныеИмпорта.Формат.Идентификатор = "Инсомния4" Тогда
		ИмпортироватьДанныеИнсомнии4(СодержимоеИмпорта);
	ИначеЕсли ДанныеИмпорта.Формат.Идентификатор = "curl" Тогда 
		ИмпортироватьДанныеCurl(СодержимоеИмпорта);
	ИначеЕсли ДанныеИмпорта.Формат.Идентификатор = "ПостманКоллекции21" Тогда 
		ИмпортироватьДанныеPostmanКоллекции21(СодержимоеИмпорта);
	ИначеЕсли ДанныеИмпорта.Формат.Идентификатор = "ПостманПеременные" Тогда 
		ИмпортироватьДанныеPostmanПеременные(СодержимоеИмпорта);
	КонецЕсли;
...
Функция СодержимоеФайлаИмпорта(ДанныеИмпорта)
	...
	Если ТипЗнч(ДанныеФайла) <> Тип("ДвоичныеДанные") Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось прочитать файл импорта.'");
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Содержимое файла не является JSON:
									 |%1'"), ОписаниеОшибки);
		УИ_ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
		Возврат Неопределено;
	КонецПопытки;
...
&НаСервере
Процедура ИмпортироватьДанныеCurl(СодержимоеИмпорта)

	Если Не УИ_РаботаСCURLКлиентСервер.ЭтоКомандаCURL(СодержимоеИмпорта) Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Импортируемые данные не являются командой cURL.'"));
		УИ_ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
		Возврат;
	КонецЕсли;

	РезультатОбработки = УИ_РаботаСCURLКлиентСервер.ОбработатьКоманду(СодержимоеИмпорта);
	ВидыТела = ВидыТелаЗапроса();
	ВидыЭлементовМультипарт = ВидыЭлементовМультипарт();
	ВидыАутентификации = ВидыАутентификации();
	...
	ОбработатьОшибкиИмпортаCURL(РезультатОбработки);

	Если РезультатОбработки.Аутентификация <> Неопределено Тогда
		ТипыАутентификации = УИ_РаботаСCURLКлиентСервер.ТипыАутентификации();
		Если РезультатОбработки.Аутентификация.Тип = ТипыАутентификации.Базовый Тогда
			НоваяСтрокаДерева.АутентификацияВид = ВидыАутентификации.Базовая;
			НоваяСтрокаДерева.АутентификацияИспользовать = Истина;
			НоваяСтрокаДерева.АутентификацияПользователь = РезультатОбработки.Аутентификация.Логин;
			НоваяСтрокаДерева.АутентификацияПароль = РезультатОбработки.Аутентификация.Пароль;
		КонецЕсли;
	КонецЕсли;

	Для Каждого ЗаголовокЗапроса Из РезультатОбработки.Заголовки Цикл
		НоваяСтрокаЗаголовка = НоваяСтрокаДерева.Заголовки.Добавить();
		НоваяСтрокаЗаголовка.Ключ = ЗаголовокЗапроса.Имя;
		НоваяСтрокаЗаголовка.Значение = ЗаголовокЗапроса.Значение;
		НоваяСтрокаЗаголовка.Использование = Истина;
	КонецЦикла;

	СписокЗначенийКуки = Новый Массив;
	Для Каждого ОписаниеКуки Из РезультатОбработки.Куки Цикл
		ЗначенияКуки = Новый Массив;
		ЗначенияКуки.Добавить(ОписаниеКуки.Ключ);
		Если ОписаниеКуки.Значение <> Неопределено Тогда
			ЗначенияКуки.Добавить(ОписаниеКуки.Значение);
		КонецЕсли;
		ЗначениеКуки = СтрСоединить(ЗначенияКуки, "=");
		СписокЗначенийКуки.Добавить(ЗначениеКуки);
	КонецЦикла;
	Если СписокЗначенийКуки.Количество() > 0 Тогда
		НоваяСтрокаЗаголовка = НоваяСтрокаДерева.Заголовки.Добавить();
		НоваяСтрокаЗаголовка.Ключ = "Cookie";
		НоваяСтрокаЗаголовка.Значение = СтрСоединить(СписокЗначенийКуки, "; ");
		НоваяСтрокаЗаголовка.Использование = Истина;
	КонецЕсли;

	НоваяСтрокаДерева.ЗапросHTTP = ВРег(РезультатОбработки.Метод);
	
	Если РезультатОбработки.СсылкиЗапроса.Количество() = 1 Тогда
		НоваяСтрокаДерева.URLЗапроса = РезультатОбработки.СсылкиЗапроса[0];
	КонецЕсли;
	
	ОбработатьФормыCURL(НоваяСтрокаДерева, ВидыЭлементовМультипарт, РезультатОбработки, ВидыТела);
	
	Если НоваяСтрокаДерева.ВидТела = ВидыТела.БезТела И РезультатОбработки.ТелоЗапроса <> Неопределено Тогда
		НоваяСтрокаДерева.ВидТела = ВидыТела.Строка;
		НоваяСтрокаДерева.ТелоСтрокой = РезультатОбработки.ТелоЗапроса.Значение;					
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ОбработатьОшибкиИмпортаCURL(РезультатОбработки)
	
	Если РезультатОбработки.Ошибки.Количество() > 0 Тогда
		Ошибки = СтрСоединить(РезультатОбработки.Ошибки, Символы.ПС);
		ТекстОшибки = СтрШаблон(НСтр("ru = 'При импорте были обнаружены ошибки:
									 |%1'"), Ошибки);
		УИ_ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;

	Если РезультатОбработки.НеопознанныеАргументы.Количество() > 0 Тогда
		Ошибки = СтрСоединить(РезультатОбработки.НеопознанныеАргументы, "; ");
		ТекстОшибки = СтрШаблон(НСтр("ru = 'При импорте были обнаружены неопознанные аргументы:
									 |%1'"), Ошибки);
		УИ_ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;

	Если РезультатОбработки.НеопознанныеПараметрыВыполнения.Количество() > 0 Тогда
		Ошибки = СтрСоединить(РезультатОбработки.НеопознанныеПараметрыВыполнения, "; ");
		ТекстОшибки = СтрШаблон(НСтр("ru = 'При импорте были обнаружены неопознанные параметры выполнения:
									 |%1'"), Ошибки);
		УИ_ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
...
&НаСервере
Процедура ОбработатьФормыCURL(НоваяСтрокаДерева, ВидыЭлементовМультипарт, РезультатОбработки, ВидыТела)
	
	Для Каждого Форма Из РезультатОбработки.Формы Цикл
		НоваяСтрокаДерева.ВидТела = ВидыТела.МультипартФорм;
		НоваяСтрокаМультипарт = НоваяСтрокаДерева.ТелоМультипарт.Добавить();
		НоваяСтрокаМультипарт.Использование = Истина;
		НоваяСтрокаМультипарт.Имя = Форма.Имя;
		ЗначениеФормы = Форма.Значение;
		Если СтрНачинаетсяС(ЗначениеФормы, "@") Тогда
			НоваяСтрокаМультипарт.Вид = ВидыЭлементовМультипарт.Файл;
			НачалоЗначения = 2;
			НоваяСтрокаМультипарт.Значение = Сред(ЗначениеФормы, НачалоЗначения);
		Иначе
			НоваяСтрокаМультипарт.Вид = ВидыЭлементовМультипарт.Строка;
			НоваяСтрокаМультипарт.Значение = ЗначениеФормы;
		КонецЕсли;
	КонецЦикла;
...
Процедура ИмпортироватьДанныеPostmanКоллекции21(СодержимоеФайла)
	...
	Если Информация = Неопределено Тогда
		УИ_ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Неправильный формат файла.'"));
		Возврат;
	КонецЕсли;
	
	Если Информация["schema"] <> "https://schema.getpostman.com/json/collection/v2.1.0/collection.json" Тогда
		УИ_ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Неправильная версия формата файла.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьСохранениеЗапросовВФайл(СохранитьКак = Ложь, ОписаниеОповещенияОЗаверешении = Неопределено)
	...
	Оповещение = Новый ОписаниеОповещения("СохранениеВФайлЗавершение", ЭтотОбъект, ДопПараметрыОповещения);
...
Процедура ПриИзмененииВидаТелаЗапроса()
	...
	УстановитьЗаголовкиПоСодержимомуТелаЗапроса();
	ОбновитьЗаголовокВкладкиТелоЗапроса();
КонецПроцедуры
...
&НаКлиенте
Процедура ПриИзмененииИспользованияКукиЗапроса()
	
	СтрокаЗапроса = ТекущаяСтрокаЗапросов();	
	Если СтрокаЗапроса = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Элементы.ДеревоЗапросовИспользоватьКукиБезОграниченияСрокаДействия.ТолькоПросмотр =  Не СтрокаЗапроса.ИспользоватьКуки;
...
Процедура ЗаполнитьЗапросыИзФайла(ЗапросФайла, КоллекцияЭлементовЗапросов, ВерсияФормата)
	...
	ВерсияФормата3 = 3;
	Если ВерсияФормата >= ВерсияФормата3 Тогда
		НоваяСтрокаЗапросов.НаКлиенте = ЗапросФайла.НаКлиенте;
	КонецЕсли;

	ВерсияФормата4 = 4;
	Если ВерсияФормата >= ВерсияФормата4 Тогда
	...
	ВерсияФормата5 = 5;
	Если ВерсияФормата >= ВерсияФормата5 Тогда
		НоваяСтрокаЗапросов.ИспользоватьКуки = ЗапросФайла.ИспользоватьКуки;
		НоваяСтрокаЗапросов.ИспользоватьКукиБезОграниченияСрокаДействия = ЗапросФайла.ИспользоватьКукиБезОграниченияСрокаДействия;
	КонецЕсли;
	...
	Если ЗапросФайла.ТелоДвоичныеДанные <> Неопределено Тогда
		...
		Исключение
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Для запроса %1 не удалось прочитать двоичные данные тела запроса.'"),
...
Процедура ЗагрузитьФайлКонсолиНаСервере(Адрес)
	...
	Переменные.Очистить();
	ВерсияФормата4 = 4;
	Если СтруктураФайла.ВерсияФормата >= ВерсияФормата4 Тогда
		ЗагрузитьФайлКонсолиВерсииФормата4(СтруктураФайла);
	КонецЕсли;

	Куки.Очистить();
	ВерсияФормата5 = 5;
	Если СтруктураФайла.ВерсияФормата >= ВерсияФормата5 Тогда
		ЗагрузитьФайлКонсолиВерсииФормата5(СтруктураФайла);
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура ЗагрузитьФайлКонсолиВерсииФормата4(СтруктураФайла)
...
&НаСервере
Процедура ЗагрузитьФайлКонсолиВерсииФормата5(СтруктураФайла)

	Для Каждого Строка Из СтруктураФайла.Куки Цикл
		НоваяСтрока = Куки.Добавить();
		НоваяСтрока.Ключ = Строка.Ключ;
		НоваяСтрока.Значение = Строка.Значение;
		НоваяСтрока.Домен = Строка.Домен;
		НоваяСтрока.Путь = Строка.Путь;
		НоваяСтрока.ДействуетДо = ПрочитатьДатуJSON(Строка.ДействуетДо, ФорматДатыJSON.ISO);
		НоваяСтрока.ТолькоЗащищенноеСоединение = Строка.ТолькоЗащищенноеСоединение;
		НоваяСтрока.ТолькоHTTPЗапросы = Строка.ТолькоHTTPЗапросы;
		НоваяСтрока.ТолькоДляДомена = Строка.ТолькоДляДомена;
	КонецЦикла;
...
Процедура ЗагрузитьФайлКонсоли(БезВыбораФайла = Ложь)
	...
	Оповещение = Новый ОписаниеОповещения("ЗагрузитьФайлКонсолиПослеПомещенияФайла", ЭтотОбъект);
...
Функция ОписаниеЗапросаДляСохраненияВФайл(СтрокаДереваЗапросов)
	...
	ОписаниеЗапроса.Вставить("ИспользоватьКуки", СтрокаДереваЗапросов.ИспользоватьКуки);
	ОписаниеЗапроса.Вставить("ИспользоватьКукиБезОграниченияСрокаДействия", СтрокаДереваЗапросов.ИспользоватьКукиБезОграниченияСрокаДействия);
...
Функция ОписаниеКуки()
	...
	Результат.Вставить("ТолькоЗащищенноеСоединение", Ложь);
	Результат.Вставить("ТолькоHTTPЗапросы", Ложь);
	Результат.Вставить("ТолькоДляДомена", Ложь);
	Возврат Результат;
...
Функция ПолучитьСтрокуДанныхФайлаДляСохраненияВФайл()
	...
	СохраняемыеДанные.Вставить("ВерсияФормата", 5);
	...
	СохраняемыеДанные.Вставить("Куки", Новый Массив);
	...
	Для Каждого СтрокаКуки Из Куки Цикл
		ОписаниеКуки = ОписаниеКуки();
		ЗаполнитьЗначенияСвойств(ОписаниеКуки, СтрокаКуки);
		СохраняемыеДанные.Куки.Добавить(ОписаниеКуки);
	КонецЦикла;
...
Процедура ОбработчикВыполненияЗапроса() 
	...
	КонецЕсли;
	ОбновитьЗаголовокВкладкиЗаголовкиОтвета();
	ОбновитьЗаголовокВкладкиИстория();
...
Процедура ВыполнитьЗапросЗавершениеЧтенияФайлаВоВременноеХранилище(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		УИ_ОбщегоНазначенияКлиент.СообщитьПользователю("Не удалось поместить файл тела во временное хранилище");
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьЗапросЗавершениеЧтенияФайловМультипартВоВременноеХранилище(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		УИ_ОбщегоНазначенияКлиент.СообщитьПользователю("Не удалось поместить файлы тела во временное хранилище");
		Возврат;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ПараметрыФиксацииЛогЗапроса()
	
	Результат = Новый Структура;
	Результат.Вставить("URLДляИсполнения", Неопределено);
	Результат.Вставить("АдресСервера", Неопределено);
	Результат.Вставить("Протокол", Неопределено);
	Результат.Вставить("HTTPЗапрос", Неопределено);
	Результат.Вставить("HTTPОтвет", Неопределено);
	Результат.Вставить("ДатаНачала", Неопределено);
	Результат.Вставить("Длительность", Неопределено);
	Результат.Вставить("ДанныеФайлаТела", Неопределено);
	Результат.Вставить("ДанныеКуки", Неопределено);
	Возврат Результат;
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Процедура ЗафиксироватьЛогЗапроса(Форма, СтрокаДереваЗапросов, ПараметрыФиксацииЛогЗапроса)

	URLДляИсполнения = ПараметрыФиксацииЛогЗапроса.URLДляИсполнения;
	АдресСервера = ПараметрыФиксацииЛогЗапроса.АдресСервера;
	Протокол = ПараметрыФиксацииЛогЗапроса.Протокол;
	HTTPЗапрос = ПараметрыФиксацииЛогЗапроса.HTTPЗапрос;
	HTTPОтвет = ПараметрыФиксацииЛогЗапроса.HTTPОтвет;
	ДатаНачала = ПараметрыФиксацииЛогЗапроса.ДатаНачала;
	Длительность = ПараметрыФиксацииЛогЗапроса.Длительность;
	ДанныеФайлаТела = ПараметрыФиксацииЛогЗапроса.ДанныеФайлаТела;
	ДанныеКуки = ПараметрыФиксацииЛогЗапроса.ДанныеКуки;
	...
	ЗаполнитьЛогТелаЗапроса(ЗаписьЛога, СтрокаДереваЗапросов, ДанныеФайлаТела, HTTPЗапрос, Форма);
	...
	Для Каждого ОписаниеКуки Из ДанныеКуки Цикл
		СтрокаКуки = ЗаписьЛога.Куки.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаКуки, ОписаниеКуки);
	КонецЦикла;

	ЗаполнитьЛогТелаОтвета(Форма, HTTPОтвет, ЗаписьЛога);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьЛогТелаОтвета(Форма, HTTPОтвет, ЗаписьЛога)
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьЛогТелаЗапроса(ЗаписьЛога, СтрокаДереваЗапросов, ДанныеФайлаТела, HTTPЗапрос, Форма)
...
&НаКлиентеНаСервереБезКонтекста
Функция ДанныеКукиHTTPОтвета(Ответ, АдресСервера)

	КукиРезультата = Новый Массив;
	ЗначениеЗаголовка = ЗначениеЗаголовка(Ответ.Заголовки, "set-cookie");
	Если ЗначениеЗаголовка = Неопределено Тогда
		Возврат КукиРезультата;
	КонецЕсли;

	ПараметрыПоискаСовпадения = УИ_РегулярныеВыраженияКлиентСервер.ПараметрыПоискаСовпадения();
	ПараметрыПоискаСовпадения.ВсеСовпадения = Истина;
	ПараметрыПоискаСовпадения.ИерархическийОбходРезультатов = Истина;
	
	ПоискКуки = "(?:^|,\s*)([!#$%&'*+\-.^_`|~0-9A-Za-z]+=[\s\S]*?)(?=\s*,\s*[!#$%&'*+\-.^_`|~0-9A-Za-z]+=|$)";
	РезультатПоиска = УИ_РегулярныеВыраженияКлиентСервер.НайтиСовпадения(ЗначениеЗаголовка,
																   ПоискКуки,
																   ПараметрыПоискаСовпадения);

	Для Каждого КлючЗначение Из РезультатПоиска Цикл
		Для Каждого ОписаниеКуки Из КлючЗначение.Значение Цикл

			ПоискПараметров = "(?:^|;\s*)(?<key>[!#$%&'*+\-.^_`|~0-9A-Za-z]+)(?:=(?<value>[^;]*))?";
			РезультатПараметров = УИ_РегулярныеВыраженияКлиентСервер.НайтиСовпадения(ОписаниеКуки,
																					 ПоискПараметров,
																					 ПараметрыПоискаСовпадения);
			...
			Если РезультатПараметров.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
			ОбработатьПараметрыКуки(КукиРезультата, АдресСервера, РезультатПараметров);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбработатьПараметрыКуки(КукиРезультата, Знач АдресСервера, ПараметрыКуки)

	АдресСервера = ДобавитьЛидирующуюТочку(АдресСервера);
	ТекущаяДата = ТекущаяУниверсальнаяДатаНаКлиентеНаСервере();
	ДействуетДоИзДлительностиЖизни = Дата(1, 1, 1);
	ОписаниеКуки = ОписаниеКуки();
	ПервыйПараметр = Истина;
	Для Каждого ОписаниеПараметра Из ПараметрыКуки Цикл
		ПараметрыКуки = ОписаниеПараметра.Значение;
		ИмяПараметра = ПараметрыКуки[0];
		ЗначениеПараметра = ПараметрыКуки[1];
		ОбработатьПараметрКуки(ОписаниеКуки, ИмяПараметра, ЗначениеПараметра, ПервыйПараметр, ТекущаяДата);
		
		// У параметра Max-Age есть приоритет над Expires, поэтому перезаписываем эту дату.
		Если СтрСравнить(ИмяПараметра, "Max-Age") = 0 Тогда
			ДействуетДоИзДлительностиЖизни = ОписаниеКуки.ДействуетДо;
		КонецЕсли;
	КонецЦикла;
	Если ПустаяСтрока(ОписаниеКуки.Домен) Тогда
		ОписаниеКуки.Домен = АдресСервера;
		ОписаниеКуки.ТолькоДляДомена = Истина;
	КонецЕсли;
	Если ЗначениеЗаполнено(ДействуетДоИзДлительностиЖизни) Тогда
		ОписаниеКуки.ДействуетДо = ДействуетДоИзДлительностиЖизни;
	КонецЕсли;
	КукиРезультата.Добавить(ОписаниеКуки);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ОбработатьПараметрКуки(СтрокаКуки, ИмяПараметра, ЗначениеПараметра, ПервыйПараметр, ТекущаяДата)

	Если ПервыйПараметр Тогда
		ПервыйПараметр = Ложь;
		СтрокаКуки.Ключ = ИмяПараметра;
		СтрокаКуки.Значение = ЗначениеПараметра;
	ИначеЕсли СтрСравнить(ИмяПараметра, "Path") = 0 Тогда
		СтрокаКуки.Путь = ЗначениеПараметра;
	ИначеЕсли СтрСравнить(ИмяПараметра, "Domain") = 0 Тогда
		СтрокаКуки.Домен = ДобавитьЛидирующуюТочку(ЗначениеПараметра);
	ИначеЕсли СтрСравнить(ИмяПараметра, "Expires") = 0 Тогда
		СтрокаКуки.ДействуетДо = ДатаИзHTTPЗаголовка(ЗначениеПараметра);
	ИначеЕсли СтрСравнить(ИмяПараметра, "Max-Age") = 0 Тогда
		СрокЖизниКуки = УИ_СтроковыеФункцииКлиентСервер.СтрокаВЧисло(ЗначениеПараметра);
		СтрокаКуки.ДействуетДо = ТекущаяДата + СрокЖизниКуки;
	ИначеЕсли СтрСравнить(ИмяПараметра, "Secure") = 0 Тогда
		СтрокаКуки.ТолькоЗащищенноеСоединение = Истина;
	ИначеЕсли СтрСравнить(ИмяПараметра, "HttpOnly") = 0 Тогда
		СтрокаКуки.ТолькоHTTPЗапросы = Истина;
	Иначе
		Возврат;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоДоменВерхнегоУровня(Знач Домен)

	ВтороеВхождение = 2;
	Возврат (СтрНайти(Домен, ".", , , ВтороеВхождение) = 0);
...
// Преобразует строковое представление даты из HTTP-заголовка в значение типа Дата
//
// Параметры:
//   ДатаСтрокой - Строка - строковое представление даты в формате RFC7231
//
// Возвращаемое значение:
//   Дата - значение даты, полученное из строки
&НаКлиентеНаСервереБезКонтекста
Функция ДатаИзHTTPЗаголовка(ДатаСтрокой)
	
	Возврат УИ_КоннекторHTTP.ДатаИзСтрокиRFC7231(ДатаСтрокой);
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ТекущаяУниверсальнаяДатаНаКлиентеНаСервере()
...
&НаКлиентеНаСервереБезКонтекста
Функция ЗначениеЗаголовка(ЗаголовкиПоиска, ИмяЗаголовка)
	
	Результат = Неопределено;
	Для Каждого Заголовок Из ЗаголовкиПоиска Цикл
		Если СтрСравнить(Заголовок.Ключ, ИмяЗаголовка) = 0 Тогда
			Результат = Заголовок.Значение;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Возврат Результат;
...
#Если ВебКлиент Тогда
			...
			Если СтрокаДереваЗапросов.ИспользоватьBOM <> 0 Тогда
				УИ_ОбщегоНазначенияКлиент.СообщитьПользователю("Настройка использования BOM игнорируется на веб клиенте");
			КонецЕсли;
#Иначе
				Если СтрокаДереваЗапросов.ИспользоватьBOM = 0 Тогда
...
#Если ВебКлиент Тогда
			Если Не БезКодировкиТекста Тогда
				...
				УИ_ОбщегоНазначенияКлиент.СообщитьПользователю("При запросе в вебклиенте тело устанавливается в кодровке UTF-8 и настройка кодировки игнорируется");
			КонецЕсли;
...
#КонецЕсли
	...
	УстановитьЗаголовокКукиВЗапросHTTP(Форма, СтрокаДереваЗапросов, СтруктураURL, Заголовки);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьЗаголовокКукиВЗапросHTTP(Форма, СтрокаДереваЗапросов, СтруктураURL, ЗаголовкиЗапроса)

	Если Не СтрокаДереваЗапросов.ИспользоватьКуки Тогда
		Возврат;
	КонецЕсли;
	
	ШаблонКуки = "%1=%2";
	СписокКуки = Новый Массив;
	КукиЗапроса = КукиДляТекущегоЗапроса(Форма,
										 СтруктураURL,
										 СтрокаДереваЗапросов.ИспользоватьКукиБезОграниченияСрокаДействия);
	Для Каждого ОписаниеКуки Из КукиЗапроса Цикл
		КлючКуки = ОписаниеКуки.Ключ;
		ЗначениеКуки = ОписаниеКуки.Значение;
		ПредставлениеКуки = СтрШаблон(ШаблонКуки, КлючКуки, ЗначениеКуки);
		СписокКуки.Добавить(ПредставлениеКуки);
	КонецЦикла;

	Если СписокКуки.Количество() > 0 Тогда
		ЗаголовкиЗапроса.Вставить("Cookie", СтрСоединить(СписокКуки, "; "));
	КонецЕсли;

КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция КукиДляТекущегоЗапроса(Форма, СтруктураURL, ИспользоватьКукиБезОграниченияСрокаДействия)
	
	ЭтоЗащищенноеСоединение = (СтрСравнить(СтруктураURL.Схема, "https") = 0);
	ДоменЗапроса  = ДобавитьЛидирующуюТочку(СтруктураURL.Сервер);
	ПутьЗапроса = ?(СтрДлина(СтруктураURL.Путь) > 1 И Не СтрЗаканчиваетсяНа(СтруктураURL.Путь, "/"),
					НРег(СтрШаблон("%1/", СтруктураURL.Путь)),
					НРег(СтруктураURL.Путь));
	ТекущаяДата = ТекущаяУниверсальнаяДатаНаКлиентеНаСервере();
	СписокКуки = Новый Массив;
	Для Каждого ОписаниеКуки Из Форма.Куки Цикл
		Если ИспользоватьКукиДляЗапросаHTTP(ОписаниеКуки,
											ДоменЗапроса,
											ПутьЗапроса,
											ТекущаяДата,
											ЭтоЗащищенноеСоединение,
											ИспользоватьКукиБезОграниченияСрокаДействия) Тогда
			СписокКуки.Добавить(ОписаниеКуки);
		КонецЕсли;
	КонецЦикла;
	Возврат СписокКуки;
...
&НаКлиентеНаСервереБезКонтекста
Функция ИспользоватьКукиДляЗапросаHTTP(ОписаниеКуки, ДоменЗапроса, ПутьЗапроса, ТекущаяДата, ЭтоЗащищенноеСоединение,
	ИспользоватьКукиБезОграниченияСрокаДействия)
	
	Результат = Истина;
	
	ДоменКуки = НРег(ОписаниеКуки.Домен);
	ПутьКуки = НРег(ОписаниеКуки.Путь);
	ОграниченияКукиНарушены = ЗначениеЗаполнено(ОписаниеКуки.ДействуетДо) И ОписаниеКуки.ДействуетДо < ТекущаяДата
		Или Не ЗначениеЗаполнено(ОписаниеКуки.ДействуетДо) И Не ИспользоватьКукиБезОграниченияСрокаДействия
		Или ПустаяСтрока(ДоменКуки)
		Или Не ОписаниеКуки.ТолькоДляДомена И ЗначениеЗаполнено(ДоменКуки) И Не СтрЗаканчиваетсяНа(ДоменЗапроса, ДоменКуки) 
		Или ОписаниеКуки.ТолькоДляДомена И СтрСравнить(ДоменЗапроса, ДоменКуки) <> 0
		Или Не СтрНачинаетсяС(ПутьЗапроса, ПутьКуки)
		Или ОписаниеКуки.ТолькоЗащищенноеСоединение И Не ЭтоЗащищенноеСоединение;
	Если ОграниченияКукиНарушены Тогда
		Результат = Ложь;
	КонецЕсли;
...
Процедура УстановитьЗаголовокАутентификацииВЗапросHTTP(Форма, СтрокаДереваЗапросов, ЗаголовкиЗапроса)
	...
	Если СтрокаДереваЗапросов.АутентификацияВид = Виды.Базовая Тогда
		...
		ИмяПользователя = СтрокаСУстановленнымиЗначениямиПеременных(СтрокаДереваЗапросов.АутентификацияПользователь,
																	Форма.Переменные);
		Пароль = СтрокаСУстановленнымиЗначениямиПеременных(СтрокаДереваЗапросов.АутентификацияПароль, Форма.Переменные);
		СтрокаДляПреобразованияВBase64 = СтрШаблон("%1:%2", ИмяПользователя, Пароль);

		ДвоичныеДанные = ПолучитьДвоичныеДанныеИзСтроки(СтрокаДляПреобразованияВBase64);
		СтрокаBase64 =  ПолучитьBase64СтрокуИзДвоичныхДанных(ДвоичныеДанные);
		СтрокаBase64БезПереносовСтрок = СтрЗаменить(СтрокаBase64, Символы.ВК + Символы.ПС, "");
		ЗначениеЗаголовка = СтрШаблон("%1 %2", Префикс, СтрокаBase64БезПереносовСтрок);
...
Процедура ПослеВыполненияЗапросаНаКлиентеНаСервере(ДополнительныеПараметры)
	...
	АдресСервера = ДополнительныеПараметры.СтруктураURL.Сервер;
	ДанныеКуки = ДанныеКукиHTTPОтвета(ДополнительныеПараметры.Ответ, АдресСервера);

	ПараметрыФиксацииЛогЗапроса = ПараметрыФиксацииЛогЗапроса();
	ПараметрыФиксацииЛогЗапроса.URLДляИсполнения = ДополнительныеПараметры.URLДляИсполнения;
	ПараметрыФиксацииЛогЗапроса.АдресСервера = АдресСервера;
	ПараметрыФиксацииЛогЗапроса.Протокол = ДополнительныеПараметры.СтруктураURL.Схема;
	ПараметрыФиксацииЛогЗапроса.HTTPЗапрос = ДополнительныеПараметры.Запрос;
	ПараметрыФиксацииЛогЗапроса.HTTPОтвет = ДополнительныеПараметры.Ответ;
	ПараметрыФиксацииЛогЗапроса.ДатаНачала = ДополнительныеПараметры.ДатаНачала;
	ПараметрыФиксацииЛогЗапроса.Длительность = ДлительностьВМилисекундах;
	ПараметрыФиксацииЛогЗапроса.ДанныеФайлаТела = ДополнительныеПараметры.ДанныеФайлаТела;
	ПараметрыФиксацииЛогЗапроса.ДанныеКуки = ДанныеКуки;
	
	ЗафиксироватьЛогЗапроса(ДополнительныеПараметры.Форма,
							ДополнительныеПараметры.СтрокаДерева,
							ПараметрыФиксацииЛогЗапроса);
	...
	Если ДополнительныеПараметры.СтрокаДерева.ИспользоватьКуки Тогда
		ДополнитьХранилищеКуки(ДополнительныеПараметры.Форма, ДанныеКуки, АдресСервера);
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ДополнитьХранилищеКуки(Форма, НовыеКуки, Знач АдресСервера)

	АдресСервера = ДобавитьЛидирующуюТочку(АдресСервера);
	Для Каждого ОписаниеКуки Из НовыеКуки Цикл
		// Куки не должны сохраняться, если передаются для чужих доменов или регистрируются для верхнего уровня доменов
		ПропуститьКуки = (Не СтрЗаканчиваетсяНа(АдресСервера, ОписаниеКуки.Домен)
						 Или ЭтоДоменВерхнегоУровня(ОписаниеКуки.Домен));
		Если ПропуститьКуки Тогда
			Продолжить;
		КонецЕсли;
		ПоискКуки = Новый Структура("Ключ, Домен, Путь", ОписаниеКуки.Ключ, ОписаниеКуки.Домен, ОписаниеКуки.Путь);
		НайденныеКуки = Форма.Куки.НайтиСтроки(ПоискКуки);
		Если НайденныеКуки.Количество() > 0 Тогда
			СтрокаКуки = НайденныеКуки[НайденныеКуки.ВГраница()];
		Иначе
			СтрокаКуки = Форма.Куки.Добавить();
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(СтрокаКуки, ОписаниеКуки);
...
Процедура ПослеВыполненияЗапросаНаКлиенте(ОтветСервера, ДополнительныеПараметры) Экспорт
	Если ТипЗнч(ОтветСервера) = Тип("ИнформацияОбОшибке") Тогда
		УИ_ОбщегоНазначенияКлиент.СообщитьПользователю(ОтветСервера.Описание);
		Ответ = Неопределено;
	Иначе
		Ответ = ОтветСервера;
	КонецЕсли;
...
Процедура ВыполнитьЗапросСЗадержкойЗавершениеВводаЗадержки(Число, ДополнительныеПараметры) Экспорт
	...
	Если Число<=0 Тогда
		УИ_ОбщегоНазначенияКлиент.СообщитьПользователю("Задержка должна быть положительным числом");
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьСПериодичностьюЗавершениеВводаПериодичности(Число, ДополнительныеПараметры) Экспорт
	...
	Если Число <= 0 Тогда
		УИ_ОбщегоНазначенияКлиент.СообщитьПользователю("Периодичность выполнения запроса должна быть положительным числом");
		Возврат;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция СтрокаВСтрокуКодаСПеременными(СтрокаДляУстановки, ЗначенияПеременных)
	СтрокаСоЗначениемПеременных = СтрокаСУстановленнымиЗначениямиПеременных(СтрокаДляУстановки, ЗначенияПеременных);
	Возврат УИ_ГенераторКода.СтрокаВСтрокуКода(СтрокаСоЗначениемПеременных);
КонецФункции
...
Функция ВозможноИсполнениеЗапроса(СтрокаЗапросов)
	...
	Если Не УИ_ОбщегоНазначенияКлиентСервер.ВерсияПлатформыНеМладше("8.3.21") Тогда
		УИ_ОбщегоНазначенияКлиент.СообщитьПользователю("Запросы в контексте веб-клиента доступны, начиная с версии платформы 8.3.21");
		Результат = Ложь;
	КонецЕсли;

	Если СтрокаЗапросов.ИспользоватьПрокси Тогда
		УИ_ОбщегоНазначенияКлиент.СообщитьПользователю("В веб клиенте не поддерживается использование прокси в HTTP запросах");
		Результат = Ложь;
	КонецЕсли;
...
Процедура ИнициализироватьФорму()
	...
	Элементы.ДеревоЗапросовАутентификацияВид.СписокВыбора.Добавить(ВидыАутентификации.BearerToken, "Bearer token (JWT)");
	Если УИ_ОбщегоНазначенияКлиентСервер.ВерсияПлатформыНеМладше("8.3.7") Тогда
...
Процедура ИнициализироватьСтрокуДереваЗапросов(СтрокаДереваЗапросов, Форма)
	...
	СтрокаДереваЗапросов.ИспользоватьКуки = Истина;
	СтрокаДереваЗапросов.ИспользоватьКукиБезОграниченияСрокаДействия = Истина;
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если ИмяФайлаЗапросов <> "" Тогда
		Заголовок = СтрШаблон(НСтр("ru = '%1: %2'"), Заголовок, ИмяФайлаЗапросов);
	КонецЕсли;
...
&НаКлиенте
Процедура ОткрытьПодробнуюИнформациюПоТекущейИстории()
	...
	СтрокаЗапроса = СтрокаЗапроса.ПолучитьИдентификатор();
	СтрокаИстории = ТекДанные.ПолучитьИдентификатор();
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("СтрокаЗапроса", СтрокаЗапроса);
	ПараметрыФормы.Вставить("СтрокаИстории", СтрокаИстории);
	
	УникальностьФормы = СтрШаблон("%1%2%3", УникальныйИдентификатор, СтрокаЗапроса, СтрокаИстории);
...
Функция СгенерированныйКодИнициализацииHTTPСоединения(СтрокаДереваЗапросов, СтруктураURL)
	...
	Иначе
		ТекстНастройкаПрокси = СтрШаблон("НастройкаПрокси = %1;", УИ_ГенераторКода.НеопределеноВСтрокуКода());
	КонецЕсли;
...
Функция СгенерированныйКодИнициализацииHTTPЗапроса(СтрокаДереваЗапросов, СтруктураURL)
	...
	АдресРесурса = УИ_ГенераторКода.СтрокаВСтрокуКода(УИ_КоннекторHTTP.УИ_СобратьАдресРесурса(СтруктураURL));
	СтрокиДляОбъединения.Добавить(СтрШаблон("НовыйЗапрос = Новый HTTPЗапрос;
											|НовыйЗапрос.АдресРесурса = %1;
											|",
											АдресРесурса));

	СгенерироватьКодИнициализацииЗаголовков(СтрокиДляОбъединения, СтрокаДереваЗапросов, СтруктураURL);

	СгенерироватьКодИнициализацииТелаЗапроса(СтрокиДляОбъединения, СтрокаДереваЗапросов);
...
&НаСервере
Процедура СгенерироватьКодИнициализацииЗаголовков(СтрокиДляОбъединения, СтрокаДереваЗапросов, СтруктураURL)
	...
	Для Каждого СтрокаЗаголовка Из СтрокаДереваЗапросов.Заголовки Цикл
		...
		КлючЗаголовка = СтрокаВСтрокуКодаСПеременными(СтрокаЗаголовка.Ключ, Переменные);
		ЗначениеЗаголовка = СтрокаВСтрокуКодаСПеременными(СтрокаЗаголовка.Значение, Переменные);
		НовыйЗаголовок = СтрШаблон("НовыйЗапрос.Заголовки.Вставить(%1, %2);", КлючЗаголовка, ЗначениеЗаголовка);
		СтрокиДляОбъединения.Добавить(НовыйЗаголовок);
	КонецЦикла;

	Если СтрокаДереваЗапросов.ИспользоватьКуки Тогда
		ШаблонКуки = "%1=%2";
		КукиЗапроса = КукиДляТекущегоЗапроса(ЭтотОбъект,
											 СтруктураURL,
											 СтрокаДереваЗапросов.ИспользоватьКукиБезОграниченияСрокаДействия);
		СписокКуки = Новый Массив;
		Для Каждого ОписаниеКуки Из КукиЗапроса Цикл
		...
		Если СписокКуки.Количество() > 0 Тогда
			ПредставлениеКуки = СтрСоединить(СписокКуки, "; ");
			ПредставлениеКукиСтрокой = УИ_ГенераторКода.СтрокаВСтрокуКода(ПредставлениеКуки);
			НовыйЗаголовок = СтрШаблон("НовыйЗапрос.Заголовки.Вставить(""Cookie"", %1);", ПредставлениеКукиСтрокой);
			СтрокиДляОбъединения.Добавить(НовыйЗаголовок);
		КонецЕсли;
	КонецЕсли;
	...
	Если СтрокаДереваЗапросов.АутентификацияИспользовать
		...
		Если СтрокаДереваЗапросов.АутентификацияВид = ВидыАутентификации.Базовая Тогда
			АутентификацияПользователь = СтрокаВСтрокуКодаСПеременными(СтрокаДереваЗапросов.АутентификацияПользователь,
																  Переменные);
			АутентификацияПароль = СтрокаВСтрокуКодаСПеременными(СтрокаДереваЗапросов.АутентификацияПароль, Переменные);
			СтрокиДляОбъединения.Добавить(СтрШаблон("
													...
													АутентификацияПользователь,
													АутентификацияПароль));
		ИначеЕсли СтрокаДереваЗапросов.АутентификацияВид = ВидыАутентификации.BearerToken Тогда

			Префикс = СокрЛП(СтрокаВСтрокуКодаСПеременными(СтрокаДереваЗапросов.АутентификацияПрефиксТокена,
														   Переменные));
			Если Не ЗначениеЗаполнено(Префикс) Тогда
				Префикс = "Bearer";
			КонецЕсли;
			ПрефиксСтрокой = УИ_ГенераторКода.СтрокаВСтрокуКода(Префикс);
			АутентификацияПарольСтрокой = СтрокаВСтрокуКодаСПеременными(СтрокаДереваЗапросов.АутентификацияПароль,
																		Переменные);
			СтрокиДляОбъединения.Добавить(СтрШаблон("
...
&НаСервере
Процедура СгенерироватьКодИнициализацииТелаЗапроса(СтрокиДляОбъединения, СтрокаДереваЗапросов)
	...
	ИначеЕсли СтрокаДереваЗапросов.ВидТела = ВидыТелаЗапроса.Строка Тогда
		СгенерироватьКодИнициализацииВидТелаСтрока(СтрокиДляОбъединения, СтрокаДереваЗапросов);
	ИначеЕсли СтрокаДереваЗапросов.ВидТела = ВидыТелаЗапроса.ДвоичныеДанные Тогда
		СгенерироватьКодИнициализацииВидТелаДвочныеДанные(СтрокиДляОбъединения, СтрокаДереваЗапросов);
	ИначеЕсли СтрокаДереваЗапросов.ВидТела = ВидыТелаЗапроса.Файл Тогда
		ИмяФайлаТела = СтрокаВСтрокуКодаСПеременными(СтрокаДереваЗапросов.ИмяФайлаТела, Переменные);
	...
	ИначеЕсли СтрокаДереваЗапросов.ВидТела = ВидыТелаЗапроса.МультипартФорм Тогда
		СгенерироватьКодИнициализацииВидТелаМультипартФорм(СтрокиДляОбъединения, СтрокаДереваЗапросов);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура СгенерироватьКодИнициализацииВидТелаМультипартФорм(СтрокиДляОбъединения, СтрокаДереваЗапросов)
	...
	Для ИндексСтроки = 0 По СтрокаДереваЗапросов.ТелоМультипарт.Количество() - 1 Цикл
		...
		Если СтрокаЭлементаМультипарт.Вид = ВидыЭлементовМультипарт.Строка Тогда
			...
			ИмяПараметра = СтрокаВСтрокуКодаСПеременными(СтрокаЭлементаМультипарт.Имя, Переменные);
			ЗначениеПараметра = СтрокаВСтрокуКодаСПеременными(СтрокаЭлементаМультипарт.Значение, Переменные);
			СтрокиДляОбъединения.Добавить(СтрШаблон(ШаблонТелоСтрока, ИмяПараметра, ЗначениеПараметра));
		...
		ИначеЕсли СтрокаЭлементаМультипарт.Вид = ВидыЭлементовМультипарт.Файл Тогда
			...
			ИмяПараметра = СтрокаВСтрокуКодаСПеременными(СтрокаЭлементаМультипарт.Имя, Переменные);
			ИмяФайла = СтрокаВСтрокуКодаСПеременными(СтрокаЭлементаМультипарт.Значение, Переменные);
			СтрокиДляОбъединения.Добавить(СтрШаблон(ШаблонТелоФайл, ИмяПараметра, ИмяФайла));
	...
	Если ЕстьФайлы Тогда
		ТекстТелоИзФайла = "
						   ...
						   |";
		СтрокиДляОбъединения.Добавить(ТекстТелоИзФайла);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура СгенерироватьКодИнициализацииВидТелаДвочныеДанные(СтрокиДляОбъединения, СтрокаДереваЗапросов)
	Если СтрокаДереваЗапросов.ТелоДвоичныеДанные <> Неопределено Тогда
		ДвоичныеДанныеТела = УИ_ОбщегоНазначения.ЗначениеИзХранилищаКонтейнераДвоичныхДанных(СтрокаДереваЗапросов.ТелоДвоичныеДанные);
		Если ТипЗнч(ДвоичныеДанныеТела) = Тип("ДвоичныеДанные") Тогда
...
&НаСервере
Процедура СгенерироватьКодИнициализацииВидТелаСтрока(СтрокиДляОбъединения, СтрокаДереваЗапросов)
	Если СтрокаДереваЗапросов.ИспользоватьBOM = 0 Тогда
		БОМ = "ИспользованиеByteOrderMark.Авто";
	ИначеЕсли (СтрокаДереваЗапросов.ИспользоватьBOM = 1) Тогда
		БОМ = "ИспользованиеByteOrderMark.Использовать";
	...
	ТекстТелоСтрокой = СтрокаВСтрокуКодаСПеременными(СтрокаДереваЗапросов.ТелоСтрокой, Переменные);
	СтрокиДляОбъединения.Добавить(СтрШаблон("// Тело строкой
											|ТелоЗапроса = %1;
											|ИспользованиеБОМ = %2;", ТекстТелоСтрокой, БОМ));
...
Функция СгенерированныйКодИсполненияНаСервере(ИдентификаторСтроки)
	...
	ТекстыИсполнения = Новый Массив;
	ЗаголовокКода = УИ_ГенераторКода.СтандартныйЗаголовокСгенерированногоКода("Консоль HTTP запросов",
																			  ИмяФайлаЗапросов,
																			  СтрокаДереваЗапросов.Имя);
	ТекстыИсполнения.Добавить(ЗаголовокКода);
	
	КодИнициализацииHTTPСоединения = СгенерированныйКодИнициализацииHTTPСоединения(СтрокаДереваЗапросов, СтруктураURL);
	ТекстыИсполнения.Добавить(КодИнициализацииHTTPСоединения);
	
	КодИнициализацииHTTPЗапроса = СгенерированныйКодИнициализацииHTTPЗапроса(СтрокаДереваЗапросов, СтруктураURL);
	ТекстыИсполнения.Добавить(КодИнициализацииHTTPЗапроса);
	
	КодИсполненияЗапроса = СгенерированныйКодИсполненияЗапроса(СтрокаДереваЗапросов);
	ТекстыИсполнения.Добавить(КодИсполненияЗапроса);
	
	ТекстИсполнения = СтрСоединить(ТекстыИсполнения, Символы.ПС);
...
&НаСервере
Процедура ЗаполнитьЗаголовкиЗапросаДляПоиска()

	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	МакетЗаголовков = ОбработкаОбъект.ПолучитьМакет("ЗаголовкиЗапроса");
	НачалоДанных = 2;
	НомерКолонки = 1;

	Для НомерСтроки = НачалоДанных По МакетЗаголовков.ВысотаТаблицы Цикл
		ОбластьЯчейки = МакетЗаголовков.Область(НомерСтроки, НомерКолонки);
		ИмяЗаголовка = СокрЛП(ОбластьЯчейки.Текст);
		Если ЗначениеЗаполнено(ИмяЗаголовка) Тогда
			ДобавитьЗаголовокВПоиск(ЭтотОбъект, ИмяЗаголовка);
		КонецЕсли;
	КонецЦикла;
...
Процедура ДополнитьСписокИспользованныхРанееЗаголовков(Форма, Заголовки)
	Для Каждого КлючЗначение Из Заголовки Цикл
		ДобавитьЗаголовокВПоиск(Форма, КлючЗначение.Ключ);
	КонецЦикла;
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьЗаголовокВПоиск(Форма, ИмяЗаголовка)

	ИмяДляПоиска = НРег(ИмяЗаголовка);
	НайденныеСтроки = Форма.ЗаголовкиЗапросаДляПоиска.НайтиСтроки(Новый Структура("ИмяДляПоиска", ИмяДляПоиска));
	Если НайденныеСтроки.Количество() = 0 Тогда
		НовыйЗаголовок = Форма.ЗаголовкиЗапросаДляПоиска.Добавить();
		НовыйЗаголовок.Имя = ИмяЗаголовка;
		НовыйЗаголовок.ИмяДляПоиска = ИмяДляПоиска;
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Функция СтрокиПоискаЗаголовковЗапроса(Знач ТекстПоиска)

	ТекстПоиска = НРег(СокрЛП(ТекстПоиска));
	СтрокиПоиска = Новый Соответствие;
	СтрокиУдаления = Новый Соответствие;
	СловаПоиска = СтрРазделить(ТекстПоиска, " ", Ложь);
	Для Каждого Слово Из СловаПоиска Цикл
		СтрокиПоиска.Вставить(НРег(Слово));
	КонецЦикла;
	Для Каждого ЭлементОсновной Из СтрокиПоиска Цикл
		ОсновноеСлово = ЭлементОсновной.Ключ;
		Если СтрокиУдаления[ОсновноеСлово] <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Для Каждого ЭлементПроверки Из СтрокиПоиска Цикл
			СловоПроверки = ЭлементПроверки.Ключ;
			Если ОсновноеСлово = СловоПроверки Или СтрокиУдаления[СловоПроверки] <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Если СтрНайти(ОсновноеСлово, СловоПроверки) > 0 Тогда
				СтрокиУдаления.Вставить(СловоПроверки, СловоПроверки);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	Для Каждого ЭлементУдаления Из СтрокиУдаления Цикл
		СтрокиПоиска.Удалить(ЭлементУдаления.Ключ);
	КонецЦикла;
	СписокСтрок = Новый СписокЗначений;
	Для Каждого ЭлементПоиска Из СтрокиПоиска Цикл
		СписокСтрок.Добавить(ЭлементПоиска.Ключ);
	КонецЦикла;
...
&НаКлиенте
Функция ДанныеВыбораЗаголовковЗапроса(СтрокиПоиска)
	
	ДанныеВыбора = Новый СписокЗначений();
	ЗеленыйЦвет = WebЦвета.Зеленый;
	ЖирныйШрифт = Новый Шрифт(Элементы.ТаблицаЗаголовковЗапросаКлюч.Шрифт, , , Истина);

	Для Каждого СтрокаЗаголовка Из ЗаголовкиЗапросаДляПоиска Цикл
		СимволыСовпадения = Новый Соответствие();
		НайденыСтрокиПоиска = Истина;
		Для Каждого ЭлементПоиска Из СтрокиПоиска Цикл
			ЕстьСовпадение = Ложь;
			НайтиСовпаденияВИмениЗаголовка(СтрокаЗаголовка, ЭлементПоиска, ЕстьСовпадение, СимволыСовпадения);
			Если Не ЕстьСовпадение Тогда
				НайденыСтрокиПоиска = ЕстьСовпадение;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если НайденыСтрокиПоиска Тогда
			ДобавитьЗаголовокВДанныеВыбора(ДанныеВыбора, СтрокаЗаголовка, СимволыСовпадения, ЖирныйШрифт, ЗеленыйЦвет);
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура НайтиСовпаденияВИмениЗаголовка(СтрокаЗаголовка, ЭлементПоиска, ЕстьСовпадение, СимволыСовпадения)
	
	СтрокаПоиска = ЭлементПоиска.Значение;
	ДлинаСтрокиЗаголовка = СтрДлина(СтрокаЗаголовка.ИмяДляПоиска);
	ДлинаСтрокиПоиска = СтрДлина(ЭлементПоиска.Значение);
	ПозицияПоиска = 1;
	НайденнаяПозиция = СтрНайти(СтрокаЗаголовка.ИмяДляПоиска, СтрокаПоиска, , ПозицияПоиска);
	Пока НайденнаяПозиция > 0 Цикл
		ЕстьСовпадение = Истина;
		Для Сч = 1 По ДлинаСтрокиПоиска Цикл
			ИндексСимвола = НайденнаяПозиция + Сч - 1;
			СимволыСовпадения.Вставить(ИндексСимвола, Истина);
		КонецЦикла;
		ПозицияПоиска = НайденнаяПозиция + ДлинаСтрокиПоиска;
		Если ПозицияПоиска > ДлинаСтрокиЗаголовка Тогда
			НайденнаяПозиция = 0;
		Иначе	
			НайденнаяПозиция = СтрНайти(СтрокаЗаголовка.ИмяДляПоиска, СтрокаПоиска, , ПозицияПоиска);
		КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьЗаголовокВДанныеВыбора(ДанныеВыбора, СтрокаЗаголовка, СимволыСовпадения, ЖирныйШрифт, ЗеленыйЦвет)
	
	СписокСимволов = Новый Массив();
	Для ИндексСимвола = 1 По СтрДлина(СтрокаЗаголовка.Имя) Цикл
		Если СимволыСовпадения[ИндексСимвола] = Неопределено Тогда
			Символ = Сред(СтрокаЗаголовка.Имя, ИндексСимвола, 1);
		Иначе
			ВыделенныйСимвол = Сред(СтрокаЗаголовка.Имя, ИндексСимвола, 1);
			Символ = Новый ФорматированнаяСтрока(ВыделенныйСимвол, ЖирныйШрифт, ЗеленыйЦвет);
		КонецЕсли;		
		СписокСимволов.Добавить(Символ);
	КонецЦикла;
	ПредставлениеЗаголовка = Новый ФорматированнаяСтрока(СписокСимволов);
	ДанныеВыбора.Добавить(СтрокаЗаголовка.Имя, ПредставлениеЗаголовка);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьЗаголовкиВкладокФормы()
	ОбновитьЗаголовокВкладкиАутентификации();
	ОбновитьЗаголовокВкладкиЗаголовкиЗапроса();
	ОбновитьЗаголовокВкладкиЗаголовкиОтвета();
	ОбновитьЗаголовокВкладкиИстория();
	ОбновитьЗаголовокВкладкиТелоЗапроса();
	ОбновитьЗаголовокВкладкиПараметрыURL();
	ОбновитьЗаголовокКнопкиУправленияКуки();
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьЗаголовокВкладкиЗаголовкиЗапроса()
	
	Если РедактированиеЗаголовковТаблицей Тогда
		ПараметрыПоиска = Новый Структура("Использование", Истина);
		ИспользуемыеЗаголовки = ТаблицаЗаголовковЗапроса.НайтиСтроки(ПараметрыПоиска);
		КоличествоЗаписей = ИспользуемыеЗаголовки.Количество();
	Иначе
		Заголовки = УИ_ОбщегоНазначенияКлиентСервер.ЗаголовкиHTTPЗапросаИзСтроки(ЗаголовкиСтрока);
		КоличествоЗаписей = Заголовки.Количество();
	КонецЕсли;
	
	НовыйЗаголовок = НСтр("ru = 'Заголовки'");
	Если КоличествоЗаписей > 0 Тогда
		НовыйЗаголовок = СтрШаблон(НСтр("ru = '%1 (%2)'"), НовыйЗаголовок, КоличествоЗаписей);
	КонецЕсли;
	Элементы.ГруппаСтраницаЗапросаЗаголовки.Заголовок = НовыйЗаголовок;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьЗаголовокВкладкиАутентификации()
	
	НовыйЗаголовок = НСтр("ru = 'Аутентификация'");
	ТекущиеДанные = Элементы.ДеревоЗапросов.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено И ТекущиеДанные.АутентификацияИспользовать Тогда
		ВидыАутентификации = ВидыАутентификации();
		Если ТекущиеДанные.АутентификацияВид = ВидыАутентификации.Нет Тогда
			ОписаниеАутентификации = "";
		ИначеЕсли ТекущиеДанные.АутентификацияВид = ВидыАутентификации.Базовая
				  Или ТекущиеДанные.АутентификацияВид = ВидыАутентификации.Базовая1С Тогда
			ОписаниеАутентификации = НСтр("ru = 'Basic'");
		ИначеЕсли ТекущиеДанные.АутентификацияВид = ВидыАутентификации.BearerToken Тогда
			ОписаниеАутентификации = НСтр("ru = 'JWT'");
		ИначеЕсли ТекущиеДанные.АутентификацияВид = ВидыАутентификации.NTML Тогда
			ОписаниеАутентификации = НСтр("ru = 'NTML'");
		Иначе
			ОписаниеАутентификации = НСтр("ru = '*'");
		КонецЕсли;
		Если ЗначениеЗаполнено(ОписаниеАутентификации) Тогда
			НовыйЗаголовок = СтрШаблон(НСтр("ru = '%1 (%2)'"), НовыйЗаголовок, ОписаниеАутентификации);
		КонецЕсли;
	КонецЕсли;
	Элементы.ГруппаСтраницаЗапросаАутентификация.Заголовок = НовыйЗаголовок;	
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьЗаголовокВкладкиТелоЗапроса()
	
	НовыйЗаголовок = НСтр("ru = 'Тело'");
	ТекущиеДанные = Элементы.ДеревоЗапросов.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда
		ВидыТела = ВидыТелаЗапроса();
		Если ТекущиеДанные.ВидТела = ВидыТела.БезТела Тогда
			ОписаниеТела = "";
		ИначеЕсли ТекущиеДанные.ВидТела = ВидыТела.ДвоичныеДанные Тогда
			ОписаниеТела = НСтр("ru = 'Дв. данные'");
		ИначеЕсли ТекущиеДанные.ВидТела = ВидыТела.МультипартФорм Тогда
			ОписаниеТела = НСтр("ru = 'Мультипарт'");
		ИначеЕсли ТекущиеДанные.ВидТела = ВидыТела.Строка Тогда
			ОписаниеТела = НСтр("ru = 'Строка'");
		ИначеЕсли ТекущиеДанные.ВидТела = ВидыТела.Файл Тогда
			ОписаниеТела = НСтр("ru = 'Файл'");
		Иначе
			ОписаниеТела = НСтр("ru = '*'");
		КонецЕсли;
		Если ЗначениеЗаполнено(ОписаниеТела) Тогда
			НовыйЗаголовок = СтрШаблон(НСтр("ru = '%1 (%2)'"), НовыйЗаголовок, ОписаниеТела);
		КонецЕсли;
	КонецЕсли;
	Элементы.ГруппаСтраницаЗапросаТело.Заголовок = НовыйЗаголовок;	
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьЗаголовокВкладкиИстория()
	
	НовыйЗаголовок = НСтр("ru = 'История'");

	ТекущиеДанные = Элементы.ДеревоЗапросов.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда
		КоличествоЗаписей = ТекущиеДанные.ИсторияЗапросов.Количество();
		Если КоличествоЗаписей > 0 Тогда
			НовыйЗаголовок = СтрШаблон(НСтр("ru = '%1 (%2)'"), НовыйЗаголовок, КоличествоЗаписей);
		КонецЕсли;
	КонецЕсли;

	Элементы.СтраницаИстория.Заголовок = НовыйЗаголовок;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьЗаголовокВкладкиЗаголовкиОтвета()
	
	НовыйЗаголовок = НСтр("ru = 'Заголовки'");
	Заголовки = УИ_ОбщегоНазначенияКлиентСервер.ЗаголовкиHTTPЗапросаИзСтроки(ЗаголовкиОтвета);
	КоличествоЗаписей = Заголовки.Количество();
	Если КоличествоЗаписей > 0 Тогда
		НовыйЗаголовок = СтрШаблон(НСтр("ru = '%1 (%2)'"), НовыйЗаголовок, КоличествоЗаписей);
	КонецЕсли;
	Элементы.СтраницаЗаголовкиОтвета.Заголовок = НовыйЗаголовок;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьЗаголовокВкладкиПараметрыURL()
	
	НовыйЗаголовок = НСтр("ru = 'Параметры URL'");

	ТекущиеДанные = Элементы.ДеревоЗапросов.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда
		ПараметрыПоиска = Новый Структура("Использование", Истина);
		ИспользуемыеПараметры = ТекущиеДанные.ПараметрыURL.НайтиСтроки(ПараметрыПоиска);
		КоличествоЗаписей = ИспользуемыеПараметры.Количество();
		Если КоличествоЗаписей > 0 Тогда
			НовыйЗаголовок = СтрШаблон(НСтр("ru = '%1 (%2)'"), НовыйЗаголовок, КоличествоЗаписей);
		КонецЕсли;
	КонецЕсли;

	Элементы.ГруппаСтраницаЗапросаПараметрыURL.Заголовок = НовыйЗаголовок;
	
КонецПроцедуры

&НаКлиенте
Процедура УправлениеКукиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Модифицированность = Истина;
	Куки.Очистить();
	Для Каждого СтрокаКуки Из Результат Цикл
		НоваяСтрока = Куки.Добавить();
...
&НаКлиенте
Процедура ОбновитьЗаголовокКнопкиУправленияКуки()

	НовыйЗаголовок = НСтр("ru = 'Управление куки'");

	КоличествоЗаписей = Куки.Количество();
	Если КоличествоЗаписей > 0 Тогда
		НовыйЗаголовок = СтрШаблон(НСтр("ru = '%1 (%2)'"), НовыйЗаголовок, КоличествоЗаписей);
	КонецЕсли;

	Элементы.УправлениеКуки.Заголовок = НовыйЗаголовок;
