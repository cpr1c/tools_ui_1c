...
Функция СгенерированныйКодИнициализацииHTTPЗапроса(СтрокаДереваЗапросов, СтруктураURL)
	...
	Для Каждого СтрокаЗаголовка Из СтрокаДереваЗапросов.Заголовки Цикл
		...
		КлючЗаголовка = УИ_ГенераторКода.СтрокаВСтрокуКода(СтрокаСУстановленнымиЗначениямиПеременных(СтрокаЗаголовка.Ключ,
																									 Переменные));
		ЗначениеЗаголовка = УИ_ГенераторКода.СтрокаВСтрокуКода(СтрокаСУстановленнымиЗначениямиПеременных(СтрокаЗаголовка.Значение,
																										 Переменные));
	...
	ЭтоЗащищенноеСоединение = (СтрСравнить(СтруктураURL.Схема, "https") = 0);
	Домен = ДобавитьЛидирующуюТочку(СтруктураURL.Сервер);
	Путь = ?(СтрДлина(СтруктураURL.Путь) > 1 И Не СтрЗаканчиваетсяНа(СтруктураURL.Путь, "/"),
			 СтрШаблон("%1/", СтруктураURL.Путь),
			 СтруктураURL.Путь);
	ТекущаяДата = ТекущаяУниверсальнаяДатаНаСервере();
	ШаблонКуки = "%1=%2";
	СписокКуки = Новый Массив;
	Для Каждого ОписаниеКуки Из Куки Цикл
		Если ИспользоватьКукиДляЗапросаHTTP(ОписаниеКуки, Домен, Путь, ТекущаяДата, ЭтоЗащищенноеСоединение) Тогда
			КлючКуки = ОписаниеКуки.Ключ;
			ЗначениеКуки = ОписаниеКуки.Значение;
			ПредставлениеКуки = СтрШаблон(ШаблонКуки, КлючКуки, ЗначениеКуки);
			СписокКуки.Добавить(ПредставлениеКуки);
	...
	Если СписокКуки.Количество() > 0 Тогда
		ОписаниеКуки = СтрСоединить(СписокКуки, "; ");
		НовыйЗаголовок = СтрШаблон("НовыйЗапрос.Заголовки.Вставить(""Cookie"", ""%1"");", ОписаниеКуки);
		СтрокиДляОбъединения.Добавить(НовыйЗаголовок);
	КонецЕсли;
...
Процедура СгенерироватьКодИнициализацииТелаЗапроса(СтрокиДляОбъединения, СтрокаДереваЗапросов)
	...
	ИначеЕсли СтрокаДереваЗапросов.ВидТела = ВидыТелаЗапроса.Строка Тогда
		...
		ТекстТелоСтрокой = УИ_ГенераторКода.СтрокаВСтрокуКода(СтрокаСУстановленнымиЗначениямиПеременных(СтрокаДереваЗапросов.ТелоСтрокой,
																										Переменные));
	...
	ИначеЕсли СтрокаДереваЗапросов.ВидТела = ВидыТелаЗапроса.МультипартФорм Тогда
		...
		Для ИндексСтроки = 0 По СтрокаДереваЗапросов.ТелоМультипарт.Количество() - 1 Цикл
			...
			Если СтрокаЭлементаМультипарт.Вид = ВидыЭлементовМультипарт.Строка Тогда
				...
				ИмяПараметра = УИ_ГенераторКода.СтрокаВСтрокуКода(СтрокаСУстановленнымиЗначениямиПеременных(СтрокаЭлементаМультипарт.Имя,
																											Переменные));
				ЗначениеПараметра = УИ_ГенераторКода.СтрокаВСтрокуКода(СтрокаСУстановленнымиЗначениямиПеременных(СтрокаЭлементаМультипарт.Значение,
																												 Переменные));
			...
			ИначеЕсли СтрокаЭлементаМультипарт.Вид = ВидыЭлементовМультипарт.Файл Тогда
				...
				ИмяПараметра = УИ_ГенераторКода.СтрокаВСтрокуКода(СтрокаСУстановленнымиЗначениямиПеременных(СтрокаЭлементаМультипарт.Имя,
																											Переменные));
				ИмяФайла = УИ_ГенераторКода.СтрокаВСтрокуКода(СтрокаСУстановленнымиЗначениямиПеременных(СтрокаЭлементаМультипарт.Значение,
																										Переменные));
