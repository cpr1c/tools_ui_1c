...
Процедура ЗагрузитьФайлКонсолиНаСервере(Адрес)
	...
	Куки.Очистить();
	Если СтруктураФайла.ВерсияФормата >= 5 Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция ИспользоватьКукиДляЗапросаHTTP(ОписаниеКуки, Домен, Путь, ТекущаяДата, ЭтоЗащищенноеСоединение)
	
	Результат = Истина;
	Если ОписаниеКуки.ДействуетДо < ТекущаяДата
		Или Не ОписаниеКуки.ТолькоДляДомена И Не СтрЗаканчиваетсяНа(Домен, ОписаниеКуки.Домен) 
		Или ОписаниеКуки.ТолькоДляДомена И СтрСравнить(Домен, ОписаниеКуки.Домен) <> 0
		Или Не СтрНачинаетсяС(Путь, ОписаниеКуки.Путь)
		Или ОписаниеКуки.ТолькоЗащищенноеСоединение И Не ЭтоЗащищенноеСоединение Тогда
		Результат = Ложь;
	КонецЕсли;
