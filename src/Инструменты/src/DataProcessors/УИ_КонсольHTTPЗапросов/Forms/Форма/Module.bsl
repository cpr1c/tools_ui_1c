...
Функция ПодготовленныйЗапросHTTP(Форма, СтрокаДереваЗапросов, СтруктураURL, ДанныеФайлаТела)
	НовыйЗапрос = УИ_МетодыСовместимостиПлатформы_8_3_21КлиентСервер.НовыйHTTPЗапрос();
...
#КонецЕсли
	...
	Иначе
		Если СтрокаДереваЗапросов.НаКлиенте Тогда
			НовыйЗапрос.УстановитьИмяФайлаТела(ДанныеФайлаТела.ПолноеИмя);
		Иначе
#Если Не ВебКлиент Тогда
