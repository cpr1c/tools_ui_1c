...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗафиксироватьЛогЗапроса(Форма, СтрокаДереваЗапросов, АдресСервера, Протокол, HTTPЗапрос, HTTPОтвет, ДатаНачала,
	Длительность)
...
#Иначе
		ОповещениеОЗавершенииВыполненияЗапроса = Неопределено;
	#КонецЕсли	
	...
	Попытка
		Если УИ_ОбщегоНазначенияКлиентСервер.ВерсияПлатформыНеМладше("8.3.21")
			...
			КонецЕсли;
#Если Клиент Тогда
			УИ_МетодыСовмеcтимостиПлатформы_8_3_18Клиент.НачатьВыполнениеОбещания(Ответ,
...
#КонецЕсли
	...
	ЗаполнитьРезультатЗапросаПоЗаписиИстории(Форма,
											 СтрокаДерева,
											 СтрокаДерева.ИсторияЗапросов[0]);

	ДополнитьСписокИспользованныхРанееЗаголовков(Форма, Запрос.Заголовки);
...
Процедура ПослеВыполненияЗапросаНаКлиенте(ОтветСервера, ДополнительныеПараметры) Экспорт
	ДополнительныеПараметры.Вставить("Ответ", ОтветСервера);
