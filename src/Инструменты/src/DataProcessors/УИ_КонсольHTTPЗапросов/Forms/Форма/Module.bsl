...
Процедура ТаблицаЗаголовковЗапросаКлючАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание,
	...
	Если СтрНайти(ТекстПоиска, "-") = 0 Тогда
		СтрокиПоиска = СформироватьСтрокиПоиска(ТекстПоиска);
		ПоискЗаголовковПоЧастям(ДанныеВыбора, СтрокиПоиска, ЖирныйШрифт, ЗеленыйЦвет);
	Иначе
		ПоискЗаголовковПоСтроке(ДанныеВыбора, ТекстПоиска, ЖирныйШрифт, ЗеленыйЦвет);
	КонецЕсли;
...
Процедура ДобавитьЗаголовокВПоиск(Форма, ИмяЗаголовка)
	...
	ИмяПоиска = НРег(ИмяЗаголовка);
	НайденныеСтроки = Форма.ЗаголовкиЗапросаДляПоиска.НайтиСтроки(Новый Структура("ИмяПоиска", ИмяПоиска));
	Если НайденныеСтроки.Количество() = 0 Тогда
		НовыйЗаголовок = Форма.ЗаголовкиЗапросаДляПоиска.Добавить();
		НовыйЗаголовок.Имя = ИмяЗаголовка;
		НовыйЗаголовок.ИмяПоиска = ИмяПоиска;
		
		СписокДанныхПоиска = Новый СписокЗначений;
		ЗначенияПоиска = СтрРазделить(ИмяПоиска, "-");
		СписокДанныхПоиска.ЗагрузитьЗначения(ЗначенияПоиска);
		НовыйЗаголовок.ДанныеПоиска = СписокДанныхПоиска;
		
		СписокДанныхПредставления = Новый СписокЗначений;
		ЗначенияПредставления = СтрРазделить(ИмяЗаголовка, "-");
		СписокДанныхПредставления.ЗагрузитьЗначения(ЗначенияПредставления);
		НовыйЗаголовок.ДанныеПредставления = СписокДанныхПредставления;
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Функция СформироватьСтрокиПоиска(ТекстПоиска)
	
	ЭлементыУдаления = Новый Массив;
	СтрокиУдаления = Новый Соответствие();
	СтрокиПроверки = Новый СписокЗначений();
	СтрокиПроверки.ЗагрузитьЗначения(СтрРазделить(ТекстПоиска, " ", Ложь));
	СтрокиПроверки.СортироватьПоЗначению(НаправлениеСортировки.Убыв);
	ПоследнийЭлемент = СтрокиПроверки.Количество() - 1;
	Для Счетчик = 0 По ПоследнийЭлемент Цикл
		Элемент = СтрокиПроверки[Счетчик];
		ОсновнаяСтрока = Элемент.Значение;
		Если СтрокиУдаления[ОсновнаяСтрока] <> Неопределено Тогда
			ЭлементыУдаления.Добавить(Элемент);
			Продолжить;
		КонецЕсли;
		Для СчетчикПроверки = Счетчик + 1 По ПоследнийЭлемент Цикл
			ЭлементПроверки = СтрокиПроверки[СчетчикПроверки];
			СтрокаПроверки = ЭлементПроверки.Значение;
			Если СтрНачинаетсяС(ОсновнаяСтрока, СтрокаПроверки) Тогда
				СтрокиУдаления.Вставить(СтрокаПроверки, СтрокаПроверки);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	Для каждого ЭлементУдаления Из ЭлементыУдаления Цикл
	    СтрокиПроверки.Удалить(ЭлементУдаления);
	КонецЦикла;
	Возврат СтрокиПроверки;
	
КонецФункции

&НаКлиенте
Процедура ПоискЗаголовковПоЧастям(ДанныеВыбора, СтрокиПоиска, ЖирныйШрифт, ЗеленыйЦвет)

	Для Каждого СтрокаЗаголовка Из ЗаголовкиЗапросаДляПоиска Цикл
		СписокЧастей = Новый Массив;
		ПерваяСтрока = Истина;
		ЕстьСовпадение = Истина;
		Для Сч = 0 По СтрокаЗаголовка.ДанныеПоиска.Количество() - 1 Цикл
			ЧастьЗаголовка = СтрокаЗаголовка.ДанныеПредставления[Сч].Значение;
			ЧастьЗаголовкаПоиска = СтрокаЗаголовка.ДанныеПоиска[Сч].Значение;
			Если ПерваяСтрока Тогда
				ПерваяСтрока = Ложь;
			Иначе
				СписокЧастей.Добавить("-");
			КонецЕсли;
			НайтиСовпадениеВЧастиСтроки(СтрокиПоиска,
										ЧастьЗаголовкаПоиска,
										ЧастьЗаголовка,
										СписокЧастей,
										ЖирныйШрифт,
										ЗеленыйЦвет);
		...
		Для Каждого ЭлементПоиска Из СтрокиПоиска Цикл
			ЕстьСовпадение = (ЕстьСовпадение И ЭлементПоиска.Пометка);
			ЭлементПоиска.Пометка = Ложь;
		КонецЦикла;
		Если ЕстьСовпадение Тогда
			ПредставлениеЗаголовка = Новый ФорматированнаяСтрока(СписокЧастей);
...
&НаКлиенте
Процедура НайтиСовпадениеВЧастиСтроки(СтрокиПоиска, ЧастьЗаголовкаПоиска, ЧастьЗаголовка, СписокЧастей, ЖирныйШрифт,
	ЗеленыйЦвет)
	
	НайденоСовпадениеЧасти = Ложь;
	Для Каждого ЭлементПоиска Из СтрокиПоиска Цикл
		Если НайденоСовпадениеЧасти Тогда
			Прервать;
		КонецЕсли;
		ТекстПоиска = ЭлементПоиска.Значение;
		ДлинаТекстаПоиска = СтрДлина(ТекстПоиска);
		Если СтрНачинаетсяС(ЧастьЗаголовкаПоиска, ТекстПоиска) Тогда
			ВыделенныйТекст = Лев(ЧастьЗаголовка, ДлинаТекстаПоиска);
			ФорматированнаяСтрока = Новый ФорматированнаяСтрока(ВыделенныйТекст, ЖирныйШрифт, ЗеленыйЦвет);
			СписокЧастей.Добавить(ФорматированнаяСтрока);
			СтрокаБезВыделения = Сред(ЧастьЗаголовка, ДлинаТекстаПоиска + 1);
			СписокЧастей.Добавить(СтрокаБезВыделения);
			ЭлементПоиска.Пометка = Истина;
			НайденоСовпадениеЧасти = Истина;
		КонецЕсли;
	КонецЦикла;
	Если Не НайденоСовпадениеЧасти Тогда
		СписокЧастей.Добавить(ЧастьЗаголовка);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ПоискЗаголовковПоСтроке(ДанныеВыбора, СтрокаПоиска, ЖирныйШрифт, ЗеленыйЦвет)
	
	Для Каждого СтрокаЗаголовка Из ЗаголовкиЗапросаДляПоиска Цикл
		ЕстьСовпадение = Ложь;
		ДлинаСтрокиЗаголовка = СтрДлина(СтрокаЗаголовка.ИмяПоиска);
		ДлинаСтрокиПоиска = СтрДлина(СтрокаПоиска);
		СписокЧастей = Новый Массив();
		ПозицияПоиска = 1;
		НайденнаяПозиция = СтрНайти(СтрокаЗаголовка.ИмяПоиска, СтрокаПоиска, , ПозицияПоиска);
		Пока НайденнаяПозиция > 0 Цикл
			ЕстьСовпадение = Истина;
			СтрокаБезВыделения = Сред(СтрокаЗаголовка.Имя, ПозицияПоиска, НайденнаяПозиция - ПозицияПоиска);
			Если СтрокаБезВыделения <> "" Тогда
				СписокЧастей.Добавить(СтрокаБезВыделения);	
			КонецЕсли;
			ВыделенныйТекст = Сред(СтрокаЗаголовка.Имя, НайденнаяПозиция, ДлинаСтрокиПоиска);
			ФорматированнаяСтрока = Новый ФорматированнаяСтрока(ВыделенныйТекст, ЖирныйШрифт, ЗеленыйЦвет);
			СписокЧастей.Добавить(ФорматированнаяСтрока);
			...
			КонецЕсли;
		КонецЦикла;
		СтрокаБезВыделения = Сред(СтрокаЗаголовка.Имя, ПозицияПоиска);
		Если СтрокаБезВыделения <> "" Тогда
			СписокЧастей.Добавить(СтрокаБезВыделения);	
		КонецЕсли;
		Если ЕстьСовпадение Тогда
