...
//		
//		ПолеНайдено = Ложь;
//		КоллекцияДобавления = ПоляСКД.ПолучитьЭлементы();
//		
//		ТекПрефиксПути = "";
//		
//		Для Каждого ЭлементПути Из МассивПути Цикл
//			ПутьКДаннымЭлементаПути = ТекПрефиксПути + ?(ЗначениеЗаполнено(ТекПрефиксПути), ".", "") + ЭлементПути;
//			
//			Для Каждого ЭлементКоллекции Из КоллекцияДобавления Цикл
//				Если НРег(ЭлементКоллекции.ПутьКДанным) = НРег(ТекПоле.ПутьКДанным) Тогда
//					ПолеНайдено = Истина;
//					Прервать;
//				ИначеЕсли НРег(ЭлементКоллекции.ПутьКДанным) = НРег(ПутьКДаннымЭлементаПути) Тогда
//					КоллекцияДобавления = ЭлементКоллекции.ПолучитьЭлементы();
//					Прервать;
//				КонецЕсли;
//			КонецЦикла;
//			
...
//		КонецЕсли;
		...
		КонецЕсли;
		НовоеПоле.Вычисляемое = ТекПоле.ВычисляемоеПоле;
		
		Если НовоеПоле.Вычисляемое Тогда
...
Процедура УИ_ДобавитьКонтекстПолей()
	...
	Для Каждого ДоступнаяПеременная Из ПоляСКД.ПолучитьЭлементы() Цикл
		...
		Иначе
			...
			Для Каждого ТекРеквизитПеременной Из КоллекцияПодчиненных Цикл
				НовоеСвойство = Новый Структура;
				НовоеСвойство.Вставить("Имя", ТекРеквизитПеременной.Поле);
				НовоеСвойство.Вставить("Тип", ТекРеквизитПеременной.ТипЗначения);
