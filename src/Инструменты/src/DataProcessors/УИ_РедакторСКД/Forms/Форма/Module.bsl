...
&НаКлиенте
Процедура ПоляОграничениеИспользованияРеквизитовПолеПриИзменении(Элемент)
	ИдентификаторСтрокиНабора = Элементы.НаборыДанных.ТекущаяСтрока;
	ТекущиеДанные = Элементы.НаборыДанныхПоля.ТекущиеДанные;
	ЗаписатьОграниченияВКэш(ИдентификаторСтрокиНабора, ТекущиеДанные);
КонецПроцедуры

&НаКлиенте
Процедура ПоляОграничениеИспользованияРеквизитовУсловиеПриИзменении(Элемент)
	ИдентификаторСтрокиНабора = Элементы.НаборыДанных.ТекущаяСтрока;
	ТекущиеДанные = Элементы.НаборыДанныхПоля.ТекущиеДанные;
	ЗаписатьОграниченияВКэш(ИдентификаторСтрокиНабора, ТекущиеДанные);
КонецПроцедуры
...
&НаКлиенте
Процедура ПоляОграничениеИспользованияРеквизитовГруппировкаПриИзменении(Элемент)
	ИдентификаторСтрокиНабора = Элементы.НаборыДанных.ТекущаяСтрока;
	ТекущиеДанные = Элементы.НаборыДанныхПоля.ТекущиеДанные;
	ЗаписатьОграниченияВКэш(ИдентификаторСтрокиНабора, ТекущиеДанные);	
КонецПроцедуры
...
&НаКлиенте
Процедура ПоляОграничениеИспользованияРеквизитовПорядокПриИзменении(Элемент)
	ИдентификаторСтрокиНабора = Элементы.НаборыДанных.ТекущаяСтрока;
	ТекущиеДанные = Элементы.НаборыДанныхПоля.ТекущиеДанные;
	ЗаписатьОграниченияВКэш(ИдентификаторСтрокиНабора, ТекущиеДанные);
КонецПроцедуры
...
Процедура ПрочитатьСхемуИзФайлаЗавершениеПомещенияФайла(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	НаборыДанныхКэш.ПолучитьЭлементы().Очистить();
	ЭтоЗагрузкаСхемыИзФайла = Истина;
	ПрочитатьСхемуИзФайлаНаСервере(Адрес);
	ЗаписатьОграниченияВКэшПриЗагрузкеИзФайла(НаборыДанных.ПолучитьЭлементы());
	ЗаполнитьВспомогательныеДанныеРесурсов();
	ЭтоЗагрузкаСхемыИзФайла = Ложь;
КонецПроцедуры
...
&НаКлиенте
Процедура ЗаписатьОграниченияВКэшПриЗагрузкеИзФайла(ЭлементыДерева)
	Для Каждого Элемент Из ЭлементыДерева Цикл
		Если Элемент.Вид = "НаборДанныхЗапросСхемыКомпоновкиДанных" Тогда
			ИдентификаторСтрокиНабора = Элемент.ПолучитьИдентификатор();
			Для Каждого Поле Из Элемент.Поля Цикл
				ЗаписатьОграниченияВКэш(ИдентификаторСтрокиНабора, Поле);
			КонецЦикла;
		Иначе
			ЗаписатьОграниченияВКэшПриЗагрузкеИзФайла(Элемент.ПолучитьЭлементы());			
		КонецЕсли; 
	КонецЦикла;
КонецПроцедуры
...
Процедура ДобавитьПолеНабора(СтрокаНабора, Колонка, ВидыПолейНаборовДанных, МассивПолей, КолонкаРодитель = Неопределено)
	...
	ЭтоПолеВыбораКомпоновкиДанных = Ложь;
	...
	ИначеЕсли ТипЗнч(Колонка) = Тип("ПолеНастройки") Тогда
		...
		Иначе
			...
			ИмяКолонки=Колонка.Имя;
		КонецЕсли;
	ИначеЕсли ТипЗнч(Колонка) = Тип("ПолеВыбораКомпоновкиДанныхСхемыЗапроса") Тогда
		Вид=ВидыПолейНаборовДанных.Поле;
		Если ЗначениеЗаполнено(Колонка.Псевдоним) Тогда
			ИмяКолонки=Колонка.Псевдоним;
		Иначе
			ИмяКолонки = Колонка.Поле.Псевдоним;
		КонецЕсли;
		ЭтоПолеВыбораКомпоновкиДанных = Истина;	
	КонецЕсли;
	...
	ИначеЕсли Вид = ВидыПолейНаборовДанных.Поле Тогда
		Если ЭтоПолеВыбораКомпоновкиДанных Тогда
			НовоеПоле.ТипЗначенияЗапроса=Колонка.Поле.ТипЗначения;
		Иначе
			НовоеПоле.ТипЗначенияЗапроса=Колонка.ТипЗначения;
		КонецЕсли;	
		
		НовоеПоле.ЭтоПолеВыбораКомпоновкиДанных = ЭтоПолеВыбораКомпоновкиДанных;
		Если ЭтоПолеВыбораКомпоновкиДанных Тогда
			НовоеПоле.ОграничениеИспользованияУсловие = Истина;
			НовоеПоле.ОграничениеИспользованияРеквизитовУсловие	= Истина;
			НовоеПоле.ИспользоватьРеквизиты = Колонка.ИспользоватьРеквизиты;
			Если Не Колонка.ИспользоватьРеквизиты Тогда
				НовоеПоле.ОграничениеИспользованияРеквизитовГруппировка = Истина;
				НовоеПоле.ОграничениеИспользованияРеквизитовПоле = Истина;
				НовоеПоле.ОграничениеИспользованияРеквизитовПорядок = Истина;
			Иначе
				Если Не ЭтоЗагрузкаСхемыИзФайла Тогда
					Успех = ВосстановитьОграниченияИзКэша(СтрокаНабора, НовоеПоле, ЭтоПолеВыбораКомпоновкиДанных);
					Если Не Успех Тогда
						НовоеПоле.ОграничениеИспользованияРеквизитовГруппировка = Ложь;
						НовоеПоле.ОграничениеИспользованияРеквизитовПоле = Ложь;
						НовоеПоле.ОграничениеИспользованияРеквизитовПорядок = Ложь;
					КонецЕсли;		
				КонецЕсли;
			КонецЕсли;			
		Иначе
			Если Не ОписаниеТиповСодержитСсылку(Колонка.ТипЗначения)
				ИЛИ Колонка.Поля.Количество() = 0 Тогда
				НовоеПоле.ИспользоватьРеквизиты = Ложь;
				НовоеПоле.ОграничениеИспользованияРеквизитовГруппировка = Истина;
				НовоеПоле.ОграничениеИспользованияРеквизитовПоле = Истина;
				НовоеПоле.ОграничениеИспользованияРеквизитовПорядок = Истина;
				НовоеПоле.ОграничениеИспользованияРеквизитовУсловие	= Истина;
			Иначе
				НовоеПоле.ИспользоватьРеквизиты = Истина;
				Если Не ЭтоЗагрузкаСхемыИзФайла Тогда
					Успех = ВосстановитьОграниченияИзКэша(СтрокаНабора, НовоеПоле, ЭтоПолеВыбораКомпоновкиДанных);
					Если Не Успех Тогда
						НовоеПоле.ОграничениеИспользованияРеквизитовГруппировка = Ложь;
						НовоеПоле.ОграничениеИспользованияРеквизитовПоле = Ложь;
						НовоеПоле.ОграничениеИспользованияРеквизитовПорядок = Ложь;
						НовоеПоле.ОграничениеИспользованияРеквизитовУсловие	= Ложь;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;				
...
Процедура ЗаполнитьПоляНабораДанныхПриИзмененииЗапросаНаСервере(ИдентификаторСтрокиНабора)
	...
	ПоляВыбораКомпоновкиДанных = Новый Массив();
	Для Каждого Колонка Из НужныйПакет.ПоляВыбораКомпоновкиДанных Цикл
		ДобавитьПолеНабора(СтрокаНабора, Колонка, ВидыПолейНаборовДанных, МассивПолей);
		ПоляВыбораКомпоновкиДанных.Добавить(Колонка.Поле.Псевдоним);
	КонецЦикла;	
	Если СтрокаНабора.АвтоЗаполнениеДоступныхПолей Тогда
		Для Каждого Колонка Из НужныйПакет.Колонки Цикл
			Если ПоляВыбораКомпоновкиДанных.Найти(Колонка.Псевдоним) = Неопределено Тогда
				ДобавитьПолеНабора(СтрокаНабора, Колонка, ВидыПолейНаборовДанных, МассивПолей);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;		
...
&НаКлиенте
Процедура ЗаписатьОграниченияВКэш(ИдентификаторСтрокиНабора, ТекущиеДанные)
	СтрокаНабора = НаборыДанных.НайтиПоИдентификатору(ИдентификаторСтрокиНабора);
	Если Не СтрокаНабора.Вид = "НаборДанныхЗапросСхемыКомпоновкиДанных" Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаНабораКэш = Неопределено;
	Для Каждого ЭлементДерева Из НаборыДанныхКэш.ПолучитьЭлементы() Цикл
		Если ЭлементДерева.ИдентификаторСтрокиНабора = ИдентификаторСтрокиНабора Тогда
			СтрокаНабораКэш = ЭлементДерева;
			Прервать;
		КонецЕсли;		
	КонецЦикла;
	Если СтрокаНабораКэш = Неопределено Тогда
		СтрокаНабораКэш = НаборыДанныхКэш.ПолучитьЭлементы().Добавить();
		СтрокаНабораКэш.ИдентификаторСтрокиНабора = ИдентификаторСтрокиНабора;
	КонецЕсли;
	ЗаполнитьЗначенияСвойств(СтрокаНабораКэш, СтрокаНабора, , "Поля");
	Отбор = Новый Структура("Поле", ТекущиеДанные.Поле);
	СтрокаКэш = СтрокаНабораКэш.Поля.НайтиСтроки(Отбор);
	Если СтрокаКэш.Количество() = 0 Тогда
		СтрокаКэш = СтрокаНабораКэш.Поля.Добавить();
	Иначе
		СтрокаКэш = СтрокаКэш[0];
	КонецЕсли;
	ЗаполнитьЗначенияСвойств(СтрокаКэш, ТекущиеДанные, , "ВыраженияУпорядочиванияКомпоновкиДанных");	
КонецПроцедуры

&НаСервере
Функция ВосстановитьОграниченияИзКэша(СтрокаНабора, НовоеПоле, ЭтоПолеВыбораКомпоновкиДанных)
	
	Успешно = Ложь;
	
	СтрокаНабораКэш = Неопределено;
	ИдентификаторСтрокиНабора = СтрокаНабора.ПолучитьИдентификатор();
	Для Каждого ЭлементДерева Из НаборыДанныхКэш.ПолучитьЭлементы() Цикл
		Если ЭлементДерева.ИдентификаторСтрокиНабора = ИдентификаторСтрокиНабора Тогда
			СтрокаНабораКэш = ЭлементДерева;
			Прервать;
	...
	Если Не СтрокаНабораКэш = Неопределено Тогда
		Отбор = Новый Структура("Поле", НовоеПоле.Поле);
		СтрокаПоляКэш = СтрокаНабораКэш.Поля.НайтиСтроки(Отбор); 
		Если Не СтрокаПоляКэш.Количество() = 0 Тогда
			СтрокаПоляКэш = СтрокаПоляКэш[0];
			НовоеПоле.ОграничениеИспользованияРеквизитовГруппировка = СтрокаПоляКэш.ОграничениеИспользованияРеквизитовГруппировка;
			НовоеПоле.ОграничениеИспользованияРеквизитовПоле = СтрокаПоляКэш.ОграничениеИспользованияРеквизитовПоле;
			НовоеПоле.ОграничениеИспользованияРеквизитовПорядок = СтрокаПоляКэш.ОграничениеИспользованияРеквизитовПорядок;
			Если Не ЭтоПолеВыбораКомпоновкиДанных Тогда
				НовоеПоле.ОграничениеИспользованияРеквизитовУсловие	= СтрокаПоляКэш.ОграничениеИспользованияРеквизитовУсловие;
			КонецЕсли;			
			Успешно = Истина;
		КонецЕсли;	
	КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	// 4. Только просмотр для галочек Ограничение поля и Ограничение реквизитов
	НовоеУО =УсловноеОформление.Элементы.Добавить();
	НовоеУО.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	
	ГруппаОтбора = НовоеУО.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
 	ГруппаОтбора.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;
 	ГруппаОтбора.Использование = Истина;
	
	ЭлементОтбора = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("НаборыДанных.Вид");  
	ЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;   
	ЭлементОтбора.ПравоеЗначение = "НаборДанныхЗапросСхемыКомпоновкиДанных";   
	ЭлементОтбора.Использование  = Истина;
	
	ЭлементОтбора = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("НаборыДанных.Поля.ИспользоватьРеквизиты");  
	ЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;   
	ЭлементОтбора.ПравоеЗначение = Ложь;   
	ЭлементОтбора.Использование  = Истина;
		
	ПолеОформления = НовоеУО.Поля.Элементы.Добавить();       
	ПолеОформления.Поле = Новый ПолеКомпоновкиДанных("ПоляОграничениеИспользованияРеквизитовПоле");
	ПолеОформления.Использование = Истина;
	ПолеОформления = НовоеУО.Поля.Элементы.Добавить();       
	ПолеОформления.Поле = Новый ПолеКомпоновкиДанных("ПоляОграничениеИспользованияРеквизитовУсловие");
	ПолеОформления.Использование = Истина;
	ПолеОформления = НовоеУО.Поля.Элементы.Добавить();       
	ПолеОформления.Поле = Новый ПолеКомпоновкиДанных("ПоляОграничениеИспользованияРеквизитовГруппировка");
	ПолеОформления.Использование = Истина;
	ПолеОформления = НовоеУО.Поля.Элементы.Добавить();       
	ПолеОформления.Поле = Новый ПолеКомпоновкиДанных("ПоляОграничениеИспользованияРеквизитовПорядок");
	ПолеОформления.Использование = Истина;
	...
	ЭлементОтбора = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("НаборыДанных.Поля.ЭтоПолеВыбораКомпоновкиДанных");  
	ЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;   
	ЭлементОтбора.ПравоеЗначение = Истина;   
	ЭлементОтбора.Использование  = Истина;
	
	ПолеОформления = НовоеУО.Поля.Элементы.Добавить();       
	ПолеОформления.Поле = Новый ПолеКомпоновкиДанных("ПоляОграничениеИспользованияУсловие");
	ПолеОформления.Использование = Истина;
	ПолеОформления = НовоеУО.Поля.Элементы.Добавить();       
	ПолеОформления.Поле = Новый ПолеКомпоновкиДанных("ПоляОграничениеИспользованияРеквизитовУсловие");
	ПолеОформления.Использование = Истина;
...
&НаСервере
Функция ОписаниеТиповСодержитСсылку(ОписаниеТипов)
	Для Каждого Тип Из ОписаниеТипов.Типы() Цикл
		Если УИ_ОбщегоНазначения.ЭтоСсылка(Тип) Тогда
			Возврат Истина;
		КонецЕсли;		
	КонецЦикла;			
	Возврат Ложь;
