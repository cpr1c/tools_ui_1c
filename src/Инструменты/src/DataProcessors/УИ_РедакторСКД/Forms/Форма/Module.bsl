...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("СКД") Тогда
		...
		Иначе
			СКД = УИ_ОбщегоНазначения.СхемаКомпоновкиДанныхИзСтроки(Параметры.СКД);
		КонецЕсли;
...
Процедура ПоляТипЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыРедактирования = УИ_ОбщегоНазначенияКлиент.НовыйПараметрыРедактированияТипа();
	ПараметрыРедактирования.ТипДанных = ТекДанные.ТипЗначения;
	ПараметрыРедактирования.РежимЗапуска = РежимыЗапуска.ТипыДляПолейСКД;
	ДополнительныеПараметры = Новый Структура("ТекСтрока", Элементы.НаборыДанныхПоля.ТекущаяСтрока);
	Оповещение = Новый ОписаниеОповещения("ПоляТипЗначенияНачалоВыбораЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	УИ_ОбщегоНазначенияКлиент.РедактироватьТип(ПараметрыРедактирования, ЭтотОбъект, Оповещение, СтандартнаяОбработка);
...
Процедура ВычисляемыеПоляВыражениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	ЗаголовокФормы = СтрШаблон(НСтр("ru = 'Редактирование выражения ресурса для %1'"), ТекДанные.ПутьКДанным);
	Оповещение = Новый ОписаниеОповещения("ВычисляемыеПоляВыражениеЗавершение", ЭтотОбъект, ДопПараметры);
	ОткрытьФормуРедактированияВыражения(ТекДанные.Выражение, Оповещение, Ложь, ЗаголовокФормы);
...
Процедура ВычисляемыеПоляДоступныеЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения("ВычисляемыеПоляДоступныеЗначенияНачалоВыбораЗавершение", ЭтотОбъект,
		ДопПараметры);
	ЗаголовокФормы = НСтр("ru = 'Редактирование списка значений'");
	УИ_ОбщегоНазначенияКлиент.ОткрытьФормуВыбораЭлементовСпискаЗначений(ТекДанные.ДоступныеЗначения,
																		Оповещение,
																		ЗаголовокФормы,
...
Процедура ВычисляемыеПоляТипЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДополнительныеПараметры = Новый Структура("ТекСтрока", Элементы.ВычисляемыеПоля.ТекущаяСтрока);
	Оповещение = Новый ОписаниеОповещения("ВычисляемыеПоляТипЗначенияНачалоВыбораЗавершение", ЭтотОбъект,
		ДополнительныеПараметры);
	
	УИ_ОбщегоНазначенияКлиент.РедактироватьТип(ПараметрыРедактирования,
													ЭтотОбъект,
													Оповещение,
...
Процедура ПараметрыСКДТипЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыРедактирования = УИ_ОбщегоНазначенияКлиент.НовыйПараметрыРедактированияТипа();
	ПараметрыРедактирования.ТипДанных = ТекДанные.ТипЗначения;
	ПараметрыРедактирования.РежимЗапуска = РежимыЗапуска.ТипыДляПараметровСКД;
	ДополнительныеПараметры = Новый Структура("ТекСтрока", Элементы.ПараметрыСКД.ТекущаяСтрока);
	Оповещение = Новый ОписаниеОповещения("ПараметрыСКДТипЗначенияНачалоВыбораЗавершение", ЭтотОбъект,
		ДополнительныеПараметры);
	УИ_ОбщегоНазначенияКлиент.РедактироватьТип(ПараметрыРедактирования,
													ЭтотОбъект,
													Оповещение,
...
Функция СхемаКомпоновкиДанныхВСтроку(АдресХранения)
	
	Схема = ПолучитьИзВременногоХранилища(АдресХранения);
	Результат = УИ_ОбщегоНазначения.СхемаКомпоновкиДанныхВСтроку(Схема);
	Возврат Результат;
...
Процедура ЗавершитьРедактирование(Команда)
	...
	СериализованнаяСхема = СхемаКомпоновкиДанныхВСтроку(АдресСхемыКомпоновкиДанных);
	Закрыть(СериализованнаяСхема);
КонецПроцедуры
...
Процедура ИнициализироватьКомпоновщикНастроекПоСобраннойСКД()
	...
	КомпоновщикТекущихНастроек.Инициализировать(ИсточникДоступныхНастроек);
