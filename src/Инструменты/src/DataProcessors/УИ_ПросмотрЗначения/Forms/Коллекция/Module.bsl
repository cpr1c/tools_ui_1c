...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПрочитатьВходящийКонтейнерВДанныеФормы();
...
// Возможен доступ к элементу коллекции по индексу.
// 
// Параметры:
//  Коллекция - ОбъектМетаданных, Произвольный, Неопределено - Коллекция
// 
&НаСервере
Процедура УстановитьВозможностьДоступаКЭлементуКоллекцииПоИндексу(Коллекция)
	Попытка
		//@skip-check module-unused-local-variable
		НулевойЭлементКоллекции = Коллекция[0];
//		ВозможенДоступКЭлементуКоллекцииПоИндексу = Истина;
//
//		Если ВходящееЗначениеКонтейнер.ОписаниеТипа <> Неопределено Тогда
//
//		КонецЕсли;
		Для Каждого ЭлементДляКжадого Из Коллекция Цикл
			НулевойЭлементКоллекции = ЭлементДляКжадого;
			Прервать;
		КонецЦикла;

		Если ТипЗнч(НулевойЭлементКоллекции) = Тип("КлючИЗначение") Или НулевойЭлементКоллекции = Неопределено Тогда
...
&НаСервере
Процедура ПрочитатьВходящийКонтейнерВДанныеФормы()
	...
	Если ЗначениеКонтейнера = Неопределено Тогда
		УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Ошибка чтения данных контейнера значения. Возможна ошибка в обработке");
		Возврат;
	КонецЕсли;
	...
	Если КоличествоЭлементов > 0 Тогда
		УстановитьВозможностьДоступаКЭлементуКоллекцииПоИндексу(ЗначениеКонтейнера);
	КонецЕсли;
