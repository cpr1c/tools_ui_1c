...
Процедура ПолучитьЛитералОписанияТипов(ОписаниеТиповЗначения, Литерал, КодСоздания)
	...
	Если ОписаниеТиповЗначения.СодержитТип(Тип("Строка")) Тогда
		КодКвалификаторы = СтрШаблон(
			...
			ПолучитьЛитералДопустимаяДлина(ОписаниеТиповЗначения.КвалификаторыСтроки.ДопустимаяДлина), КодКвалификаторы);
	Иначе
	...
	Если ОписаниеТиповЗначения.СодержитТип(Тип("Число")) Тогда
		КодКвалификаторы = СтрШаблон(
			...
			ОписаниеТиповЗначения.КвалификаторыЧисла.ДопустимыйЗнак), КодКвалификаторы);
	Иначе
		Если ЗначениеЗаполнено(КодКвалификаторы) Тогда
			КодКвалификаторы = ", " + КодКвалификаторы;
		КонецЕсли;
	...
	Если маТипы.Количество() = 1 Тогда
		Литерал = СтрШаблон(
			"Новый ОписаниеТипов(""%1""%2)", Обработка.ПолучитьИмяТипа(маТипы[0]), КодКвалификаторы);
		Возврат;
	КонецЕсли;
	...
	Литерал = СтрШаблон(
		"Новый ОписаниеТипов(ТипыКолонки%1)", КодКвалификаторы);
...
Функция ПолучитьЛитералЗначения(Значение, ИмяПараметра = Неопределено)
	...
	ИначеЕсли ТипЗначения = Тип("Дата") Тогда
		...
		Если Ч <> 0 Или Мин <> 0 И Сек <> 0 Тогда
			КодЗначения = КодЗначения + Формат(Ч, "ЧЦ=2; ЧВН=; ЧГ=") + Формат(Мин, "ЧЦ=2; ЧВН=; ЧГ=") + Формат(Сек,
				"ЧЦ=2; ЧВН=; ЧГ=");
		КонецЕсли;
	...
	ИначеЕсли ТипЗначения = Тип("Граница") Тогда
		...
		Литерал = СтрШаблон("Новый Граница(%1, %2)", стЛитералЗначенияГраницы.Литерал, СтрШаблон("ВидГраницы.%1",
			Значение.ВидГраницы));
	ИначеЕсли ТипЗначения = Тип("МоментВремени") Тогда
		...
		Литерал = СтрШаблон("Новый МоментВремени(%1, %2)", стЛитералДаты.Литерал, стЛитералСсылки.Литерал);
...
Процедура СоздатьКодСПараметрами()
	...
	Для Каждого СтрокаПараметра Из тзПараметрыЗапроса Цикл
		...
		Если УстановитьЗначенияПараметров Тогда
			...
			КодУстановкиПараметра = СтрШаблон("	%1.УстановитьПараметр(""%2"", %3);", ИмяЗапроса, ИмяПараметра,
				стЛитерал.Литерал);
