...
Процедура ДобавитьТекстЗапроса(ДанныеТерминов)
	...
	Если Событие.Свойство("Sql", ТекстСвойства) Тогда
		...
		ТекстЗапросаВТерминах1С = ДобавитьРазделительЗапросовЕслиНеПустой(ТекстЗапросаВТерминах1С)
			+ Обработка.SQLЗапросВТермины1С(ТекстСвойства, ДанныеТерминов);
	КонецЕсли;
...
Функция ФорматироватьЧисло(Ч, Точность, ДесятичныйРазделитель = ",")
	...
	ПредставлениеЧисла = Формат(Ч, СтрШаблон("ЧЦ=%1; ЧДЦ=%2; ЧН=; ЧГ=; ЧРД=%3", Точность.Длина, Точность.Точность,
...
Процедура ДобавитьПланЗапроса_DBMSSQL(ДанныеТерминов)
	...
	ПланТекстовый.ДобавитьСтроку(
		"(rows, executes, estimate rows, estimate i/o, estimate cpu, avg. row size, totat subtree cost, estimate executions, |-- operators...)");
	ПланТекстовый.ДобавитьСтроку("");
	...
	Для й = 1 По ТекстПлан.КоличествоСтрок() Цикл
		...
		ДанныеОператора = Новый Структура("Rows, Executes, Estimate_rows, Estimate_IO, Estimate_CPU, Avg_row_size, Totat_subtree_cost, Estimate_executions, СтрокаОператоров",
			Rows, Executes, Estimate_rows, Estimate_IO, Estimate_CPU, Avg_row_size, Totat_subtree_cost,
			Estimate_executions, СтрокаОператоров);
	...
	Для Каждого стДанныеОператора Из маПданТекст Цикл
		...
		СтрокаТекстовогоПлана = СтрШаблон("%1, %2, %3, %4, %5, %6, %7, %8, %9", ФорматироватьЧисло(
			стДанныеОператора.Rows, Точность_Rows, "."), ФорматироватьЧисло(стДанныеОператора.Executes,
			Точность_Executes, "."), ФорматироватьЧисло(стДанныеОператора.Estimate_rows, Точность_Estimate_rows, "."),
			ФорматироватьЧисло(стДанныеОператора.Estimate_IO, Точность_Estimate_IO, "."), ФорматироватьЧисло(
			стДанныеОператора.Estimate_CPU, Точность_Estimate_CPU, "."), ФорматироватьЧисло(
			стДанныеОператора.Avg_row_size, Точность_Avg_row_size, "."), ФорматироватьЧисло(
			стДанныеОператора.Totat_subtree_cost, Точность_Totat_subtree_cost, "."), ФорматироватьЧисло(
			стДанныеОператора.Estimate_executions, Точность_Estimate_executions, "."),
	...
	Иначе
		...
		Если тзСтоимости.Количество() > 0 Тогда
			...
			Для Каждого Строка Из тзСтоимости Цикл
				Строка.Узел.СтоимостьОператораПроцент = СтрШаблон("%1%%", Формат(Строка.СтоимостьОператора * 100
					/ чСтоимостьВсего, "ЧЦ=5; ЧДЦ=2; ЧН="));
				Строка.Узел.СтоимостьУзлаПроцент = СтрШаблон("%1%%", Формат(Строка.СтоимостьУзла * 100
					/ чСтоимостьВсего, "ЧЦ=5; ЧДЦ=2; ЧН="));
			КонецЦикла;
...
Функция ТехнологическийЖурнал_РазобратьСобытие(Знач СтрокаТехнологическогоЖурнала)
	...
	стСобытияСложноеЗначение = Новый Структура("Sql, Prm, planSQLText, Context", "Prm, Rows, Context, planSQLText",
		"RowsAffected, planSQLText", "Context, RowsAffected");
	Для Каждого кз Из стСобытияСложноеЗначение Цикл
		...
		Если кз.Значение <> Неопределено Тогда
			...
			Для Каждого СледующееИмя Из маСледИмена Цикл
				ч = СтрНайти(СтрокаТехнологическогоЖурнала, "," + СокрЛП(СледующееИмя) + "=", ,
					чНачальнаяПозицияЗначения);
		...
		стСобытие.Вставить(кз.Ключ, Сред(СтрокаТехнологическогоЖурнала, чНачальнаяПозицияЗначения, чКонечнаяПозиция
			- чНачальнаяПозицияЗначения));

		СтрокаТехнологическогоЖурнала = Лев(СтрокаТехнологическогоЖурнала, чНачальнаяПозиция) + Прав(
			СтрокаТехнологическогоЖурнала, СтрДлина(СтрокаТехнологическогоЖурнала) - чКонечнаяПозиция);
