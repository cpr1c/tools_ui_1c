...
Процедура ПараметрыЗапросаЗначениеПриИзменении(Элемент)
	...
	ПараметрыОбработчика.ТекущееОписаниеТиповЗначения = ХранилищеОписанияТиповСтрокиПараметра(ТекДанные);
...
&НаКлиенте
Процедура ПараметрыЗапросаТипЗначенияПриИзменении(Элемент)
	ТекДанные = Элементы.ПараметрыЗапроса.ТекущиеДанные;
	Если ТекДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПараметрыЗапросаТипЗначенияОчистка(Элемент, СтандартнаяОбработка)
	ТекДанные = Элементы.ПараметрыЗапроса.ТекущиеДанные;
	Если ТекДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПараметрыЗапросаТипЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения("ПараметрыЗапросаТипЗначенияНачалоВыбораЗавершение", ЭтотОбъект,
...
Процедура УИ_ЗаменитьЗапросыКПредставлениямВиртуальныхТаблиц(Команда)
	...
	Если Не ПустаяСтрока(ТекущийТекстЗапроса) Тогда
		...
		УИ_РедакторКодаКлиент.УстановитьТекстРедактораЭлементаФормы(ЭтотОбъект, Элементы.ТекстЗапроса, Запрос.Текст);
		...
		Для Каждого Параметр Из Запрос.Параметры Цикл
			...
			Если ПараметрыЗапроса.НайтиСтроки(ПараметрыОтбора).Количество() = 0 Тогда
				ЗаполнитьСтрокуПараметраЗапросаПоСтруктуреПараметровДляПакета(ПараметрыЗапроса.Добавить(), Параметр);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура УИ_ЗаполнитьДаннымиОтладки()
	...
	Если ДанныеОтладки.Свойство("Параметры") Тогда
		Для Каждого ТекПараметр Из ДанныеОтладки.Параметры Цикл
			...
			НовыйПараметр=НовыйСтруктураПараметраДляПакетаЗапросов();
			НовыйПараметр.Использовать = Истина;
			НовыйПараметр.Имя = ТекПараметр.Ключ;
			НовыйПараметр.ТипЗначения = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеВСтруктуруДанныхДляПоляСКонтейнером(ТипЗнч(ТекПараметр.Значение));
			НовыйПараметр.ТипЗначения = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеВСтруктуруДанныхДляПоляСКонтейнером(ТекПараметр.Значение);
...
Процедура УИ_ЗаполнитьЗапросыИзСКД()
	...
	Для Каждого ДанныеЗапроса Из МассивЗапросовИзСКД Цикл
		...
		Если ДанныеЗапроса.Свойство("Параметры") Тогда
			...
			КонецЦикла;
		КонецЕсли;
		Если ДанныеЗапроса.Свойство("ВременныеТаблицы") Тогда
			НоваяСтрока.ВременныеТаблицы=Новый СписокЗначений;

			Для Каждого КлючЗначение Из ДанныеЗапроса.ВременныеТаблицы Цикл
				ВременнаяТаблица=Новый Структура;
				ВременнаяТаблица.Вставить("Имя", КлючЗначение.Ключ);
				ВременнаяТаблица.Вставить("Контейнер",
										  ОбработкаОбъект.Контейнер_СохранитьЗначение(КлючЗначение.Значение));
				ВременнаяТаблица.Вставить("Значение", ВременнаяТаблица.Контейнер.Представление);
...
Процедура СтруктураЗаписиРезультата_ЗаполнитьСтруктуруЗаписи(рзВыборка = Неопределено)
	...
	СтруктураХраненияТипа = СтруктураХраненияТипаЗначенияПараметраЗапроса();
	Для Каждого СтрокаПараметра Из ПараметрыЗапроса Цикл
		...
		ЭлементСтруктуры.Тип = УИ_ОбщегоНазначенияКлиентСервер.СтруктураЗначенияПоДаннымФормыПоляСКонтейнером(СтрокаПараметра,
																											  СтруктураХраненияТипа);
																											  
		ОписаниеТиповТип = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеПоляСКонтейнеромЗначенияПоСтруктуреДанных(ЭлементСтруктуры.Тип);
		ЭлементСтруктуры.Картинка = Обработка.ПолучитьКартинкуПоТипу(Обработка.ТипБезПустых(ОписаниеТиповТип), Картинки);
...
Процедура УстановитьИмяФайлаЗапросов(стрПолноеИмя = "")
	...
	Если ЗначениеЗаполнено(ИмяФайлаЗапросов) Тогда
		...
		Заголовок = ПакетЗапросов_ИмяДляОтображения;
	Иначе
...
// Параметры запроса в список значений.
// 
// Параметры:
//  ПараметрыДанныеФормыКоллекция - ДанныеФормыКоллекция: см. Обработка.УИ_КонсольЗапросов.Форма.Форма.ПараметрыЗапроса
// 
// Возвращаемое значение:
//  СписокЗначений из см. НовыйСтруктураПараметраДляПакетаЗапросов- Параметры запроса в список значений
&НаКлиенте
Функция ПараметрыЗапросаВСписокЗначений(ПараметрыДанныеФормыКоллекция)

	СтруктураХраненияТипа = СтруктураХраненияТипаЗначенияПараметраЗапроса();
	СтруктураХраненияЗначения = СтруктураХраненияЗначенияПараметраЗапроса();
	
	СписокПараметрыЗапроса = Новый СписокЗначений;
	Для Каждого СтрокаПараметра Из ПараметрыДанныеФормыКоллекция Цикл
		ПараметрДляСписка = НовыйСтруктураПараметраДляПакетаЗапросов();
		ПараметрДляСписка.Имя = СтрокаПараметра.Имя;
		ПараметрДляСписка.Использовать = СтрокаПараметра.Использовать;

		ПараметрДляСписка.ТипЗначения = УИ_ОбщегоНазначенияКлиентСервер.СтруктураЗначенияПоДаннымФормыПоляСКонтейнером(СтрокаПараметра,
																													   СтруктураХраненияТипа);

		ПараметрДляСписка.Значение = УИ_ОбщегоНазначенияКлиентСервер.СтруктураЗначенияПоДаннымФормыПоляСКонтейнером(СтрокаПараметра,
																													   СтруктураХраненияЗначения);

		СписокПараметрыЗапроса.Добавить(ПараметрДляСписка);
	КонецЦикла;

	Возврат СписокПараметрыЗапроса;

КонецФункции

// Параметры запроса из списка значений.
// 
// Параметры:
//  СписокПараметрыЗапроса - СписокЗначений из см. НовыйСтруктураПараметраДляПакетаЗапросов - Сз параметры
//  ПараметрыДанныеФормыКоллекция -  ДанныеФормыКоллекция: см. Обработка.УИ_КонсольЗапросов.Форма.Форма.ПараметрыЗапроса - Параметры данные формы коллекция
&НаКлиенте
Процедура ПараметрыЗапросаИзСпискаЗначений(СписокПараметрыЗапроса, ПараметрыДанныеФормыКоллекция)

	ПараметрыДанныеФормыКоллекция.Очистить();

	Если СписокПараметрыЗапроса = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	
	Для Каждого ЭлементСписка Из СписокПараметрыЗапроса Цикл
		ПараметрДляПакетаЗапроса = ЭлементСписка.Значение;

		НоваяСтрокаПараметров = ПараметрыДанныеФормыКоллекция.Добавить();
		ЗаполнитьСтрокуПараметраЗапросаПоСтруктуреПараметровДляПакета(НоваяСтрокаПараметров, ПараметрДляПакетаЗапроса);
	КонецЦикла;
...
// Новый структура параметра для пакета запросов.
// 
// Возвращаемое значение:
//  Структура - Новый структура параметра для пакета запросов:
// * Имя - Строка - 
// * Использовать - Булево - 
// * ТипЗначения 	- Неопределено - 
// 					- см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСтруктураДанныхДляПоляСКонтейнеромФормы
// * Значение - Неопределено - 
// 					- см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСтруктураДанныхДляПоляСКонтейнеромФормы
&НаКлиентеНаСервереБезКонтекста
Функция НовыйСтруктураПараметраДляПакетаЗапросов()
	Данные = Новый Структура;
	Данные.Вставить("Имя", "");
	Данные.Вставить("Использовать", Ложь);
	Данные.Вставить("ТипЗначения", Неопределено);
...
// Структура хранения значения параметра запроса.
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСтруктураХраненияРеквизитаНаФормеСКонейнером
&НаКлиентеНаСервереБезКонтекста
Функция СтруктураХраненияЗначенияПараметраЗапроса()
	Возврат УИ_ОбщегоНазначенияКлиентСервер.НовыйСтруктураХраненияРеквизитаНаФормеСКонейнером("Значение");
КонецФункции

// Структура хранения типа значения параметра запроса.
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСтруктураХраненияРеквизитаНаФормеСКонейнером
&НаКлиентеНаСервереБезКонтекста
Функция СтруктураХраненияТипаЗначенияПараметраЗапроса()
...
Функция ПараметрыЗапроса_ПолучитьЗначение(СтрокаИд)
	...
	СтруктураХраненияЗначения = СтруктураХраненияЗначенияПараметраЗапроса();
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ЗначениеПоляСКонтейнеромЗначения(СтрокаПараметр, СтруктураХраненияЗначения);

КонецФункции

// Заполнить строку параметра запроса по структуре параметров для пакета.
// 
// Параметры:
//  СтрокаПараметра- ДанныеФормыЭлементКоллекции: см. Обработка.УИ_КонсольЗапросов.Форма.Форма.ПараметрыЗапроса
//  ПараметрДляПакетаЗапросов - см. НовыйСтруктураПараметраДляПакетаЗапросов
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьСтрокуПараметраЗапросаПоСтруктуреПараметровДляПакета(СтрокаПараметра, ПараметрДляПакетаЗапросов)
	СтруктураХраненияТипа = СтруктураХраненияТипаЗначенияПараметраЗапроса();
	СтруктураХраненияЗначения = СтруктураХраненияЗначенияПараметраЗапроса();

	СтрокаПараметра.Имя = ПараметрДляПакетаЗапросов.Имя;
	СтрокаПараметра.Использовать = ПараметрДляПакетаЗапросов.Использовать;

	УИ_ОбщегоНазначенияКлиентСервер.УстановитьЗначениеПоляСКонтейнеромПоСтруктуреДанныхДляПоляСКонтейнером(СтрокаПараметра,
																										   СтруктураХраненияТипа,
																										   ПараметрДляПакетаЗапросов.ТипЗначения);
	УИ_ОбщегоНазначенияКлиентСервер.УстановитьЗначениеПоляСКонтейнеромПоСтруктуреДанныхДляПоляСКонтейнером(СтрокаПараметра,
																										   СтруктураХраненияЗначения,
																										   ПараметрДляПакетаЗапросов.Значение);
...
Процедура ПараметрыЗапроса_СохранитьЗначение(СтрокаИд, Знач Значение)
	...
	СтруктураХраненияЗначения = СтруктураХраненияЗначенияПараметраЗапроса();
	УИ_ОбщегоНазначенияКлиентСервер.УстановитьЗначениеПоляСКонтейнером(СтрокаПараметр,
																	   СтруктураХраненияЗначения,
																	   Значение);
...
// 	Неопределено
&НаКлиентеНаСервереБезКонтекста
Функция ХранилищеОписанияТиповСтрокиПараметра(ДанныеСтроки)
	СтруктураХранения = СтруктураХраненияТипаЗначенияПараметраЗапроса();
...
Функция ПакетЗапросов_СтрокиВМассив(Строки)
	...
	Для Каждого Элемент Из Строки.ПолучитьЭлементы() Цикл
		стЭлемент = Новый Структура("Имя, ТекстЗапроса, ТекстКод, МетодИсполненияКода, ВременныеТаблицы, Строки, Инфо,
		...
		стЭлемент.Вставить("ПараметрыЗапроса", Новый Массив);
		
		Для Каждого ТекПараметрЭлемент ИЗ Элемент.ПараметрыЗапроса Цикл
			ТекПараметр = ТекПараметрЭлемент.Значение;
			СохраяемыйПараметр = НовыйСтруктураПараметраДляПакетаЗапросов();
			СохраяемыйПараметр.Имя = ТекПараметр.Имя;
			СохраяемыйПараметр.Использовать = ТекПараметр.Использовать;
			СохраяемыйПараметр.Значение = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеПоляСКонтейнеромЗначенияПоСтруктуреДанных(ТекПараметр.Значение);
			СохраяемыйПараметр.ТипЗначения = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеПоляСКонтейнеромЗначенияПоСтруктуреДанных(ТекПараметр.ТипЗначения);
			
			стЭлемент.ПараметрыЗапроса.Добавить(СохраяемыйПараметр);
...
&НаСервере
Функция ПакетЗапросов_СохранитьНаСервере(Знач стДанныеДляЗаписи, фТолькоЗаголовок)
...
Процедура ПакетЗапросов_Сохранить(Оповещение, стрИмяФайла, фТолькоЗаголовок = Ложь)
	...
	ДанныеДляЗаписи = ПакетЗапросов_СохранитьНаСервере(стДанныеДляЗаписи, фТолькоЗаголовок);
...
Функция ПакетЗапросов_ЗагрузитьНаСервере(Знач стрЗагруженныеДанные)
	ПоддержкаРегулярныхВыражений = УИ_ОбщегоНазначенияКлиентСервер.ВерсияПлатформыНеМладше("8.3.23");
	
	КоличествоСекцийПараметров = СтрЧислоВхождений(стрЗагруженныеДанные, "ПараметрыЗапроса");	
	стЗагруженныеДанные = Новый Структура;
	Чтение = Новый ЧтениеXML;                     
	
	Для КоличествоПопыток = 1 По КоличествоСекцийПараметров + 1 Цикл
		...
		Исключение  
			...
			Пока Чтение.Прочитать() Цикл  
				Атрибут = Чтение.ПолучитьАтрибут(0);
				Если Чтение.ТипУзла = ТипУзлаXML.НачалоЭлемента
						И Атрибут = "http://v8.1c.ru/8.1/data/enterprise/current-config"
						Тогда        
					АтрибутПараметра = Чтение.ПолучитьАтрибут(1);
					СтрПараметр = Прав(АтрибутПараметра, СтрДлина(АтрибутПараметра) - СтрНайти(АтрибутПараметра, ":"));
					Прервать;
				КонецЕсли;
			КонецЦикла;	                                     
			Если ПоддержкаРегулярныхВыражений Тогда   
				УдалитьСтрокуПараметраВФайлеЗапроса(стрЗагруженныеДанные, СтрПараметр);
			Иначе
				УдалитьСтрокуПараметраВФайлеЗапросаСовместимостьДо8_3_23(стрЗагруженныеДанные, СтрПараметр);			
			КонецЕсли;			
		КонецПопытки;
	КонецЦикла;
	Чтение.Закрыть();
...
&НаСервереБезКонтекста
Процедура УдалитьСтрокуПараметраВФайлеЗапроса(стрЗагруженныеДанные, Знач СтрПараметр) 
	СтрПараметр = СтрЗаменить(СтрПараметр, ".", "\.");
	Паттерн = СтрШаблон("<Type xmlns.*%1</Type>", СтрПараметр);
	
	Параметры = Новый Структура;
	Параметры.Вставить("стрЗагруженныеДанные", стрЗагруженныеДанные);
	Параметры.Вставить("Паттерн", Паттерн);
	стрЗагруженныеДанные = УИ_ОбщегоНазначения.ВычислитьВБезопасномРежиме(
							"СтрЗаменитьПоРегулярномуВыражению(ПараметрыМетода.стрЗагруженныеДанные, ПараметрыМетода.Паттерн, """")",
							Параметры);
КонецПроцедуры

&НаСервереБезКонтекста
Процедура УдалитьСтрокуПараметраВФайлеЗапросаСовместимостьДо8_3_23(стрЗагруженныеДанные, Знач СтрПараметр)
	Текст = Новый ТекстовыйДокумент;
	Текст.УстановитьТекст(стрЗагруженныеДанные);
	
	Для НомерСтроки = 1 По Текст.КоличествоСтрок() Цикл
		ТекущаяСтрока = СокрЛП(Текст.ПолучитьСтроку(НомерСтроки));
		Если СтрНачинаетсяС(ТекущаяСтрока, "<Type") И СтрНайти(ТекущаяСтрока, СтрПараметр) > 0 Тогда
			Текст.УдалитьСтроку(НомерСтроки);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	стрЗагруженныеДанные = Текст.ПолучитьТекст();
КонецПроцедуры
...
// Параметры запроса тип значения начало выбора завершение.
// 
// Параметры:
...
//   	* ТекСтрока - Число
&НаКлиенте
Процедура ПараметрыЗапросаТипЗначенияНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	ПараметрыЗапросаТипЗначенияПриИзмененииНаСервере(ДополнительныеПараметры.ТекСтрока);
КонецПроцедуры
 
// Параметры запроса тип значения при изменении на сервере.
// 
// Параметры:
//  ИдентификаторСтроки - Число - Идентификатор строки
&НаСервере
Процедура ПараметрыЗапросаТипЗначенияПриИзмененииНаСервере(ИдентификаторСтроки)
	СтрокаПараметра = ПараметрыЗапроса.НайтиПоИдентификатору(ИдентификаторСтроки);
	Если СтрокаПараметра = Неопределено Тогда
		Возврат;
	КонецЕсли;

	СтруктураХраненияТипа = СтруктураХраненияТипаЗначенияПараметраЗапроса();
	ОписаниеТиповПараметра = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеПоляСКонтейнеромЗначения(СтрокаПараметра,
																							  СтруктураХраненияТипа);

	СтруктураХраненияЗначения = СтруктураХраненияЗначенияПараметраЗапроса();
	ЗначениеПоля = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеПоляСКонтейнеромЗначения(СтрокаПараметра,
																					СтруктураХраненияЗначения);

	НовоеЗначениеПоля = ОписаниеТиповПараметра.ПривестиЗначение(ЗначениеПоля);

	Если ЗначениеПоля <> НовоеЗначениеПоля Или ТипЗнч(ЗначениеПоля) <> ТипЗнч(НовоеЗначениеПоля) Тогда
		УИ_ОбщегоНазначенияКлиентСервер.УстановитьЗначениеПоляСКонтейнером(СтрокаПараметра,
																		   СтруктураХраненияЗначения,
																		   НовоеЗначениеПоля);
	КонецЕсли;
...
Процедура ЗагрузитьПакетЗапросов(ДополнительныеПараметры = Неопределено) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыбораФайлаДляЗагрузкиПакетаЗапросов", ЭтотОбъект,
		ДополнительныеПараметры);
...
Функция ПараметрыЗаполнитьИзЗапросаНаСервере(ТекущийТекстЗапроса)
	...
	Для Каждого Параметр Из НайденныеПараметры Цикл
		...
		НайденныеСтрокиПараметра = ПараметрыЗапроса.НайтиСтроки(Новый Структура("Имя", Параметр.Имя));
		Если НайденныеСтрокиПараметра.Количество() > 0 Тогда
			СтрокаПараметра = НайденныеСтрокиПараметра[0];
		Иначе
			СтрокаПараметра = ПараметрыЗапроса.Добавить();
		...
		СтруктураХраненияТипаЗначения = СтруктураХраненияТипаЗначенияПараметраЗапроса();
		ОписаниеТипаСтрокиПараметра = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеПоляСКонтейнеромЗначения(СтрокаПараметра,
																									   СтруктураХраненияТипаЗначения);
		Если Не ЗначениеЗаполнено(ОписаниеТипаСтрокиПараметра) Тогда
			УИ_ОбщегоНазначенияКлиентСервер.УстановитьЗначениеПоляСКонтейнером(СтрокаПараметра,
																			   СтруктураХраненияТипаЗначения,
...
Процедура УИ_ДобавитьПараметрыЗапросаИзПредставления(Знач ТекстЗапросаПредставления)
	...
	Для Каждого ПараметрЗапросаФормы Из ПараметраЗапросаФормы Цикл
		...
		Если ПараметрыЗапроса.НайтиСтроки(ПараметрыОтбора).Количество() = 0 Тогда
			ЗаполнитьСтрокуПараметраЗапросаПоСтруктуреПараметровДляПакета(ПараметрыЗапроса.Добавить(),
																		  ПараметрЗапросаФормы);
		КонецЕсли;
	КонецЦикла;
...
// 
// Возвращаемое значение:
//  Массив Из см. НовыйСтруктураПараметраДляПакетаЗапросов
&НаСервере
Функция УИ_ПараметрыЗапросаВПараметрыЗапросаФормы(Знач пПараметрыЗапроса)
	...
	Для Каждого ЭлементКоллекции Из пПараметрыЗапроса Цикл
		Если ТипЗнч(ЭлементКоллекции) = Тип("ОписаниеПараметраЗапроса") Тогда
			...
			ТипПараметра = ЭлементКоллекции.ТипЗначения;
		...
		ПараметрЗапроса = НовыйСтруктураПараметраДляПакетаЗапросов();
		ПараметрЗапроса.Использовать = Истина;
		ПараметрЗапроса.Имя = ИмяПараметра;
		ПараметрЗапроса.ТипЗначения = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеВСтруктуруДанныхДляПоляСКонтейнером(ТипПараметра);
		ПараметрЗапроса.Значение = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеВСтруктуруДанныхДляПоляСКонтейнером(ЗначениеПараметра);
