...
Процедура ОткрытьНастройки(Команда)
	...
	ПараметрыФормыНастроек.Вставить("НастройкиКонсоли", НастройкиКонсоли());

	Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияНастроекКонсоли", ЭтотОбъект);
	ОткрытьФорму("Обработка.УИ_КонсольЗапросов.Форма.ФормаНастройкиКонсолиЗапросов", ПараметрыФормыНастроек, ЭтотОбъект,,,,
...
// Возвращает настройки консоли запросов, собранные из реквизитов формы и реквизитов объекта обработки.
//
// Возвращаемое значение:
//  Структура:
//   * ОпцияАвтосохранениеПередВыполнениемЗапроса - Булево - признак автосохранения перед выполнением запроса
//   * ОпцияИнтервалАвтосохранения - Число - интервал автосохранения, секунд
//   * ОпцияСохранятьКомментарии - Булево - признак сохранения комментариев к запросам
//   * ОпцияОбрабатывать__ - Булево - признак обработки результата запроса
//   * ОпцияИнтервалОбновленияВыполненияАлгоритма - Число - интервал обновления выполнения алгоритма, секунд
//
&НаКлиенте
Функция НастройкиКонсоли()

	ПараметрыКонсоли = Новый Структура;
	ПараметрыКонсоли.Вставить("ОпцияАвтосохранениеПередВыполнениемЗапроса", ОпцияАвтосохранениеПередВыполнениемЗапроса);
	ПараметрыКонсоли.Вставить("ОпцияИнтервалАвтосохранения", ОпцияИнтервалАвтосохранения);
	ПараметрыКонсоли.Вставить("ОпцияСохранятьКомментарии", ОпцияСохранятьКомментарии);
	ПараметрыКонсоли.Вставить("ОпцияОбрабатывать__", Объект.ОпцияОбрабатывать__);
	ПараметрыКонсоли.Вставить("ОпцияИнтервалОбновленияВыполненияАлгоритма",
		 Объект.ОпцияИнтервалОбновленияВыполненияАлгоритма);

	Возврат ПараметрыКонсоли;

КонецФункции

// Применяет измененные настройки консоли запросов к реквизитам формы и реквизитам объекта
// после закрытия формы настроек.
//
// Параметры:
//   Результат - Структура, Неопределено - новые значения настроек консоли. Неопределено,
//             если пользователь закрыл форму настроек без сохранения изменений
//   ДополнительныеПараметры - Произвольный - произвольные параметры обработчика,
//             переданные при открытии формы настроек
//
&НаКлиенте
Процедура ПослеЗакрытияНастроекКонсоли(Результат, ДополнительныеПараметры) Экспорт

	Если Результат = Неопределено ИЛИ ТипЗнч(Результат) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;

	ТекущиеНастройки = НастройкиКонсоли();

	НастройкиОбъекта = Новый Массив();
	НастройкиОбъекта.Добавить("ОпцияИнтервалОбновленияВыполненияАлгоритма");
	НастройкиОбъекта.Добавить("ОпцияОбрабатывать__");

	Для каждого КлючЗначение Из ТекущиеНастройки Цикл

		Если КлючЗначение.Значение <> Результат[КлючЗначение.Ключ] Тогда

			Если НастройкиОбъекта.Найти(КлючЗначение.Ключ) <> Неопределено Тогда
				Объект[КлючЗначение.Ключ] = Результат[КлючЗначение.Ключ];
			Иначе
				ЭтотОбъект[КлючЗначение.Ключ] = Результат[КлючЗначение.Ключ];
			КонецЕсли;

			Модифицированность = Истина;
			Если КлючЗначение.Ключ = "ОпцияИнтервалАвтосохранения" Тогда
		...
		Если ДанныеЗапроса.Свойство("Параметры") Тогда
			Для Каждого ТекПараметр Из ДанныеЗапроса.Параметры Цикл
				...
				ИначеЕсли НовыйПараметр.ТипКонтейнера = 3 Тогда
					НовыйПараметр.Вставить("ТипЗначения", "Таблица значений");
					НовыйПараметр.Вставить("Значение", НовыйПараметр.Контейнер.Представление);
				ИначеЕсли ТипЗнч(ТекПараметр.Значение) = Тип("ВыражениеКомпоновкиДанных") Тогда
					Попытка
						НовоеЗначение = УИ_ОбщегоНазначения.ВычислитьВБезопасномРежиме(Строка(ТекПараметр.Значение));
					Исключение
						ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось вычислить выражение ""%1"" для параметра ""%2"" в запросе ""%3"".'"),
					...
					КонецПопытки;
					НовыйПараметр.Вставить("ТипЗначения", ОписаниеТиповПоТипу(ТипЗнч(НовоеЗначение)));
					НовыйПараметр.Вставить("Контейнер", НовоеЗначение);
					НовыйПараметр.Вставить("Значение", НовоеЗначение);
				Иначе
					НовыйПараметр.Вставить("ТипЗначения", ОписаниеТиповПоТипу(ТипЗнч(ТекПараметр.Значение)));
					НовыйПараметр.Вставить("Значение", НовыйПараметр.Контейнер);
