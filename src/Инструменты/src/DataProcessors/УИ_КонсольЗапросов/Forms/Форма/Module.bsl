...
Процедура ПоместитьРедактируемыйЗапрос()
	...
	Если РедактируемыйЗапрос >= 0 Тогда
		...
		стрТекстАлгоритм = ТекстАлгоритм;
...
Процедура ИзвлечьРедактируемыйЗапрос(ЗапросКроме = Неопределено, ВосстановитьПозийиюРедактирования = Истина)
	...
	ТекстАлгоритм = стДанныеЗапроса.ТекстКод;
...
Функция ЗапуститьОбработкуНаСервере(фПострочно = Истина)
	...
	ПараметрыВыполнения.Добавить(ТекстАлгоритм);
...
Функция ВыполнитьАлгоритм()
	...
	Попытка
		ВыполнитьКод(ТекстАлгоритм, выбВыборка, стРезультатЗапроса.Параметры);
	Исключение
...
Функция ВыполнитьАлгоритмПострочноСИндикацией()
	...
	Пока Истина Цикл
		...
		стРезультат = ВыполнитьАлгоритмНаСервереПострочно(АдресСостояния, АдресРезультатаЗапроса, РезультатВПакете,
			ТекстАлгоритм, Объект.ОпцияИнтервалОбновленияВыполненияАлгоритма);
