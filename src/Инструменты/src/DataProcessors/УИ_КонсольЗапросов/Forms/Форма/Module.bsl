...
Процедура ЗаполнитьПараметрИзТабличногоДокумента(Команда)
	...
	ОписаниеОповещенияОЗакрытииОткрываемойФормы = Новый ОписаниеОповещения("ВводДанныхВТабличноеПолеЗакрытиеФормы", ЭтаФорма);
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ВнешнийИсточникДанных", Истина);
	ОткрытьФорму(ПолноеИмяФормы("ФормаТабличногоДокумента"), ПараметрыОткрытия, ЭтаФорма,,,,ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаСервере
Функция ИзвлечьРезультатКакКонтейнерДляЗагрузкиДанныхЧерезТабличныйДокумент(Массив) 
