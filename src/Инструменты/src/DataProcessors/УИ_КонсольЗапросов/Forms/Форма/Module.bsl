...
&НаСервере
Функция ВыполнитьЗапросНаСервере(ТекстЗапросаДляВыполнения)
	Перем НомерСтроки, НомерКолонки;
	...
	Попытка
		...
		СхемаЗапроса.УстановитьТекстЗапроса(ТекстЗапросаДляВыполнения);
	Исключение
		СтрокаОшибки = ОписаниеОшибки();
...
Функция Запрос_ПолучитьДанныеЗапроса(идЗапрос)
	...
	Возврат Новый Структура("Имя, Запрос, ТекстКод, МетодИсполненияКода, Параметры, ВременныеТаблицы, ВКонструкторе, КурсорНачалоСтроки, КурсорНачалоКолонки, КурсорКонецСтроки, КурсорКонецКолонки, КодКурсорНачалоСтроки, КодКурсорНачалоКолонки, КодКурсорКонецСтроки, КодКурсорКонецКолонки, ЗапросОригинальный, ТекстКодОригинальный",
...
Процедура ВыполнитьЗапрос(фИспользоватьВыделение)
	...
	ГраницыВыделенияЗапроса = ГраницыВыделенияЗапроса();
	фВесьТекст = Не фИспользоватьВыделение
...
&НаКлиенте
Процедура УИ_РезультатСвойствоСтроки(Команда)
	
	Если ВидРезультата = "таблица" Тогда
		Элементы.УИ_РезультатЗапросаСвойствоСтроки.Пометка = НЕ Элементы.УИ_РезультатЗапросаСвойствоСтроки.Пометка;
		фВидимостьСвойстваСтроки = Элементы.УИ_РезультатЗапросаСвойствоСтроки.Пометка;
	ИначеЕсли ВидРезультата = "дерево" Тогда
		Элементы.УИ_РезультатЗапросаДеревоСвойствоСтроки.Пометка = НЕ Элементы.УИ_РезультатЗапросаДеревоСвойствоСтроки.Пометка;
		фВидимостьСвойстваСтроки = Элементы.УИ_РезультатЗапросаДеревоСвойствоСтроки.Пометка;
	Иначе
		фВидимостьСвойстваСтроки = Ложь;
	КонецЕсли;
	Элементы.УИ_СвойстваСтроки.Видимость = фВидимостьСвойстваСтроки;
	Если фВидимостьСвойстваСтроки Тогда
...
Процедура УИ_АктивизироватьСтрокуСвойстваСтроки(ИмяИсточника)
	...
	Если ТекущиеДанные = Неопределено 
		ИЛИ НЕ Элементы["УИ_" + ИмяИсточника + "СвойствоСтроки"].Пометка Тогда
		Возврат;
	КонецЕсли;
