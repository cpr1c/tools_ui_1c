...
Функция ПакетЗапросов_ЗагрузитьНаСервере(Знач стрЗагруженныеДанные)
	сообщить("начальная длина: "+СтрДлина(стрЗагруженныеДанные));
		...
		Исключение  
			...
			Пока Чтение.Прочитать() Цикл  
				...
				Если Чтение.ТипУзла = ТипУзлаXML.НачалоЭлемента
					...
					ЗначениеПараметра = Прав(АтрибутПараметра, СтрДлина(АтрибутПараметра) - СтрНайти(АтрибутПараметра, ":"));
					Прервать;
				КонецЕсли;
			КонецЦикла;	                                     
		   ...
		   ВерсияПриложения = СистемнаяИнформация.ВерсияПриложения;
			...
			Если ВерсияПриложения < "8.3.23" Тогда
				//стрЗагруженныеДанные = УИ_РегулярныеВыраженияКлиентСервер.Заменить(стрЗагруженныеДанные, Паттерн, "");
				УдалитьПараметрВФайлеЗапроса(стрЗагруженныеДанные, ЗначениеПараметра);			
			Иначе
				УдалитьСтрокуПараметраВФайлеЗапроса(стрЗагруженныеДанные, ЗначениеПараметра);
			КонецЕсли;
			
		КонецПопытки;
	КонецЦикла;
	Чтение.Закрыть();
	сообщить("конечная длина: "+СтрДлина(стрЗагруженныеДанные));
	Возврат стЗагруженныеДанные;

КонецФункции 

&НаСервереБезКонтекста
Процедура УдалитьСтрокуПараметраВФайлеЗапроса(стрЗагруженныеДанные, Знач ЗначениеПараметра) 
	ЗначениеПараметра = СтрЗаменить(ЗначениеПараметра, ".", "\.");
	Паттерн = СтрШаблон("<Type xmlns.*%1</Type>", ЗначениеПараметра);
	
	КодДляВыполнения = "
		|Результат = СтрНайтиПоРегулярномуВыражению(Параметры.стрЗагруженныеДанные, Параметры.Паттерн, , , , Истина);
		|Если Не ПустаяСтрока(Результат.Значение) Тогда
		|    Параметры.стрЗагруженныеДанные = СтрЗаменить(Параметры.стрЗагруженныеДанные, Результат.Значение, """");
		|КонецЕсли;"; 
	...
	стрЗагруженныеДанные = Параметры.стрЗагруженныеДанные;
КонецПроцедуры

&НаСервереБезКонтекста
Процедура УдалитьСтрокуПараметраВФайлеЗапроса_Тест(стрЗагруженныеДанные, Знач ЗначениеПараметра) 
	ЗначениеПараметра = СтрЗаменить(ЗначениеПараметра, ".", "\.");
	Паттерн = СтрШаблон("<Type xmlns.*%1</Type>", ЗначениеПараметра);
...
&НаСервереБезКонтекста
Функция УдалитьПараметрВФайлеЗапроса(стрЗагруженныеДанные, ЗначениеПараметра)
	Текст = Новый ТекстовыйДокумент;
	Текст.УстановитьТекст(стрЗагруженныеДанные);
	
	Для НомерСтроки = 1 По Текст.КоличествоСтрок() Цикл
		ТекущаяСтрока = СокрЛП(Текст.ПолучитьСтроку(НомерСтроки));
		Если СтрНачинаетсяС(ТекущаяСтрока, "<Type") И СтрНайти(ТекущаяСтрока, ЗначениеПараметра) > 0 Тогда
