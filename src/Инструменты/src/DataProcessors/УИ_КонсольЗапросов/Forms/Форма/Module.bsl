...
&НаКлиенте
Процедура ЗаполнитьПараметрИзТабличногоДокумента(Команда)
	...
	ОткрытьФорму(ПолноеИмяФормы("ФормаТабличногоДокумента"), ПараметрыОткрытия, ЭтотОбъект, , , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВводДанныхВТабличноеПолеЗакрытиеФормы(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Таблица = ИзвлечьРезультатКакКонтейнерДляЗагрузкиДанныхЧерезТабличныйДокумент(РезультатЗакрытия.МассивСДанными);	
	
	ПараметрыОповещения = Новый Структура("Таблица, Строка, Поле", "ПараметрыЗапроса", Неопределено, "ТипЗначения");
	ОписаниеОповещенияОЗакрытииОткрываемойФормы = Новый ОписаниеОповещения("ОкончаниеРедактированияСтроки", ЭтотОбъект,
		ПараметрыОповещения);
	ПараметрыОткрытия = Новый Структура("Объект, ТипЗначения, ТипКонтейнера, Имя, ВЗапросРазрешено, ВПараметр", Объект,
		Таблица, 3, , Ложь, Истина);
...
&НаСервере
Функция ИзвлечьРезультатКакКонтейнерДляЗагрузкиДанныхЧерезТабличныйДокумент(Знач Массив) 
	
	Обработка = РеквизитФормыВЗначение("Объект");
	
	ТаблицаЗначений = УИ_ОбщегоНазначения.МассивСтруктурВТаблицуЗначений(Массив);
	
	Обработка.ТаблицаЗначений_УдалитьТипNull(ТаблицаЗначений);
		
	Возврат Обработка.Контейнер_СохранитьЗначение(ТаблицаЗначений);	
