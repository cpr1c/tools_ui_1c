...
Процедура ПараметрыЗапросаИзСпискаЗначений(СписокПараметрыЗапроса, ПараметрыДанныеФормыКоллекция)
	...
	Для Каждого ЭлементСписка Из СписокПараметрыЗапроса Цикл
		...
		НоваяСтрокаПараметров.Имя = ПараметрДляПакетаЗапроса.Имя;
		НоваяСтрокаПараметров.Использовать = ПараметрДляПакетаЗапроса.Использовать;

		УИ_ОбщегоНазначенияКлиентСервер.УстановитьЗначениеПоляСКонтейнеромПоСтруктуреДанныхДляПоляСКонтейнером(НоваяСтрокаПараметров,
																											   СтруктураХраненияТипа,
																											   ПараметрДляПакетаЗапроса.ТипЗначения);
		УИ_ОбщегоНазначенияКлиентСервер.УстановитьЗначениеПоляСКонтейнеромПоСтруктуреДанныхДляПоляСКонтейнером(НоваяСтрокаПараметров,
																											   СтруктураХраненияЗначения,
																											   ПараметрДляПакетаЗапроса.Значение);
	КонецЦикла;
