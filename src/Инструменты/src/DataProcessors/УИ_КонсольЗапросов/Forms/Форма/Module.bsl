	...
	Для КоличествоПопыток = 1 По КоличествоСекцийпараметров + 1 Цикл
		...
		Исключение  
			...
			Если ВерсияПриложения < "8.3.23" Тогда
				УдалитьСтрокуПараметраВФайлеЗапросаДо8323(стрЗагруженныеДанные, СтрПараметр);			
			Иначе
...
&НаСервереБезКонтекста
Функция УдалитьСтрокуПараметраВФайлеЗапросаДо8323(стрЗагруженныеДанные, СтрПараметр)
