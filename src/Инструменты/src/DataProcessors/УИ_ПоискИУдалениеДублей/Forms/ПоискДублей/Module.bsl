...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НастройкиФормы = Неопределено Тогда
		...
		НастройкиФормы.Вставить("ЗаменаЧерезSQL", Ложь);
		НастройкиФормы.Вставить("ТипСУБД", "PostgreSQL");
	КонецЕсли;
	...
	ДобавитьКомандыМассовойПометки();
КонецПроцедуры
...
&НаКлиенте
Процедура ЗаменаЧерезSQLПриИзменении(Элемент)
	Если ЗаменаЧерезSQL Тогда
		УчитыватьПрикладныеПравила = Ложь;
		ЗаменаПарыВТранзакции = Ложь;
		Если ПустаяСтрока(ТипСУБД) Тогда
			ТипСУБД = "PostgreSQL";
		КонецЕсли;
	КонецЕсли;
	УстановитьДоступностьПараметровЗамены(ЭтотОбъект);
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьДоступностьПараметровЗамены(Форма)
	Форма.Элементы.УчитыватьПрикладныеПравила.Доступность = Не Форма.ЗаменаЧерезSQL;
	Форма.Элементы.ЗаменаПарыВТранзакции.Доступность      = Не Форма.ЗаменаЧерезSQL;
	Форма.Элементы.ТипСУБД.Доступность                     = Форма.ЗаменаЧерезSQL;
КонецПроцедуры
...
Процедура ПриАктивацииШагаМастера()
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		...
		НастройкиМастера.ПоказатьДиалогПередЗакрытием = Истина;
		ЖурналУдаления = "";
		КлиентскоеВремя = ТекущаяДата();
		ВремяНачалаУдаления = ДатаНачалаУдаленияНаСервере();
		СмещениеВремениКлиентСервер = КлиентскоеВремя - ВремяНачалаУдаления;
		НайтиИУдалитьДублиКлиент();
...
Процедура ШагМастераОтмена()
	...
	Если ТекущаяСтраница = Элементы.ШагВыполнениеПоиска Или ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		...
		Если ЗначениеЗаполнено(ИдентификаторТекущегоЗадания) Тогда
			ОтменитьФоновоеЗаданиеНаСервере(ИдентификаторТекущегоЗадания);
			ИдентификаторТекущегоЗадания = Неопределено;
		КонецЕсли;
...
Процедура ОтменитьФоновоеЗаданиеНаСервере(Знач ИдентификаторСтрокой)
	УИ_ДлительныеОперации.ОтменитьВыполнениеЗадания(
		Новый УникальныйИдентификатор(ИдентификаторСтрокой));
КонецПроцедуры
...
&НаКлиенте
Процедура УстановитьПометкуВсемДублям(ЗначениеПометки)

	Для Каждого ГруппаДублей Из НайденныеДубли.ПолучитьЭлементы() Цикл
		ГруппаДублей.Пометка = ЗначениеПометки;
		Для Каждого Потомок Из ГруппаДублей.ПолучитьЭлементы() Цикл
			Если Не Потомок.Основной Тогда
				Потомок.Пометка = ЗначениеПометки;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
&НаСервере
Процедура ДобавитьКомандыМассовойПометки()
	ОписаниеКнопок = Новый Массив;
	ОписаниеКнопок.Добавить(Новый Структура("Имя, Заголовок, Действие, Подсказка, Картинка",
		"УстановитьПометкиЗаменыВсемГруппам",
		НСтр("ru = 'Отметить все группы'"),
		"УстановитьПометкиЗаменыВсемГруппам",
		НСтр("ru = 'Установить пометки замены у всех кандидатов во всех группах дублей'"),
		БиблиотекаКартинок.УстановитьФлажки));
	ОписаниеКнопок.Добавить(Новый Структура("Имя, Заголовок, Действие, Подсказка, Картинка",
		"СнятьПометкиЗаменыСоВсехГрупп",
		НСтр("ru = 'Снять пометки со всех групп'"),
		"СнятьПометкиЗаменыСоВсехГрупп",
		НСтр("ru = 'Снять пометки замены у всех кандидатов во всех группах дублей'"),
		БиблиотекаКартинок.СнятьФлажки));

	Для Каждого ОписаниеКнопки Из ОписаниеКнопок Цикл
		Если Команды.Найти(ОписаниеКнопки.Имя) = Неопределено Тогда
			НоваяКоманда = Команды.Добавить(ОписаниеКнопки.Имя);
			НоваяКоманда.Действие = ОписаниеКнопки.Действие;
			НоваяКоманда.Заголовок = ОписаниеКнопки.Заголовок;
			НоваяКоманда.Подсказка = ОписаниеКнопки.Подсказка;
			НоваяКоманда.Картинка = ОписаниеКнопки.Картинка;
		КонецЕсли;

		Если Элементы.Найти(ОписаниеКнопки.Имя) = Неопределено Тогда
			Кнопка = Элементы.Добавить(ОписаниеКнопки.Имя, Тип("КнопкаФормы"),
				Элементы.НайденныеДублиКоманднаяПанель);
			Кнопка.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
			Кнопка.ИмяКоманды = ОписаниеКнопки.Имя;
...
&НаКлиенте
Процедура УстановитьПометкиЗаменыВсемГруппам(Команда)
	ИзменитьПометкиЗаменыВоВсехГруппах(1);
КонецПроцедуры

&НаКлиенте
Процедура СнятьПометкиЗаменыСоВсехГрупп(Команда)
	ИзменитьПометкиЗаменыВоВсехГруппах(0);
КонецПроцедуры

&НаКлиенте
Процедура ИзменитьПометкиЗаменыВоВсехГруппах(НоваяПометка)
	Для Каждого ГруппаДублей Из НайденныеДубли.ПолучитьЭлементы() Цикл
		ЕстьПомеченные = Ложь;
		Для Каждого Кандидат Из ГруппаДублей.ПолучитьЭлементы() Цикл
			Если Кандидат.Основной Тогда
				Продолжить;
			КонецЕсли;
			Кандидат.Пометка = НоваяПометка;
			ЕстьПомеченные = ЕстьПомеченные Или НоваяПометка = 1;
		КонецЦикла;
		ГруппаДублей.Пометка = ?(ЕстьПомеченные, 1, 0);
	КонецЦикла;
КонецПроцедуры
...
Процедура ПриСозданииНаСервереИнициализацияДанных(НастройкиФормы)
	...
	ЗначениеЗаменаЧерезSQL = УИ_ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкиФормы, "ЗаменаЧерезSQL");
	ЗаменаЧерезSQL = ?(ТипЗнч(ЗначениеЗаменаЧерезSQL) = Тип("Булево"), ЗначениеЗаменаЧерезSQL, Ложь);
	ЗначениеТипСУБД = УИ_ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкиФормы, "ТипСУБД");
	ТипСУБД = ?(ТипЗнч(ЗначениеТипСУБД) = Тип("Строка") И Не ПустаяСтрока(ЗначениеТипСУБД),
		ЗначениеТипСУБД, "PostgreSQL");

	Элементы.ТипСУБД.СписокВыбора.Добавить("PostgreSQL");
	Элементы.ТипСУБД.СписокВыбора.Добавить("MSSQL");
	УстановитьДоступностьПараметровЗамены(ЭтотОбъект);
...
Процедура СохранитьПользовательскиеНастройки()
	...
	НастройкиФормы.Вставить("ЗаменаЧерезSQL", ЗаменаЧерезSQL);
	НастройкиФормы.Вставить("ТипСУБД", ТипСУБД);
...
Процедура НайтиИУдалитьДублиКлиент()
	...
	ИдентификаторТекущегоЗадания = Строка(Задание.ИдентификаторЗадания);
...
Функция НайтиИУдалитьДубли()
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		...
		ПараметрыПроцедуры.Вставить("ЗаменаЧерезSQL", ЗаменаЧерезSQL);
		ПараметрыПроцедуры.Вставить("ТипСУБД", ТипСУБД);
	Иначе
...
Процедура НайтиИУдалитьДублиПрогресс(Прогресс, ДополнительныеПараметры) Экспорт
	...
	// Для шага удаления — читаем лог из ЖР.
	Если ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		ДанныеЖР = ПрочитатьЖурналУдаленияИзЖР(ВремяНачалаУдаления, СмещениеВремениКлиентСервер);
		ЖурналУдаления = ДанныеЖР.Текст;
		Элементы.ЖурналУдаления.Заголовок = СтрШаблон(
			НСтр("ru = 'Удаление дублей... (обработано %1 из %2)'"),
			ДанныеЖР.Количество, ВсегоНайденоДублей);
		// Автоскролл — прокручиваем поле лога к концу.
		ТекущийЭлемент = Элементы.ЖурналУдаления;
		ПодключитьОбработчикОжидания("ПрокрутитьЖурналУдаленияВниз", 0.1, Истина);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПрокрутитьЖурналУдаленияВниз()
	Попытка
		КонецТекста = СтрДлина(ЖурналУдаления) + 1;
		Элементы.ЖурналУдаления.УстановитьГраницыВыделения(КонецТекста, КонецТекста);
	Исключение
		// Поле может быть ещё не готово — игнорируем.
	КонецПопытки;
КонецПроцедуры
...
Процедура НайтиИУдалитьДублиЗавершение(Задание, ДополнительныеПараметры) Экспорт
	...
	Если ТекущаяСтраница = Элементы.ШагВыполнениеПоиска Тогда
		...
		КонецЕсли;
	ИначеЕсли ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		// Полный лог из ЖР — все записи гарантированно на месте после завершения ФЗ.
		ДанныеЖР = ПрочитатьЖурналУдаленияИзЖР(ВремяНачалаУдаления, СмещениеВремениКлиентСервер);
		ТекстSQLСкрипта = "";
		Успех = ЗаполнитьРезультатыУдаленияДублей(Задание.АдресРезультата, ТекстSQLСкрипта);
		// Дописываем итог в конец лога (текст из ОписаниеСостоянияНайденныхДублей).
		ЖурналУдаления = ДанныеЖР.Текст + Символы.ПС
			+ "--- " + Строка(ОписаниеСостоянияНайденныхДублей) + " ---";
		// Дописываем SQL-скрипт в конец лога, если он есть — пользователь сможет
		// скопировать его прямо из поля ЖурналУдаления и выполнить в СУБД.
		Если Не ПустаяСтрока(ТекстSQLСкрипта) Тогда
			ЖурналУдаления = ЖурналУдаления + Символы.ПС + Символы.ПС
				+ "=========== SQL-СКРИПТ ДЛЯ ДОВЫПОЛНЕНИЯ ЗАМЕН ==========="
				+ Символы.ПС + ТекстSQLСкрипта
				+ Символы.ПС + "==========================================================";
		КонецЕсли;
...
&НаСервере
Функция ЗаполнитьРезультатыУдаленияДублей(Знач АдресРезультата, ТекстSQLСкрипта = "")
	...
	// Собираем SQL-скрипт, если есть замены, требующие SQL — будет дописан в ЖурналУдаления вызывающим кодом.
	ТекстSQLСкрипта = "";
	СтрокиSQL = ТаблицаОшибок.НайтиСтроки(Новый Структура("ТипОшибки", "ТребуетсяSQL_Итог"));
	Если СтрокиSQL.Количество() > 0 Тогда
		Для Каждого СтрокаSQL Из СтрокиSQL Цикл
			Если Не ПустаяСтрока(ТекстSQLСкрипта) Тогда
				ТекстSQLСкрипта = ТекстSQLСкрипта + Символы.ПС;
			КонецЕсли;
			ТекстSQLСкрипта = ТекстSQLСкрипта + СтрокаSQL.ТекстОшибки;
		КонецЦикла;
	КонецЕсли;

	// SQL-связанные строки ("ТребуетсяSQL_Итог", "ТребуетсяSQL") не считаем настоящими ошибками —
	// замены через платформу выполнились успешно, требуется только применить SQL-скрипт для регистров бухгалтерии.
	КоличествоНастоящихОшибок = 0;
	Для Каждого СтрокаОшибки Из ТаблицаОшибок Цикл
		Если СтрокаОшибки.ТипОшибки <> "ТребуетсяSQL_Итог" И СтрокаОшибки.ТипОшибки <> "ТребуетсяSQL" Тогда
			КоличествоНастоящихОшибок = КоличествоНастоящихОшибок + 1;
		КонецЕсли;
	КонецЦикла;
	ЗавершеноБезОшибок = КоличествоНастоящихОшибок = 0;
...
&НаСервереБезКонтекста
Функция ДатаНачалаУдаленияНаСервере()
	Возврат ТекущаяДата();
КонецФункции

&НаСервереБезКонтекста
Функция ПрочитатьЖурналУдаленияИзЖР(Знач ДатаНачала, Знач СмещениеСекунд = 0)

	СобытиеБазовое = НСтр("ru='Поиск и удаление ссылок'",
		УИ_ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());

	МассивСобытий = Новый Массив;
	МассивСобытий.Добавить(СобытиеБазовое);
	МассивСобытий.Добавить(СобытиеБазовое + ".Прогресс");
	...
	ВыгрузитьЖурналРегистрации(СобытияЖР, Отбор);

	ТекстЛога = "";
	ОбработаноДублей = 0;
	Для Каждого Запись Из СобытияЖР Цикл
		КлиентскаяДата = Запись.Дата + СмещениеСекунд;
		Комментарий = Запись.Комментарий;
		ТекстЛога = ТекстЛога
			+ Формат(КлиентскаяДата, "ДФ=HH:mm:ss") + " "
			+ Комментарий + Символы.ПС;
		// Итоговые строки начинаются с ✓ или ✗.
		Если СтрНачинаетсяС(Комментарий, "✓") Или СтрНачинаетсяС(Комментарий, "✗") Тогда
			ОбработаноДублей = ОбработаноДублей + 1;
		КонецЕсли;
	КонецЦикла;

	Результат = Новый Структура;
	Результат.Вставить("Текст", ТекстЛога);
	Результат.Вставить("Количество", ОбработаноДублей);
	Возврат Результат;
