...
Процедура ДобавитьКомандыМассовойПометки()
	...
	ОписаниеКнопок.Добавить(Новый Структура("Имя, Заголовок, Действие, Подсказка, ИменаКартинок",
		...
		"ПометитьВсе,УстановитьОтметкуДляВсехВыделенных,ВыделитьВсе,УИ_ВыбранныеОбъекты"));
	ОписаниеКнопок.Добавить(Новый Структура("Имя, Заголовок, Действие, Подсказка, ИменаКартинок",
		...
		"СнятьПометки,СнятьОтметкуДляВсехВыделенных,Очистить,УИ_Очистить"));
	...
	Для Каждого ОписаниеКнопки Из ОписаниеКнопок Цикл
		Если Команды.Найти(ОписаниеКнопки.Имя) = Неопределено Тогда
			...
			НоваяКоманда.Картинка = ПервоеНайденноеИзображение(ОписаниеКнопки.ИменаКартинок);
		КонецЕсли;
...
Функция ПервоеНайденноеИзображение(ИменаКартинокСтрокой)
	Для Каждого ИмяКартинки Из СтрРазделить(ИменаКартинокСтрокой, ",", Ложь) Цикл
		Попытка
			Возврат Вычислить("БиблиотекаКартинок." + ИмяКартинки);
		Исключение
			// Картинки нет — пробуем следующую
		КонецПопытки;
	КонецЦикла;
