...
&НаКлиенте
Перем ПредыдущиеПараметрыЗамены;
...
Процедура ЗаменаЧерезSQLПриИзменении(Элемент)
	Если ЗаменаЧерезSQL Тогда
		// Запоминаем выбор пользователя, чтобы вернуть его при выключении SQL-режима.
		ПредыдущиеПараметрыЗамены = Новый Структура("УчитыватьПрикладныеПравила, ЗаменаПарыВТранзакции",
			УчитыватьПрикладныеПравила, ЗаменаПарыВТранзакции);
		УчитыватьПрикладныеПравила = Ложь;
		ЗаменаПарыВТранзакции = Ложь;
		Если ПустаяСтрока(ТипСУБД) Тогда
			ТипСУБД = "PostgreSQL";
		КонецЕсли;
	ИначеЕсли ПредыдущиеПараметрыЗамены <> Неопределено Тогда
		УчитыватьПрикладныеПравила = ПредыдущиеПараметрыЗамены.УчитыватьПрикладныеПравила;
		ЗаменаПарыВТранзакции = ПредыдущиеПараметрыЗамены.ЗаменаПарыВТранзакции;
		ПредыдущиеПараметрыЗамены = Неопределено;
	КонецЕсли;
...
Процедура ПриСозданииНаСервереИнициализацияДанных(НастройкиФормы)
	...
	ЗаменаЧерезSQL = (ТипЗнч(ЗначениеЗаменаЧерезSQL) = Тип("Булево")) И ЗначениеЗаменаЧерезSQL;
...
Процедура НайтиИУдалитьДублиЗавершение(Задание, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		ТекстСкриптаSQL = "";
		Успех = ЗаполнитьРезультатыУдаленияДублей(Задание.АдресРезультата, ТекстСкриптаSQL);
		Если ЗаменаЧерезSQL Тогда
			// SQL-режим: журнал = только готовый скрипт (самодостаточный, без лишнего текста и рамок).
			ЖурналУдаления = ТекстСкриптаSQL;
		Иначе
			// Полный лог из ЖР - все записи гарантированно на месте после завершения ФЗ;
			// в SQL-режиме он не нужен, поэтому читаем только здесь.
			ДанныеЖР = ПрочитатьЖурналУдаленияИзЖР(ВремяНачалаУдаления, СмещениеВремениКлиентСервер);
...
&НаСервере
Функция ЗаполнитьРезультатыУдаленияДублей(Знач АдресРезультата, ТекстСкриптаSQL = "")
	...
	// Строки "ТребуетсяSQL_Итог": в SQL-режиме это готовый скрипт, в прямом - признак,
	// что часть ссылок не заменена средствами платформы (нужен режим «Замена через SQL»).
	СтрокиSQL = ТаблицаОшибок.НайтиСтроки(Новый Структура("ТипОшибки", "ТребуетсяSQL_Итог"));
	ТекстСкриптаSQL = "";
	Для Каждого СтрокаSQL Из СтрокиSQL Цикл
		Если Не ПустаяСтрока(ТекстСкриптаSQL) Тогда
			ТекстСкриптаSQL = ТекстСкриптаSQL + Символы.ПС;
		КонецЕсли;
		ТекстСкриптаSQL = ТекстСкриптаSQL + СтрокаSQL.ТекстОшибки;
	КонецЦикла;
	...
	Для Каждого СтрокаОшибки Из ТаблицаОшибок Цикл
		...
		КонецЕсли;
	КонецЦикла;
	// В SQL-режиме "ТребуетсяSQL_Итог" - успешно сформированный скрипт, не ошибка;
	// в прямом режиме наличие таких строк = непрошедшие замены → шаг «Неудачные замены».
	ЗавершеноБезОшибок = (КоличествоНастоящихОшибок = 0) И (ЗаменаЧерезSQL Или СтрокиSQL.Количество() = 0);
	Если (Не ЗаменаЧерезSQL) И СтрокиSQL.Количество() > 0 Тогда
		Элементы.ЗаголовокСтраницыНеудачныеЗамены.Заголовок =
			НСтр("ru = 'Ряд ссылок заменить не удалось. Воспользуйтесь режимом «Замена через SQL». Затронутые дубли:'");
	Иначе
		Элементы.ЗаголовокСтраницыНеудачныеЗамены.Заголовок =
			НСтр("ru = 'Не удалось заменить дубли на оригиналы в некоторых местах использования:'");
	КонецЕсли;
	ПоследнийКандидат  = Неопределено;

	Если ЗавершеноБезОшибок И ЗаменаЧерезSQL Тогда
		// SQL-режим: данные не изменены, сформирован только скрипт.
		ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(НСтр(
			"ru = 'SQL-скрипт сформирован. Данные в 1С не изменены - скопируйте скрипт ниже и выполните в СУБД, затем «Пересчёт итогов».'"));
	ИначеЕсли ЗавершеноБезОшибок Тогда
		ВсегоОбработано = 0;
		ВсегоОсновных   = 0;
		Для Каждого ГруппаДублей Из НайденныеДубли.ПолучитьЭлементы() Цикл
