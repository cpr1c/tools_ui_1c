...
Процедура ФоновоеУдалениеДублей(Знач Параметры, Знач АдресРезультата) Экспорт
	...
	Если Параметры.Свойство("ЗаменаЧерезSQL") И Параметры.ЗаменаЧерезSQL Тогда
		СгенерироватьТолькоSQL(Параметры.ПарыЗамен, Параметры.ТипСУБД, АдресРезультата);
		Возврат;
	КонецЕсли;
...
// Режим "Только SQL": для всех заявленных пар замен собирает места использования и
// генерирует SQL-скрипт без выполнения реальной замены через 1С. Результат укладывается
// в временное хранилище в формате таблицы ошибок (одна строка с ТипОшибки="ТребуетсяSQL_Итог"),
// чтобы существующий код формы (ЗаполнитьРезультатыУдаленияДублей) автоматически дописал
// SQL в ЖурналУдаления.
//
// Параметры:
//     ПарыЗамен       - Соответствие - Ключ: дубль, Значение: оригинал.
//     ТипСУБД         - Строка       - "PostgreSQL" или "MSSQL".
//     АдресРезультата - Строка       - Адрес во временном хранилище.
//
Процедура СгенерироватьТолькоSQL(Знач ПарыЗамен, Знач ТипСУБД, АдресРезультата)

	// DEBUG: счётчики для диагностики, чтобы понять где пара теряется по пути
	// ПарыЗамен → Дубли → НайтиПоСсылкам → ЗаменыТребующиеSQL.
	ОтладкаПоДублю = Новый Соответствие; // дубль → структура счётчиков

	Дубли = Новый Массив;
	Для Каждого КлючЗначение Из ПарыЗамен Цикл
		Если КлючЗначение.Ключ <> КлючЗначение.Значение И Не КлючЗначение.Ключ.Пустая() Тогда
			Дубли.Добавить(КлючЗначение.Ключ);
			ОтладкаПоДублю.Вставить(КлючЗначение.Ключ,
				Новый Структура("Оригинал, МестВсего, СкипСлуж, СкипНетПары, ДобавленоОсновных, ДобавленоИзДвижений",
					КлючЗначение.Значение, 0, 0, 0, 0, 0));
		КонецЕсли;
	КонецЦикла;
	...
	Если Дубли.Количество() > 0 Тогда
		МестаИспользования = УИ_ОбщегоНазначения.МестаИспользования(Дубли);
		ВсегоМест = МестаИспользования.Количество();
		Для Каждого МестоИспользования Из МестаИспользования Цикл
			Сч = ОтладкаПоДублю[МестоИспользования.Ссылка];
			Если Сч <> Неопределено Тогда Сч.МестВсего = Сч.МестВсего + 1; КонецЕсли;

			Если МестоИспользования.ЭтоСлужебныеДанные Тогда
				Если Сч <> Неопределено Тогда Сч.СкипСлуж = Сч.СкипСлуж + 1; КонецЕсли;
				Продолжить;
			КонецЕсли;
			Оригинал = ПарыЗамен[МестоИспользования.Ссылка];
			Если Оригинал = Неопределено Тогда
				Если Сч <> Неопределено Тогда Сч.СкипНетПары = Сч.СкипНетПары + 1; КонецЕсли;
				Продолжить;
			КонецЕсли;
			НоваяСтрока = ЗаменыТребующиеSQL.Добавить();
			НоваяСтрока.ПолноеИмяМетаданных = МестоИспользования.Метаданные.ПолноеИмя();
			НоваяСтрока.Дубль = МестоИспользования.Ссылка;
			НоваяСтрока.Оригинал = Оригинал;
			Если Сч <> Неопределено Тогда Сч.ДобавленоОсновных = Сч.ДобавленоОсновных + 1; КонецЕсли;

			// НайтиПоСсылкам для подчинённых регистратору регистров сворачивает место
			// использования к Документу-регистратору, а не к набору записей. Чтобы SQL-генератор
			// получил таблицы _AccumRg<N>/_AccountRg<N>/_InfoRg<N>/_Seq<N>, для каждого Документа
			// в местах использования добавляем строки по всем его движениям и последовательностям —
			// тем же путём, что использует штатная (не-SQL) замена в ИзмененныеОбъектыПриЗаменеВОбъекте.
			Если Метаданные.Документы.Содержит(МестоИспользования.Метаданные) Тогда
				КолДо = ЗаменыТребующиеSQL.Количество();
				ДобавитьДвиженияИПоследовательностиВSQL(ЗаменыТребующиеSQL,
					МестоИспользования.Метаданные, МестоИспользования.Ссылка, Оригинал);
				Если Сч <> Неопределено Тогда
					Сч.ДобавленоИзДвижений = Сч.ДобавленоИзДвижений + (ЗаменыТребующиеSQL.Количество() - КолДо);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;

	SQLСкрипт = УИ_ОбщегоНазначения.СгенерироватьSQLСкриптЗамены(ЗаменыТребующиеSQL, ТипСУБД);

	// DEBUG-шапка перед SQL: показывает по каждому дублю, сколько мест нашёл НайтиПоСсылкам
	// и сколько строк попало в ЗаменыТребующиеSQL. Если для пары видно "МестВсего: 0" — НайтиПоСсылкам
	// её не нашёл (хотя в БД ссылки есть — копаем в МестаИспользования/НайтиПоСсылкам).
	// Если "МестВсего > 0" но "ДобавленоОсновных = 0" — отсекает фильтр ЭтоСлужебныеДанные или ПарыЗамен[Ссылка].
	ОтладкаЗаголовок = Новый Массив;
	ОтладкаЗаголовок.Добавить("-- ============ DEBUG: СгенерироватьТолькоSQL ============");
	ОтладкаЗаголовок.Добавить("-- ПарыЗамен.Количество = " + ПарыЗамен.Количество()
		+ ", после фильтра Дубли = " + Дубли.Количество());
	ОтладкаЗаголовок.Добавить("-- НайтиПоСсылкам вернул всего строк: " + ВсегоМест);
	ОтладкаЗаголовок.Добавить("-- Итого ЗаменыТребующиеSQL: " + ЗаменыТребующиеSQL.Количество());
	ОтладкаЗаголовок.Добавить("-- Per-дубль:");
	Для Каждого КЗ Из ОтладкаПоДублю Цикл
		Сч = КЗ.Значение;
		ОтладкаЗаголовок.Добавить("--   "
			+ УИ_ОбщегоНазначения.ПредставлениеВОднуСтроку(КЗ.Ключ) + " (" + XMLСтрока(КЗ.Ключ) + ")"
			+ " → " + УИ_ОбщегоНазначения.ПредставлениеВОднуСтроку(Сч.Оригинал) + " (" + XMLСтрока(Сч.Оригинал) + ")"
			+ " | МестВсего=" + Сч.МестВсего
			+ ", СкипСлуж=" + Сч.СкипСлуж
			+ ", СкипНетПары=" + Сч.СкипНетПары
			+ ", ДобавленоОсновных=" + Сч.ДобавленоОсновных
			+ ", ДобавленоИзДвижений=" + Сч.ДобавленоИзДвижений);
	КонецЦикла;
	ОтладкаЗаголовок.Добавить("-- ============ /DEBUG ============");
	ОтладкаЗаголовок.Добавить("");
	SQLСкрипт = СтрСоединить(ОтладкаЗаголовок, Символы.ПС) + Символы.ПС + SQLСкрипт;

	ТаблицаОшибок = Новый ТаблицаЗначений;
	ТаблицаОшибок.Колонки.Добавить("Ссылка");
	ТаблицаОшибок.Колонки.Добавить("ОбъектОшибки");
	ТаблицаОшибок.Колонки.Добавить("ПредставлениеОбъектаОшибки", ТипСтрока);
	ТаблицаОшибок.Колонки.Добавить("ТипОшибки", ТипСтрока);
	ТаблицаОшибок.Колонки.Добавить("ТекстОшибки", ТипСтрока);

	СтрокаSQL = ТаблицаОшибок.Добавить();
	СтрокаSQL.ТипОшибки = "ТребуетсяSQL_Итог";
	СтрокаSQL.ТекстОшибки = SQLСкрипт;
	СтрокаSQL.ПредставлениеОбъектаОшибки = НСтр("ru = 'SQL-скрипт замены дублей'");

	ПоместитьВоВременноеХранилище(ТаблицаОшибок, АдресРезультата);

КонецПроцедуры

// Для метаданных Документа добавляет в ЗаменыТребующиеSQL строки по каждому его движению
// (РегистрНакопления/Сведений/Бухгалтерии/Расчета) и каждой Последовательности. Использует
// УИ_ОбщегоНазначения.ОписаниеДвижений / ОписаниеПоследовательностей — те же функции, через
// которые ходит штатная (не-SQL) замена в ИзмененныеОбъектыПриЗаменеВОбъекте, поэтому покрытие
// гарантированно совпадает. Дубли по ключу (Дубль+Оригинал+Метаданные) дедупит
// СгенерироватьSQLСкриптЗамены.
Процедура ДобавитьДвиженияИПоследовательностиВSQL(ЗаменыТребующиеSQL, МетаДок, Дубль, Оригинал)

	Для Каждого Описание Из УИ_ОбщегоНазначения.ОписаниеДвижений(МетаДок) Цикл
		НоваяСтрока = ЗаменыТребующиеSQL.Добавить();
		НоваяСтрока.ПолноеИмяМетаданных = Описание.ПространствоБлокировки;
		НоваяСтрока.Дубль = Дубль;
		НоваяСтрока.Оригинал = Оригинал;
	КонецЦикла;

	Для Каждого Описание Из УИ_ОбщегоНазначения.ОписаниеПоследовательностей(МетаДок) Цикл
		НоваяСтрока = ЗаменыТребующиеSQL.Добавить();
		// ПространствоБлокировки последовательности — "Последовательность.X.Записи".
		// Структура хранения индексирует таблицу по "Последовательность.X" — отсекаем суффикс.
		НоваяСтрока.ПолноеИмяМетаданных = СтрЗаменить(Описание.ПространствоБлокировки, ".Записи", "");
		НоваяСтрока.Дубль = Дубль;
		НоваяСтрока.Оригинал = Оригинал;
	КонецЦикла;
