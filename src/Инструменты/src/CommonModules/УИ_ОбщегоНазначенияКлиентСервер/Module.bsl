...
//
// Параметры:
//  СписокПриемник - СписокЗначений из Произвольный
//  СписокИсточник - СписокЗначений из Произвольный
...
// Тип объекта для отладки объект базы данных.
//
// Возвращаемое значение:
//  Строка - Тип объекта для отладки объект базы данных
Функция ТипОбъектаДляОтладкиОбъектБазыДанных() Экспорт
	Возврат "ОбъектБазыДанных";
КонецФункции

// Тип объекта для отладки HTTPЗапрос.
//
// Возвращаемое значение:
//  Строка - Тип объекта для отладки HTTPЗапрос
Функция ТипОбъектаДляОтладкиHTTPЗапрос() Экспорт
	Возврат "HTTPЗапрос";
КонецФункции

// Тип объекта для отладки ЗАПРОС.
//
// Возвращаемое значение:
//  Строка - Тип объекта для отладки ЗАПРОС
Функция ТипОбъектаДляОтладкиЗАПРОС() Экспорт
	Возврат "ЗАПРОС";
КонецФункции

// Тип объекта для отладки менеджер временных таблиц.
//
// Возвращаемое значение:
//  Строка - Тип объекта для отладки менеджер временных таблиц
Функция ТипОбъектаДляОтладкиМенеджерВременныхТаблиц() Экспорт
	Возврат "МенеджерВременныхТаблиц";
КонецФункции

// Тип объекта для отладки схема компоновки данных.
//
// Возвращаемое значение:
//  Строка - Тип объекта для отладки схема компоновки данных
Функция ТипОбъектаДляОтладкиСхемаКомпоновкиДанных() Экспорт
	Возврат "СхемаКомпоновкиДанных";
КонецФункции

// Тип объекта для отладки таблица формы.
//
// Возвращаемое значение:
//  Строка - Тип объекта для отладки таблица формы
Функция ТипОбъектаДляОтладкиТаблицаФормы() Экспорт
	Возврат "ТаблицаФормы";
КонецФункции

// Тип объекта для отладки ТаблицаЗначений.
//
// Возвращаемое значение:
//  Строка - Тип объекта для отладки ТаблицаЗначений
Функция ТипОбъектаДляОтладкиТаблицаЗначений() Экспорт
	Возврат "ТаблицаЗначений";
КонецФункции

// Тип объекта для отладки дерево значений.
//
// Возвращаемое значение:
//  Строка - Тип объекта для отладки дерево значений
Функция ТипОбъектаДляОтладкиДеревоЗначений() Экспорт
	Возврат "ДеревоЗначений";
КонецФункции

// Новый значения дополнительных данных отладки.
//
// Возвращаемое значение:
//  Структура - Новый значения дополнительных данных отладки:
// * Наименование - Строка, Неопределено -
// * НастройкиКомпоновки - Неопределено, НастройкиКомпоновкиДанных -
// * ВнешниеНаборыДанных - Неопределено, Структура -
// * МенеджерВременныхТаблиц - Неопределено, МенеджерВременныхТаблиц -
// * HTTPСоединение - Неопределено, HTTPСоединение -
Функция НовыйЗначенияДополнительныхДанныхОтладки() Экспорт
	Данные = Новый Структура;
	Данные.Вставить("Наименование", "");
	Данные.Вставить("НастройкиКомпоновки", Неопределено);
	Данные.Вставить("ВнешниеНаборыДанных", Неопределено);
	Данные.Вставить("МенеджерВременныхТаблиц", Неопределено);
	Данные.Вставить("HTTPСоединение", Неопределено);

	Возврат Данные;
КонецФункции

// Новый сериализованный для отладки объект базы данных.
//
// Возвращаемое значение:
//  Структура - Новый сериализованный для отладки объект базы данных:
//  * ВерсияФормата - Число -
// 	* Объект - Неопределено, ЛюбаяСсылка -
Функция НовыйСериализованныйДляОтладкиОбъектБазыДанных() Экспорт
	Данные = Новый Структура;
	Данные.Вставить("ВерсияФормата", 1);
	Данные.Вставить("Объект", Неопределено);

	Возврат Данные;
КонецФункции

// Новый сериализованный для отладки запрос.
//
// Возвращаемое значение:
//  Структура - Новый сериализованный для отладки запрос:
// * ВерсияФормата - Число -
// * Имя - Строка -
// * Текст - Строка -
// * Параметры - Структура -
// * ВременныеТаблицы - Структура -
Функция НовыйСериализованныйДляОтладкиЗапрос() Экспорт
	СтруктураОбъекта = Новый Структура;
	СтруктураОбъекта.Вставить("ВерсияФормата", 1);
	СтруктураОбъекта.Вставить("Имя", "Отладка");
	СтруктураОбъекта.Вставить("Текст", "");
	СтруктураОбъекта.Вставить("Параметры", Новый Структура);
	СтруктураОбъекта.Вставить("ВременныеТаблицы", Новый Структура);

	Возврат СтруктураОбъекта;
Конецфункции

// Новый сериализованный для отладки HTTPЗапрос.
//
// Возвращаемое значение:
//  Структура - Новый сериализованный для отладки HTTPЗапрос:
// * ВерсияФормата - Число -
// * АдресСервера - Строка -
// * Порт - Число -
// * ИспользоватьHTPPS - Булево -
// * Протокол - Строка -
// * СоединениеИспользоватьАутентификациюОС - Булево -
// * ПроксиСервер - Строка -
// * ПроксиПорт - Число -
...
// * ТелоЗапроса - Строка -
// * Заголовки - Строка -
// * ДвоичныеДанныеТела - Неопределено, ДвоичныеДанные -
// * ДвоичныеДанныеТелаСтрокой - Строка -
// * ИмяФайлаЗапроса - Строка -
Функция НовыйСериализованныйДляОтладкиHTTPЗапрос() Экспорт
	Данные = Новый Структура;
	Данные.Вставить("ВерсияФормата", 1);
	Данные.Вставить("АдресСервера", "");
	Данные.Вставить("Порт", 0);
	Данные.Вставить("ИспользоватьHTPPS", Ложь);
	Данные.Вставить("Протокол", "http");
	Данные.Вставить("СоединениеИспользоватьАутентификациюОС", Ложь);


	Данные.Вставить("ПроксиСервер", "");
	Данные.Вставить("ПроксиПорт", 0);
	Данные.Вставить("ПроксиПользователь", "");
	Данные.Вставить("ПроксиПароль", "");
	Данные.Вставить("ИспользоватьАутентификациюОС", Ложь);

	Данные.Вставить("Запрос", "");
	Данные.Вставить("ТелоЗапроса", "");
	Данные.Вставить("Заголовки", "");

	Данные.Вставить("ДвоичныеДанныеТела", Неопределено);
	Данные.Вставить("ДвоичныеДанныеТелаСтрокой", "");

	Данные.Вставить("ИмяФайлаЗапроса", "");

	Возврат Данные;
КонецФункции

// Новый сериализованная для отладки схема компоновки данных.
//
// Возвращаемое значение:
//  Структура - Новый сериализованная для отладки схема компоновки данных:
// * ВерсияФормата - Число -
// * ТекстСКД - Строка -
// * ТекстНастроекСКД - Строка -
// * ВнешниеНаборыДанных - Структура -
// * ВременныеТаблицы - Структура -
Функция НовыйСериализованнаяДляОтладкиСхемаКомпоновкиДанных() Экспорт
	Данные = Новый Структура;
	Данные.Вставить("ВерсияФормата", 2);
	Данные.Вставить("ТекстСКД", "");
	Данные.Вставить("ТекстНастроекСКД", "");
	Данные.Вставить("ВнешниеНаборыДанных", Новый Структура);
	Данные.Вставить("ВременныеТаблицы", Новый Структура);
...
// 	ПараметрОтладки2 - Неопределено, HTTPСоединение, НастройкиКомпоновкиДанных, Структура, Строка, МенеджерВременныхТаблиц -
// 	ПараметрОтладки3 - Неопределено, HTTPСоединение, НастройкиКомпоновкиДанных, Структура, Строка, МенеджерВременныхТаблиц -
// 	ПараметрОтладки4 - Неопределено, HTTPСоединение, НастройкиКомпоновкиДанных, Структура, Строка, МенеджерВременныхТаблиц -
// 	ПараметрОтладки5 - Неопределено, HTTPСоединение, НастройкиКомпоновкиДанных, Структура, Строка, МенеджерВременныхТаблиц -
//
// Возвращаемое значение:
//  Неопределено, Строка - Отладить объект
//@skip-check method-too-many-params
Функция ОтладитьОбъект(ОбъектДляОтладки, СохранитьВФайл, ПараметрОтладки2 = Неопределено,
	ПараметрОтладки3 = Неопределено, ПараметрОтладки4 = Неопределено, ПараметрОтладки5 = Неопределено) Экспорт
...
#КонецЕсли
	...
	ПараметрыОбъектаОтладки = Новый Массив;
	Если ПараметрОтладки2 <> Неопределено Тогда
		ПараметрыОбъектаОтладки.Добавить(ПараметрОтладки2);
	КонецЕсли;
	Если ПараметрОтладки3 <> Неопределено Тогда
		ПараметрыОбъектаОтладки.Добавить(ПараметрОтладки3);
	КонецЕсли;
	Если ПараметрОтладки4 <> Неопределено Тогда
		ПараметрыОбъектаОтладки.Добавить(ПараметрОтладки4);
	КонецЕсли;
	Если ПараметрОтладки5 <> Неопределено Тогда
		ПараметрыОбъектаОтладки.Добавить(ПараметрОтладки5);
	КонецЕсли;

	ЗначенияДополнительныхДанныхОтладки = ЗначенияДополнительныхДанныхОтладки(ПараметрыОбъектаОтладки);

	ТипВсеСсылки = УИ_ОбщегоНазначенияПовтИсп.ОписаниеТипаВсеСсылки();
	СериализованныйОбъект = Неопределено;//СериализоватьОбъектДляОтладкиВСтруктуру(ОбъектДляОтладки, НастройкиСКДИлиСоединениеHTTP, ВнешниеНаборыДанных);
	Если ТипВсеСсылки.СодержитТип(ТипЗнч(ОбъектДляОтладки)) Тогда
		ТипОбъектаОтладки = ТипОбъектаДляОтладкиОбъектБазыДанных();
		СериализованныйОбъект = СериализованныйДляОтладкиОбъектБазыДанных(ОбъектДляОтладки);

	ИначеЕсли ТипЗнч(ОбъектДляОтладки) = Тип("HTTPЗапрос") Тогда
		ТипОбъектаОтладки = ТипОбъектаДляОтладкиHTTPЗапрос();
		СериализованныйОбъект = УИ_ОбщегоНазначенияВызовСервера.СериализованныйДляОтладкиHTTPЗапрос(ОбъектДляОтладки,
																									ЗначенияДополнительныхДанныхОтладки);
	ИначеЕсли ТипЗнч(ОбъектДляОтладки) = Тип("Запрос") Тогда
		ТипОбъектаОтладки = ТипОбъектаДляОтладкиЗАПРОС();
		СериализованныйОбъект = УИ_ОбщегоНазначенияВызовСервера.СериализованныйДляОтладкиОбъектЗапроса(ОбъектДляОтладки);

	ИначеЕсли ТипЗнч(ОбъектДляОтладки) = Тип("МенеджерВременныхТаблиц") Тогда
		ТипОбъектаОтладки = ТипОбъектаДляОтладкиМенеджерВременныхТаблиц();
		СериализованныйОбъект = УИ_ОбщегоНазначенияВызовСервера.СериализованныйДляОтладкиМенеджерВременныхТаблиц(ОбъектДляОтладки);
	ИначеЕсли ТипЗнч(ОбъектДляОтладки) = Тип("СхемаКомпоновкиДанных") Тогда
		ТипОбъектаОтладки = ТипОбъектаДляОтладкиСхемаКомпоновкиДанных();
		СериализованныйОбъект = УИ_ОбщегоНазначенияВызовСервера.СериализованныйДляОтладкиОбъектСхемыКомпоновкиДанных(ОбъектДляОтладки,
																													 ЗначенияДополнительныхДанныхОтладки);
	ИначеЕсли ТипЗнч(ОбъектДляОтладки) = Тип("ТаблицаФормы") Тогда
		ТипОбъектаОтладки = ТипОбъектаДляОтладкиТаблицаФормы();
		Если ЭтоКонтекстСервера() Тогда
			СКД = ОбъектДляОтладки.ПолучитьИсполняемуюСхемуКомпоновкиДанных();
			ЗначенияДополнительныхДанныхОтладки.НастройкиКомпоновки = ОбъектДляОтладки.ПолучитьИсполняемыеНастройкиКомпоновкиДанных();

			СериализованныйОбъект = УИ_ОбщегоНазначенияВызовСервера.СериализованныйДляОтладкиОбъектСхемыКомпоновкиДанных(СКД,
																														 ЗначенияДополнительныхДанныхОтладки);
		КонецЕсли;
	ИначеЕсли ТипЗнч(ОбъектДляОтладки) = Тип("ТаблицаЗначений") Тогда
		ТипОбъектаОтладки = ТипОбъектаДляОтладкиТаблицаЗначений();
		СериализованныйОбъект = УИ_ОбщегоНазначенияВызовСервера.СериализованныйДляОтладкиОбъектТаблицаЗначений(ОбъектДляОтладки);
	ИначеЕсли ТипЗнч(ОбъектДляОтладки) = Тип("ДеревоЗначений") Тогда
		ТипОбъектаОтладки = ТипОбъектаДляОтладкиДеревоЗначений();
		СериализованныйОбъект = УИ_ОбщегоНазначенияВызовСервера.СериализованныйДляОтладкиОбъектДеревоЗначений(ОбъектДляОтладки);
...
#КонецЕсли
	...
	ИначеЕсли СохранитьВФайл Тогда
		Возврат УИ_ОбщегоНазначенияВызовСервера.ЗаписатьДанныеДляОтладкиВФайл(ТипОбъектаОтладки,
																			  ...
																			  ЗначенияДополнительныхДанныхОтладки.Наименование);
	Иначе
...
// Версия платформы поддерживает асинхронные функции.
//
// Возвращаемое значение:
//  Булево -  Истина, если версия платформы поддерживает асинхронные функции.
Функция ВерсияПлатформыПоддерживаетАсинхронныеФункции() Экспорт
	Возврат ВерсияПлатформыНеМладше(МинимальнаяВерсияПлатформыСПоддержкойАсинхронныхФункций());
КонецФункции

// Минимальная версия платформы с поддержкой асинхронных функций.
//
// Возвращаемое значение:
//  Строка -  Минимальная версия платформы с поддержкой асинхронных функций.
Функция МинимальнаяВерсияПлатформыСПоддержкойАсинхронныхФункций() Экспорт
	Возврат "8.3.18";
КонецФункции
...
// Переменные окружения из строки вывода команды системы.
//
// Параметры:
//  ТекстВывода -Строка-Текст вывода
//
// Возвращаемое значение:
// Соответствие Из КлючИЗначение:
// 	* Ключ - Строка
// 	* Значение - Строка
Функция ПеременныеОкруженияИзСтрокиВыводаКомандыСистемы(ТекстВывода) Экспорт
	ПеременныеОкружения = Новый Соответствие();

	МаркерБлокаПеременныхОкружения = МаркерБлокаПеременныхОкружения();

	НачалоБлокаПеременных = СтрНайти(ТекстВывода, МаркерБлокаПеременныхОкружения);
	ОкончаниеБлокаПеременных = СтрНайти(ТекстВывода, МаркерБлокаПеременныхОкружения, НаправлениеПоиска.СКонца);
	Если Не ЗначениеЗаполнено(НачалоБлокаПеременных)
		 Или Не ЗначениеЗаполнено(ОкончаниеБлокаПеременных) Или НачалоБлокаПеременных = ОкончаниеБлокаПеременных Тогда
		Возврат ПеременныеОкружения;
	КонецЕсли;

	ТекстБлокаПеременныхОкружения = Сред(ТекстВывода, НачалоБлокаПеременных + СтрДлина(МаркерБлокаПеременныхОкружения), ОкончаниеБлокаПеременных
																														- НачалоБлокаПеременных
																														+ 1
																														- СтрДлина(МаркерБлокаПеременныхОкружения));

	ИмяСлужебнойПеременнойОкружения = ИмяСлужебнойПеременнойОкруженияДляФиксацииКодаВозвратаКомандыСистемы();

	МассивСтрокБлокаПеременныхОкружения = СтрРазделить(ТекстБлокаПеременныхОкружения, Символы.ПС);
	Для Каждого ТекСтрока Из МассивСтрокБлокаПеременныхОкружения Цикл
		Если Не ЗначениеЗаполнено(ТекСтрока) Тогда
			Продолжить;
		КонецЕсли;

		СимволРавно = СтрНайти(ТекСтрока, "=");
		Если Не ЗначениеЗаполнено(СимволРавно) Тогда
			Продолжить;
		КонецЕсли;

		ИмяПеременной = Лев(ТекСтрока, СимволРавно - 1);
		Если ИмяПеременной = ИмяСлужебнойПеременнойОкружения Тогда
			Продолжить;
		КонецЕсли;
		ЗначениеПеременной = Сред(ТекСтрока, СимволРавно + 1);
...
// Имя служебной переменной окружения для фиксации кода возврата команды системы.
//
// Возвращаемое значение:
//  Строка - Имя служебной переменной окружения для фиксации кода возврата команды системы
Функция ИмяСлужебнойПеременнойОкруженияДляФиксацииКодаВозвратаКомандыСистемы() Экспорт
	Возврат "TOOLS_UI_1C_EXECUTE_CODE";
КонецФункции

// Результат разбора вывода команды системы.
//
// Параметры:
//  СтрокаВывода -Строка-Строка вывода
//
// Возвращаемое значение:
//  Структура - Результат разбора вывода команды системы:
// * УдалосьРаспарситьВыводКоманды - Булево
// * ВыводКоманды - Строка -
// * КодВозврата - Число -
// * ТекущийКаталог - Строка -
// * ПеременныеОкруженияДо - Соответствие Из КлючИЗначение:
// 		** Ключ - Строка
// 		** Значение - Строка
// * ПеременныеОкруженияПосле - Соответствие Из КлючИЗначение:
// 		** Ключ - Строка
// 		** Значение - Строка
Функция РезультатРазбораВыводаКомандыСистемы(СтрокаВывода) Экспорт
	Результат = Новый Структура;
	Результат.Вставить("УдалосьРаспарситьВыводКоманды", Ложь);
	Результат.Вставить("ВыводКоманды", "");
	Результат.Вставить("КодВозврата", 0);
	Результат.Вставить("ТекущийКаталог", "");
	Результат.Вставить("ПеременныеОкруженияДо", Новый Соответствие);
	Результат.Вставить("ПеременныеОкруженияПосле", Новый Соответствие);

	НайденныйСимволНачалоВыполненияКоманды = СтрНайти(СтрокаВывода, МаркерНачалаВыводаКомандыСистемы());
	Если НайденныйСимволНачалоВыполненияКоманды = 0 Тогда
		Результат.ВыводКоманды = СтрокаВывода;
		Возврат Результат;
	КонецЕсли;

	ТекстВыводаДоНачалаВыполненияКоманды = Лев(СтрокаВывода, НайденныйСимволНачалоВыполненияКоманды - 1);
	Результат.ПеременныеОкруженияДо = ПеременныеОкруженияИзСтрокиВыводаКомандыСистемы(ТекстВыводаДоНачалаВыполненияКоманды);

	НайденныйСимволОкончаниеВыполненияКоманды = СтрНайти(СтрокаВывода,
														 МаркерКонцаВыводаКомандыСистемы(),
														 НаправлениеПоиска.СКонца);
	Если НайденныйСимволОкончаниеВыполненияКоманды = 0 Тогда
		Результат.ВыводКоманды = СтрокаВывода;
		Возврат Результат;
	КонецЕсли;

	Результат.ВыводКоманды = Сред(СтрокаВывода,
								  НайденныйСимволНачалоВыполненияКоманды + СтрДлина(МаркерНачалаВыводаКомандыСистемы()),
								  НайденныйСимволОкончаниеВыполненияКоманды
								  - НайденныйСимволНачалоВыполненияКоманды
								  - СтрДлина(МаркерКонцаВыводаКомандыСистемы())-1);

	ТекстВыводаПослеОкончанияВыполненияКоманды = Прав(СтрокаВывода, СтрДлина(СтрокаВывода)
																	- НайденныйСимволОкончаниеВыполненияКоманды
																	- СтрДлина(МаркерКонцаВыводаКомандыСистемы()));
	Результат.ПеременныеОкруженияПосле = ПеременныеОкруженияИзСтрокиВыводаКомандыСистемы(ТекстВыводаПослеОкончанияВыполненияКоманды);

	КодСимволаПеременныхОкружения = СтрНайти(ТекстВыводаПослеОкончанияВыполненияКоманды,
											 МаркерБлокаПеременныхОкружения());
	ТекстВыводаПослеОкончанияБезПеременныхОкружения = Лев(ТекстВыводаПослеОкончанияВыполненияКоманды,
														  КодСимволаПеременныхОкружения - 1);

	МассивСтрокВыводаПослеВыполненияКоманды = СтрРазделить(ТекстВыводаПослеОкончанияБезПеременныхОкружения, Символы.ПС);
	Для Каждого СтрокаВыводаПослеВыполненияКоманды Из МассивСтрокВыводаПослеВыполненияКоманды Цикл
		Если Не ЗначениеЗаполнено(СтрокаВыводаПослеВыполненияКоманды) Тогда
			Продолжить;
		КонецЕсли;

		СимволРавно = СтрНайти(СтрокаВыводаПослеВыполненияКоманды, "=");
		Если Не ЗначениеЗаполнено(СимволРавно) Тогда
			Продолжить;
		КонецЕсли;

		ИмяПеременной = Лев(СтрокаВыводаПослеВыполненияКоманды, СимволРавно - 1);
		ЗначениеПеременной = Сред(СтрокаВыводаПослеВыполненияКоманды, СимволРавно + 1);

		Если ИмяПеременной = "КОДВОЗВРАТА" Тогда
			Результат.КодВозврата = УИ_СтроковыеФункцииКлиентСервер.СтрокаВЧисло(ЗначениеПеременной);
		ИначеЕсли ИмяПеременной = "ТЕКУЩИЙКАТАЛОГ" Тогда
			Результат.ТекущийКаталог = ЗначениеПеременной;
		КонецЕсли;
	КонецЦикла;

	Результат.УдалосьРаспарситьВыводКоманды = Истина;
	Возврат Результат;
КонецФункции

// Расширение файла запуска команды системы.
//
// Параметры:
//  ЭтоWindows - Булево
//
// Возвращаемое значение:
//  Строка - Расширение файла запуска команды системы
Функция РасширениеФайлаЗапускаКомандыСистемы(ЭтоWindows) Экспорт
...
// Маркер конца вывода команды системы.
//
// Возвращаемое значение:
//  Строка
Функция МаркерКонцаВыводаКомандыСистемы() Экспорт
	Возврат "___КонецИсполненияCкрипта___";
КонецФункции

// Маркер начала вывода команды системы.
//
// Возвращаемое значение:
//  Строка
Функция МаркерНачалаВыводаКомандыСистемы() Экспорт
	Возврат "___НачалоИсполненияCкрипта___";
КонецФункции

// Маркер блока переменных окружения.
//
// Возвращаемое значение:
//  Строка - Маркер блока переменных окружения
Функция МаркерБлокаПеременныхОкружения() Экспорт
	Возврат "___ПеременныеОкружения___";
КонецФункции

// Текст блока скрипта получение списка переменных окружения.
//
// Параметры:
//  ЭтоWindows -Булево-Это windows
//
// Возвращаемое значение:
//  Строка - Текст блока скрипта получение списка переменных окружения
Функция ТекстБлокаСкриптаПолучениеСпискаПеременныхОкружения(ЭтоWindows) Экспорт
	Если ЭтоWindows Тогда
		КомандаПолученияСпискаПеременныхОкружения = "set";
	Иначе
		КомандаПолученияСпискаПеременныхОкружения = "printenv";
	КонецЕсли;

	Возврат СтрШаблон("echo %1
					  |%2
					  |echo %1", МаркерБлокаПеременныхОкружения(), КомандаПолученияСпискаПеременныхОкружения);
КонецФункции

// Текст блока скрипта выполнения команды системы установка переменных окружения.
//
// Параметры:
//  ЭтоWindows -Булево-Это windows
//  ПеременныеОкружения - Соответствие Из КлючИЗначение:
// 		* Ключ - Строка
// 		* Значение - Строка
//
// Возвращаемое значение:
//  Строка
Функция ТекстБлокаСкриптаВыполненияКомандыСистемыУстановкаПеременныхОкружения(ЭтоWindows, ПеременныеОкружения) Экспорт
	Если ЭтоWindows Тогда
		ШаблонСтрокиКоманды = "set %1=%2";
	Иначе
		ШаблонСтрокиКоманды = "export %1=%2";
	КонецЕсли;

	СтрокиКоманды = Новый Массив;
	Для Каждого КлючЗначение Из ПеременныеОкружения Цикл
		Если Не ЗначениеЗаполнено(КлючЗначение.Ключ) Тогда
			Продолжить;
		КонецЕсли;

		СтрокиКоманды.Добавить(СтрШаблон(ШаблонСтрокиКоманды, КлючЗначение.Ключ, КлючЗначение.Значение));
	КонецЦикла;

	Возврат СтрСоединить(СтрокиКоманды, Символы.ПС);
КонецФункции

// Текст файла запуска команды системы.
//
// Параметры:
//  ПараметрыВыполнения -см. НовыйПараметрыВыполненияКомандыСистемы
//
// Возвращаемое значение:
//  Строка - Текст файла запуска команды системы
Функция ТекстФайлаЗапускаКомандыСистемы(ПараметрыВыполнения) Экспорт
	ТекстБлокаПолученияПеременныхОкружения = ТекстБлокаСкриптаПолучениеСпискаПеременныхОкружения(ПараметрыВыполнения.ЭтоWindows);
	ТекстБлокаУстановкаПеременныхОкружения = ТекстБлокаСкриптаВыполненияКомандыСистемыУстановкаПеременныхОкружения(ПараметрыВыполнения.ЭтоWindows,
																												   ПараметрыВыполнения.ПеременныеОкружения);

	Если ПараметрыВыполнения.ЭтоWindows Тогда
		СтрокиКоманды = Новый Массив;
//		СтрокиКоманды.Добавить("chcp 65001");
		СтрокиКоманды.Добавить(ТекстБлокаПолученияПеременныхОкружения);
		СтрокиКоманды.Добавить(ТекстБлокаУстановкаПеременныхОкружения);
		СтрокиКоманды.Добавить("echo " + МаркерНачалаВыводаКомандыСистемы());
		СтрокиКоманды.Добавить(ПараметрыВыполнения.Команда);
		СтрокиКоманды.Добавить("");
		СтрокиКоманды.Добавить("set "
							   + ИмяСлужебнойПеременнойОкруженияДляФиксацииКодаВозвратаКомандыСистемы()
							   + "=%ERRORLEVEL%");
		СтрокиКоманды.Добавить("echo " + МаркерКонцаВыводаКомандыСистемы());
		СтрокиКоманды.Добавить("echo КОДВОЗВРАТА=%"
							   + ИмяСлужебнойПеременнойОкруженияДляФиксацииКодаВозвратаКомандыСистемы()
							   + "%");
		СтрокиКоманды.Добавить("echo ТЕКУЩИЙКАТАЛОГ=%cd%");
		СтрокиКоманды.Добавить(ТекстБлокаПолученияПеременныхОкружения);
//		ШаблонТекста = "%4
//					   |%6
//					   |echo %2
...
//					   |set %5=%ERRORLEVEL%
//					   |echo %3
//					   |echo КОДВОЗВРАТА=%TOOLS_UI_1C_EXECUTE_CODE%
//					   |echo ТЕКУЩИЙКАТАЛОГ=%cd%
//					   |%4";
		Возврат СтрСоединить(СтрокиКоманды, Символы.ПС);
	Иначе
		ШаблонТекста = "#!/bin/bash
					   |%4
					   |%6
					   |echo %2
					   |%1
					   ...
					   |echo %3
					   |echo КОДВОЗВРАТА=$%5
					   |echo ТЕКУЩИЙКАТАЛОГ=$PWD
					   |%4";
		Возврат СтрШаблон(ШаблонТекста,
						  ПараметрыВыполнения.Команда,
						  МаркерНачалаВыводаКомандыСистемы(),
						  МаркерКонцаВыводаКомандыСистемы(),
						  ТекстБлокаПолученияПеременныхОкружения,
						  ИмяСлужебнойПеременнойОкруженияДляФиксацииКодаВозвратаКомандыСистемы(),
						  ТекстБлокаУстановкаПеременныхОкружения);
	КонецЕсли;

КонецФункции

// Строка запуска команды системы.
//
// Параметры:
//  ИмяИсполняемогоФайла - Строка
//  ИмяФайлаВыводаИсполнения - Строка -Имя файла вывода
//  ЭтоWindows -Булево-Это windows
//
// Возвращаемое значение:
//  Строка - Строка запуска команды системы
Функция СтрокаЗапускаКомандыСистемы(ИмяИсполняемогоФайла, ИмяФайлаВыводаИсполнения, ЭтоWindows) Экспорт
	Если ЭтоWindows Тогда
		ШаблонСтрокиЗапуска = "cmd /q /s /c "" ""%1"">""%2"" 2>&1""";
	Иначе
		ШаблонСтрокиЗапуска = "bash -c '""%1"" &>""%2""'";
	КонецЕсли;

	Возврат СтрШаблон(ШаблонСтрокиЗапуска, ИмяИсполняемогоФайла, ИмяФайлаВыводаИсполнения);
КонецФункции

// Новый параметры выполнения команды системы.
//
// Возвращаемое значение:
//  Структура - Новый параметры выполнения команды системы:
// * Каталог - Строка -
// * Команда - Строка -
// * ЭтоWindows - Булево -
// * РазделительПути - Строка -
// * ОписаниеОповещенияОЗавершении - Неопределено, ОписаниеОповещения -
// * ПеременныеОкружения - Соответствие Из КлючИЗначение:
// 		** Ключ - Строка
// 		** Значение - Строка
Функция НовыйПараметрыВыполненияКомандыСистемы() Экспорт
	Данные = Новый Структура;
	Данные.Вставить("Каталог", "");
	Данные.Вставить("Команда", "");
	Данные.Вставить("ЭтоWindows", ЭтоWindows());
	Данные.Вставить("РазделительПути", ПолучитьРазделительПути());
	Данные.Вставить("ОписаниеОповещенияОЗавершении", Неопределено);
	Данные.Вставить("ПеременныеОкружения", Новый Соответствие);

	Возврат Данные;
КонецФункции

// Новый результат выполнения команды системы.
//
// Возвращаемое значение:
//  Структура - Новый результат выполнения команды системы:
// * ТекущийКаталог - Строка -
// * ВыводКоманды - Строка -
// * КодВозврата - Число -
// * ПеременныеОкруженияДо - Соответствие Из КлючИЗначение:
...
// 		** Ключ - Строка
// 		** Значение - Строка
// * ИзмененныеПеременныеОкружения - Соответствие Из КлючИЗначение:
// 		** Ключ - Строка
// 		** Значение - Строка
Функция НовыйРезультатВыполненияКомандыСистемы() Экспорт
	Данные = Новый Структура;
	Данные.Вставить("ТекущийКаталог", "");
	Данные.Вставить("ВыводКоманды","");
	Данные.Вставить("КодВозврата", 0);
	Данные.Вставить("ПеременныеОкруженияДо", Новый Соответствие);
	Данные.Вставить("ПеременныеОкруженияПосле", Новый Соответствие);
	Данные.Вставить("ИзмененныеПеременныеОкружения", Новый Соответствие);

	Возврат Данные;
КонецФункции

// Дополнить результат выполнения команды системы данными вывода команды.
//
// Параметры:
//  Результат - см. НовыйРезультатВыполненияКомандыСистемы
//  ВыводКоманды - Строка
Процедура ДополнитьРезультатВыполненияКомандыСистемыДаннымиВыводаКоманды(Результат, ВыводКоманды) Экспорт
	РезультатРазбораВывода = РезультатРазбораВыводаКомандыСистемы(ВыводКоманды);
	Результат.ВыводКоманды = РезультатРазбораВывода.ВыводКоманды;
	Если Не РезультатРазбораВывода.УдалосьРаспарситьВыводКоманды Тогда
		Возврат;
	КонецЕсли;
	Результат.ТекущийКаталог = РезультатРазбораВывода.ТекущийКаталог;
	Результат.КодВозврата = РезультатРазбораВывода.КодВозврата;
	Результат.ПеременныеОкруженияДо = РезультатРазбораВывода.ПеременныеОкруженияДо;
	Результат.ПеременныеОкруженияПосле = РезультатРазбораВывода.ПеременныеОкруженияПосле;

	Для Каждого КлючЗначение Из Результат.ПеременныеОкруженияПосле Цикл
		ЗначениеДо = Результат.ПеременныеОкруженияДо[КлючЗначение.Ключ];
		Если ЗначениеДо = КлючЗначение.Значение Тогда
			Продолжить;
		КонецЕсли;
		Результат.ИзмененныеПеременныеОкружения.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
	КонецЦикла;
КонецПроцедуры
...
//
// Возвращаемое значение:
//  см. НовыйРезультатВыполненияКомандыСистемы
...
// Имя расширения инструментов.
//
// Возвращаемое значение:
//  Строка - Имя расширения инструментов
Функция ИмяРасширенияИнструментов() Экспорт
...
// Описание типов из типа.
//
// Параметры:
//  Тип - Тип
//
// Возвращаемое значение:
//  ОписаниеТипов - Описание типов из типа
Функция ОписаниеТиповИзТипа(Тип) Экспорт
	Типы = Новый Массив;
	Типы.Добавить(Тип);
	Возврат Новый ОписаниеТипов(Типы);
КонецФункции

// Представление модификаторов типа.
//
// Параметры:
//  ОписаниеТипов -ОписаниеТипов -Описание типов
//
// Возвращаемое значение:
//  Строка - Представление модификаторов типа
Функция ПредставлениеМодификаторовТипа(ОписаниеТипов) Экспорт
	Результат = "";

	Если ОписаниеТипов.СодержитТип(Тип("Строка")) Тогда
		Результат = Результат + СтрШаблон("Строка: Длина %1; ", ОписаниеТипов.КвалификаторыСтроки.Длина);
	КонецЕсли;

	Если ОписаниеТипов.СодержитТип(Тип("Дата")) Тогда
		Результат = Результат + СтрШаблон("Дата: %1; ", ОписаниеТипов.КвалификаторыДаты.ЧастиДаты);
	КонецЕсли;

	Если ОписаниеТипов.СодержитТип(Тип("Число")) Тогда
		Разрядность = СтрШаблон("%1.%2",
								ОписаниеТипов.КвалификаторыЧисла.Разрядность,
								ОписаниеТипов.КвалификаторыЧисла.РазрядностьДробнойЧасти);
		Результат = Результат + СтрШаблон("Число: Знак %1 %2; ",
...
// Возвращает структуру с предопределенными режимами запуска редактора типов.
//
// Возвращаемое значение:
//   Структура:
//     * ВсеТипы - Неопределено - разрешен выбор любого типа
//     * ТипыХраненияДанных - Число - режим для типов, доступных для хранения в базе данных
//     * ТипыДляПараметровЗапроса - Число - режим для типов, допустимых в параметрах запросов
//     * ТипыДляПолейСКД - Число - режим для типов полей системы компоновки данных
//     * ТипыДляПараметровСКД - Число - режим для типов параметров системы компоновки данных
Функция РежимыЗапускаРедактораТипа() Экспорт

	Режимы = Новый Структура;
	Режимы.Вставить("ВсеТипы", Неопределено);
	Режимы.Вставить("ТипыХраненияДанных", 0);
	Режимы.Вставить("ТипыДляПараметровЗапроса", 1);
	Режимы.Вставить("ТипыДляПолейСКД", 2);
	Режимы.Вставить("ТипыДляПараметровСКД", 3);
	Возврат Режимы;
...
// Возвращаемое значение:
// Массив из Строка -  Все наборы типов для редактирования
Функция ВсеНаборыТиповДляРедактированияБезСоставныхСсылок() Экспорт
	ДоступныеНаборы = ДоступныеНаборыТиповДляРедактирования();

	СоставныеСсылки = ДоступныеНаборы.СоставныеСсылки;

	Наборы = Новый Массив;

	Для Каждого КлючЗначение Из ДоступныеНаборы Цикл
		Если КлючЗначение.Значение = СоставныеСсылки Тогда
			Продолжить;
		КонецЕсли;
		Наборы.Добавить(КлючЗначение.Значение);
	КонецЦикла;
...
// Типы значений хранимые в контейнерах.
//
// Возвращаемое значение:
...
// Новый структура данных для поля с контейнером формы.
//
// Возвращаемое значение:
//  Структура - Новый структура данных для поля с контейнером формы:
// * Значение - Неопределено, Произвольный -
// * Контейнер 	- Неопределено -
// 				- см. НовыйКонтейнерЗначения
// * Тип	- ОписаниеТипов -
// * ПредставлениеТипа - Строка -
Функция НовыйСтруктураДанныхДляПоляСКонтейнеромФормы() Экспорт
	Данные = Новый Структура;
	Данные.Вставить("Значение", Неопределено);
	Данные.Вставить("Контейнер", Неопределено);
	Данные.Вставить("Тип", Новый ОписаниеТипов);
	Данные.Вставить("ПредставлениеТипа", "");
...
//  ПараметрыХраненияПоляСКонтейнером -см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСтруктураХраненияРеквизитаНаФормеСКонейнером
//
// Возвращаемое значение:
//  Произвольный
Функция ЗначениеПоляСКонтейнеромЗначения(СтруктураДанныхФормы, ПараметрыХраненияПоляСКонтейнером) Экспорт
	ДанныеПоля = СтруктураЗначенияПоДаннымФормыПоляСКонтейнером(СтруктураДанныхФормы, ПараметрыХраненияПоляСКонтейнером);

	Возврат ЗначениеПоляСКонтейнеромЗначенияПоСтруктуреДанных(ДанныеПоля);
КонецФункции
...
Процедура УстановитьЗначениеПоляСКонтейнером(СтруктураДанныхФормы, ПараметрыХраненияПоляСКонтейнером, НовоеЗначение) Экспорт
	ДанныеДляКонтейнера = ЗначениеВСтруктуруДанныхДляПоляСКонтейнером(НовоеЗначение);
	УстановитьЗначениеПоляСКонтейнеромПоСтруктуреДанныхДляПоляСКонтейнером(СтруктураДанныхФормы,
																		   ПараметрыХраненияПоляСКонтейнером,
																		   ДанныеДляКонтейнера);
КонецПроцедуры

// Установить значение поля с контейнером по структуре данных для поля с контейнером.
//
// Параметры:
//  СтруктураДанныхФормы - ФормаКлиентскогоПриложения, ДанныеФормыСтруктура, ДанныеФормыЭлементДерева, ДанныеФормыЭлементКоллекции - Структура данных формы
//  ПараметрыХраненияПоляСКонтейнером -см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСтруктураХраненияРеквизитаНаФормеСКонейнером
//  Данные - см. НовыйСтруктураДанныхДляПоляСКонтейнеромФормы
Процедура УстановитьЗначениеПоляСКонтейнеромПоСтруктуреДанныхДляПоляСКонтейнером(СтруктураДанныхФормы,
	ПараметрыХраненияПоляСКонтейнером, Данные) Экспорт
	СтруктураДанныхФормы[ПараметрыХраненияПоляСКонтейнером.ИмяПоляПредставленияТипаЗначения] = Данные.ПредставлениеТипа;
	СтруктураДанныхФормы[ПараметрыХраненияПоляСКонтейнером.ИмяПоляТипаЗначения] = Данные.Тип;
	СтруктураДанныхФормы[ПараметрыХраненияПоляСКонтейнером.ИмяПоляВСтруктуре] = Данные.Значение;
	СтруктураДанныхФормы[ПараметрыХраненияПоляСКонтейнером.ИмяПоляКонтейнера] = Данные.Контейнер;
КонецПроцедуры

// Пустое значение типа для поля с контейнером.
//
// Параметры:
//  ТипЗначения - Тип
//  			- ОписаниеТипов
...
//
// Возвращаемое значение:
//  см. НовыйСтруктураДанныхДляПоляСКонтейнеромФормы
Функция ПустоеЗначениеТипаДляПоляСКонтейнером(ТипЗначения, ХранитьВКонтейнере) Экспорт
	Если Не ХранитьВКонтейнере Тогда
		Возврат ЗначениеВСтруктуруДанныхДляПоляСКонтейнером(ПустоеЗначениеТипа(ТипЗначения));
	КонецЕсли;

#Если НаКлиенте Тогда
	Если ТипЗнч(ТипЗначения) = Тип("Структура") И ХранитьВКонтейнере Тогда
		Возврат УИ_ОбщегоНазначенияВызовСервера.ПустоеЗначениеТипаДляПоляСКонтейнером(ТипЗначения, ХранитьВКонтейнере);
	КонецЕсли;
#КонецЕсли

	ОписаниеТипов = ОписаниеТиповПоТипуДляПоляСКонтейнером(ТипЗначения);

	Возврат ЗначениеВСтруктуруДанныхДляПоляСКонтейнером(ПустоеЗначениеТипа(ОписаниеТипов));
КонецФункции

// Описание типов для поля тип хранимого в контейнере формы значения.
//
// Возвращаемое значение:
//  ОписаниеТипов - Описание типов для поля тип хранимого в контейнере формы значения
Функция ОписаниеТиповДляПоляТипХранимогоВКонтейнереФормыЗначения() Экспорт
	ДлинаСтроки = 100;
	Возврат ОписаниеТипаСтрока(ДлинаСтроки);

КонецФункции

// Значение в структуру данных для поля с контейнером.
//
// Параметры:
//  Значение - Неопределено, Произвольный -
//
// Возвращаемое значение:
//  см. НовыйСтруктураДанныхДляПоляСКонтейнеромФормы
Функция ЗначениеВСтруктуруДанныхДляПоляСКонтейнером(Значение) Экспорт
	...
	Данные = НовыйСтруктураДанныхДляПоляСКонтейнеромФормы();
	Данные.ПредставлениеТипа = Строка(ОписаниеТипа);
	Если Не ХранитьТипВКонтейнере(ТипЗначения) Тогда
		Данные.Тип = ОписаниеТипа;
		Данные.Значение = Значение;
	Иначе
		ДлинаСтроки = 100;
		Данные.Тип = ОписаниеТиповДляПоляТипХранимогоВКонтейнереФормыЗначения();
		Контейнер = УИ_ОбщегоНазначения.ЗначениеВКонтейнерДляФормы(Значение);
		УстановитьПредставлениеКонтейнера(Контейнер);
		Данные.Контейнер = Контейнер;
		Данные.Значение = Контейнер.Представление;
	КонецЕсли;
...
// Значение поля с конейнером значения по структуре данных.
//
// Параметры:
//  СтруктураЗначения - см. НовыйСтруктураДанныхДляПоляСКонтейнеромФормы
//
// Возвращаемое значение:
//  Произвольный
Функция ЗначениеПоляСКонтейнеромЗначенияПоСтруктуреДанных(СтруктураЗначения) Экспорт
	ЗначениеКонтейнера = СтруктураЗначения.Контейнер;
	ЗначениеПоля = СтруктураЗначения.Значение;
...
// Структура значения по данным формы поля с конейнером.
//
// Параметры:
//  СтруктураДанныхФормы - ФормаКлиентскогоПриложения, ДанныеФормыСтруктура, ДанныеФормыЭлементДерева, ДанныеФормыЭлементКоллекции - Структура данных формы
//  ПараметрыХраненияПоляСКонтейнером -см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСтруктураХраненияРеквизитаНаФормеСКонейнером
...
// Возвращаемое значение:
//  см. НовыйСтруктураДанныхДляПоляСКонтейнеромФормы
Функция СтруктураЗначенияПоДаннымФормыПоляСКонтейнером(СтруктураДанныхФормы, ПараметрыХраненияПоляСКонтейнером) Экспорт
	Данные = НовыйСтруктураДанныхДляПоляСКонтейнеромФормы();
	Данные.Значение = СтруктураДанныхФормы[ПараметрыХраненияПоляСКонтейнером.ИмяПоляВСтруктуре];
	Данные.Контейнер = СтруктураДанныхФормы[ПараметрыХраненияПоляСКонтейнером.ИмяПоляКонтейнера];
	Данные.Тип = СтруктураДанныхФормы[ПараметрыХраненияПоляСКонтейнером.ИмяПоляТипаЗначения];
	Данные.ПредставлениеТипа = СтруктураДанныхФормы[ПараметрыХраненияПоляСКонтейнером.ИмяПоляПредставленияТипаЗначения];

	Возврат Данные;
КонецФункции

// Приведенное к типу значение для поля с контейнером.
//
// Параметры:
//  ДанныеПоля - см. НовыйСтруктураДанныхДляПоляСКонтейнеромФормы
//
//  Тип - Тип -
//  	- ОписаниеТипов
//  	- см. НовыйХранилищеЗначенияТипа
//
// Возвращаемое значение:
//  см. НовыйСтруктураДанныхДляПоляСКонтейнеромФормы
Функция ПриведенноеКТипуЗначениеДляПоляСКонтейнером(ДанныеПоля, Тип) Экспорт
#Если НаКлиенте Тогда
	Если ДанныеПоля.Контейнер <> Неопределено Тогда
		Возврат УИ_ОбщегоНазначенияВызовСервера.ПриведенноеКТипуЗначениеДляПоляСКонтейнером(ДанныеПоля, Тип);
	КонецЕсли;
#КонецЕсли

	ОписаниеТипов = ОписаниеТиповПоТипуДляПоляСКонтейнером(Тип);

	Значение = ЗначениеПоляСКонтейнеромЗначенияПоСтруктуреДанных(ДанныеПоля);

	ПриведенноеЗначение = ОписаниеТипов.ПривестиЗначение(Значение);
	Возврат ЗначениеВСтруктуруДанныхДляПоляСКонтейнером(ПриведенноеЗначение);
КонецФункции

// Описание типов по типу для поля с контейнером.
//
// Параметры:
//  Тип - Тип
//  	- ОписаниеТипов
...
// Возвращаемое значение:
//  ОписаниеТипов
Функция ОписаниеТиповПоТипуДляПоляСКонтейнером(Тип) Экспорт
	Если ТипЗнч(Тип) = Тип("ОписаниеТипов") Тогда
		ОписаниеТипов = Тип;
	ИначеЕсли ТипЗнч(Тип) = Тип("Тип") Тогда
		ОписаниеТипов = ОписаниеТиповИзТипа(Тип);
	ИначеЕсли ТипЗнч(Тип) = Тип("Структура") Тогда
		ОписаниеТипов = УИ_ОбщегоНазначения.ЗначениеИзХранилищаКонтейнераОписанияТипов(Тип);
	Иначе
...
// Этот тип контейнера поля формы редактируется через структуру.
//
// Параметры:
//  ТипКонтейнера -Строка-Тип контейнера
//  ДоступныеТипыКонтейнеров -см. ТипыКонтейнеровЗначения
//
// Возвращаемое значение:
//  Булево - Этот тип контейнера поля формы редактируется через структуру
Функция ЭтотТипКонтейнераПоляФормыРедактируетсяЧерезСтруктуру(ТипКонтейнера, ДоступныеТипыКонтейнеров) Экспорт
	Возврат ТипКонтейнера = ДоступныеТипыКонтейнеров.Структура
			Или ТипКонтейнера = ДоступныеТипыКонтейнеров.ФиксированнаяСтруктура;
КонецФункции

// Этот тип контейнера поля формы редактируется через соответствие.
//
// Параметры:
//  ТипКонтейнера -Строка-Тип контейнера
//  ДоступныеТипыКонтейнеров -см. ТипыКонтейнеровЗначения
//
// Возвращаемое значение:
//  Булево - Этот тип контейнера поля формы редактируется через соответствие
Функция ЭтотТипКонтейнераПоляФормыРедактируетсяЧерезСоответствие(ТипКонтейнера, ДоступныеТипыКонтейнеров) Экспорт
	Возврат ТипКонтейнера = ДоступныеТипыКонтейнеров.Соответствие
			Или ТипКонтейнера = ДоступныеТипыКонтейнеров.ФиксированноеСоответствие;
КонецФункции

// Этот тип контейнера поля формы редактируется через массив.
//
// Параметры:
//  ТипКонтейнера -Строка-Тип контейнера
//  ДоступныеТипыКонтейнеров -см. ТипыКонтейнеровЗначения
//
// Возвращаемое значение:
//  Булево - Этот тип контейнера поля формы редактируется через массив
Функция ЭтотТипКонтейнераПоляФормыРедактируетсяЧерезМассив(ТипКонтейнера, ДоступныеТипыКонтейнеров) Экспорт
	Возврат ТипКонтейнера = ДоступныеТипыКонтейнеров.Массив
			Или ТипКонтейнера = ДоступныеТипыКонтейнеров.ФиксированныйМассив;
КонецФункции
...
// Текущая версия платформы 1С предприятие.
//
// Возвращаемое значение:
...
// Текущая версия платформы 1С предприятия числом.
//
// Возвращаемое значение:
//  Число
...
Функция ИмяПараметраОтмененныхДлительныхОпераций() Экспорт
...
//   ОжидаемыеТипы  - ОписаниеТипов, Тип, Массив из Тип - тип(ы) параметра процедуры или функции.
...
// Возвращает использование групп и элементов по свойству ГруппыИЭлементы.
//
// Параметры:
//  ОписаниеГруппыИЭлементы - ГруппыИЭлементы - свойство элемента
//
// Возвращаемое значение:
//  ИспользованиеГруппИЭлементов
Функция ИспользованиеГруппИЭлементов(ОписаниеГруппыИЭлементы) Экспорт

	Если ОписаниеГруппыИЭлементы = ГруппыИЭлементы.Группы Тогда
		Результат = ИспользованиеГруппИЭлементов.Группы;
	ИначеЕсли ОписаниеГруппыИЭлементы = ГруппыИЭлементы.ГруппыИЭлементы Тогда
		Результат = ИспользованиеГруппИЭлементов.ГруппыИЭлементы;
	Иначе
		Результат = ИспользованиеГруппИЭлементов.Элементы;
	КонецЕсли;
	Возврат Результат;
...
// Значения дополнительных данных отладки.
//
// Параметры:
//  ДополнительныеДанныеОтладки - Массив из Неопределено, HTTPСоединение, НастройкиКомпоновкиДанных, Структура, Строка, МенеджерВременныхТаблиц -
//
// Возвращаемое значение:
//  см. НовыйЗначенияДополнительныхДанныхОтладки
Функция ЗначенияДополнительныхДанныхОтладки(ДополнительныеДанныеОтладки)
	ЗначенияДопДанных = НовыйЗначенияДополнительныхДанныхОтладки();

	Для Каждого ЭлементДанных Из ДополнительныеДанныеОтладки Цикл
		Если ТипЗнч(ЭлементДанных) = Тип("Строка") Тогда
			ЗначенияДопДанных.Наименование = ЭлементДанных;
		ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("НастройкиКомпоновкиДанных") Тогда
			ЗначенияДопДанных.НастройкиКомпоновки = ЭлементДанных;
		ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("HTTPСоединение") Тогда
			ЗначенияДопДанных.HTTPСоединение = ЭлементДанных;
		ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("Структура") Тогда
			ЗначенияДопДанных.ВнешниеНаборыДанных = ЭлементДанных;
		ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("МенеджерВременныхТаблиц") Тогда
			ЗначенияДопДанных.МенеджерВременныхТаблиц = ЭлементДанных;
		КонецЕсли;
	КонецЦикла;

	Возврат ЗначенияДопДанных;
КонецФункции

// Сериализовать объект БДДля отладки.
//
// Параметры:
//  ОбъектОтладки - ЛюбаяСсылка - Объект отладки
//
// Возвращаемое значение:
//  см. НовыйСериализованныйДляОтладкиОбъектБазыДанных
Функция СериализованныйДляОтладкиОбъектБазыДанных(ОбъектОтладки)
	СериализованныйОбъект = НовыйСериализованныйДляОтладкиОбъектБазыДанных();
	СериализованныйОбъект.Объект = ОбъектОтладки;
