...
Функция ДанныеБиблиотекиРедактора(ИдентификаторФормы, ЭтоWindowsКлиент, ЭтоВебКлиент, ВидРедактора = Неопределено)
	...
	Для Каждого ФайлБиблиотеки Из ФайлыАрхива Цикл
		...
		МассивДляВставки = Неопределено;
		Если НРег(ФайлБиблиотеки.Расширение) = ".js" Тогда
			МассивДляВставки = ДанныеБиблиотеки.Скрипты;
		ИначеЕсли НРег(ФайлБиблиотеки.Расширение) = ".css" Тогда
			МассивДляВставки = ДанныеБиблиотеки.Стили;
		КонецЕсли;
		
		Если МассивДляВставки = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ДвоичныеДанныеФайла = Новый ДвоичныеДанные(ФайлБиблиотеки.ПолноеИмя);
		МассивДляВставки.Добавить(ПоместитьВоВременноеХранилище(ДвоичныеДанныеФайла, ИдентификаторФормы));
...
// 
// Параметры:
//  ДанныеБиблиотеки - см. ДанныеБиблиотекиРедактора
// 
// Возвращаемое значение:
//  Строка
Функция ТекстПоляHTMLРедактораAce(ДанныеБиблиотеки)
	...
	Для Каждого ТекСтиль Из ДанныеБиблиотеки.Стили Цикл
		ТекстHTML = ТекстHTML + "
								|<link href=""" + ТекСтиль + """ rel=""stylesheet"">";
	КонецЦикла;
