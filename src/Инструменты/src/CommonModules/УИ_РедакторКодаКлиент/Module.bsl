...
Процедура ПреобразоватьТекстЗапросаИзВыраженияВстроенногоЯзыкаРедактораКода(Форма, ИдентификаторРедактора) Экспорт
	...
	НовыйТекстРедактора = Новый ТекстовыйДокумент();
	...
	Для НомерСтроки =1 По ТекстовыйДокументРедактора.КоличествоСтрок() Цикл
		...
		НовыйТекстРедактора.ДобавитьСтроку(ТекСтрокаКода);
	КонецЦикла;
	
	УстановитьТекстРедактора(Форма, ИдентификаторРедактора, НовыйТекстРедактора.ПолучитьТекст());
КонецПроцедуры
...
// Параметры:
//  Форма -ФормаКлиентскогоПриложения-Форма
//  Элемент -ПолеФормы-Идентификатор редактора
