...
Функция ГраницыВыделенияРедактора(Форма, ИдентификаторРедактора) Экспорт
	...
	ИначеЕсли ВидРедактора = ВидыРедакторов.Monaco Тогда
		ДокументHTML=Форма.Элементы[ПараметрыРедактора.ПолеРедактора].Документ.defaultView;
		СтрокаСостоянияРедактора = ДокументHTML.saveViewState();
		СтруктураJSON = УИ_ОбщегоНазначенияКлиентСервер.мПрочитатьJSON(СтрокаСостоянияРедактора, Истина);
	
		ДанныеКурсора = СтруктураJSON["cursorState"];
		Если ДанныеКурсора.Количество()>0 Тогда
			ДанныеНачалаВыделения = ДанныеКурсора[0]["selectionStart"];
			Если ДанныеНачалаВыделения <> Неопределено Тогда
				ДанныеКонцаВыделения = ДанныеКурсора[0]["position"];
			
				Если ДанныеНачалаВыделения["lineNumber"] < ДанныеКонцаВыделения["lineNumber"] Тогда
					ДанныеНачало = ДанныеНачалаВыделения;
					ДанныеКонца = ДанныеКонцаВыделения;
				ИначеЕсли ДанныеНачалаВыделения["lineNumber"] > ДанныеКонцаВыделения["lineNumber"] Тогда 
					ДанныеНачало = ДанныеКонцаВыделения;
					ДанныеКонца = ДанныеНачалаВыделения;
				ИначеЕсли ДанныеНачалаВыделения["column"] < ДанныеКонцаВыделения["column"] Тогда
					ДанныеНачало = ДанныеНачалаВыделения;
					ДанныеКонца = ДанныеКонцаВыделения;
				Иначе
					ДанныеНачало = ДанныеКонцаВыделения;
				...
				ГраницыВыделения.НачалоСтроки = ДанныеНачало["lineNumber"];
				ГраницыВыделения.НачалоКолонки = ДанныеНачало["column"];
				ГраницыВыделения.КонецСтроки = ДанныеКонца["lineNumber"];
				ГраницыВыделения.КонецКолонки = ДанныеКонца["column"];
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли ВидРедактора = ВидыРедакторов.Monaco Тогда
		...
		Если ДанныеКурсора.Количество()>0 Тогда
			...
			Если ДанныеНачалаВыделения <> Неопределено Тогда
				...
				ДанныеНачалаВыделения["lineNumber"] = НачалоСтроки;
				ДанныеНачалаВыделения["column"] = НачалоКолонки;
				ДанныеКонцаВыделения["lineNumber"] = КонецСтроки;
				ДанныеКонцаВыделения["column"] = КонецКолонки;
			
				ДокументHTML.restoreViewState(УИ_ОбщегоНазначенияКлиентСервер.мЗаписатьJSON(СтруктураJSON));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
