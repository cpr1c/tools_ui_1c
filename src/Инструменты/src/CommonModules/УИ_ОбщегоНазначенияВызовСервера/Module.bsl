...
//
// Параметры:
//   Значение - ЛюбаяСсылка, ОбъектМетаданных - значение для получения полного имени объекта метаданных
//
// Возвращаемое значение:
//   Строка - полное имя объекта метаданных
//
Функция ПолноеИмяОбъектаМетаданныхПоЗначению(Значение) Экспорт
	Возврат УИ_ОбщегоНазначения.ПолноеИмяОбъектаМетаданныхПоЗначению(Значение);
КонецФункции
	...
	Если КлиентскоеПриложение.ТекущийВариантИнтерфейса() = ВариантИнтерфейсаКлиентскогоПриложения.Версия8_2 Тогда
		...
		Если Не ЗначениеЗаполнено(ИменаГрупп) Тогда
			Для Каждого Элемент Из Форма.Элементы Цикл
				Если ТипЗнч(Элемент) = Тип("ГруппаФормы") И Элемент.Вид = ВидГруппыФормы.ОбычнаяГруппа
...
// Сериализованный для отладки объект запроса.
//
// Параметры:
//  ОбъектОтладки - Запрос
//
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСериализованныйДляОтладкиЗапрос
Функция СериализованныйДляОтладкиОбъектЗапроса(ОбъектОтладки) Экспорт
	СериализованныйОбъект = УИ_ОбщегоНазначенияКлиентСервер.НовыйСериализованныйДляОтладкиЗапрос();
	СериализованныйОбъект.Текст = ОбъектОтладки.Текст;
	СериализованныйОбъект.Параметры = Новый Структура;
	Для Каждого КлючЗначение Из ОбъектОтладки.Параметры Цикл
		ТипПараметра = ТипЗнч(КлючЗначение.Значение);

		Если УИ_ОбщегоНазначения.ЭтоТабличнаяЧасть(ТипПараметра) Тогда
			СкопированноеЗначениеПараметра = КлючЗначение.Значение.Выгрузить();
		Иначе
			СкопированноеЗначениеПараметра = УИ_ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(КлючЗначение.Значение);
		КонецЕсли;

		СериализованныйОбъект.Параметры.Вставить(КлючЗначение.Ключ, СкопированноеЗначениеПараметра);
	КонецЦикла;

//	УИ_ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ОбъектОтладки.Параметры);
	Если ОбъектОтладки.МенеджерВременныхТаблиц <> Неопределено Тогда
		СериализованныйОбъект.ВременныеТаблицы = СтруктураВременныхТаблицМенеджераВременныхТаблиц(ОбъектОтладки.МенеджерВременныхТаблиц);
	КонецЕсли;
...
// Сериализовать менеджер временных таблиц для отладки.
//
// Параметры:
//  ОбъектОтладки - МенеджерВременныхТаблиц - Объект отладки
//
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСериализованныйДляОтладкиЗапрос
Функция СериализованныйДляОтладкиМенеджерВременныхТаблиц(ОбъектОтладки) Экспорт
	СериализованныйОбъект = УИ_ОбщегоНазначенияКлиентСервер.НовыйСериализованныйДляОтладкиЗапрос();
	СериализованныйОбъект.ВременныеТаблицы = СтруктураВременныхТаблицМенеджераВременныхТаблиц(ОбъектОтладки);
...
//  ДополнительныеЗначенияОбъектаОтладки - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйЗначенияДополнительныхДанныхОтладки
//
// Возвращаемое значение:
//	см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСериализованныйДляОтладкиHTTPЗапрос
Функция СериализованныйДляОтладкиHTTPЗапрос(ЗапросHTTP, ДополнительныеЗначенияОбъектаОтладки) Экспорт
	СериализованныйОбъект = УИ_ОбщегоНазначенияКлиентСервер.НовыйСериализованныйДляОтладкиHTTPЗапрос();
	СериализованныйОбъект.Запрос = ЗапросHTTP.АдресРесурса;
	СериализованныйОбъект.ТелоЗапроса = ЗапросHTTP.ПолучитьТелоКакСтроку();
	СериализованныйОбъект.Заголовки = УИ_ОбщегоНазначенияКлиентСервер.ПолучитьСтрокуЗаголовковHTTP(ЗапросHTTP.Заголовки);

	ДвоичныеДанныеТела = ЗапросHTTP.ПолучитьТелоКакДвоичныеДанные();
	СериализованныйОбъект.ДвоичныеДанныеТела = ДвоичныеДанныеТела;
	СериализованныйОбъект.ДвоичныеДанныеТелаСтрокой = Строка(ДвоичныеДанныеТела);
	СериализованныйОбъект.ИмяФайлаЗапроса = ЗапросHTTP.ПолучитьИмяФайлаТела();

	Если ДополнительныеЗначенияОбъектаОтладки.HTTPСоединение = Неопределено Тогда
		Возврат СериализованныйОбъект;
	КонецЕсли;
	СоединениеHTTP = ДополнительныеЗначенияОбъектаОтладки.HTTPСоединение;

	СериализованныйОбъект.АдресСервера = СоединениеHTTP.Сервер;
	СериализованныйОбъект.Порт = СоединениеHTTP.Порт;
	СериализованныйОбъект.ИспользоватьHTPPS = СоединениеHTTP.ЗащищенноеСоединение <> Неопределено;
	Если СоединениеHTTP.ЗащищенноеСоединение = Неопределено Тогда
		СериализованныйОбъект.Протокол = "http";
	Иначе
		СериализованныйОбъект.Протокол = "https";
	КонецЕсли;
	Если УИ_ОбщегоНазначенияКлиентСервер.ВерсияПлатформыНеМладше("8.3.7") Тогда
		СериализованныйОбъект.СоединениеИспользоватьАутентификациюОС = СоединениеHTTP.ИспользоватьАутентификациюОС;
	КонецЕсли;

	СериализованныйОбъект.ПроксиСервер = СоединениеHTTP.Прокси.Сервер(СериализованныйОбъект.Протокол);
	СериализованныйОбъект.ПроксиПорт = СоединениеHTTP.Прокси.Порт(СериализованныйОбъект.Протокол);
	СериализованныйОбъект.ПроксиПользователь = СоединениеHTTP.Прокси.Пользователь(СериализованныйОбъект.Протокол);
	СериализованныйОбъект.ПроксиПароль = СоединениеHTTP.Прокси.Пароль(СериализованныйОбъект.Протокол);
	СериализованныйОбъект.ИспользоватьАутентификациюОС = СоединениеHTTP.Прокси.ИспользоватьАутентификациюОС(СериализованныйОбъект.Протокол);
...
// Сериализованный для отладки объект схемы компоновки данных.
//
// Параметры:
//  ОбъектОтладки - СхемаКомпоновкиДанных
//  ДополнительныеЗначенияОбъектаОтладки - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйЗначенияДополнительныхДанныхОтладки
//
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСериализованнаяДляОтладкиСхемаКомпоновкиДанных
Функция СериализованныйДляОтладкиОбъектСхемыКомпоновкиДанных(ОбъектОтладки, ДополнительныеЗначенияОбъектаОтладки) Экспорт
	СериализованныйОбъект = УИ_ОбщегоНазначенияКлиентСервер.НовыйСериализованнаяДляОтладкиСхемаКомпоновкиДанных();
	СериализованныйОбъект.ТекстСКД = УИ_ОбщегоНазначения.СхемаКомпоновкиДанныхВСтроку(ОбъектОтладки);

	Если ДополнительныеЗначенияОбъектаОтладки.НастройкиКомпоновки = Неопределено Тогда
		Настройки = ОбъектОтладки.НастройкиПоУмолчанию;
	Иначе
		Настройки = ДополнительныеЗначенияОбъектаОтладки.НастройкиКомпоновки;
	КонецЕсли;

	СериализованныйОбъект.ТекстНастроекСКД = УИ_ОбщегоНазначения.НастройкиКомпоновкиДанныхВСтроку(Настройки);

	Если ДополнительныеЗначенияОбъектаОтладки.ВнешниеНаборыДанных <> Неопределено Тогда
		Для Каждого КлючЗначение Из ДополнительныеЗначенияОбъектаОтладки.ВнешниеНаборыДанных Цикл
			Если ТипЗнч(КлючЗначение.Значение) <> Тип("ТаблицаЗначений") Тогда
				Продолжить;
			КонецЕсли;

			СериализованныйОбъект.ВнешниеНаборыДанных.Вставить(КлючЗначение.Ключ,
															   ЗначениеВСтрокуВнутр(КлючЗначение.Значение));
		КонецЦикла;
	КонецЕсли;

	Если ДополнительныеЗначенияОбъектаОтладки.МенеджерВременныхТаблиц <> Неопределено Тогда
		СериализованныйОбъект.ВременныеТаблицы = СтруктураВременныхТаблицМенеджераВременныхТаблиц(ДополнительныеЗначенияОбъектаОтладки.МенеджерВременныхТаблиц);
	КонецЕсли;
...
// Сериализованный для отладки объект таблица значений.
//
// Параметры:
//  ОбъектОтладки - ТаблицаЗначений
//
// Возвращаемое значение:
//  Строка
Функция СериализованныйДляОтладкиОбъектТаблицаЗначений(ОбъектОтладки) Экспорт
	СериализованныйОбъект = ЗначениеВСтрокуВнутр(ОбъектОтладки);
	Возврат СериализованныйОбъект;

КонецФункции

// Сериализованный для отладки объект дерево значений.
//
// Параметры:
//  ОбъектОтладки - ДеревоЗначений
//
// Возвращаемое значение:
//  Строка - Сериализованный для отладки объект дерево значений
Функция СериализованныйДляОтладкиОбъектДеревоЗначений(ОбъектОтладки) Экспорт
	СериализованныйОбъект = ЗначениеВСтрокуВнутр(ОбъектОтладки);
	Возврат СериализованныйОбъект;
...
// Структура временных таблиц менеджера временных таблиц.
//
// Параметры:
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - Менеджер временных таблиц
//
// Возвращаемое значение:
//  Структура - Структура временных таблиц менеджера временных таблиц
...
#Область УниверсальноеПолеВвода

// Хранить тип в контейнере.
//
// Параметры:
//  ХранилищеТипа - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипа
//
// Возвращаемое значение:
//  Булево -  Хранить тип в контейнере
Функция ХранитьТипВКонтейнереПоКонтейнеруОписанияТипов(ХранилищеТипа) Экспорт
	Возврат УИ_ОбщегоНазначения.ХранитьТипВКонтейнереПоКонтейнеруОписанияТипов(ХранилищеТипа);
КонецФункции
...
Функция ПустоеЗначениеТипаДляПоляСКонтейнером(ТипЗначения, ХранитьВКонтейнере) Экспорт
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ПустоеЗначениеТипаДляПоляСКонтейнером(ТипЗначения, ХранитьВКонтейнере);
КонецФункции

// Приведенное к типу значение для поля с контейнером.
//
// Параметры:
//  ДанныеПоля - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйСтруктураДанныхДляПоляСКонтейнеромФормы
//
//  Тип - Тип -
//  	- ОписаниеТипов
...
Функция ПриведенноеКТипуЗначениеДляПоляСКонтейнером(ДанныеПоля, Тип) Экспорт
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ПриведенноеКТипуЗначениеДляПоляСКонтейнером(ДанныеПоля, Тип);
КонецФункции
