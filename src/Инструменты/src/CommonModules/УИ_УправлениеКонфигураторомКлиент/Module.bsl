...
Функция ПутьКВременнойБазе() Экспорт
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ОбъединитьПути(КаталогСборки(), "v8r_TempDB");
КонецФункции
...
//    КонецЕсли;
	...
	ЗапуститьИПодождать(ДополнительныеПараметры.КонтекстКоманды,
						ПараметрыЗапуска,
						Новый ОписаниеОповещения("НачатьСозданиеФайловойБазыЗавершениеВыполнения", ЭтотОбъект,
...
//  	* ОписаниеОповещенияОЗавершении - ОписаниеОповещения
//  	* ПутьКШаблону - Строка
Процедура НачатьСозданиеФайловойБазыЗавершениеВыполнения(КодВозврата, ДополнительныеПараметры) Экспорт
...
Процедура ЗапуститьИПодождать(КонтекстКоманды, Параметры, ОписаниеОповещенияОЗавершении)
	...
	НачатьЗапускПриложения(ОписаниеОповещенияОЗавершении, СтрокаЗапуска, , Истина);
