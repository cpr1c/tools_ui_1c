...
Функция _От(ОбъектОтладки, ПараметрОтладки2 = Неопределено, ПараметрОтладки3 = Неопределено,
	ПараметрОтладки4 = Неопределено, ПараметрОтладки5 = Неопределено) Экспорт
...
// Результат выполнения сохранения данных отладки
//@skip-check method-too-many-params
Функция _ОтФ(ОбъектОтладки, ПараметрОтладки2 = Неопределено, ПараметрОтладки3 = Неопределено,
	ПараметрОтладки4 = Неопределено, ПараметрОтладки5 = Неопределено) Экспорт
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ОтладитьОбъект(ОбъектОтладки,
														   Истина,
														   ПараметрОтладки2,
...
// 
// Параметры:
//  ОбъектОтладки - Запрос, СхемаКомпоновкиДанных, HTTPЗапрос, ЛюбаяСсылка, ТаблицаФормы - 
//  ВФайл -Булево-В файл
//  Параметр2 - HTTPСоединение, НастройкиКомпоновкиДанных, Строка, МенеджерВременныхТаблиц -
//  Параметр3 - HTTPСоединение, НастройкиКомпоновкиДанных, Строка, МенеджерВременныхТаблиц -
//  Параметр4 - HTTPСоединение, НастройкиКомпоновкиДанных, Строка, МенеджерВременныхТаблиц -
// 
// Возвращаемое значение:
//  Строка
Функция _Отладить_(ОбъектОтладки, ВФайл, Параметр2, Параметр3, Параметр4)
