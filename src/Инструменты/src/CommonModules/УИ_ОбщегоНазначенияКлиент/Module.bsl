...
Процедура ПолеФормыОбработчикНачалоВыбораЗначенияЗавершениеВыбораТипа(Результат, ДополнительныеПараметры) Экспорт
	...
	Если УИ_ОбщегоНазначения.ЭтоСсылка(ТипЗначения) И Не УИ_ОбщегоНазначения.ЭтоПеречислениеПоТипу(ТипЗначения) Тогда
	...
	ИначеЕсли ДополнительныеПараметры.ДоступенКонтейнер Тогда
