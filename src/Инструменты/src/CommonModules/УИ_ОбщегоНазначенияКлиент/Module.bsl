...
Функция КаталогВспомогательныхБиблиотекИнструментов() Экспорт
	...
	Возврат СтруктураФайловыхПеременных.РабочийКаталогДанныхПользователя + ?(СтрЗаканчиваетсяНа(
		СтруктураФайловыхПеременных.РабочийКаталогДанныхПользователя, ПолучитьРазделительПути()), "",
		ПолучитьРазделительПути()) + "tools_ui_1c" + ПолучитьРазделительПути() + Формат(
		УИ_ОбщегоНазначенияКлиентСервер.Версия(), "ЧГ=0;");
КонецФункции
#КонецОбласти
