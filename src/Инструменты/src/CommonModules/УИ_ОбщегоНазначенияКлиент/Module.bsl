...
Процедура НачатьВыборЗначенияПеречисления(Значение, ОписаниеОповещенияОЗавершении = Неопределено,
	...
	ОткрытьФорму("ОбщаяФорма.УИ_ФормаПеречисления",
				 ПараметрыФормы,
				 Владелец,
...
Процедура ПолеФормыНачалоВыбораЗначения(Форма, Элемент, Значение, СтандартнаяОбработка,
	...
	Если Значение = Неопределено Тогда
		...
		ДопПараметрыОповещения.Вставить("Элемент", Элемент);
		ДопПараметрыОповещения.Вставить("ОповещениеОВыбореПустогоЗначения", ОповещениеОВыбореПустогоЗначения);
		ДопПараметрыОповещения.Вставить("ОповещениеОВыбореТипа", ОповещениеОВыбореТипа);
	...
	ИначеЕсли Элемент.ОграничениеТипа <> Новый ОписаниеТипов Тогда
		НовоеЗначение = Элемент.ОграничениеТипа.ПривестиЗначение(Значение);
		...
		Если УИ_ОбщегоНазначенияВызовСервера.ЭтоПеречислениеПоТипу(ТипЗнч(Значение)) Тогда
			СтандартнаяОбработка = Ложь;
			НачатьВыборЗначенияПеречисления(Значение, , Элемент);
		ИначеЕсли ТипЗнч(Значение) = Тип("УникальныйИдентификатор") Тогда
			НачатьРедактированияУникальногоИдентификатора(Значение, , Элемент);
		КонецЕсли;
