...
Процедура НачатьВыборСсылкиРедактированиеСпецифичногоЗначения(Значение, ОписаниеОповещенияОЗавершении, ВладелецФормы,
	...
	Если КонтейнерЗначения <> Неопределено Тогда
		ОбработкаОткрытияФормыКонтейнераЗначения(КонтейнерЗначения,
												 ОписаниеОповещенияОЗавершении,
												 ВладелецФормы,
												 СтандартнаяОбработка);
	Иначе
		ОбработкаОткрытияФормыЗначенияДанных(Значение,
											 ОписаниеОповещенияОЗавершении,
											 ВладелецФормы,
											 ПараметрыРедактирования,
											 СтандартнаяОбработка);
...
Процедура ПолеФормыОбработчикНачалоВыбораЗначения(ПараметрыОбработчика, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Значение = Неопределено Тогда
		...
		Если ПараметрыОбработчика.НаборТипов = Неопределено Тогда
			ПараметрыРедактирования.НаборТипов =
				УИ_ОбщегоНазначенияКлиентСервер.НаборыТиповДляРедактированияПоУмолчаниюДляПоляФормы();
		Иначе
		...
		РедактироватьТип(ПараметрыРедактирования, ПараметрыОбработчика.Элемент, ОписаниеОповещенияЗавершенияВыбораТипа,
			СтандартнаяОбработка);
...
Процедура ОткрытьФормуВыбораОбъекта(Значение, ОписаниеОповещенияОЗавершении = Неопределено,
	...
	КонецЕсли;
	ЕстьВыборГрупп = (СтрНачинаетсяС(ИмяОбъекта, "Справочник.")
...
Процедура ОбработкаОткрытияФормыКонтейнераЗначения(КонтейнерЗначения, ОписаниеОповещенияОЗавершении, ВладелецФормы,
	СтандартнаяОбработка)
	...
	ИначеЕсли КонтейнерЗначения.Тип = ТипыКонтейнеров.СписокЗначений Тогда
		РедактироватьСписокЗначенийКакХранилищеКонтейнера(КонтейнерЗначения.Хранилище,
														  ...
														  ВладелецФормы);
	Иначе
		ОбработкаОткрытияФормыКонтейнераЗначенияПродолжение(КонтейнерЗначения,
															ТипыКонтейнеров,
															ОписаниеОповещенияОЗавершении,
															ВладелецФормы);
	КонецЕсли;
...
Процедура ОбработкаОткрытияФормыКонтейнераЗначенияПродолжение(КонтейнерЗначения, ТипыКонтейнеров,
	ОписаниеОповещенияОЗавершении, ВладелецФормы)
	
	Если КонтейнерЗначения.Тип = ТипыКонтейнеров.Массив
...
Процедура ОбработкаОткрытияФормыЗначенияДанных(Значение, ОписаниеОповещенияОЗавершении, ВладелецФормы,
	ПараметрыРедактирования, СтандартнаяОбработка)
	...
	ИначеЕсли УИ_ОбщегоНазначения.ЭтоСсылка(ТипЗначения) Тогда
		Если ИспользоватьДинамическийСписок Тогда
			...
			УИ_ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыФормы,
				ПараметрыРедактирования.ПараметрыФормы, Истина);
