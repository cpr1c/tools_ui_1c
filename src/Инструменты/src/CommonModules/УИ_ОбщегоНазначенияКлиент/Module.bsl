...
Процедура НачатьВыборСсылкиРедактированиеСпецифичногоЗначения(Значение, ОписаниеОповещенияОЗавершении, ВладелецФормы,
	...
	Иначе
		...
		Если УИ_ОбщегоНазначенияВызовСервера.ЭтоПеречислениеПоТипу(ТипЗначения) Тогда
			СтандартнаяОбработка = Ложь;
...
Функция НовыйБазовыеПараметрыОбработчикаСобытияПоля(Форма, Элемент) Экспорт
	...
	ПараметрыОбработчика.Вставить("ИмяПоляКонтейнера", Неопределено);
	ПараметрыОбработчика.Вставить("ИмяПоляТипаЗначения", Неопределено);
	ПараметрыОбработчика.Вставить("ИмяПоляПредставленияТипаЗначения", Неопределено);
...
Функция НовыйПараметрыОбработчикаСобытияНачалоВыбораЗначения(Форма, Элемент) Экспорт
	ПараметрыОбработчика = НовыйБазовыеПараметрыОбработчикаСобытияПоля(Форма, Элемент);
...
// Имя поля хранения контейнера для поля с контейнером.
// 
// Параметры:
//  ПараметрыОбработчика - см. НовыйБазовыеПараметрыОбработчикаСобытияПоля
// 
// Возвращаемое значение:
//  Строка
Функция ИмяПоляХраненияКонтейнераДляПоляСКонтейнером(ПараметрыОбработчика) Экспорт
	Если ПараметрыОбработчика.ИмяПоляКонтейнера <> Неопределено Тогда
		Возврат ПараметрыОбработчика.ИмяПоляКонтейнера;
	КонецЕсли;

	Возврат ПараметрыОбработчика.ИмяПоляВСтруктуре
			+ УИ_ОбщегоНазначенияКлиентСервер.СуффиксИмениПоляХраненияКонтейнераДляПоляСКонтейнером();
КонецФункции

// Имя поля хранения ТипаЗначения для поля с контейнером.
// 
// Параметры:
//  ПараметрыОбработчика - см. НовыйБазовыеПараметрыОбработчикаСобытияПоля
// 
// Возвращаемое значение:
//  Строка
Функция ИмяПоляХраненияТипаЗначенияДляПоляСКонтейнером(ПараметрыОбработчика) Экспорт
	Если ПараметрыОбработчика.ИмяПоляТипаЗначения <> Неопределено Тогда
		Возврат ПараметрыОбработчика.ИмяПоляТипаЗначения;
	КонецЕсли;
...
// Имя поля хранения ПредставленияТипаЗначения для поля с контейнером.
// 
// Параметры:
//  ПараметрыОбработчика - см. НовыйБазовыеПараметрыОбработчикаСобытияПоля
// 
// Возвращаемое значение:
//  Строка
Функция ИмяПоляХраненияПредставленияТипаЗначенияДляПоляСКонтейнером(ПараметрыОбработчика) Экспорт
	Если ПараметрыОбработчика.ИмяПоляПредставленияТипаЗначения <> Неопределено Тогда
		Возврат ПараметрыОбработчика.ИмяПоляПредставленияТипаЗначения;
	КонецЕсли;
...
Процедура ПолеФормыОбработчикОчистка(ПараметрыОбработчика, СтандартнаяОбработка) Экспорт
	...
	Если ПараметрыОбработчика.ДоступенКонтейнер Тогда
		ПараметрыОбработчика.СтруктураХраненияЗначения[ИмяПоляХраненияКонтейнераДляПоляСКонтейнером(ПараметрыОбработчика)]=Неопределено;
		ПараметрыОбработчика.СтруктураХраненияЗначения[ИмяПоляХраненияПредставленияТипаЗначенияДляПоляСКонтейнером(ПараметрыОбработчика)]="";
		ПараметрыОбработчика.СтруктураХраненияЗначения[ИмяПоляХраненияТипаЗначенияДляПоляСКонтейнером(ПараметрыОбработчика)]=ПустоеОписаниеТипов;
	КонецЕсли;
...
Процедура ПолеФормыОбработчикНачалоВыбораЗначения(ПараметрыОбработчика, СтандартнаяОбработка) Экспорт
	...
	Если ПараметрыОбработчика.ДоступенКонтейнер Тогда
		ЗначениеКонтейнера = ПараметрыОбработчика.СтруктураХраненияЗначения[ИмяПоляХраненияКонтейнераДляПоляСКонтейнером(ПараметрыОбработчика)];
	КонецЕсли;
...
Процедура ПолеФормыОбработчикНачалоВыбораЗначенияЗавершениеВыбораТипа(Результат, ДополнительныеПараметры) Экспорт
	...
	ТипыДляКонтейнеров = УИ_ОбщегоНазначенияКлиентСервер.ТипыЗначенийХранимыеВКонтейнерах();

	ИмяПоляТипаЗначения = ИмяПоляХраненияТипаЗначенияДляПоляСКонтейнером(ДополнительныеПараметры);
	ИмяПоляПредставленияТипаЗначения = ИмяПоляХраненияПредставленияТипаЗначенияДляПоляСКонтейнером(ДополнительныеПараметры);
	ИмяПоляКонтейнера = ИмяПоляХраненияКонтейнераДляПоляСКонтейнером(ДополнительныеПараметры);
