...
// Выполняет инициализацию параметров сеанса при начале работы системы.
// Проверяет необходимость завершения работы при изменении прав на расширение,
// сохраняет параметры клиента и сеанса в хранилище общих настроек.
...
//
// См. УИ_ОбщегоНазначения.СообщитьПользователю
//
// Параметры:
...
//
//   ДополнительныеПараметры       - см. ПараметрыВопросаПользователю
//
//
Процедура ПоказатьВопросПользователю(ОписаниеОповещенияОЗавершении, ТекстВопроса, Кнопки,
	...
	ПоказатьВопрос(ОповещениеДляЗапуска,
				   ТекстВопроса,
				   КнопкиПараметр,
				   Параметры.Таймаут,
				   Параметры.КнопкаПоУмолчанию,
				   Параметры.Заголовок,
...
//    * КнопкаПоУмолчанию             - Произвольный,Неопределено - определяет кнопку по умолчанию по типу кнопки или по связанному
//                                                     с ней значению.
//    * Таймаут                       - Число        - интервал времени в секундах до автоматического закрытия окна
//                                                     вопроса.
//    * КнопкаТаймаута                - Произвольный, Неопределено - кнопка (по типу кнопки или по связанному с ней значению),
...
// Выполняет алгоритм по ссылке с указанными параметрами. Возвращает результат выполнения алгоритма.
//
// Параметры:
//   АлгоритмСсылка - СправочникСсылка.УИ_Алгоритмы - ссылка на выполняемый алгоритм
//   ВходящиеПараметры - Структура, Неопределено - параметры для выполнения алгоритма
//   ОшибкаВыполнения - Булево - признак ошибки выполнения
...
Функция ВыполнитьАлгоритм(АлгоритмСсылка, ВходящиеПараметры = Неопределено, ОшибкаВыполнения = Ложь,
	...
	Возврат УИ_АлгоритмыКлиентСервер.ВыполнитьАлгоритм(АлгоритмСсылка,
													   ВходящиеПараметры,
													   ОшибкаВыполнения,
													   СообщениеОбОшибке)
КонецФункции
...
//  ДанныеДляОтладки - Строка - Данные для отладки
//  УникальностьФормыКонсоли - Неопределено, Строка - Уникальность формы консоли
Процедура ОткрытьКонсольОтладки(ТипОбъектаОтладки, ДанныеДляОтладки, УникальностьФормыКонсоли = Неопределено) Экспорт
	ТипОбъектаОтладкиВВерхнемРегистре = ВРег(ТипОбъектаОтладки);

	Если ТипОбъектаОтладкиВВерхнемРегистре = ВРег(УИ_ОбщегоНазначенияКлиентСервер.ТипОбъектаДляОтладкиЗАПРОС())
		 Или ТипОбъектаОтладкиВВерхнемРегистре = ВРег(УИ_ОбщегоНазначенияКлиентСервер.ТипОбъектаДляОтладкиМенеджерВременныхТаблиц()) Тогда
		ИмяФормыКонсоли = "Обработка.УИ_КонсольЗапросов.Форма";
	ИначеЕсли ТипОбъектаОтладкиВВерхнемРегистре = ВРег(УИ_ОбщегоНазначенияКлиентСервер.ТипОбъектаДляОтладкиСхемаКомпоновкиДанных())
			  Или ТипОбъектаОтладкиВВерхнемРегистре = ВРег(УИ_ОбщегоНазначенияКлиентСервер.ТипОбъектаДляОтладкиТаблицаФормы()) Тогда
		ИмяФормыКонсоли = "Отчет.УИ_КонсольОтчетов.Форма";
	ИначеЕсли ТипОбъектаОтладкиВВерхнемРегистре = ВРег(УИ_ОбщегоНазначенияКлиентСервер.ТипОбъектаДляОтладкиОбъектБазыДанных()) Тогда
		ИмяФормыКонсоли = "Обработка.УИ_РедакторРеквизитовОбъекта.Форма";
	ИначеЕсли ТипОбъектаОтладкиВВерхнемРегистре = ВРег(УИ_ОбщегоНазначенияКлиентСервер.ТипОбъектаДляОтладкиHTTPЗапрос()) Тогда
		ИмяФормыКонсоли = "Обработка.УИ_КонсольHTTPЗапросов.Форма";
	ИначеЕсли ТипОбъектаОтладкиВВерхнемРегистре = ВРег(УИ_ОбщегоНазначенияКлиентСервер.ТипОбъектаДляОтладкиТаблицаЗначений()) Тогда
		ИмяФормыКонсоли = "ОбщаяФорма.УИ_РедакторТаблицыЗначений";
	ИначеЕсли ТипОбъектаОтладкиВВерхнемРегистре = ВРег(УИ_ОбщегоНазначенияКлиентСервер.ТипОбъектаДляОтладкиДеревоЗначений()) Тогда
		ИмяФормыКонсоли = "ОбщаяФорма.УИ_РедакторТаблицыЗначений";
	Иначе
		Возврат;
	КонецЕсли;
...
// Запустить консоль отладки по адресу хранения данных отладки.
//
// Параметры:
//  АдресХранилищаНастроекОтладки - Строка
//  ЭтоФайл - Булево
//  ТипОбъектаОтладки - Строка
...
Процедура ЗапуститьКонсольОтладкиПоАдресуХраненияДанныхОтладки(АдресХранилищаНастроекОтладки, ЭтоФайл, ТипОбъектаОтладки,
	...
	Если АдресОбъектаОтладки = Неопределено Тогда
		УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не удалось получить данные для консоли из хранилища");
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФормуРедактированияТекста(Текст, ОписаниеОповещенияОЗакрытии, Заголовок = "",
	...
	Если ЭтоАдресВременногоХранилища(Текст) Тогда
		НачатьРедактированиеВременногоХранилищаПоАдресу(Текст, ОписаниеОповещенияОЗакрытии, Заголовок);
		Возврат;
	КонецЕсли;
...
// Начать редактирование временного хранилища по адресу.
//
// Параметры:
...
//  Заголовок - Строка -
//  Владелец - Неопределено, ФормаКлиентскогоПриложения, ПолеФормы - Владелец
Процедура НачатьРедактированиеВременногоХранилищаПоАдресу(АдресВоВременномХранилище,
	ОписаниеОповещенияОЗавершении = Неопределено, Заголовок = "", Владелец = Неопределено) Экспорт

	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Заголовок", Заголовок);
	ПараметрыФормы.Вставить("АдресХранилища", АдресВоВременномХранилище);
...
// Открывает форму редактирования объекта базы данных.
//
// Параметры:
//  СсылкаНаОбъект - Произвольный - Ссылка на объект
//  АдресаХранилищ - Структура - Адреса хранилищ
Процедура РедактироватьОбъект(СсылкаНаОбъект, АдресаХранилищ = Неопределено) Экспорт
	...
	Если АдресаХранилищ <> Неопределено Тогда
		ПараметрыФормы.Вставить("АдресаХранилищ", АдресаХранилищ);
	КонецЕсли;
...
// Открывает динамический список объекта метаданных.
//
// Параметры:
//  ИмяОбъектаМетаданных - Строка - Имя объекта метаданных
//  ОписаниеОповещенияОЗавершении - ОписаниеОповещения - Описание оповещения о завершении
//  ПараметрыФормы - Структура - Параметры формы
//  ВладелецФормы - Произвольный - Владелец формы
Процедура ОткрытьДинамическийСписок(ИмяОбъектаМетаданных, ОписаниеОповещенияОЗавершении = Неопределено,
		ПараметрыФормы = Неопределено, ВладелецФормы = Неопределено) Экспорт

	Если ПараметрыФормы = Неопределено Тогда
		СтрукПараметры = Новый Структура();
	Иначе
		СтрукПараметры = ПараметрыФормы;
	КонецЕсли;
	СтрукПараметры.Вставить("ИмяОбъектаМетаданных", ИмяОбъектаМетаданных);

	Если ОписаниеОповещенияОЗавершении = Неопределено Тогда
		ОткрытьФорму("Обработка.УИ_ДинамическийСписок.Форма", СтрукПараметры, ВладелецФормы, ИмяОбъектаМетаданных);
	Иначе
		ОткрытьФорму("Обработка.УИ_ДинамическийСписок.Форма", СтрукПараметры, ВладелецФормы, ИмяОбъектаМетаданных, , ,
			ОписаниеОповещенияОЗавершении);
	КонецЕсли;
...
// Открывает консоль кода.
Процедура ОткрытьКонсольКода() Экспорт

	ОткрытьФорму("Обработка.УИ_КонсольКода.Форма");
...
// Открывает редактирование схемы компоновки данных. В толстом клиенте используется стандартный конструктор,
// в остальных случаях открывается форма редактора.
//
// Параметры:
//   СхемаКомпоновкиДанных - СхемаКомпоновкиДанных - Передается только для толстого клиента
//   					   - Строка - Передается в сериализованном виде для остальных видов клиента
//   ФормаВладелец - Форма - форма, для которой открывается редактор
//   ОписаниеОповещенияОЗакрытии - ОписаниеОповещения
//                               - Неопределено - оповещение о закрытии редактора
Процедура ОткрытьКонструкторСхемыКомпоновкиДанных(СхемаКомпоновкиДанных, ФормаВладелец,
...
#Если ТолстыйКлиентОбычноеПриложение Или ТолстыйКлиентУправляемоеПриложение Тогда
	...
	Конструктор.Редактировать(ФормаВладелец);
#Иначе
	ПараметрыРедактора = Новый Структура;
	ПараметрыРедактора.Вставить("СКД", СхемаКомпоновкиДанных);
	ОткрытьФорму("Обработка.УИ_РедакторСКД.Форма", ПараметрыРедактора, ФормаВладелец, , , , ОписаниеОповещенияОЗакрытии);
#КонецЕсли

КонецПроцедуры

#Область НавигаторПоМетаданным

// Открыть свойства объекта метаданных в навигаторе.
//
// Параметры:
//  ПолноеИмяОбъектаМетаданных - Строка - Полное имя объекта метаданных
Процедура ОткрытьСвойстваОбъектаМетаданныхВНавигаторе(ПолноеИмяОбъектаМетаданных) Экспорт
	ПараметрыОткрытия = НовыйПараметрыОткрытияОбработкиДляПросмотраСвойствОбъектаМетаданных();
	ПараметрыОткрытия.Параметры.ПолноеИмя = ПолноеИмяОбъектаМетаданных;
	ФормаНавигатора = ФормаНавигатораПоМетаданным(Истина);

	Результат = ФормаНавигатора.вОбработатьВнешнийВызов(ПараметрыОткрытия);
	Если Результат.Отказ Тогда
		СообщитьПользователю(Результат.ПричинаОтказа);
	КонецЕсли;
КонецПроцедуры

// Открывает форму обработки "навигатор по метаданным" для просмотра свойств метаданных
//
// Параметры:
//   СсылкаНаОбъект - ЛюбаяСсылка - ссылка на объект, свойства метаданных которого нужно просмотреть
//
Процедура ОткрытьСвойстваМетаданыхСсылкиВНавигаторе(СсылкаНаОбъект) Экспорт
	ПолноеИмяОбъектаМетаданных = УИ_ОбщегоНазначенияВызовСервера.ПолноеИмяОбъектаМетаданныхПоЗначению(СсылкаНаОбъект);
	Если ПолноеИмяОбъектаМетаданных = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ОткрытьСвойстваОбъектаМетаданныхВНавигаторе(ПолноеИмяОбъектаМетаданных);

КонецПроцедуры

// Открывает навигатор по метаданным.
Процедура ОткрытьНавигаторПоКонфигурации() Экспорт
	ОткрытьФорму("Обработка.УИ_НавигаторПоМетаданным.Форма");
КонецПроцедуры
...
Функция ФормаНавигатораПоМетаданным(ОткрытьФорму = Ложь) Экспорт
	...
	ФормаНавигатора = ОткрытаяФормаНавигатораПоМетаданным();

	Если ОткрытьФорму И ФормаНавигатора = Неопределено Тогда
		ОткрытьНавигаторПоКонфигурации();
...
// Открывает форму обработки "Навигатор по метаданным". Используется в БСП "Подключаемые команды"
//
// Параметры:
// СсылкаНаОбъект - СправочникСсылка, ДокументСсылка - Ссылка на объект, для которого открывается форма обработки
// Контекст - Структура - Структура, содержащая контекст выполнения команды
//
Процедура ОбработчикКомандыНавигаторПоМетаданным(СсылкаНаОбъект, Контекст) Экспорт
	ОткрытьСвойстваМетаданыхСсылкиВНавигаторе(СсылкаНаОбъект);
КонецПроцедуры
...
// Новый параметры редактирования типа.
//
// Возвращаемое значение:
//  Структура - Новый параметры редактирования типа:
// * ТипДанных - Неопределено, ОписаниеТипов, Строка - начальное значение типа
// * РежимЗапуска - Неопределено, Число - режим запуска редактора типа
// 										см. УИ_ОбщегоНазначенияКлиентСервер.РежимыЗапускаРедактораТипа
// * НаборТипов - Строка - Перечисление наборов
//  - СписокЗначений из Строка - Список наборов
//  - Массив из Строка - Массив наборов
//  - Неопределено - Когда набор не определен
// * ОграничениеТипа - Неопределено, ОписаниеТипов, Строка, Структура - Ограничение списка типов
// * ДоступенСоставнойТип - Булево - Можно указать составной тип
// * РежимВыбора - Булево - Устанавливается режим выбора
// * ИспользоватьВыборФормыВыбораСсылочногоЗначения - Булево - использовать выбор формы выбора для ссылочного значения
// * ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения - Булево
//  - использовать динамический список из инструмента см. Обработка.УИ_ДинамическийСписок
// * ХранилищеКонтейнераЗначенияОписанияТипа - Неопределено, Структура -
//  - Описание хранилища см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипа
// * ХранилищеКонтейнераЗначенияТипа - Неопределено, Структура -
//  - Описание хранилища см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипа
Функция НовыйПараметрыРедактированияТипа() Экспорт

	РежимыЗапуска = УИ_ОбщегоНазначенияКлиентСервер.РежимыЗапускаРедактораТипа();
	ПараметрыРедактирования = Новый Структура;
	ПараметрыРедактирования.Вставить("ТипДанных", Неопределено);
	ПараметрыРедактирования.Вставить("РежимЗапуска", РежимыЗапуска.ВсеТипы);
	ПараметрыРедактирования.Вставить("НаборТипов", Неопределено);
	ПараметрыРедактирования.Вставить("ОграничениеТипа", Неопределено);
	ПараметрыРедактирования.Вставить("ДоступенСоставнойТип", Истина);
	ПараметрыРедактирования.Вставить("РежимВыбора", Ложь);
	ПараметрыРедактирования.Вставить("ИспользоватьВыборФормыВыбораСсылочногоЗначения", Истина);
	ПараметрыРедактирования.Вставить("ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения", Ложь);
	ПараметрыРедактирования.Вставить("ХранилищеКонтейнераЗначенияОписанияТипа", Неопределено);
	ПараметрыРедактирования.Вставить("ХранилищеКонтейнераЗначенияТипа", Неопределено);
	Возврат ПараметрыРедактирования;
...
// Открывает форму редактирования типа.
//
// Параметры:
//  ПараметрыРедактирования - Структура - Параметры редактирования типа см. НовыйПараметрыРедактированияТипа
//  ВладелецФормы - Неопределено, УправляемаяФорма, ПолеФормы - Владелец формы
//  ОписаниеОповещенияОЗавершении - Неопределено, ОписаниеОповещения - Описание оповещения о завершении
//  СтандартнаяОбработка - Неопределено, Булево - Стандартная обработка события начало выбора
Процедура РедактироватьТип(ПараметрыРедактирования, ВладелецФормы = Неопределено,
	ОписаниеОповещенияОЗавершении = Неопределено, СтандартнаяОбработка = Неопределено) Экспорт

	СтандартнаяОбработка = Ложь;
	ОписаниеОповещенияДляФормы = ОписаниеОповещенияОЗавершении;
	Если ОписаниеОповещенияДляФормы = Неопределено Тогда
		ОписаниеОповещенияДляФормы = НовыйПустоеОписаниеОповещения();
	КонецЕсли;
...
//  ОписаниеОповещенияОЗавершении - ОписаниеОповещения -Описание оповещения о завершении
//  ВладелецФормы - Неопределено, УправляемаяФорма, ПолеФормы -
//  ПараметрыРедактирования - Структура - структура см. НовыйПараметрыНачатьВыборСсылкиРедактирования
//  СтандартнаяОбработка - Булево
Процедура НачатьВыборСсылкиРедактированиеСпецифичногоЗначения(Значение, ОписаниеОповещенияОЗавершении, ВладелецФормы,
	ПараметрыРедактирования, СтандартнаяОбработка = Истина) Экспорт

	КонтейнерЗначения = ПараметрыРедактирования.КонтейнерЗначения;
	Если КонтейнерЗначения <> Неопределено Тогда
		НачатьРедактированиеКонтейнераЗначенияВСпециализированнойФорме(КонтейнерЗначения,
																	   ОписаниеОповещенияОЗавершении,
																	   ВладелецФормы,
																	   СтандартнаяОбработка);
	Иначе
		НачатьРедактированиеЗначенияБезКонтейнераНаФормеВСпециализированнойФорме(Значение,
																				 ОписаниеОповещенияОЗавершении,
																				 ВладелецФормы,
																				 ПараметрыРедактирования,
																				 СтандартнаяОбработка);
...
// * ТекущееОписаниеТиповЗначения 	- Неопределено -
// 									- ОписаниеТипов -
...
// * ПараметрыФормы - Структура - Параметры открытия формы редактирования специфичных значений
// * ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения - Булево -
// * ОграничениеВыбораЗначенияОписанияТипов - ОписаниеТипов -
Функция НовыйПараметрыОбработчикаСобытияНачалоВыбораЗначения(Форма, Элемент, ИмяПоля) Экспорт
	...
	ПараметрыОбработчика.Вставить("ПараметрыФормы", Новый Структура());
	ПараметрыОбработчика.Вставить("ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения", Ложь);
	ПараметрыОбработчика.Вставить("ОграничениеВыбораЗначенияОписанияТипов", Новый ОписаниеТипов());
...
// Новый параметры начать выбор ссылки редактирования.
//
// Возвращаемое значение:
//  Структура - Новый параметры начать выбор ссылки редактирования:
//  * КонтейнерЗначения - Неопределено, Структура - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйКонтейнерЗначения
//  * ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения - Булево
//  * ОграничениеВыбораЗначенияОписанияТипов - ОписаниеТипов
//  * ПараметрыФормы - Структура - Параметры открытия формы редактирования специфичных значений
Функция НовыйПараметрыНачатьВыборСсылкиРедактирования() Экспорт
	НовыеПараметры = Новый Структура;
	НовыеПараметры.Вставить("КонтейнерЗначения", Неопределено);
	НовыеПараметры.Вставить("ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения", Ложь);
	НовыеПараметры.Вставить("ОграничениеВыбораЗначенияОписанияТипов", Новый ОписаниеТипов());
	НовыеПараметры.Вставить("ПараметрыФормы", Новый Структура());
...
//	НовоеОписаниеТипов = Новый ОписаниеТипов;
	...
	Если ТипЗнч(ПараметрыОбработчика.ТекущееОписаниеТиповЗначения) = Тип("ОписаниеТипов") Тогда
	...
	Если ПараметрыОбработчика.ДоступенКонтейнер Тогда
		Если ТипЗнч(ПараметрыОбработчика.ТекущееОписаниеТиповЗначения) = Тип("Структура") Тогда
			ХранитьВКонтейнере = УИ_ОбщегоНазначения.ХранитьТипВКонтейнереПоКонтейнеруОписанияТипов(ПараметрыОбработчика.ТекущееОписаниеТиповЗначения);
			НовоеЗначение = УИ_ОбщегоНазначенияКлиентСервер.ПустоеЗначениеТипаДляПоляСКонтейнером(ПараметрыОбработчика.ТекущееОписаниеТиповЗначения,
																								  ХранитьВКонтейнере);
			УИ_ОбщегоНазначенияКлиентСервер.УстановитьЗначениеПоляСКонтейнеромПоСтруктуреДанныхДляПоляСКонтейнером(ПараметрыОбработчика.СтруктураХраненияЗначения,
...
Процедура ПолеФормыОбработчикНачалоВыбораЗначения(ПараметрыОбработчика, СтандартнаяОбработка) Экспорт
	...
	Если ПараметрыОбработчика.ДоступенКонтейнер И ЗначениеКонтейнера <> Неопределено Тогда
		...
		ПараметрыРедактирования = НовыйПараметрыНачатьВыборСсылкиРедактирования();
		ПараметрыРедактирования.КонтейнерЗначения = ЗначениеКонтейнера;
		ПараметрыРедактирования.ПараметрыФормы = ПараметрыОбработчика.ПараметрыФормы;
		ПараметрыРедактирования.ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения =
	...
	ИначеЕсли Значение = Неопределено Тогда
		...
		ПараметрыРедактирования = НовыйПараметрыРедактированияТипа();
		ПараметрыРедактирования.ДоступенСоставнойТип = Ложь;
		ПараметрыРедактирования.РежимВыбора = Истина;
		ПараметрыРедактирования.ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения = ПараметрыОбработчика.ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения;
		Если ПараметрыОбработчика.НаборТипов = Неопределено Тогда
			ПараметрыРедактирования.НаборТипов = УИ_ОбщегоНазначенияКлиентСервер.НаборыТиповДляРедактированияПоУмолчаниюДляПоляФормы();
		Иначе
			ПараметрыРедактирования.НаборТипов = ПараметрыОбработчика.НаборТипов;
		КонецЕсли;
		Если ТипЗнч(ПараметрыОбработчика.ТекущееОписаниеТиповЗначения) = Тип("ОписаниеТипов") Тогда
			МассивТипов = ПараметрыОбработчика.ТекущееОписаниеТиповЗначения.Типы();
			КоличествоТиповОграничения = МассивТипов.Количество();
			ПараметрыРедактирования.ОграничениеТипа = ПараметрыОбработчика.ТекущееОписаниеТиповЗначения;
		ИначеЕсли ТипЗнч(ПараметрыОбработчика.ТекущееОписаниеТиповЗначения) = Тип("Структура") Тогда
			КоличествоТиповОграничения = ПараметрыОбработчика.ТекущееОписаниеТиповЗначения.КоличествоТипов;
			ПараметрыРедактирования.ОграничениеТипа = ПараметрыОбработчика.ТекущееОписаниеТиповЗначения;
		Иначе
			КоличествоТиповОграничения = 0;
		КонецЕсли;
		Если КоличествоТиповОграничения = 1 Тогда
			...
			РезультатВыбораТипов.Вставить("ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения",
										  ПараметрыОбработчика.ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения);
		...
		РедактироватьТип(ПараметрыРедактирования,
						 ПараметрыОбработчика.Элемент,
	...
	Иначе//Если ПараметрыОбработчика.Элемент.ОграничениеТипа <> Новый ОписаниеТипов Тогда
		...
		ПараметрыРедактирования.ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения =
			ПараметрыОбработчика.ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения;
		ПараметрыРедактирования.ОграничениеВыбораЗначенияОписанияТипов =
			ПараметрыОбработчика.ОграничениеВыбораЗначенияОписанияТипов;
		НачатьВыборСсылкиРедактированиеСпецифичногоЗначения(Значение,
															ОписаниеЗавершенияВыбораЗначения,
															ПараметрыОбработчика.Элемент,
															ПараметрыРедактирования,
...
//  			- см. УИ_ОбщегоНазначенияКлиентСервер.НовыйКонтейнерЗначения
//  * ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения - Булево
...
Процедура ПолеФормыОбработчикНачалоВыбораЗначенияЗавершениеВыбораТипа(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	КоличествоТиповВОписании = 0;
	Если ТипЗнч(Результат.Описание) = Тип("ОписаниеТипов") Тогда
		КоличествоТиповВОписании = Результат.Описание.Типы().Количество();
	ИначеЕсли ТипЗнч(Результат.Описание) = Тип("Структура") Тогда
		КоличествоТиповВОписании = Результат.Описание.КоличествоТипов;
	КонецЕсли;

	Если КоличествоТиповВОписании = 0 Тогда
		Возврат;
	КонецЕсли;

	ДополнительныеПараметры.Форма.Модифицированность = Истина;

	Если ТипЗнч(Результат.Описание) = Тип("Структура") Тогда
		ХранитьВКонтейнере = УИ_ОбщегоНазначения.ХранитьТипВКонтейнереПоКонтейнеруОписанияТипов(Результат.Описание);
	Иначе
		ХранитьВКонтейнере = УИ_ОбщегоНазначенияКлиентСервер.ХранитьТипВКонтейнере(Результат.Описание.Типы()[0]);
	КонецЕсли;
	...
	ДанныеДляПоля = УИ_ОбщегоНазначенияКлиентСервер.ПустоеЗначениеТипаДляПоляСКонтейнером(Результат.Описание,
																						  ХранитьВКонтейнере);

	Если ДополнительныеПараметры.ДоступенКонтейнер Тогда
		УИ_ОбщегоНазначенияКлиентСервер.УстановитьЗначениеПоляСКонтейнеромПоСтруктуреДанныхДляПоляСКонтейнером(ДополнительныеПараметры.СтруктураХраненияЗначения,
																											   ДополнительныеПараметры,
																											   ДанныеДляПоля);
	Иначе
		ДополнительныеПараметры.Элемент.ОграничениеТипа = ДанныеДляПоля.Тип;
	КонецЕсли;

	ДополнительныеПараметры.СтруктураХраненияЗначения[ДополнительныеПараметры.ИмяПоляВСтруктуре] = ДанныеДляПоля.Значение;

	Если Не ХранитьВКонтейнере Тогда
		ТипЗначения = ДанныеДляПоля.Тип.Типы()[0];
	...
	ПараметрыРедактирования = НовыйПараметрыНачатьВыборСсылкиРедактирования();
	Если ДополнительныеПараметры.ДоступенКонтейнер Тогда
		ПараметрыРедактирования.КонтейнерЗначения = ДанныеДляПоля.Контейнер;
	КонецЕсли;
	ПараметрыРедактирования.ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения = Результат.ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения;
	ПараметрыРедактирования.ПараметрыФормы = ДополнительныеПараметры.ПараметрыФормы;
	НачатьВыборСсылкиРедактированиеСпецифичногоЗначения(ДанныеДляПоля.Значение,
														ОписаниеЗавершенияВыбораЗначения,
...
//  ПаузаПередВосстановлениемПароля - Число - Пауза в секундах перед восстановлением пароля
//
Процедура ЗапуститьСеанс1С(ТипКлиента, Пользователь, РежимЗапускаПодПользователем = Ложь,
	...
	Иначе
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось определить строку соединения с текущей базой: %1'"),
			СтрокаСоединения);
	...
	Попытка
		НачатьЗапускПриложения(Новый ОписаниеОповещения("ЗапуститьСеанс1СЗавершение", ЭтотОбъект,
...
// Завершение запуска сеанс 1С.
//
// Параметры:
//  Результат - Число, Неопределено - Код возврата, тип Число или Неопределено, если соответствующий входной параметр <ДождатьсяЗавершения> не указан
//  ДополнительныеПараметры - Структура -  значение, которое было указано при создании объекта ОписаниеОповещения
Процедура ЗапуститьСеанс1СЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
// Открытая форма навигатора по метаданным.
//
// Возвращаемое значение:
//  Неопределено, ФормаКлиентскогоПриложения - Открытая форма навигатора по метаданным
Функция ОткрытаяФормаНавигатораПоМетаданным()
...
// Открывает форму выбора объекта.
//
// Параметры:
//  Значение - ЛюбаяСсылка - Значение
//  ОписаниеОповещенияОЗавершении - ОписаниеОповещения - Описание оповещения о завершении
//  ПараметрыФормы - Структура - Параметры формы
//  ВладелецФормы - ПолеФормы - Владелец формы
Процедура ОткрытьФормуВыбораОбъекта(Значение, ОписаниеОповещенияОЗавершении = Неопределено,
	ПараметрыФормы = Неопределено, ВладелецФормы = Неопределено)
	...
	Если ПараметрыФормы <> Неопределено Тогда
		УИ_ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыФормыВыбора, ПараметрыФормы, Истина);
	КонецЕсли;
	ЕстьВыборГрупп = (СтрНачинаетсяС(ИмяОбъекта, "Справочник.") Или СтрНачинаетсяС(ИмяОбъекта,
																				   "ПланВидовХарактеристик."));
	Если ЕстьВыборГрупп
		 И ПараметрыФормыВыбора.Свойство("ВыборГруппИЭлементов")
		 И ПараметрыФормыВыбора.ВыборГруппИЭлементов = ИспользованиеГруппИЭлементов.Группы Тогда
		ФормаВыбора = "ФормаВыбораГруппы";
	Иначе
		ФормаВыбора = "ФормаВыбора";
	КонецЕсли;
	ПутьКФорме = СтрШаблон("%1.%2", ИмяОбъекта, ФормаВыбора);
	Если ОписаниеОповещенияОЗавершении = Неопределено Тогда
		ОткрытьФорму(ПутьКФорме, ПараметрыФормыВыбора, ВладелецФормы);
	Иначе
		ОткрытьФорму(ПутьКФорме, ПараметрыФормыВыбора, ВладелецФормы, , , , ОписаниеОповещенияОЗавершении);
	КонецЕсли;

КонецПроцедуры

// Начать редактирование контейнера значения в специализированной форме.
//
// Параметры:
...
//  ВладелецФормы - Неопределено, УправляемаяФорма, ПолеФормы -
//  СтандартнаяОбработка - Булево
Процедура НачатьРедактированиеКонтейнераЗначенияВСпециализированнойФорме(КонтейнерЗначения,
	ОписаниеОповещенияОЗавершении, ВладелецФормы, СтандартнаяОбработка)

	СтандартнаяОбработка = Ложь;
	ТипыКонтейнеров = УИ_ОбщегоНазначенияКлиентСервер.ТипыКонтейнеровЗначения();

	ТипКонтейнера = КонтейнерЗначения.Тип;

	Если ТипКонтейнера = ТипыКонтейнеров.Граница Тогда
		РедактироватьТипГраница(КонтейнерЗначения.Хранилище, ОписаниеОповещенияОЗавершении, ВладелецФормы);
	ИначеЕсли ТипКонтейнера = ТипыКонтейнеров.МоментВремени Тогда
		РедактироватьМоментВремени(КонтейнерЗначения.Хранилище, ОписаниеОповещенияОЗавершении, ВладелецФормы);
	ИначеЕсли ТипКонтейнера = ТипыКонтейнеров.ТаблицаЗначений Тогда
		РедактироватьТаблицуЗначенийКакХранилищеКонтейнера(КонтейнерЗначения.Хранилище,
														   ОписаниеОповещенияОЗавершении,
														   ВладелецФормы);
	ИначеЕсли ТипКонтейнера = ТипыКонтейнеров.ДеревоЗначений Тогда
		РедактироватьДеревоЗначенийКакХранилищеКонтейнера(КонтейнерЗначения.Хранилище,
														  ОписаниеОповещенияОЗавершении,
														  ВладелецФормы);
	ИначеЕсли ТипКонтейнера = ТипыКонтейнеров.Тип Тогда

		ПараметрыОткрытия = НовыйПараметрыРедактированияТипа();
		ПараметрыОткрытия.ХранилищеКонтейнераЗначенияТипа = КонтейнерЗначения.Хранилище;
		ПараметрыОткрытия.НаборТипов = УИ_ОбщегоНазначенияКлиентСервер.ВсеНаборыТиповДляРедактирования();
		ПараметрыОткрытия.ДоступенСоставнойТип = Ложь;
		ПараметрыОткрытия.РежимВыбора = Истина;
		РедактироватьТип(ПараметрыОткрытия, ВладелецФормы, ОписаниеОповещенияОЗавершении);

	ИначеЕсли ТипКонтейнера = ТипыКонтейнеров.ОписаниеТипов Тогда

		ПараметрыОткрытия = НовыйПараметрыРедактированияТипа();
		ПараметрыОткрытия.ХранилищеКонтейнераЗначенияОписанияТипа = КонтейнерЗначения.Хранилище;
		ПараметрыОткрытия.НаборТипов = УИ_ОбщегоНазначенияКлиентСервер.ВсеНаборыТиповДляРедактирования();
		РедактироватьТип(ПараметрыОткрытия, ВладелецФормы, ОписаниеОповещенияОЗавершении);

	ИначеЕсли УИ_ОбщегоНазначенияКлиентСервер.ЭтотТипКонтейнераПоляФормыРедактируетсяЧерезСтруктуру(ТипКонтейнера,
																									ТипыКонтейнеров) Тогда
		РедактироватьТипСтруктура(КонтейнерЗначения.Хранилище, ОписаниеОповещенияОЗавершении, ВладелецФормы);
	ИначеЕсли УИ_ОбщегоНазначенияКлиентСервер.ЭтотТипКонтейнераПоляФормыРедактируетсяЧерезСоответствие(ТипКонтейнера,
																									   ТипыКонтейнеров) Тогда
		РедактироватьТипСоответствие(КонтейнерЗначения.Хранилище, ОписаниеОповещенияОЗавершении, ВладелецФормы);
	ИначеЕсли ТипКонтейнера = ТипыКонтейнеров.ТабличныйДокумент Тогда
		РедактироватьТабличныйДокументКакХранилищеКонтейнера(КонтейнерЗначения.Хранилище,
															 ОписаниеОповещенияОЗавершении,
															 ВладелецФормы);
	ИначеЕсли ТипКонтейнера = ТипыКонтейнеров.ХранилищеЗначения Тогда
		РедактироватьТипХранилищеЗначенияКакХранилищеКонтейнера(КонтейнерЗначения.Хранилище,
																ОписаниеОповещенияОЗавершении,
																ВладелецФормы);
	ИначеЕсли ТипКонтейнера = ТипыКонтейнеров.СписокЗначений Тогда
		РедактироватьСписокЗначенийКакХранилищеКонтейнера(КонтейнерЗначения.Хранилище,
														  ОписаниеОповещенияОЗавершении,
														  ВладелецФормы);
	ИначеЕсли УИ_ОбщегоНазначенияКлиентСервер.ЭтотТипКонтейнераПоляФормыРедактируетсяЧерезМассив(ТипКонтейнера,
																								 ТипыКонтейнеров) Тогда
		РедактироватьМассивКакХранилищеКонтейнера(КонтейнерЗначения.Хранилище,
												  ОписаниеОповещенияОЗавершении,
												  ВладелецФормы);
	ИначеЕсли ТипКонтейнера = ТипыКонтейнеров.Картинка Тогда
		РедактироватьКартинкуКакХранилищеКонтейнера(КонтейнерЗначения.Хранилище,
													ОписаниеОповещенияОЗавершении,
													ВладелецФормы);
	ИначеЕсли ТипКонтейнера = ТипыКонтейнеров.ДвоичныеДанные Тогда
...
// Начать редактирование значения без контейнера на форме в специализированной форме.
//
// Параметры:
//  Значение  - Произвольный -Значение
//  ОписаниеОповещенияОЗавершении - ОписаниеОповещения -Описание оповещения о завершении
...
//  ПараметрыРедактирования - Структура - структура см. НовыйПараметрыНачатьВыборСсылкиРедактирования
//  СтандартнаяОбработка - Булево
Процедура НачатьРедактированиеЗначенияБезКонтейнераНаФормеВСпециализированнойФорме(Значение,
	ОписаниеОповещенияОЗавершении, ВладелецФормы, ПараметрыРедактирования, СтандартнаяОбработка)

	ИспользоватьДинамическийСписок = ПараметрыРедактирования.ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения;
	ОграничениеВыбораЗначенияОписанияТипов = ПараметрыРедактирования.ОграничениеВыбораЗначенияОписанияТипов;
	...
	ИначеЕсли УИ_ОбщегоНазначения.ЭтоСсылка(ТипЗначения) Тогда
		СтандартнаяОбработка = Ложь;
		Если ИспользоватьДинамическийСписок Тогда
			ИмяОбъекта = УИ_ОбщегоНазначения.ИмяТаблицыПоСсылке(Значение);
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("РежимВыбора", Истина);
			ПараметрыФормы.Вставить("ТекущаяСтрока", Значение);
			УИ_ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыФормы,
															   ПараметрыРедактирования.ПараметрыФормы,
															   Истина);
			ОткрытьДинамическийСписок(ИмяОбъекта, , ПараметрыФормы, ВладелецФормы);
		Иначе
			ОткрытьФормуВыбораОбъекта(Значение, , ПараметрыРедактирования.ПараметрыФормы, ВладелецФормы);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("ОписаниеТипов") Тогда
		СтандартнаяОбработка = Ложь;
		ПараметрыОткрытия = НовыйПараметрыРедактированияТипа();
		ПараметрыОткрытия.ДоступенСоставнойТип = Истина;
		ПараметрыОткрытия.ТипДанных = Значение;
		ПараметрыОткрытия.ОграничениеТипа = ОграничениеВыбораЗначенияОписанияТипов;
		ПараметрыОткрытия.НаборТипов = УИ_ОбщегоНазначенияКлиентСервер.НаборыТиповДляРедактированияПоУмолчаниюДляПоляФормы();
		ПараметрыОткрытия.ИспользоватьВыборФормыВыбораСсылочногоЗначения = Ложь;
		РедактироватьТип(ПараметрыОткрытия, ВладелецФормы, ОписаниеОповещенияОЗавершении);
	Иначе
		// Выполнять типовое поведение редактирование значения
