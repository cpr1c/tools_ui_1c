...
// Возвращает данные буфера обмена.
// 
// Возвращаемое значение:
//  Строка, Неопределено - строковое значение из буфера обмена, Неопределено - если не удалось получить данные
Функция ДанныеБуфераОбмена() Экспорт
	
	Попытка
		COMОбъект = Новый COMОбъект("htmlfile");
		Данные = COMОбъект.ParentWindow.ClipboardData.Getdata("Text");
	Исключение
		Данные = Неопределено;
	КонецПопытки;
...
// 
// Параметры:
//  Данные - Строка - Данные, которые нужно поместить в буфер обмена
// 
// Возвращаемое значение:
//  Булево - успешность помещения данных в буфер обмена
Функция ПоместитьДанныеВБуферОбмена(Знач Данные) Экспорт
	
	Попытка
		COMОбъект = Новый COMОбъект("htmlfile");
    	COMОбъект.ParentWindow.ClipboardData.SetData("Text", Данные);
    	Результат = Истина;
	Исключение
		Результат = Ложь;
	КонецПопытки;
