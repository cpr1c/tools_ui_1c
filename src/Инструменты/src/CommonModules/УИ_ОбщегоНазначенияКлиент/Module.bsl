...
Процедура РедактироватьМоментВремени(КонтейнерЗначения, ОписаниеОповещенияОЗавершении, ВладелецФормы = Неопределено) Экспорт
	ПараметрыФормы = Новый Структура;
...
Процедура НачатьВыборСсылкиРедактированиеСпецифичногоЗначения(Значение, ОписаниеОповещенияОЗавершении,
	КонтейнерЗначения = Неопределено, СтандартнаяОбработка = Истина) Экспорт
	Если КонтейнерЗначения <> Неопределено Тогда
		...
		ИначеЕсли КонтейнерЗначения.Тип = ТипыКонтейнеров.МоментВремени Тогда
			РедактироватьМоментВремени(КонтейнерЗначения, ОписаниеОповещенияОЗавершении);
		КонецЕсли;
	Иначе
		...
		ИначеЕсли УИ_ОбщегоНазначенияВызовСервера.ЭтоПеречислениеПоТипу(ТипЗначения) Тогда
			СтандартнаяОбработка = Ложь;
			НачатьВыборЗначенияПеречисления(Значение, ОписаниеОповещенияОЗавершении);
		КонецЕсли;
...
Функция НовыйПараметрыОбработчикаСобытияНачалоВыбораЗначения(Форма, Элемент) Экспорт
	...
	ПараметрыОбработчика.Вставить("ИмяПоляВСтруктуре", "");
...
// 
// Возвращаемое значение:
//  Структура -  Новый параметры обработчика события очистка:
// * Форма -ФормаКлиентскогоПриложения-
// * Элемент -ПолеФормы
// * ДоступенКонтейнер - Булево - 
// * СтруктураХраненияЗначения - Неопределено, ФормаКлиентскогоПриложения, ДанныеФормыСтруктура, ДанныеФормыЭлементДерева, ДанныеФормыЭлементКоллекции - 
// * ИмяПоляВСтруктуре - Строка - 
Функция НовыйПараметрыОбработчикаСобытияОчистка(Форма, Элемент) Экспорт
	ПараметрыОбработчика = Новый Структура;
	ПараметрыОбработчика.Вставить("Форма", Форма);
	ПараметрыОбработчика.Вставить("Элемент", Элемент);
	ПараметрыОбработчика.Вставить("ДоступенКонтейнер", Ложь);
...
Процедура ПолеФормыОбработчикОчистка(ПараметрыОбработчика, СтандартнаяОбработка) Экспорт
	...
	ПараметрыОбработчика.Элемент.ОграничениеТипа = ПустоеОписаниеТипов;
	ПараметрыОбработчика.СтруктураХраненияЗначения[ПараметрыОбработчика.ИмяПоляВСтруктуре] = Неопределено;

	Если ПараметрыОбработчика.ДоступенКонтейнер Тогда
		ПараметрыОбработчика.СтруктураХраненияЗначения[ПараметрыОбработчика.ИмяПоляВСтруктуре
													   + УИ_ОбщегоНазначенияКлиентСервер.СуффиксИмениПоляХраненияКонтейнераДляПоляСКонтейнером()]=Неопределено;
		ПараметрыОбработчика.СтруктураХраненияЗначения[ПараметрыОбработчика.ИмяПоляВСтруктуре
													   + УИ_ОбщегоНазначенияКлиентСервер.СуффиксИмениПоляХраненияПредставленияТипаЗначенияДляПоляСКонтейнером()]="";
		ПараметрыОбработчика.СтруктураХраненияЗначения[ПараметрыОбработчика.ИмяПоляВСтруктуре
													   + УИ_ОбщегоНазначенияКлиентСервер.СуффиксИмениПоляХраненияТипаЗначенияДляПоляСКонтейнером()]=ПустоеОписаниеТипов;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПолеФормыОбработчикНачалоВыбораЗначения(ПараметрыОбработчика, СтандартнаяОбработка) Экспорт
	Если ПараметрыОбработчика.ДоступенКонтейнер Тогда
		...
		ЗначениеКонтейнера = ПараметрыОбработчика.СтруктураХраненияЗначения[ПараметрыОбработчика.ИмяПоляВСтруктуре
		...
		ИначеЕсли Значение = Неопределено Тогда 
			...
			ОткрытьФорму("ОбщаяФорма.УИ_РедактированиеТипаЗначения",
						 ...
						 ,
						 ,
						 Новый ОписаниеОповещения("ПолеФормыОбработчикНачалоВыбораЗначенияЗавершениеВыбораТипа",
				ЭтотОбъект, ПараметрыОбработчика),
						 РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		ИначеЕсли ПараметрыОбработчика.Элемент.ОграничениеТипа <> Новый ОписаниеТипов Тогда
			ОписаниеЗавершенияВыбораЗначения = Новый ОписаниеОповещения("ПолеФормыОбработчикНачалоВыбораЗначенияЗавершениеВыбораЗначения",
				ЭтотОбъект, ПараметрыОбработчика);
			НачатьВыборСсылкиРедактированиеСпецифичногоЗначения(Значение,
																ОписаниеЗавершенияВыбораЗначения,
	...
	Иначе
		...
		Если ПараметрыОбработчика.Значение = Неопределено Тогда
			СтандартнаяОбработка = Ложь;
			...
			ДопПараметрыОповещения.Вставить("Форма", ПараметрыОбработчика.Форма);
			ДопПараметрыОповещения.Вставить("Элемент", ПараметрыОбработчика.Элемент);
			ДопПараметрыОповещения.Вставить("ОповещениеОВыбореПустогоЗначения",
											ПараметрыОбработчика.ОповещениеОВыбореПустогоЗначения);
			ДопПараметрыОповещения.Вставить("ОповещениеОВыбореТипа", ПараметрыОбработчика.ОповещениеОВыбореТипа);
			...
			ОткрытьФорму("ОбщаяФорма.УИ_РедактированиеТипаЗначения",
						 ...
						 ,
						 ,
						 Новый ОписаниеОповещения("ПолеФормыНачалоВыбораЗначенияЗавершениеВыбораТипа", ЭтотОбъект,
				ДопПараметрыОповещения),
						 РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);

		ИначеЕсли ПараметрыОбработчика.Элемент.ОграничениеТипа <> Новый ОписаниеТипов Тогда
			НовоеЗначение = ПараметрыОбработчика.Элемент.ОграничениеТипа.ПривестиЗначение(ПараметрыОбработчика.Значение);
			Если НовоеЗначение <> ПараметрыОбработчика.Значение Тогда
				Типы = Новый Массив;
				Типы.Добавить(ТипЗнч(ПараметрыОбработчика.Значение));
				ПараметрыОбработчика.Элемент.ОграничениеТипа = Новый ОписаниеТипов(Типы);
			КонецЕсли;

			Если УИ_ОбщегоНазначенияВызовСервера.ЭтоПеречислениеПоТипу(ТипЗнч(ПараметрыОбработчика.Значение)) Тогда
				СтандартнаяОбработка = Ложь;
				НачатьВыборЗначенияПеречисления(ПараметрыОбработчика.Значение, , ПараметрыОбработчика.Элемент);
			ИначеЕсли ТипЗнч(ПараметрыОбработчика.Значение) = Тип("УникальныйИдентификатор") Тогда
				НачатьРедактированияУникальногоИдентификатора(ПараметрыОбработчика.Значение,
															  ,
															  ПараметрыОбработчика.Элемент);
			КонецЕсли;
		//		ОткрытьФорму("Справочник._ДемоБанковскиеСчета.Форма.ФормаВыбора", , Элемент);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПолеФормыОбработчикНачалоВыбораЗначенияЗавершениеВыбораТипа(Результат, ДополнительныеПараметры) Экспорт
	...
	ТипыДляКонтейнеров = УИ_ОбщегоНазначенияКлиентСервер.ТипыЗначенийХранимыеВКонрейнерах();
	ИмяПоляТипаЗначения = ДополнительныеПараметры.ИмяПоляВСтруктуре
						  + УИ_ОбщегоНазначенияКлиентСервер.СуффиксИмениПоляХраненияТипаЗначенияДляПоляСКонтейнером();
	ИмяПоляПредставленияТипаЗначения = ДополнительныеПараметры.ИмяПоляВСтруктуре
									   + УИ_ОбщегоНазначенияКлиентСервер.СуффиксИмениПоляХраненияПредставленияТипаЗначенияДляПоляСКонтейнером();
	ИмяПоляКонтейнера = ДополнительныеПараметры.ИмяПоляВСтруктуре
						+ УИ_ОбщегоНазначенияКлиентСервер.СуффиксИмениПоляХраненияКонтейнераДляПоляСКонтейнером();
						
	ХранитьВКонтейнере = ТипыДляКонтейнеров.Найти(ТипЗначения) <> Неопределено;
	...
	Если ДополнительныеПараметры.ДоступенКонтейнер Тогда
		...
		Если ХранитьВКонтейнере Тогда
			ДополнительныеПараметры.Элемент.РедактированиеТекста = Ложь;
		...
		Иначе
			ДополнительныеПараметры.Элемент.РедактированиеТекста = Истина;
	...
	Если Не ХранитьВКонтейнере Тогда
		ПустоеЗначениеТипа = Результат.Описание.ПривестиЗначение(Неопределено);
	Иначе
		ПустоеЗначениеТипа = "";
	КонецЕсли;
	...
	Иначе
		НачатьВыборСсылкиРедактированиеСпецифичногоЗначения(ПустоеЗначениеТипа, ОписаниеЗавершенияВыбораЗначения);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПолеФормыОбработчикНачалоВыбораЗначенияЗавершениеВыбораЗначения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТекущееЗначениеКонтейнера <> Неопределено Тогда
		ДополнительныеПараметры.СтруктураХраненияЗначения[ИмяПоляКонтейнера] = Результат;
		ДополнительныеПараметры.СтруктураХраненияЗначения[ДополнительныеПараметры.ИмяПоляВСтруктуре] = Результат.Представление;	
	Иначе
...
Процедура НачатьОбеспечениеКаталога(Каталог, ОписаниеОпопвещенияОЗавершении) Экспорт
	...
	Файл.НачатьПроверкуСуществования(Новый ОписаниеОповещения("НачатьОбеспечениеКаталогаЗавершениеПроверкиНаличияКаталога", ЭтотОбъект, ПараметрыОповещения));
...
Процедура НачатьОбеспечениеКаталогаЗавершениеПроверкиНаличияКаталога(Существует, ДополнительныеПараметры) Экспорт
	Если Существует Тогда
		...
		Возврат;
	КонецЕсли;
	Оповещение = Новый ОписаниеОповещения("НачатьОбеспечениеКаталогаЗавершениеСозданияКаталога", 
	ЭтотОбъект, 
	ДополнительныеПараметры, 
	"НачатьОбеспечениеКаталогаЗавершениеСозданияКаталогаСОшибкой", 
	ЭтотОбъект);
...
//  	* ОповещениеОЗавершении - ОписаниеОповещения
//  	* Каталог - Строка
Процедура НачатьОбеспечениеКаталогаЗавершениеСозданияКаталогаСОшибкой(ИнформацияОбОшибке, СтандартнаяОбработка,ДополнительныеПараметры) Экспорт
	СтандартнаяОбработка = Ложь;
...
//  Элемент  - ПолеФормы-Элемент
//  СтандартнаяОбработка - Булево - Стандартная обработка
Процедура ПолеФормыОчистка(Форма, Элемент, СтандартнаяОбработка) Экспорт
	Элемент.ОграничениеТипа = Новый ОписаниеТипов;
КонецПроцедуры

// Устарела.
// Поле формы начало выбора значения.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения- Форма
//  Элемент -ПолеФормы-Элемент
//  Значение -Произвольный - Значение
//  СтандартнаяОбработка -Булево - Стандартная обработка
//  ОповещениеОВыбореПустогоЗначения - Неопределено, ОписаниеОповещения -  Оповещение о выборе пустого значения
//  НаборТипов - Строка -  Разделенная запятыми строка, содержащая перечисление используемый набор типов. По умолчанию - Ссылки,Примитивные,УНИКАЛЬНЫЙИДЕНТИФИКАТОР 
...
//  ОповещениеОВыбореТипа - Неопределено -  Оповещение о выборе типа
Процедура ПолеФормыНачалоВыбораЗначения(Форма, Элемент, Значение, СтандартнаяОбработка,
	ОповещениеОВыбореПустогоЗначения = Неопределено, НаборТипов = Неопределено, ОповещениеОВыбореТипа = Неопределено) Экспорт
	
	ПараметрыОбработчика = НовыйПараметрыОбработчикаСобытияНачалоВыбораЗначения(Форма, Элемент);
	ПараметрыОбработчика.Значение = Значение;
	ПараметрыОбработчика.НаборТипов = НаборТипов;
	ПараметрыОбработчика.ОповещениеОВыбореПустогоЗначения = ОповещениеОВыбореПустогоЗначения;
	ПараметрыОбработчика.ОповещениеОВыбореТипа = ОповещениеОВыбореТипа;
	
	ПолеФормыОбработчикНачалоВыбораЗначения(ПараметрыОбработчика, СтандартнаяОбработка);
	
КонецПроцедуры

// Устарела. 
// Поле формы начало выбора значения завершение выбора типа.
// 
// Параметры:
//  Результат Результат
...
Процедура ПолеФормыНачалоВыбораЗначенияЗавершениеВыбораТипа(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ДополнительныеПараметры.ОповещениеОВыбореТипа) = Тип("ОписаниеОповещения") Тогда
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеОВыбореТипа, Результат.Описание);
	КонецЕсли;
	...
	ИначеЕсли ТипЗначения = Тип("УникальныйИдентификатор") Тогда 
		ПустоеЗначениеТипа = УИ_ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор();
	...
	Если ТипЗначения = Тип("УникальныйИдентификатор") Тогда 
		НачатьРедактированияУникальногоИдентификатора(,,ДополнительныеПараметры.Элемент);
		
		Возврат;
	КонецЕсли;
	
	ИмяОбъекта = УИ_ОбщегоНазначения.ИмяТаблицыПоСсылке(ПустоеЗначениеТипа);
	Если СтрНачинаетсяС(НРег(ИмяОбъекта), "перечисление.") Тогда
		НачатьВыборЗначенияПеречисления(ПустоеЗначениеТипа, , ДополнительныеПараметры.Элемент);
	
	ИначеЕсли Результат.ИспользоватьДинамическийСписокДляВыбораСсылочногоЗначения Тогда
		ПараметрыФормы = Новый Структура;
