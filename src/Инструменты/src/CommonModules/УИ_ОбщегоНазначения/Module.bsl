...
Функция ЗначениеХранилищаКонтейнераОписаниеТипов(ОписаниеТипов) Экспорт
	...
	Хранилище.КоличествоТипов = ОписаниеТипов.Типы().Количество();
...
Функция ХранитьТипВКонтейнереПоКонтейнеруОписанияТипов(Хранилище) Экспорт
	ОписаниеТипов = ЗначениеИзХранилищаКонтейнераТип(Хранилище); //ОписаниеТипов
	...
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ХранитьТипВКонтейнере(Типы[0]);
КонецФункции
#КонецОбласти
...
Процедура СообщитьПользователю(Знач ТекстСообщенияПользователю, Знач КлючДанных = Неопределено,	Знач Поле = "",
	...
	Сообщение = УИ_ОбщегоНазначенияСлужебныйКлиентСервер.СообщениеПользователю(ТекстСообщенияПользователю,
...
#Область КомандыСистемы

// Результат выполнения команды системы.
//
// Параметры:
//  ПараметрыВыполнения - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйПараметрыВыполненияКомандыСистемы
//
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйРезультатВыполненияКомандыСистемы
Функция РезультатВыполненияКомандыСистемы(ПараметрыВыполнения) Экспорт
	//Подготовить файл запуска
	КаталогВыполнения = ПолучитьИмяВременногоФайла();
	СоздатьКаталог(КаталогВыполнения);

	ТекстФайлаЗапуска = УИ_ОбщегоНазначенияКлиентСервер.ТекстФайлаЗапускаКомандыСистемы(ПараметрыВыполнения);

	РасширениеФайлаЗапуска = УИ_ОбщегоНазначенияКлиентСервер.РасширениеФайлаЗапускаКомандыСистемы(ПараметрыВыполнения.ЭтоWindows);
	ИмяФайлаЗапуска = УИ_ОбщегоНазначенияКлиентСервер.ОбъединитьПути(КаталогВыполнения, "execute."
																						+ РасширениеФайлаЗапуска);

	КодировкаФайла = КодировкаТекста.UTF8;
	Если ПараметрыВыполнения.ЭтоWindows Тогда
		КодировкаФайла = КодировкаТекста.OEM;
	КонецЕсли;

	ДвоичныеДанныеСтроки = ПолучитьДвоичныеДанныеИзСтроки(ТекстФайлаЗапуска, КодировкаФайла, Ложь);
	ДвоичныеДанныеСтроки.Записать(ИмяФайлаЗапуска);

	ИмяФайлаВыводаКоманды = УИ_ОбщегоНазначенияКлиентСервер.ОбъединитьПути(КаталогВыполнения, "output.txt");

	СтрокаЗапуска = УИ_ОбщегоНазначенияКлиентСервер.СтрокаЗапускаКомандыСистемы(ИмяФайлаЗапуска,
																				ИмяФайлаВыводаКоманды,
																				ПараметрыВыполнения.ЭтоWindows);

	РезультатВыполненияКоманды = УИ_ОбщегоНазначенияКлиентСервер.НовыйРезультатВыполненияКомандыСистемы();
	РезультатВыполненияКоманды.ТекущийКаталог = ПараметрыВыполнения.Каталог;
	Если Не ПараметрыВыполнения.ЭтоWindows Тогда
		ЗапуститьПриложение(СтрШаблон("chmod u+x ""%1""", ИмяФайлаЗапуска), ПараметрыВыполнения.Каталог, Истина);
	КонецЕсли;
	Если ПараметрыВыполнения.ЭтоWindows И ИнформационнаяБазаФайловая() Тогда
		Попытка
			WshShell = Новый COMОбъект("WScript.Shell");
		Исключение
			ВызватьИсключение "Не удалось инициализировать WScript.Shell.
							  |Подробнее:
							  |" + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		КонецПопытки;
		Если ЗначениеЗаполнено(ПараметрыВыполнения.Каталог) Тогда
			WshShell.CurrentDirectory = ПараметрыВыполнения.Каталог;
		КонецЕсли;

		WshShell.Run(СтрокаЗапуска, 0, Истина);
		WshShell = Неопределено;
	Иначе
		ЗапуститьПриложение(СтрокаЗапуска, ПараметрыВыполнения.Каталог, Истина);
	КонецЕсли;

	ТекстовыйДокументЛогаВыполнения = Новый ТекстовыйДокумент;
	ТекстовыйДокументЛогаВыполнения.Прочитать(ИмяфайлаВыводаКоманды,КодировкаФайла);
	ТекстВыводаКоманды = ТекстовыйДокументЛогаВыполнения.ПолучитьТекст();

	УИ_ОбщегоНазначенияКлиентСервер.ДополнитьРезультатВыполненияКомандыСистемыДаннымиВыводаКоманды(РезультатВыполненияКоманды,
																								   ТекстВыводаКоманды);

	//@skip-check empty-except-statement
	Попытка
		УдалитьФайлы(КаталогВыполнения);
	Исключение
	КонецПопытки;
...
// Результат выполнения команды системы упрощенно.
//
// Параметры:
//  СтрокаВыполнения -Строка -Строка выполнения
//  ТекущийКаталог - Строка - Текущий каталог
//
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйРезультатВыполненияКомандыСистемы
Функция РезультатВыполненияКомандыСистемыУпрощенно(СтрокаВыполнения, ТекущийКаталог = "") Экспорт
	ПараметрыВыполнения = УИ_ОбщегоНазначенияКлиентСервер.НовыйПараметрыВыполненияКомандыСистемы();
	ПараметрыВыполнения.Каталог = ТекущийКаталог;
	ПараметрыВыполнения.Команда = СтрокаВыполнения;

	Возврат РезультатВыполненияКомандыСистемы(ПараметрыВыполнения);
КонецФункции
...
// Возвращает полное имя объекта метаданных по переданному значению.
//
// Параметры:
//   Значение - ЛюбаяСсылка, ОбъектМетаданных - значение, для которого
//           требуется получить полное имя объекта метаданных
//
// Возвращаемое значение:
//   Строка - полное имя объекта метаданных.
//
// Пример:
//  ПолноеИмяОбъектаМетаданныхПоЗначению(Справочники.Организации) = "Справочник.Организации"
//
Функция ПолноеИмяОбъектаМетаданныхПоЗначению(Значение) Экспорт
...
Процедура НакопитьЗаменуДляSQL(Результат, Мета, ПарыЗамен)
	Если НЕ Результат.Свойство("ЗаменыТребующиеSQL") Тогда
		Возврат;
	КонецЕсли;
	Для Каждого КлючЗначение Из ПарыЗамен Цикл
		НоваяСтрока = Результат.ЗаменыТребующиеSQL.Добавить();
		НоваяСтрока.ПолноеИмяМетаданных = Мета.ПолноеИмя();
		НоваяСтрока.Дубль = КлючЗначение.Ключ;
		НоваяСтрока.Оригинал = КлючЗначение.Значение;
	КонецЦикла;
КонецПроцедуры
...
//		КонецЦикла;
	...
	ИначеЕсли ИмяДляСравнения = "<имя объекта описания метаданного>" Тогда
		Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева Цикл
			ИменаСвойств.Добавить(ТекЭлемент.Имя);
		КонецЦикла;
	ИначеЕсли ИмяДляСравнения = "<имя картинки>" Тогда
		Для Каждого ТекЭлемент Из Метаданные.ОбщиеКартинки Цикл
			ИменаСвойств.Добавить(ТекЭлемент.Имя);
		КонецЦикла;
	ИначеЕсли ИмяДляСравнения = "<имя элемента стиля>" Тогда
		ВидДляФильтра = Неопределено;
		Если Нрег(ОписаниеТипаПлатформы.Имя) = "цветастиля" Тогда
			ВидДляФильтра = Метаданные.СвойстваОбъектов.ВидЭлементаСтиля.Цвет;
		ИначеЕсли Нрег(ОписаниеТипаПлатформы.Имя) = "рамкистиля" Тогда
			ВидДляФильтра = Метаданные.СвойстваОбъектов.ВидЭлементаСтиля.Рамка;
		ИначеЕсли Нрег(ОписаниеТипаПлатформы.Имя) = "шрифтыстиля" Тогда
			ВидДляФильтра = Метаданные.СвойстваОбъектов.ВидЭлементаСтиля.Шрифт;
		КонецЕсли;

		Если ВидДляФильтра = Неопределено Тогда
			Возврат;
		КонецЕсли;
		Для Каждого ТекЭлемент Из Метаданные.ЭлементыСтиля Цикл
			Если ТекЭлемент.Вид <> ВидДляФильтра Тогда
				Продолжить;
			КонецЕсли;
			ИменаСвойств.Добавить(ТекЭлемент.Имя);
		КонецЦикла;
...
// Типы без клиент серверной передачи.
//
// Возвращаемое значение:
//  Массив из Тип- Типы без клиент серверной передачи
Функция ТипыБезКлиентСервернойПередачи() Экспорт
...
//           "ТребуетсяSQL"      - замена средствами платформы невозможна (регистр бухгалтерии), требуется режим «Замена через SQL».
//           "ТребуетсяSQL_Итог" - в прямом режиме: признак, что часть ссылок не заменена (нужен режим «Замена через SQL»);
//                                 в режиме «Замена через SQL» в ТекстОшибки лежит готовый SQL-скрипт (см. ObjectModule.СгенерироватьТолькоSQL).
...
Функция ЗаменитьСсылки(Знач ПарыЗамен, Знач Параметры = Неопределено) Экспорт
	...
	ЗаменыТребующиеSQL = Новый ТаблицаЗначений;
	ЗаменыТребующиеSQL.Колонки.Добавить("ПолноеИмяМетаданных", ТипСтрока);
	ЗаменыТребующиеSQL.Колонки.Добавить("Дубль");
	ЗаменыТребующиеSQL.Колонки.Добавить("Оригинал");
	Результат.Вставить("ЗаменыТребующиеSQL", ЗаменыТребующиеSQL);
...
//		КонецЕсли;
	...
	// Если часть ссылок нельзя заменить средствами платформы (регистры бухгалтерии),
	// SQL больше не генерируем - формируем одно итоговое сообщение, прогон уходит на шаг «Неудачные замены».
	Если Результат.ЗаменыТребующиеSQL.Количество() > 0 Тогда
		ТекстСообщения = НСтр("ru = 'Ряд ссылок заменить не удалось. Воспользуйтесь режимом «Замена через SQL».'");
		...
		// Помечаем затронутые дубли как непрошедшие, чтобы они попали в дерево «Неудачные замены»
		// (в том числе застрявшие только в движениях регистров бухгалтерии документов).
		Для Каждого СтрокаЗамены Из Результат.ЗаменыТребующиеSQL Цикл
			Если Результат.Ошибки.НайтиСтроки(Новый Структура("Ссылка, ТипОшибки", СтрокаЗамены.Дубль, "ТребуетсяSQL")).Количество() = 0 Тогда
				ЗарегистрироватьОшибкуЗамены(Результат, СтрокаЗамены.Дубль,
					ОписаниеОшибкиЗамены("ТребуетсяSQL", СтрокаЗамены.Дубль, СтрокаЗамены.ПолноеИмяМетаданных, ТекстСообщения));
			КонецЕсли;
		КонецЦикла;
		// Схлопываем "ТребуетсяSQL" до одной строки на дубль: ПроизвестиЗаменуВНаборе регистрирует
		// по строке на каждое место использования, иначе дерево «Неудачные замены» дублирует дубли.
		ОбработанныеДубли = Новый Соответствие;
		Для Каждого СтрокаОшибкиSQL Из Результат.Ошибки.НайтиСтроки(Новый Структура("ТипОшибки", "ТребуетсяSQL")) Цикл
			Если ОбработанныеДубли[СтрокаОшибкиSQL.Ссылка] = Неопределено Тогда
				ОбработанныеДубли.Вставить(СтрокаОшибкиSQL.Ссылка, Истина);
			Иначе
				Результат.Ошибки.Удалить(СтрокаОшибкиSQL);
			КонецЕсли;
		КонецЦикла;
		ОшибкаSQL = Результат.Ошибки.Добавить();
		ОшибкаSQL.ТипОшибки = "ТребуетсяSQL_Итог";
		ОшибкаSQL.ТекстОшибки = ТекстСообщения;
		ОшибкаSQL.ПредставлениеОбъектаОшибки = НСтр("ru = 'Не все ссылки заменены'");
	КонецЕсли;
...
Процедура ЗаменитьСсылкиКороткимиТранзакциями(Результат, Знач ПараметрыВыполнения, Знач Дубли, Знач ТаблицаПоиска)
	...
	СобытиеПрогресса = СобытиеЖурналаРегистрацииЗаменыСсылок() + ".Прогресс";
	Для Каждого Дубль Из Дубли Цикл
		БылиОшибки = Результат.ЕстьОшибки;
		Результат.ЕстьОшибки = Ложь;

		ФильтрСсылок.Ссылка = Дубль;
		ПредставлениеДубля = Строка(Дубль);

		ФильтрСсылок.КлючЗамены = "Константа";
		МестаИспользования = ТаблицаПоиска.НайтиСтроки(ФильтрСсылок);
		Для Каждого МестоИспользования Из МестаИспользования Цикл
			ЗаписьЖурналаРегистрации(СобытиеПрогресса, УровеньЖурналаРегистрации.Информация, , ,
				"[" + ПредставлениеДубля + "] " + Строка(МестоИспользования.Данные),
				РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
		...
		// Итог по дублю.
		Если Результат.ЕстьОшибки И Не БылиОшибки Тогда
			ЗаписьЖурналаРегистрации(СобытиеПрогресса, УровеньЖурналаРегистрации.Предупреждение, , ,
				"✗ " + ПредставлениеДубля + " (с ошибками)",
				РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
		Иначе
			ЗаписьЖурналаРегистрации(СобытиеПрогресса, УровеньЖурналаРегистрации.Информация, , ,
				"✓ " + ПредставлениеДубля,
...
Процедура ПроизвестиЗаменуВОбъекте(Результат, Знач МестоИспользования, Знач ПараметрыВыполнения,
	...
	Попытка
		...
		ОбъектыЗаписи = ИзмененныеОбъектыПриЗаменеВОбъекте(Результат, ПараметрыВыполнения, МестоИспользования,
...
Процедура ПроизвестиЗаменуВНаборе(Результат, Знач МестоИспользования, Знач ПараметрыВыполнения,
	...
	// Регистры бухгалтерии: Выгрузить()/Загрузить() включает виртуальные поля ВидСубконтоДт*,
	// а Записать() выполняет DELETE+INSERT — рискованно. Пропускаем, копим для SQL.
	Если Метаданные.РегистрыБухгалтерии.Содержит(Мета) Тогда
		ТекстПредупреждения = СтрШаблон(
			НСтр("ru = 'Замена ссылок в регистре бухгалтерии ""%1"" не может быть выполнена средствами платформы. Требуется SQL-скрипт.'"),
			Строка(Мета));
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииЗаменыСсылок(),
			УровеньЖурналаРегистрации.Предупреждение, Мета, , ТекстПредупреждения,
			РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
		Для Каждого Строка Из ОбрабатываемыеСтроки Цикл
			ЗарегистрироватьОшибкуЗамены(Результат, Строка.Ссылка,
				ОписаниеОшибкиЗамены("ТребуетсяSQL", Данные, ПредставлениеДанных, ТекстПредупреждения));
		КонецЦикла;
		НакопитьЗаменуДляSQL(Результат, Мета, ПарыЗамен);
		// Помечаем как обработанные
		Для Каждого Строка Из ОбрабатываемыеСтроки Цикл
			Строка.КлючЗамены = "";
		КонецЦикла;
	...
	Попытка
		...
		Если НЕ НаборЗаписей.Модифицированность() Тогда
			// Набор не изменён — замена не потребовалась, пропускаем запись.
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииЗаменыСсылок(),
				УровеньЖурналаРегистрации.Информация, Мета, ,
				СтрШаблон(НСтр("ru = 'Набор записей %1 не изменён, замена не требуется'"), ПредставлениеДанных),
				РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
			Если ВнутренняяТранзакция Тогда
				ЗафиксироватьТранзакцию();
			КонецЕсли;
...
Функция ИзмененныеОбъектыПриЗаменеВОбъекте(Результат, ПараметрыВыполнения, МестоИспользования, ОбрабатываемыеСтроки)
	Данные = МестоИспользования.Данные;
	...
	// Заменяем сразу все варианты — строится заранее, нужно для накопления пар замен по РБ.
	ПарыЗамен = Новый Соответствие;
	...
	Для Каждого ОписаниеДвижения Из ОписаниеДвижений Цикл
		МетаРегистра = Метаданные.НайтиПоПолномуИмени(ОписаниеДвижения.ПространствоБлокировки);
		Если МетаРегистра <> Неопределено И Метаданные.РегистрыБухгалтерии.Содержит(МетаРегистра) Тогда
			// Регистры бухгалтерии: Выгрузить()/Загрузить() включает виртуальные поля ВидСубконтоДт*,
			// а Записать() выполняет DELETE+INSERT — это перераспределяет субконто. Пропускаем, копим для SQL.
			ОписаниеДвижения.Вставить("ПропуститьИзЗаРБ", Истина);
			НакопитьЗаменуДляSQL(Результат, МетаРегистра, ПарыЗамен);
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииЗаменыСсылок(),
				УровеньЖурналаРегистрации.Информация, МетаРегистра, Данные,
				СтрШаблон(НСтр("ru = 'Замена ссылок в движениях регистра бухгалтерии ""%1"" документа ""%2"" будет выполнена SQL-скриптом.'"),
					Строка(МетаРегистра), Строка(Данные)),
				РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
			Продолжить;
		КонецЕсли;
	...
	// Движения
	Для Каждого ОписаниеДвижения Из ОписаниеДвижений Цикл
		Если ОписаниеДвижения.Свойство("ПропуститьИзЗаРБ") И ОписаниеДвижения.ПропуститьИзЗаРБ Тогда
			Продолжить;
		КонецЕсли;
		ЗаменитьВКоллекцииСтрок(
	...
	// После замены ссылок в движениях удаляем записи с дублирующимися ключевыми полями
	// в регистрах сведений (замена ссылки-измерения может привести к совпадению ключей).
	Для Каждого ОписаниеДвижения Из ОписаниеДвижений Цикл
		Если ОписаниеДвижения.Свойство("ПропуститьИзЗаРБ") И ОписаниеДвижения.ПропуститьИзЗаРБ Тогда
			Продолжить;
		КонецЕсли;
		Если ОписаниеДвижения.НаборЗаписей.Модифицированность() Тогда
			УдалитьДубликатыКлючейРегистраСведений(ОписаниеДвижения.НаборЗаписей,
...
Функция ОписаниеДвижений(Знач Мета) Экспорт
...
Функция ОписаниеПоследовательностей(Знач Мета) Экспорт
...
// Удаляет записи с дублирующимися ключевыми полями в наборе записей регистра сведений.
// После замены ссылок измерение-дубль может совпасть с измерением-оригиналом,
// что приводит к ошибке "Запись с такими ключевыми полями существует".
Процедура УдалитьДубликатыКлючейРегистраСведений(НаборЗаписей, Знач ПолноеИмяРегистра)

	МетаРегистра = Метаданные.НайтиПоПолномуИмени(ПолноеИмяРегистра);
	Если МетаРегистра = Неопределено Или Не Метаданные.РегистрыСведений.Содержит(МетаРегистра) Тогда
		Возврат;
	КонецЕсли;
	...
	// Ключевые поля регистра сведений: Период (если периодический) + все Измерения.
	...
	ВстретившиесяКлючи = Новый Соответствие;
	ИндексыУдаления = Новый Массив;

	Для Индекс = 0 По ТаблицаЗаписей.Количество() - 1 Цикл
		Строка = ТаблицаЗаписей[Индекс];
		КлючСтрока = "";
		Для Каждого ИмяПоля Из КлючевыеПоля Цикл
			КлючСтрока = КлючСтрока + ";" + XMLСтрока(Строка[ИмяПоля]);
		КонецЦикла;

		Если ВстретившиесяКлючи[КлючСтрока] <> Неопределено Тогда
			ИндексыУдаления.Добавить(Индекс);
		Иначе
			ВстретившиесяКлючи[КлючСтрока] = Истина;
		КонецЕсли;
	КонецЦикла;

	Если ИндексыУдаления.Количество() > 0 Тогда
		// Удаляем с конца, чтобы не сбить индексы.
		Для Обр = 1 По ИндексыУдаления.Количество() Цикл
			ТаблицаЗаписей.Удалить(ИндексыУдаления[ИндексыУдаления.Количество() - Обр]);
		КонецЦикла;
...
// Получает структуру хранения базы данных.
// Обёртка над платформенным методом ПолучитьСтруктуруХраненияБазыДанных()
// с добавлением индекса по Метаданные.
//
// Параметры:
//   ОбъектыМетаданных - Массив, Неопределено - массив объектов метаданных для фильтрации.
//       Неопределено - все объекты.
//   ИменаХранения - Булево - Истина: имена в терминах СУБД, Ложь: в терминах 1С.
//
// Возвращаемое значение:
//   ТаблицаЗначений - результат ПолучитьСтруктуруХраненияБазыДанных с индексом по Метаданные.
//       Колонки: ИмяТаблицы, ИмяТаблицыХранения, Метаданные, Назначение, Поля, Индексы.
//       Поля: ИмяПоляХранения, ИмяПоля, Метаданные.
//
Функция СтруктураХраненияБазыДанных(ОбъектыМетаданных = Неопределено, ИменаХранения = Истина) Экспорт
	тзСтруктура = ПолучитьСтруктуруХраненияБазыДанных(ОбъектыМетаданных, ИменаХранения);
	тзСтруктура.Индексы.Добавить("Метаданные");
	Возврат тзСтруктура;
КонецФункции

// Генерирует SQL-скрипт для замены ссылок, которые не удалось заменить средствами платформы.
//
// Параметры:
//   ЗаменыТребующиеSQL - ТаблицаЗначений - замены, требующие SQL. Колонки:
//     * ПолноеИмяМетаданных - Строка - полное имя метаданных (напр. "РегистрБухгалтерии.Хозрасчетный").
//     * Дубль - ЛюбаяСсылка - ссылка-дубль, которую нужно заменить.
//     * Оригинал - ЛюбаяСсылка - ссылка-оригинал, на которую нужно заменить.
//   ТипСУБД - Строка - диалект СУБД: "PostgreSQL", "MSSQL" или "Авто" (автоопределение по умолчанию).
//
// Возвращаемое значение:
//   Строка - текст SQL-скрипта.
//
Функция СгенерироватьSQLСкриптЗамены(ЗаменыТребующиеSQL, ТипСУБД = "Авто") Экспорт

	тзСтруктура = СтруктураХраненияБазыДанных(, Истина);

	// Определяем какие секции выводить.
	// При явно переданном ТипСУБД ("PostgreSQL"/"MSSQL") выводится только нужная секция.
	// При "Авто" (или пустом значении) - авто-определение по строке соединения, как раньше:
	// серверные ИБ часто НЕ содержат DBMS= - выводим тогда обе ветки, но MS SQL ставим
	// закомментированной (префикс "-- "), чтобы запуск всего файла в PostgreSQL-клиенте
	// (DataGrip, psql) не падал на синтаксисе BEGIN TRY.
	ВыводитьPostgreSQL = Истина;
	ВыводитьMSSQL      = Истина;
	Если ТипСУБД = "PostgreSQL" Тогда
		ВыводитьMSSQL = Ложь;
	ИначеЕсли ТипСУБД = "MSSQL" Или ТипСУБД = "MSSQLSERVER" Тогда
		ВыводитьPostgreSQL = Ложь;
	Иначе
		СтрокаСоединения = ВРег(СтрокаСоединенияИнформационнойБазы());
		Если СтрНайти(СтрокаСоединения, "DBMS=POSTGRESQL") > 0 Тогда
			ВыводитьMSSQL = Ложь;
		ИначеЕсли СтрНайти(СтрокаСоединения, "DBMS=MSSQLSERVER") > 0 Тогда
			ВыводитьPostgreSQL = Ложь;
		КонецЕсли;
	КонецЕсли;
	КомментироватьMSSQL = ВыводитьPostgreSQL И ВыводитьMSSQL;
	ПрефиксMSSQL = ?(КомментироватьMSSQL, "-- ", "");

	Строки = Новый Массив;
	Строки.Добавить("-- SQL-скрипт замены ссылок (сгенерирован инструментом ""Поиск и удаление дублей"")");
	Строки.Добавить("-- Дата: " + Формат(ТекущаяДата(), "ДЛФ=ДВ"));
	Строки.Добавить("--");
	Строки.Добавить("-- ПОСЛЕ ВЫПОЛНЕНИЯ СКРИПТА: запустите ""Пересчёт итогов"" для затронутых");
	Строки.Добавить("-- регистров бухгалтерии/накопления в 1С (Все функции → Стандартные →");
	Строки.Добавить("-- Управление итогами), либо программно: РегистрыБухгалтерии.<Имя>.ПересчитатьИтоги().");
	Строки.Добавить("-- Без этого ОСВ и другие отчёты на агрегатах могут показывать устаревшие данные.");
	Если КомментироватьMSSQL Тогда
		Строки.Добавить("--");
		Строки.Добавить("-- ПРИМЕЧАНИЕ: для MS SQL Server раскомментируйте блок ниже (уберите префикс ""-- "")");
		Строки.Добавить("--             и закомментируйте PostgreSQL-секцию.");
	КонецЕсли;
	Строки.Добавить("");

	// Группировка строк по паре (Дубль, Оригинал) - каждая пара пойдёт в собственной транзакции
	// BEGIN/COMMIT (PG) или BEGIN TRANSACTION/COMMIT (MSSQL). При потере коннекта в середине
	// большого скрипта PostgreSQL/MSSQL откатит только текущую пару - уже закоммиченные пары
	// останутся применёнными. Сохраняем порядок появления пар, дедуп метаданных делаем внутри
	// каждой группы.
	ГруппыПоПаре = Новый Соответствие;
	ПорядокПар   = Новый Массив;
	Для Каждого СтрокаЗамены Из ЗаменыТребующиеSQL Цикл
		КлючПары = XMLСтрока(СтрокаЗамены.Дубль) + "->" + XMLСтрока(СтрокаЗамены.Оригинал);
		Группа = ГруппыПоПаре[КлючПары];
		Если Группа = Неопределено Тогда
			Группа = Новый Массив;
			ГруппыПоПаре.Вставить(КлючПары, Группа);
			ПорядокПар.Добавить(КлючПары);
		КонецЕсли;
		Группа.Добавить(СтрокаЗамены);
	КонецЦикла;

	Если ВыводитьPostgreSQL Тогда
		Строки.Добавить("-- === PostgreSQL ===");
		Строки.Добавить("");

		Для Каждого КлючПары Из ПорядокПар Цикл
			Группа = ГруппыПоПаре[КлючПары];
			ПерваяСтрока = Группа[0];
			HexДубль    = СсылкаВHex(ПерваяСтрока.Дубль);
			HexОригинал = СсылкаВHex(ПерваяСтрока.Оригинал);

			Строки.Добавить("-- Замена: "
				+ ПредставлениеВОднуСтроку(ПерваяСтрока.Дубль) + " (" + XMLСтрока(ПерваяСтрока.Дубль) + ")"
				+ " → "
				+ ПредставлениеВОднуСтроку(ПерваяСтрока.Оригинал) + " (" + XMLСтрока(ПерваяСтрока.Оригинал) + ")");
			Строки.Добавить("BEGIN;");

			ОбработанныеМетаданные = Новый Соответствие;
			Для Каждого СтрокаЗамены Из Группа Цикл
				Если ОбработанныеМетаданные[СтрокаЗамены.ПолноеИмяМетаданных] <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				ОбработанныеМетаданные.Вставить(СтрокаЗамены.ПолноеИмяМетаданных, Истина);

				Строки.Добавить("-- Объект метаданных: " + СтрокаЗамены.ПолноеИмяМетаданных);

				// Найти все физические таблицы для данного объекта метаданных.
				СтрокиСтруктуры = ВсеТаблицыМетаданныхВключаяТабЧасти(тзСтруктура, СтрокаЗамены.ПолноеИмяМетаданных);

				Если СтрокиСтруктуры.Количество() = 0 Тогда
					Строки.Добавить("-- ВНИМАНИЕ: таблицы для " + СтрокаЗамены.ПолноеИмяМетаданных + " не найдены в структуре хранения!");
					Продолжить;
				КонецЕсли;

				Для Каждого СтрокаТаблицы Из СтрокиСтруктуры Цикл

					ИмяТаблицыSQL = СтрокаТаблицы.ИмяТаблицыХранения;
					Назначение = ВРег(СтрокаТаблицы.Назначение);

					// Пропускаем итоговые таблицы (_AccRgAT*, _AccRgCT*, _AccumRgT*) и таблицы
					// настроек хранения итогов. UPDATE по ним вызывает конфликты UNIQUE constraint
					// (когда у Дубля и Оригинала совпадают остальные ключи) и не имеет смысла -
					// агрегаты всё равно нужно пересчитывать через "Пересчёт итогов" в 1С.
					Если СтрНайти(Назначение, "ИТОГ") > 0 Или СтрНайти(Назначение, "НАСТРОЙК") > 0 Тогда
						Строки.Добавить("-- Таблица: " + ИмяТаблицыSQL + " (" + СтрокаТаблицы.ИмяТаблицы + ", " + СтрокаТаблицы.Назначение + ") -- ПРОПУЩЕНА (требуется Пересчёт итогов в 1С)");
						Продолжить;
					КонецЕсли;

					Строки.Добавить("-- Таблица: " + ИмяТаблицыSQL + " (" + СтрокаТаблицы.ИмяТаблицы + ", " + СтрокаТаблицы.Назначение + ")");

					Для Каждого СтрокаПоля Из СтрокаТаблицы.Поля Цикл

						ИмяПоляSQL = СтрокаПоля.ИмяПоляХранения;

						// Пропускаем служебные поля TYPE и RTRef - обновляем только RRRef (само значение ссылки).
						// Также пропускаем _IDRRef - это первичный ключ таблицы; UPDATE по нему всегда конфликтует
						// с уже существующей строкой оригинала и не имеет смысла (сама строка дубля удаляется
						// отдельно через пометку на удаление в режиме реальной замены).
						Если СтрЗаканчиваетсяНа(ИмяПоляSQL, "_TYPE") Или СтрЗаканчиваетсяНа(ИмяПоляSQL, "_RTRef")
							Или СтрЗаканчиваетсяНа(ИмяПоляSQL, "_IDRRef") Тогда
							Продолжить;
						КонецЕсли;

						// Для полей _RRRef или обычных полей ссылочного типа.
						// Сначала пытаемся выполнить bulk UPDATE (быстрый путь) и через GET DIAGNOSTICS получить
						// количество затронутых строк - если > 0, выводим NOTICE для диагностики (DataGrip и pgAdmin
						// не показывают row count для DO-блоков, без NOTICE невозможно понять что-то ли обновилось).
						// Если bulk UPDATE падает на unique_violation (запись с такой же комбинацией ключевых полей
						// уже есть для оригинала - типично для регистров сведений с измерением-ссылкой), переходим
						// в построчный режим: для каждой строки дубля делаем UPDATE; если конфликт - DELETE строки
						// дубля (она избыточна).
						// Внешний EXCEPTION WHEN OTHERS ловит неожиданные ошибки (например поля с RRef-суффиксом,
						// у которых не-bytea тип - _KindDt*/_KindCt* в регистре бухгалтерии).
						Если СтрЗаканчиваетсяНа(ИмяПоляSQL, "_RRRef") Или СтрЗаканчиваетсяНа(ИмяПоляSQL, "RRef") Тогда
							Строки.Добавить("DO $$");
							Строки.Добавить("DECLARE _r record;");
							Строки.Добавить("DECLARE _n int;");
							Строки.Добавить("BEGIN");
							Строки.Добавить("    BEGIN");
							Строки.Добавить("        UPDATE " + ИмяТаблицыSQL
								+ " SET " + ИмяПоляSQL + " = decode('" + HexОригинал + "', 'hex')"
								+ " WHERE " + ИмяПоляSQL + " = decode('" + HexДубль + "', 'hex');");
							Строки.Добавить("        GET DIAGNOSTICS _n = ROW_COUNT;");
							Строки.Добавить("        IF _n > 0 THEN RAISE NOTICE '✓ "
								+ ИмяТаблицыSQL + "." + ИмяПоляSQL + ": % строк', _n; END IF;");
							Строки.Добавить("    EXCEPTION WHEN unique_violation THEN");
							Строки.Добавить("        _n := 0;");
							Строки.Добавить("        FOR _r IN SELECT ctid FROM " + ИмяТаблицыSQL
								+ " WHERE " + ИмяПоляSQL + " = decode('" + HexДубль + "', 'hex') LOOP");
							Строки.Добавить("            BEGIN");
							Строки.Добавить("                UPDATE " + ИмяТаблицыSQL
								+ " SET " + ИмяПоляSQL + " = decode('" + HexОригинал + "', 'hex')"
								+ " WHERE ctid = _r.ctid;");
							Строки.Добавить("                _n := _n + 1;");
							Строки.Добавить("            EXCEPTION WHEN unique_violation THEN");
							Строки.Добавить("                DELETE FROM " + ИмяТаблицыSQL + " WHERE ctid = _r.ctid;");
							Строки.Добавить("                _n := _n + 1;");
							Строки.Добавить("            END;");
							Строки.Добавить("        END LOOP;");
							Строки.Добавить("        IF _n > 0 THEN RAISE NOTICE '✓ "
								+ ИмяТаблицыSQL + "." + ИмяПоляSQL + ": % строк (row-by-row)', _n; END IF;");
							Строки.Добавить("    END;");
							Строки.Добавить("EXCEPTION WHEN OTHERS THEN RAISE NOTICE 'Пропущено "
								+ ИмяТаблицыSQL + "." + ИмяПоляSQL + ": %', SQLERRM;");
							Строки.Добавить("END $$;");
						КонецЕсли;
			...
			Строки.Добавить("COMMIT;");
			Строки.Добавить("");
		КонецЦикла;
	КонецЕсли;

	Если ВыводитьMSSQL Тогда

		Строки.Добавить("");
		Строки.Добавить(ПрефиксMSSQL + "-- === MS SQL Server ===");
		// @rc объявляется один раз на батч - DECLARE действует на весь скрипт, а не транзакцию.
		Строки.Добавить(ПрефиксMSSQL + "DECLARE @rc int;");
		Строки.Добавить("");
		...
		Для Каждого КлючПары Из ПорядокПар Цикл
			...
			Строки.Добавить(ПрефиксMSSQL + "-- Замена: "
				+ ПредставлениеВОднуСтроку(ПерваяСтрока.Дубль) + " (" + XMLСтрока(ПерваяСтрока.Дубль) + ")"
				+ " → "
				+ ПредставлениеВОднуСтроку(ПерваяСтрока.Оригинал) + " (" + XMLСтрока(ПерваяСтрока.Оригинал) + ")");
			Строки.Добавить(ПрефиксMSSQL + "BEGIN TRANSACTION;");
			...
			Для Каждого СтрокаЗамены Из Группа Цикл
				...
				Для Каждого СтрокаТаблицы Из СтрокиСтруктуры Цикл
					...
					// Пропускаем итоговые таблицы и таблицы настроек хранения итогов -
					// см. PG секцию выше для обоснования (UNIQUE constraint conflicts + Пересчёт итогов).
					Если СтрНайти(Назначение, "ИТОГ") > 0 Или СтрНайти(Назначение, "НАСТРОЙК") > 0 Тогда
						Строки.Добавить(ПрефиксMSSQL + "-- Таблица: " + ИмяТаблицыSQL + " -- ПРОПУЩЕНА (требуется Пересчёт итогов в 1С)");
						Продолжить;
					КонецЕсли;
					...
					Для Каждого СтрокаПоля Из СтрокаТаблицы.Поля Цикл
						...
						// См. PG-секцию выше - те же причины пропуска _TYPE/_RTRef/_IDRRef.
						Если СтрЗаканчиваетсяНа(ИмяПоляSQL, "_TYPE") Или СтрЗаканчиваетсяНа(ИмяПоляSQL, "_RTRef")
							Или СтрЗаканчиваетсяНа(ИмяПоляSQL, "_IDRRef") Тогда
							Продолжить;
						КонецЕсли;

						// Сначала bulk UPDATE; при unique violation (коды 2627/2601) переходим в построчный
						// режим: WHILE EXISTS + UPDATE TOP(1)/DELETE TOP(1). Каждая итерация обрабатывает одну
						// строку дубля: либо UPDATE удаётся (строка перестаёт совпадать с WHERE), либо CATCH
						// делает DELETE TOP(1). Прочие ошибки печатаются и не разрывают транзакцию.
						Если СтрЗаканчиваетсяНа(ИмяПоляSQL, "_RRRef") Или СтрЗаканчиваетсяНа(ИмяПоляSQL, "RRef") Тогда
							Строки.Добавить(ПрефиксMSSQL + "BEGIN TRY");
							Строки.Добавить(ПрефиксMSSQL + "    UPDATE " + ИмяТаблицыSQL
								+ " SET " + ИмяПоляSQL + " = 0x" + HexОригинал
								+ " WHERE " + ИмяПоляSQL + " = 0x" + HexДубль + ";");
							Строки.Добавить(ПрефиксMSSQL + "    SET @rc = @@ROWCOUNT;");
							Строки.Добавить(ПрефиксMSSQL + "    IF @rc > 0 PRINT '✓ "
								+ ИмяТаблицыSQL + "." + ИмяПоляSQL + ": ' + CAST(@rc AS varchar);");
							Строки.Добавить(ПрефиксMSSQL + "END TRY BEGIN CATCH");
							Строки.Добавить(ПрефиксMSSQL + "    IF ERROR_NUMBER() IN (2627, 2601)");
							Строки.Добавить(ПрефиксMSSQL + "    BEGIN");
							Строки.Добавить(ПрефиксMSSQL + "        WHILE EXISTS (SELECT 1 FROM " + ИмяТаблицыSQL
								+ " WHERE " + ИмяПоляSQL + " = 0x" + HexДубль + ")");
							Строки.Добавить(ПрефиксMSSQL + "        BEGIN");
							Строки.Добавить(ПрефиксMSSQL + "            BEGIN TRY");
							Строки.Добавить(ПрефиксMSSQL + "                UPDATE TOP (1) " + ИмяТаблицыSQL
								+ " SET " + ИмяПоляSQL + " = 0x" + HexОригинал
								+ " WHERE " + ИмяПоляSQL + " = 0x" + HexДубль + ";");
							Строки.Добавить(ПрефиксMSSQL + "            END TRY BEGIN CATCH");
							Строки.Добавить(ПрефиксMSSQL + "                DELETE TOP (1) FROM " + ИмяТаблицыSQL
								+ " WHERE " + ИмяПоляSQL + " = 0x" + HexДубль + ";");
							Строки.Добавить(ПрефиксMSSQL + "            END CATCH");
							Строки.Добавить(ПрефиксMSSQL + "        END");
							Строки.Добавить(ПрефиксMSSQL + "    END");
							Строки.Добавить(ПрефиксMSSQL + "    ELSE PRINT 'Пропущено "
								+ ИмяТаблицыSQL + "." + ИмяПоляSQL + ": ' + ERROR_MESSAGE();");
							Строки.Добавить(ПрефиксMSSQL + "END CATCH;");
						КонецЕсли;
			...
			Строки.Добавить(ПрефиксMSSQL + "COMMIT;");
			Строки.Добавить("");
		КонецЦикла;
...
// Возвращает однострочное представление значения для безопасной вставки в SQL-комментарий "-- ...".
// Переносы строк, возвраты каретки и табуляции заменяются пробелами - иначе многострочное
// представление объекта (например наименование с переносом) уехало бы на новую строку без
// префикса "--" и сломало бы выполнение всего скрипта при копировании.
//
// Параметры:
//   Значение - Произвольный - значение, представление которого нужно получить.
//
// Возвращаемое значение:
//   Строка - однострочное строковое представление (без переносов строк и табуляций).
Функция ПредставлениеВОднуСтроку(Знач Значение) Экспорт
...
Функция СсылкаВHex(Ссылка)
	УИД = СтрЗаменить(XMLСтрока(Ссылка), "-", "");
	Если СтрДлина(УИД) <> 32 Тогда
		Возврат УИД;
	КонецЕсли;
	// 1С хранит UUID в bytea PG / varbinary(16) MS SQL в полностью реверсированном порядке сегментов:
	//   stored = clock_seq(2 байта) + node(6 байт) + timestamp_hi+ver(2) + timestamp_mid(2) + timestamp_low(4)
	// Что в hex-строке (32 символа без дефисов) даёт перестановку:
	//   stored_hex = chars[17..32] + chars[13..16] + chars[9..12] + chars[1..8]
	// Проверено сравнением reference XMLString и encode(_RecorderRRef, 'hex') в реальной базе.
	Возврат Сред(УИД, 17, 16) + Сред(УИД, 13, 4) + Сред(УИД, 9, 4) + Лев(УИД, 8);
КонецФункции
...
Функция ДатаСозданияСсылки(Ссылка) Экспорт
	...
	Возврат ДатаСозданияГУИД(ГУИД);

КонецФункции

// Возвращает дату создания GUID.
//
// Параметры:
//  ГУИД - УникальныйИдентификатор, Строка - уникальный идентификатор, для которого требуется получить результат функции
//
// Возвращаемое значение:
//  Дата - дата создания уникального идентификатора.
//
Функция ДатаСозданияГУИД(Знач ГУИД) Экспорт

	ГУИД = Строка(ГУИД);
	Если Не УИ_СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(ГУИД) Тогда
		ВызватьИсключение НСтр("ru = 'Переданный параметр не является уникальным идентификатором'");
	КонецЕсли;
...
// Возвращает ссылку на объект по навигационной ссылке.
//
// Параметры:
//  НавигационнаяСсылка - Строка - Навигационная ссылка
//
// Возвращаемое значение:
//  ЛюбаяСсылка, Неопределено - найденный объект
Функция СсылкаНаОбъектПоНавигационнойСсылке(Знач НавигационнаяСсылка) Экспорт

	Результат = Неопределено;
	ОбъектИБ = "e1cib/data/";
	ПараметрСсылка = "?ref=";
	НавигационнаяСсылка = НРег(НавигационнаяСсылка);
	ПозицияОбъектаИБ = СтрНайти(НавигационнаяСсылка, ОбъектИБ);
    ПозицияПараметраСсылки = СтрНайти(НавигационнаяСсылка, ПараметрСсылка);

	Если ПозицияОбъектаИБ = 0 или ПозицияПараметраСсылки = 0 Тогда
		Возврат Результат;
	КонецЕсли;

	ДлинаОбъектаИБ = СтрДлина(ОбъектИБ);
	ДлинаПараметраСсылки = СтрДлина(ПараметрСсылка);
	ПустойИдентификатор = СтрЗаменить(УИ_ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор(), "-",  "");
	Попытка
	    ИмяТипа = Сред(НавигационнаяСсылка, ПозицияОбъектаИБ + ДлинаОбъектаИБ,
	    	ПозицияПараметраСсылки - ПозицияОбъектаИБ - ДлинаОбъектаИБ);
	    ШаблонЗначения = ЗначениеВСтрокуВнутр(ПредопределенноеЗначение(ИмяТипа + ".ПустаяСсылка"));
	    ЗначениеСсылки = СтрЗаменить(ШаблонЗначения, ПустойИдентификатор,
	    	Сред(НавигационнаяСсылка, ПозицияПараметраСсылки + ДлинаПараметраСсылки));
...
// Данные по общим реквизитам.
//
// Параметры:
//  АдресОбщихРеквизитов - Строка - адрес, где хранятся данные общих реквизитов, если передан пустой адрес, то после выполнения функции - адрес заполнится
//
// Возвращаемое значение:
//  ТаблицаЗначений - Данные по общим реквизитам:
// * Имя - Строка - Имя общего реквизита
// * Представление - Строка - Представление общего реквизита
// * Комментарий - Строка - Комментарий к общему реквизиту
// * Индексирование - Строка - Свойство "Индексировать" общего реквизита
// * Тип - ОписаниеТипов - описание типа общего реквизита
// * ВыборГруппИЭлементов - ИспользованиеГруппИЭлементов - использование групп и элементов для выбора значения
// * Объект - Строка - объек метаданных, который использует общий реквизит
Функция ДанныеПоОбщимРеквизитам(АдресОбщихРеквизитов) Экспорт

	Перем Результат;
	Если Не ПустаяСтрока(АдресОбщихРеквизитов) Тогда
		Результат = ПолучитьИзВременногоХранилища(АдресОбщихРеквизитов);
	КонецЕсли;
	...
	Иначе
		...
		Результат.Колонки.Добавить("Имя", ОписаниеСтроки);
		Результат.Колонки.Добавить("Представление", ОписаниеСтроки);
		Результат.Колонки.Добавить("Комментарий", ОписаниеСтроки);
		Результат.Колонки.Добавить("Индексирование", ОписаниеСтроки);
		Результат.Колонки.Добавить("Тип", Новый ОписаниеТипов("ОписаниеТипов"));
		Результат.Колонки.Добавить("ВыборГруппИЭлементов", Новый ОписаниеТипов("ИспользованиеГруппИЭлементов"));
		Результат.Колонки.Добавить("Объект", ОписаниеСтроки);
		...
		Для Каждого ОбщийРеквизит Из Метаданные.ОбщиеРеквизиты Цикл
			...
			Если ОбщийРеквизит.РазделениеДанных = СвойстваОбъектов.РазделениеДанныхОбщегоРеквизита.Разделять Тогда
				Продолжить;
			КонецЕсли;
			АвтоИспользование = (ОбщийРеквизит.АвтоИспользование = СвойстваОбъектов.АвтоИспользованиеОбщегоРеквизита.Использовать);
			Для каждого СоставРеквизита из ОбщийРеквизит.Состав Цикл
				Если АвтоИспользование Тогда
					РеквизитИспользуется = (СоставРеквизита.Использование <> ИспользованиеОбщегоРеквизита.НеИспользовать);
				Иначе
					РеквизитИспользуется = (СоставРеквизита.Использование = ИспользованиеОбщегоРеквизита.Использовать);
				КонецЕсли;

				Если РеквизитИспользуется Тогда
					НоваяСтрока = Результат.Добавить();
					НоваяСтрока.Имя = ОбщийРеквизит.Имя;
					НоваяСтрока.Представление = ОбщийРеквизит.Представление();
					НоваяСтрока.Комментарий = ОбщийРеквизит.Комментарий;
					НоваяСтрока.Индексирование = СвойствоИндексирование(ОбщийРеквизит);
					НоваяСтрока.Тип = ОбщийРеквизит.Тип;
					НоваяСтрока.ВыборГруппИЭлементов = ОбщийРеквизит.ВыборГруппИЭлементов;
					НоваяСтрока.Объект = СоставРеквизита.Метаданные.ПолноеИмя();
				КонецЕсли;
			КонецЦикла;

		КонецЦикла;
		Результат.Индексы.Добавить("Объект");
		АдресОбщихРеквизитов = ПоместитьВоВременноеХранилище(Результат, Новый УникальныйИдентификатор());
		Возврат Результат;
	КонецЕсли;
...
//
// Возвращаемое значение:
//  Булево - Это табличная часть
Функция ЭтоТабличнаяЧасть(ТипЗначения) Экспорт
	ОбъектМД = Метаданные.НайтиПоТипу(ТипЗначения);
	Если ОбъектМД = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;

	ВариантыПоискаТЧ = Новый Массив;
	ВариантыПоискаТЧ.Добавить(ВРег("ТабличнаяЧасть"));
	ВариантыПоискаТЧ.Добавить(ВРег("TabularSection"));

	ПолноеИмя = ВРег(ОбъектМД.ПолноеИмя());
	//Например, Документ.ПоступлениеТоваров.ТабличнаяЧасть.Товары
	ЧастиИмени = СтрРазделить(ПолноеИмя, ".");
	Если ЧастиИмени.Количество() < 4 Тогда
		Возврат Ложь;
	КонецЕсли;

	ПризнакТЧ = ЧастиИмени[ЧастиИмени.Количество() - 2];
	Возврат ВариантыПоискаТЧ.Найти(ПризнакТЧ) <> Неопределено;
...
// Изменить тип колонки таблицы значений.
//
// Параметры:
//  ТаблицаДанных - ТаблицаЗначений
//  ИмяКолонки - Строка
//  НовыйТипКолонки - ОписаниеТипов
Процедура ИзменитьТипКолонкиТаблицыЗначений(ТаблицаДанных, ИмяКолонки, НовыйТипКолонки) Экспорт
	ИмяВременнойКолонки = ИмяКолонки + "_Вр31415926";
	МассивДанныхКолонки = ТаблицаДанных.ВыгрузитьКолонку(ИмяКолонки);

	ТаблицаДанных.Колонки.Добавить(ИмяВременнойКолонки, НовыйТипКолонки);
	ТаблицаДанных.ЗагрузитьКолонку(МассивДанныхКолонки, ИмяВременнойКолонки);
	ТаблицаДанных.Колонки.Удалить(ИмяКолонки);
	ТаблицаДанных.Колонки[ИмяВременнойКолонки].Имя = ИмяКолонки;
КонецПроцедуры

// Установить тип колонок без типа в таблице значений.
//
// Параметры:
//  ТаблицаДанных - ТаблицаЗначений
Процедура УстановитьТипКолонокБезУказанияТипаВТаблицеЗначений(ТаблицаДанных) Экспорт

//	Обработка = РеквизитФормыВЗначение("Объект");
	ПустойТип = Новый ОписаниеТипов;
	НеЗначащиеТипы = Новый Массив;
	НеЗначащиеТипы.Добавить("Неопределено");
	НеЗначащиеТипы.Добавить("Null");

	ОбрабатываемыеКолонки = Новый Массив;
	ТипыКолонок = Новый Массив;
	Для Каждого Колонка Из ТаблицаДанных.Колонки Цикл
		//маТипы = Колонка.ТипЗначения.Типы();
		Если Колонка.ТипЗначения = ПустойТип Тогда
			ОбрабатываемыеКолонки.Добавить(Колонка.Имя);
			ТипыКолонок.Добавить(Новый Массив);
		КонецЕсли;
	КонецЦикла;

	Если ОбрабатываемыеКолонки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	Для Каждого Строка Из ТаблицаДанных Цикл
		Для й = 0 По ОбрабатываемыеКолонки.Количество() - 1 Цикл
			ИмяКолонки = ОбрабатываемыеКолонки[й];
			ТипыКолонок[й].Добавить(ТипЗнч(Строка[ИмяКолонки]));
		КонецЦикла;
	КонецЦикла;
	...
	Для й = 0 По ОбрабатываемыеКолонки.Количество() - 1 Цикл
		...
		СтарыйТипЗначения = ТаблицаДанных.Колонки[ИмяКолонки].ТипЗначения;
		НовыйТипКолонки = Новый ОписаниеТипов(ТипыКолонок[й], СтарыйТипЗначения.КвалификаторыЧисла,
			СтарыйТипЗначения.КвалификаторыСтроки, СтарыйТипЗначения.КвалификаторыДаты,
			СтарыйТипЗначения.КвалификаторыДвоичныхДанных);

		ЗначимыеТипы = Новый ОписаниеТипов(НовыйТипКолонки, , НеЗначащиеТипы);
		...
		ИзменитьТипКолонкиТаблицыЗначений(ТаблицаДанных, ИмяКолонки, НовыйТипКолонки);
...
//  ИмяТаблицы - Строка
//  ТаблицаДанных - ТаблицаЗначений
Процедура ДобавитьТаблицуЗначенийВМенеджерВременныхТаблиц(МенеджерВременныхТаблиц, ИмяТаблицы, ТаблицаДанных) Экспорт

	УстановитьТипКолонокБезУказанияТипаВТаблицеЗначений(ТаблицаДанных);
	...
	НовыеВыраженияПолей = Новый Структура;
//	ДополнительныеИсточники = "";
//	ДополнительныеЗапросы = "";
	ИмяПромежуточнойТаблицы = ИмяТаблицы + "_Вр31415926";
	ПодготовитьТаблицуЗначенийДляВыборкиВоВременнуюТаблицуКолонокСТипомТип(ТаблицаДанных, НовыеВыраженияПолей);
//	ПодготовитьТаблицуЗначенийДляВыборкиВоВременнуюТаблицуКолонокСТипомМоментВремени(ТаблицаДанных, ИмяПромежуточнойТаблицы, НовыеВыраженияПолей,
//		ДополнительныеИсточники, ДополнительныеЗапросы);
	Если НовыеВыраженияПолей.Количество() > 0 Тогда

		ВыраженияПолей = Новый Массив;
		Для Каждого Колонка Из ТаблицаДанных.Колонки Цикл
			Выражение = "Таблица." + Колонка.Имя + " КАК " + Колонка.Имя;
			ВыраженияПолей.Добавить(Выражение);
		КонецЦикла;
		ВыраженияПолей = СтрСоединить(ВыраженияПолей, ",
													  |");
	...
	Для Каждого ИмяКолонки Из ПоляТаблицы Цикл
		Если Не НовыеВыраженияПолей.Свойство(ИмяКолонки, Выражение) Тогда
			Выражение = "Таблица." + ИмяКолонки + " КАК " + ИмяКолонки;
		КонецЕсли;
		МассивВыраженийПолей.Добавить(Выражение);
	КонецЦикла;
	ВыраженияПолей = СтрСоединить(МассивВыраженийПолей, ",
													|");
	...
	Если НовыеВыраженияПолей.Количество() > 0 Тогда
		ТекстыЗапросов.Добавить("УНИЧТОЖИТЬ "+ИмяПромежуточнойТаблицы);
	КонецЕсли;

	Запрос.УстановитьПараметр(ИмяТаблицы, ТаблицаДанных);
...
// Функция - Массив структур в таблицу значений
...
// Схема компоновки данных в строку.
//
// Параметры:
...
// Возвращаемое значение:
//  Строка
Функция СхемаКомпоновкиДанныхВСтроку(СхемаКомпоновкиДанных) Экспорт
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.УстановитьСтроку();
...
// Схема компоновки данных из строки.
//
// Параметры:
//  СериализованнаяСтрокаСКД - Строка - Данные СКД
//
// Возвращаемое значение:
//  Неопределено, СхемаКомпоновкиДанных -
Функция СхемаКомпоновкиДанныхИзСтроки(СериализованнаяСтрокаСКД) Экспорт
	Если Не ЗначениеЗаполнено(СериализованнаяСтрокаСКД) Тогда
		Возврат Неопределено;
	КонецЕсли;

	ЧтениеXML = Новый ЧтениеXML;
	ЧтениеXML.УстановитьСтроку(СериализованнаяСтрокаСКД);
	Попытка
...
// Настройки схемы компоновки данных в строку.
//
// Параметры:
...
// Возвращаемое значение:
//  Строка
Функция НастройкиКомпоновкиДанныхВСтроку(НастройкиКомпоновкиДанных) Экспорт
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.УстановитьСтроку();
	СериализаторXDTO.ЗаписатьXML(ЗаписьXML,
...
// Настройки компоновки данных из строки.
//
// Параметры:
//  СериализованнаяСтрокаНастроек -Строка-Сериализованная строка настроек
//
// Возвращаемое значение:
//  Неопределено, НастройкиКомпоновкиДанных - Настройки компоновки данных из строки
Функция НастройкиКомпоновкиДанныхИзСтроки(СериализованнаяСтрокаНастроек) Экспорт
	Если Не ЗначениеЗаполнено(СериализованнаяСтрокаНастроек) Тогда
		Возврат Неопределено;
	КонецЕсли;

	ЧтениеXML = Новый ЧтениеXML;
	ЧтениеXML.УстановитьСтроку(СериализованнаяСтрокаНастроек);
	Попытка
...
// Макет компоновки данных в строку.
//
// Параметры:
...
// Возвращаемое значение:
//  Строка
Функция МакетКомпоновкиДанныхВСтроку(МакетКомпоновкиДанных) Экспорт
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.УстановитьСтроку();
...
// Макет компоновки данных из строки.
//
// Параметры:
//  СериализованнаяСтрока-Строка- Сериализованная строка
//
// Возвращаемое значение:
//  МакетКомпоновкиДанных, Неопределено -
Функция МакетКомпоновкиДанныхИзСтроки(СериализованнаяСтрока) Экспорт
	ЧтениеXML = Новый ЧтениеXML;
	ЧтениеXML.УстановитьСтроку(СериализованнаяСтрока);
	Попытка
		Возврат СериализаторXDTO.ПрочитатьXML(ЧтениеXML, Тип("МакетКомпоновкиДанных"));
	Исключение
		Возврат Неопределено;
	КонецПопытки;
...
// Данные расшифровки компоновки данных в строку.
//
// Параметры:
//  ДанныеРасшифровкиКомпоновкиДанных - ДанныеРасшифровкиКомпоновкиДанных -
//
// Возвращаемое значение:
//  Строка - Данные расшифровки компоновки данных в строку
Функция ДанныеРасшифровкиКомпоновкиДанныхВСтроку(ДанныеРасшифровкиКомпоновкиДанных) Экспорт
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.УстановитьСтроку();
	СериализаторXDTO.ЗаписатьXML(ЗаписьXML,
...
// Данные расшифровки компоновки данных из строки.
//
// Параметры:
//  СериализованнаяСтрока -Строка -Сериализованная строка
//
// Возвращаемое значение:
//  Неопределено, ДанныеРасшифровкиКомпоновкиДанных - Данные расшифровки компоновки данных из строки
Функция ДанныеРасшифровкиКомпоновкиДанныхИзСтроки(СериализованнаяСтрока) Экспорт
	ЧтениеXML = Новый ЧтениеXML;
	ЧтениеXML.УстановитьСтроку(СериализованнаяСтрока);
	Попытка
		Возврат СериализаторXDTO.ПрочитатьXML(ЧтениеXML, Тип("ДанныеРасшифровкиКомпоновкиДанных"));
	Исключение
		Возврат Неопределено;
	КонецПопытки;
...
// Возвращает все строки структуры хранения для метаданных, ВКЛЮЧАЯ табличные части.
// Стандартный НайтиСтроки({Метаданные: "Документ.X"}) возвращает только основную таблицу,
// потому что ТЧ хранится со своим путём "Документ.X.ТабличнаяЧасть.Y". Без этой обёртки
// SQL-генератор не выдавал UPDATE для полей табличных частей и они оставались с дублями.
//
// Параметры:
//   тзСтруктура - ТаблицаЗначений - структура хранения БД (см. СтруктураХраненияБазыДанных).
//   ИмяМетаданных - Строка - полное имя объекта метаданных.
//
// Возвращаемое значение:
//   Массив - строки структуры хранения (основная таблица и табличные части).
Функция ВсеТаблицыМетаданныхВключаяТабЧасти(тзСтруктура, Знач ИмяМетаданных)
	Результат = Новый Массив;
	Префикс = ИмяМетаданных + ".";
	Для Каждого Стр Из тзСтруктура Цикл
		Если Стр.Метаданные = ИмяМетаданных
			Или (СтрДлина(Стр.Метаданные) > СтрДлина(Префикс)
				И Лев(Стр.Метаданные, СтрДлина(Префикс)) = Префикс) Тогда
...
Функция СвойствоИндексирование(Знач ОбъектМетаданных)

	ЕстьСвойство = УИ_ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ОбъектМетаданных, "Индексирование");
	Если ЕстьСвойство
		И ОбъектМетаданных.Индексирование <> Метаданные.СвойстваОбъектов.Индексирование.НеИндексировать Тогда
		Результат = ОбъектМетаданных.Индексирование;
	Иначе
		Результат = Неопределено;
	КонецЕсли;
	Возврат Результат;
...
// Подготовить таблицу значений для выборки во временную таблицу колонок с типом тип.
//
// Параметры:
//  ТаблицаДанных - ТаблицаЗначений
//  НовыеВыраженияПолей -Структура-Новые выражения полей
Процедура ПодготовитьТаблицуЗначенийДляВыборкиВоВременнуюТаблицуКолонокСТипомТип(ТаблицаДанных, НовыеВыраженияПолей)
	...
	Для Каждого Колонка Из ТаблицаДанных.Колонки Цикл
		...
		ТаблицаДанных.Колонки.Добавить(ИмяКолонкиТип);
		Если Не ТолькоТип Тогда
			ТаблицаДанных.Колонки.Добавить(ИмяКолонкиВременной, ТипБезТипаТип);
		КонецЕсли;
		...
		Если ТолькоТип Тогда
			...
			Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
				...
				ОписаниеТипа = УИ_ОбщегоНазначенияКлиентСервер.ОписаниеТиповИзТипа(СтрокаДанных[ИмяКолонки]);
		...
		ТаблицаДанных.Колонки.Удалить(ИмяКолонки);
		Если Не ТолькоТип Тогда
			ТаблицаДанных.Колонки[ИмяКолонкиВременной].Имя = ИмяКолонки;
		КонецЕсли;

		ТипКолонкиТипа = Новый ОписаниеТипов(маТипы);
		ИзменитьТипКолонкиТаблицыЗначений(ТаблицаДанных, ИмяКолонкиТип, ТипКолонкиТипа);

			//стВыраженияПолей.Вставить(стВыраженияПолей

		Если ТолькоТип Тогда
			НовыеВыраженияПолей.Вставить(ИмяКолонки, "ТИПЗНАЧЕНИЯ(Таблица." + ИмяКолонкиТип + ") КАК " + ИмяКолонки);
		Иначе
			НовыеВыраженияПолей.Вставить(ИмяКолонки, "ISNULL(Таблица."
													 + ИмяКолонки
													 + ", ТИПЗНАЧЕНИЯ(Таблица."
													 + ИмяКолонкиТип
													 + ")) КАК "
