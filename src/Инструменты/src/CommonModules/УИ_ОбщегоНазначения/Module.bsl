...
//
Функция СгенерироватьSQLСкриптЗамены(ЗаменыТребующиеSQL) Экспорт

	тзСтруктура = СтруктураХраненияБазыДанных(, Истина);

	// Определяем тип СУБД по строке соединения (если в ней есть DBMS=).
	// Серверные ИБ часто НЕ содержат DBMS= — выводим тогда обе ветки,
	// но MS SQL ставим закомментированной (префикс "-- "), чтобы запуск всего файла
	// в PostgreSQL-клиенте (DataGrip, psql) не падал на синтаксисе BEGIN TRY.
	...
	Для Каждого СтрокаЗамены Из ЗаменыТребующиеSQL Цикл
		...
		КлючПары = XMLСтрока(СтрокаЗамены.Дубль) + "->" + XMLСтрока(СтрокаЗамены.Оригинал)
			+ "@" + СтрокаЗамены.ПолноеИмяМетаданных;
		Если ОбработанныеПары[КлючПары] <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ОбработанныеПары.Вставить(КлючПары, Истина);

		HexДубль    = СсылкаВHex(СтрокаЗамены.Дубль);
		HexОригинал = СсылкаВHex(СтрокаЗамены.Оригинал);

		Строки.Добавить("-- Замена: " + Строка(СтрокаЗамены.Дубль) + " → " + Строка(СтрокаЗамены.Оригинал));
		...
		СтрокиСтруктуры = тзСтруктура.НайтиСтроки(Новый Структура("Метаданные", СтрокаЗамены.ПолноеИмяМетаданных));
		...
		Для Каждого СтрокаТаблицы Из СтрокиСтруктуры Цикл
			...
			Для Каждого СтрокаПоля Из СтрокаТаблицы.Поля Цикл
				...
				Если СтрЗаканчиваетсяНа(ИмяПоляSQL, "_TYPE") Или СтрЗаканчиваетсяНа(ИмяПоляSQL, "_RTRef") Тогда
					Продолжить;
				КонецЕсли;

				// Для полей _RRRef или обычных полей ссылочного типа.
				// Каждый UPDATE оборачиваем в DO-блок с EXCEPTION — некоторые поля с RRef-суффиксом
				// фактически имеют не-bytea тип в PG (например _KindDt*/_KindCt* в регистре бухгалтерии),
				// для них UPDATE упадёт с "оператор не существует: bytea = numeric", блок поймает ошибку
				// и продолжит выполнение с RAISE NOTICE — в логе psql будут видны пропущенные поля.
				Если СтрЗаканчиваетсяНа(ИмяПоляSQL, "_RRRef") Или СтрЗаканчиваетсяНа(ИмяПоляSQL, "RRef") Тогда
					Строки.Добавить("DO $$ BEGIN");
	...
	Если ВыводитьMSSQL Тогда
		...
		Для Каждого СтрокаЗамены Из ЗаменыТребующиеSQL Цикл
			...
			Строки.Добавить(ПрефиксMSSQL + "-- Замена: " + Строка(СтрокаЗамены.Дубль) + " → " + Строка(СтрокаЗамены.Оригинал));
			...
			Для Каждого СтрокаТаблицы Из СтрокиСтруктуры Цикл
				...
				Для Каждого СтрокаПоля Из СтрокаТаблицы.Поля Цикл
					...
					// Каждый UPDATE оборачиваем в TRY/CATCH по тем же причинам что и для PostgreSQL —
					// поля с RRef-суффиксом могут иметь не-binary тип, тогда UPDATE упадёт; CATCH печатает
					// сообщение в PRINT и не разрывает транзакцию.
					Если СтрЗаканчиваетсяНа(ИмяПоляSQL, "_RRRef") Или СтрЗаканчиваетсяНа(ИмяПоляSQL, "RRef") Тогда
						...
						Строки.Добавить(ПрефиксMSSQL + "END TRY BEGIN CATCH PRINT 'Пропущено "
							+ ИмяТаблицыSQL + "." + ИмяПоляSQL + ": ' + ERROR_MESSAGE(); END CATCH;");
					КонецЕсли;
