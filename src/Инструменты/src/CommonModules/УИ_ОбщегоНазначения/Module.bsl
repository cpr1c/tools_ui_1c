...
//           "ТребуетсяSQL"      - замена средствами платформы невозможна (регистр бухгалтерии), требуется SQL-скрипт.
//           "ТребуетсяSQL_Итог" - итоговая строка с полным SQL-скриптом для завершения замены.
...
//		КонецЕсли;
	...
	// Если есть замены, требующие SQL — генерируем скрипт
	Если Результат.ЗаменыТребующиеSQL.Количество() > 0 Тогда
		SQLСкрипт = СгенерироватьSQLСкриптЗамены(Результат.ЗаменыТребующиеSQL);
		ТекстСообщения = НСтр("ru = 'Не удалось обновить все ссылки средствами платформы.
			|Для завершения корректировки выполните следующий SQL-скрипт:'")
			+ Символы.ПС + Символы.ПС + SQLСкрипт;
		...
		ОшибкаSQL.ПредставлениеОбъектаОшибки = НСтр("ru = 'SQL-скрипт для завершения замены ссылок'");
	КонецЕсли;
...
Функция СгенерироватьSQLСкриптЗамены(ЗаменыТребующиеSQL, ТипСУБД = "Авто") Экспорт
	...
	Если ВыводитьPostgreSQL Тогда
		...
		Для Каждого КлючПары Из ПорядокПар Цикл
			...
			Строки.Добавить("-- Замена: " + Строка(ПерваяСтрока.Дубль) + " → " + Строка(ПерваяСтрока.Оригинал));
	...
	Если ВыводитьMSSQL Тогда
		...
		Для Каждого КлючПары Из ПорядокПар Цикл
			...
			Строки.Добавить(ПрефиксMSSQL + "-- Замена: " + Строка(ПерваяСтрока.Дубль) + " → " + Строка(ПерваяСтрока.Оригинал));
