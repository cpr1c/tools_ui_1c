...
Функция ЗначениеВКонтейнерДляФормы(Значение) Экспорт
	...
	Если НовыйКонтейнер.Тип = ТипыКонтейнеров.Граница Тогда
		...
		Хранилище.Дата = Значение.Значение;
		Если Значение.ВидГраницы = ВидГраницы.Включая Тогда
			Хранилище.ВидГраницы = "Включая";
		Иначе
			Хранилище.ВидГраницы = "Исключая";
		КонецЕсли;
		
		НовыйКонтейнер.Хранилище = Хранилище;
		НовыйКонтейнер.Представление = Строка(Хранилище.Дата) + " " + Хранилище.ВидГраницы;
...
Функция ЗначениеИзКонтейнераФормы(Контейнер) Экспорт
	...
	Если Контейнер.Тип = ТипыКонтейнеров.Граница Тогда
		Возврат Новый Граница(Контейнер.Хранилище.Дата, ВидГраницы[Контейнер.Хранилище.ВидГраницы]);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.МоментВремени Тогда
		Возврат Новый МоментВремени(Контейнер.Хранилище.Дата, Контейнер.Хранилище.Ссылка);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.ТаблицаЗначений Тогда 
		Попытка
			Возврат ЗначениеИзСтрокиВнутр(Контейнер.Хранилище.Значение);
		Исключение
	...
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.ОписаниеТипов Тогда 
		Попытка
			Возврат ЗначениеИзСтрокиXML(Контейнер.Хранилище.Значение, Тип("ОписаниеТипов"));
		Исключение
			Возврат Неопределено;
		КонецПопытки;
	...
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.ТабличныйДокумент Тогда 
		Возврат Контейнер.Хранилище.Значение;
	КонецЕсли;
