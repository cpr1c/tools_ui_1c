...
//           "ТребуетсяSQL_Итог" - итоговая строка-признак: часть ссылок не заменена, нужен режим «Замена через SQL».
...
//
// Параметры:
//   ЗаменыТребующиеSQL - ТаблицаЗначений - с колонками:
...
Функция СгенерироватьSQLСкриптЗамены(ЗаменыТребующиеSQL, ТипСУБД = "Авто") Экспорт
	...
	Если ВыводитьPostgreSQL Тогда
		...
		Для Каждого КлючПары Из ПорядокПар Цикл
			...
			Строки.Добавить("-- Замена: " + ПредставлениеВОднуСтроку(ПерваяСтрока.Дубль) + " (" + XMLСтрока(ПерваяСтрока.Дубль) + ")" + " → " + ПредставлениеВОднуСтроку(ПерваяСтрока.Оригинал) + " (" + XMLСтрока(ПерваяСтрока.Оригинал) + ")");
	...
	Если ВыводитьMSSQL Тогда
		...
		Для Каждого КлючПары Из ПорядокПар Цикл
			...
			Строки.Добавить(ПрефиксMSSQL + "-- Замена: " + ПредставлениеВОднуСтроку(ПерваяСтрока.Дубль) + " (" + XMLСтрока(ПерваяСтрока.Дубль) + ")" + " → " + ПредставлениеВОднуСтроку(ПерваяСтрока.Оригинал) + " (" + XMLСтрока(ПерваяСтрока.Оригинал) + ")");
