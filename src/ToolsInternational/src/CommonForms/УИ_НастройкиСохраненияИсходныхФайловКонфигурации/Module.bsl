...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Для Каждого Стр Из КаталогиСохранения Цикл
		...
		Если Не ЗначениеЗаполнено(Стр.Каталог) Тогда
			UT_CommonClientServer.СообщитьПользователю("Для источника "+Стр.Источник+" не указан каталог сохранения", , , , Отказ);
		КонецЕсли;
	КонецЦикла;
...
Процедура ФайлЗапускаПлатформыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	UT_CommonClient.ДобавитьФорматВОписаниеФайлаСохранения(ОписаниеФайла, "Файл толстого клиента 1С("+ИмяФайла+")", "",ИмяФайла);
