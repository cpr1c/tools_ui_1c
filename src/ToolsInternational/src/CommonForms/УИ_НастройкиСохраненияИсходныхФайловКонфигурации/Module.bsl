...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	Для Каждого Стр Из КаталогиСохранения Цикл
		...
		Если Не ЗначениеЗаполнено(Стр.Каталог) Тогда
			UT_CommonClientServer.MessageToUser("Для источника "+Стр.Источник+" не указан каталог сохранения", , , , Cancel);
		КонецЕсли;
	КонецЦикла;
	
	Если РасположениеБазы = 0 Тогда
		CheckedAttributes.Добавить("КаталогИнформационнойБазы");
	Иначе
		CheckedAttributes.Добавить("СерверИБ");
		CheckedAttributes.Добавить("ИмяБазы");
	КонецЕсли;
EndProcedure
...
&AtClient
Procedure ФайлЗапускаПлатформыStartChoice(Item, ChoiceData, StandardProcessing)
