...
Функция ПроверитьЗаданиеВыполнено(ФормаЗакрывается)
	...
	Задание = UT_TimeConsumingOperations.ОперацияВыполнена(ИдентификаторЗадания, Ложь, Параметры.ВыводитьПрогрессВыполнения,
...
Функция ВозвращатьРезультатВОбработкуВыбора()
	Возврат ОписаниеОповещенияОЗакрытии = Неопределено И Параметры.ПолучатьРезультат И ТипЗнч(ВладелецФормы) = UT_CommonClientServer.ТипУправляемойФормы();
КонецФункции
