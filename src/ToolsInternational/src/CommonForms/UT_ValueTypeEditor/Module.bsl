...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	РежимРаботы=Параметры.StartMode;
...
Процедура ДеревоТиповВыбранПриИзменении(Элемент)
	...
	Если ТекСтрока.Выбран Тогда
		Если Не CompositeDataType Тогда
...
Процедура Apply(Command)
	...
	Для Каждого Тип ИЗ TypesArray Цикл
...
Функция ДобавитьТипВДеревоТипов(ЗаполнятьВыбранныеТипы,ИмяТипа, Картинка, Представление = "", СтрокаДерева = Неопределено, ЭтоГруппа = Ложь, Групповой=Ложь, НедоступенДляСоставногоТипа=Ложь)
	...
	Если ЗначениеЗаполнено(SearchString) и Не Групповой Тогда
		Если СтрНайти(НРег(ПредставлениеТипа), НРег(SearchString))=0 Тогда
...
&НаСервере
Процедура ЗаполнитьТипыПоВидуОбъекта(ВидОбъектовМетаданных, TypePrefix, Картинка,ЗаполнятьВыбранныеТипы)
	КоллекцияОбъектов=Метаданные[ВидОбъектовМетаданных];
	
	СтрокаКоллекции=ДобавитьТипВДеревоТипов(ЗаполнятьВыбранныеТипы,TypePrefix,Картинка,TypePrefix,,,Истина);
...
Процедура УдалитьСтрокуДереваЕслиНетПодчиненныхПриПоиске(СтрокаДерева)
	Если Не ЗначениеЗаполнено(SearchString) Тогда
		Возврат;
	КонецЕсли;
...
Procedure AddTypesToArrayByMetadataCollection(TypesArray, Collection, TypePrefix)
	For each MetadataObject in Collection do
		TypesArray.Add(Тип(TypePrefix+MetadataObject.Name));
	Enddo;
EndProcedure

&НаСервере
Функция МассивВыбранныхТипов()
	TypesArray=Новый Массив;
	
	Для Каждого ЭлементТипа Из ВыбранныеТипы Цикл
		СтрокаТипа=ЭлементТипа.Значение;
		
		Если НРег(СтрокаТипа)="любаяссылка" Тогда
			AddTypesToArrayByMetadataCollection(TypesArray, Метаданные.Справочники,"СправочникСсылка.");
			AddTypesToArrayByMetadataCollection(TypesArray, Метаданные.Документы,"ДокументСсылка.");
			AddTypesToArrayByMetadataCollection(TypesArray, Метаданные.ПланыВидовХарактеристик,"ПланВидовХарактеристикСсылка.");
			AddTypesToArrayByMetadataCollection(TypesArray, Метаданные.ПланыСчетов,"ПланСчетовСсылка.");
			AddTypesToArrayByMetadataCollection(TypesArray, Метаданные.ПланыВидовРасчета,"ПланВидовРасчетаСсылка.");
			AddTypesToArrayByMetadataCollection(TypesArray, Метаданные.ПланыОбмена,"ПланОбменаСсылка.");
			AddTypesToArrayByMetadataCollection(TypesArray, Метаданные.Перечисления,"ПеречислениеСсылка.");
			AddTypesToArrayByMetadataCollection(TypesArray, Метаданные.БизнесПроцессы,"БизнесПроцессСсылка.");
			AddTypesToArrayByMetadataCollection(TypesArray, Метаданные.Задачи,"ЗадачаСсылка.");
		...
		ИначеЕсли ЭлементТипа.Пометка Тогда
			...
			Для Каждого ТекТип ИЗ ОписаниеТипа.Типы() Цикл
				TypesArray.Добавить(ТекТип);
			КонецЦикла;
			
		Иначе
			TypesArray.Добавить(ЭлементТипа.Значение);
		КонецЕсли;
	КонецЦикла;
