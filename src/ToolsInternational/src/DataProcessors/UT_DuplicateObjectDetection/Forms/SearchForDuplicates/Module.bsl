...
Процедура ВсеМестаИспользованияНажатие(Элемент)
	...
	ПоказатьМестаИспользования(DuplicatesFound);
...
&НаКлиенте
Процедура DuplicatesFoundOnActivateRow(Элемент)
...
Процедура ОтложенныйОбработчикАктивизацииСтрокиДублей()
	ИдентификаторСтроки = Элементы.DuplicatesFound.ТекущаяСтрока;
...
Процедура ОбновитьМестаИспользованияКандидата(Знач ИдентификаторСтроки)
	ДанныеСтроки = DuplicatesFound.НайтиПоИдентификатору(ИдентификаторСтроки);
...
&НаКлиенте
Процедура DuplicatesFoundSelection(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
...
&НаКлиенте
Процедура DuplicatesFoundCheckOnChange(Элемент)

	ДанныеСтроки = Элементы.DuplicatesFound.ТекущиеДанные;
	...
	Для Каждого Дубль Из DuplicatesFound.ПолучитьЭлементы() Цикл
...
Процедура ОбновитьМестаИспользованияНеобработанныхДубли(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Тогда
		// Описание группы
		UnprocessedItemsUsageInstances.Очистить();
	...
	UnprocessedItemsUsageInstances.Загрузить(Данные);
...
&НаКлиенте
Процедура UnprocessedItemsUsageInstancesOnActivateRow(Элемент)

	ТекущиеДанные = Элемент.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		UnprocessedItemsErrorDescription = "";
	Иначе
		UnprocessedItemsErrorDescription = ТекущиеДанные.ТекстОшибки;
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура UnprocessedItemsUsageInstancesSelection(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)

	ТекущиеДанные = UnprocessedItemsUsageInstances.НайтиПоИдентификатору(ВыбраннаяСтрока);
	ПоказатьЗначение( , ТекущиеДанные.ОбъектОшибки);

КонецПроцедуры
&НаКлиенте
Процедура РедактироватьМестоИспользованияНеобработанных(Команда)
	ТекущиеДанные = Элементы.UnprocessedItemsUsageInstances.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьКандидатаВДубли(Команда)
	...
	ОткрытьФормуДубля(Элементы.DuplicatesFound.ТекущиеДанные);
...
Процедура РедактироватьНайденныйДубль(Команда)
	ТекДанные=Элементы.DuplicatesFound.ТекущиеДанные;
...
Процедура РазвернутьГруппуДублейИерархически(Знач СтрокаДанных = Неопределено)
	Если СтрокаДанных <> Неопределено Тогда
		Элементы.DuplicatesFound.Развернуть(СтрокаДанных, Истина);
	КонецЕсли;
	
	// Все первого уровня
	ВсеСтроки = Элементы.DuplicatesFound;
	Для Каждого ДанныеСтроки Из DuplicatesFound.ПолучитьЭлементы() Цикл
...
Процедура СвернутьГруппуДублейИерархически(Знач СтрокаДанных = Неопределено)
	Если СтрокаДанных <> Неопределено Тогда
		Элементы.DuplicatesFound.Свернуть(СтрокаДанных);
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьОписаниеСостоянияНайденныхДублей(Форма)
	...
	Форма.DuplicatesFoundStateDetails = Новый ФорматированнаяСтрока(Описание + Символы.ПС + НСтр(
...
Процедура УстановитьЦветаИУсловноеОформление()
	...
	ОтборОформления.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("DuplicatesFound.Ссылка");
	...
	ПолеОформления.Поле = Новый ПолеКомпоновкиДанных("DuplicatesFoundCount");
	...
	ОтборОформления = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборОформления.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("DuplicatesFound.Main");
	...
	ПолеОформления = ЭлементОформления.Поля.Элементы.Добавить();
	ПолеОформления.Поле = Новый ПолеКомпоновкиДанных("DuplicatesFoundMain");
	...
	ПолеОформления = ЭлементОформления.Поля.Элементы.Добавить();
	ПолеОформления.Поле = Новый ПолеКомпоновкиДанных("DuplicatesFoundCheck");
	...
	ОтборОформления = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборОформления.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("DuplicatesFound.Count");
	ОтборОформления.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	...
	ОтборОформления = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборОформления.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("DuplicatesFound.Check");
	ОтборОформления.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборОформления.ПравоеЗначение = 0;
	...
	ПолеОформления = ЭлементОформления.Поля.Элементы.Добавить();
	ПолеОформления.Поле = Новый ПолеКомпоновкиДанных("DuplicatesFound");
...
Функция ПарыЗаменДублей()
	...
	ДеревоДублей = РеквизитФормыВЗначение("DuplicatesFound");
...
Функция ЗаполнитьРезультатыПоискаДублей(Знач АдресРезультата)
	...
	ЭлементыДерева = DuplicatesFound.ПолучитьЭлементы();
...
Функция ЗаполнитьРезультатыУдаленияДублей(Знач АдресРезультата)
	...
	Если ЗавершеноБезОшибок Тогда
		ВсегоОбработано = 0;
		ВсегоОсновных   = 0;
		Для Каждого ГруппаДублей Из DuplicatesFound.ПолучитьЭлементы() Цикл
		...
		Если ВсегоОсновных = 1 Тогда
			...
			Если ПоследнийКандидат = Неопределено Тогда
				DuplicatesFoundStateDetails = Новый ФорматированнаяСтрока(СтрШаблон(
	...
	Если ЗавершеноБезОшибок Тогда
		DuplicatesFound.ПолучитьЭлементы().Очистить();
		Возврат Истина;
	КонецЕсли;
	...
	ЗначениеВРеквизитФормы(РеквизитФормыВЗначение("DuplicatesFound"), "UnprocessedDuplicates");
