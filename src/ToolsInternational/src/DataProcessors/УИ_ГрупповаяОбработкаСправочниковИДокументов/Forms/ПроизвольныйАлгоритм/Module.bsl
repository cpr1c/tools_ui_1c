...
Функция ВыполнитьОбработку() Экспорт
	Если Открыта() Тогда
		ТекстПроизвольногоАлгоритма = UT_CodeEditorClient.ТекстКодаРедактора(ЭтотОбъект, "Редактор");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	UT_CodeEditorClient.FormOnOpen(ЭтотОбъект);
...
Процедура Подключаемый_РедакторКодаЗавершениеИнициализации() Экспорт
	UT_CodeEditorClient.УстановитьТекстРедактора(ЭтотОбъект, "Редактор", ТекстПроизвольногоАлгоритма);
