...
Процедура НаборыДанныхПоляРольПредставлениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОповещения.Вставить("RowID", Элементы.НаборыДанныхПоля.ТекущаяСтрока);
	ПараметрыОповещения.Вставить("ИдентификаторСтрокиНабора", Элементы.НаборыДанных.ТекущаяСтрока);

	ОткрытьФорму("Обработка.UT_DCSEditor.Форма.ФормаРедактированияРолиПоляНабораДанных", ПараметрыФормы, ЭтотОбъект, ,
...
Процедура ПоляДоступныеЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДопПараметры.Вставить("RowID", ТекДанные.ПолучитьИдентификатор());
	ДопПараметры.Вставить("ИдентификаторСтрокиНабора", Элементы.НаборыДанных.ТекущаяСтрока);

	UT_CommonClient.OpenValueListChoiceItemsForm(ТекДанные.ДоступныеЗначения,
...
Процедура НаборыДанныхПоляПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ЧислоВКонце=UT_StringFunctionsClientServer.NumberAtStringEnd(ТекСтрока.ПутьКДанным);
	...
	Иначе
		...
		UT_StringFunctionsClientServer.DeleteLastCharInString(НовыйПутьКДанным, СтрДлина(Суффикс));
	...
	ТекСтрока.Заголовок=UT_StringFunctionsClientServer.IdentifierPresentation(ТекСтрока.ПутьКДанным);
...
Процедура ПоляПутьКДаннымПриИзменении(Элемент)
	...
	Если ТекДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекДанные.Заголовок=UT_StringFunctionsClientServer.IdentifierPresentation(ТекДанные.ПутьКДанным);
КонецПроцедуры
...
Процедура ПоляТипЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	UT_CommonClient.EditType(ТекДанные.ТипЗначения, 2,СтандартнаяОбработка,ЭтотОбъект, Новый ОписаниеОповещения("ПоляТипЗначенияНачалоВыбораЗавершение",ЭтотОбъект, Новый Структура("ТекСтрока",Элементы.НаборыДанныхПоля.ТекущаяСтрока)));
КонецПроцедуры
...
Процедура РесурсыВыражениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	ДопПараметры=Новый Структура;
	ДопПараметры.Вставить("RowID", Элементы.Ресурсы.ТекущаяСтрока);

	UT_CommonClient.OpenTextEditingForm(ТекДанные.Выражение,
...
Процедура ВычисляемыеПоляВыражениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	ДопПараметры=Новый Структура;
	ДопПараметры.Вставить("RowID", Элементы.ВычисляемыеПоля.ТекущаяСтрока);
...
Процедура ВычисляемыеПоляТипЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	UT_CommonClient.EditType(ТекДанные.ТипЗначения, 2,СтандартнаяОбработка,ЭтотОбъект, Новый ОписаниеОповещения("ВычисляемыеПоляТипЗначенияНачалоВыбораЗавершение",ЭтотОбъект, Новый Структура("ТекСтрока",Элементы.ВычисляемыеПоля.ТекущаяСтрока)));
КонецПроцедуры
...
Процедура ПараметрыСКДЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	UT_CommonClient.OpenValueListChoiceItemsForm(ТекДанные.Значение,
...
Процедура ПараметрыСКДВыражениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	ДопПараметры=Новый Структура;
	ДопПараметры.Вставить("RowID", Элементы.ПараметрыСКД.ТекущаяСтрока);
...
Процедура ПараметрыСКДИмяПриИзменении(Элемент)
	...
	ТекДанные.Заголовок=UT_StringFunctionsClientServer.IdentifierPresentation(ТекДанные.Имя);
КонецПроцедуры
...
Процедура ПараметрыСКДТипЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	UT_CommonClient.EditType(ТекДанные.ТипЗначения, 3,СтандартнаяОбработка,ЭтотОбъект, Новый ОписаниеОповещения("ПараметрыСКДТипЗначенияНачалоВыбораЗавершение",ЭтотОбъект, Новый Структура("ТекСтрока",Элементы.ПараметрыСКД.ТекущаяСтрока)));
КонецПроцедуры
#КонецОбласти
...
Процедура КомпоновщикНастроекНастройкиПриАктивизацииСтроки(Элемент)
	...
	ИначеЕсли ТипЭлемента = Тип("НастройкиКомпоновкиДанных") Или ТипЭлемента = Тип(
		...
		LocalSelectedFields = Истина;
		Элементы.LocalSelectedFields.ТолькоПросмотр = Истина;
		Элементы.СтраницыПолейВыбора.ТекущаяСтраница = Элементы.НастройкиВыбранныхПолей;

		LocalFilter = Истина;
		Элементы.LocalFilter.ТолькоПросмотр = Истина;
		Элементы.СтраницыОтбора.ТекущаяСтраница = Элементы.НастройкиОтбора;

		LocalOrder = Истина;
		Элементы.LocalOrder.ТолькоПросмотр = Истина;
		Элементы.СтраницыПорядка.ТекущаяСтраница = Элементы.НастройкиПорядка;

		LocalConditionalAppearance = Истина;
		Элементы.LocalConditionalAppearance.ТолькоПросмотр = Истина;
		Элементы.СтраницыУсловногоОформления.ТекущаяСтраница = Элементы.НастройкиУсловногоОформления;

		LocalOutputParameters = Истина;
		Элементы.LocalOutputParameters.ТолькоПросмотр = Истина;
...
Процедура ЛокальныеВыбранныеПоляПриИзменении(Элемент)
	...
	Если LocalSelectedFields Тогда
...
Процедура ЛокальноеУсловноеОформлениеПриИзменении(Элемент)
	...
	Если LocalConditionalAppearance Тогда
...
Процедура ЛокальныеПараметрыВыводаПриИзменении(Элемент)
	...
	Если LocalOutputParameters Тогда
...
Функция ПодготовитьСКДДляСохраненияВФайл()
	...
	DCSText=UT_Common.ValueToXMLString(ПолучитьИзВременногоХранилища(АдресСхемыКомпоновкиДанных));

	Возврат ПоместитьВоВременноеХранилище(DCSText, УникальныйИдентификатор);
КонецФункции
...
Процедура ПрочитатьСхемуИзФайлаНаСервере(АдресФайла)
	...
	Попытка
		СКД=UT_Common.ValueFromXMLString(Текст.ПолучитьТекст());
	Исключение
	...
	Для Каждого ТекНабор Из СтрокаНабора.ПолучитьЭлементы() Цикл
		Для Каждого ТекПоле Из ТекНабор.Поля Цикл
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				НовоеПоле.Заголовок=UT_StringFunctionsClientServer.IdentifierPresentation(НовоеПоле.ПутьКДанным);
...
Процедура ОткрытьКонструкторЗапросаЗавершение(Текст, ДополнительныеПараметры) Экспорт
	...
	RowID=ДополнительныеПараметры.ТекСтрока;
	СтрокаНабора=НаборыДанных.НайтиПоИдентификатору(RowID);
...
Процедура ДобавитьНаборДанных(Вид)
	...
	Если Вид = ВидыНаборовДанных.Запрос Тогда
		НаборДанных.Картинка=БиблиотекаКартинок.УИ_DataSetСКДЗапрос;
		НаборДанных.АвтоЗаполнениеДоступныхПолей=Истина;
		НаборДанных.ИспользоватьГруппировкиЗапросаЕслиВозможно=Истина;
	ИначеЕсли Вид = ВидыНаборовДанных.Объект Тогда
		НаборДанных.Картинка=БиблиотекаКартинок.UT_DataSetDCSObject;
	ИначеЕсли Вид = ВидыНаборовДанных.Объединение Тогда
		НаборДанных.Картинка=БиблиотекаКартинок.UT_DataSetDCSUnion;
	КонецЕсли;
...
Процедура НаборыДанныхПоляРольПредставлениеНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СтрокаПоля=СтрокаНабора.Поля.НайтиПоИдентификатору(ДополнительныеПараметры.RowID);
...
Процедура ВычисляемыеПоляВыражениеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СтрокаРесурса=ВычисляемыеПоля.НайтиПоИдентификатору(ДополнительныеПараметры.RowID);
...
Процедура ВычисляемыеПоляДоступныеЗначенияНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СтрокаПоля=ВычисляемыеПоля.НайтиПоИдентификатору(ДополнительныеПараметры.RowID);
	СтрокаПоля.ДоступныеЗначения=Результат;
КонецПроцедуры
...
Процедура РесурсыГруппировкиНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СтрокаРесурса=Ресурсы.НайтиПоИдентификатору(ДополнительныеПараметры.RowID);
...
Процедура ПараметрыСКДЗначениеНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СтрокаРесурса=ПараметрыСКД.НайтиПоИдентификатору(ДополнительныеПараметры.RowID);
...
&НаКлиенте
Процедура GroupFieldsNotAvailable()
...
&НаКлиенте
Процедура SelectedFieldsAvailable(ЭлементСтруктуры)
	...
	Иначе
		...
		LocalSelectedFields = Ложь;
		Элементы.СтраницыПолейВыбора.ТекущаяСтраница = Элементы.ОтключенныеНастройкиВыбранныхПолей;

	КонецЕсли;

	Элементы.LocalSelectedFields.ТолькоПросмотр = Ложь;

КонецПроцедуры

&НаКлиенте
Процедура SelectedFieldsUnavailable()
...
&НаКлиенте
Процедура FilterAvailable(ЭлементСтруктуры)
	...
	Элементы.LocalFilter.ТолькоПросмотр = Ложь;

КонецПроцедуры

&НаКлиенте
Процедура FilterUnavailable()
...
&НаКлиенте
Процедура OrderAvailable(ЭлементСтруктуры)
	...
	Элементы.LocalOrder.ТолькоПросмотр = Ложь;

КонецПроцедуры

&НаКлиенте
Процедура OrderUnavailable()
...
&НаКлиенте
Процедура ConditionalAppearanceAvailable(ЭлементСтруктуры)
	...
	Иначе
		...
		LocalConditionalAppearance = Ложь;
		Элементы.СтраницыУсловногоОформления.ТекущаяСтраница = Элементы.ОтключенныеНастройкиУсловногоОформления;

	КонецЕсли;

	Элементы.LocalConditionalAppearance.ТолькоПросмотр = Ложь;

КонецПроцедуры

&НаКлиенте
Процедура ConditionalAppearanceUnavailable()
...
&НаКлиенте
Процедура OutputParametersAvailable(ЭлементСтруктуры)
	...
	Иначе
		...
		LocalOutputParameters = Ложь;
		Элементы.СтраницыПараметровВывода.ТекущаяСтраница = Элементы.ОтключенныеНастройкиПараметровВывода;

	КонецЕсли;

	Элементы.LocalOutputParameters.ТолькоПросмотр = Ложь;

КонецПроцедуры

&НаКлиенте
Процедура OutputParametersUnavailable()
...
Процедура СохранитьВТаблицуФормыНастройкуТекущегоВариантаНастроек()
	...
	СтрокаПредыдущегоВарианта.Настройки=UT_Common.ValueToXMLString(
		КомпоновщикТекущихНастроек.ПолучитьНастройки());
КонецПроцедуры

&НаСервере
Процедура ВариантыНастроекПриАктивизацииСтрокиНаСервере(RowID)
	Если RowID = ИдентификаторСтрокиТекущегоВариантаНастроек Тогда
		Возврат;
	КонецЕсли;

	ТекДанные=ВариантыНастроек.НайтиПоИдентификатору(RowID);
	Если ТекДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекДанные.Настройки) Тогда
		Настройки=UT_Common.ValueFromXMLString(ТекДанные.Настройки);
	Иначе
...
Процедура ПрочитатьНаборыДанныхСКДВДанныеФормы(СКДНаборыДанных, СтрокаРодительскогоНабора = Неопределено)
	...
	Для Каждого СтрокаНабора Из СКДНаборыДанных Цикл
		...
		Если ТипЗнч(СтрокаНабора) = Тип("НаборДанныхЗапросСхемыКомпоновкиДанных") Тогда
			...
			НовыйНабор.Картинка=БиблиотекаКартинок.УИ_DataSetСКДЗапрос;
		ИначеЕсли ТипЗнч(СтрокаНабора) = Тип("НаборДанныхОбъектСхемыКомпоновкиДанных") Тогда
			НовыйНабор.Вид=ВидыНаборов.Объект;
			НовыйНабор.Картинка=БиблиотекаКартинок.UT_DataSetDCSObject;
		Иначе
			НовыйНабор.Вид=ВидыНаборов.Объединение;
			НовыйНабор.Картинка=БиблиотекаКартинок.UT_DataSetDCSUnion;
		КонецЕсли;
...
Процедура ПрочитатьВариантыНастроекСКДВДанныеФормы(СКД)
	...
	Для Каждого СтрокаВарианта Из СКД.ВариантыНастроек Цикл
		...
		НовыеДанные.Настройки=UT_Common.ValueToXMLString(СтрокаВарианта.Настройки);
	КонецЦикла;
...
Процедура ЗаполнитьВариантыНастроекСКДПоДаннымФормы(СКД)
	...
	Для Каждого СтрокаВарианта Из ВариантыНастроек Цикл
		...
		Если ЗначениеЗаполнено(СтрокаВарианта.Настройки) Тогда
			UT_CommonClientServer.CopyDataCompositionSettings(НовыеДанные.Настройки,
				UT_Common.ValueFromXMLString(СтрокаВарианта.Настройки));
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
&НаСервере
