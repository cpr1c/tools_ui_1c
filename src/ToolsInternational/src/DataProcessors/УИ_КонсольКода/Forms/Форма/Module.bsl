...
Процедура ДобавитьДополнительныйКонтекстВРедакторКода(ИдентификаторРедактора)
	...
	Для Каждого Пер Из ПеременныеКонтекста Цикл
		Если Не UT_CommonClientServer.ИмяПеременнойКорректно(Пер.Имя) Тогда
			Продолжить;
		КонецЕсли;
	...
	UT_CodeEditorClient.ДобавитьКонтекстРедактораКода(ЭтотОбъект, ИдентификаторРедактора, СтруктураДополнительногоКонтекста);
КонецПроцедуры
...
Процедура ОбновитьЗначениеПеременныхАлгоритмовИзРедактора()
	ТекстАлгоритмаКлиент=UT_CodeEditorClient.ТекстКодаРедактора(ЭтотОбъект, "Клиент");
	ТекстАлгоритмаСервер=UT_CodeEditorClient.ТекстКодаРедактора(ЭтотОбъект, "Сервер");
КонецПроцедуры

&НаКлиенте
Процедура УстановитьТекстРедактора(ИдентификаторРедактора, ТекстАлгоритма)
	UT_CodeEditorClient.УстановитьТекстРедактора(ЭтотОбъект, ИдентификаторРедактора, ТекстАлгоритма);
