...
Процедура Подключаемый_РедакторКодаЗавершениеИнициализации() Экспорт
	Если ЗначениеЗаполнено(ИмяФайлаАлгоритма) Тогда
		UT_CommonClient.ПрочитатьДанныеКонсолиИзФайла("КонсольКода", СтруктураОписанияСохраняемогоФайла(),
...
Функция СтруктураОписанияСохраняемогоФайла()
	...
	UT_CommonClient.ДобавитьФорматВОписаниеФайлаСохранения(Структура, "Файл алгоритма(*.xbsl)", "xbsl");
	Возврат Структура;
КонецФункции
...
Процедура СохранитьФайлНаДиск(СохранитьКак = Ложь)
	...
	UT_CommonClient.СохранитьДанныеКонсолиВФайл("КонсольКода", СохранитьКак,
...
Процедура УстановитьТекстРедактора(ИдентификаторРедактора, ТекстАлгоритма)
	УИ_РедакторКодаКлиент.УстановитьТекстРедактора(ЭтотОбъект, ИдентификаторРедактора, ТекстАлгоритма);
...
Процедура РедактироватьЗначениеПеременной(ТаблицаФормы)
	...
	UT_CommonClient.РедактироватьОбъект(ТекДанные.Значение);
КонецПроцедуры
