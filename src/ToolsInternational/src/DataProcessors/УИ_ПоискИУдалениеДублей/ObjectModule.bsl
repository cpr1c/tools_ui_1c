...
Функция НаименованиеПодсистемы(ДляПользователя) Экспорт
	КодЯзыка = ?(ДляПользователя, UT_CommonClientServer.КодОсновногоЯзыка(), "");
...
Функция ГруппыДублей(Знач ПараметрыПоиска, Знач ЭталонныйОбъект = Неопределено) Экспорт
	...
	РазмерВозвращаемойПорции = UT_CommonClientServer.СвойствоСтруктуры(ПараметрыПоиска,
	...
	РассчитыватьМестаИспользования = UT_CommonClientServer.СвойствоСтруктуры(ПараметрыПоиска,
	...
	ДополнительныеПараметры = UT_CommonClientServer.СвойствоСтруктуры(ПараметрыПоиска,
	...
	Если ИспользоватьПрикладныеПравила Тогда
		...
		МенеджерОбластиПоиска = UT_Common.МенеджерОбъектаПоПолномуИмени(ПолноеИмяОМ);
...
Функция МестаИспользованияСсылок(Знач НаборСсылок, Знач АдресРезультата = "")
	...
	Возврат UT_Common.МестаИспользования(НаборСсылок, АдресРезультата);
...
Процедура ЗаменитьСсылки(Знач ПарыЗамен, Знач Параметры = Неопределено, Знач АдресРезультата = "")
	...
	Результат = UT_Common.ЗаменитьСсылки(ПарыЗамен, Параметры);
