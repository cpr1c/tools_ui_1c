...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Попытка
		FuzzySearch = UT_Common.ПодключитьКомпонентуИзМакета("FuzzyStringMatchExtension",
	...
	НастройкиФормы = UT_Common.ХранилищеОбщихНастроекЗагрузить(ИмяФормы, "");
	...
	UT_Common.ФормаИнструментаПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка, Элементы.ПанельДействийМастера);
...
Процедура СсылкаПодробнееНажатие(Элемент)
	UT_CommonClient.ПоказатьПодробнуюИнформацию(Неопределено, Элемент.Подсказка);
КонецПроцедуры
...
Процедура РедактироватьНеобработанныйДубль(Команда)
	...
	UT_CommonClient.РедактироватьОбъект(ТекущиеДанные.Ссылка);
КонецПроцедуры
#КонецОбласти
...
Процедура РедактироватьМестоИспользованияНеобработанных(Команда)
	...
	UT_CommonClient.РедактироватьОбъект(ТекущиеДанные.ОбъектОшибки);
КонецПроцедуры
...
Процедура РедактироватьМестоИспользованияКандидата(Команда)
	...
	UT_CommonClient.РедактироватьОбъект(ТекущиеДанные.Данные);
КонецПроцедуры
#КонецОбласти
...
Процедура РедактироватьНайденныйДубль(Команда)
	...
	UT_CommonClient.РедактироватьОбъект(ТекДанные.Ссылка);
КонецПроцедуры
...
Процедура ИнициализироватьКомпоновщикОтбораИПравила(НастройкиФормы)
	...
	НастройкиКД = UT_CommonClientServer.СвойствоСтруктуры(НастройкиФормы, "НастройкиКД");
	...
	СохраненныеПравила = UT_CommonClientServer.СвойствоСтруктуры(НастройкиФормы, "ПравилаПоиска");
	...
	Если Не ОтборыЗагружены Тогда
		UT_CommonClientServer.УстановитьЭлементОтбора(
	...
	Если СтрокаТаблицыНастроек.СобытиеПараметрыПоискаДублей Тогда
		...
		МенеджерОбъектаМетаданных = UT_Common.МенеджерОбъектаПоПолномуИмени(ОбъектМетаданных.ПолноеИмя());
		//@skip-warning
...
Процедура ПриСозданииНаСервереИнициализацияДанных(НастройкиФормы)
	УчитыватьПрикладныеПравила = UT_CommonClientServer.СвойствоСтруктуры(НастройкиФормы,
		"УчитыватьПрикладныеПравила");
	ОбластьПоискаДублей        = UT_CommonClientServer.СвойствоСтруктуры(НастройкиФормы,
		"ОбластьПоискаДублей");
...
Процедура СохранитьПользовательскиеНастройки()
	...
	UT_Common.ХранилищеОбщихНастроекСохранить(ИмяФормы, "", НастройкиФормы);
КонецПроцедуры
...
Функция НайтиИУдалитьДубли()
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		...
		ПараметрыПроцедуры.Вставить("ПараметрыЗаписи", UT_CommonClientServer.ПараметрыЗаписиФормы(ЭтотОбъект));
		ПараметрыПроцедуры.Вставить("УчитыватьПрикладныеПравила", УчитыватьПрикладныеПравила);
	...
	НастройкиЗапуска = UT_TimeConsumingOperations.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НаименованиеМетода;

	Возврат UT_TimeConsumingOperations.ВыполнитьВФоне(ИмяПроцедуры, ПараметрыПроцедуры, НастройкиЗапуска);
КонецФункции
...
Функция ЗаполнитьРезультатыУдаленияДублей(Знач АдресРезультата)
	...
	Если ЗавершеноБезОшибок Тогда
		...
		Если ВсегоОсновных = 1 Тогда
			...
			Иначе
				ПоследнийКандидатСтрокой = UT_Common.ПредметСтрокой(ПоследнийКандидат);
				ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(СтрШаблон(
