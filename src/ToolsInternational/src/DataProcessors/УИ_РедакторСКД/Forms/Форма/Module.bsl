...
Процедура ПоляДоступныеЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	UT_CommonClient.ОткрытьФормуВыбораЭлементовСпискаЗначений(ТекДанные.ДоступныеЗначения,
...
Процедура НаборыДанныхПоляПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ЧислоВКонце=UT_StringFunctionsClientServer.ЧислоВКонцеСтроки(ТекСтрока.ПутьКДанным);
	...
	Иначе
		...
		UT_StringFunctionsClientServer.УдалитьПоследнийСимволВСтроке(НовыйПутьКДанным, СтрДлина(Суффикс));
	...
	ТекСтрока.Заголовок=UT_StringFunctionsClientServer.ПредставлениеИдентификатора(ТекСтрока.ПутьКДанным);
...
Процедура ПоляПутьКДаннымПриИзменении(Элемент)
	...
	Если ТекДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекДанные.Заголовок=UT_StringFunctionsClientServer.ПредставлениеИдентификатора(ТекДанные.ПутьКДанным);
КонецПроцедуры
...
Процедура ПоляТипЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	UT_CommonClient.РедактироватьТип(ТекДанные.ТипЗначения, 2,СтандартнаяОбработка,ЭтотОбъект, Новый ОписаниеОповещения("ПоляТипЗначенияНачалоВыбораЗавершение",ЭтотОбъект, Новый Структура("ТекСтрока",Элементы.НаборыДанныхПоля.ТекущаяСтрока)));
КонецПроцедуры
...
Процедура РесурсыВыражениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	UT_CommonClient.ОткрытьФормуРедактированияТекста(ТекДанные.Выражение,
...
Процедура РесурсыГруппировкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	UT_CommonClient.ОткрытьФормуВыбораЭлементовСпискаЗначений(СписокДоступныхГруппировок,
...
Процедура ВычисляемыеПоляТипЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	UT_CommonClient.РедактироватьТип(ТекДанные.ТипЗначения, 2,СтандартнаяОбработка,ЭтотОбъект, Новый ОписаниеОповещения("ВычисляемыеПоляТипЗначенияНачалоВыбораЗавершение",ЭтотОбъект, Новый Структура("ТекСтрока",Элементы.ВычисляемыеПоля.ТекущаяСтрока)));
КонецПроцедуры
...
Процедура ПараметрыСКДЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	UT_CommonClient.ОткрытьФормуВыбораЭлементовСпискаЗначений(ТекДанные.Значение,
...
Процедура ПараметрыСКДИмяПриИзменении(Элемент)
	...
	ТекДанные.Заголовок=UT_StringFunctionsClientServer.ПредставлениеИдентификатора(ТекДанные.Имя);
КонецПроцедуры
...
Процедура ПараметрыСКДТипЗначенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	UT_CommonClient.РедактироватьТип(ТекДанные.ТипЗначения, 3,СтандартнаяОбработка,ЭтотОбъект, Новый ОписаниеОповещения("ПараметрыСКДТипЗначенияНачалоВыбораЗавершение",ЭтотОбъект, Новый Структура("ТекСтрока",Элементы.ПараметрыСКД.ТекущаяСтрока)));
КонецПроцедуры
#КонецОбласти
...
Процедура ОткрытьКонструкторЗапроса(Команда)
	...
	Если UT_CommonClientServer.ВерсияПлатформыНеМладше_8_3_14() Тогда
...
Процедура СохранитьСхемуВФайл(Команда)
	UT_CommonClient.ПодключитьРасширениеРаботыСФайламиСВозможнойУстановкой(
...
Функция ПодготовитьСКДДляСохраненияВФайл()
	...
	ТекстСКД=UT_Common.ЗначениеВСтрокуXML(ПолучитьИзВременногоХранилища(АдресСхемыКомпоновкиДанных));
...
Процедура ПрочитатьСхемуИзФайлаНаСервере(АдресФайла)
	...
	Попытка
		СКД=UT_Common.ЗначениеИзСтрокиXML(Текст.ПолучитьТекст());
	Исключение
	...
	Для Каждого ТекНабор Из СтрокаНабора.ПолучитьЭлементы() Цикл
		Для Каждого ТекПоле Из ТекНабор.Поля Цикл
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				НовоеПоле.Заголовок=UT_StringFunctionsClientServer.ПредставлениеИдентификатора(НовоеПоле.ПутьКДанным);
...
Процедура СохранитьВТаблицуФормыНастройкуТекущегоВариантаНастроек()
	...
	СтрокаПредыдущегоВарианта.Настройки=UT_Common.ЗначениеВСтрокуXML(
...
Процедура ВариантыНастроекПриАктивизацииСтрокиНаСервере(ИдентификаторСтроки)
	...
	Если ЗначениеЗаполнено(ТекДанные.Настройки) Тогда
		Настройки=UT_Common.ЗначениеИзСтрокиXML(ТекДанные.Настройки);
	Иначе
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	UT_CommonClientServer.УстановитьЭлементОтбора(НовоеУО.Отбор,
...
Процедура ПрочитатьВариантыНастроекСКДВДанныеФормы(СКД)
	...
	Для Каждого СтрокаВарианта Из СКД.ВариантыНастроек Цикл
		...
		НовыеДанные.Настройки=UT_Common.ЗначениеВСтрокуXML(СтрокаВарианта.Настройки);
	КонецЦикла;
...
Процедура ЗаполнитьВариантыНастроекСКДПоДаннымФормы(СКД)
	...
	Для Каждого СтрокаВарианта Из ВариантыНастроек Цикл
		...
		Если ЗначениеЗаполнено(СтрокаВарианта.Настройки) Тогда
			UT_CommonClientServer.СкопироватьНастройкиКомпоновкиДанных(НовыеДанные.Настройки,
				UT_Common.ЗначениеИзСтрокиXML(СтрокаВарианта.Настройки));
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
&НаСервере
