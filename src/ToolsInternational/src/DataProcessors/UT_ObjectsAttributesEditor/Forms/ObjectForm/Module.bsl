...
Процедура _ОткрытьФормуСписка(Команда)
	Если мОбъектСсылка <> Неопределено Тогда
		пПолноеИмя = вПолучитьПолноеИмяМД(мОбъектСсылка);
		UT_CommonClient.ОткрытьДинамическийСписок(пПолноеИмя);
	КонецЕсли;
КонецПроцедуры
...
Процедура _ОткрытьРедакторДвижений(Команда)
	...
	Попытка
		ОткрытьФорму("Обработка.UT_ObjectsAttributesEditor.Форма.ФормаРедакторДвижений", СтрукПарам, , мОбъектСсылка);
	Исключение
