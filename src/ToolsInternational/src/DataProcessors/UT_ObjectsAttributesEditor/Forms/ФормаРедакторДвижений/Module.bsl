...
Процедура _ОткрытьВНовомОкне(Команда)
	...
	ОткрытьФорму("Обработка.UT_ObjectsAttributesEditor.Форма.ФормаРедакторДвижений", СтрукПарам, , ТекущаяДата(), , ,
...
Процедура _ЗагрузитьДвиженияДругогоДокумента(Команда)
	...
	СтрукПарам = Новый Структура("CloseOnOwnerClose, MetadataGroups", Истина, "Documents");
	ОткрытьФорму("ОбщаяФорма.UT_MetadataSelectionForm", СтрукПарам, ЭтаФорма, , , , ,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
