...
Процедура ОбновитьЧерезСкачиваниеФайла()
	ИмяФайла=UT_CommonClientServer.ИмяФайлаСкачивания();
...
Процедура ОбновитьЧерезОбновлениеРасширения()
	...
	Иначе
		UT_CommonClientServer.СообщитьПользователю("Ошибка применения обновления " + РезультатьОбновления);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьТекущуюВерсию()
	ТекущаяВерсия = UT_CommonClientServer.Версия();
	ВариантПоставки=UT_CommonClientServer.ВариантПоставки();
	ИмяФайлаСкачки=UT_CommonClientServer.ИмяФайлаСкачивания();
...
//Получаем список всех релизов
	...
	Для Каждого ТекРелиз Из МассивРелизов Цикл
		...
		Если UT_CommonClientServer.СравнитьВерсииБезНомераСборки(ВерсияТекРелиза, ТекущаяВерсия) > 0 Тогда
			СоответствиеОписанияРелизов.Вставить(ВерсияТекРелиза, ТекРелиз);
		КонецЕсли;

		Если UT_CommonClientServer.СравнитьВерсииБезНомераСборки(ВерсияТекРелиза, МаксимальныйРелиз) <= 0 Тогда
			Продолжить;
		КонецЕсли;
...
Процедура УстановитьНеобходимостьОбновления()
	Если UT_CommonClientServer.СравнитьВерсииБезНомераСборки(АктуальнаяВерсия, ТекущаяВерсия) > 0 Тогда
