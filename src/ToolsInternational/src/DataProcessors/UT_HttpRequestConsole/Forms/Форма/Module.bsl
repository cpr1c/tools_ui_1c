...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	UT_Common.ToolFormOnCreateAtServer(ЭтотОбъект, Отказ, СтандартнаяОбработка, Элементы.ГруппаКоманднаяПанельФормы);
...
Процедура СохранитьЗапросыВФайл(Команда)
	UT_CommonClient.СохранитьДанныеКонсолиВФайл("HTTPREQUESTCONSOLE", Ложь,
...
Процедура СохранитьЗапросыВФайлКак(Команда)
	UT_CommonClient.СохранитьДанныеКонсолиВФайл("HTTPREQUESTCONSOLE", Истина,
		СтруктураОписанияСохраняемогоФайла(), ПоместитьДанныеИсторииВоВременноеХранилище(),
		Новый ОписаниеОповещения("СохранениеВФайлЗавершение", ЭтотОбъект));
КонецПроцедуры
...
Процедура РедактироватьТелоЗапросаВРедактореJSON(Команда)
	UT_CommonClient.РедактироватьJSON(ТелоЗапроса, Ложь,
...
Процедура РедактироватьТелоОтветаВРедактореJSONАнализируемогоЗапроса(Команда)
	UT_CommonClient.РедактироватьJSON(ТелоОтветаСтрокой, Истина);
КонецПроцедуры
...
Процедура ЗагрузитьФайлКонсолиНаСервере(Адрес)
	...
	ТаблицаИстории = Обработки.UT_HttpRequestConsole.ДанныеСохраненияИзСериализованнойСтроки(Адрес, ИмяФайлаЗапросов);
...
Процедура ЗагрузитьФайлКонсоли()
	...
	UT_CommonClient.ПрочитатьДанныеКонсолиИзФайла("HTTPREQUESTCONSOLE",
...
Функция ПодготовленныйЗапросHTTP(СтруктураURL)
	...
	Если РедактированиеЗаголовковТаблицей Тогда
		...
		КонецЦикла;
	Иначе
		Заголовки = UT_CommonClientServer.HTTPRequestHeadersFromString(ЗаголовкиСтрока);
	КонецЕсли;
...
Процедура ВыполнитьЗапросНаСервере()
	СтруктураURL = UT_HTTPConnector.РазобратьURL(URLЗапроса);
...
Процедура УстановитьСтраницуРедактированияЗаголовковЗапроса()
	...
	Иначе
		ЗаголовкиСтрока = UT_CommonClientServer.GetHTTPHeadersString(ТаблицаЗаголовковЗапроса);
	КонецЕсли;
КонецПроцедуры
