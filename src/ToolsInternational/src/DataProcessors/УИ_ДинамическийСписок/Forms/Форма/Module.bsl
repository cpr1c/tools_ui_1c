...
Процедура РедактироватьОбъект(Команда)
	...
	UT_CommonClient.РедактироватьОбъект(ТекСсылка);
КонецПроцедуры
#КонецОбласти
...
Процедура УстановитьПараметрыДинамическогоСпискаНаСервере(ВыбраннаяСтрока)
	...
	ОписаниеКнопки=UT_Forms.НовыйОписаниеКомандыКнопки();
	...
	UT_Forms.СоздатьКнопкуПоОписанию(ЭтотОбъект, ОписаниеКнопки);
...
Функция МассивТаблицРеквизитовДляОбъектаМетаданных(ОбъектМетаданных)
	...
	Если UT_Common.ЭтоРегистрСведений(ОбъектМетаданных) Или UT_Common.ЭтоРегистрНакопления(
		ОбъектМетаданных) Или UT_Common.ЭтоРегистрРасчета(ОбъектМетаданных)
		Или UT_Common.ЭтоРегистрБухгалтерии(ОбъектМетаданных) Тогда
		МассивТаблицРеквизитов.Добавить(ОбъектМетаданных.Измерения);
		МассивТаблицРеквизитов.Добавить(ОбъектМетаданных.Ресурсы);
	КонецЕсли;
	
	Если UT_Common.ЭтоЖурналДокументов(ОбъектМетаданных) Тогда
		МассивТаблицРеквизитов.Добавить(ОбъектМетаданных.Графы);
	Иначе	
		МассивТаблицРеквизитов.Добавить(ОбъектМетаданных.Реквизиты);
	КонецЕсли;
	Если UT_Common.ЭтоПланСчетов(ОбъектМетаданных) Тогда
...
Процедура ДобавитьВДеревоВидОбъекта(Картинка, ИмяТипаМетаданных, ИмяТипаОбъектов, Представление)
	...
	Для Каждого мдОбъекта Из Метаданные[ИмяТипаМетаданных] Цикл
		...
		ЭтоЖурналДокументов=UT_Common.ЭтоЖурналДокументов(мдОбъекта);
