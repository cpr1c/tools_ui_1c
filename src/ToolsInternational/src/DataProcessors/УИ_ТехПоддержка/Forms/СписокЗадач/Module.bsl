...
Процедура СписокЗадачВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	НачатьЗапускПриложения(UT_CommonClient.ПустоеОписаниеОповещенияДляЗапускаПриложения(), ДанныеСтроки.URL);
КонецПроцедуры
...
Процедура СоздатьНовуюЗадачу(Команда)
	...
	Если АдресЗадачи <> Неопределено Тогда
		UT_CommonClientServer.СообщитьПользователю("Задача успешно создана");
		НачатьЗапускПриложения(UT_CommonClient.ПустоеОписаниеОповещенияДляЗапускаПриложения(), АдресЗадачи);
	Иначе
		UT_CommonClientServer.СообщитьПользователю("Создание задачи не удалось");
	КонецЕсли;
