...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПереданныйСписокЭлементов = РеквизитФормыВЗначение("Объект").Container_RestoreValue(Параметры.Значение);
...
Функция ПолучитьВозвращаемоеЗначение()
	...
	Если ТипКонтейнера = 2 Тогда
		Возврат РеквизитФормыВЗначение("Объект").Container_SaveValue(СписокЭлементов.ВыгрузитьЗначения());
	КонецЕсли;
	
	Возврат РеквизитФормыВЗначение("Объект").Container_SaveValue(СписокЭлементов);
