...
Процедура Команда_ПолучитьКодДляПерехватаВОтладчике(Команда)
	...
	Если Объект.ExternalDataProcessorMode Тогда
		...
		//"ВнешниеОбработки.Создать(""" + стрИмяФайлаОбработкиНаСервере + """, Ложь).СохранитьЗапрос(" + Формат(Объект.SessionID, "ЧГ=0") + ", Запрос)";
	...
	Иначе
		...
		стрКод = СтрШаблон("Обработки.%1.Создать().СохранитьЗапрос(%2, Запрос)", Объект.DataProcessorName, Формат(
