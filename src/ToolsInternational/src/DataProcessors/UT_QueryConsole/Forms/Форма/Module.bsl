...
Функция ПоместитьОбработкуНаСервер(Адрес)
	...
	Файл = Новый Файл(Объект.DataProcessorFileName);
	СтрокаИмяФайлаОбработкиНаСервере = КаталогВременныхФайлов() + Файл.ИмяБезРасширения + "_" + Объект.DataProcessorVersion
		+ Файл.Расширение;
...
Процедура ПоказатьПредупреждениеКонсоли(ТекстПредупреждения) Экспорт
	ПоказатьПредупреждение( , ТекстПредупреждения, , Объект.Title);
КонецПроцедуры
...
Функция ПараметрыЗапросаФормаПриИзмененииСЗИзТЗ(Контейнер)
	...
	Возврат Обработка.Container_SaveValue(сзСписок);
...
Процедура УстановитьТипКолонокБезТипа(тзДанные)
	...
	Если маОбрабатываемыеКолонки.Количество() > 0 Тогда
		...
		Для й = 0 По маОбрабатываемыеКолонки.Количество() - 1 Цикл
			...
			Обработка.ChangeValueTableColumnType(тзДанные, ИмяКолонки, НовыйТипКолонки);
...
Функция ЗагрузитьВременныеТаблицы()
	...
	Для Каждого СтрокаВременнаяТаблица Из ВременныеТаблицы Цикл
		...
		тзДанные = РеквизитФормыВЗначение("Объект").Container_RestoreValue(СтрокаВременнаяТаблица.Контейнер);
...
Процедура ВыборкуВДерево(выбВыборка, Узел, й, фЕстьКонтейнеры, Обработка, фЕстьМакроколонки, стМакроколонки)
	...
	Пока выбВыборка.Следующий() Цикл
		...
		Если фЕстьКонтейнеры Тогда
			Обработка.AddContainers(РезультатЗапросаСтрока, выбВыборка, РезультатЗапросаКолонкиКонтейнера);
		КонецЕсли;
...
Функция ИзвлечьРезультатКакКонтейнер(фУдалитьТипNull = Истина)
	...
	Возврат Обработка.Container_SaveValue(тз);
		...
		Если ТипЗнч(ЭлементСтруктуры.Тип) = Тип("ОписаниеТипов") Тогда
			...
			Для Каждого кз Из соСчетчики Цикл
				...
				Если кз.Значение = маТипы.Количество() Тогда//Добавляем только те реквизиты, которые есть во всех типах составного типа.
					...
					НовыйЭлементСтруктуры.Картинка = Обработка.GetPictureByType(НовыйЭлементСтруктуры.Тип,
						Картинки);
...
Функция ИзвлечьРезультатВДанныеФормы(РезультатВПакете)
	...
	Если Число(РезультатВПакете) <= 0 Тогда
		РеквизитФормыВЗначение("Объект").CreateTableAttributesByColumns(ЭтаФорма, ИмяРеквизитаРезультата,
		...
		Иначе
			...
			Пока выбЗапрос.Следующий() Цикл
				...
				Если ЕстьКонтейнеры Тогда
					Обработка.AddContainers(РезультатЗапросаСтрока, выбЗапрос, РезультатЗапросаКолонкиКонтейнера);
				КонецЕсли;
...
&НаКлиенте
Процедура SavedStates_Save(ИмяЗначения, Значение) Экспорт

	Если Не ЗначениеЗаполнено(Объект.SavedStates) Тогда
		Объект.SavedStates = Новый Структура;
	КонецЕсли;

	Объект.SavedStates.Вставить(ИмяЗначения, Значение);

КонецПроцедуры

&НаКлиенте
Функция SavedStates_Get(ИмяЗначения, ЗначениеПоУмолчанию) Экспорт
	Перем Значение;

	Если Не ЗначениеЗаполнено(Объект.SavedStates) Или Не Объект.SavedStates.Свойство(ИмяЗначения,
		Значение) Тогда
...
Функция РазобратьВыражениеМакроКолонки(СтрокаМакроВыражения)
	...
	Если маЪ.Количество() > 1 Тогда
		...
		Если ТипЗначения <> Неопределено Тогда
			стМакроколонка = Новый Структура("Тип, ТипЗначения, SourceColumn", стрТипМакро, ТипЗначения,
				стрSourceColumn);
		КонецЕсли;
...
Функция ПолучитьМакроколонки(ЗапросСхемы)
	...
	Если Не Объект.OptionProcessing__ Тогда
...
Функция ВыполнитьПакет(Запрос, СхемаЗапроса)
	...
	Для Каждого ЗапросСхемы Из СхемаЗапроса.ПакетЗапросов Цикл
		...
		Если ТипЗнч(ЗапросСхемы) = Тип("ЗапросВыбораСхемыЗапроса") Тогда
			...
			Если Объект.TechLogEnabled Тогда
...
Процедура УстановитьПараметрыМакроколонокЗапроса(Запрос)
	...
	Если Объект.OptionProcessing__ Тогда
...
Функция ВыполнитьЗапросНаСервере(ТекстЗапроса)
	...
	Для Каждого СтрокаПараметра Из ПараметрыЗапроса Цикл
		...
		Если Объект.OptionProcessing__ И СтрНачинаетсяС(СтрокаПараметра.Имя, МакроПараметр) Тогда
			Продолжить;
		КонецЕсли;
...
Функция ПараметрыЗапроса_ПолучитьЗначение(СтрокаИд)
	...
	Если СтрокаПараметр.ТипКонтейнера = 0 Или СтрокаПараметр.ТипКонтейнера = 1 Или СтрокаПараметр.ТипКонтейнера = 2
		...
		Возврат РеквизитФормыВЗначение("Объект").Container_RestoreValue(СтрокаПараметр.Контейнер);
	Иначе
...
Процедура ПараметрыЗапроса_СохранитьЗначение(СтрокаИд, Знач Значение)
	...
	Если СтрокаПараметр.ТипКонтейнера = 0 Тогда
		СтрокаПараметр.Контейнер = РеквизитФормыВЗначение("Объект").Container_SaveValue(Значение);
...
Функция Контейнер12ВМассив(Контейнер)
	...
	Значение = РеквизитФормыВЗначение("Объект").Container_RestoreValue(Контейнер);
...
Процедура ПараметрыЗапроса_УстановитьТип(СтрокаИд, ТипКонтейнера, ТипЗначения)
	...
	Если ТипКонтейнера = 1 Или ТипКонтейнера = 2 Тогда
		...
		ИначеЕсли СтрокаПараметр.ТипКонтейнера = 3 Тогда
			...
			Таблица = РеквизитФормыВЗначение("Объект").Container_RestoreValue(Контейнер);
			КонтейнерМассив = Таблица.ВыгрузитьКолонку(0);
		ИначеЕсли СтрокаПараметр.ТипКонтейнера = 0 Тогда

			КонтейнерМассив = Новый Массив;

			КонтейнерМассив.Добавить(РеквизитФормыВЗначение("Объект").Container_RestoreValue(Контейнер));
		...
		Если ТипКонтейнера = 1 Тогда
			...
			СтрокаПараметр.Контейнер = РеквизитФормыВЗначение("Объект").Container_SaveValue(НовыйСписок);
		ИначеЕсли ТипКонтейнера = 2 Тогда
			НовыйСписок = Новый СписокЗначений;
			НовыйСписок.ЗагрузитьЗначения(КонтейнерМассив);
			НовыйСписок.ТипЗначения = ТипЗначения;
			КонтейнерМассив = НовыйСписок.ВыгрузитьЗначения();
			СтрокаПараметр.Контейнер = РеквизитФормыВЗначение("Объект").Container_SaveValue(КонтейнерМассив);
		КонецЕсли;

	ИначеЕсли ТипКонтейнера = 3 Тогда

		Если СтрокаПараметр.ТипКонтейнера = 1 Или СтрокаПараметр.ТипКонтейнера = 2 Тогда
			Таблица = РеквизитФормыВЗначение("Объект").Container_RestoreValue(ТипЗначения);
			Значение = РеквизитФормыВЗначение("Объект").Container_RestoreValue(Контейнер);
			Если ТипЗнч(Значение) = Тип("СписокЗначений") Тогда
			...
			КонецЦикла;
			НовыйКонтейнер = РеквизитФормыВЗначение("Объект").Container_SaveValue(Таблица);
		ИначеЕсли СтрокаПараметр.ТипКонтейнера = 3 Тогда
	...
	ИначеЕсли ТипКонтейнера = 0 Тогда
		...
		Если СтрокаПараметр.ТипКонтейнера = 1 Или СтрокаПараметр.ТипКонтейнера = 2 Тогда
			...
			Если КонтейнерМассив.Количество() > 0 Тогда
				СтрокаПараметр.Контейнер = РеквизитФормыВЗначение("Объект").Container_SaveValue(
		...
		ИначеЕсли СтрокаПараметр.ТипКонтейнера = 0 Тогда
			СтрокаПараметр.Контейнер = РеквизитФормыВЗначение("Объект").Container_SaveValue(
				ТипЗначения.ПривестиЗначение(РеквизитФормыВЗначение("Объект").Container_RestoreValue(
...
Процедура СкопироватьДанныеКонтейнераТипа3(КонтейнерНовый, КонтейнерСтарый)
	...
	Обработка = РеквизитФормыВЗначение("Объект");
	ТаблицаНовая = Обработка.StringToValue(КонтейнерНовый.Значение);
	...
	КонтейнерНовый.Значение = Обработка.ValueToString(ТаблицаНовая);
	КонтейнерНовый.Представление = Обработка.Container_GetPresentation(КонтейнерНовый);
...
Функция ПараметрыЗапроса_ПолучитьКакСтроку()
	...
	Возврат РеквизитФормыВЗначение("Объект").ValueToString(тзПараметры);
...
Процедура ПакетЗапросов_Новый()
	...
	Объект.OptionProcessing__ = Истина;
	Объект.AlgorithmExecutionUpdateIntervalOption = 1000;
...
Процедура ПакетЗапросов_Сохранить(Оповещение, стрИмяФайла, фТолькоЗаголовок = Ложь)
	...
	SavedStates_Save("ВидРезультата", ВидРезультата);
	SavedStates_Save("ОграничениеВыводимыхСтрок", ОграничениеВыводимыхСтрок);
	SavedStates_Save("ОграничениеВыводимыхСтрокВкл", ОграничениеВыводимыхСтрокВкл);
	SavedStates_Save("ОграничениеВыводимыхСтрокПервые", ОграничениеВыводимыхСтрокПервые);
	SavedStates_Save("ОграничениеВыводимыхСтрокПервыеВкл", ОграничениеВыводимыхСтрокПервыеВкл);
	//сохраняемые состояния --
	...
	стОпции = Новый Структура("
		...
		Объект.OptionProcessing__, Объект.AlgorithmExecutionUpdateIntervalOption, ОпцияИнтервалАвтосохранения);
	...
	стДанныеДляЗаписи = Новый Структура("
		...
		Объект.SavedStates, ?(ТекущаяСтрокаПакета <> Неопределено, ТекущаяСтрокаПакета.Имя, Неопределено),
		стОпции);
...
Процедура ПакетЗапросов_ЗагрузитьЗавершение(ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(Объект.SavedStates) Тогда
		Если стЗагруженныеДанные.Версия >= 11 Тогда
			Объект.SavedStates = стЗагруженныеДанные.СохраняемыеСостояния;
		Иначе
			Объект.SavedStates = Новый Структура;
		КонецЕсли;
	КонецЕсли;
	...
	Если стЗагруженныеДанные.Версия >= 8 Тогда
		Объект.OptionProcessing__ = стОпции.OptionProcessing__;
	Иначе
		Объект.OptionProcessing__ = Истина;
	КонецЕсли;

	Если стЗагруженныеДанные.Версия >= 10 Тогда
		Объект.AlgorithmExecutionUpdateIntervalOption = стОпции.AlgorithmExecutionUpdateIntervalOption;
	Иначе
		Объект.AlgorithmExecutionUpdateIntervalOption = 1000;
	КонецЕсли;
...
Процедура ПриОткрытииЗавершениеНаСервере()
	...
	Попытка
		Если Обработка.TechnologicalLog_ConsoleLogExists() Тогда
...
Процедура ПриОткрытииЗавершение()
	...
	Если Не Объект.ExternalDataProcessorMode Тогда
		РазрешитьПерехват();
		РазрешитьВыполнениеВФоне();
	КонецЕсли;
...
#Область УИ_ПриОткрытии
	Если UT_CommonClientServer.ПолеHTMLПостроеноНаWebkit() Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.Title = "Консоль запросов 9000 v" + Объект.DataProcessorVersion;
...
#Область УИ_ПриСозданииНаСервере
	УИ_ВходитВСоставУниверсальныхИнструментов = ОбъектОбработки.DataProcessorIsPartOfUniversalTools();
	Если УИ_ВходитВСоставУниверсальныхИнструментов Тогда
		...
		UT_CodeEditorServer.СоздатьЭлементыРедактораКода(ЭтотОбъект, "Алгоритм", Элементы.ТекстАлгоритм);
	КонецЕсли;
#КонецОбласти
...
Функция ЗаполнитьИзЧтениеXML(ЧтениеXML)
	...
	Если СтруктураПараметров.Свойство("ВременныеТаблицы", маТаблицы) Тогда
		...
		Для Каждого стТаблица Из маТаблицы Цикл
			...
			СтрокаТаблицы.Контейнер = РеквизитФормыВЗначение("Объект").Container_SaveValue(тзТаблица);
			СтрокаТаблицы.Значение = СтрокаТаблицы.Контейнер.Представление;
...
Функция ПараметрыЗаполнитьИзЗапросаНаСервере()
	...
	Для Каждого Параметр Из НайденныеПараметры Цикл
		...
		Если Объект.OptionProcessing__ И СтрНачинаетсяС(Параметр.Имя, МакроПараметр) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура Команда_ПолучитьКодДляПерехватаВОтладчике(Команда)
	...
	Если Объект.ExternalDataProcessorMode Тогда

		стрИмяФайлаОбработкиНаСервере = ПолучитьИмяФайлаОбработкиНаСервере();
		//"ВнешниеОбработки.Создать(""" + стрИмяФайлаОбработкиНаСервере + """, Ложь).SaveQuery(" + Формат(Объект.SessionID, "ЧГ=0") + ", Запрос)";
		стрКод = СтрШаблон("ВнешниеОбработки.Создать(""%1"", Ложь).SaveQuery(%2, Запрос)",
			стрИмяФайлаОбработкиНаСервере, Формат(Объект.SessionID, "ЧГ=0"));
	Иначе

		стрКод = СтрШаблон("Обработки.%1.Создать().SaveQuery(%2, Запрос)", Объект.DataProcessorName, Формат(
			Объект.SessionID, "ЧГ=0"));
...
Процедура Команда_ДобавитьПерехваченныеЗапросы(Команда)
	...
	маФайлыЗапросов = ПолучитьСписокФайловНаСервереВоВременных("*." + Объект.LockedQueriesExtension);
...
Функция ЗапуститьОбработкуНаСервере(Алгоритм, фПострочно = Истина)
	...
	ПараметрыВыполнения.Добавить(Объект.AlgorithmExecutionUpdateIntervalOption);
	...
	Иначе
		ПараметрыМетода = Новый Структура("
										  ...
										  |DataProcessorName,
										  |ИмяМетода,
										  |ПараметрыВыполнения", Ложь, Неопределено, Объект.DataProcessorName,
			"ExecuteUserAlgorithm", ПараметрыВыполнения);
	КонецЕсли;
	...
	Задание = ФоновыеЗадания.Выполнить("ДлительныеОперации.ВыполнитьПроцедуруМодуляОбъектаОбработки",
		ПараметрыФоновогоЗадания, , Объект.Title);
...
Функция ПолучитьСостояниеФоновогоЗадания()
	...
	Для Каждого Сообщение Из СообщенияПользователю Цикл
		Если СтрНачинаетсяС(Сообщение.Текст, ФоновоеЗаданиеАдресРезультата) Тогда
			СостояниеИзСообщения = РеквизитФормыВЗначение("Объект").StringToValue(Прав(Сообщение.Текст, СтрДлина(
...
Процедура ОтобразитьСостояниеФоновогоЗадания() Экспорт
	...
	Если СостояниеЗадания.Состояние = 0 Тогда
		...
		ПодключитьОбработчикОжидания("ОтобразитьСостояниеФоновогоЗадания",
			Объект.AlgorithmExecutionUpdateIntervalOption / 1000 / 2, Истина);
...
Функция ВыполнитьАлгоритмПострочноСИндикацией()
	...
	Пока Истина Цикл
		...
		стРезультат = ВыполнитьАлгоритмНаСервереПострочно(АдресСостояния, АдресРезультатаЗапроса, РезультатВПакете,
			ТекущийТекстАлгоритма(), Объект.AlgorithmExecutionUpdateIntervalOption);
...
Процедура Команда_ОтобразитьСкрытьПакетРезультатаЗапроса(Команда)
	...
	Элементы.ПакетРезультатаЗапроса.Видимость = фВидимостьПакетаРезультатаЗапроса;
	Элементы.ГруппаРезультатВПакете.Видимость = Не фВидимостьПакетаРезультатаЗапроса;
	Объект.SavedStates.Вставить("ВидимостьПакетаРезультатаЗапроса", фВидимостьПакетаРезультатаЗапроса);
КонецПроцедуры
...
&НаСервере
Процедура TechnologicalLog_Disable()
	Обработка = РеквизитФормыВЗначение("Объект");
...
&НаСервере
Процедура TechnologicalLog_Enable()
	Обработка = РеквизитФормыВЗначение("Объект");
...
&НаСервере
Функция TechnologicalLog_Enabled()
	Обработка = РеквизитФормыВЗначение("Объект");
...
&НаСервере
Функция TechnologicalLog_Disabled()
	Обработка = РеквизитФормыВЗначение("Объект");
...
Процедура ТехнологическийЖурнал_ОжиданиеВключения() Экспорт
	...
	Если TechnologicalLog_Enabled() Тогда
...
Процедура ТехнологическийЖурнал_ИндикацияВключения(фВключен)
	Если фВключен Тогда
		...
		ТехнологическийЖурналВключенИРаботает = Объект.TechLogEnabled
...
Процедура ТехнологическийЖурнал_ОжиданиеВыключения() Экспорт
	...
	Если TechnologicalLog_Disabled() Тогда
...
Функция ПолучитьСписокФайловЖурнала()
	маЛоги = НайтиФайлы(Объект.TechLogFolder, "*.log", Истина);
	Возврат маЛоги;
КонецФункции
...
Процедура УстановитьТекстАлгоритма(НовыйТекст)
	Если УИ_ВходитВСоставУниверсальныхИнструментов Тогда
		UT_CodeEditorClient.УстановитьТекстРедактора(ЭтотОбъект, "Алгоритм", НовыйТекст);
	Иначе
...
Функция ТекущийТекстАлгоритма()
	Если УИ_ВходитВСоставУниверсальныхИнструментов Тогда
		Возврат UT_CodeEditorClient.ТекстКодаРедактора(ЭтотОбъект, "Алгоритм");
	Иначе
...
Функция ГраницыВыделенияАлгоритма()
	Если УИ_ВходитВСоставУниверсальныхИнструментов Тогда
		Возврат UT_CodeEditorClient.ГраницыВыделенияРедактора(ЭтотОбъект, "Алгоритм");
	Иначе
...
Процедура УИ_ЗаполнитьДаннымиОтладки()
	...
	Если Объект.SavedStates = Неопределено Тогда
		Объект.SavedStates = Новый Структура;
	КонецЕсли;
	...
	Если ДанныеОтладки.Свойство("Параметры") Тогда
		Для Каждого ТекПараметр Из ДанныеОтладки.Параметры Цикл
			...
			НовыйПараметр.Вставить("Контейнер", ОбработкаОбъект.Container_SaveValue(ТекПараметр.Значение));
...
Процедура УИ_ДобавитьКонтекстСтруктурыРезультатаАлгоритм()
	...
	Для Каждого ДоступнаяПеременная Из СтруктураЗаписиРезультата.ПолучитьЭлементы() Цикл
		...
		СтруктураПеременной.Вставить("ПодчиненныеСвойства", Новый Массив);
		
		Для Каждого ТекРеквизитПеременной ИЗ ДоступнаяПеременная.ПолучитьЭлементы() Цикл
			НовоеСвойство = Новый Структура;
			НовоеСвойство.Вставить("Имя", ТекРеквизитПеременной.Имя);
			НовоеСвойство.Вставить("Тип", ТекРеквизитПеременной.Тип);
			
			СтруктураПеременной.ПодчиненныеСвойства.Добавить(НовоеСвойство);
		КонецЦикла;
		
		СтруктураДополнительногоКонтекста.Вставить(ДоступнаяПеременная.Имя, СтруктураПеременной);
	КонецЦикла;
	
	UT_CodeEditorClient.ДобавитьКонтекстРедактораКода(ЭтотОбъект, "Алгоритм", СтруктураДополнительногоКонтекста);
