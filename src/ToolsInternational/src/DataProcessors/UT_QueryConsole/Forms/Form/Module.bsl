...
Function SetDataProcessorToServer(Address)
	...
	DataProcessorServerFileNameString = КаталогВременныхФайлов() + Файл.ИмяБезРасширения + "_" + Object.DataProcessorVersion
		+ Файл.Расширение;
	ДвоичныеДанные = ПолучитьИзВременногоХранилища(Address);
	ДвоичныеДанные.Записать(DataProcessorServerFileNameString);
...
Procedure РазрешитьПерехват()
	Элементы.QueryBatchHookingSubmenu.Доступность = True;
EndProcedure
...
Procedure ПоместитьОбработкуНаСерверПродолжение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	DataProcessorServerFileName = SetDataProcessorToServer(Адрес);
