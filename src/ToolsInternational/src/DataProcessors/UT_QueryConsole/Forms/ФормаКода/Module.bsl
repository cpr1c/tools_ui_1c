...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьЗначенияПараметров = РеквизитФормыВЗначение("Объект").SavedStates_Get("УстановитьЗначенияПараметров", Истина);
...
Процедура ПолучитьЛитералОписанияТипов(ОписаниеТиповЗначения, Литерал, КодСоздания)
	...
	Если маТипы.Количество() = 1 Тогда
		Литерал = СтрШаблон(
			...
			Обработка.GetTypeName(маТипы[0]),
			КодКвалификаторы);
		Возврат;
	КонецЕсли;
	...
	Для Каждого Т Из маТипы Цикл
		маСозданиеТипа.Добавить(СтрШаблон("	ТипыКолонки.Добавить(Тип(""%1""));", Обработка.GetTypeName(Т)));
	КонецЦикла;
...
Функция ПолучитьЛитералЗначения(Значение, ИмяПараметра = Неопределено)
	...
	ИначеЕсли ТипЗначения = Тип("Тип") Тогда
		Литерал = СтрШаблон("Тип(""%1"")", РеквизитФормыВЗначение("Объект").GetTypeName(Значение));
...
Процедура СоздатьКодСПараметрами()
	...
	тзПараметрыЗапроса = РеквизитФормыВЗначение("Объект").StringToValue(ПараметрыЗапроса);
