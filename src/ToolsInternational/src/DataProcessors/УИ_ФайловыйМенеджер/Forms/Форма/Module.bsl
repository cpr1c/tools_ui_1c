...
Процедура ПриОткрытии(Отказ)
	WindowsКлиент=UT_CommonClientServer.ЭтоWindows();
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	WindowsСервер=UT_CommonClientServer.ЭтоWindows();
...
Функция СоздатьКаталогНаСервере(ПолноеИмя)
	...
	Если Файл.Существует() Тогда
		UT_CommonClientServer.СообщитьПользователю("Такой каталог уже существует");
...
Функция ИндексКартинки(Знач Расширение, ЭтоКаталог)
	Если ЭтоКаталог Тогда
		Возврат 2;
	Иначе
		Возврат UT_CommonClientServer.ПолучитьИндексПиктограммыФайла(Расширение);
	КонецЕсли;
КонецФункции
