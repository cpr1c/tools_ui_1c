...
Процедура УдалитьВыбранныеНастройки(Команда)
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		UT_CommonClientServer.СообщитьПользователю(НСтр("ru = 'Не выбраны настройки для удаления'"), , , ,
			ЕстьОшибка);
	КонецЕсли;
...
Процедура СкопироватьВыбранныеНастройки(Команда)
	...
	НайденныеСтроки = ТаблицаНастроек.НайтиСтроки(Отбор);
	Если НайденныеСтроки.Количество() = 0 Тогда
		UT_CommonClientServer.СообщитьПользователю(НСтр("ru = 'Не выбраны настройки для копирования'"), , , ,
			ЕстьОшибка);
	КонецЕсли;
	НайденныеСтроки = Пользователи.НайтиСтроки(Отбор);
	Если НайденныеСтроки.Количество() = 0 Тогда
		UT_CommonClientServer.СообщитьПользователю(НСтр("ru = 'Не указаны пользователи (кому копировать)'"),
			, , , ЕстьОшибка);
	КонецЕсли;
