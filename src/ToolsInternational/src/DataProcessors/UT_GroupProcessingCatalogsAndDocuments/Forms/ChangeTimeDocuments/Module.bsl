...
Procedure ОбработатьОбъект(Reference, ПорядковыйНомерОбъекта, ПараметрыЗаписиОбъектов)
	...
	If ВидИзмененияВремени = "ВНачалоДня" Then
		Object.Date = BegOfDay(Reference.Date);
	ElsIf ВидИзмененияВремени = "ВКонецДня" Then
		Object.Date = EndOfDay(Reference.Date);
	Else
		Object.Date = BegOfDay(Reference.Date) + (ВведенноеВремя - '00010101');
	EndIf;
	
//	Object.Write(DocumentWriteMode.Record, DocumentPostingMode.Regular);
...
&AtClient
Procedure ВидИзмененияВремениПриИзменении(Item)
	УстановитьДоступность();
EndProcedure

&AtClient
Procedure УстановитьДоступность()
	Items.ВведенноеВремя.Visible=ВидИзмененияВремени = "ВвестиВручную";
EndProcedure
...
мНастройка.Insert("ВидИзмененияВремени", "ВКонецДня");
мНастройка.Insert("ВведенноеВремя");
