...
//	КонецЕсли;
	...
	Если Правило.Отбор <> Неопределено Тогда
		...
		UT_CommonClientServer.СкопироватьЭлементы(КомпоновщикНастроек.Настройки.Отбор, Правило.Отбор);
		УстановитьНастройкуСтруктурыВыводаРезультата(КомпоновщикНастроек.Настройки);

		UT_CommonClientServer.SetDCSParemetrValue(КомпоновщикНастроек, "ДатаНачала", ДатаНачала);
		UT_CommonClientServer.SetDCSParemetrValue(КомпоновщикНастроек, "ДатаОкончания",
			ДатаОкончания);
...
Функция SubsystemExists(ПолноеИмяПодсистемы) Экспорт
...
//	Если SubsystemExists("СтандартныеПодсистемы.БазоваяФункциональность") Тогда
