...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы._DisplayObjectsRights.Доступность = пЕстьПраваАдминистратора;
...
Процедура кПоказатьСвойстваОбъекта(Команда)
	Если Элементы.ГруппаСтраницы.ТекущаяСтраница.Имя = "СтрСтруктураХранения" Тогда
		...
		Если ТекДанные <> Неопределено Тогда
			...
			СтрукПараметры = Новый Структура("ПолноеИмя, ПутьКФормам, _StorageAddresses, ОписаниеПравДоступа",
				пПолноеИмя, ПутьКФормам, _StorageAddresses, мОписаниеПравДоступа);
	...
	Если ТекДанные <> Неопределено Тогда
		Если ТекДанные.ВидУзла = "ОбъектМД" Тогда
			...
			Иначе
				СтрукПараметры = Новый Структура("ПолноеИмя, ПутьКФормам, _StorageAddresses, ОписаниеПравДоступа",
					ТекДанные.ПолноеИмя, ПутьКФормам, _StorageAddresses, мОписаниеПравДоступа);
				СтрукПараметры.Вставить("НастройкиОбработки", вСформироватьСтруктуруНастроекФормыСвойствОбъекта());
		...
		ИначеЕсли ТекДанные.ВидУзла = "Конфигурация" Тогда
			СтрукПараметры = Новый Структура("ПолноеИмя, ПутьКФормам, _StorageAddresses, ОписаниеПравДоступа",
				"Конфигурация", ПутьКФормам, _StorageAddresses, мОписаниеПравДоступа);
			СтрукПараметры.Вставить("НастройкиОбработки", вСформироватьСтруктуруНастроекФормыСвойствОбъекта());
			ОткрытьФорму(ПутьКФормам + "ФормаСвойств", СтрукПараметры, , ТекДанные.ПолноеИмя, , , ,
				РежимОткрытияОкнаФормы.Независимый);
		КонецЕсли;
...
Процедура кОбновтьТаблицуНастроек(Команда)
	...
	Если ТекДанные <> Неопределено И ТекДанные.ВидУзла = "Х" Тогда
		...
		_NameOfSettingsManager = ТекДанные.Имя;
...
Процедура вОбновитьЗаголовкиНастройки()
	Элементы.ДекорацияНастройки.Заголовок = _NameOfSettingsManager + " (" + ТабНастройки.Количество() + " шт.)";
КонецПроцедуры
		...
		Если Не ПустаяСтрока(СтрукСвойства.Представление) Тогда
			...
			Иначе
				...
				Если СтрДЗ.Имя = "ПереключитьМонопольныйРежим" Тогда
					СтрДЗ.Представление = ?(_FormContext.МонопольныйРежим, "Отключить монопольный режим",
...
Процедура ТабРолиСДоступомВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекДанные <> Неопределено Тогда
		...
		СтрукПараметры = Новый Структура("ПолноеИмя, ПутьКФормам, _StorageAddresses, ОписаниеПравДоступа", пПолноеИмя,
			ПутьКФормам, _StorageAddresses, мОписаниеПравДоступа);
		СтрукПараметры.Вставить("НастройкиОбработки", вСформироватьСтруктуруНастроекФормыСвойствОбъекта());
		ОткрытьФорму(ПутьКФормам + "ФормаСвойств", СтрукПараметры, , пПолноеИмя, , , ,
			РежимОткрытияОкнаФормы.Независимый);
	КонецЕсли;
