...
Процедура вЗапуститьСеанс1С(ТипЗапуска)
	UT_CommonClient.ЗапуститьСеанс1С(ТипЗапуска, ИмяПользователя());
КонецПроцедуры
	...
	Если ТекДанные <> Неопределено Тогда
		Если ТекДанные.ВидУзла = "ОбъектМД" И Не вЭтоПрочаяКоманда(ТекДанные.ПолноеИмя) Тогда
			...
			Если НадоОбработать Тогда
				UT_CommonClient.ОткрытьДинамическийСписок(ТекДанные.ПолноеИмя);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗапускПодПользователемКонфигуратор(Команда)
	...
	UT_CommonClient.ЗапуститьСеанс1С(1, ТекДанные.Имя, Истина,
...
Процедура ЗапускПодПользователемТолстыйКлиентОбычноеПриложение(Команда)
	...
	UT_CommonClient.ЗапуститьСеанс1С(2, ТекДанные.Имя, Истина,
		ВремяОжиданияЗапускаПодПользователемПередВосстановлениемПароля);
КонецПроцедуры
...
Процедура ЗапускПодПользователемТолстыйКлиент(Команда)
	...
	UT_CommonClient.ЗапуститьСеанс1С(3, ТекДанные.Имя, Истина,
		ВремяОжиданияЗапускаПодПользователемПередВосстановлениемПароля);
КонецПроцедуры
...
Процедура ЗапускПодПользователемТонкийКлиент(Команда)
	...
	UT_CommonClient.ЗапуститьСеанс1С(4, ТекДанные.Имя, Истина,
		ВремяОжиданияЗапускаПодПользователемПередВосстановлениемПароля);
КонецПроцедуры
