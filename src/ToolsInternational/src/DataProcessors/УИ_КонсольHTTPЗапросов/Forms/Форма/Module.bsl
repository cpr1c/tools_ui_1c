...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	UT_Common.ФормаИнструментаПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка, Элементы.ГруппаКоманднаяПанельФормы);
...
Функция ПодготовленныйЗапросHTTP(СтруктураURL)
	...
	Если РедактированиеЗаголовковТаблицей Тогда
		...
		КонецЦикла;
	Иначе
		Заголовки = UT_CommonClientServer.ЗаголовкиHTTPЗапросаИзСтроки(ЗаголовкиСтрока);
	КонецЕсли;
...
Процедура ЗафиксироватьЛогЗапроса(АдресСервера, Протокол, HTTPЗапрос, HTTPОтвет, ДатаНачала, Длительность)
	...
	ЗаписьЛога.ЗаголовкиЗапроса = UT_CommonClientServer.ПолучитьСтрокуЗаголовковHTTP(HTTPЗапрос.Заголовки);
	...
	ЗаписьЛога.ЗаголовкиОтвета = UT_CommonClientServer.ПолучитьСтрокуЗаголовковHTTP(HTTPОтвет.Заголовки);
...
Процедура ЗаполнитьТаблицуЗаголовковПоСтроке(СтрокаЗаголовков)
	ЗаголовкиПоСтроке = UT_CommonClientServer.ЗаголовкиHTTPЗапросаИзСтроки(СтрокаЗаголовков);
...
Процедура УстановитьСтраницуРедактированияЗаголовковЗапроса()
	...
	Иначе
		ЗаголовкиСтрока = UT_CommonClientServer.ПолучитьСтрокуЗаголовковHTTP(ТаблицаЗаголовковЗапроса);
	КонецЕсли;
КонецПроцедуры
