...
Процедура УдалитьОставшиесяОбъектыВОднойТранзакции(ПараметрыВыполнения)
	...
	Для Каждого Препятствующий Из ПараметрыВыполнения.ПрепятствующиеУдалению Цикл
		...
		Если Препятствующий.ОбнаруженныйТип <> Тип("Строка") И Не UT_Common.IsRefTypeObject(
...
Функция СформироватьИсключающееПравило(ПараметрыВыполнения, ОбнаруженныйМетаданные)
	...
	Если ЭтоРегистрСведений Или Метаданные.РегистрыБухгалтерии.Содержит(ОбнаруженныйМетаданные) // IsAccountingRegister

		Или Метаданные.РегистрыНакопления.Содержит(ОбнаруженныйМетаданные) Тогда // IsAccumulationRegister
...
//			Если UT_Common.SubsystemExists("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
