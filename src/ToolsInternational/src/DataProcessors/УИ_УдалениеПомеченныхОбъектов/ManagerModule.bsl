...
Процедура УдалитьОставшиесяОбъектыВОднойТранзакции(ПараметрыВыполнения)
	...
	Для Каждого Препятствующий Из ПараметрыВыполнения.ПрепятствующиеУдалению Цикл
		...
		Если Препятствующий.ОбнаруженныйТип <> Тип("Строка") И Не UT_Common.ЭтоОбъектСсылочногоТипа(
...
Процедура ИнициализироватьПараметры(ПараметрыВыполнения)
	...
	Если Не ПараметрыВыполнения.Свойство("ВсеПомеченныеНаУдаление") Тогда
		...
		UT_CommonClientServer.ДополнитьМассив(ПараметрыВыполнения.ВсеПомеченныеНаУдаление,
	...
	Если Не ПараметрыВыполнения.Свойство("МодельСервиса") Тогда
		ПараметрыВыполнения.Вставить("МодельСервиса", UT_Common.РазделениеВключено());
...
//			КонецЕсли;
			...
			ПараметрыВыполнения.Вставить("ВОбластиДанных", UT_Common.ДоступноИспользованиеРазделенныхДанных());
...
Процедура ЗаписатьПредупреждение(Ссылка, ИнформацияОбОшибке)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Удаление помеченных'", UT_CommonClientServer.КодОсновногоЯзыка()),
...
Процедура ЗаписатьПричинуВРезультат(ПараметрыВыполнения, СтрокаТаблицы)
	...
	Если ОбнаруженныйИнформация.Ссылочный Тогда
		Причина.ОбнаруженныйПометкаУдаления = UT_Common.ЗначениеРеквизитаОбъекта(Причина.ОбнаруженныйСсылка,
			"ПометкаУдаления");
	Иначе
