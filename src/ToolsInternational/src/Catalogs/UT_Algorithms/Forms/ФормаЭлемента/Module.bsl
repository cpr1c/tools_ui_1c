...
Процедура ДобавитьПараметр(Команда)
	...
	ОткрытьФорму("Справочник.UT_Algorithms.Форма.ФормаПараметра", П, ЭтотОбъект);
КонецПроцедуры
...
//	Storage = Новый ХранилищеЗначения(Параметры);
//	Записать();
КонецПроцедуры
...
//СписокВыбора ОбъектМетаданных КомандныйИнтерфейс
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
				   ...
				   |ИЗ
				   |   Справочник.UT_Algorithms.Параметры КАК УИ_АлгоритмыПараметры";
...
Процедура ПослеполученияФайла(ПолученныеФайлы, ДопПараметры) Экспорт
	Если ТипЗнч(ПолученныеФайлы) = Тип("Массив") Тогда
		UT_CommonClientServer.MessageToUser("Файл " + ПолученныеФайлы[0].Имя + " записан");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПроверкаПараметровПрограммныйИнтерфейс(Команда)
	Object.Параметры.Очистить();
	...
	Для Каждого ЭлементСоответствия Из Заполнение Цикл
		НоваяСтрока = Object.Параметры.Добавить();
