...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Ключ.Пустая() Тогда
		...
		Для Каждого ЭлементСписка Из НастройкиТекстАлгоритма Цикл
			...
			Элементы.AlgorithmText[ЭлементСписка.Значение] = НастройкаТекстАлгоритма;
		КонецЦикла;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "ParameterChanged" Тогда
		Прочитать();
		ЗаполнитьТаблицуПараметров();
...
Процедура ГруппаПанельСтраницПриСменеСтраницы(Элемент, ТекущаяСтраница)
	Если Модифицированность И ТекущаяСтраница.Name <> "ГруппаКод" Тогда
		Записать();
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьПараметр(Команда)
	П = Новый Структура("Ключ", Object.Ссылка);
	ОткрытьФорму("Справочник.UT_Algorithms.Форма.ParameterForm", П, ЭтотОбъект);
КонецПроцедуры
...
Процедура ИзменитьЗначение(Команда)
	Если Элементы.ТаблицаПараметров.ТекущиеДанные <> Неопределено Тогда
		...
		П.Вставить("ParameterType", Элементы.ТаблицаПараметров.ТекущиеДанные.ОписаниеТипа);
		ОткрытьФорму("Справочник.UT_Algorithms.Форма.ParameterForm", П, ЭтотОбъект);
	КонецЕсли;
КонецПроцедуры
...
Процедура ВыполнитьПроцедуру(Команда)
	...
	Если Object.AtClient Тогда
		UT_CommonClient.ExecuteAlgorithm(Object.Ссылка, , Ошибка, СообщениеОбОшибке);
	Иначе
		UT_CommonServerCall.ExecuteAlgorithm(Object.Ссылка, , Ошибка, СообщениеОбОшибке);
	КонецЕсли;
	Если Ошибка Тогда
...
Процедура КонструкторЗапросаПоказать(Команда)
	...
	ВыделенныйТекст = Элементы.AlgorithmText.ВыделенныйТекст;
	ВесьТекст = Элементы.AlgorithmText.ТекстРедактирования;
...
Процедура ОткрытьЖурналРегистрации(Команда)
	...
	ПараметрыОткрытия.Вставить("Данные", Object.Ссылка);
...
Процедура ЗаполнитьТаблицуПараметров()
	...
	СтруктураПараметров = ВыбОбъект.Storage.Получить();
...
Процедура ИзменитьПараметр(НовыеДанные) Экспорт
	...
	Если ТипЗнч(НовыеДанные.ЗначениеПараметра) = Тип("Строка") Тогда
		Если Лев(НовыеДанные.ЗначениеПараметра, 1) = "{" Тогда
			...
			Иначе
				Если Object.ThrowException Тогда
	...
	Если ПозОшибки > 0 Тогда
		...
		Если ПозСкобкаЗакрылась > 0 Тогда
			...
			Попытка
				...
				мСтрок = СтрРазделить(Object.Текст, Символы.ПС, Истина);
				...
				Если ПозОшибки2 > 0 Тогда
					...
					КонецЦикла;
				КонецЕсли;
				Object.Текст = СтрСоединить(мСтрок, Символы.ПС);
			Исключение
				Возврат;
			КонецПопытки;
		КонецЕсли;
...
Процедура ПолучитьТекстЗапроса(Текст, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ПервыйВызовКонструктора Тогда
		...
		Пока Найти(ТекстЗапроса, "&") > 0 Цикл
			ПарметрЗапроса = UT_AlgorithmsClientServer.ПолучитьПервоеВхождениеСловоБезПрефикса(ТекстЗапроса, "&");
	...
	Если ПустаяСтрока(ДополнительныеПараметры.ВыделенныйТекст) Тогда
		Object.Текст = Object.Текст + ТекстВставки;
		Элементы.AlgorithmText.ОбновитьТекстРедактирования();
	Иначе
		Object.Текст = СтрЗаменить(ДополнительныеПараметры.ВесьТекст, ДополнительныеПараметры.ВыделенныйТекст,
			ТекстВставки);
		Элементы.AlgorithmText.ОбновитьТекстРедактирования();
	КонецЕсли;
...
//СписокВыбора ОбъектМетаданных КомандныйИнтерфейс
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
				   ...
				   |ИЗ
				   |   Справочник.UT_Algorithms.Parameters КАК УИ_АлгоритмыПараметры";
...
Процедура УстановитьВидимостьИДоступность()
	...
	Элементы.ГрСервер.Видимость=Не Object.AtClient;
КонецПроцедуры
...
Процедура ВнешнийФайлНачалоВыбораЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Иначе
		UT_CommonClientServer.MessageToUser("Нет файла");
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗакончитьПомещениеФайла(Результат, АдресХранилища, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Иначе
		UT_CommonClientServer.MessageToUser("Ошибка помещения файла в хранилище");
	КонецЕсли;
КонецПроцедуры
...
Процедура ВыборКаталогаЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	Если (ТипЗнч(ВыбранныеФайлы) = Тип("Массив") И ВыбранныеФайлы.Количество() > 0) Тогда
		...
		ОписаниеФайла.Name = Каталог + ПолучитьРазделительПути() + ИмяФайла + РасширениеФайла;
...
Процедура ПослеполученияФайла(ПолученныеФайлы, ДопПараметры) Экспорт
	Если ТипЗнч(ПолученныеФайлы) = Тип("Массив") Тогда
		UT_CommonClientServer.MessageToUser("Файл " + ПолученныеФайлы[0].Name + " записан");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПроверкаПараметровПрограммныйИнтерфейс(Команда)
	Object.Parameters.Очистить();
	...
	Пока Найти(КодАлгоритма, Префикс) > 0 Цикл
		Слово = UT_AlgorithmsClientServer.ПолучитьПервоеВхождениеСловоБезПрефикса(КодАлгоритма, Префикс, мИсключая);
	...
	Префикс = "$";
	Пока Найти(КодАлгоритма, Префикс) > 0 Цикл
		Слово = UT_AlgorithmsClientServer.ПолучитьПервоеВхождениеСловоБезПрефикса(Текст, Префикс, мИсключая);
	...
	АдресХранилища = UT_AlgorithmsClientServer.ПолучитьПараметры(Object.Ссылка, Истина);

	ХранимыеПараметры = ПолучитьИзВременногоХранилища(АдресХранилища);

	Для Каждого ЭлементСоответствия Из Заполнение Цикл
		НоваяСтрока = Object.Parameters.Добавить();
		НоваяСтрока.Entry = ЭлементСоответствия.Значение;
		НоваяСтрока.Name = ЭлементСоответствия.Ключ;
		Если НоваяСтрока.Entry И ХранимыеПараметры.Свойство(ЭлементСоответствия.Ключ) Тогда
			НоваяСтрока.ParameterType = ПолучитьСтрокуОписаниеТипа(ХранимыеПараметры[ЭлементСоответствия.Ключ]);
			НоваяСтрока.ByDefault = Строка(ХранимыеПараметры[ЭлементСоответствия.Ключ]);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура СоздатьРегламентноеЗадание()
	...
	МассивПараметров.Добавить(Object.Ссылка);
	Отбор = Новый Структура;
	Отбор.Вставить("Ключ", Object.Ссылка.УникальныйИдентификатор());
	...
	Иначе
		...
		Задание.Наименование = Object.Наименование;
		Задание.Ключ = Отбор.Ключ;
		Задание.Использование = Ложь;
		Задание.Parameters = МассивПараметров;
		Задание.Записать();
		Сообщить("Создано регламентное задание " + Object.Наименование + " с  ключом " + Отбор.Ключ);
	КонецЕсли;
КонецПроцедуры
...
Процедура УдалитьРегламентноеЗаданиеНаСервере()
	...
	Если МассивЗаданий.Количество() >= 1 Тогда
		...
		Сообщить("Удалено регламентное задание  " + Object.Наименование);
	КонецЕсли;
