...
Процедура ВыполнитьПроцедуру(Команда)
	...
	Если Объект.AtClient Тогда
		UT_CommonClient.ExecuteAlgorithm(Объект.Ссылка, , Ошибка, СообщениеОбОшибке);
	Иначе
		UT_CommonServerCall.ExecuteAlgorithm(Объект.Ссылка, , Ошибка, СообщениеОбОшибке);
	КонецЕсли;
	Если Ошибка Тогда
...
Процедура УстановитьВидимостьИДоступность()
	...
	Элементы.ГрСервер.Видимость=Не Объект.AtClient;
КонецПроцедуры
...
Процедура ПроверкаПараметровПрограммныйИнтерфейс(Команда)
	...
	АдресХранилища = UT_AlgorithmsClientServer.ПолучитьПараметры(Объект.Ссылка, Истина);
