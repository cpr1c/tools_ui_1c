...
Процедура ВыполнитьПроцедуру(Команда)
	...
	КонецЕсли;
	Если Ошибка Тогда
		UT_CommonClientServer.СообщитьПользователю(СообщениеОбОшибке);
...
Процедура ВнешнийФайлНачалоВыбораЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Иначе
		UT_CommonClientServer.СообщитьПользователю("Нет файла");
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗакончитьПомещениеФайла(Результат, АдресХранилища, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Иначе
		UT_CommonClientServer.СообщитьПользователю("Ошибка помещения файла в хранилище");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПослеполученияФайла(ПолученныеФайлы, ДопПараметры) Экспорт
	Если ТипЗнч(ПолученныеФайлы) = Тип("Массив") Тогда
		UT_CommonClientServer.СообщитьПользователю("Файл " + ПолученныеФайлы[0].Имя + " записан");
	КонецЕсли;
КонецПроцедуры
