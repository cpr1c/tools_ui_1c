...
Функция _От(ОбъектОтладки, НастройкиСКДИлиСоединениеHTTP = Неопределено, ВнешниеНаборыДанных = Неопределено) Экспорт
	Возврат UT_CommonClientServer.ОтладитьОбъект(ОбъектОтладки, НастройкиСКДИлиСоединениеHTTP,ВнешниеНаборыДанных);
КонецФункции
...
Функция _ВТ(ЗапросИЛИМенеджерВременныхТаблиц) Экспорт
	Если ТипЗнч(ЗапросИЛИМенеджерВременныхТаблиц) = Тип("МенеджерВременныхТаблиц") Тогда
		Возврат UT_CommonServerCall.СтруктураВременныхТаблицМенеджераВременныхТаблиц(
...
Функция _ТЗСр(ТаблицаБазовая, ТаблицаСравнения, СписокКолонок = Неопределено) Экспорт
	...
	Попытка
		Возврат UT_CommonServerCall.ВыполнитьСравнениеДвухТаблицЗначений(ТаблицаБазовая, ТаблицаСравнения,
			КолонкиДляСравнения);
	Исключение
