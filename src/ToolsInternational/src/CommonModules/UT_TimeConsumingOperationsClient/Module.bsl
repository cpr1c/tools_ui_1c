...
Функция АктивныеДлительныеОперации() Экспорт
	...
	ИмяПараметра = "UT_UniversalTools.АктивныеДлительныеОперации";
...
Процедура ПроверитьПараметрыОжидатьЗавершение(Знач ДлительнаяОперация, Знач ОповещениеОЗавершении,
	...
	UT_CommonClientServer.ПроверитьПараметр("УИ_ДлительныеОперацииКлиент.ОжидатьЗавершение",
	...
	Если ПараметрыОжидания <> Неопределено Тогда
		...
		Если ПараметрыОжидания.ФормаВладелец <> Неопределено Тогда
			ТипыСвойств.Вставить("ФормаВладелец", UT_CommonClientServer.ManagedFormType());
		КонецЕсли;
		...
		UT_CommonClientServer.Validate(ПараметрыОжидания.Интервал = 0 Или ПараметрыОжидания.Интервал >= 1,
			НСтр("ru = 'Параметр ПараметрыОжидания.Интервал должен быть больше или равен 1'"),
			"УИ_ДлительныеОперацииКлиент.ОжидатьЗавершение");
		UT_CommonClientServer.Validate(Не (ПараметрыОжидания.ОповещениеОПрогрессеВыполнения <> Неопределено
...
//
// 
Процедура UpdateIdleHandlerParameters(IdleHandlerParameters) Экспорт

	IdleHandlerParameters.CurrentInterval = IdleHandlerParameters.CurrentInterval
		* IdleHandlerParameters.КоэффициентУвеличенияИнтервала;
	Если IdleHandlerParameters.CurrentInterval > IdleHandlerParameters.МаксимальныйИнтервал Тогда
		IdleHandlerParameters.CurrentInterval = IdleHandlerParameters.МаксимальныйИнтервал;
	КонецЕсли;
...
// 
// Параметры:
//  IdleHandlerParameters - Структура - заполняется значениями по умолчанию. 
//
// 
Процедура InitializeIdleHandlerParameters(IdleHandlerParameters) Экспорт

	IdleHandlerParameters = Новый Структура;
	IdleHandlerParameters.Вставить("МинимальныйИнтервал", 1);
	IdleHandlerParameters.Вставить("МаксимальныйИнтервал", 15);
	IdleHandlerParameters.Вставить("ТекущийИнтервал", 1);
	IdleHandlerParameters.Вставить("КоэффициентУвеличенияИнтервала", 1.4);
