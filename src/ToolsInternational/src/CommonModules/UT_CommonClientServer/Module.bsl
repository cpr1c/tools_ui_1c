...
#Иначе
		Возврат UT_CommonCached.КодОсновногоЯзыка();
#КонецЕсли
КонецФункции
...
Функция ВерсияПлатформыНеМладше(ВерсияДляСравнения) Экспорт
	ВерсияБезСборки=ConfigurationVersionWithOutReleaseSubnumber(CurrentAppVersion());
...
Функция СериализоватьЗапросДляОтладки(ОбъектОтладки)
	...
	Если ОбъектОтладки.МенеджерВременныхТаблиц <> Неопределено Тогда
		СтруктураВременныхТаблиц = UT_CommonServerCall.СтруктураВременныхТаблицМенеджераВременныхТаблиц(
...
Функция СериализоватьСКДДляОтладки(СКД, НастройкиСКД, ВнешниеНаборыДанных)
	Возврат UT_CommonServerCall.СериализоватьОбъектСКДДляОтладки(СКД, НастройкиСКД, ВнешниеНаборыДанных);
КонецФункции
...
Функция СериализоватьHTTPЗапросДляОтладки(ЗапросHTTP, СоединениеHTTP)
	...
	СтруктураОбъекта.Вставить("Заголовки", UT_CommonClientServer.ПолучитьСтрокуЗаголовковHTTP(
...
#КонецЕсли
		Возврат Неопределено;
	Иначе
		Возврат UT_CommonServerCall.ЗаписатьДанныеДляОтладкиВСправочник(ТипОбъектаОтладки,
...
#Если ВебКлиент Тогда
	Возврат UT_CommonServerCall.мПрочитатьJSON(Значение);
#Иначе
...
#Если ВебКлиент Тогда
	Возврат UT_CommonServerCall.мЗаписатьJSON(СтруктураДанных);
#Иначе
...
Function IsWindows() Export
	...
	Возврат SystemInfo.PlatformType = PlatformType.Windows_x86 Или SystemInfo.PlatformType
...
Function PlatformVersionNotLess(ComparingVersion) Export
	VersionWithOutBuild=ConfigurationVersionWithOutReleaseSubnumber(CurrentAppVersion());
