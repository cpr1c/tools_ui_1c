...
Функция ПодсистемаСуществует(ПолноеИмяПодсистемы) Экспорт
	...
	ИменаПодсистем = UT_CommonCached.ИменаПодсистем();
...
Процедура ПроверитьИмяПроцедурыКонфигурации(Знач ИмяПроцедуры)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Безопасное выполнение метода'",
			UT_CommonClientServer.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , ,
...
Функция СократитьСтрокуКонтрольнойСуммой(Строка, МаксимальнаяДлина) Экспорт
	UT_CommonClientServer.Проверить(МаксимальнаяДлина >= 32, НСтр(
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты, ВыбратьРазрешенные = Ложь) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("Строка") Тогда
		...
		Попытка
			Ссылка = UT_CommonClientServer.ПредопределенныйЭлемент(ПолноеИмяПредопределенногоЭлемента);
		Исключение
...
//	КонецЕсли;
	...
	UT_CommonClientServer.ДополнитьМассив(ИсключенияПоиска, ИсключенияПоискаИнтеграция);
	...
	Если ПодключитьВнешнююКомпоненту(Местоположение, СимволическоеИмя) Тогда
		...
		Если ПодключаемыйМодуль = Неопределено Тогда
			...
			ЗаписьЖурналаРегистрации(
				...
				UT_CommonClientServer.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , ТекстОшибки);
...
Функция ОписаниеТипаВсеСсылки() Экспорт
	...
	Возврат UT_CommonCached.ОписаниеТипаВсеСсылки();
...
Процедура ДобавитьМассивОбъектовКСравнению(Объекты) Экспорт
	...
	UT_Common.ХранилищеСистемныхНастроекСохранить(
		UT_CommonClientServer.КлючОбъектаВХранилищеНастроек(), КлючНастроекОбъектовКСравнению(),
		МассивОбъектовКСравнению);

КонецПроцедуры

Функция ОбъектыДобавленныеКСравнению() Экспорт
	КлючОбъекта=UT_CommonClientServer.КлючОбъектаВХранилищеНастроек();
...
Процедура ОчиститьОбъектыДобавленныеКСравнению() Экспорт
	UT_Common.ХранилищеСистемныхНастроекСохранить(
		UT_CommonClientServer.КлючОбъектаВХранилищеНастроек(), КлючНастроекОбъектовКСравнению(), Новый Массив);
КонецПроцедуры
...
Функция ЗаменитьСсылки(Знач ПарыЗамен, Знач Параметры = Неопределено) Экспорт
	...
	ПараметрыЗаписи=UT_CommonClientServer.СтруктураПараметровЗаписиПоУмолчанию();
	
	// Переданные значения.
	ЗначениеПараметра = UT_CommonClientServer.СвойствоСтруктуры(Параметры, "СпособУдаления");
	...
	ЗначениеПараметра = UT_CommonClientServer.СвойствоСтруктуры(Параметры, "ЗаменаПарыВТранзакции");
	Если ТипЗнч(ЗначениеПараметра) = Тип("Булево") Тогда
		ЗаменаПарыВТранзакции = ЗначениеПараметра;
	КонецЕсли;

	ЗначениеПараметра = UT_CommonClientServer.СвойствоСтруктуры(Параметры, "УчитыватьПрикладныеПравила");
	Если ТипЗнч(ЗначениеПараметра) = Тип("Булево") Тогда
		ПараметрыВыполнения.УчитыватьПрикладныеПравила = ЗначениеПараметра;
	КонецЕсли;

	ЗначениеПараметра = UT_CommonClientServer.СвойствоСтруктуры(Параметры, "ПараметрыЗаписи");
	...
	Попытка
		...
		Попытка
			Если ПараметрыВыполнения.ПараметрыЗаписи.ЗаписьВРежимеЗагрузки Тогда
				...
				НовыеПараметрыВыполнения = UT_CommonClientServer.СкопироватьРекурсивно(ПараметрыВыполнения);
...
Процедура ЗарегистрироватьФактЗамены(Объект, СсылкаДубля, СсылкаОригинала, ВидРеквизита, ИмяРеквизита,
	...
	ВыполненныеЗамены = UT_CommonClientServer.СвойствоСтруктуры(ДопСвойства, "ВыполненныеЗамены");
...
Функция СобытиеЖурналаРегистрацииЗаменыСсылок()
	...
	Возврат НСтр("ru='Поиск и удаление ссылок'", UT_CommonClientServer.КодОсновногоЯзыка());
...
Функция ИнформацияОТипе(ПолноеИмяИлиМетаданныеИлиТип, Кэш)
	...
	ИнформацияОТипах = UT_CommonClientServer.СвойствоСтруктуры(Кэш, "ИнформацияОТипах");
...
Функция ВыполнитьПроцедуруПередЗаписьюОбъекта(Объект, _ТекстПроцедурыПередЗаписью)
	...
	Исключение
		UT_CommonClientServer.СообщитьПользователю(СтрШаблон("Объект: %1. Ошибка при выполнении процедуры ПередЗаписью:
...
Функция ЗаписатьОбъектВБазу(Объект, ПараметрыЗаписи, Знач Действие = "Запись", Знач РежимЗаписи = Неопределено,
	...
	Исключение
		UT_CommonClientServer.СообщитьПользователю(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Результат=Ложь;
	КонецПопытки;
