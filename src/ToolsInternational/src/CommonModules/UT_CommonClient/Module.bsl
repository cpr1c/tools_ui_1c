...
Процедура OnStart() Экспорт
	...
	Если ПараметрыСтартаСеанса.ExtensionRightsAdded Тогда
		ЗавершитьРаботуСистемы(Ложь, Истина);
	КонецЕсли;

	UT_ApplicationParameters.Вставить("SessionNumber", ПараметрыСтартаСеанса.SessionNumber);
	UT_ApplicationParameters.Вставить("ConfigurationScriptVariant", ПараметрыСтартаСеанса.ConfigurationScriptVariant);
...
Процедура EditRecordingParameters(Форма) Экспорт
	ПараметрыФормы = Новый Структура;
...
#Иначе
		...
		StoredIBUserPasswordData = Неопределено;
		...
		ДополнительныеПараметрыОповещения.Вставить("StoredIBUserPasswordData",
...
Процедура ЗапуститьСеанс1СЗавершениеЗапуска(КодВозврата, ДополнительныеПараметры) Экспорт
	...
	UT_CommonServerCall.RestoreUserDataAfterUserSessionStart(
		ДополнительныеПараметры.Пользователь, ДополнительныеПараметры.StoredIBUserPasswordData);
КонецПроцедуры
