...
Процедура ВызватьИсключениеЕслиНетПраваАдминистрирования() Экспорт
	...
	Если UT_Common.РазделениеВключено() И UT_Common.ДоступноИспользованиеРазделенныхДанных() Тогда
...
Процедура ОбновленнаяТаблицаРегламентныхЗаданий(Параметры, АдресХранилища) Экспорт
	...
	ПодсистемаРаботаВМоделиСервисаСуществует = UT_Common.ПодсистемаСуществует(
	...
	Для Каждого ОбъектМетаданных Из Метаданные.РегламентныеЗадания Цикл
		Если Не РегламентноеЗаданиеДоступноПоФункциональнымОпциям(ОбъектМетаданных, ПараметрыРегламентныхЗаданий) Тогда
			...
			Продолжить;
		КонецЕсли;
		Если Не UT_Common.РазделениеВключено() И ПодсистемаРаботаВМоделиСервисаСуществует Тогда
	...
	Если ИдентификаторРегламентногоЗадания = Неопределено Тогда
		...
		Для Каждого Задание Из ТекущиеЗадания Цикл
			Если Не UT_Common.РазделениеВключено() И ЗаданияВМоделиСервиса[Задание.Метаданные.Имя]
				<> Неопределено Тогда
...
Функция РегламентноеЗаданиеДоступноПоФункциональнымОпциям(Задание, ЗависимостиЗаданий = Неопределено) Экспорт
	...
	ЭтоПодчиненныйУзелРИБ        = UT_Common.ЭтоПодчиненныйУзелРИБ();
	ЭтоРазделенныйРежим          = UT_Common.РазделениеВключено();
	ЭтоАвтономноеРабочееМесто 	 = UT_Common.ЭтоАвтономноеРабочееМесто();
