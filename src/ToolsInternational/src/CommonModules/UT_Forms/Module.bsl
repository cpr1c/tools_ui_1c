...
//@skip-warning
	...
	ИначеЕсли РодительКнопки = Форма.КоманднаяПанель Тогда
		Возврат Истина;
	ИначеЕсли ТипЗнч(РодительКнопки) = UT_CommonClientServer.ТипУправляемойФормы() Тогда
		Возврат Ложь;
	Иначе
...
//@skip-warning
	...
	ИначеЕсли ТипЗнч(РодительЭлемента) = UT_CommonClientServer.ТипУправляемойФормы() Тогда
		Возврат "";
	Иначе
