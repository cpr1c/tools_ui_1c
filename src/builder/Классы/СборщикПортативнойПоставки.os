...
Процедура СобратьБинарныеФайлыПроекта(ОписаниеКонфигурации, КаталогИсходниковПоставки, КаталогПортативнойСборки)
	...
	Для Каждого ТекМодуль Из ОписаниеКонфигурации.ОбщиеМодули Цикл
		Попытка
			СформироватьБинарныйФайл(Конфигуратор, ОбъединитьПути(КаталогИсходниковОбработокОбщихМодулей, ТекМодуль.Имя + ".xml"), ОбъединитьПути(КаталогОбщихМодулейСборки, ТекМодуль.Имя + ".erf"));
		Исключение
		КонецПопытки;
	КонецЦикла;
...
Процедура СконвертироватьМодулиПортативнойПоставки(ОписаниеКонфигурации, 
	...
	Для Каждого Обработка Из ОписаниеКонфигурации.Обработки Цикл
		Для Каждого ИмяФормы Из Обработка.Формы Цикл
			...
			СконвертироватьМодульФормыОбработки(ИмяФайлаФормы,ИмяФормы,ОписаниеКонфигурации);
		КонецЦикла;
...
Процедура СконвертироватьМодульФормыОбработки(ИмяФайлаМодуля, ИмяФормы, ОписаниеКонфигурации)
	...
	Для Каждого Модуль Из ОписаниеКонфигурации.ОбщиеМодули Цикл
		Если  Модуль.ВызовСервера
			...
			СтруктураПереименованийПеременныхМодулей.Вставить(Модуль.Имя, ПрефиксПеременныхИПроцедур+"КлиентскийОбщийМодульПоИмени("""+Модуль.Имя+""")");
		Иначе
			СтруктураПереименованийПеременныхМодулей.Вставить(Модуль.Имя, ПрефиксПеременныхИПроцедур+"СерверныйОбщийМодульПоИмени("""+Модуль.Имя+""")");
