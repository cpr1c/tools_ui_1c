...
Процедура ПреобразоватьПодчиненныеЭлементыФормы(ЭлементыФормы, ОписанияКомандФормы, ПапкаФормы, ЕстьИзменения)
	...
	Для Каждого ОписаниеЭлемента Из ЭлементыФормы._Элементы Цикл
		...
		Если Не ЕстьКартинка 
			И УзелОписания.Ключ="Button" Тогда

			ИмяКоманды=УзелОписания.Значение._Элементы["CommandName"]._Значение;
			...
			Если МассивИменаКоманды.Количество()=3 Тогда
				Если МассивИменаКоманды[0]="Form"
					...
					ОписаниеКоманды=ОписанияКомандФормы[НРег(МассивИменаКоманды[2])];
		...
		ПреобразоватьПодчиненныеЭлементыФормы(УзелОписания.Значение._Элементы["ChildItems"], ОписанияКомандФормы, ПапкаФормы,ЕстьИзменения);
	КонецЦикла;
КонецПроцедуры
...
Процедура ПреобразоватьОписаниеФормыДляПереводаВоВнешнююОбработку(КаталогИсходниковОбработки, ИмяИсходнойОбработки, ИмяФормы,ЭтоОтчет)
	...
	ОписанияКомандФормы=Новый Соответствие;
	...
	Если КомандыФормы<>Неопределено Тогда
		Для Каждого ТекКоманда Из КомандыФормы._Элементы Цикл
			...
			ОписанияКомандФормы.Вставить(НРег(ОписаниеКоманды.Имя),ОписаниеКоманды);
		КонецЦикла;
	КонецЕсли;


	// преобразуем элементы формы.
	ПреобразоватьПодчиненныеЭлементыФормы(ЭлементыXMLФорма._Элементы["ChildItems"],ОписанияКомандФормы, ПапкаФормы, БылиИзменения);

	КоманднаяПанельФормыXML=ЭлементыXMLФорма._Элементы["AutoCommandBar"];

	Если КоманднаяПанельФормыXML<>Неопределено Тогда
		ПреобразоватьПодчиненныеЭлементыФормы(КоманднаяПанельФормыXML._Элементы["ChildItems"],ОписанияКомандФормы, ПапкаФормы, БылиИзменения);
	КонецЕсли;
...
Процедура СобратьБинарныеФайлыПроекта(КаталогИсходниковПоставки, КаталогПортативнойСборки)
	...
	Для Каждого ТекОбработка ИЗ ОписаниеРасширения.Обработки Цикл
		...
		Если ТекОбработка.МодульМенеджера Тогда
			СформироватьБинарныйФайл(Конфигуратор, ОбъединитьПути(КаталогИсходниковОбработокОбщихМодулей, ТекОбработка.Имя + "_МодульМенеджера.xml"), ОбъединитьПути(КаталогОбщихМодулейСборки, ТекОбработка.Имя + "_МодульМенеджера.epf"));
		КонецЕсли;
	КонецЦикла;
	...
	ДополнитьГенерируемымКодомМодульФормыГлавнойОбработки(УспешноСобранныеИнструменты, КаталогИсходниковГлавнойОбработки);
...
Процедура ДополнитьГенерируемымКодомМодульФормыГлавнойОбработки(УспешноСобранныеИнструменты, КаталогИсходниковГлавнойОбработки)
	...
	ПараметрыПлагинУстановкаТекстаПроцедурыПолученияОписанияПодключенныхМодулейПортативнойПоставки.Вставить("ОписаниеКонфигурации", ОписаниеРасширения);
	...
	СконвертироватьМодуль(ИмяФайлаМодуля,"Форма", Истина, Истина);
...
Процедура СконвертироватьМодулиПортативнойПоставки(КаталогИсходниковОбработокОбщихМодулей, КаталогИсходниковПоставки, КаталогИсходниковИнструментов)

	Для Каждого Обработка Из ОписаниеРасширения.Обработки Цикл
		Для Каждого ИмяФормы Из Обработка.Формы Цикл
			...
			СконвертироватьМодуль(ИмяФайлаФормы,ИмяФормы,Истина);
		КонецЦикла;

		ИмяФайлаМодуля = ОбъединитьПути(КаталогИсходниковИнструментов, Обработка.Имя, "Ext", "ObjectModule.bsl");
		СконвертироватьМодуль(ИмяФайлаМодуля, Обработка.Имя, Ложь);

		Если Обработка.МодульМенеджера Тогда
			ИмяФайлаМодуля = ОбъединитьПути(КаталогИсходниковОбработокОбщихМодулей, Обработка.Имя+"_МодульМенеджера", "Ext", "ObjectModule.bsl");
			
			СконвертироватьМодуль(ИмяФайлаМодуля, Обработка.Имя, Ложь);
		КонецЕсли;
	КонецЦикла;

	Для Каждого Обработка Из ОписаниеРасширения.Отчеты Цикл
		Для Каждого ИмяФормы Из Обработка.Формы Цикл
			ИмяФайлаФормы=ОбъединитьПути(ОбъединитьПути(КаталогИсходниковИнструментов,Обработка.Имя,"Forms", ИмяФормы),"Ext","Form","Module.bsl");
			СконвертироватьМодуль(ИмяФайлаФормы,ИмяФормы, Истина);
		КонецЦикла;
	...
	Для Каждого ТекМодуль Из ОписаниеРасширения.ОбщиеМодули Цикл
		...
		Если ТекМодуль.Клиент Тогда
			...
			СконвертироватьМодуль(ИмяФайлаФормы, ИмяФормы, Истина,,Истина);
		...
		Если ТекМодуль.Сервер Тогда
			...
			СконвертироватьМодуль(ИмяФайлаМодуля, ТекМодуль.Имя, Ложь);
	...
	Для Каждого ТекФорма Из ОписаниеРасширения.ОбщиеФормы Цикл
		...
		СконвертироватьМодуль(ИмяФайлаФормы,ТекФорма.Имя, Истина);
	КонецЦикла;
КонецПроцедуры

Процедура СконвертироватьМодуль(ИмяФайлаМодуля, ИмяФормы, ЭтоМодульФормы, ЭтоГлавнаяОбработка=Ложь, ЭтоОбщийМодуль=Ложь)
	Если Не ФС.ФайлСуществует(ИмяФайлаМодуля) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Модуль Из ОписаниеРасширения.ОбщиеМодули Цикл
	...
	ПараметрыДобавленияОбщихМодулей = Новый Структура;
	ПараметрыДобавленияОбщихМодулей.Вставить("МассивОбщихМодулей", ОписаниеРасширения.ОбщиеМодули);
	ПараметрыДобавленияОбщихМодулей.Вставить("ЭтоМодульФормы", ЭтоМодульФормы);
	ПараметрыДобавленияОбщихМодулей.Вставить("ПрефиксПеременныхИПроцедур", ПрефиксПеременныхИПроцедур);
...
Процедура ДобавитьВГлавнуюОбработкуОбщиеМакеты(УзелОбработки, КаталогИсходниковДляСборки)
...
Процедура ДобавитьВГлавнуюОбработкуОбщиеФормы(УзелОбработки, КаталогИсходниковДляСборки)
	ПодчиненныеОбъектыОбработки = УзелОбработки._Элементы["ChildObjects"];
...
Процедура СобратьГлавнуюОбработку(КаталогИсходныхФайловРасширения, КаталогИсходниковПоставки, КаталогПортативнойСборки)
	...
	ДобавитьВГлавнуюОбработкуОбщиеМакеты(УзелОбработки,КаталогИсходниковДляСборки);
	ДобавитьВГлавнуюОбработкуОбщиеФормы(УзелОбработки,КаталогИсходниковДляСборки);
...
Функция СинонимОбработки(ОписаниеОбработки, ЭтоОтчет)
	Синоним="";

	УзелОбработки=ОписаниеОбработки["MetaDataObject"]._Элементы["External"+?(ЭтоОтчет,"Report", "DataProcessor")]._Элементы["Properties"];

	УзелСинонима=УзелОбработки["Synonym"];
	...
	Если ТипЗнч(УзелСинонима)=Тип("Соответствие") Тогда
		УзелЭлементаСинонима=УзелСинонима["v8:item"];
	Иначе
		УзелЭлементаСинонима=Неопределено;
		Для Каждого Уз Из УзелСинонима Цикл
			УзелЭлемента=Уз["v8:item"];
			Если УзелЭлемента["v8:lang"]=Язык Тогда
	...
	Синоним=УзелЭлементаСинонима["v8:content"];

	Возврат Синоним;
КонецФункции

Процедура СконвертироватьОбработку(КаталогИсходников, Описание, ЭтоОтчет=Ложь)
	...
	Описание.Вставить("Синоним", СинонимОбработки(ОписаниеОбработки, ЭтоОтчет));
...
Процедура СконвертироватьМодульМенеджераИнструментаВОбщийМодуль(ОписаниеИнструмента, КаталогОбработок, КаталогИсходниковОбработокОбщихМодулей, КаталогШаблонаОбработки)
	ОписаниеИнструмента.Вставить("МодульМенеджера", Ложь);
	ИмяФайлаМодуляМенеджера=ОбъединитьПути(КаталогОбработок,ОписаниеИнструмента.Имя,"Ext","ManagerModule.bsl");
	...
	ОписаниеИнструмента.Вставить("МодульМенеджера", Истина);

	КопироватьФайл(
		ОбъединитьПути(КаталогШаблонаОбработки, "ШаблонОбработки.xml"), 
		ОбъединитьПути(КаталогИсходниковОбработокОбщихМодулей, ОписаниеИнструмента.Имя + "_МодульМенеджера.xml"));
	
	ФС.КопироватьСодержимоеКаталога(
		ОбъединитьПути(КаталогШаблонаОбработки, "ШаблонОбработки"),
		ОбъединитьПути(КаталогИсходниковОбработокОбщихМодулей, ОписаниеИнструмента.Имя+"_МодульМенеджера"));
	
	СконвертироватьОбщийМодуль(КаталогИсходниковОбработокОбщихМодулей, ОписаниеИнструмента, Истина);
	
КонецПроцедуры

Процедура СконвертироватьОбщийМодуль(КаталогИсходников, ОписаниеМодуля, ЭтоМодульМенеджера=Ложь)
	НовоеИмяМодуля=ОписаниеМодуля.Имя;
	Если ЭтоМодульМенеджера Тогда
		НовоеИмяМодуля=НовоеИмяМодуля+"_МодульМенеджера";
	КонецЕсли;
	ИмяИсходногоФайлаОбработки=ОбъединитьПути(КаталогИсходников, НовоеИмяМодуля+".xml");
	...
	ФайлОписанияФормы=ОбъединитьПути(ОбъединитьПути(КаталогИсходников,НовоеИмяМодуля,"Forms","Форма"),"Ext","Form.xml");
	...
	Если ЭтоМодульМенеджера Тогда
		ФайлИсходногоМодуля=ОбъединитьПути(ОписаниеМодуля.Каталог, "Ext", "ManagerModule.bsl");
	Иначе
	...
	Если ЭтоМодульМенеджера Тогда
		ЗаписатьМодуль(ТекстМодуля, ОбъединитьПути(КаталогИсходников, НовоеИмяМодуля, "Ext", "ObjectModule.bsl"));
	Иначе
		...
		Если ОписаниеМодуля.Клиент
			...
			ЗаписатьМодуль(ТекстМодуля, ОбъединитьПути(ОбъединитьПути(КаталогИсходников, НовоеИмяМодуля, "Forms", "Форма"), "Ext", "Form", "Module.bsl"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьКонвертацию(КаталогПортативнойСборки) Экспорт
	...
	Для Каждого ТекОбработка ИЗ ОписаниеРасширения.Обработки Цикл
		...
		СконвертироватьОбработку(КаталогИсходниковИнструментов, ТекОбработка);

		СконвертироватьМодульМенеджераИнструментаВОбщийМодуль(ТекОбработка,КаталогОбработок, КаталогИсходниковОбработокОбщихМодулей,КаталогШаблонаОбработки)
	КонецЦикла;	
	...
	Для Каждого ТекОбработка ИЗ ОписаниеРасширения.Отчеты Цикл
		...
		СконвертироватьОбработку(КаталогИсходниковИнструментов, ТекОбработка, Истина);
		СконвертироватьМодульМенеджераИнструментаВОбщийМодуль(ТекОбработка,КаталогОтчетов, КаталогИсходниковОбработокОбщихМодулей,КаталогШаблонаОбработки)
	...
	Для Каждого ТекКартинка Из ОписаниеРасширения.ОбщиеКартинки Цикл
		...
		ТекКартинка.Вставить("РасширениеКартинки", ФайлКартинки.Расширение);
		КопироватьФайл(ФайлКартинки.ПолноеИмя,ОбъединитьПути(КаталогОбщихКартинок,ТекКартинка.Имя+ФайлКартинки.Расширение));
	КонецЦикла;

	СобратьГлавнуюОбработку(КаталогИсходныхФайловРасширения, КаталогИсходниковПоставки, КаталогПортативнойСборки);
	
	СконвертироватьМодулиПортативнойПоставки(КаталогИсходниковОбработокОбщихМодулей, КаталогИсходниковПоставки, КаталогИсходниковИнструментов);

	СобратьБинарныеФайлыПроекта(КаталогИсходниковПоставки, КаталогПортативнойСборки);
