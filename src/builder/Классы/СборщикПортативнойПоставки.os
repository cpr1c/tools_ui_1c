...
Процедура ПреобразоватьФайлыОписанийФормДляПереводаВоВнешнююОбработку(КаталогИсходниковОбработки, ИмяИсходнойОбработки, ОписаниеОбработки, ЭтоОтчет)
	...
	Для Каждого СоответствиеПодчиненнойФормы ИЗ НастройкиОбработки._Элементы["ChildObjects"] Цикл
		...
		Если ИмяФормы = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ПреобразоватьОписаниеФормыДляПереводаВоВнешнююОбработку(КаталогИсходниковОбработки, ИмяИсходнойОбработки, ИмяФормы, ЭтоОтчет);
...
// Функция ОписаниеОбщихМодулейРасширения(ОписаниеРасширения)
...
Процедура СобратьГлавнуюОбработку(КаталогИсходныхФайловРасширения, КаталогИсходниковПоставки, ОписаниеКонфигурации,КаталогПортативнойСборки)
	...
	СформироватьБинарныйФайл(ИмяИсходногоФайлаОбработки, ОбъединитьПути(КаталогПортативнойСборки,ИмяГлавнойОбработки+".epf"));
КонецПроцедуры
...
Процедура СконвертироватьОбработку(КаталогИсходников, ИмяОбработки, ОписаниеКонфигурации, ЭтоОтчет=Ложь)
	...
	ПреобразоватьФайлыОписанийФормДляПереводаВоВнешнююОбработку(КаталогИсходников, ИмяОбработки, ОписаниеОбработки, ЭтоОтчет);
...
Процедура СконвертироватьОбщийМодуль(КаталогИсходников, ОписаниеМодуля)
	...
	КонецЕсли;
	Если ОписаниеМодуля.Клиент Тогда
...
Процедура ВыполнитьКонвертацию(КаталогПортативнойСборки) Экспорт
	...
	Для Каждого ТекОбработка ИЗ Описание.Отчеты Цикл
		...
		СконвертироватьОбработку(КаталогИсходниковИнструментов, ТекОбработка.Имя, Описание, Истина);
	КонецЦикла;
	...
	Для Каждого ТекОбработка ИЗ Описание.Отчеты Цикл
		Попытка
			СформироватьБинарныйФайл(ОбъединитьПути(КаталогИсходниковИнструментов, ТекОбработка.Имя + ".xml"), ОбъединитьПути(КаталогОбработокСборки, ТекОбработка.Имя + ".erf"));
			УспешноСобранныеИнструменты.Добавить(ТекОбработка.Имя);			
		Исключение
		КонецПопытки;
	КонецЦикла;
	...
	Для Каждого ТекМодуль Из Описание.ОбщиеМодули Цикл
		Попытка
			СформироватьБинарныйФайл(ОбъединитьПути(КаталогИсходниковОбработокОбщихМодулей, ТекМодуль.Имя + ".xml"), ОбъединитьПути(КаталогОбщихМодулейСборки, ТекМодуль.Имя + ".erf"));
		Исключение
		КонецПопытки;
	КонецЦикла;
