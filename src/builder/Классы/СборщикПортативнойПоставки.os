...
Процедура ВыполнитьКонвертацию(КаталогПортативнойСборки) Экспорт
	...
	Лог.Информация("Копирую файлы инструментов и модулей для конвертации во внешние обработки");
	СкопироватьИсходныеФайлыДляКонвертации();
	
	Лог.Информация("Выполняю конвертацию файлов инструментов во внешние обработки");
	...
	Лог.Информация("Выполняю конвертацию файлов общих модулей во внешние обработки");
	...
	Лог.Информация("Собираю главную обработку");
...
Процедура СкопироватьИнструментВКаталогИсходников(ОписаниеМодуля)
	...
	ОписаниеИнструмента = СтрукутураОписанияИнструмента(ИмяФайлаОписанияИнструмента);
	ПрочитатьПодсистемыВОписаниеИнструмента(ОписаниеИнструмента, ОписаниеРасширения.Подсистемы);
	СкопироватьМодульМенеджераВоВнешнююОбработку(ОписаниеИнструмента);

	СтруктураПортативныхИнструментов.Инструменты.Вставить(ОписаниеМодуля.Имя, ОписаниеИнструмента);
...
Процедура ПрочитатьОписанияПодсистемКонфигурации(ПодсистемыДляЗаполнения, КаталогИсходниковИнструментов)
	Подсистемы = ПрочитатьМассивМетаданных(ОбъединитьПути(КаталогИсходниковИнструментов, "Subsystems"));	
	Для Каждого ТекПодсистема Из Подсистемы Цикл
		ТекПодсистема.Вставить("Состав", Новый Массив);
		ТекПодсистема.Вставить("Подсистемы", Новый Массив);
		
		ОписаниеXML = ПроцессорXMLБезУпрощения.ПрочитатьИзФайла(ТекПодсистема.ФайлОписания);
		
		ТекПодсистема.Вставить("Синоним", СинонимОбработки(ОписаниеXML,"Subsystem"));
		
		ОписаниеОбъектаМетаданного = ОписаниеXML._Элементы["MetaDataObject"];
		ОписаниеПодсистемы = ОписаниеОбъектаМетаданного._Элементы["Subsystem"];

		СвойстваПодсистемы = ОписаниеПодсистемы._Элементы["Properties"];
		СоставПодсистемы = СвойстваПодсистемы._Элементы["Content"];

		Для Каждого ТекЭлементСостава ИЗ СоставПодсистемы._Элементы Цикл
			Если ТипЗнч(ТекЭлементСостава) = Тип("КлючИЗначение") Тогда
				ЗначениеЭлемента = ТекЭлементСостава.Значение;
			Иначе
				ЗначениеЭлемента = ТекЭлементСостава["xr:Item"];
			КонецЕсли;
			Если ЗначениеЭлемента = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			ИмяОбъектаСостава = ЗначениеЭлемента._Значение;
			ТекПодсистема.Состав.Добавить(ИмяОбъектаСостава);
		КонецЦикла;
		ПрочитатьОписанияПодсистемКонфигурации(ТекПодсистема.Подсистемы, ТекПодсистема.Каталог);

		ПодсистемыДляЗаполнения.Добавить(ТекПодсистема);
	КонецЦикла;
КонецПроцедуры
...
Процедура ПрочитатьОписаниеКонфигурации(КаталогИсходниковИнструментов)
	...
	КонецЦикла;
	ПрочитатьОписанияПодсистемКонфигурации(ОписаниеРасширения.Подсистемы, КаталогИсходниковИнструментов);
КонецПроцедуры
...
Процедура ПрочитатьПодсистемыВОписаниеИнструмента(ОписаниеИнструмента, Подсистемы)
	Если ОписаниеИнструмента.ЭтоОтчет Тогда
		ИмяДляПоиска = "Report";
	Иначе
		ИмяДляПоиска = "DataProcessor";
	КонецЕсли;
	ИмяДляПоиска = ИмяДляПоиска +"."+ОписаниеИнструмента.Имя;
	Для Каждого ТекПодсистема Из Подсистемы Цикл
		Если ТекПодсистема.Состав.Найти(ИмяДляПоиска) <> Неопределено Тогда
			ОписаниеИнструмента.Подсистемы.Добавить(ТекПодсистема.Имя);
		КонецЕсли;

		ПрочитатьПодсистемыВОписаниеИнструмента(ОписаниеИнструмента, ТекПодсистема.Подсистемы);
	КонецЦикла;
КонецПроцедуры
...
Процедура ПреобразоватьКорневойФайлОбработкиДляПереводаЕеВоВнешнюю(ТекИнструмент)
	...
	ПодчиненныеОбъекты = НастройкиОбработки._Элементы["ChildObjects"];
	Если ПодчиненныеОбъекты <> Неопределено Тогда
		Если ТипЗнч(ПодчиненныеОбъекты._Элементы) = Тип("Массив") Тогда
			КоличествоПодчиненныхОбъектов = ПодчиненныеОбъекты._Элементы.Количество();
			Для ИндексЭлемента = 0 По КоличествоПодчиненныхОбъектов - 1 Цикл
				ИденксПодчиненногоЭлемента = КоличествоПодчиненныхОбъектов - 1 - ИндексЭлемента;
				ТекПодчиненныйОбъект = ПодчиненныеОбъекты._Элементы[ИденксПодчиненногоЭлемента];
				Команда = ТекПодчиненныйОбъект["Command"];
				Если Команда = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				ПодчиненныеОбъекты._Элементы.Удалить(ИденксПодчиненногоЭлемента);
			КонецЦикла;
		ИначеЕсли ТипЗнч(ПодчиненныеОбъекты._Элементы) = Тип("Соответствие") Тогда
			Команда = ПодчиненныеОбъекты._Элементы["Command"];
			Если Команда <> Неопределено Тогда
				ПодчиненныеОбъекты._Элементы.Удалить("Command");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьИнструкцииПрепроцессораВКлиентскийМодульВМодульОбработкиОбщегоМодуляДляПоддрежкиТолстогоКлиентаФайловойБазы(ИмяФайлаМодуля)
	ТекстОбщегоМодуля = ТекстМодуля(ИмяФайлаМодуля);
	ТекстОбщегоМодуля = "#Если ТолстыйКлиентУправляемоеПриложение ИЛИ ТолстыйКлиентОбычноеПриложение Тогда
	|
	|
	|"+ТекстОбщегоМодуля+"
	|
	|#КонецЕсли";
	ЗаписатьМодуль(ТекстОбщегоМодуля, ИмяФайлаМодуля);
...
Процедура СконвертироватьМодулиПортативнойПоставки()
	Лог.Информация("Выполняю конвертацию модулей инструментов");
	...
	Лог.Информация("Выполняю конвертацию модулей обработок общих модулей");
	ИмяФормы = "Форма";
	Для Каждого ОбщийМодуль Из СтруктураПортативныхИнструментов.ОбщиеМодули Цикл
		...
		Если ОписаниеИнструмента.Клиент И Не ОписаниеИнструмента.Сервер Тогда
			ДобавитьИнструкцииПрепроцессораВКлиентскийМодульВМодульОбработкиОбщегоМодуляДляПоддрежкиТолстогоКлиентаФайловойБазы(ИмяФайлаМодуля);
		КонецЕсли;
...
Процедура ДополнитьГенерируемымКодомМодульФормыГлавнойОбработки()
	...
	ПараметрыПлагинУстановкаТекстаПроцедурыПолученияОписанияПодключенныхМодулейПортативнойПоставки.Вставить("ОписаниеРасширения", ОписаниеРасширения);
...
Функция КонфигураторИнициализированный()
	...
	СбощикБинарников = Новый СборщикБинарныхФайлов1С;
	СбощикБинарников.УстановитьКаталогВременныхФайлов(МенеджерВременныхФайлов.СоздатьКаталог());
	СбощикБинарников.УстановитьЛог(Лог);
	СбощикБинарников.УстановитьПутьКПлатформе1С(ИмяКаталогаПлатформы);
	СбощикБинарников.УстановитьИмяКаталогаВременнойБазы(КаталогБазы);
	СбощикБинарников.СоздатьФайловуюБазу();

	Возврат СбощикБинарников;
КонецФункции
...
Процедура СобратьБинарныеФайлыМодулей(КаталогПортативнойСборки)
	Лог.Информация("Собираю файлы обработок общих модулей и инструментов в бинарный формат");
...
Процедура СформироватьБинарныйФайл(Конфигуратор, ИмяИсходногоФайлаОбработки, ИмяФайлаОбработки)
	Конфигуратор.СформироватьБинарныйФайлОбработки(ИмяИсходногоФайлаОбработки, ИмяФайлаОбработки);
КонецПроцедуры
