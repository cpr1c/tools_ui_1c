...
Процедура ВыполнитьКонвертацию(КаталогПортативнойСборки) Экспорт
	...
	Сообщить(СтруктураПортативныхИнструментов.КаталогИсходников);
...
Процедура УстановитьИмяСтандартногоСобытияТаблицыСвязаннойСТабличнойчастьюИнструмента(ИмяКоманды, УзелОписания, ИдентификарЭлементаТаблицы)
	...
	УзелИмениКоманды._Значение=ИдентификарЭлементаТаблицы+":"+Замены[МассивИмениКоманды[4]];
КонецПроцедуры

Процедура ПреобразоватьПодчиненныеЭлементыФормы(ТекИнструмент, ЭлементыФормы, ОписанияКомандФормы, ПапкаФормы, ВерхнитьЭлементСвязанСТабличнойЧастью=Ложь,ИдентификарЭлементаТаблицыВерхнегоУровня=Неопределено)
	Если ЭлементыФормы=Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ОписаниеЭлемента Из ЭлементыФормы._Элементы Цикл
		ЭлементНаОснованииТЧИнструмента=Ложь;
		ИдентификарЭлементаТаблицы=Неопределено;
				...
				Если Найти(НРег(УзелПутьКДанным._Значение),"объект")>0 Тогда
					...
					Если МассивПутьКДанным.Количество()=2 Тогда
						Если ТекИнструмент.ТабличныеЧасти.Найти(МассивПутьКДанным[1])<>Неопределено Тогда
							...
							ЭлементНаОснованииТЧИнструмента=Истина;
							ИдентификарЭлементаТаблицы=УзелОписания.Значение._Атрибуты["id"];
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		Если Не ЕстьКартинка 
			...
			Если ВерхнитьЭлементСвязанСТабличнойЧастью Тогда
				
				УстановитьИмяСтандартногоСобытияТаблицыСвязаннойСТабличнойчастьюИнструмента(ИмяКоманды, УзелОписания, ИдентификарЭлементаТаблицыВерхнегоУровня);	
		...
		ПреобразоватьПодчиненныеЭлементыФормы(ТекИнструмент, УзелОписания.Значение._Элементы["ChildItems"], ОписанияКомандФормы, ПапкаФормы,ЭлементНаОснованииТЧИнструмента,ИдентификарЭлементаТаблицы);

		КоманднаяПанельФормыXML=УзелОписания.Значение._Элементы["AutoCommandBar"];

		Если КоманднаяПанельФормыXML<>Неопределено Тогда
			ПреобразоватьПодчиненныеЭлементыФормы(ТекИнструмент, КоманднаяПанельФормыXML._Элементы["ChildItems"],ОписанияКомандФормы, ПапкаФормы,ЭлементНаОснованииТЧИнструмента,ИдентификарЭлементаТаблицы);
		КонецЕсли;
...
Процедура ПреобразоватьОписаниеФормыДляПереводаВоВнешнююОбработку(ТекИнструмент, ИмяФормы)
	...
	ПреобразоватьПодчиненныеЭлементыФормы(ТекИнструмент, ЭлементыXMLФорма._Элементы["ChildItems"],ОписанияКомандФормы, ПапкаФормы);

	КоманднаяПанельФормыXML=ЭлементыXMLФорма._Элементы["AutoCommandBar"];

	Если КоманднаяПанельФормыXML<>Неопределено Тогда
		ПреобразоватьПодчиненныеЭлементыФормы(ТекИнструмент, КоманднаяПанельФормыXML._Элементы["ChildItems"],ОписанияКомандФормы, ПапкаФормы);
	КонецЕсли;
...
Процедура СконвертироватьОбщийМодуль(ТекИнструмент)
	...
	УзелВспомогательнойФормы=Properties._Элементы["AuxiliaryForm"];
	Если УзелВспомогательнойФормы<>Неопределено Тогда
		УзелВспомогательнойФормы._Значение=СтрЗаменить(УзелВспомогательнойФормы._Значение,"ШаблонОбработки",НовоеИмяМодуля);
	КонецЕсли;
