...
Функция ТекстОпределенияПеременныхОбщихМодулей()
	...
	Если ЭтоМодульФормы Тогда
		...
		|Функция " + ПрефиксПеременных + "ОбщийМодульПоИмени(ИмяМодуля, УИ_ГЕНЕРАЦИЯ_КэшОбщихМодулей=Неопределено)
		|";
...
Процедура ПосетитьМодуль(Модуль) Экспорт
	...
	Для НомерОбъявления=1 По КоличествоОбъявлений Цикл
		...
		Если ТекОбъявление.Тип=Типы.ИнструкцияПрепроцессораЕсли
			Или ТекОбъявление.Тип=Типы.ИнструкцияПрепроцессораИначеЕсли
			Или ТекОбъявление.Тип=Типы.ИнструкцияПрепроцессораКонецЕсли Тогда
			Продолжить;
		КонецЕсли;
