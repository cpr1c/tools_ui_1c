...
Функция ТекстОпределенияПеременныхОбщихМодулей()
	...
	Иначе
		...
		|	Если Не ЭтоАдресВременногоХранилища(ИмяФайла) Тогда
		|		РазделительПути=ПолучитьРазделительПути();
		|		
		|		МассивИмени=_СтрРазделить(ИмяФайла,ПолучитьРазделительПути());
		|		МассивИмени.Удалить(МассивИмени.Количество()-1);
		|		МассивИмени.Удалить(МассивИмени.Количество()-1);
		|		МассивИмени.Добавить(""ОбщиеМодули"");
		|		МассивИмени.Добавить("+ПрефиксПеременных+"ИмяФайлаОбщегоМодуля(ИмяМодуля));
		|		
		...
		|		
		|		ИмяФайлаМодуля=_СтрСоединить(МассивИмени, РазделительПути);
		|		
		|		Попытка
		|			ОбщийМодуль=ВнешниеОбработки.Создать(ИмяМодуля);
		|		Исключение
	...
	ТекстПроцедур=ТекстПроцедур+"
	|	Функция "+ПрефиксПеременных+"ИмяФайлаОбщегоМодуля(ИмяОбщегоМодуля)
	...
	ТекстПроцедур=ТекстПроцедур+"
	|		Возврат Структура[ИмяОбщегоМодуля];
	|	КонецФункции";
