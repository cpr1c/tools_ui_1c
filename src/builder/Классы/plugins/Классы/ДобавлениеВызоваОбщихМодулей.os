...
Процедура ПосетитьМодуль(Модуль) Экспорт
	Если Модуль.Операторы.Количество() > 0 Тогда
		...
		// Вставка(ТекстОпределенияПеременныхОбщихМодулей(), Модуль.Операторы[Модуль.Операторы.Количество() - 1].Конец.Позиция+Модуль.Операторы[Модуль.Операторы.Количество() - 1].Конец.Длина+1);
