...
Функция ТекстОпределенияПеременныхОбщихМодулей()
	...
	|
	|&НаКлиенте
	|Функция "+ПрефиксПеременных+"КлиентскийОбщийМодульПоИмени(ИмяМодуля) 
	|	ОбщийМодуль=УИ_ГЕНЕРАЦИЯ_КэшОбщихМодулей[ИмяМодуля];
	|	
	|	Если ОбщийМодуль=Неопределено Тогда
	|		ОбщийМодуль=ПолучитьФорму(""ВнешняяОбработка.""+ИмяМодуля+"".Форма"");
	|		УИ_ГЕНЕРАЦИЯ_КэшОбщихМодулей.Вставить(ИмяМодуля, ОбщийМодуль);
	|	КонецЕсли;
	|
	|	Возврат ОбщийМодуль;
	|КонецФункции
	...
	|Функция "+ПрефиксПеременных+"СерверныйОбщийМодульПоИмени(ИмяМодуля)
	|	Возврат ВнешниеОбработки.Создать(ИмяМодуля);
	|КонецФункции
	|";
