...
Процедура ЗаполнитьТекстОписанияПодсистем(Тексты, Подсистемы) 
	Для Каждого ТекПодсистема Из Подсистемы Цикл
		Тексты.Добавить(СтрШаблон("ОписаниеПодсистем.Подсистемы.Вставить(""%1"",""%2"");", ТекПодсистема.Имя, ТекПодсистема.Синоним));
		Тексты.Добавить(СтрШаблон("ОписаниеПодсистем.Порядок.Добавить(""%1"");", ТекПодсистема.Имя));
		ЗаполнитьТекстОписанияПодсистем(Тексты, ТекПодсистема.Подсистемы);
	КонецЦикла;
КонецПроцедуры

Функция ТекстОписанияПодсистем() 
	МассивТекстов = Новый Массив;
	МассивТекстов.Добавить("ОписаниеПодсистем = НовыйОписаниеПодсистем();");

	ЗаполнитьТекстОписанияПодсистем(МассивТекстов, ОписаниеРасширения.Подсистемы);

	МассивТекстов.Добавить("Возврат ОписаниеПодсистем;");
...
Процедура ПосетитьОбъявлениеМетода(Описание) Экспорт
	Если НРег(Описание.Сигнатура.Имя) = "описаниемодулейинструментовдляподключения" Тогда
		...
		Для Каждого КлючЗначение ИЗ СтруктураПортативныхИнструментов.Инструменты Цикл
			...
			Если ТекИнструмент.НеДляВыводаВИнтерфейс Тогда
				ТекстПроцедуры=ТекстПроцедуры+"
				|ОписаниеИнструмента.НеВыводитьВИнтерфейс=Истина;
				|";
			КонецЕсли;

			Для Каждого ТекРаздел Из ТекИнструмент.Подсистемы Цикл
				ТекстПроцедуры = ТекстПроцедуры+"
				|ОписаниеИнструмента.Подсистемы.Добавить("""+ТекРаздел+""");";
			КонецЦикла;
	...
	ИначеЕсли НРег(Описание.Сигнатура.Имя) = "поставщик" Тогда
		...
		Вставка(СтрокаВставки, Описание.Конец.Позиция);
	ИначеЕсли НРег(Описание.Сигнатура.Имя) = "описаниеподсистеминструментовдляподключения" Тогда
		ТекстПроцедуры = ТекстОписанияПодсистем();
