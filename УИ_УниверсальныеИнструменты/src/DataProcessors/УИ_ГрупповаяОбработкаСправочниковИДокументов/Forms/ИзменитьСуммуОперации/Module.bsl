...
Процедура ОбработатьОбъект(Ссылка, ПорядковыйНомерОбъекта)
	...
	Если ОбрабатыватьТабличныеЧасти Тогда
		СтрокаТЧ=Объект[НайденныеОбъектыТЧ[ПорядковыйНомерОбъекта].Т_ТЧ][НайденныеОбъектыТЧ[ПорядковыйНомерОбъекта].Т_НомерСтроки-1];
	КонецЕсли;
...
Функция ВыполнитьОбработку() Экспорт
	...
	Для Индекс = 0 По НайденныеОбъекты.Количество() - 1 Цикл
		...
		Если СтрокаНайденныхОбъектов.Выбрать Тогда//
			...
			ОбработатьОбъект(СтрокаНайденныхОбъектов.Объект,Индекс);
		КонецЕсли;
	КонецЦикла;
	...
	Если Параметры.Свойство("ТаблицаРеквизитов") Тогда
		...
		ДЛя Каждого Реквизит Из Параметры.ТаблицаРеквизитов Цикл
			...
			Если Реквизит.Тип= ОписаниеТипа("Число") Тогда
				Элементы.ТекущийРеквизит.СписокВыбора.Добавить(Реквизит.Имя+?(Реквизит.ЭтоТЧ,"_ТЧ_12345",""),Реквизит.Представление+?(Реквизит.ЭтоТЧ," [ТЧ]",""));
			КонецЕсли;
		КонецЦикла;
