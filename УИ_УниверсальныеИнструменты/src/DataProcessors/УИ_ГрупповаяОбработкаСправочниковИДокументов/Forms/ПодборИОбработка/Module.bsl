...
Процедура ДоступныеОбработкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Родитель = Неопределено Тогда
		Если НЕ ОбработкаДоступна(?(Объект.ТипОбъекта=0,"Справочник","Документ"), ТекущаяСтрока.ИмяФормы) Тогда
			Предупреждение("Данная обработка недоступна для типа <" + ?(Объект.ТипОбъекта=0,"Справочник","Документ") + ">");
			Возврат;
		КонецЕсли;
...
//	МассивНастроекОтбора     = Новый Массив;
	...
	КонтрольОстатковНоменклатуры = (Объект.ТипОбъекта = 0) И (ИмяВидаОдногоТипа = "Номенклатура") И ВКонфигурацииЕстьОстаткиНоменклатуры;
	//
	ДоступностьВВебПриложенииНоменклатуры = ВКонфигурацииЕстьУправлениеЗаказами И (ОБъект.ТипОбъекта = 0) И (ИмяВидаОдногоТипа = "Номенклатура");
...
//	ОписаниеВсехТипов = Новый ОписаниеТипов(МассивТипов);
	...
	Для каждого Строка Из ТабличноеПолеВидыОбъектов Цикл
		...
		ТекстЗапросаОбъект = ТекстЗапросаОбъект + "," + Символы.ПС + "	"   +ПсевдонимТаблицы+"."+Префикс+"Ссылка КАК Объект";
		...
		Если Объект.ТипОбъекта = 0  И МетаданныеОбъектов[ИмяВида].Иерархический И МетаданныеОбъектов[ИмяВида].ВидИерархии = Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов Тогда
		...
		ТекстЗапроса = ?(ТекстЗапроса = "","Выбрать ",ТекстЗапроса + Символы.ПС + Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС + Символы.ПС + "ВЫБРАТЬ") + ТекстЗапросаОбъект;
