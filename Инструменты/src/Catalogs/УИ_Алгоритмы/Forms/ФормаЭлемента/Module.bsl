...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Ключ.Пустая() Тогда
		...
		НастройкиТекстАлгоритма = ХранилищеОбщихНастроек.ПолучитьСписок(Строка(Параметры.Ключ)
			+ "-n1");

		Для Каждого ЭлементСписка Из НастройкиТекстАлгоритма Цикл

			НастройкаТекстАлгоритма = ХранилищеОбщихНастроек.Загрузить(Строка(Параметры.Ключ)
				+ "-n1", ЭлементСписка.Значение);
...
Процедура ТаблицаПараметровПередУдалением(Элемент, Отказ)
	ПоказатьВопрос(Новый ОписаниеОповещения("ТаблицаПараметровПередУдалениемЗавершение", ЭтотОбъект, Новый Структура("Строка,Параметр", Элемент.ТекущаяСтрока, Элемент.ТекущиеДанные.Параметр)), "Элемент структуры настроек будет удален без возможности  восстановления !"
		+ Символы.ПС + "Продолжить выполнение ? ", РежимДиалогаВопрос.ДаНетОтмена);
	Отказ = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаПараметровПередУдалениемЗавершение(Результат,
...
Процедура ВыполнитьПроцедуру(Команда)
	...
	КонецЕсли;
	Элементы.ВыполнитьПроцедуру.Заголовок = "Выполнить процедуру ("
		+ Строка(ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяСтарт) + " мс.)";
КонецПроцедуры
	...
	Если ТипЗнч(НовыеДанные.ЗначениеПараметра) = Тип("Строка") Тогда
		Если Лев(НовыеДанные.ЗначениеПараметра, 1) = "{" Тогда
			...
			Если Поз > 0 Тогда
				...
				РасширениеФайла = СтрЗаменить(Сред(НовыеДанные.ЗначениеПараметра, 2, Поз
					- 2), Символ(32), "");
				НаименованиеПараметра = "Файл" + ВРег(РасширениеФайла) + "_"
	...
	Если ПозОшибки > 0 Тогда
		...
		Если ПозСкобкаЗакрылась > 0 Тогда
			...
			Если ПозЗапятая > 0 Тогда
				ТекстНомерСтроки = Сред(ТекстОшибки, ПозОшибки
					+ 2, СтрДлина(Лев(ТекстОшибки, ПозЗапятая))
					- СтрДлина(Лев(ТекстОшибки, ПозОшибки)) - 2);
			Иначе
				ТекстНомерСтроки = Сред(ТекстОшибки, ПозОшибки
					+ 2, СтрДлина(Лев(ТекстОшибки, ПозСкобкаЗакрылась))
					- СтрДлина(Лев(ТекстОшибки, ПозОшибки)) - 2);
			КонецЕсли;
			...
			Если ПозОшибки2 > 0 Тогда
				...
				Если ПозСкобкаЗакрылась2 > 0 Тогда
					...
					Если ПозЗапятая2 > 0 Тогда
						ТекстНомерСтроки2 = Сред(ТекстОшибки, ПозОшибки2
							+ 2, СтрДлина(Лев(ТекстОшибки, ПозЗапятая2))
							- СтрДлина(Лев(ТекстОшибки, ПозОшибки2)) - 2);
					Иначе
						ТекстНомерСтроки2 = Сред(ТекстОшибки, ПозОшибки2
							+ 2, СтрДлина(Лев(ТекстОшибки, ПозСкобкаЗакрылась2))
							- СтрДлина(Лев(ТекстОшибки, ПозОшибки2)) - 2);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	Если СтрДлина(ВыделенныйТекст) > 10 Тогда // нужен уникальный текст , по хорошему нужно проверить количество включений
		...
		Для А = 1 По СтрЧислоВхождений(ВесьТекст, """") Цикл
			...
			Если НашлиКавычкуПосле > ИщемЗдесь Тогда
				ВыделенныйТекст = Сред(ВесьТекст, НашлиКавычкуДо
					+ 1, СтрДлина(Лев(Весьтекст, НашлиКавычкуПосле))
	...
	Если ДополнительныеПараметры.ПервыйВызовКонструктора Тогда
		...
		Пока Найти(ТекстЗапроса, "&") > 0 Цикл
			...
			ТекстВставки = ТекстВставки + "
				...
				+ " );";
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&" + ПарметрЗапроса, "~"
				+ ПарметрЗапроса);
		КонецЦикла;
		Текст = Текст + "
...
&НаСервере
Процедура ВыполнитьПроцедуруНаСервере(ОшибкаВыполнения = Ложь,
		СообщениеОбОшибке = "")
	ВыбОбъект = РеквизитФормыВЗначение("Объект");
...
//Импорт
&НаКлиенте
Процедура ВнешнийФайлНачалоВыбораЗавершение(ВыбранныеФайлы,
		ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗакончитьПомещениеФайла(Результат, АдресХранилища, ВыбранноеИмяФайла,
		ДополнительныеПараметры) Экспорт
...
&НаСервере
Процедура ПрочитатьНаСервере(АдресХранилища, ВыбранноеИмяФайла,
...
//Экспорт
&НаКлиенте
Процедура ВыборКаталогаЗавершение(ВыбранныеФайлы,
		ДополнительныеПараметры) Экспорт
	Если (ТипЗнч(ВыбранныеФайлы) = Тип("Массив")
			И ВыбранныеФайлы.Количество() > 0) Тогда
		...
		ОписаниеФайла.Имя = Каталог
			+ ПолучитьРазделительПути() + ИмяФайла
			+ РасширениеФайла;
...
Процедура ПослеполученияФайла(ПолученныеФайлы, ДопПараметры) Экспорт
	Если ТипЗнч(ПолученныеФайлы) = Тип("Массив") Тогда
		УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Файл " + ПолученныеФайлы[0].Имя
			+ " записан");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверкаПараметровПрограммныйИнтерфейс(Команда)
	...
	Для каждого ЭлементСоответствия Из Заполнение Цикл
		...
		Если НоваяСтрока.Вход
				И ХранимыеПараметры.Свойство(ЭлементСоответствия.Ключ) Тогда
...
Процедура СоздатьРегламентноеЗадание()
	...
	Иначе
		...
		Задание.Записать();
		Сообщить("Создано регламентное задание " + Объект.Наименование
			+ " с  ключом " + Отбор.Ключ);
	КонецЕсли;
КонецПроцедуры
