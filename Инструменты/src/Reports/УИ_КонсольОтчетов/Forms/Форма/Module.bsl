...
&НаСервере
Функция СгенерироватьИмяСервер(ТипСтроки, БазоваяЧастьИмени,
		КоллекцияЭлементов, Рекурсивно)
...
//   Строка - сгенерированное имя.
&НаКлиенте
Функция СгенерироватьИмя(ТипСтроки, БазоваяЧастьИмени, КоллекцияЭлементов,
		Рекурсивно)
...
&НаСервере
Процедура НайтиУникальныеИменаСервер(Элементы, ТипСтроки, УникальныеИмена,
		Рекурсивно)
...
//  Рекурсивно - необходимость рекурсивного получения вложенных имен.
&НаКлиенте
Процедура НайтиУникальныеИмена(Элементы, ТипСтроки, УникальныеИмена,
		Рекурсивно)
...
&НаСервере
Процедура ЗагрузитьСхемуКомпоновкиДанныхВКомпоновщикНастроек(ЭлементДерева,
...
&НаСервере
Процедура ВывестиМакетКомпоновкиДанныхВТабличныйДокумент(МакетКомпоновкиДанных,
...
&НаСервере
Функция ПолучитьТекстРезультатаКомпоновкиДанных(МакетКомпоновкиДанных,
		ДанныеРасшифровкиОбъект)
...
&НаСервере
Процедура ВывестиМакетКомпоновкиДанныхВРезультатXML(МакетКомпоновкиДанных,
		ДанныеРасшифровкиОбъект)
...
Функция СформироватьНаСервере()
	...
	СформироватьНаСервереВТабличныйДокумент();
	СформироватьНаСервереВКоллекцию();
...
&НаКлиенте
Процедура ПодтвердитьЗакрытиеЗавершение(РезультатВопроса,
		ДополнительныеПараметры) Экспорт
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяФормыРасшифровки = "Обработка.УИ_КонсольОтчетов.Форма.ФормаРасшифровки";
...
Процедура ПриОткрытии(Отказ)
	...
	//Устанавливаем текущую строку отчета
	ЭлементыДерева = ДеревоОтчетов.ПолучитьЭлементы();
	Если ЭлементыДерева.Количество() > 0 Тогда
		ЭлементыОтчетов = ЭлементыДерева[0].получитьЭлементы();
		Если ЭлементыОтчетов.Количество() > 0 Тогда
			Элементы.ДеревоОтчетов.ТекущаяСтрока = ЭлементыОтчетов[0].ПолучитьИдентификатор();
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьСхемуКомпоновкиДанных(Команда)
	...
	Пока ТекущийЭлементДерева <> Неопределено Цикл
		...
		Если ТекущийЭлементДерева.ТипСтроки = 0
				...
				И ТипПапки(ТекущийЭлементДерева) = 4)
...
// Завершение обработчика открытия файла.
&НаКлиенте
Процедура ОткрытьФайлОтчетовЗавершение(Результат,
		ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура НовыйФайлОтчетовЗавершение(Результат,
		ДополнительныеПараметры) Экспорт
...
Процедура СохранитьЭталонМакетаКомпоновкиДанных(Команда)
	...
	#Если ТолстыйКлиентУправляемоеПриложение Или ТолстыйКлиентОбычноеПриложение Тогда
	ЗаписьТекста = Новый ЗаписьТекста(ИмяФайлаЭталонаМакета(), , Символы.ВК
		+ Символы.ПС, , "");
...
Процедура СравнитьСЭталономМакетКомпоновкиДанных(Команда)
	...
	#Если ТолстыйКлиентУправляемоеПриложение Или ТолстыйКлиентОбычноеПриложение Тогда
	ЗаписьТекста = Новый ЗаписьТекста(ИмяФайлаМакета(), , Символы.ВК
		+ Символы.ПС, , "");
	ЗаписьТекста.ЗаписатьСтроку(ТекстМакетаКомпоновкиДанных);
	ЗаписьТекста.Закрыть();
...
Процедура СохранитьЭталонИсполняемыхНастроек(Команда)
	...
	#Если ТолстыйКлиентУправляемоеПриложение Или ТолстыйКлиентОбычноеПриложение Тогда
	ЗаписьТекста = Новый ЗаписьТекста(ИмяФайлаЭталонаИсполняемыхНастроек(), , Символы.ВК
		+ Символы.ПС, , "");
...
Процедура СравнитьСЭталономИсполняемыхНастроек(Команда)
	...
	#Если ТолстыйКлиентУправляемоеПриложение Или ТолстыйКлиентОбычноеПриложение Тогда
	ЗаписьТекста = Новый ЗаписьТекста(ИмяФайлаИсполняемыхНастроек(), , Символы.ВК
		+ Символы.ПС, , "");
	ЗаписьТекста.ЗаписатьСтроку(ИсполняемыеНастройкиXML);
	ЗаписьТекста.Закрыть();
...
Процедура СохранитьЭталонРезультатаXML(Команда)
	...
	#Если ТолстыйКлиентУправляемоеПриложение Или ТолстыйКлиентОбычноеПриложение Тогда
	ЗаписьТекста = Новый ЗаписьТекста(ИмяФайлаЭталонаРезультатаXML(), , Символы.ВК
		+ Символы.ПС, , "");
...
Процедура СравнитьСЭталономРезультатXML(Команда)
	...
	#Если ТолстыйКлиентУправляемоеПриложение Или ТолстыйКлиентОбычноеПриложение Тогда
	ЗаписьТекста = Новый ЗаписьТекста(ИмяФайлаРезультатаXML(), , Символы.ВК
		+ Символы.ПС, , "");
	ЗаписьТекста.ЗаписатьСтроку(ТекстРезультатаКомпоновкиДанных);
	ЗаписьТекста.Закрыть();
...
Процедура СохранитьЭталонМакетаКомпоновкиДанныхДляКоллекции(Команда)
	...
	#Если ТолстыйКлиентУправляемоеПриложение Или ТолстыйКлиентОбычноеПриложение Тогда
	ЗаписьТекста = Новый ЗаписьТекста(ИмяФайлаЭталонаМакетаДляКоллекции(), , Символы.ВК
		+ Символы.ПС, , "");
...
Процедура СравнитьСЭталономМакетКомпоновкиДанныхДляКоллекции(Команда)
	...
	#Если ТолстыйКлиентУправляемоеПриложение Или ТолстыйКлиентОбычноеПриложение Тогда
	ЗаписьТекста = Новый ЗаписьТекста(ИмяФайлаМакетаДляКоллекции(), , Символы.ВК
		+ Символы.ПС, , "");
	ЗаписьТекста.ЗаписатьСтроку(ТекстМакетаКомпоновкиДанныхДляКоллекции);
	ЗаписьТекста.Закрыть();
...
Процедура СохранитьЭталонРезультатаXMLДляКоллекции(Команда)
	...
	#Если ТолстыйКлиентУправляемоеПриложение Или ТолстыйКлиентОбычноеПриложение Тогда
	ЗаписьТекста = Новый ЗаписьТекста(ИмяФайлаЭталонаРезультатаXMLДляКоллекции(), , Символы.ВК
		+ Символы.ПС, , "");
...
Процедура СравнитьСЭталономРезультатXMLДляКоллекции(Команда)
	...
	#Если ТолстыйКлиентУправляемоеПриложение Или ТолстыйКлиентОбычноеПриложение Тогда
	ЗаписьТекста = Новый ЗаписьТекста(ИмяФайлаРезультатаXMLДляКоллекции(), , Символы.ВК
		+ Символы.ПС, , "");
	ЗаписьТекста.ЗаписатьСтроку(ТекстРезультатаКомпоновкиДанныхДляКоллекции);
	ЗаписьТекста.Закрыть();
...
&НаКлиенте
Процедура СохранитьСхемуВФайлПослеПодключенияРасширения(Подключено,
		ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура СохранитьСхемуВФайлПослеВыбораФайла(ВыбранныеФайлы,
		ДополнительныеПараметры) Экспорт
	...
	НачатьПолучениеФайлов(Новый ОписаниеОповещения("СохранитьСхемуВФайлЗавершение", ЭтаФорма), ПолучаемыеФайлы, "", Ложь);

КонецПроцедуры

&НаКлиенте
Процедура СохранитьСхемуВФайлЗавершение(ПолученныеФайлы,
		ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗагрузитьСхемуИзФайлаПослеПодключенияРасширения(Подключено,
		ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗагрузитьСхемуИзФайлаПослеПомещенияФайлов(ПомещенныеФайлы,
		ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗагрузитьСхемуИзФайлаПослеПомещенияФайла(Результат, Адрес,
...
&НаКлиенте
Процедура ДеревоОтчетовПередНачаломДобавления(Элемент, Отказ, Копирование,
		Родитель, Группа)
...
&НаКлиенте
Процедура ДеревоОтчетовВыбор(Элемент, ВыбраннаяСтрока, Поле,
...
&НаКлиенте
Процедура РезультатТабличныйДокументОбработкаДополнительнойРасшифровки(Элемент,
...
&НаКлиенте
Процедура РезультатТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка,
		СтандартнаяОбработка)
...
&НаКлиенте
Процедура РезультатТабличныйДокументОбработкаРасшифровкиЗавершение(ВыполненноеДействие,
...
&НаКлиенте
Процедура РезультатДеревоВыбор(Элемент, ВыбраннаяСтрока, Поле,
		СтандартнаяОбработка)
