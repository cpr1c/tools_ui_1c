...
Функция СформироватьШаблон(Маска)
	...
	Код1С = "//	Выражение = " + Маска + "
				  ...
				  |//	Правая скобка параметра = " + Объект.ПраваяСкобкаПараметра + "
	...
	Для каждого Уни Из Сборка Цикл
		Код1С = Код1С + "//" + Символы.Таб + "+" + ?(Уни.Пометка, "+", "-") + Символы.Таб + Уни.Значение + " = " + Уни.Представление + Символы.ПС;
	КонецЦикла;
	...
	Пока Указатель < ПорядокСборки.Количество()  Цикл
		...
		Для каждого Стр Из Найдено Цикл
			...
			ТекШаблон = СтрЗаменить(ТекШаблон, Наименование, """ + " + ПреобразоватьИмяПараметраВИдентификатор(Наименование) + " + """);
		КонецЦикла;
	...
	Код1СФрагмент = "	
		...
		|RegExpLocal.Global = " + ?(Объект.Global, "Истина", "Ложь") + ";
...
Процедура ПоискВыраженийФрагмент()
	...
	Для каждого Стр Из Найдено Цикл
		...
		ЭлементыФД = Представление.СформироватьЭлементы(Представление.ПолучитьЗакладкуПоПозиции(Начало), Представление.ПолучитьЗакладкуПоПозиции(Конец));
...
Процедура Замена(Команда)
	...
	Если НЕ Объект.Multiline Тогда
		...
		Для Инд = 1 По ВсегоСтрок Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Объект.Результат = Объект.Результат + ДополнитьСтроку ("", "·", 28) + "конец обработки" + ДополнитьСтроку ("", "·", 28);
...
Процедура ПереименоватьПараметры()
	...
	Для каждого Парам Из ПараметрыШаблона Цикл
		...
		НовоеИмя = Объект.ЛеваяСкобкаПараметра + Сред(СтароеИмя, ДЛС+1, СтрДлина(СтароеИмя)- ДС) + Объект.ПраваяСкобкаПараметра;
...
Процедура ЗаполнитьРаскраску()
	...
	Для каждого Ф Из Фрагменты Цикл
		...
		Код = Код + НачальнаяМетка(Ф.ПП) + НачПС + СтрЗаменить(Сред(Объект.Текст, Ф.Индекс+1, Ф.Длина), Символы.ПС, КонПС+Символы.ПС+НачПС) + КонПС + КонечнаяМетка(Ф.ПП);
...
Процедура ПараметрыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Копирование Тогда
		...
		НовоеНаименованиеПриКопировании = Объект.ЛеваяСкобкаПараметра + НормализоватьИмя(НовоеНаименованиеПриКопировании) + Объект.ПраваяСкобкаПараметра;
...
Процедура ЗаполнитьВесьПроект(Команда)
	...
	Для каждого Парам Из Объект.Параметры Цикл
		...
		Код1С = Код1С + "//" + Символы.Таб + ?(Парам.Замещать, "+", "-") + ?(Парам.Извлечь, "+", "-") + Символы.Таб + Парам.Наименование + " = " + Значение + Символы.ПС;
	КонецЦикла;
...
Процедура ВключитьВПроект(Команда)
	...
	idЗамещать = "(\+|-)";
	idПусто = "[ \t]*?";
	idИмпортПараметров = "^" + idПусто + "//\t" + idЗамещать + idИзвлечь + "\t" + idЛСП + idНаименование + idПСК + " = " + idЗначениеПараметра + "$";
	...
	Для каждого Стр Из Найдено Цикл
		...
		ТекПарам.Вставить("Наименование", Объект.ЛеваяСкобкаПараметра + Стр.SubMatches(2) + Объект.ПраваяСкобкаПараметра);
		
		НовоеЗначениеПараметра = Стр.SubMatches(3);
		УдалилСкобки = Ложь;
		Если Лев(НовоеЗначениеПараметра,1) = "(" И Лев(НовоеЗначениеПараметра,3) <> "(?:" И Прав(НовоеЗначениеПараметра,1) = ")"  Тогда
		...
		Если НЕ НовыйПараметр Тогда
			...
			Если УдалилСкобки И Лев(ТекПарам.ПрежнееЗначениеПараметра,3) = "(?:" И Прав(ТекПарам.ПрежнееЗначениеПараметра,1) = ")" Тогда
	...
	СписокПараметров = ОткрытьФормуМодально("ВнешняяОбработка.РегулярныеВыражения.Форма.ИмпортВыражений", Новый Структура("СписокПараметров", СписокПараметров));
...
Функция СохранитьВариант(Знач Варианты, Значение)
	...
	Если ТакогоНет Тогда
		Структура = Новый Структура("Выражение,Форматирование,Комментарий", ТекВыражение, ТекФорматирование, ТекущаяДата());
...
Процедура ПоказатьРезультат(Команда)
	...
	"Имеем в результате такое выражение:", Объект.МаскаРезультата, СписокДляВыбора, Объект.ЛеваяСкобкаПараметра, Объект.ПраваяСкобкапараметра, Объект.ШаблонИмениПараметра, Истина);
	...
	Для каждого Парам Из Объект.Параметры Цикл
		Если ВРег(ПолноеИмяТэга) =  ВРег(Парам.Наименование) Тогда
			...
			Если ВключитьВСостав Тогда
				Если Вопрос("Такое имя существует. Включить в состав?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Нет  Тогда
					Возврат;
				КонецЕсли;
