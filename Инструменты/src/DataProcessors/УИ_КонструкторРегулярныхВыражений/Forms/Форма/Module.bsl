...
Процедура НайтиФрагментВРаскраске()
	...
	ТекФайл.НачатьЧтение(Новый ОписаниеОповещения("НайтиФрагментВРаскраскеЗавершение", ЭтаФорма, Новый Структура("ТД, ТекФайл", ТД, ТекФайл)), РаскраскаИмяФайла);
...
Процедура НайтиФрагментВРаскраскеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекФайл.УстановитьТекст(ХТМЛ);
  	ТекФайл.НачатьЗапись(Новый ОписаниеОповещения("НайтиФрагментВРаскраскеЗавершениеЗаверешениеЗаписиФайла",ЭтотОбъект,Новый Структура("ТД",ТД)),РаскраскаИмяФайла);
...
Процедура ПрочитатьИзФайлаЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ОглавлениеКниги = "";
	Текст = Новый ТекстовыйДокумент;
	Текст.НачатьЧтение(Новый ОписаниеОповещения("ПрочитатьИзФайлаЗавершениеЧтенияТекста", ЭтаФорма, Новый Структура("Текст", Текст)), Диалог.ПолноеИмяФайла);
...
Процедура ПрисвоитьИмяЗначению(Команда)
	...
	ИмяТэга = "";
	ПоказатьВводСтроки(Новый ОписаниеОповещения("ПрисвоитьИмяЗначениюЗавершение", ЭтаФорма, Новый Структура("ИмяТэга", ИмяТэга)), ИмяТэга, ВыделенныйТекст + " - имя?", 0);
	...
	Для Каждого Парам Из Объект.Параметры Цикл
		Если ВРег(ПолноеИмяТэга) = ВРег(Парам.Наименование) Тогда
			...
			Если ВключитьВСостав Тогда
				ПоказатьВопрос(Новый ОписаниеОповещения("ПрисвоитьИмяЗначениюЗавершениеЗавершение", ЭтаФорма, Новый Структура("Значение, ПолноеИмяТэга", Значение, ПолноеИмяТэга)), "Такое имя существует. Включить в состав?", РежимДиалогаВопрос.ДаНет);
                Возврат;
				Прервать;
			Иначе
...
Процедура УдалитьВсеПараметры(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("УдалитьВсеПараметрыЗавершение", ЭтаФорма), "Все параметры будут удалены. Выполнить?", РежимДиалогаВопрос.ДаНет);
