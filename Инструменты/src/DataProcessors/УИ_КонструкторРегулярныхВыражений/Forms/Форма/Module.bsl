...
Функция ПрижатьВлево(Стр, RegExpLocal = Неопределено)
	...
	Если RegExpLocal = Неопределено Тогда
		RegExpLocal = УИ_РегулярныеВыраженияКлиентСервер.КомпонентаРегулярныхВыражений();//Новый COMОбъект("VBScript.RegExp");
	КонецЕсли;
	
	
//	RegExpLocal.Multiline = Ложь;
	RegExpLocal.ВсеСовпадения = Истина;
	RegExpLocal.ИгнорироватьРегистр = Истина;
//	RegExpLocal.Pattern = "(^[ \s]*(?=[^ \s]))";
	РегВыражение="(^[ \s]*(?=[^ \s]))";
		
	//Возврат RegExpLocal.Replace (Стр, "...");
	Если Не УИ_РегулярныеВыраженияКлиентСервер.Совпадает(Стр, РегВыражение,RegExpLocal) Тогда
		Возврат Стр;
	КонецЕсли;
	
	Рез = Стр;
//	Найдено = RegExpLocal.Execute(Стр);
	Найдено = УИ_РегулярныеВыраженияКлиентСервер.НайтиСовпадения(Стр, РегВыражение, RegExpLocal);
	Для Каждого СтрНайденная Из Найдено Цикл
		
//		Длина = Стр.Length;
		Длина=СтрДлина(СтрНайденная);
		Если Длина <> 0 Тогда
...
//	RegExpLocal.Pattern = "(" + ПолучитьШаблонПараметра() + ")";
	Pattern = "(" + ПолучитьШаблонПараметра() + ")";

//	Найдено = RegExpLocal.Execute(Шаблон);
	Найдено=УИ_РегулярныеВыраженияКлиентСервер.НайтиСовпадения(Шаблон, Pattern,RegExpLocal);
...
//		Найдено = RegExpLocal.Execute(ТекШаблон);
		Найдено = УИ_РегулярныеВыраженияКлиентСервер.НайтиСовпадения(ТекШаблон, Pattern, RegExpLocal);
		Для Каждого СтрНайденная Из Найдено Цикл

//			Наименование = Стр.Value;
			Наименование = СтрНайденная;
	...
	Если Контроль <> Шаблон Тогда
		Предупреждение("В сформированном шаблоне регулярного выражения для 1С обнаружены ошибки!");
	КонецЕсли;
...
//	RegExpLocal.Multiline = Объект.Multiline;
	RegExpLocal.ВсеСовпадения = Объект.Global;
	RegExpLocal.ИгнорироватьРегистр = Объект.IgnoreCase;
//	RegExpLocal.Pattern = Объект.МаскаРезультата;
	Pattern = Объект.МаскаРезультата;
...
//			Если RegExpLocal.Test(Стр) Тогда
			Если УИ_РегулярныеВыраженияКлиентСервер.Совпадает(Стр, Pattern, RegExpLocal) Тогда
				Сообщение = "(!!)     " + Стр;
			Иначе
				Сообщение = "(#)     " + Стр;
			КонецЕсли;
...
//		Если RegExpLocal.Test(Объект.Текст + Объект.КонецТекста) Тогда
		Если УИ_РегулярныеВыраженияКлиентСервер.Совпадает(Объект.Текст + Объект.КонецТекста, Pattern, RegExpLocal) Тогда
...
//	Найдено = RegExpLocal.Execute(Объект.Текст + Объект.КонецТекста);
	ТекстПоиска=Объект.Текст + Объект.КонецТекста;
	Найдено = УИ_РегулярныеВыраженияКлиентСервер.НайтиСовпадения(ТекстПоиска, Pattern, RegExpLocal);
	НомерРез = 0;
	Для Каждого Стр Из Найдено Цикл
		НомерРез = НомерРез + 1;
		...
		//КвоПодстрок = Стр.SubMatches.Count;
		//Значение = Стр.Value;
		Начало = СтрНайти(ТекстПоиска,Стр);
		Длина = СтрДлина(Стр);
		Конец = Начало + Длина;
		КвоПодстрок = 0;//Стр.SubMatches.Count;
		Значение = Стр;
...
//			Если RegExpLocal.Test(СтрокаТекста) Тогда
			Если УИ_РегулярныеВыраженияКлиентСервер.Совпадает(СтрокаТекста, Pattern, RegExpLocal) Тогда
//				СтрокаТекста = RegExpLocal.Replace (СтрокаТекста, Объект.ТекстПодстановки);
				СтрокаТекста = УИ_РегулярныеВыраженияКлиентСервер.Заменить(СтрокаТекста, Pattern, Объект.ТекстПодстановки, RegExpLocal);
...
//		Объект.Результат = RegExpLocal.Replace (Объект.Текст + Объект.КонецТекста, Объект.ТекстПодстановки);
		Объект.Результат = УИ_РегулярныеВыраженияКлиентСервер.Заменить(Объект.Текст + Объект.КонецТекста, Pattern, Объект.ТекстПодстановки, RegExpLocal);
		Объект.Результат = Объект.Результат + ДополнитьСтроку("", "·", 28) + "конец обработки" + ДополнитьСтроку("",
			"·", 28);
	КонецЕсли;
...
Процедура МаскаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Пункт = ВыбратьИзМеню(Элемент.СписокВыбора);
...
//				RegExpLocal.Pattern = Маска.Значение;
				Pattern = Маска.Значение;

//				Если Не RegExpLocal.Test(Строка(ТекВар)) Тогда
				Если Не УИ_РегулярныеВыраженияКлиентСервер.Совпадает(Строка(ТекВар), Pattern, RegExpLocal) Тогда
					Включить = Ложь;
					Прервать;
				КонецЕсли;
			КонецЦикла;
...
//		Если RegExpLocal.Test(НоваяСтрока.Значение) Тогда
		Если УИ_РегулярныеВыраженияКлиентСервер.Совпадает(НоваяСтрока.Значение, Pattern, RegExpLocal) Тогда
...
//	Найдено = RegExpLocal.Execute(СтрокаДерева.Значение);
	Найдено = УИ_РегулярныеВыраженияКлиентСервер.НайтиСовпадения(СтрокаДерева.Значение,Pattern,RegExpLocal);
	Список = Новый СписокЗначений;

	Для Каждого Парам Из Найдено Цикл
		ДобавитьВСписокБезПовторений(Список, Парам);
	КонецЦикла;
...
Процедура ОбновитьУзел(Элемент)
	...
	Иначе
		...
		СтрокаТЧ = ПараметрыШаблона.НайтиСтроки(Условие);
		Если СтрокаТЧ.Количество() = 0 Тогда
			Сообщить("Не найден параметр по ссылке - " + Ссылка);
			Возврат;
		КонецЕсли;
		Парам = СтрокаТЧ[0];
	КонецЕсли;
...
//	Если RegExpLocal.Test(ТД.Значение) И Подчиненные.Количество() = 0 Тогда
	Если УИ_РегулярныеВыраженияКлиентСервер.Совпадает(ТД.Значение, Pattern,RegExpLocal) И Подчиненные.Количество() = 0 Тогда
...
Процедура ДеревоПараметровПередНачаломИзменения(Элемент, Отказ)
	...
	ТекЗначение = ОткрытьФормуМодально("ВнешняяОбработка.РегулярныеВыражения.Форма.РедакторВыражения", Парам, ЭтаФорма);

	Если ТекЗначение <> Неопределено Тогда

		Выражение = ТекЗначение.Выражение;
		НовыеПараметры = ТекЗначение.НовыеПараметры;
		ДобавитьНовыеПараметрыИзСписка(НовыеПараметры);

		ТД.Форматирование = ИзвлечьФорматирование(Выражение);
		ТД.Значение = Выражение;
		ТД.Варианты = ТекЗначение.Варианты;
...
//	Результат = RegExpLocal.Execute (ФорматированнаяСтрока);
	Результат = УИ_РегулярныеВыраженияКлиентСервер.НайтиСовпадения(ФорматированнаяСтрока,Pattern,RegExpLocal);
	Для Каждого Стр Из Результат Цикл
//		Поз = Стр.FirstIndex;
		Поз = СтрНайти(ФорматированнаяСтрока, Стр);
		Сим = Стр;
...
Процедура ЗаполнитьРаскраску()
	...
	Для Сч = 1 По СтрЧислоСтрок(Код) Цикл
		...
		Результат = УИ_РегулярныеВыраженияКлиентСервер.НайтиСовпадения(Рстр,Pattern,RegExpLocal);
		Для Каждого Стр Из Результат Цикл
			Таб = Стр;//Стр.SubMatches(0);
...
//	Если RegExpLocal.Test(Объект.Маска) Тогда
	Если УИ_РегулярныеВыраженияКлиентСервер.Совпадает(Объект.Маска, Pattern,RegExpLocal) Тогда
		Условие = Новый Структура("Наименование", Объект.Маска);
...
Процедура НайтиФрагментВРаскраске()
	...
	ТекФайл.Прочитать(РаскраскаИмяФайла);
...
//	RegExpLocal.Pattern = "<SPAN style=""COLOR: green"">";
	Pattern = "<SPAN style=""COLOR: green"">";
	СтрокаЗамены = "<SPAN style=""COLOR: red"">";
//	ХТМЛ = RegExpLocal.Replace (ХТМЛ, СтрокаЗамены);
	ХТМЛ = УИ_РегулярныеВыраженияКлиентСервер.Заменить(ХТМЛ,Pattern,СтрокаЗамены,RegExpLocal);
	
//	RegExpLocal.Pattern = "(" + НачальнаяМетка(ТД.ПП) + "(?:.|\n)*?" + КонечнаяМетка(ТД.ПП) + ")";
	Pattern = "(" + НачальнаяМетка(ТД.ПП) + "(?:.|\n)*?" + КонечнаяМетка(ТД.ПП) + ")";
	
//	Найдено = RegExpLocal.Execute(ХТМЛ);
	Найдено = УИ_РегулярныеВыраженияКлиентСервер.НайтиСовпадения(ХТМЛ,Pattern,RegExpLocal);
	Для Каждого Стр Из Найдено Цикл
		Значение = Стр;
		Прервать;
	КонецЦикла;
	...
	ТекФайл.Записать(РаскраскаИмяФайла);
...
Процедура ПереименоватьПараметр(Команда)
	...
	Если Не ВвестиСтроку(НовоеНаименование, "Новое наименование?") Тогда
		Возврат;
	КонецЕсли;

	НовоеНаименование = Объект.ЛеваяСкобкаПараметра + НормализоватьИмя(НовоеНаименование) + Объект.ПраваяСкобкаПараметра;

	Условие = Новый Структура("Наименование", НовоеНаименование);
	СтрокаТЧ = ПараметрыШаблона.НайтиСтроки(Условие);
	Если СтрокаТЧ.Количество() <> 0 Тогда
		Предупреждение("Такое имя уже существует, - миссия не выполнима!");
		Возврат;
	КонецЕсли;
...
Функция НормализоватьИмя(Знач Имя)
	...
	//RegExpLocal.Multiline = Ложь;
	RegExpLocal.ВсеСовпадения = Истина;
	RegExpLocal.ИгнорироватьРегистр = Ложь;
	Pattern = "( +)";

	//Имя = RegExpLocal.Replace (Имя, " ");
	Имя = УИ_РегулярныеВыраженияКлиентСервер.Заменить(Имя, Pattern," ", RegExpLocal);
...
Процедура ПараметрыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Копирование Тогда
		...
		Если Не ВвестиСтроку(НовоеНаименованиеПриКопировании, "Новое наименование?") Тогда
			Возврат;
		КонецЕсли;

		НовоеНаименованиеПриКопировании = Объект.ЛеваяСкобкаПараметра + НормализоватьИмя(
			НовоеНаименованиеПриКопировании) + Объект.ПраваяСкобкаПараметра;
...
//	Найдено = RegExpLocal.Execute(Код1С);
	Найдено = УИ_РегулярныеВыраженияКлиентСервер.НайтиСовпадения(Код1С,Pattern,RegExpLocal);
	Для Каждого Стр Из Найдено Цикл
	...
	СписокПараметров = ОткрытьФормуМодально("ВнешняяОбработка.РегулярныеВыражения.Форма.ИмпортВыражений",
		Новый Структура("СписокПараметров", СписокПараметров));
...
//	Если Не RegExpLocal.Test(Маска) Тогда
	Если Не УИ_РегулярныеВыраженияКлиентСервер.Совпадает(Маска, Pattern,RegExpLocal) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьРезультат(Команда)
	...
	ОткрытьФормуМодально("ВнешняяОбработка.РегулярныеВыражения.Форма.РедакторВыражения", Парам, ЭтаФорма);
...
Процедура ПометитьНаУдаление(Команда)
	...
	ПараметрыШаблона = Объект.Параметры;
	Для Каждого Парам Из ПараметрыШаблона Цикл
		Если Найти(Парам.Значение, Наименование) <> 0 Тогда
			УдалятьНельзя = Истина;
		КонецЕсли;
	КонецЦикла;

	Если УдалятьНельзя Тогда
		Предупреждение("Не может быть удален. Где-то используется...");
		Возврат;
	КонецЕсли;
...
Процедура ПрисвоитьИмяЗначению(Команда)
	...
	ИмяТэга = "";
	Если Не ВвестиСтроку(ИмяТэга, ВыделенныйТекст + " - имя?", 0) Тогда
		Возврат;
	КонецЕсли;
...
//			ВключитьВСостав = RegExpLocal.Test(Значение);
			ВключитьВСостав = УИ_РегулярныеВыраженияКлиентСервер.Совпадает(Значение, Pattern,RegExpLocal);

			Если ВключитьВСостав Тогда
				Если Вопрос("Такое имя существует. Включить в состав?", РежимДиалогаВопрос.ДаНет)
				...
				Прервать;
			Иначе
				Предупреждение("Такое имя уже существует.Миссия невозможна.");
				Возврат;
			КонецЕсли;
...
Функция КонтрольУникальностиПараметра(Наименование, Метка = "")
	...
	Если СтрокаТЧ.Количество() <> 0 Тогда
		...
		Если Не ПустаяСтрока(Метка) Тогда
			Предупреждение(Метка + ": продублирован параметр с именем - " + Наименование);
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьВсеПараметры(Команда)
	...
	Если Вопрос("Все параметры будут удалены. Выполнить?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;
...
Процедура СтатьяНаИнфоСтартНажатие(Элемент)
	...
	ЗапуститьПриложение("http://infostart.ru/public/592108");
