...
Процедура ПриОткрытии(Отказ)
	...
	RegExp = УИ_РегулярныеВыраженияКлиентСервер.КомпонентаРегулярныхВыражений();//Новый COMОбъект("VBScript.RegExp");
...
Функция СоздатьВариант(Знач ТекВыражение, Знач ТекКомментарий)
	...
	Если ТакогоНет Тогда
		Структура = Новый Структура("Выражение,Форматирование,Комментарий", ТекВыражение, ТекФорматирование, ТекКомментарий);
...
Функция ИмеетсяФорматирование(ФорматированнаяСтрока)
	...
	RegExp.ВсеСовпадения = Истина;
	RegExp.ИгнорироватьРегистр = Истина;
	//RegExp.Pattern = "\t|\n";
	Pattern = "\t|\n";
	
	//Возврат RegExp.Test (ФорматированнаяСтрока);
	Возврат УИ_РегулярныеВыраженияКлиентСервер.Совпадает(ФорматированнаяСтрока, Pattern,RegExp);
...
Функция ИзвлечьФорматирование(ФорматированнаяСтрока)
	...
	Результат = УИ_РегулярныеВыраженияКлиентСервер.НайтиСовпадения(ФорматированнаяСтрока, Pattern, RegExp);
	Для каждого Стр Из Результат Цикл
...
Процедура ОтформатироватьВыражение()
	...
	Для Инд = 2 По КвоСтр Цикл
		...
		Иначе
			...
			Выражение = Выражение + ?(ПустаяСтрока(Выражение), "", Символы.ПС) + ПовторитьСимвол(Символы.Таб, Сдвиг) + ТекСтр;
			Сдвиг = Сдвиг + 1;
		КонецЕсли;
	КонецЦикла;
...
Процедура Разобрать(Представление, ИмяТэга, Полное=Истина, Значение)
	...
	RegExp.IgnoreCase = Истина;
	Если Полное Тогда
		RegExp.Pattern = "^(" + ЛеваяСкобкаПараметра + ШаблонИмениПараметра
						 	 + ПраваяСкобкаПараметра + ") = (.*)$"; 
	Иначе
		RegExp.Pattern = "^" + ЛеваяСкобкаПараметра + "(" + ШаблонИмениПараметра + ")"
						 	 + ПраваяСкобкаПараметра + " = (.*)$"; 
	КонецЕсли;
...
Функция НормализоватьИмя(Знач Имя)
	...
	//RegExp.Multiline = Ложь;
	RegExp.ВсеСовпадения = Истина;
	RegExp.ИгнорироватьРегистр = Ложь;
	////RegExp.Pattern = "( +)";
	Pattern = "( +)";
		
	//Имя = RegExp.Replace (Имя, " ");
	Имя = УИ_РегулярныеВыраженияКлиентСервер.Заменить(Имя, Pattern, " ", RegExp);
