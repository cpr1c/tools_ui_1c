...
Процедура ПрисвоитьИмяЗначению(Команда)
	...
	Если ПустаяСтрока(ИмяТэга) Тогда
		ПоказатьВводСтроки(Новый ОписаниеОповещения("ПрисвоитьИмяЗначениюЗавершение", ЭтаФорма, Новый Структура("ВремИмяТэга, ВыделенныйТекст, Значение, ИмяТэга", ВремИмяТэга, ВыделенныйТекст, Значение, ИмяТэга)), ИмяТэга, ВыделенныйТекст + " - имя?", 0);
        Возврат;
	КонецЕсли;
