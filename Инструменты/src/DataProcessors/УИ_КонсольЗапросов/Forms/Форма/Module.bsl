...
Функция вЗадатьТекстЗапроса(Текст, ТекстHTML = Неопределено)
	...
	ИначеЕсли ТекстЗапроса <> Текст Тогда
		ТекстЗапроса = Текст;
		ТекстЗапросаHTML = "";
		Если Объект.РежимПодсветки Тогда
...
Процедура вПродолжитьСущестованиеЗагрузитьЗапросыИзФайла(Существует,
	...
	Если Существует Тогда
		флагСжиматьФайлы = Объект.ИспользоватьСжатиеФайлов ; //на веб-клиенте не работает
...
Процедура вОбработкаАвтосохранения()
	Если Объект.ИспользоватьАвтосохранение
			И Объект.ИнтервалАвтосохранения > 0 Тогда
...
Функция СтруктураНастроекКонсоли()
	...
	СтруктураНастроек.Вставить("ИспользоватьАвтосохранение", Объект.ИспользоватьАвтосохранение);
	СтруктураНастроек.Вставить("ИнтервалАвтосохранения", Объект.ИнтервалАвтосохранения);
	СтруктураНастроек.Вставить("ИспользоватьБэкапЗапросов", Объект.ИспользоватьБэкапЗапросов);
	СтруктураНастроек.Вставить("ИспользоватьТолькоВыделеннуюОбласть", Объект.ИспользоватьТолькоВыделеннуюОбласть);
	СтруктураНастроек.Вставить("ИспользоватьСжатиеФайлов", Объект.ИспользоватьСжатиеФайлов);
	СтруктураНастроек.Вставить("РежимПодсветки", Объект.РежимПодсветки);
...
Процедура моВосстановитьНастройки() Экспорт
	...
	Для Каждого КлючЗначение Из СтруктураНастроек Цикл
		...
		Объект[КлючЗначение.Ключ] = ЗначениеНастройки;
	КонецЦикла;
КонецПроцедуры
...
Процедура УправлениеВидимостьюПодсветкаЗапроса()
	...
	Элементы.КнопкаПодсветкаЗапроса.Пометка = Объект.РежимПодсветки;
	Элементы.ТекстЗапроса.Видимость = НЕ Объект.РежимПодсветки;
	Элементы.ТекстЗапросаHTML.Видимость = Объект.РежимПодсветки;
...
Процедура КнопкаПодсветкаЗапроса(Команда)
	Объект.РежимПодсветки = НЕ Объект.РежимПодсветки;
	Если Объект.РежимПодсветки Тогда
		ТекстЗапросаHTML = РаскраситьТекстЗапроса(ТекстЗапроса);
	КонецЕсли;
	УправлениеВидимостьюПодсветкаЗапроса();
