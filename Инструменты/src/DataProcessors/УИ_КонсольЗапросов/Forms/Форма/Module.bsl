...
Процедура ПараметрыЗапросаЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.ТипКонтейнера > 0 Тогда
		...
		ОткрытьФорму("Обработка.УИ_КонсольЗапросов.Форма." + ИмяФормыРедактирования, ПараметрыОткрытия, ЭтаФорма, Ложь, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	...
	ИначеЕсли ТипЗнч(ТекущиеДанные.Контейнер) = Тип("Структура") Тогда
		...
		Если ТекущиеДанные.Контейнер.Тип = "МоментВремени" ИЛИ ТекущиеДанные.Контейнер.Тип = "Граница" Тогда
			...
			ОткрытьФорму("Обработка.УИ_КонсольЗапросов.Форма.РедактированиеГраницыМомента", ПараметрыОткрытия, ЭтаФорма, Ложь, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		ИначеЕсли ТекущиеДанные.Контейнер.Тип = "Тип" Тогда
			...
			ОткрытьФорму("Обработка.УИ_КонсольЗапросов.Форма.РедактированиеТипа", ПараметрыОткрытия, ЭтаФорма, Истина, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	...
	Иначе
		Если ТипЗнч(ТекущиеДанные.Значение) = Тип("УникальныйИдентификатор") Тогда
			...
			ОткрытьФорму("Обработка.УИ_КонсольЗапросов.Форма.РедактированиеУникальногоИдентификатора", ПараметрыОткрытия, ЭтаФорма, Истина, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	КонецЕсли;
...
Процедура ВременныеТаблицыЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.УИ_КонсольЗапросов.Форма.РедактированиеТаблицы", ПараметрыОткрытия, ЭтаФорма, Ложь, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура РезультатЗапросаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если РезультатЗапросаКолонкиКонтейнера.Свойство(стрИмяКолонки) Тогда
		...
		Если Контейнер.Тип = "ТаблицаЗначений" Тогда
			...
			ОткрытьФорму("Обработка.УИ_КонсольЗапросов.Форма.РедактированиеТаблицы", ПараметрыОткрытия, ЭтаФорма, Ложь, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
...
//	стрИмяФайлаОбработкиНаСервере = ПолучитьИмяФайлаОбработкиНаСервере();
	...
	ОткрытьФорму("Обработка.УИ_КонсольЗапросов.Форма.Информация", ПараметрыОткрытия, ЭтаФорма, Ложь, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура Команда_ВыполнитьОбработку(Команда)
	Если УИ_ОбщегоНазначенияКлиентСервер.ПолеHTMLПостроеноНаWebkit() Тогда
		ТекстАлгоритм.УстановитьТекст(УИ_РедакторКодаКлиентСервер.ТекстРедактораИзПоляРедактора(Элементы.ПолеАлгоритмаКлиент));
	КонецЕсли;
...
Процедура Команда_ВставитьПредопределенноеЗначение(Команда)
	...
	ОткрытьФорму("Обработка.УИ_КонсольЗапросов.Форма.ВыборПредопределенного", ПараметрыОткрытия, ЭтаФорма, Истина, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура Команда_ПолучитьКодСПараметрами(Команда)
	...
	ОткрытьФорму("Обработка.УИ_КонсольЗапросов.Форма.ФормаКода", ПараметрыОткрытия, ЭтаФорма, Ложь, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
