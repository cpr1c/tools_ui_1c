...
Процедура Настройки(Команда)
	...
	ОткрытьФорму("Обработка.УИ_КонсольЗапросов.Форма.ФормаНастройкиУпр",формПараметры,ЭтаФорма,,,,оповещениеОЗакрытии);
...
Процедура ПолучитьКодПродолжить(пНазваниеЗапроса,ДополнительныеПараметры) Экспорт
	...
	ФормаКода = ПолучитьФорму("Обработка.УИ_КонсольЗапросов.Форма.ФормаКодаЗапросаУпр",,ЭтаФорма);
...
Процедура ВыполнитьЗапрос(Элемент = Неопределено, Пакетный = Неопределено, ТекстЗапроса = Неопределено)
	...
	КонецЕсли;
	ВремяСтрокой = Формат(резВыполнения.ВремяВыполнения, "ЧДЦ=3; ЧРД=.; ЧН=0; ЧГ=");
...
Функция ВыполнитьЗапросНаСервере(Пакетный,ВыполняемыйТекстЗапроса)
	...
	резВыполнения.Вставить("ВремяВыполнения",(ВремяКонцаВыполнения-ВремяНачала)/1000);
...
Процедура УправлениеВидимостьюПодсветкаЗапроса()
	...
	Элементы.ГруппаКонструкторЗапросов.ПодчиненныеЭлементы.КнопкаПодсветкаЗапроса.Пометка = Объект.РежимПодсветки_УФ;
...
Процедура РезультатТаблицаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если СодержимоеЯчейки = "ТаблицаЗначений" И ЗначениеЗаполнено(адрес) Тогда 
		ФормаВложеннойТаблицы = ПолучитьФорму("Обработка.УИ_КонсольЗапросов.Форма.ФормаВложеннойТаблицыУпр",Неопределено, ЭтаФорма);
...
Процедура ПриОткрытии(Отказ)
	...
	мФормаПараметров = ПолучитьФорму("Обработка.УИ_КонсольЗапросов.Форма.ФормаПараметровУпр", Неопределено, ЭтаФорма);
...
Процедура ВыбратьДействиеВыполнитьЗапрос(Команда)
	Элементы.ФормаДействиеПоУмолчанию.Заголовок = "Выполнить запрос";
