...
Процедура ПолучитьСписокЛицензийНаСервере()
	...
	Пока стр <> Неопределено Цикл
		...
		Если мПинЛицензия.Количество() < 2 Тогда Продолжить; КонецЕсли;
...
Функция ЗапросИнформацииОЛицезнии(ИмяЛицензии)
	...
	Пока стр <> Неопределено Цикл
		...
		ИначеЕсли СтрНайти(стр, "Middle name:") > 0 Тогда
			СтруктураОтвета.Отчество = Прав(стр, СтрДлина(стр) - СтрНайти(стр, "Middle name:") - СтрДлина("Middle name:"));
		...
		ИначеЕсли СтрНайти(стр, "Description:") > 0 Тогда
			СтруктураОтвета.Описание = Прав(стр, СтрДлина(стр) - СтрНайти(стр, "Description:") - СтрДлина("Description:"));
		...
		ИначеЕсли СтрНайти(стр, "License generation date:") > 0 Тогда
			СтруктураОтвета.ДатаАктивации = СтрокуВДату(Прав(стр, СтрДлина(стр) - СтрНайти(стр, "License generation date:") - СтрДлина("License generation date:")));
		ИначеЕсли СтрНайти(стр, "Distribution kit registration number:") > 0 Тогда
			СтруктураОтвета.РегистрационныйНомер = Прав(стр, СтрДлина(стр) - СтрНайти(стр, "Distribution kit registration number:") - СтрДлина("Distribution kit registration number:"));
		ИначеЕсли СтрНайти(стр, "Product code:") > 0 Тогда
			СтруктураОтвета.КодПродукта = Прав(стр, СтрДлина(стр) - СтрНайти(стр, "Product code:") - СтрДлина("Product code:"));
		КонецЕсли;
	КонецЦикла;
	Текст.Закрыть();
	УдалитьФайлы(ИмяВременногоФайла);
...
Процедура ПовторнаяАктивацияЛицензииНаСервере(ПереданныеПараметры)
	...
	ТекстCMD = Новый ЗаписьТекста;
	ТекстCMD.Открыть(ИмяВременногоФайлаCMD, КодировкаТекста.ANSI);
	ТекстCMD.ЗаписатьСтроку("call ring > " + ИмяВременногоФайла + " license activate"
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Имя)," --first-name " + ПереданныеПараметры.Имя, "") 
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Отчество)," --middle-name " + ПереданныеПараметры.Отчество, "")
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Фамилия)," --last-name " + ПереданныеПараметры.Фамилия, "")
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.EMail)," --email " + ПереданныеПараметры.EMail, "")
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Компания)," --company " + Символ(34) + СтрЗаменить(ПереданныеПараметры.Компания, Символ(34),"") + Символ(34), "")
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Страна)," --country " + Символ(34) + ПереданныеПараметры.Страна + Символ(34), "")
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Индекс)," --zip-code " + ПереданныеПараметры.Индекс, "")
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Город)," --town " + Символ(34) + ПереданныеПараметры.Город + Символ(34), "")
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Регион), " --region " + Символ(34) + ПереданныеПараметры.Регион + Символ(34), "")
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Район), " --district " + Символ(34) + ПереданныеПараметры.Район + Символ(34), "")  
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Улица)," --street " + Символ(34) + ПереданныеПараметры.Улица + Символ(34), "")
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Дом)," --house " + Символ(34) + ПереданныеПараметры.Дом + Символ(34), "")
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Строение), " --building " + Символ(34) + ПереданныеПараметры.Строение + Символ(34), "")
																+ ?(ЗначениеЗаполнено(ПереданныеПараметры.Квартира), " --apartment " + Символ(34) + ПереданныеПараметры.Квартира + Символ(34), "")

																+ " --serial " + ПереданныеПараметры.НомерЛицензии
																+ " --pin " + ПереданныеПараметры.НовыйПинКод
																+ " --previous-pin " + ПереданныеПараметры.ПинКод
																+ " --validate");
	ТекстCMD.Закрыть();
	ЗапуститьПриложение(ИмяВременногоФайлаCMD, КаталогВременныхФайлов(), истина);
	Текст = Новый ЧтениеТекста;
...
Процедура ПовторнаяАктивацияЛицензии(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ПослеВводаСтрокиПинкода", 
        ЭтотОбъект, СтруктураПараметров);
 
    ПоказатьВводСтроки(
        Оповещение,
        , // пропускаем начальное значение
        "Введите пин-код для лицензии " + ТекущаяСтрока["НомерЛицензии"], 
		0, // (необ.) длина
...
Процедура АктивироватьЛицензию(Команда)
	...
	ОткрытьФорму("ВнешняяОбработка.БКМ_ИнформацияОЛицензиях1С.Форма.ФормаАктивацииЛицензии", ПараметрыОткрытия);
КонецПроцедуры
