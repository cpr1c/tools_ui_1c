...
КонецПроцедуры
&НаКлиенте
Процедура ФайлыЛеваяПанельПриАктивизацииСтроки(Элемент)
...
&НаКлиенте
Процедура ФайлыПраваяПанельПриАктивизацииСтроки(Элемент)
	ТекущаяТаблицаФайлов=Элемент.Имя;
КонецПроцедуры
...
Процедура СкопироватьНаСервер(Команда)
	...
	Для Каждого ИдентификаторСтроки Из ЭлементТаблицы.ВыделенныеСтроки Цикл
		...
		СтрокаСтруктура.Вставить("АдресВХранилище",ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(СтрокаСтруктура.ПолноеИмя), УникальныйИдентификатор));
...
Процедура Удалить(Команда)
	...
	УИ_ОбщегоНазначенияКлиент.ПоказатьВопросПользователю(
		...
		Новый Структура("ЭтоЛеваяПанель,ПолноеИмя", ЭтоЛеваяПанель, ТекДанные.ПолноеИмя)),
		"Удалить выбранный файл?", РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура ПерейтиВКаталог(ТаблицаФайлов, ПолноеИмяНовогоКаталога, ЭтоЛеваяТаблица)
	...
	Если ПолноеИмяНовогоКаталога = ".." Тогда
		МассивСтрокКаталога=СтрРазделить(ТекущийКаталог, РазделительПути, Ложь);
...
Функция ПолучитьСодержимоеКаталога(Каталог, РазделительПути, ЭтоWindows)
	...
	Для Каждого Файл Из Файлы Цикл
		...
		ПолноеИмяФайла=СтрЗаменить(Файл.ПолноеИмя, РазделительПути + РазделительПути, РазделительПути) + ?(
			ЭтоКаталог, РазделительПути, "");

		Если ПолноеИмяФайла = "/./" Или ПолноеИмяФайла = "/../" Тогда
			Продолжить;
		КонецЕсли;
...
Функция НайтиВсеФайлыНаСервере(ТекущийКаталог, РазделительПути, УникальныйИдентификаторФормы)
	...
	Для Каждого Файл Из НайденныеФайлы Цикл
		...
		СтрокаСтруктура.Вставить("АдресВХранилище",ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(СтрокаСтруктура.ПолноеИмя), УникальныйИдентификаторФормы));
	КонецЦикла;
