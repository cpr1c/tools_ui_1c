...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СинонимЭтойОбработки = ОбработкаОбъект.Метаданные().Синоним;
	ВерсияОбработки = ОбработкаОбъект.Версия;	
	
	Заголовок = СтрШаблон("%1 (Версия %2)",
		СинонимЭтойОбработки,
		ВерсияОбработки);
	...
	ОтборПоХранилищамНастроек.Добавить("ХранилищеПользовательскихНастроекДинамическихСписков",
		НСтр("ru = 'Хранилище пользовательских настроек динамических списков'"));
	ОтборПоХранилищамНастроек.Добавить("ХранилищеПользовательскихНастроекОтчетов", НСтр("ru = 'Хранилище пользовательских настроек отчетов'"));	
...
Процедура ДеревоКонфигурацииПриАктивизацииСтроки(Элемент)
	...
	Элементы.ТаблицаНастроек.ОтборСтрок = ?(ТекДанные.ИдентификаторОтбора = 0,
		Неопределено,
...
Процедура ТаблицаНастроекНастройкиОткрытие(Элемент, СтандартнаяОбработка)
	...
	ПросмотрНастроекНаСервере(ТекДанные.ИмяХранилищаНастроек, ТекДанные.КлючОбъекта, ТекДанные.КлючНастроек, ВладелецНастроек);
...
Процедура ТаблицаПользователиПометкаПриИзменении(Элемент)
	...
	ТекДанные = Элементы.ТаблицаПользователи.ТекущиеДанные;
	Если ТекДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ТекДанные.Пометка И НЕ ПроверитьПользователяИБ(ТекДанные.Пользователь, Истина) Тогда
...
Процедура УдалитьВыбранныеНастройки(Команда)
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбраны настройки для удаления'"), , , , ЕстьОшибка);
	КонецЕсли; 
	...
	ПоказатьВопрос(
		Новый ОписаниеОповещения("ВопросУдалитьНастройкиЗавершение", ЭтаФорма),
		СтрШаблон(НСтр("ru = 'Удалить выбранные настройки у пользователя %1?'"), ВладелецНастроек),
...
Процедура СкопироватьВыбранныеНастройки(Команда)
	...
	НайденныеСтроки = ТаблицаНастроек.НайтиСтроки(Отбор);
	Если НайденныеСтроки.Количество() = 0 Тогда
		УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбраны настройки для копирования'"), , , , ЕстьОшибка);
	КонецЕсли; 
	НайденныеСтроки = ТаблицаПользователи.НайтиСтроки(Отбор);
	Если НайденныеСтроки.Количество() = 0 Тогда
		УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указаны пользователи (кому копировать)'"), , , , ЕстьОшибка);
	КонецЕсли; 
	...
	ПоказатьВопрос(
		Новый ОписаниеОповещения("ВопросСкопироватьНастройкиЗавершение", ЭтаФорма),
		НСтр("ru = 'Скопировать выбранные настройки выбранным пользователям?'"),
		РежимДиалогаВопрос.ДаНет,
		,
		КодВозвратаДиалога.Нет,
		НСтр("ru = 'Внимание!'")
...
Процедура ВопросСкопироватьНастройкиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Успешно = СкопироватьВыбранныеНастройкиНаСервере();
	
	Если Успешно Тогда
		ПоказатьПредупреждение(
			,
			НСтр("ru = 'Копирование настроек выполнено'")
			);
	КонецЕсли; 
...
Процедура ОбновитьТаблицуПользователей()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И НЕ Пользователи.Ссылка = &Ссылка
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Пользователи.Наименование";
	
	Запрос.УстановитьПараметр("Ссылка", ВладелецНастроек);
	
	ЗначениеВРеквизитФормы(Запрос.Выполнить().Выгрузить(), "ТаблицаПользователи");
...
Процедура ОбновитьОтображениеЭлементов(ЭлементыУправления = Неопределено)
	...
	Если МассивЭУ.Количество() = 0 ИЛИ МассивЭУ.Найти("ПоказатьВыбранныеНастройки") <> Неопределено Тогда
		...
		Элементы.ДеревоКонфигурацииПоказатьВыбранныеНастройки.Заголовок = ?(Элементы.ГруппаВыбранныеНастройки.Видимость,
			НСтр("ru = 'Скрыть выбранные настройки'"),
			НСтр("ru = 'Показать выбранные настройки'")
			);
	КонецЕсли;
		
	Если МассивЭУ.Количество() = 0 ИЛИ МассивЭУ.Найти("ПоказатьВыбранныхПользователей") <> Неопределено Тогда
		Элементы.ТаблицаПользователи.ОтборСтрок = ?(Элементы.ТаблицаПользователиПоказатьВыбранныхПользователей.Пометка,
			Новый ФиксированнаяСтруктура("Пометка", Истина),
			Неопределено
			);
		Элементы.ТаблицаПользователиПоказатьВыбранныхПользователей.Заголовок = ?(Элементы.ТаблицаПользователи.ОтборСтрок <> Неопределено,
			НСтр("ru = 'Показать всех'"),
			НСтр("ru = 'Показать выбранных'")
			);
	КонецЕсли;	
...
Процедура ОбновитьНастройкиВладельцаНаСервере()
	...
	Если ПроверитьПользователяИБ(ВладелецНастроек, Истина, ПользовательИБ) Тогда
		
		Отбор = Новый Структура("Пользователь", ПользовательИБ.Имя);
...
Процедура ДополнитьДеревоНастроек(Выборка, ИмяХранилищаНастроек, ДополнительныеПараметры)
	...
	Пока Выборка.Следующий() Цикл
		...
		ПутьОбъектаКонфигурации = ?(МассивКлюч[0] = "Общее" И МассивКлюч[1] = "ИсторияПоискаТаблицы", МассивКлюч[2], МассивКлюч[0]);				
...
Функция ИнициализироватьПараметрыДляСозданияДереваКонфигурации(ТаблицаЗначенийНастроек)
	...
	ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.Вставить("ПланВидовХарактеристик", "ПланВидовХарактеристик");
...
Процедура СоздатьВеткуКонфигурации(СтрокаРодитель, ДополнительныеПараметры, Уровень = 0)
	...
	Если Уровень = 0 И 
		(ДополнительныеПараметры.МассивПуть[Уровень] = "ОбщаяФорма" 
			ИЛИ ДополнительныеПараметры.МассивПуть[Уровень] = "ХранилищеНастроек" 
			ИЛИ ДополнительныеПараметры.МассивПуть[Уровень] = "ПланОбмена"
			ИЛИ ДополнительныеПараметры.МассивПуть[Уровень] = "Подсистема"
		)
		Тогда
...
Функция ЗаполнитьСтрокуДереваКонфигурации(СтрокаДерева, ДополнительныеПараметры, Индекс)
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.Подсистема Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.ХранилищеНастроек Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.ПланОбмена Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.ОбщаяФорма Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.Константа Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.Справочник Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.Документ Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.ЖурналДокументов Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.Перечисление Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.Отчет Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.ВнешнийОтчет Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.Обработка Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.ВнешняяОбработка Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.ПланВидовХарактеристик Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.ПланСчетов Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.ПланВидовРасчета Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.РегистрСведений Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.РегистрНакопления Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.РегистрБухгалтерии Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.РегистрРасчета Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.БизнесПроцесс Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.Задача Тогда
	...
	ИначеЕсли ДополнительныеПараметры.МассивПуть[Индекс] = ДополнительныеПараметры.ПредопределенныеВеткиКонфигурации.ВнешнийИсточникДанных Тогда
	...
	Если Индекс > 0 И СтрокаДерева.Родитель.ФормироватьПредставлениеПодчиненныхСтрокИзСинонимаМетаданных Тогда
		ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(СтрокаДерева.Родитель.КлассОбъектовМетаданных + "." + ДополнительныеПараметры.МассивПуть[Индекс]);
...
Процедура УдалитьВыбранныеНастройкиНаСервере()
	...
	ПользовательИБ = Неопределено;
	Если НЕ ПроверитьПользователяИБ(ВладелецНастроек, Истина, ПользовательИБ) Тогда
		Возврат;
	КонецЕсли; 
	...
	Для каждого Строка Из НайденныеСтроки Цикл
		...
		ХрНастроек.Удалить(Строка.КлючОбъекта, Строка.КлючНастроек, ПользовательИБ.Имя);
	КонецЦикла; 
	
КонецПроцедуры

&НаСервере
Функция СкопироватьВыбранныеНастройкиНаСервере()
	...
	ТЗ = РеквизитФормыВЗначение("ТаблицаПользователи");
	ВыбранныеПользователи = ТЗ.НайтиСтроки(Отбор);
	
	ТЗ = РеквизитФормыВЗначение("ТаблицаНастроек");
	ВыбранныеНастроки = ТЗ.НайтиСтроки(Отбор);
	
	Отбор = Новый Структура;
	Отбор.Вставить("КлючОбъекта", "");
	Отбор.Вставить("КлючНастроек", "");		
	...
	Для каждого СтрокаПользователь Из ВыбранныеПользователи Цикл
		Для каждого СтрокаНастройка Из ВыбранныеНастроки Цикл
			...
			Отбор.КлючОбъекта = СтрокаНастройка.КлючОбъекта;
			Отбор.КлючНастроек = СтрокаНастройка.КлючНастроек;
			ВыборкаНастроекИсточника = ХрНастроек.Выбрать(Отбор);
			ВыборкаНастроекИсточника.Следующий();
			
			ПользовательИБ = Неопределено;
			ПроверитьПользователяИБ(СтрокаПользователь.Пользователь, Истина, ПользовательИБ);
			
			// Скопировать настройки новому пользователю
			ХрНастроек.Сохранить(
				СтрокаНастройка.КлючОбъекта,
				СтрокаНастройка.КлючНастроек,
				ВыборкаНастроекИсточника.Настройки,
				СтрокаНастройка.Представление,
				ПользовательИБ.Имя
				);
...
&НаСервере
Функция ПроверитьПользователяИБ(ПользовательСсылка, ПоказатьСообщениеОбОшибке = Ложь, ПользовательИБ_Возврат = "Пусто")
	...
	ПользовательИБ = ПользователиИнформационнойБазы.НайтиПоУникальномуИдентификатору(
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПользовательСсылка, "ИдентификаторПользователяИБ")
		);
		
	Если НЕ ПользовательИБ_Возврат = "Пусто" Тогда
		ПользовательИБ_Возврат = ПользовательИБ;
	КонецЕсли; 
	
	Если ПоказатьСообщениеОбОшибке И ПользовательИБ = Неопределено Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Пользователь %1 не найден в списке пользователей информационной базы'"),
			ПользовательСсылка
			);
		УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли; 
	
	Возврат (НЕ ПользовательИБ = Неопределено);
	
КонецФункции

&НаСервере
Процедура ПросмотрНастроекНаСервере(ИмяХранилищаНастроек, КлючОбъекта, КлючНастроек, ПользовательСсылка)

	ПользовательИБ = Неопределено;
	Если НЕ ПроверитьПользователяИБ(ПользовательСсылка, Истина, ПользовательИБ) Тогда
		Возврат;
	КонецЕсли; 
