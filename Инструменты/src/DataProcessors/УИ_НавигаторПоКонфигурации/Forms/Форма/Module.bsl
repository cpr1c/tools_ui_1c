...
#Иначе
		...
		СтрокаЗапуска = СтрШаблон(СтрокаЗапуска + " /IBConnectionString %1 /N""%2""",
			СтрокаСоединенияИнформационнойБазы(), ИмяПользователя());
	...
	Если ТекДанные <> Неопределено Тогда
		Если ТекДанные.ВидУзла = "ОбъектМД" И Не вЭтоПрочаяКоманда(ТекДанные.ПолноеИмя) Тогда
			...
			Если НадоОбработать Тогда
				...
				ОткрытьФорму("Обработка.УИ_ДинамическийСписок.Форма", СтрукПараметры, , ТекДанные.ПолноеИмя, , , ,
					РежимОткрытияОкнаФормы.Независимый);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
