...
Процедура ВыгрузитьДанныеМассиваСсылок(МассивСсылок, СтрокаИмяДляЗапроса, ЗаписьXML, ТолькоПроверкаНедопустимыхСимволов = Ложь, ОбъектыВыгруженныеСОшибками = Неопределено,ТаблицаОтбора1)
...
//
Процедура ВыгрузкаДанных(ЗаписьXML, ТолькоПроверкаНедопустимыхСимволов = Ложь, ОбъектыВыгруженныеСОшибками = Неопределено,ТаблицаОтбора1)
	...
	Попытка
		...
		Для Каждого СтрокаТаблицыВыгрузки Из СоставПолнойВыгрузки Цикл
			...
			Если Метаданные.Константы.Содержит(СтрокаДереваМетаданных.ОбъектМД) Тогда
				...
				ЗаписьКонстанты(ЗаписьXML, СтрокаДереваМетаданных.ОбъектМД, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов,ТаблицаОтбора1);
			...
			ИначеЕсли Метаданные.РегистрыСведений.Содержит(СтрокаДереваМетаданных.ОбъектМД) Или
				...
				ЗаписьРегистра(ЗаписьXML, СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов,,ТаблицаОтбора1);
				
			ИначеЕсли Метаданные.РегистрыБухгалтерии.Содержит(СтрокаДереваМетаданных.ОбъектМД) Тогда
				
				ЗаписьРегистра(ЗаписьXML, СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов, Истина,ТаблицаОтбора1);
				
			ИначеЕсли ТипЗнч(СтрокаДереваМетаданных.ЭлементОписания.Менеджер) = Тип("Строка") Тогда
				// специальный случай для перерасчетов
				ЗаписьПерерасчета(ЗаписьXML, СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов,ТаблицаОтбора1);
				
			ИначеЕсли Метаданные.Последовательности.Содержит(СтрокаДереваМетаданных.ОбъектМД) Тогда
				
				ЗаписьПоследовательности(ЗаписьXML, СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов,ТаблицаОтбора1);
				
			Иначе
				
				ЗаписьДанныхОбъектногоТипа(СтрокаДереваМетаданных, ЗаписьXML, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов,ТаблицаОтбора1);
		...
		Для Каждого СтрокаТаблицыВыгрузки Из ДополнительныеОбъектыДляВыгрузки Цикл
			...
			Если ТекущееИмяЗапроса <> СтрокаТаблицыВыгрузки.ИмяОбъектаДляЗапроса Тогда
				...
				ВыгрузитьДанныеМассиваСсылок(ТекущийМассивСсылок, ТекущееИмяЗапроса, ЗаписьXML, ТолькоПроверкаНедопустимыхСимволов, ОбъектыВыгруженныеСОшибками,ТаблицаОтбора1);
...
// Для внутреннего использования
//
Функция ПолучитьТекстЗапросаДляРегистраСведений(ИмяМетаданных, ОбъектМетаданных, ЕстьДопОтборы, СтрокаПолейДляВыборки = "",СтрокаПолейДляВыборки1 = "",ТаблицаОтбора1)
	...
	ТекстЗапроса = "ВЫБРАТЬ Разрешенные " + СтрокаПолейДляВыборки + " ИЗ " + ИмяМетаданных + " КАК ТаблицаОбъекта_"+ОбъектМетаданных.ИМЯ;
	...
	ИначеЕсли ТипВыгрузкиПериодическихРегистров = 1 Тогда
		...
		ТекстЗапроса = "ВЫБРАТЬ Разрешенные " + СтрокаПолейДляВыборки + " ИЗ " + ИмяМетаданных + ".СрезПоследних(&ДатаОкончания) КАК ТаблицаОбъекта_"+ОбъектМетаданных.ИМЯ;	
		
	ИначеЕсли ТипВыгрузкиПериодическихРегистров = 2 Тогда
		
		ТекстЗапроса = "ВЫБРАТЬ Разрешенные " + СтрокаПолейДляВыборки + " ИЗ " + ИмяМетаданных + ".СрезПервых(&ДатаНачала) КАК ТаблицаОбъекта_"+ОбъектМетаданных.ИМЯ;
		
	ИначеЕсли ТипВыгрузкиПериодическихРегистров = 3 Тогда
		
			ТекстЗапроса = "ВЫБРАТЬ Разрешенные " + СтрокаПолейДляВыборки1 + " ИЗ " + ИмяМетаданных + ".СрезПоследних(&ДатаНачала) КАК ТаблицаОбъекта_"+ОбъектМетаданных.ИМЯ+"1
			|
			|Объединить все
			|
			|ВЫБРАТЬ " + СтрокаПолейДляВыборки + " ИЗ " + ИмяМетаданных + " КАК ТаблицаОбъекта_"+ОбъектМетаданных.ИМЯ+"
			|";
...
Функция ПолучитьТекстЗапросаДляРегистра(ИмяМетаданных, ОбъектМетаданных, ЕстьДопОтборы, СтрокаПолейДляВыборки = "",ТаблицаОтбора1)
	
	ЕстьОграничениеПоДатам = (ЗначениеЗаполнено(ДатаНачала) Или ЗначениеЗаполнено(ДатаОкончания)) и ИспользоватьОтборПоДатеДляВсехОбъектов;
...
// Для внутреннего использования
//
Функция ПолучитьТекстЗапросаПоСтроке(СтрокаДереваМетаданных, ЕстьДопОтборы, СтрокаПолейДляВыборки = "",СтрокаПолейДляВыборки1 = "",ТаблицаОтбора1) Экспорт
	...
	Если Метаданные.РегистрыСведений.Содержит(ОбъектМетаданных) Тогда
		...
		ТекстЗапроса = ПолучитьТекстЗапросаДляРегистраСведений(ИмяМетаданных, ОбъектМетаданных, ЕстьДопОтборы, СтрокаПолейДляВыборки,СтрокаПолейДляВыборки1,ТаблицаОтбора1);
		Возврат ТекстЗапроса;
	...
	ИначеЕсли   Метаданные.РегистрыНакопления.Содержит(ОбъектМетаданных)
		...
		ТекстЗапроса = ПолучитьТекстЗапросаДляРегистра(ИмяМетаданных, ОбъектМетаданных, ЕстьДопОтборы, СтрокаПолейДляВыборки,ТаблицаОтбора1);
		Возврат ТекстЗапроса;
	...
	ТекстЗапроса = "ВЫБРАТЬ Разрешенные "+ СтрокаПолейДляВыборки + " ИЗ " + ИмяМетаданных + " КАК ТаблицаОбъекта_"+СтрокаДереваМетаданных.ПолноеИмяМетаданных;
	...
	Для Каждого Строка ИЗ ТаблицаОтбора1 Цикл
		Если СтрокаДереваМетаданных.ПолноеИмяМетаданных=Строка.имяреквизита И СтрокаДереваМетаданных.Родитель.Метаданные=Строка.ИмяОбъектаМетаданных Тогда
			Для Каждого СтрокаЭлементы ИЗ Строка.Отбор.Элементы Цикл
				Если СтрокаЭлементы.Использование  Тогда
...
Функция ПолучитьВидСравненияВзапрос(Строка,СтрокаЭлементы,ВидСравнения)
	...
	Если ВидСравнения=ВидСравненияКомпоновкиданных.Больше тогда
		ВозвращЗначение="ТаблицаОбъекта_"+Строка.ИмяРеквизита+"."+СтрокаЭлементы.ЛевоеЗначение+">&"+ЛевоеЗначение+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.БольшеИлиРавно Тогда 	
		ВозвращЗначение="ТаблицаОбъекта_"+Строка.ИмяРеквизита+"."+СтрокаЭлементы.ЛевоеЗначение+">=&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.ВИерархии Тогда 	
		ВозвращЗначение="ТаблицаОбъекта_"+Строка.ИмяРеквизита+"."+СтрокаЭлементы.ЛевоеЗначение+" В ИЕРАРХИИ(&"+ЛевоеЗначение+")"+ Символы.ПС;
    ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.ВСписке Тогда 	
		ВозвращЗначение="ТаблицаОбъекта_"+Строка.ИмяРеквизита+"."+СтрокаЭлементы.ЛевоеЗначение+" В (&"+ЛевоеЗначение+")"+ Символы.ПС;
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеВИерархии Тогда 	
	    ВозвращЗначение=" НЕ "+"ТаблицаОбъекта_"+Строка.ИмяРеквизита+"."+СтрокаЭлементы.ЛевоеЗначение+" В ИЕРАРХИИ(&"+ЛевоеЗначение+")"+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеВСписке Тогда 	
		ВозвращЗначение=" НЕ "+"ТаблицаОбъекта_"+Строка.ИмяРеквизита+"."+СтрокаЭлементы.ЛевоеЗначение+" В (&"+ЛевоеЗначение+")"+ Символы.ПС;
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеСодержит Тогда 	
		ВозвращЗначение=" НЕ "+"ТаблицаОбъекта_"+Строка.ИмяРеквизита+"."+СтрокаЭлементы.ЛевоеЗначение+" ПОДОБНО &"+ЛевоеЗначение+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.Подобно Тогда 	
		ВозвращЗначение="ТаблицаОбъекта_"+Строка.ИмяРеквизита+"."+СтрокаЭлементы.ЛевоеЗначение+" ПОДОБНО &"+ЛевоеЗначение+ Символы.ПС;
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.Заполнено Тогда 
		ВозвращЗначение="ТаблицаОбъекта_"+Строка.ИмяРеквизита+"."+СтрокаЭлементы.ЛевоеЗначение+" ЕСТЬ NULL "+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НЕЗаполнено Тогда 
		ВозвращЗначение=" НЕ "+"ТаблицаОбъекта_"+Строка.ИмяРеквизита+"."+СтрокаЭлементы.ЛевоеЗначение+" ЕСТЬ NULL "+ Символы.ПС;
	КонецЕсли;
...
Функция ПолучитьВидСравненияВзапросКонстанта(Строка,СтрокаЭлементы,ВидСравнения)
	...
	Если ВидСравнения=ВидСравненияКомпоновкиданных.Больше тогда
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+">&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.БольшеИлиРавно Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+">=&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.ВИерархии Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+" В ИЕРАРХИИ(&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+")"+ Символы.ПС;
    ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.ВСписке Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+" В (&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+")"+ Символы.ПС;
    ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.ВСпискеПоИерархии Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+" В ИЕРАРХИИ(&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+")"+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.Меньше Тогда 	
	    ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+"<&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
    ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.МеньшеИлиРавно Тогда 	
	    ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+"<=&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеВИерархии Тогда 	
	    ВозвращЗначение=" НЕ "+Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+" В ИЕРАРХИИ(&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+")"+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеВСписке Тогда 	
		ВозвращЗначение=" НЕ "+Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"" В (&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+")"+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеВСпискеПоИерархии Тогда 	
	    ВозвращЗначение=" НЕ "+Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+" В ИЕРАРХИИ(&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+")"+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеРавно Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+"<>&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеСодержит Тогда 	
		ВозвращЗначение=" НЕ "+Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+" ПОДОБНО &"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.Подобно Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+" ПОДОБНО &"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеПодобно Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+" ПОДОБНО &"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.Равно Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+".Значение"+"=&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
...
Функция ПолучитьВидСравненияВзапросРегистр(Строка,СтрокаЭлементы,ВидСравнения)
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.БольшеИлиРавно Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+">=&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.ВИерархии Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+" В ИЕРАРХИИ(&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+")"+ Символы.ПС;
    ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.ВСписке Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+" В (&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+")"+ Символы.ПС;
    ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.ВСпискеПоИерархии Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+" В ИЕРАРХИИ(&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+")"+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.Меньше Тогда 	
	    ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+"<&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
    ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.МеньшеИлиРавно Тогда 	
	    ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+"<=&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеВИерархии Тогда 	
	    ВозвращЗначение=" НЕ "+СтрокаЭлементы.ЛевоеЗначение+" В ИЕРАРХИИ(&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+")"+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеВСписке Тогда 	
		ВозвращЗначение=" НЕ "+Строка(СтрокаЭлементы.ЛевоеЗначение)+" В (&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+")"+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеВСпискеПоИерархии Тогда 	
	    ВозвращЗначение=" НЕ "+Строка(СтрокаЭлементы.ЛевоеЗначение)+" В ИЕРАРХИИ(&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+")"+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеРавно Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+"<>&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеСодержит Тогда 	
		ВозвращЗначение=" НЕ "+Строка(СтрокаЭлементы.ЛевоеЗначение)+" ПОДОБНО &"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.Подобно Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+" ПОДОБНО &"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.НеПодобно Тогда 	
		ВозвращЗначение=" НЕ "+Строка(СтрокаЭлементы.ЛевоеЗначение)+" ПОДОБНО &"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиданных.Равно Тогда 	
		ВозвращЗначение=Строка(СтрокаЭлементы.ЛевоеЗначение)+"=&"+Строка(СтрокаЭлементы.ЛевоеЗначение)+ Символы.ПС;
...
// Для внутреннего использования
//
Функция ПодготовитьПостроительДляВыгрузки(СтрокаДереваМетаданных, СтрокаПолейДляВыборки = "",СтрокаПолейДляВыборки1 = "",ТаблицаОтбора1) Экспорт
	
	ЕстьДопОтборы = (СтрокаДереваМетаданных.НастройкиПостроителя <> Неопределено); 
	
	ИтоговыйТекстЗапроса = ПолучитьТекстЗапросаПоСтроке(СтрокаДереваМетаданных, ЕстьДопОтборы, СтрокаПолейДляВыборки,СтрокаПолейДляВыборки1,ТаблицаОтбора1);
	...
	Для Каждого СтрокаТабл ИЗ ТаблицаОтбора1 Цикл
		Если СтрокаДереваМетаданных.Метаданные.Имя=СтрокаТабл.ИмяРеквизита И  СтрокаТабл.ИмяОбъектаМетаданных=СтрокаДереваМетаданных.Родитель.Метаданные Тогда
			Для Каждого СтрокаЭлемент ИЗ СтрокаТабл.Отбор.Элементы Цикл
				...
				Если СтрокаЭлемент.ВидСравнения=ВидСравненияКомпоновкиДанных.Содержит ИЛИ СтрокаЭлемент.ВидСравнения=ВидСравненияКомпоновкиДанных.НеСодержит Тогда
...
Процедура ЗаписьДанныхОбъектногоТипа(СтрокаДереваМетаданных, ЗаписьXML, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов = Ложь,ТаблицаОтбора1)
...
//
Процедура ЗаписьКонстанты(ЗаписьXML, МД_Константа, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов,ТаблицаОтбора1)
	...
	Для Каждого Строка ИЗ ТаблицаОтбора1 Цикл
		Если  МД_Константа.ИМЯ=Строка.имяреквизита Тогда
			...
			Для Каждого СтрокаЭлементы ИЗ Строка.Отбор.Элементы Цикл
				Если СтрокаЭлементы.Использование Тогда
					Если Не Первая Тогда
						ТекстЗапроса = ТекстЗапроса + Символы.ПС+" И "+ ПолучитьВидСравненияВЗапросКонстанта(Строка,СтрокаЭлементы,СтрокаЭлементы.ВидСравнения);
					Иначе
						ТекстЗапроса = ТекстЗапроса + Символы.ПС+" ГДЕ "+ ПолучитьВидСравненияВЗапросКонстанта(Строка,СтрокаЭлементы,СтрокаЭлементы.ВидСравнения);
					КонецЕсли;
					Первая=Ложь;
				КонецЕсли;
			КонецЦикла;
...
//
Процедура ЗаписьРегистра(ЗаписьXML, СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов, РегистрБухгалтерии = Ложь,ТаблицаОтбора1)
	...
	ЗаписьЧерезНаборЗаписей(ЗаписьXML, МенеджерНабораЗаписей, ИмяТаблицыДляЗапроса, 
		СтрокаДереваМетаданных.ОбъектМД.Имя, СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов, РегистрБухгалтерии,ТаблицаОтбора1);
...
//   СтрокаДереваМетаданных - строка дерева метаданных, соответствующая регистру
//
Процедура ЗаписьПерерасчета(ЗаписьXML, СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов,ТаблицаОтбора1)
...
//   СтрокаДереваМетаданных - строка дерева метаданных, соответствующая регистру
//
Процедура ЗаписьПоследовательности(ЗаписьXML, СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов,ТаблицаОтбора1)
	
	МенеджерНабораЗаписей = СтрокаДереваМетаданных.ЭлементОписания.Менеджер[СтрокаДереваМетаданных.ОбъектМД.Имя];
	
	ЗаписьЧерезНаборЗаписей(ЗаписьXML, МенеджерНабораЗаписей, СтрокаДереваМетаданных.ЭлементОписания.ДляЗапроса, 
		СтрокаДереваМетаданных.ОбъектМД.Имя, СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов,,ТаблицаОтбора1);		
...
Процедура ЗаписьЧерезНаборЗаписей(ЗаписьXML, МенеджерНабораЗаписей, ДляЗапроса, ИмяОбъекта, 
	СтрокаДереваМетаданных = Неопределено, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов, РегистрБухгалтерии = Ложь,ТаблицаОтбора1)
				...
				Для Каждого СтрокаЭлементы ИЗ Строка.Отбор.Элементы Цикл
					Если СтрокаЭлементы.Использование Тогда
						...
						Если Строка(ТипЗнч(СтрокаЭлементы.ПравоеЗначение))="Стандартная дата начала" Тогда
							Запрос.УстановитьПараметр(Строка(СтрокаЭлементы.ЛевоеЗначение),СтрокаЭлементы.ПравоеЗначение.Дата);
						Иначе
							Запрос.УстановитьПараметр(Строка(СтрокаЭлементы.ЛевоеЗначение),СтрокаЭлементы.ПравоеЗначение);
						КонецЕсли;
	...
	ПостроительОтчета = ПодготовитьПостроительДляВыгрузки(СтрокаДереваМетаданных, СтрокаПолейОтбора,СтрокаПолейОтбора1,ТаблицаОтбора1); 
	ПостроительОтчета.Выполнить();
