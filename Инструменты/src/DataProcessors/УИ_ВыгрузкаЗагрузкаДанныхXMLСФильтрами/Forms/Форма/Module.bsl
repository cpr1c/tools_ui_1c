...
&НаКлиенте
Процедура ИмяФайлаВыгрузкиНачалоВыбора(Элемент, ДанныеВыбора,
...
&НаКлиенте
Процедура ИмяФайлаЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора,
		СтандартнаяОбработка)
...
&НаКлиенте
Процедура ДополнительныеОбъектыДляВыгрузкиПередНачаломДобавления(Элемент,
		Отказ, Копирование, Родитель, Группа)
...
Процедура ВыгрузитьДанные(Команда)
	...
	Если РежимРаботыНаКлиенте
			И Не ПустаяСтрока(АдресФайлаВоВременномХранилище) Тогда
...
Процедура ЗагрузитьДанные(Команда)
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда
		...
		Для Каждого ИмяФайла Из МассивФайлов Цикл
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ЗагрузитьДанныеЗавершение", ЭтотОбъект);
			НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаВоВременномХранилище, ВыбФайл.ПолноеИмя, Истина, ЭтаФорма.УникальныйИдентификатор);
		...
		КонецЦикла;
	Иначе
		Сообщ1 = Новый СообщениеПользователю();
		Сообщ1.Текст = НСтр("ru = 'Файл(ы) не выбран!'; en = 'File(s) not selected!'");
		Сообщ1.Сообщить();
	КонецЕсли;
	...
	//	КонецЕсли;
	//	
	//	ЗагрузитьДанныеЗавершение(АдресФайлаВоВременномХранилище, ИмяФайлаЗагрузки);
	//	
	//КонецЕсли;
КонецПроцедуры
...
Функция ИмяФормыНастроекКонсолиЗапросов()
	...
	Обработка = РеквизитФормыВЗначение("Объект");
	ИмяФормыНастроек = Обработка.Метаданные().ПолноеИмя()
		+ ".Форма.НастройкиКонсолиЗапросов";
...
&НаСервере
Процедура ПроставитьПометкиВыгружаемыхДанных(СтрокиИсходногоДерева,
		СтрокиЗаменяемогоДерева)
...
&НаКлиенте
Процедура ЗагрузитьДанныеЗавершение(Параметр1, Параметр2) Экспорт

	Если НЕ Параметр1.ПомещениеФайлаОтменено Тогда
		Состояние(Нстр("ru = 'Выполняется загрузка данных. Пожалуйста, подождите...'"));

		ЗагрузитьДанныеНаСервере(Параметр1.Адрес);
	КонецЕсли;
...
Процедура ИнициализироватьСКД()
	...
	Для Каждого ОбъектВыбора Из РезультатВыбора Цикл
		...
		Если ОбъектВыбора.Значение = "РегистрыСведений"
				или ОбъектВыбора.Значение = "РегистрыНакопления"
			...
			Для Каждого ОбъектМетаданных Из Метаданные[ОбъектВыбора.Значение][Строка(ОбъектВыбора)].СтандартныеРеквизиты Цикл
		...
		ИначеЕсли ОбъектВыбора.Значение <> "Константы"
				И ОбъектВыбора.Значение <> "Последовательности" Тогда
		...
		Если ОбъектВыбора.Значение = "Задачи" Тогда
			...
			КоличествоРеквизитов = КоличествоРеквизитов
				+ Метаданные[ОбъектВыбора.Значение][Строка(ОбъектВыбора)].РеквизитыАдресации.Количество();
		...
		Если ОбъектВыбора.Значение = "РегистрыСведений"
				или ОбъектВыбора.Значение = "РегистрыНакопления"
				или ОбъектВыбора.Значение = "РегистрыБухгалтерии" Тогда
			КоличествоРеквизитов = Метаданные[ОбъектВыбора.Значение][Строка(ОбъектВыбора)].СтандартныеРеквизиты.Количество()
				+ Метаданные[ОбъектВыбора.Значение][Строка(ОбъектВыбора)].СтандартныеРеквизиты.Количество()
				+ Метаданные[ОбъектВыбора.Значение][Строка(ОбъектВыбора)].Измерения.Количество()
				+ Метаданные[ОбъектВыбора.Значение][Строка(ОбъектВыбора)].Ресурсы.Количество();
...
&НаСервере
Процедура ПодготовитьСписокВыбранныхОбъектов(ТекущиеДанныеИмя,
	...
	ВосстановитьОтборИзКэша(Объект.Компоновщик.Настройки.Отбор.Элементы, КэшОтбора, Объект.Компоновщик.Настройки.Отбор.ДоступныеПоляОтбора);
...
&НаСервере
Процедура ПодготовитьСписокВыбранных(СписокВыбора, ТекущиеДанныеИмя,
		ИмяОбъектаМетаданных)
	...
	//Если ЭлементСтрока.Выгружать>0 Тогда
		Для Каждого ЭлементМетаданных Из ЭлементСтрока.Строки Цикл
			Если ЭлементМетаданных.ПолноеИмяМетаданных = ТекущиеДанныеИмя
					И ЭлементМетаданных.ИмяОбъектаМетаданных = ИмяОбъектаМетаданных Тогда
						...
						Для Каждого СтрокаИзмерения ИЗ МетадатаИзмерения Цикл
							Если СтрокаИзмерения.Имя = Строка(СтрокаОтбора.ЛевоеЗначение) Тогда
								...
								Для Каждого СтрокаОтбораТаблица ИЗ ТаблицаОтбора1 Цикл
									Если СтрокаОтбораТаблица.ИмяРеквизита = ВетвьДерева.ПолноеИмяМетаданных Тогда
										...
										Если НЕ НайденоРеквизит Тогда
											...
											НовоеПоле = СтрокаОтбораТаблица.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
								...
								//НоваяСтрока.ИмяРеквизита=ТекущиеДанныеИмя;
									...
									НовоеПоле = НоваяСтрока.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
									ЗаполнитьЗначенияСвойств(НовоеПоле, СтрокаОтбора);
						...
						Если Найти(Строка(СтрокаОтбора.ЛевоеЗначение), ".") > 0 Тогда
							...
							Для Каждого СтрокаРеквизиты ИЗ Стандартныереквизиты Цикл
								Если Лев(СтрокаОтбора.ЛевоеЗначение, Найти(Строка(СтрокаОтбора.ЛевоеЗначение), ".")
									...
									Если НайтиРеквизитВДеревеРевизитовМетаданных(Метаданные[СтрокаРодитель.ПолноеИмяМетаданных][ВетвьДерева.ПолноеИмяМетаданных], Лев(СтрокаОтбора.ЛевоеЗначение, Найти(Строка(СтрокаОтбора.ЛевоеЗначение), ".")
...
&наСервере
Функция НайтиРеквизитВДеревеРевизитовМетаданных(Метаданные1, Реквизит2,
		Строка1)
	...
	Для Каждого Строка ИЗ СтандартныеРеквизиты1 Цикл
		Если Строка.имя = Реквизит2 Тогда
			...
			Если НЕ (Строка(Строка.Тип.Типы()[0]) = "Строка"
					ИЛИ Строка(Строка.Тип.Типы()[0]) = "Дата"
					ИЛИ Строка(Строка.Тип.Типы()[0]) = "Число"
					ИЛИ Строка(Строка.Тип.Типы()[0]) = "Булево") Тогда
...
&НаСервере
Процедура ОбновитьОтборПоАктивизацииНаСервере(ТекущиеДанныеИмя,
		ИмяОбъектаМетаданных);
	...
	Для Каждого Строка ИЗ ТаблицаОтбора1 Цикл
		Если Строка.ИмяРеквизита = ТекущиеданныеИмя
				И Строка.ИмяОбъектаМетаданных = ИмяОбъектаМетаданных Тогда
...
&НаКлиенте
Процедура ОтборПриОкончанииРедактирования(Элемент, НоваяСтрока,
		ОтменаРедактирования)
		// Вставить содержимое обработчика.
	Если Не ОтменаРедактирования Тогда

		Выделить = ОбновитьТаблицуОтборанаСервере(Элементы.ДеревоМетаданных.ТекущиеДанные.ПолноеИмяМетаданных, Элементы.ДеревоМетаданных.ТекущиеДанные.ИмяОбъектаМетаданных);
		...
		Если Выделить и НЕ Элементы.ДеревоМетаданных.ТекущиеДанные.Выделить Тогда
			ЭтаФорма.Элементы.ДеревоМетаданных.ТекущиеДанные.ИндексКартинки = ЭтаФорма.Элементы.ДеревоМетаданных.ТекущиеДанные.ИндексКартинки
				+ 1;
		ИначеЕСЛИ Не (Выделить
				и Элементы.ДеревоМетаданных.ТекущиеДанные.Выделить) Тогда
			ЭтаФорма.Элементы.ДеревоМетаданных.ТекущиеДанные.ИндексКартинки = ЭтаФорма.Элементы.ДеревоМетаданных.ТекущиеДанные.ИндексКартинки
				- 1;
		КонецЕсли;
...
&НаСервере
Процедура ВосстановитьОтборИзКэша(ЭлементыОтбора, КэшОтбора,
		ОтборДоступныеПоляОтбора)
...
&НаСервере
Функция ОбновитьТаблицуОтборанаСервере(ПолноеИмяМетаданных,
		ИмяОбъектаМетаданных)
	...
	Пока Колво > 0 Цикл
		...
		Если Строка.ИмяРеквизита = ПолноеИмяМетаданных
				И Строка.ИмяОбъектаМетаданных = ИмяОбъектаМетаданных Тогда
...
Процедура УдалитьПредопределенные(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Внимание! после удаления всех предопределенных элементов будет нарушена структура данных и   
		...
		+ " en = 'Warning! after the deleting predefined elements  structure of the data will be broken 
