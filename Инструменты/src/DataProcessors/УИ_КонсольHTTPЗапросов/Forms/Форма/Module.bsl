...
// Пока закоментим сохранение в JSON, т.к. библиотека ошибки выдает на двоичных данных
	...
	Формат.Имя="Файл запросов консоли HTPP в JSON (*.jhttp)";
	Формат.Расширение="jhttp";
	Структура.Форматы.Добавить(Формат);

	Формат=УИ_ОбщегоНазначенияКлиент.ПустоеОписаниеФорматаВыбираемогоФайла();
	Формат.Имя="Файл запросов консоли HTPP (*.xhttp)";
	Формат.Расширение="xhttp";
	Структура.Форматы.Добавить(Формат);

//	Формат=УИ_ОбщегоНазначенияКлиент.ПустоеОписаниеФорматаВыбираемогоФайла();
//	Формат.Имя="Все файлы (*.*)";
//	Формат.Расширение="";
//	Структура.Форматы.Добавить(Формат);

//	Структура.СериализуемыеФорматыФайлов.Добавить("jhttp");
...
Процедура РедактироватьТелоЗапросаВРедактореJSON(Команда)
	УИ_ОбщегоНазначенияКлиент.РедактироватьJSON(ТелоЗапроса, Ложь,Новый ОписаниеОповещения("РедактироватьТелоЗапросаВРедактореJSONЗавершение", ЭтотОбъект));
КонецПроцедуры
