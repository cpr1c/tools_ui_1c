...
Процедура УстановитьПараметрыДинамическогоСпискаНаСервере(ВыбраннаяСтрока)
	...
	Для Каждого ТекРеквизит Из ТекДанные.Реквизиты Цикл
		...
		Исключение
			Сообщить("Не удалось создать колонку списка для реквизита "
				+ ТекРеквизит.Имя);
		КонецПопытки;
	КонецЦикла;
...
Функция МассивТаблицРеквизитовДляОбъектаМетаданных(ОбъектМетаданных)
	...
	Если УИ_ОбщегоНазначения.ЭтоРегистрСведений(ОбъектМетаданных)
			ИЛИ УИ_ОбщегоНазначения.ЭтоРегистрНакопления(ОбъектМетаданных)
			ИЛИ УИ_ОбщегоНазначения.ЭтоРегистрРасчета(ОбъектМетаданных)
...
Процедура ДобавитьВДеревоВидОбъекта(Картинка, ИмяТипаМетаданных,
	...
	Для каждого мдОбъекта из Метаданные[ИмяТипаМетаданных] Цикл
		СинонимОбъектаМетаданных = ?(ПустаяСтрока(мдОбъекта.Синоним), "", " ("
			+ мдОбъекта.Синоним + ")");
