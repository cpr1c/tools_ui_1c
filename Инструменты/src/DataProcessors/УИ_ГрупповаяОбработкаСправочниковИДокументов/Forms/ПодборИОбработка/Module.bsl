...
Процедура ОткрытьФормуВыбораТаблицы()
	...
	ОткрытьФорму(ПолучитьПолноеИмяФормы("ФормаВыбораТаблиц"),СтруктураПараметров,ЭтотОбъект,,,,Новый ОписаниеОповещения("ОткрытьФормуВыбораТаблицыЗавершение", ЭтотОбъект),РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
// () 
&НаКлиенте
Процедура ТабличноеПолеВидыОбъектовПередНачаломДобавления(Элемент, Отказ,
...
Функция СформироватьУсловиеПоискаПоСтроке()
	...
	Если СтрокаПоиска <> "" Тогда
		...
		ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ИскомыйОбъект.Тип + "."
			+ ИскомыйОбъект.Имя);
		...
		Если ИскомыйОбъект.Тип = "Справочник" Тогда
			...
			Если ОбъектМетаданных.ДлинаКода <> 0
					И ОбъектМетаданных.ТипКода = Метаданные.СвойстваОбъектов.ТипКодаСправочника.Строка Тогда
				Если УсловиеПоискаПоСтроке <> "" Тогда
					УсловиеПоискаПоСтроке = УсловиеПоискаПоСтроке + " ИЛИ ";
				КонецЕсли;
		...
		Для Каждого Реквизит Из ОбъектМетаданных.Реквизиты Цикл
			Если Реквизит.Тип.СодержитТип(Тип("Строка")) Тогда
				Если УсловиеПоискаПоСтроке <> "" Тогда
					УсловиеПоискаПоСтроке = УсловиеПоискаПоСтроке + " ИЛИ ";
				КонецЕсли;
				УсловиеПоискаПоСтроке = УсловиеПоискаПоСтроке + Реквизит.Имя
					+ " ПОДОБНО ""%" + СтрокаДляПоиска + "%""";
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьТекстЗапроса()
	...
	ТекстЗапроса = ТекстЗапроса + "	" + ИскомыйОбъект.Тип + "."
		+ ОбъектМетаданных.Имя + " КАК _Таблица" + Символы.ПС;
...
&НаКлиенте
Процедура ОбъектПоискаОбработкаВыбора(Элемент, ВыбранноеЗначение,
	...
	Иначе
		...
		Если ОтборДанных <> Неопределено Тогда
			Для Каждого ЭлементОтбора Из ОтборДанных.Отбор.Элементы Цикл
				...
				ИмяПараметра = СтрЗаменить(Строка(ЭлементОтбора.ЛевоеЗначение)
					+ Индекс, ".", "");

				СписокУсловий = СписокУсловий + ?(СписокУсловий = "", "", "
					|	И ")
					+ ОбработкаОбъект.ПолучитьВидСравнения(ЭлементОтбора.ЛевоеЗначение, ЭлементОтбора.ВидСравнения, ИмяПараметра);
...
Процедура ВыполнитьОбработку(Команда)
	Для каждого Строка из ВыбранныеОбработки Цикл
		...
		Если НЕ ОбработкаДоступна(?(Объект.ТипОбъекта = 0, "Справочник", "Документ"), ИмяФормыОбработки) Тогда
			Сообщить("Обработка " + ИмяФормы + " недоступна для типа <"
				+ ОбъектПоиска.Тип + ">");
			Продолжить;
		КонецЕсли;
...
&НаСервере
Процедура СоздатьКолонки(ТЗ,
		МассивРеквизитовПоУмолчанию = Неопределено) Экспорт
	...
	Для Каждого Колонка Из ТЗ.Колонки Цикл
		...
		Если Колонка.Имя = "Представление"
				ИЛИ Найти(ТипКолонки, "Хранилище значения") > 0 Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого Реквизит Из МассивРеквизитов Цикл
		...
		Элемент = Элементы.Добавить(ТаблицаЭлемент.Имя
			+ Реквизит.Имя, Тип("ПолеФормы"), ТаблицаЭлемент);
...
Процедура Отбор(Команда)
	...
	ОткрытьФорму(ПолучитьПолноеИмяФормы("ФормаОтбора"), СтруктураПараметров, ЭтотОбъект, , , , Новый ОписаниеОповещения("ОтборЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
&НаКлиенте
Процедура ДоступныеОбработкиВыбор(Элемент, ВыбраннаяСтрока, Поле,
		СтандартнаяОбработка)
	Если ТабличноеПолеВидыОбъектов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Если Родитель = Неопределено Тогда
		Если НЕ ОбработкаДоступна(?(Объект.ТипОбъекта = 0, "Справочник", "Документ"), ТекущаяСтрока.ИмяФормы) Тогда
			ПоказатьПредупреждение(,"Данная обработка недоступна для типа <"
				+ ?(Объект.ТипОбъекта = 0, "Справочник", "Документ") + ">");
			Возврат;
		КонецЕсли;
	...
	ОткрытьФорму(ИмяФормыДляОткрытия,СтруктураПараметров,ЭтотОбъект,,,,Новый ОписаниеОповещения("ДоступныеОбработкиВыборЗавершение", ЭтотОбъект),РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
&НаКлиенте
Процедура ДоступныеОбработкиПередНачаломДобавления(Элемент, Отказ, Копирование,
		Родитель, Группа)
	Если ТабличноеПолеВидыОбъектов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Если Элемент.ТекущиеДанные.ПолучитьРодителя() = Неопределено Тогда
		Если Копирование Тогда
			Отказ = Истина;
		Иначе
			Если НЕ ОбработкаДоступна(?(Объект.ТипОбъекта = 0, "Справочник", "Документ"), Элемент.ТекущиеДанные.ИмяФормы) Тогда
				ПоказатьПредупреждение(,"Данная обработка недоступна для типа <"
					+ ?(Объект.ТипОбъекта = 0, "Справочник", "Документ") + ">");
				Отказ = Истина;
				Возврат;
		...
		КонецЕсли;
	Иначе
		Если НЕ ОбработкаДоступна(?(Объект.ТипОбъекта = 0, "Справочник", "Документ"), Элемент.ТекущиеДанные.ПолучитьРодителя().ИмяФормы) Тогда
			ПоказатьПредупреждение(,"Данная обработка недоступна для типа <"
				+ ?(Объект.ТипОбъекта = 0, "Справочник", "Документ") + ">");
			Отказ = Истина;
			Возврат;
...
Процедура ДоступныеОбработкиПередУдалением(Элемент, Отказ)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ДоступныеОбработкиПередУдалениемЗавершение", ЭтаФорма, Новый Структура("ТекущаяСтрока", Элемент.ТекущаяСтрока)), "Удалить настройку?", РежимДиалогаВопрос.ОКОтмена, , КодВозвратаДиалога.ОК);
КонецПроцедуры
...
&НаКлиенте
Процедура ДоступныеОбработкиНачалоПеретаскивания(Элемент,
...
&НаКлиенте
Процедура ВыбранныеОбработкиПеретаскивание(Элемент, ПараметрыПеретаскивания,
...
&НаКлиенте
Процедура ДоступныеОбработкиПриОкончанииРедактирования(Элемент, НоваяСтрока,
...
Процедура ТабличноеПолеВидыОбъектовПередУдалением(Элемент, Отказ)
	Отказ=Истина;
	ПроверкаНеобходимостиОчищатьРезультаты(Новый ОписаниеОповещения("ТабличноеПолеВидыОбъектовПередУдалениемЗавершение",ЭтотОбъект,Новый Структура("ТекущаяСтрока",Элементы.ТабличноеПолеВидыОбъектов.ТекущаяСтрока)));
КонецПроцедуры
...
Процедура ТабличноеПолеВидыОбъектовПередУдалениемЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТабличноеПолеВидыОбъектов.Удалить(ТекСтрока);
...
Процедура ИнициализацияЗапроса()
	...
	Для каждого Строка Из ТабличноеПолеВидыОбъектов Цикл
		...
		Если Объект.ТипОбъекта = 1 Тогда
			Если МетаданныеОбъектов[ИмяВида].ДлинаНомера > 0 Тогда
				СтруктураРеквизитовШапки.Вставить("Номер", ?(СтруктураРеквизитовШапки.Свойство("Номер", ПрошлоеЗначение), ПрошлоеЗначение
					+ 1, 1));
			КонецЕсли;
			...
			Если МетаданныеОбъектов[ИмяВида].ТипНомера = Метаданные.СвойстваОбъектов.ТипНомераДокумента.Строка Тогда
			...
			СтруктураРеквизитовШапки.Вставить("Проведен", ?(СтруктураРеквизитовШапки.Свойство("Проведен", ПрошлоеЗначение), ПрошлоеЗначение
				+ 1, 1));
		...
		Иначе
			Если МетаданныеОбъектов[ИмяВида].ДлинаКода > 0 Тогда
				...
				Отбор.Вставить("ЭтоТЧ", Ложь);
				МассивСтрок = ТаблицаРеквизитов.НайтиСтроки(Отбор);
				Если МетаданныеОбъектов[ИмяВида].ТипКода = Метаданные.СвойстваОбъектов.ТипКодаСправочника.Строка Тогда
					ТекТип = ОписаниеТипа("Строка");
				Иначе
					ТекТип = ОписаниеТипа("Число");
				КонецЕсли;
			...
			Если МетаданныеОбъектов[ИмяВида].ДлинаНаименования > 0 Тогда
				СтруктураРеквизитовШапки.Вставить("Наименование", ?(СтруктураРеквизитовШапки.Свойство("Наименование", ПрошлоеЗначение), ПрошлоеЗначение
					+ 1, 1));
		...
		Для каждого РеквизитМетаданного Из МетаданныеОбъектов[ИмяВида].Реквизиты Цикл
			...
			СтруктураРеквизитовШапки.Вставить(РеквизитМетаданного.Имя, ?(СтруктураРеквизитовШапки.Свойство(РеквизитМетаданного.Имя, ПрошлоеЗначение), ПрошлоеЗначение
				+ 1, 1));
			...
			СтрокаРеквизитов.Тип = Новый ОписаниеТипов(УсечьМассив(СтрокаРеквизитов.Тип.Типы(), РеквизитМетаданного.Тип.Типы()));
		...
		Если Объект.ОбрабатыватьТабличныеЧасти Тогда
			...
			Для каждого РеквизитМетаданного Из МетаданныеРеквизитовТЧ Цикл
				...
				СтруктураРеквизитовТЧ.Вставить(РеквизитМетаданного.Имя, ?(СтруктураРеквизитовТЧ.Свойство(РеквизитМетаданного.Имя, ПрошлоеЗначение), ПрошлоеЗначение
					+ 1, 1));
			...
			КонецЦикла;
		КонецЕсли;
		СтруктураТиповОбъектов.Вставить(МетаданныеОбъектов[ИмяВида].Имя, Тип(ИмяТипаТаблицы
			+ "Ссылка." + ИмяВида));
	КонецЦикла;
	...
	КонтрольОстатковНоменклатуры = (Объект.ТипОбъекта = 0)
		И (ИмяВидаОдногоТипа = "Номенклатура")
		И ВКонфигурацииЕстьОстаткиНоменклатуры;
		//
	ДоступностьВВебПриложенииНоменклатуры = ВКонфигурацииЕстьУправлениеЗаказами
		И (ОБъект.ТипОбъекта = 0) И (ИмяВидаОдногоТипа = "Номенклатура");
	...
	СписокПредставлений.Добавить("Вид " + ИмяТипаТаблицы
		+ "а", "Ш_ВидПредставление");
	...
	СписокПредставлений.Добавить(Префикс
		+ "Пометка удаления", "Ш_ПометкаУдаления");
	...
	Для каждого КлючИЗначение Из СтруктураРеквизитовШапки Цикл
		...
		Если Не МетаданныеРеквизита = Неопределено Тогда
			СписокПредставлений.Добавить(Префикс
				+ МетаданныеРеквизита.Представление(), КлючИЗначение.Значение);
		Иначе
			СписокПредставлений.Добавить(Префикс
	...
	Для каждого Строка Из ТабличноеПолеВидыОбъектов Цикл
		...
		ТекстЗапросаОбъект = "";
		ТекстЗапросаОбъект = ТекстЗапросаОбъект + "" + Символы.ПС + "	""" + ИмяВида
			+ """ КАК Ш_Вид";
		ТекстЗапросаОбъект = ТекстЗапросаОбъект + "," + Символы.ПС + "	"""
			+ СтрЗаменить(МетаданныеОбъектов[ИмяВида].Представление(), """", "")
			+ """ КАК Ш_ВидПредставление";
		ТекстЗапросаОбъект = ТекстЗапросаОбъект + "," + Символы.ПС + "	"
			+ ПсевдонимТаблицы + "." + Префикс + "Ссылка КАК Объект";

		Для каждого КлючИЗначение Из СтруктураРеквизитовШапки Цикл
			МетаданноеРеквизита = МетаданныеРеквизитов.Найти(КлючИЗначение.Ключ);
			Если Не МетаданноеРеквизита = Неопределено
					И МетаданноеРеквизита.Тип.СодержитТип(Тип("Строка"))
					и МетаданноеРеквизита.Тип.КвалификаторыСтроки.Длина = 0 Тогда
				ТекстЗапросаОбъект = ТекстЗапросаОбъект + "," + Символы.ПС + "	ПОДСТРОКА("
					+ ПсевдонимТаблицы + "." + Префикс + КлючИЗначение.Ключ + ",1,"
					+ ОграничениеНаСтрокиНеограниченнойДлины + ")";
			Иначе
				ТекстЗапросаОбъект = ТекстЗапросаОбъект + "," + Символы.ПС + "	"
					+ ПсевдонимТаблицы + "." + Префикс + КлючИЗначение.Ключ;
			КонецЕсли;
		...
		Если Объект.ОбрабатыватьТабличныеЧасти Тогда
			...
			ТекстЗапросаОбъект = ТекстЗапросаОбъект + "," + Символы.ПС + "	""" + ИмяТЧ
				+ """ КАК Т_ТЧ";
			ТекстЗапросаОбъект = ТекстЗапросаОбъект + "," + Символы.ПС + "	"""
				+ МетаданныеОбъектов[ИмяВида].ТабличныеЧасти[ИмяТЧ].Представление()
				+ """ КАК Т_ТЧПредставление";
			...
			Для каждого КлючИЗначение Из СтруктураРеквизитовТЧ Цикл
				...
				Если Не МетаданноеРеквизита = Неопределено
					...
					ТекстЗапросаОбъект = ТекстЗапросаОбъект + "," + Символы.ПС + "	ПОДСТРОКА("
						+ ПсевдонимТаблицы + "." + КлючИЗначение.Ключ + ",1,"
						+ ОграничениеНаСтрокиНеограниченнойДлины + ")";

				Иначе

					ТекстЗапросаОбъект = ТекстЗапросаОбъект + "," + Символы.ПС + "	"
						+ ПсевдонимТаблицы + "." + КлючИЗначение.Ключ;
		...
		ТекстЗапросаОбъект = ТекстЗапросаОбъект + Символы.ПС + "ИЗ" + Символы.ПС + "	"
		...
		Если Объект.ТипОбъекта = 0 И МетаданныеОбъектов[ИмяВида].Иерархический
				И МетаданныеОбъектов[ИмяВида].ВидИерархии = Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов Тогда
		...
		ТекстЗапроса = ?(ТекстЗапроса = "", "Выбрать ", ТекстЗапроса + Символы.ПС
			+ Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС + Символы.ПС + "ВЫБРАТЬ")
			+ ТекстЗапросаОбъект;
	...
	НовыйТекстЗапроса = "Выбрать РАЗРЕШЕННЫЕ * ИЗ (" + ТекстЗапроса
		+ ") КАК _Таблица";
...
Процедура ПроверкаНеобходимостиОчищатьРезультаты(ОписаниеОповещенияОЗавершении)
	...
	Если Не мСформированныйРежим = Неопределено
			И мСформированныйРежим.ДанныеОтобраны Тогда
		ВопросОбОчисткеРезультатаОтбора(Новый ОписаниеОповещения("ПроверкаНеобходимостиОчищатьРезультатыЗавершение",ЭтотОбъект,ДополнительныеПараметрыОповщения));		
	КонецЕсли;
...
Процедура ВопросОбОчисткеРезультатаОтбора(ОписаниеОповещенияОЗавершении) Экспорт
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ВопросОбОчисткеРезультатаОтбораЗавершение", ЭтаФорма, Новый Структура("ОписаниеОповещенияОЗавершении", ОписаниеОповещенияОЗавершении)), "Результат отбора будет очищен. Продолжить?", РежимДиалогаВопрос.ОКОтмена);
КонецПроцедуры
...
Процедура ТабличноеПолеВидыОбъектовПослеУдаления(Элемент)
	ПроверкаНеобходимостиОчищатьРезультаты(Новый ОписаниеОповещения("ТабличноеПолеВидыОбъектовПослеУдаленияЗавершение",ЭтотОбъект));
КонецПроцедуры
...
Процедура ОбрабатыватьТабличныеЧастиПриИзменении(Элемент)
	ПроверкаНеобходимостиОчищатьРезультаты(Новый ОписаниеОповещения("ОбрабатыватьТабличныеЧастиПриИзмененииЗавершение",ЭтотОбъект,Новый Структура("Элемент",Элемент)));
КонецПроцедуры
