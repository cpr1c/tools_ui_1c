...
Процедура ОткрытьФормуВыбораТаблицы()
	...
	ФормаОтбора = ПолучитьФорму(ПолучитьПолноеИмяФормы("ФормаВыбораТаблиц"), СтруктураПараметров, ЭтаФорма);
	...
	Для каждого Значение Из РезультатОтбора Цикл
...
Процедура ДоступныеОбработкиВыбор(Элемент, ВыбраннаяСтрока, Поле,
	...
	Если Родитель = Неопределено Тогда
		Если НЕ ОбработкаДоступна(?(Объект.ТипОбъекта = 0, "Справочник", "Документ"), ТекущаяСтрока.ИмяФормы) Тогда
			Предупреждение("Данная обработка недоступна для типа <"
