...
Процедура ТабличноеПолеВидыОбъектовПередУдалением(Элемент, Отказ)
	Отказ=Истина;
	ПроверкаНеобходимостиОчищатьРезультаты(Новый ОписаниеОповещения("ТабличноеПолеВидыОбъектовПередУдалениемЗавершение",
		ЭтотОбъект, Новый Структура("ТекущаяСтрока", Элементы.ТабличноеПолеВидыОбъектов.ТекущаяСтрока)));
КонецПроцедуры
...
Процедура ИнициализацияЗапроса()
	...
	Для Каждого Строка Из ТабличноеПолеВидыОбъектов Цикл
		...
		Если Объект.ТипОбъекта = 0 И МетаданныеСтрокиОбъектов.Иерархический
			И МетаданныеСтрокиОбъектов.ВидИерархии = Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов Тогда
