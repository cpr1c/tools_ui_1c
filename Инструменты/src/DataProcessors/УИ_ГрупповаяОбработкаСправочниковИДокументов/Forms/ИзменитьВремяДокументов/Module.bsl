...
Процедура ОбработатьОбъект(Ссылка, ПорядковыйНомерОбъекта, ПараметрыЗаписиОбъектов)
	...
	Если ВидИзмененияВремени = "ВНачалоДня" Тогда
		Объект.Дата = НачалоДня(Ссылка.Дата);
	ИначеЕсли ВидИзмененияВремени = "ВКонецДня" Тогда
		Объект.Дата = КонецДня(Ссылка.Дата);
	Иначе
		Объект.Дата = НачалоДня(Ссылка.Дата) + (ВведенноеВремя - '00010101');
	КонецЕсли;
...
Процедура ТекущаяНастройкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Не ТекущаяНастройка = ВыбранноеЗначение Тогда
		...
		Если ЭтаФорма.Модифицированность Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("ТекущаяНастройкаОбработкаВыбораЗавершение", ЭтаФорма, Новый Структура("ВыбранноеЗначение", ВыбранноеЗначение)), "Сохранить текущую настройку?", РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
            Возврат;
		КонецЕсли;
...
&НаКлиенте
Процедура ВидИзмененияВремениПриИзменении(Элемент)
	УстановитьДоступность();
КонецПроцедуры

&НаКлиенте
Процедура УстановитьДоступность()
	Элементы.ВведенноеВремя.Видимость=ВидИзмененияВремени = "ВвестиВручную";
КонецПроцедуры
...
мНастройка.Вставить("ВидИзмененияВремени", "ВКонецДня");
мНастройка.Вставить("ВведенноеВремя");
