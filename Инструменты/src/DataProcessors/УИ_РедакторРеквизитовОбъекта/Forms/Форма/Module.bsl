...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОписаниеОповщения, "Записать сделанные изменения перед закрытием?",
			РежимДиалогаВопрос.ДаНетОтмена, 15, КодВозвратаДиалога.Да, , КодВозвратаДиалога.Отмена);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ПередЗакрытиемЗаписатьПослеОтветаНаВопрос(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
//@skip-warning
&НаКлиенте
Процедура Подключаемый_ТабличнаяЧастьПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
...
//@skip-warning
&НаКлиенте
Процедура Подключаемый_РеквизитШапкиПриИзменении(Элемент)
	//Установим признак измененности
	ИмяЭлементаФормы=Элемент.Имя;
	Для Каждого СтруктураРеквизита Из РеквизитыОбъекта.Реквизиты Цикл
		Если ИмяЭлементаФормы <> СтруктураРеквизита.ИмяРеквизитаФормы Тогда
			Продолжить;
		КонецЕсли;

		Если СтруктураРеквизита.ЕстьХранилищеЗначений Тогда
			СтруктураРеквизита.Изменен=Истина;
		ИначеЕсли (Истина) Тогда
			СтруктураРеквизита.Изменен=СтруктураРеквизита.НачальноеЗначение <> ЭтотОбъект[ИмяЭлементаФормы];
		КонецЕсли;

		Если СтруктураРеквизита.Изменен Тогда
			Элемент.ЦветТекстаЗаголовка = WebЦвета.СветлоГрифельноСиний;
		Иначе
...
Процедура ЗаполнитьДанныеШапкиОбъектаНаФорме()
	Для Каждого Реквизит Из РеквизитыОбъекта.Реквизиты Цикл
		...
		Реквизит.НачальноеЗначение = ЭтотОбъект[Реквизит.ИмяРеквизитаФормы];
	КонецЦикла;
КонецПроцедуры
...
&НаСервере
Процедура ПрочитатьСпецифическиеРеквизитыДвиженияРегистраБухгалтерии(ОписаниеРегистра, СтрокаТаблицыФормы,
	ЗаписьРегистра, МассивПропускаемыхРеквизитов)
	...
	Для Каждого КлючЗначение Из СоответствиеСчетовИВидовСубконто Цикл
		Для НомерСубконто = 1 По ОписаниеРегистра.МаксКоличествоСубконто Цикл
			...
			МассивПропускаемыхРеквизитов.Добавить(ВРег(КлючЗначение.Значение + НомерСубконто));
		КонецЦикла;
...
Процедура ПрочитатьДвиженияРегистра(ОписаниеРегистра, ТаблицаФормы)
	...
	Для Каждого Запись Из НаборЗаписей Цикл
		...
		Если ЭтоРегистрБухгалтерии Тогда
			ПрочитатьСпецифическиеРеквизитыДвиженияРегистраБухгалтерии(ОписаниеРегистра, НоваяСтрока, Запись,
				МассивПропускаемыхРеквизитов);
		КонецЕсли;
...
&НаСервере
Процедура ДобавитьРеквизитВКоллекциюРеквизитов(КоллекцияРеквизитов, Имя, Тип, Заголовок, ИмяРеквизитаФормы,
	...
	НовыйРеквизит.Вставить("Изменен", Ложь);
	НовыйРеквизит.Вставить("НачальноеЗначение", Неопределено);

	НовыйРеквизит.Вставить("ЕстьХранилищеЗначений", Тип.СодержитТип(Тип("ХранилищеЗначения")));
...
&НаСервере
Процедура ДобавитьТабличнуюЧастьВКоллекциюРеквизитов(Коллекция, Имя, Заголовок, ИмяРеквизитаФормы)
...
Функция ОписаниеРегистраДвиженийДокумента(МетаданныеРегистра)
	...
	ОписаниеРегистра.Вставить("ИмяРеквизитаФормы", "" + ОписаниеРегистра.ТипРегистра + "_" + МетаданныеРегистра.Имя);
...
Процедура ДобавитьРеквизитыСубконтоРегистраДвиженияВДобавляемыеРеквизитыФормы(ЭтоРегистрБухгалтерии,
	...
	Для НомерСубконто = 1 По РегистрМетаданные.ПланСчетов.МаксКоличествоСубконто Цикл
		Если РегистрМетаданные.Корреспонденция Тогда
			...
			НовыйРеквизит = Новый РеквизитФормы(ИмяРеквизита, РегистрМетаданные.ПланСчетов.ВидыСубконто.Тип,
				ОписаниеРегистра.ИмяРеквизитаФормы, ТекСиноним + НомерСубконто + "(Дт)", Истина);
			МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);

			ДобавитьРеквизитВКоллекциюРеквизитов(ОписаниеРегистра.Реквизиты, НовыйРеквизит.Имя,
				НовыйРеквизит.ТипЗначения, НовыйРеквизит.Заголовок, НовыйРеквизит.Имя, , Ложь);

			ИмяРеквизита = ТекИмя + "Кт" + НомерСубконто;

			НовыйРеквизит = Новый РеквизитФормы(ИмяРеквизита, РегистрМетаданные.ПланСчетов.ВидыСубконто.Тип,
				ОписаниеРегистра.ИмяРеквизитаФормы, ТекСиноним + НомерСубконто + "(Кт)", Истина);
			МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);
		...
		Иначе
			...
			НовыйРеквизит = Новый РеквизитФормы(ИмяРеквизита, РегистрМетаданные.ПланСчетов.ВидыСубконто.Тип,
				ОписаниеРегистра.ИмяРеквизитаФормы, ТекСиноним + НомерСубконто, Истина);
			МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);
...
Процедура ДобавитьРеквизитыСчетовРегистраДвиженияВДобавляемыеРеквизитыФормы(ЭтоРегистрБухгалтерии,
	...
	НовыйРеквизит = Новый РеквизитФормы(ИмяРеквизита, Новый ОписаниеТипов("ПланСчетовСсылка."
		+ РегистрМетаданные.ПланСчетов.Имя), ОписаниеРегистра.ИмяРеквизитаФормы, ТекСиноним + "(Дт)", Истина);
	МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);

	ДобавитьРеквизитВКоллекциюРеквизитов(ОписаниеРегистра.Реквизиты, НовыйРеквизит.Имя, НовыйРеквизит.ТипЗначения,
		НовыйРеквизит.Заголовок, НовыйРеквизит.Имя, , Ложь);

	ИмяРеквизита = ТекИмя + "Кт";

	НовыйРеквизит = Новый РеквизитФормы(ИмяРеквизита, Новый ОписаниеТипов("ПланСчетовСсылка."
		+ РегистрМетаданные.ПланСчетов.Имя), ОписаниеРегистра.ИмяРеквизитаФормы, ТекСиноним + "(Кт)", Истина);
	МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);
...
&НаСервере
Процедура ДобавитьРеквизитыРегистраДвиженияВДобавляемыеРеквизитыФормы(МассивДобавляемыхРеквизитов, ОписаниеРегистра,
	РегистрМетаданные, ЭтоРегистрБухгалтерии, МассивСтандартныхРеквизитовНеТребующихДобавления)
	...
	Для Каждого ТекРеквизит Из РегистрМетаданные.СтандартныеРеквизиты Цикл
		...
		НовыйРеквизит = Новый РеквизитФормы(ТекРеквизит.Имя, ТекРеквизит.Тип, ОписаниеРегистра.ИмяРеквизитаФормы,
			ТекРеквизит.Синоним, Истина);
		МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);

		ДобавитьРеквизитВКоллекциюРеквизитов(ОписаниеРегистра.Реквизиты, ТекРеквизит.Имя, ТекРеквизит.Тип,
			ТекРеквизит.Синоним, ТекРеквизит.Имя);

	КонецЦикла;

	ДобавитьРеквизитыСчетовРегистраДвиженияВДобавляемыеРеквизитыФормы(ЭтоРегистрБухгалтерии,
		МассивДобавляемыхРеквизитов, РегистрМетаданные, ОписаниеРегистра);

	// Измерения
	Для Каждого ТекРеквизит Из РегистрМетаданные.Измерения Цикл

		Если ЭтоРегистрБухгалтерии И Не ТекРеквизит.Балансовый И РегистрМетаданные.Корреспонденция Тогда
			ИмяРеквизита = ТекРеквизит.Имя + "Дт";

			НовыйРеквизит = Новый РеквизитФормы(ИмяРеквизита, ТекРеквизит.Тип, ОписаниеРегистра.ИмяРеквизитаФормы,
				ТекРеквизит.Синоним + "(Дт)", Истина);
			МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);

			ДобавитьРеквизитВКоллекциюРеквизитов(ОписаниеРегистра.Реквизиты, НовыйРеквизит.Имя,
				НовыйРеквизит.ТипЗначения, НовыйРеквизит.Заголовок, НовыйРеквизит.Имя, , ТекРеквизит.Балансовый);

			ИмяРеквизита = ТекРеквизит.Имя + "Кт";

			НовыйРеквизит = Новый РеквизитФормы(ИмяРеквизита, ТекРеквизит.Тип, ОписаниеРегистра.ИмяРеквизитаФормы,
				ТекРеквизит.Синоним + "(Кт)", Истина);
			МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);
	...
	ДобавитьРеквизитыСубконтоРегистраДвиженияВДобавляемыеРеквизитыФормы(ЭтоРегистрБухгалтерии,
		МассивДобавляемыхРеквизитов, РегистрМетаданные, ОписаниеРегистра);
...
Процедура ДобавитьНаборыДвиженийДокументаВРеквизитыФормы(МетаданныеОбъекта)
	...
	Для Каждого Регистр Из МетаданныеОбъекта.Движения Цикл
		...
		НовыйРеквизит = Новый РеквизитФормы(ОписаниеРегистра.ИмяРеквизитаФормы, Новый ОписаниеТипов("ТаблицаЗначений"),
			"", ОписаниеРегистра.Заголовок, Истина);
		МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);
	...
	Для Каждого Регистр Из МетаданныеОбъекта.Движения Цикл
		...
		Если ЭтоРегистрБухгалтерии Тогда
			...
			Для НомерСубконто = 1 По Регистр.ПланСчетов.МаксКоличествоСубконто Цикл
				МассивСтандартныхРеквизитовНеТребующихДобавления.Добавить("ВИДСУБКОНТО" + НомерСубконто);
				МассивСтандартныхРеквизитовНеТребующихДобавления.Добавить("СУБКОНТО" + НомерСубконто);
			КонецЦикла;

		КонецЕсли;

		ДобавитьРеквизитыРегистраДвиженияВДобавляемыеРеквизитыФормы(МассивДобавляемыхРеквизитов, ОписаниеРегистра,
			Регистр, ЭтоРегистрБухгалтерии, МассивСтандартныхРеквизитовНеТребующихДобавления);
	КонецЦикла;
...
&НаСервере
Процедура ДобавитьРеквизитКДобавлениюНаФорму(КоллекцияРеквизитов, ТекРеквизитМетаданных, ПрефиксИмениФормы,
	МассивДобавляемыхРеквизитов, ПредставлениеГруппыРеквизитов, ИмяРеквизитаФормыТЧ = "")
	...
	ДобавитьРеквизитВКоллекциюРеквизитов(КоллекцияРеквизитов, ТекРеквизитМетаданных.Имя, ТекРеквизитМетаданных.Тип,
		Синоним, ИмяРеквизита, ПредставлениеГруппыРеквизитов);

КонецПроцедуры

&НаСервере
Процедура ДобавитьСтандартныеРеквизитыМетаданногоКДобавлениюНаФорму(МетаданныеОбъекта, МассивДобавляемыхРеквизитов)

	Для Каждого ТекРеквизит Из МетаданныеОбъекта.СтандартныеРеквизиты Цикл
		ДобавитьРеквизитКДобавлениюНаФорму(РеквизитыОбъекта.Реквизиты, ТекРеквизит, "СтандартныйРеквизит_",
			МассивДобавляемыхРеквизитов, "Стандартные реквизиты");
	КонецЦикла;

КонецПроцедуры

&НаСервере
Процедура ДобавитьРеквизитыМетаданногоКДобавлениюНаФорму(МетаданныеОбъекта, МассивДобавляемыхРеквизитов)

	Для Каждого ТекРеквизит Из МетаданныеОбъекта.Реквизиты Цикл
		ДобавитьРеквизитКДобавлениюНаФорму(РеквизитыОбъекта.Реквизиты, ТекРеквизит, "Реквизит_",
			МассивДобавляемыхРеквизитов, "Реквизиты");
	КонецЦикла;

КонецПроцедуры

&НаСервере
Процедура ДобавитьСпецифичныеРеквизитыПланаСчетовКДобавлениюНаФорму(МетаданныеОбъекта, МассивДобавляемыхРеквизитов)
	...
	Для Каждого ТекРеквизит Из МетаданныеОбъекта.ПризнакиУчета Цикл
		ДобавитьРеквизитКДобавлениюНаФорму(РеквизитыОбъекта.Реквизиты, ТекРеквизит, "ПризнакУчета_",
			МассивДобавляемыхРеквизитов, "Признаки учета");
	КонецЦикла;

КонецПроцедуры

&НаСервере
Процедура ДобавитьСпецифичныеРеквизитыЗадачиКДобавлениюНаФорму(МетаданныеОбъекта, МассивДобавляемыхРеквизитов)
	...
	Для Каждого ТекРеквизит Из МетаданныеОбъекта.РеквизитыАдресации Цикл
		ДобавитьРеквизитКДобавлениюНаФорму(РеквизитыОбъекта.Реквизиты, ТекРеквизит, "РеквизитАдресации_",
			МассивДобавляемыхРеквизитов, "Реквизиты адресации");
	КонецЦикла;

КонецПроцедуры

&НаСервере
Процедура ДобавитьТабличнуюЧастьКДобавлениюНаФорму(ТабличнаяЧастьМетаданные, ПрефиксИмени, МассивДобавляемыхРеквизитов)
	...
	ДобавитьТабличнуюЧастьВКоллекциюРеквизитов(РеквизитыОбъекта.ТабличныеЧасти, ТабличнаяЧастьМетаданные.Имя, Синоним,
		ИмяРеквизита);
...
// Добавим реквизиты табличных частей
	...
	Для Каждого ОписаниеТабличнойЧасти Из РеквизитыОбъекта.ТабличныеЧасти Цикл
		...
		Для Каждого Реквизит Из ТабличнаяЧасть.СтандартныеРеквизиты Цикл
			ДобавитьРеквизитКДобавлениюНаФорму(ОписаниеТабличнойЧасти.Реквизиты, Реквизит, "",
				МассивДобавляемыхРеквизитов, "Реквизиты", ОписаниеТабличнойЧасти.ИмяРеквизитаФормы);
		КонецЦикла;
		...
		Если УИ_ОбщегоНазначения.ЭтоПланСчетов(МетаданныеОбъекта) Тогда
			Для Каждого ПризнакУчетаСубконто Из МетаданныеОбъекта.ПризнакиУчетаСубконто Цикл
				ДобавитьРеквизитКДобавлениюНаФорму(ОписаниеТабличнойЧасти.Реквизиты, ПризнакУчетаСубконто, "",
					МассивДобавляемыхРеквизитов, "Реквизиты", ОписаниеТабличнойЧасти.ИмяРеквизитаФормы);
			КонецЦикла;
		КонецЕсли;
...
Процедура СоздатьРеквизитыФормыОбъекта(МетаданныеОбъекта)
	...
	Если УИ_ОбщегоНазначения.ЭтоПланСчетов(МетаданныеОбъекта) Или УИ_ОбщегоНазначения.ЭтоПланВидовРасчета(
		МетаданныеОбъекта) Тогда
		Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.СтандартныеТабличныеЧасти Цикл
			ДобавитьТабличнуюЧастьКДобавлениюНаФорму(ТабличнаяЧасть, "СтандартнаяТабличнаяЧасть_",
...
Процедура ДобавитьЭлементыФормыДляРеквизитовОбъекта()
	...
	Для Каждого ОписаниеРеквизита Из РеквизитыОбъекта.Реквизиты Цикл
		...
		// Делаем группу под каждую константу, чтобы ее можно было разрисовывать
			...
			ОписаниеГруппы.Имя = "Группа_Реквизиты_" + ОписаниеРеквизита.ГруппаРеквизитов;
		...
		Если (ОписаниеРеквизита.Тип.Типы().Количество() = 1 И ОписаниеРеквизита.Тип.СодержитТип(Тип("Булево"))) Тогда
		...
		ОписаниеЭлемента.Действия.Вставить("ПриИзменении", "Подключаемый_РеквизитШапкиПриИзменении");
...
Процедура ДобавитьЭлементыДляТабличныхЧастейОбъекта()
	...
	// Делаем группу под каждую константу, чтобы ее можно было разрисовывать
		...
		ОписаниеЭлемента.Действия.Вставить("ПриНачалеРедактирования",
			"Подключаемый_ТабличнаяЧастьПриНачалеРедактирования");
		...
		Для Каждого РеквизитТЧ Из ОписаниеТЧ.Реквизиты Цикл
			...
			ОписаниеЭлемента.ПутьКДанным = ОписаниеТЧ.ИмяРеквизитаФормы + "." + РеквизитТЧ.ИмяРеквизитаФормы;
			ОписаниеЭлемента.Вставить("ПутьКРеквизиту", ОписаниеТЧ.ИмяРеквизитаФормы + "."
			...
			Если (РеквизитТЧ.Тип.Типы().Количество() = 1 И РеквизитТЧ.Тип.СодержитТип(Тип("Булево"))) Тогда
				ОписаниеЭлемента.Параметры.Вставить("Вид", ВидПоляФормы.ПолеФлажка);
			КонецЕсли;
...
Процедура ДобавитьЭлементыДляДвиженийДокумента()
	...
	СоответствиеСтраницПоТипамРегистра.Вставить("РегистрБухгалтерии",
		Элементы.ГруппаДвиженияСтраницыРегистровБухгалтерии);
	...
	// Под каждый регистр нам нужно сделать свою закладку и на ней разместить табличку редактирования регистра
		ОписаниеГруппы = УИ_РаботаСФормами.НовыйОписаниеГруппыФормы();
		ОписаниеГруппы.Имя = "Группа_" + ОписаниеРегистра.ТипРегистра + "_" + ОписаниеРегистра.ИмяРеквизитаФормы;
		...
		Для Каждого РеквизитТЧ Из ОписаниеРегистра.Реквизиты Цикл
			...
			ОписаниеЭлемента.Имя = РеквизитТЧ.ИмяРеквизитаФормы;
			ОписаниеЭлемента.Заголовок = РеквизитТЧ.Заголовок;
			ОписаниеЭлемента.ПутьКДанным = ОписаниеРегистра.ИмяРеквизитаФормы + "." + РеквизитТЧ.ИмяРеквизитаФормы;
			ОписаниеЭлемента.Вставить("ПутьКРеквизиту", ОписаниеРегистра.ИмяРеквизитаФормы + "."
				+ РеквизитТЧ.ИмяРеквизитаФормы);
			ОписаниеЭлемента.РодительЭлемента = ТаблицаТЧ;
...
&НаСервере
Процедура ЗаписатьСпецифическиеДляРегистраБухгалтерииРеквизитыДвижения(НоваяЗапись, СтрокаТаблицыФормы,
	ОписаниеРегистра, МассивПропускаемыхРеквизитов)
	СоответствиеСчетовИВидовСубконто = Новый Соответствие;
	Если ОписаниеРегистра.Корреспонденция Тогда
		СоответствиеСчетовИВидовСубконто.Вставить("СчетДт", "СубконтоДт");
		СоответствиеСчетовИВидовСубконто.Вставить("СчетКт", "СубконтоКт");
...
&НаСервере
Процедура ЗаполнитьНаборЗаписейДвиженийПоТаблицеФормы(НаборЗаписей, ТаблицаФормы, ОписаниеРегистра,
	...
	Для Каждого СтрокаТз Из ТаблицаФормы Цикл
		...
		Если ЭтоРегистрБухгалтерии Тогда
			ЗаписатьСпецифическиеДляРегистраБухгалтерииРеквизитыДвижения(НоваяЗапись, СтрокаТз, ОписаниеРегистра,
				МассивПропускаемыхРеквизитов);
		КонецЕсли;
...
Процедура ЗаполнитьРеквизитыНаСервереПередЗаписью(ОбъектЗаписи)
	...
	Для Каждого ТекРеквизит Из РеквизитыОбъекта.Реквизиты Цикл
		...
		// устанавливаем только измененные реквизиты
		Если Не ТекРеквизит.Изменен Тогда
			Продолжить;
		КонецЕсли;
		
		ТекРеквизит.Изменен=Ложь;
		ТекРеквизит.НачальноеЗначение=ЭтотОбъект[ТекРеквизит.ИмяРеквизитаФормы];
		Элементы[ТекРеквизит.ИмяРеквизитаФормы].ЦветТекстаЗаголовка=Новый Цвет;
...
&НаСервере
Процедура ЗаполнитьТабличнуюЧастьНаСервереПередЗаписью(ОбъектЗаписи, ТекТабличнаяЧасть)
...
Процедура ВыполнитьЗаписьОбъектаВБазу(ОбъектЗаписи)
	...
	Попытка
		...
		Модифицированность = Ложь;
	Исключение
		УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Запись не удалась: " + ОписаниеОшибки());
	КонецПопытки;
КонецПроцедуры
