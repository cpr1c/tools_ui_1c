...
Процедура _ОткрытьВНовомОкне(Команда)
	...
	ОткрытьФорму("Обработка.УИ_РедакторРеквизитовОбъекта.Форма.ФормаРедакторДвижений",СтрукПарам,,ТекущаяДата(),,,,РежимОткрытияОкнаФормы.Независимый);
КонецПроцедуры
...
Процедура _ЗагрузитьДвиженияДругогоДокумента(Команда)
	...
	СтрукПарам = новый Структура("ЗакрыватьПриЗакрытииВладельца, ПереченьРазделов", истина, "Документы");
	ОткрытьФорму("ОбщаяФорма.УИ_ФормаВыбораТипаОбъекта", СтрукПарам, ЭтаФорма,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
