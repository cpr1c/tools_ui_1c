...
Процедура мОбъектСсылкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если мОбъектСсылка = Неопределено Тогда
		...
		ОткрытьФорму("ОбщаяФорма.УИ_ФормаВыбораТипаОбъекта", СтрукПарам, Элемент,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		Массив = новый Массив;
		...
		Если _ИспользоватьНеСтандартнуюФормуДляВыбора Тогда
			...
			СтрукПараметры = новый Структура("ПутьКФормам, ПолноеИмя", ПутьКФормам, пПолноеИмя);
...
Процедура ОткрытьОбъект(Команда)
	...
	Если ЗначениеЗаполнено(Значение) Тогда
		...
		ИначеЕсли вЭтоОбъектМетаданных(ТипЗнч(Значение)) Тогда
			СтрукПарам = новый Структура("ПутьКФормам, мОбъектСсылка", ПутьКФормам, Значение);
...
Функция вСформироватьОтчетПоДвижениям(Знач ДокСсылка, Знач СписокРегистров, Знач пКонфигурацияДопускаетДопДвижения)
	...
	Для каждого Элем из СписокРегистров Цикл
		...
		Если Лев(Элем.Значение,1) = "+" Тогда
			...
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ " + Инд + ", КОЛИЧЕСТВО(*) ИЗ " + Сред(Элем.Значение,2) + " КАК т ГДЕ т.ДокументРегистратор = &Регистратор ИМЕЮЩИЕ КОЛИЧЕСТВО(*) > 0";
		Иначе
			ТекстЗапроса = ТекстЗапроса + "
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ " + Инд + ", КОЛИЧЕСТВО(*) ИЗ " + Элем.Значение + " КАК т ГДЕ т.Регистратор = &Регистратор ИМЕЮЩИЕ КОЛИЧЕСТВО(*) > 0";
		КонецЕсли;
	КонецЦикла;
...
Процедура _ОткрытьРедакторДвижений(Команда)
	СтрукПарам = новый Структура("ПутьКФормам, мОбъектСсылка, ОписаниеРазработки", ПутьКФормам, мОбъектСсылка, _ОписаниеРазработки);
	Попытка
...
Процедура _ОткрытьРедакторДвиженийДоп(Команда)
	СтрукПарам = новый Структура("ПутьКФормам, мОбъектСсылка, ОписаниеРазработки", ПутьКФормам, мОбъектСсылка, _ОписаниеРазработки);
	Попытка
		ОткрытьФорму("Обработка.УИ_РедакторРеквизитовОбъекта.Форма.ФормаРедакторДвиженийДоп", СтрукПарам, ,мОбъектСсылка);
	Исключение
		Сообщить("Не найдена форма ""ФормаРедакторДвижений""!");
	КонецПопытки;
КонецПроцедуры
