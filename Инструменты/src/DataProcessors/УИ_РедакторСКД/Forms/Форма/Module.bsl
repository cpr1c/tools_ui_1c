...
Процедура ЗаполнитьПоляНабораСКДПоДаннымФормы(НовыйНабор, СтрокаНабора)
	...
	Для Каждого СтрокаПоля Из СтрокаНабора.Поля Цикл
		...
		ЗаполнитьЗначенияСвойств(НовоеПоле, СтрокаПоля, ,
			"ВыраженияУпорядочивания,ОграничениеИспользования,ОграничениеИспользованияРеквизитов,Оформление,ПараметрыРедактирования,Роль");
		...
		ЗаполнитьОграничениеИспользованияПоляСхемыКомпоновкиДанных(НовоеПоле.ОграничениеИспользованияРеквизитов,
			...
			СтрокаПоля.ОграничениеИспользованияРеквизитовГруппировка, СтрокаПоля.ОграничениеИспользованияРеквизитовПорядок);
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗаполнитьПараметрыСКДПоДаннымФормы(СКД)
	...
	Для Каждого ТекДанные Из ПараметрыСКД Цикл
		...
		ЗаполнитьЗначенияСвойств(НовыеДанные, ТекДанные, , "ПараметрыРедактирования, Использование");
...
//		ПараметрыРед=ТекДанные.ПараметрыРедактирования.
	КонецЦикла;
КонецПроцедуры

&НаСервере
Процедура СобратьСКДПоДаннымФормы()
	СКД=Новый СхемаКомпоновкиДанных;
