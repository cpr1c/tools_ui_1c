...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Тест
	ПрочитатьСКДВДанныеФормы(Отчеты.ЦПР_БС_ПериодическиеПлатежи.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных"));
КонецПроцедуры
...
Процедура ПрочитатьНаборыДанныхСКДВДанныеФормы(СКДНаборыДанных, СтрокаРодительскогоНабора = Неопределено)
	...
	Для Каждого СтрокаНабора Из СКДНаборыДанных Цикл
		...
		Если ТипЗнч(СтрокаНабора) = Тип("НаборДанныхЗапросСхемыКомпоновкиДанных") Тогда
			...
			НовыйНабор.Картинка=БиблиотекаКартинок.НоваяТаблицаКомпоновкиДанных;
		...
		Если НовыйНабор.Вид = ВидыНаборов.Объединение Тогда
			ЗаполнитьНаборыДанныхСКДПоДаннымФормы(СтрокаНабора.Элементы, НовыйНабор);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗаполнитьРольПоляНабораДанныхПоДаннымФормы(РольНабора, РольРедактора)
	...
	РольНабора.ТипОстатка=ТипОстаткаКомпоновкиДанных[РольРедактора.ТипБухгалтерскогоОстатка];
...
Процедура ЗаполнитьПараметрыСКДПоДаннымФормы(СКД)
	...
	Для Каждого ТекДанные Из ПараметрыСКД Цикл
		...
		Если ТекДанные.ДоступныеЗначения.Количество() = 0 Тогда
			НовыеДанные.УстановитьДоступныеЗначения();
		Иначе
			НовыеДанные.УстановитьДоступныеЗначения(ТекДанные.ДоступныеЗначения);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
