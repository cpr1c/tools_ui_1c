...
Процедура ОткрытьКонструкторЗапроса(Команда)
	...
	Если ТекНабор = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если ЗначениеЗаполнено(ТекНабор.Запрос) Тогда
		Конструктор=Новый КонструкторЗапроса(ТекНабор.Запрос);
	Иначе
		Конструктор=Новый КонструкторЗапроса;
	КонецЕсли;

	Если УИ_ОбщегоНазначенияКлиентСервер.ВерсияПлатформыСтарше_8_3_14() Тогда
...
Процедура ДобавитьПолеНабора(СтрокаНабора, Колонка, ВидыПолейНаборовДанных, МассивПолей, КолонкаРодитель = Неопределено)
	Если КолонкаРодитель = Неопределено Тогда
		Поле=Колонка.Псевдоним;
	Иначе
		Поле=КолонкаРодитель.Псевдоним + "." + Колонка.Псевдоним;
	КонецЕсли;
...
//	СтрокаНабора.Поля.Очистить();
	...
	Для Каждого СтрокаПоля Из СтрокаНабора.Поля Цикл
		Если МассивПолей.Найти(СтрокаПоля.Поле) = Неопределено Тогда
...
Функция ДоступноДобавлениеПоляНабораПоле(ТекСтрокаНабора)
	Возврат Не (ТекСтрокаНабора.Вид = ВидыНаборовДанных.Запрос И ТекСтрокаНабора.АвтоЗаполнениеДоступныхПолей)
		И ТекСтрокаНабора.Вид <> ВидыНаборовДанных.Объединение;
КонецФункции
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	//2. Если Набор данных запрос и с признаком автозаполнение то блочим добавление полей не папок
	НовоеУО=УсловноеОформление.Элементы.Добавить();
	НовоеУО.Использование=Истина;
//	УИ_ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(НовоеУО.Отбор, "Элементы.НаборыДанных.ТекущиеДанные.АвтоЗаполнениеДоступныхПолей", Истина);
	УИ_ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(НовоеУО.Отбор, "Элементы.НаборыДанных.ТекущиеДанные.Вид",
		ВидыНаборов.Запрос);

	Поле=НовоеУО.Поля.Элементы.Добавить();
	Поле.Использование=Истина;
	Поле.Поле=Новый ПолеКомпоновкиДанных("НаборыДанныхПоляДобавитьПолеНабораПоле");
	Поле=НовоеУО.Поля.Элементы.Добавить();
	Поле.Использование=Истина;
	Поле.Поле=Новый ПолеКомпоновкиДанных("НаборыДанныхПоляДобавитьПолеНабораНабор");

	Оформление=НовоеУО.Оформление.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Видимость"));
	Оформление.Использование=Истина;
...
Процедура ПрочитатьПоляНабораСКДВДанныеФормы(НовыйНабор, СтрокаНабора)
	...
	Для Каждого СтрокаПоля Из СтрокаНабора.Поля Цикл
		...
		ИначеЕсли ТипЗнч(СтрокаПоля) = Тип(ВидыПолейНабораДанныхСКД.Папка) Тогда
			...
			НовоеПоле.Картинка=БиблиотекаКартинок.СоздатьГруппу;
		Иначе
		...
		ПрочитатьОграничениеИспользованияПоляСхемыКомпоновкиДанныхВДанныеФормы(
			...
			НовоеПоле.ОграничениеИспользованияРеквизитовУсловие, НовоеПоле.ОграничениеИспользованияРеквизитовГруппировка,
