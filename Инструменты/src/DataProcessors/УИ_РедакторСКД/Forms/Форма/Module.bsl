...
Процедура ЗаполнитьПоляНабораСКДПоДаннымФормы(НовыйНабор, СтрокаНабора)
	...
	Для Каждого СтрокаПоля Из СтрокаНабора.Поля Цикл
		...
		Если ЗначениеЗаполнено(СтрокаПоля.Роль) Тогда
			НовоеПоле.Роль.Остаток=Истина;
			Представление1=Строка(НовоеПоле.Роль);
//			Роль=УИ_ОбщегоНазначения.ЗначениеИзСтрокиXML(СтрокаПоля.Роль);
//			ЗаполнитьЗначенияСвойств(НовоеПоле.Роль, Роль);
		КонецЕсли;
		НовоеПоле.Роль.Остаток=Истина;
		Представление1=Строка(НовоеПоле.Роль);
