...
Процедура СравнитьДанныеНаКлиентеПередатьФайлА(СтруктураПараметровНаКлиенте, ТекстОшибок)
	...
	Если Объект.ТипБазыА = 3 И Объект.ПодключениеКВнешнейБазеАУстройствоХраненияФайла = 1 Тогда
		...
		НачатьПомещениеФайла(Новый ОписаниеОповещения("СравнитьДанныеНаКлиентеПередатьФайлАЗавершение", ЭтаФорма,
			Новый Структура("СтруктураПараметровНаКлиенте, ТекстОшибок", СтруктураПараметровНаКлиенте, ТекстОшибок)),
			АдресВременногоХранилищаФайлаА, Объект.ПодключениеКВнешнейБазеАПутьКФайлу, Ложь);
	Иначе
...
Процедура СравнитьДанныеНаКлиентеПередатьФайлАЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		СравнитьДанныеНаКлиентеПередатьФайлБ(СтруктураПараметровНаКлиенте, ТекстОшибок);
	Иначе
		ТекстОшибок = "Не удалось поместить во временное хранилище файл А: """
			+ Объект.ПодключениеКВнешнейБазеАПутьКФайлу + """";
...
Процедура СравнитьДанныеНаКлиентеПередатьФайлБ(СтруктураПараметровНаКлиенте, ТекстОшибок)
	...
	Если Объект.ТипБазыБ = 3 И Объект.ПодключениеКВнешнейБазеБУстройствоХраненияФайла = 1 Тогда
		...
		НачатьПомещениеФайла(Новый ОписаниеОповещения("СравнитьДанныеНаКлиентеПередатьФайлБЗавершение", ЭтаФорма,
			Новый Структура("СтруктураПараметровНаКлиенте, ТекстОшибок", СтруктураПараметровНаКлиенте, ТекстОшибок)),
			АдресВременногоХранилищаФайлаБ, Объект.ПодключениеКВнешнейБазеБПутьКФайлу, Ложь);
		Возврат;
	Иначе
...
Процедура СравнитьДанныеНаКлиентеПередатьФайлБЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстОшибок = "Не удалось поместить во временное хранилище файл Б: """
			+ Объект.ПодключениеКВнешнейБазеБПутьКФайлу + """";
		Сообщить(ТекстОшибок);
		Возврат;
	КонецЕсли;
...
&НаСервере
Функция ПолучитьДанныеВВидеСтруктуры()

	ФормаОбъект = РеквизитФормыВЗначение("Объект");
	ДанныеСтруктура = ФормаОбъект.ПолучитьДанныеВВидеСтруктурыНаСервере();
...
#КонецЕсли
	...
	ИначеЕсли
	Объект["ТипБазы" + ИдентификаторБазы] = 1 Тогда

		Если Объект["ВариантРаботыВнешнейБазы" + ИдентификаторБазы] = 0 Тогда
			ПараметрСоединения = "File=""" + Объект["ПодключениеКВнешнейБазе" + ИдентификаторБазы + "ПутьКБазе"]
				+ """;Usr=""" + Объект["ПодключениеКВнешнейБазе" + ИдентификаторБазы + "Логин"] + """;Pwd="""
				+ Объект["ПодключениеКВнешнейБазе" + ИдентификаторБазы + "Пароль"] + """;";
		Иначе
			ПараметрСоединения = "Srvr=""" + Объект["ПодключениеКВнешнейБазе" + ИдентификаторБазы + "Сервер"]
				+ """;Ref=""" + Объект["ПодключениеКВнешнейБазе" + ИдентификаторБазы + "ПутьКБазе"] + """;Usr="""
				+ Объект["ПодключениеКВнешнейБазе" + ИдентификаторБазы + "Логин"] + """;Pwd="""
				+ Объект["ПодключениеКВнешнейБазе" + ИдентификаторБазы + "Пароль"] + """;";
		КонецЕсли;
		Попытка
			Application = Новый COMОбъект(СтрЗаменить(Объект["ВерсияПлатформыВнешнейБазы" + ИдентификаторБазы], ".",
				"") + ".Application");
	...
	Если Не Объект.КодДляВыводаСтрокРедактируетсяВручную Тогда
		...
		Иначе
			...
			Для Каждого СтрокаТЧ Из Объект.УсловияВыводаСтрок Цикл
				...
				Объект.КодДляВыводаСтрок = Объект.КодДляВыводаСтрок + ?(ПустаяСтрока(Объект.КодДляВыводаСтрок), "",
					Символы.ПС + Объект.ЛогическийОператорДляУсловийВыводаСтрок + " ") + КодИзСтрокиТЧ;
	...
	Если Не Объект.КодДляЗапретаВыводаСтрокРедактируетсяВручную Тогда
		...
		Иначе
			...
			Для Каждого СтрокаТЧ Из Объект.УсловияЗапретаВыводаСтрок Цикл
				...
				КодИзСтрокиТЧ = ПреобразоватьСтрокуРеквизитовВКодДляВыводаИЗапретаВыводаСтрок(СтрокаТЧ);
				Объект.КодДляЗапретаВыводаСтрок = Объект.КодДляЗапретаВыводаСтрок + ?(ПустаяСтрока(
					Объект.КодДляЗапретаВыводаСтрок), "", Символы.ПС
					+ Объект.ЛогическийОператорДляУсловийЗапретаВыводаСтрок + " ") + КодИзСтрокиТЧ;
	...
	Если СтрокаТЧ.Условие <> "Заполнен" Тогда
		...
		Если СтрокаТЧ.ТипСравнения = "Значение" Тогда
			Если ТипЗнч(СтрокаТЧ.СравниваемоеЗначение) = Тип("Дата") Тогда
				ПраваяСторона = "Дата(" + Год(СтрокаТЧ.СравниваемоеЗначение) + "," + Месяц(СтрокаТЧ.СравниваемоеЗначение)
					+ "," + День(СтрокаТЧ.СравниваемоеЗначение) + "," + Час(СтрокаТЧ.СравниваемоеЗначение) + ","
					+ Минута(СтрокаТЧ.СравниваемоеЗначение) + "," + Секунда(СтрокаТЧ.СравниваемоеЗначение) + ")";
		...
		КодИзСтрокиТЧ = СтрокаТЧ.ИмяСравниваемогоРеквизита + " " + СтрокаТЧ.Условие + " " + ПраваяСторона;

	Иначе

		КодИзСтрокиТЧ = "ЗначениеЗаполнено(" + СтрокаТЧ.ИмяСравниваемогоРеквизита + ")";
...
Процедура ПолучитьПараметрыИзЗапросаНаСервере(ИдентификаторБазы)
	...
	Для Каждого ПараметрЗапроса Из ПараметрыЗапроса Цикл
		...
		НайденныеПараметры = Объект["СписокПараметров" + ИдентификаторБазы].НайтиСтроки(Новый Структура("ИмяПараметра",
			ИмяПараметра));
...
Процедура КомандаВыгрузитьРезультатВФайлНаКлиентеЗавершениеВопрос(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ДиалогСохраненияФайла.Показать(Новый ОписаниеОповещения("КомандаВыгрузитьРезультатВФайлНаКлиентеЗавершение",
		ЭтаФорма, Новый Структура("ДанныеФайла, ДиалогСохраненияФайла", ДанныеФайла, ДиалогСохраненияФайла)));
...
Процедура КомандаВыгрузитьРезультатВФайлНаКлиентеЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ВыбранныеФайлы <> Неопределено) Тогда
		...
		ДанныеФайла.НачатьЗапись(
			Новый ОписаниеОповещения("КомандаВыгрузитьРезультатВФайлНаКлиентеЗавершениеЗаписиФайла", ЭтотОбъект,
			Новый Структура("ПолноеИмяФайла", ДиалогСохраненияФайла.ПолноеИмяФайла)),
...
&НаКлиенте
Процедура КомандаВыгрузитьРезультатВФайлНаКлиентеЗавершениеЗаписиФайла(ДополнительныеПараметры) Экспорт
	Сообщить(Формат(ТекущаяДата(), "ДФ='yyyy.MM.dd HH.mm.ss'") + ": формирование файла завершено ("
...
Процедура ОбновитьВидимостьДоступностьРеквизитаПроизвольныйКод(НомерЭлементаФормы, ИдентификаторБазы)
	...
	Элементы["ПроизвольныйКодКлюча" + НомерЭлементаФормы
		+ ИдентификаторБазы].Видимость = Объект["ВыполнятьПроизвольныйКодКлюча" + НомерЭлементаФормы
		+ ИдентификаторБазы];
...
Процедура ОбновитьВидимостьДоступностьЭлементовФормыПоИдентификаторуБазы(ИдентификаторБазы)
	...
	Элементы["ГруппаСтраницаПараметрыЗапроса" + ИдентификаторБазы].Видимость = Объект["ТипБазы" + ИдентификаторБазы]
		<= 1;
	Элементы["СписокПараметров" + ИдентификаторБазы + "КомандаПолучитьПараметрыЗапроса"
		+ ИдентификаторБазы].Видимость = Объект["ТипБазы" + ИдентификаторБазы] <= 2;
	...
	Если Объект["ТипБазы" + ИдентификаторБазы] = 1 Тогда
		...
		Элементы["ГруппаСтраницаТекстЗапроса"
			+ ИдентификаторБазы].Заголовок							= "Текст запроса";
		...
		Элементы["ПриводитьКлючКСтроке"
			+ ИдентификаторБазы].Видимость 									= Не Объект["ИспользоватьВКачествеКлючаУникальныйИдентификатор"
			+ ИдентификаторБазы];
		Элементы["ПриводитьКлюч2КСтроке"
			+ ИдентификаторБазы].Видимость 								= Не Объект["ИспользоватьВКачествеКлюча2УникальныйИдентификатор"
			+ ИдентификаторБазы];
		Элементы["ПриводитьКлюч3КСтроке"
			+ ИдентификаторБазы].Видимость 								= Не Объект["ИспользоватьВКачествеКлюча3УникальныйИдентификатор"
			+ ИдентификаторБазы];
		Элементы["ДлинаКлючаПриПриведенииКСтроке"
			+ ИдентификаторБазы].Видимость 						= Не Объект["ИспользоватьВКачествеКлючаУникальныйИдентификатор"
			+ ИдентификаторБазы];
		Элементы["ДлинаКлюча2ПриПриведенииКСтроке"
			+ ИдентификаторБазы].Видимость 						= Не Объект["ИспользоватьВКачествеКлюча2УникальныйИдентификатор"
			+ ИдентификаторБазы];
		Элементы["ДлинаКлюча3ПриПриведенииКСтроке"
			+ ИдентификаторБазы].Видимость 						= Не Объект["ИспользоватьВКачествеКлюча3УникальныйИдентификатор"
			+ ИдентификаторБазы];
		Элементы["ДлинаКлючаПриПриведенииКСтроке"
			+ ИдентификаторБазы].ТолькоПросмотр 					= Не Объект["ПриводитьКлючКСтроке"
			+ ИдентификаторБазы];
		Элементы["ДлинаКлюча2ПриПриведенииКСтроке"
			+ ИдентификаторБазы].ТолькоПросмотр 					= Не Объект["ПриводитьКлюч2КСтроке"
			+ ИдентификаторБазы];
		Элементы["ДлинаКлюча3ПриПриведенииКСтроке"
			+ ИдентификаторБазы].ТолькоПросмотр 					= Не Объект["ПриводитьКлюч3КСтроке"
			+ ИдентификаторБазы];
	...
	ИначеЕсли Объект["ТипБазы" + ИдентификаторБазы] = 2 Тогда
		...
		Элементы["ПодключениеКВнешнейБазе" + ИдентификаторБазы + "Сервер"].Видимость 					= Истина;
		Элементы["ПодключениеКВнешнейБазе" + ИдентификаторБазы
			+ "ПутьКБазе"].Заголовок 				= "Имя базы данных";
	...
	ИначеЕсли Объект["ТипБазы" + ИдентификаторБазы] = 3 Тогда
		...
		Элементы["ГруппаПараметрыПодключенияКФайлуXMLJSON"
			+ ИдентификаторБазы].Видимость 				= ФайлФорматаXML;
		Элементы["ГруппаПараметрыПодключенияКФайлуXML"
			+ ИдентификаторБазы].Видимость 					= ФайлФорматаXML;
		Элементы["ГруппаПараметрыПодключенияКФайлуНеXML"
			+ ИдентификаторБазы].Видимость					= Не ФайлФорматаXML;
		Элементы["ПодключениеКВнешнейБазе" + ИдентификаторБазы
			+ "НомерТаблицыВФайле"].Видимость		= ФайлФорматаXLS Или ФайлФорматаDOC;
		Элементы["ПодключениеКВнешнейБазе" + ИдентификаторБазы + "НомерТаблицыВФайле"].Заголовок		= ?(
			ФайлФорматаXLS, "Номер книги", "Номер таблицы");
		...
		Элементы["НомерСтолбцаСКлючомИзФайла"
			+ ИдентификаторБазы].Видимость 							= Не ФайлФорматаXML;
		Элементы["НомерСтолбцаСКлючом2ИзФайла"
			+ ИдентификаторБазы].Видимость 							= Не ФайлФорматаXML;
		Элементы["НомерСтолбцаСКлючом3ИзФайла"
			+ ИдентификаторБазы].Видимость 							= Не ФайлФорматаXML;
		Элементы["ИмяСтолбцаСКлючомИзФайла"
			+ ИдентификаторБазы].Видимость 								= ФайлФорматаXML;
		...
		Элементы["НастройкиФайла" + ИдентификаторБазы
			+ "НомерКолонки"].Видимость						= Не ФайлФорматаXML;
		Элементы["НастройкиФайла" + ИдентификаторБазы
			+ "ИмяКолонки"].Видимость							= ФайлФорматаXML;
...
Процедура ОбновитьВидимостьДоступностьВкладкиУсловияВыводаСтрок()
	...
	Элементы.ГруппаУсловияВыводаСтрок.ЦветФона = ?(Объект.УсловияВыводаСтрокОтключены, WebЦвета.Розовый,
		ЦветФонаФормыПоУмолчанию);
	;
...
Процедура ОбновитьВидимостьДоступностьВкладкиУсловияЗапретаВыводаСтрок()
	...
	Элементы.ГруппаУсловияЗапретаВыводаСтрок.ЦветФона = ?(Объект.УсловияЗапретаВыводаСтрокОтключены, WebЦвета.Розовый,
		ЦветФонаФормыПоУмолчанию);
	;
...
#Область Настройки
&НаКлиенте
Процедура СохранитьНастройкиВФайлНаКлиенте()
	...
	ДиалогВыбора.Показать(Новый ОписаниеОповещения("СохранитьНастройкиВФайлНаКлиентеЗавершение", ЭтаФорма,
		Новый Структура("ДиалогВыбора", ДиалогВыбора)));
...
Процедура СохранитьНастройкиВФайлНаКлиентеЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ВыбранныеФайлы <> Неопределено) Тогда
		...
		Объект.Заголовок = Сред(ДиалогВыбора.ПолноеИмяФайла, СтрНайти(ДиалогВыбора.ПолноеИмяФайла, "\",
			НаправлениеПоиска.СКонца) + 1);
		Адрес = СохранитьНастройкиВФайлНаСервере();
		ДвоичныеДанные = ПолучитьИзВременногоХранилища(Адрес);
		ДвоичныеДанные.НачатьЗапись(Новый ОписаниеОповещения("СохранитьНастройкиВФайлНаКлиентеЗавершениеЗаписиФайла",
			ЭтотОбъект), ДиалогВыбора.ПолноеИмяФайла);
...
&НаКлиенте
Процедура СохранитьНастройкиВФайлНаКлиентеЗавершениеЗаписиФайла(ДополнительныеПараметры) Экспорт

КонецПроцедуры

&НаКлиенте
Процедура ОткрытьНастройкиИзФайлаНаКлиенте(Знач Оповещение)
	...
	ДиалогВыбора.Показать(Новый ОписаниеОповещения("ОткрытьНастройкиИзФайлаНаКлиентеЗавершение", ЭтаФорма,
		Новый Структура("ДиалогВыбора, Оповещение", ДиалогВыбора, Оповещение)));
...
Процедура ОткрытьНастройкиИзФайлаНаКлиентеЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ВыбранныеФайлы <> Неопределено) Тогда
		...
		ОткрытьНастройкиИзФайлаНаСервере(Адрес);
...
Функция СохранитьНастройкиВФайлНаСервере()
	...
	Данные = ПолучитьДанныеВВидеСтруктуры();
...
&НаСервере
Процедура ОткрытьНастройкиИзФайлаНаСервере(Адрес)
...
&НаСервере
Процедура ОткрытьНастройкиИзБазыНаСервере(ВыбранныйЭлемент)

	ФормаОбъект = РеквизитФормыВЗначение("Объект");
	ФормаОбъект.ОткрытьНастройкиИзБазыНаСервере(ВыбранныйЭлемент);
	ЗначениеВРеквизитФормы(ФормаОбъект, "Объект");

КонецПроцедуры

&НаСервере
Процедура СохранитьНастройкиВБазуНаСервере(ВыбранныйЭлемент)

	ФормаОбъект = РеквизитФормыВЗначение("Объект");
	ФормаОбъект.СохранитьНастройкиВБазуНаСервере(ВыбранныйЭлемент);
	ЗначениеВРеквизитФормы(ФормаОбъект, "Объект");
...
Процедура СохранитьВСвязаннуюОперациюЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		СохранитьНастройкиВБазуНаСервере(Объект.СвязаннаяОперацияСравненияДанных);
		ОбновитьЗаголовок();
...
&НаКлиенте
Процедура ОткрытьФормуВыбораОперацииДляЗаписи()

	ВыбранныйЭлемент = Неопределено;
	ОткрытьФорму("Справочник.ВС_ОперацииСравненияДанных.ФормаВыбора", , , , , ,
		Новый ОписаниеОповещения("СохранитьВВыбраннуюОперациюЗавершение", ЭтаФорма),
...
Процедура СохранитьВВыбраннуюОперациюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйЭлемент <> Неопределено Тогда
		...
		СохранитьНастройкиВБазуНаСервере(ВыбранныйЭлемент);
		ОбновитьЗаголовок();
...
Процедура ОткрытьНастройкиИзБазыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйЭлемент <> Неопределено Тогда
		...
		ОткрытьНастройкиИзБазыНаСервере(ВыбранныйЭлемент);
		ОбновитьВидимостьДоступностьЭлементовФормы();
		ОбновитьВидимостьДоступностьЭлементовРеляционнаяОперация();
		ОбновитьВидимостьДоступностьЭлементовВыводИЗапретаВыводаСтрок();
...
Процедура ПриНачалеВыбораЗначенияПараметра(ИдентификаторБазы, СтандартнаяОбработка)
	...
	Если Объект["ТипБазы" + ИдентификаторБазы] = 1 Тогда
		...
		ПоказатьВыборИзСписка(Новый ОписаниеОповещения("ПриНачалеВыбораЗначенияПараметраЗавершение4", ЭтаФорма,
			Новый Структура("ИдентификаторБазы", ИдентификаторБазы)), СписокДоступныхТипов, Элементы["СписокПараметров"
			+ ИдентификаторБазы + "ЗначениеПараметра"]);
...
Процедура ПриНачалеВыбораЗначенияПараметраЗавершение4(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйТип.Значение = "Число" Тогда
		ПоказатьВводЧисла(Новый ОписаниеОповещения("ПриНачалеВыбораЗначенияПараметраЗавершение3", ЭтаФорма,
			Новый Структура("ВыбранныйТип, ТекущиеДанные, ТекущееЗначениеПараметра", ВыбранныйТип, ТекущиеДанные,
			ТекущееЗначениеПараметра)), ТекущееЗначениеПараметра);
		Возврат;
	ИначеЕсли ВыбранныйТип.Значение = "Строка" Тогда
		ПоказатьВводСтроки(Новый ОписаниеОповещения("ПриНачалеВыбораЗначенияПараметраЗавершение2", ЭтаФорма,
			Новый Структура("ВыбранныйТип, ТекущиеДанные, ТекущееЗначениеПараметра", ВыбранныйТип, ТекущиеДанные,
			ТекущееЗначениеПараметра)), ТекущееЗначениеПараметра);
		Возврат;
	ИначеЕсли ВыбранныйТип.Значение = "Дата" Тогда
		ПоказатьВводДаты(Новый ОписаниеОповещения("ПриНачалеВыбораЗначенияПараметраЗавершение1", ЭтаФорма,
			Новый Структура("ВыбранныйТип, ТекущиеДанные, ТекущееЗначениеПараметра", ВыбранныйТип, ТекущиеДанные,
			ТекущееЗначениеПараметра)), ТекущееЗначениеПараметра);
		Возврат;
	ИначеЕсли ВыбранныйТип.Значение = "Булево" Тогда
		ПоказатьВводЗначения(Новый ОписаниеОповещения("ПриНачалеВыбораЗначенияПараметраЗавершение", ЭтаФорма,
			Новый Структура("ТекущиеДанные, ТекущееЗначениеПараметра", ТекущиеДанные, ТекущееЗначениеПараметра)),
			ТекущееЗначениеПараметра, , Новый ОписаниеТипов("Булево"));
		Возврат;
	КонецЕсли;
...
Процедура СохранитьНастройкиВБазу(Команда)
	...
	Если ЗначениеЗаполнено(Объект.СвязаннаяОперацияСравненияДанных) Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("СохранитьВСвязаннуюОперациюЗавершение", ЭтотОбъект, Истина),
			"Обновить элемент справочника """ + Объект.СвязаннаяОперацияСравненияДанных + """?",
...
Процедура ОткрытьНастройкиИзБазы(Команда)
	...
	ВыбранныйЭлемент = Неопределено;
	ОткрытьФорму("Справочник.ВС_ОперацииСравненияДанных.ФормаВыбора", , , , , ,
		Новый ОписаниеОповещения("ОткрытьНастройкиИзБазыЗавершение", ЭтаФорма),
		РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		МакетКартинкаАктивнаяОперация1 		= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаАктивнаяОперация1");
		МакетКартинкаНеактивнаяОперация1 	= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаНеактивнаяОперация1");
		МакетКартинкаАктивнаяОперация2 		= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаАктивнаяОперация2");
		МакетКартинкаНеактивнаяОперация2 	= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаНеактивнаяОперация2");
		МакетКартинкаАктивнаяОперация3 		= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаАктивнаяОперация3");
		МакетКартинкаНеактивнаяОперация3 	= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаНеактивнаяОперация3");
		МакетКартинкаАктивнаяОперация4 		= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаАктивнаяОперация4");
		МакетКартинкаНеактивнаяОперация4 	= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаНеактивнаяОперация4");
		МакетКартинкаАктивнаяОперация5 		= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаАктивнаяОперация5");
		МакетКартинкаНеактивнаяОперация5 	= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаНеактивнаяОперация5");
		МакетКартинкаАктивнаяОперация6 		= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаАктивнаяОперация6");
		МакетКартинкаНеактивнаяОперация6 	= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаНеактивнаяОперация6");
		МакетКартинкаАктивнаяОперация7 		= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаАктивнаяОперация7");
		МакетКартинкаНеактивнаяОперация7 	= РеквизитФормыВЗначение("Объект").ПолучитьМакет(
			"КартинкаНеактивнаяОперация7");
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	Если Не ЗакрытиеФормыПодтверждено Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтаФорма),
			"Закрыть консоль сравнения данных?", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ЗначениеЗаполнено(Объект.СвязаннаяОперацияСравненияДанных) И Не Объект.ПользовательскийРежим Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("СохранитьВСвязаннуюОперациюЗавершение", ЭтотОбъект, Ложь),
			"Обновить элемент справочника """ + Объект.СвязаннаяОперацияСравненияДанных + """?",
			РежимДиалогаВопрос.ДаНет);
...
Процедура КодДляВыводаСтрокРедактируетсяВручнуюПриИзменении(Элемент)
	...
	Если Не Объект.КодДляВыводаСтрокРедактируетсяВручную Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КодДляВыводаСтрокРедактируетсяВручнуюПриИзмененииЗавершение",
			ЭтаФорма), "Код, внесенный вручную будет утерян. Продолжить?", РежимДиалогаВопрос.ДаНет);
		Возврат;
...
Процедура КодДляЗапретаВыводаСтрокРедактируетсяВручнуюПриИзменении(Элемент)
	...
	Если Не Объект.КодДляЗапретаВыводаСтрокРедактируетсяВручную Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КодДляЗапретаВыводаСтрокРедактируетсяВручнуюПриИзмененииЗавершение",
			ЭтаФорма), "Код, внесенный вручную будет утерян. Продолжить?", РежимДиалогаВопрос.ДаНет);
		Возврат;
...
Процедура ПодключениеКВнешнейБазеАПутьКФайлуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.ПодключениеКВнешнейБазеАФорматФайла = "XLS" Или Объект.ПодключениеКВнешнейБазеАФорматФайла = "DOC" Тогда
		ДиалогВыбораФайла.Фильтр = "*." + Объект.ПодключениеКВнешнейБазеАФорматФайла + ";*."
			+ Объект.ПодключениеКВнешнейБазеАФорматФайла + "X" + "|*." + Объект.ПодключениеКВнешнейБазеАФорматФайла
	...
	ДиалогВыбораФайла.Показать(Новый ОписаниеОповещения("ПодключениеКВнешнейБазеАПутьКФайлуНачалоВыбораЗавершение",
		ЭтаФорма, Новый Структура("ДиалогВыбораФайла", ДиалогВыбораФайла)));
...
Процедура ПодключениеКВнешнейБазеБПутьКФайлуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.ПодключениеКВнешнейБазеБФорматФайла = "XLS" Или Объект.ПодключениеКВнешнейБазеБФорматФайла = "DOC" Тогда
		ДиалогВыбораФайла.Фильтр = "*." + Объект.ПодключениеКВнешнейБазеБФорматФайла + ";*."
			+ Объект.ПодключениеКВнешнейБазеБФорматФайла + "X" + "|*." + Объект.ПодключениеКВнешнейБазеБФорматФайла
	...
	ДиалогВыбораФайла.Заголовок = "Выберите файл";
	ДиалогВыбораФайла.ИндексФильтра = 0;
	ДиалогВыбораФайла.Показать(Новый ОписаниеОповещения("ПодключениеКВнешнейБазеБПутьКФайлуНачалоВыбораЗавершение",
		ЭтаФорма, Новый Структура("ДиалогВыбораФайла", ДиалогВыбораФайла)));
...
Процедура СписокПараметровАЗначениеПараметраНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Элементы.СписокПараметровАЗначениеПараметра.ВыбиратьТип = ТипЗнч(
		Элементы.СписокПараметровА.ТекущиеДанные.Значениепараметра) = Тип("Неопределено");
...
Процедура СписокПараметровАЗначениеПараметраПриИзменении(Элемент)
	...
	ТекущийПараметр = Объект.СписокПараметровА.НайтиПоИдентификатору(
		Элементы.СписокПараметровА.ТекущиеДанные.ПолучитьИдентификатор());
...
Процедура СписокПараметровБЗначениеПараметраНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Элементы.СписокПараметровБЗначениеПараметра.ВыбиратьТип = ТипЗнч(
		Элементы.СписокПараметровБ.ТекущиеДанные.Значениепараметра) = Тип("Неопределено");
...
Процедура СписокПараметровБЗначениеПараметраПриИзменении(Элемент)
	...
	ТекущийПараметр = Объект.СписокПараметровБ.НайтиПоИдентификатору(
		Элементы.СписокПараметровБ.ТекущиеДанные.ПолучитьИдентификатор());
	ТекущийПараметр.ТипПараметра = ТипЗнч(ТекущийПараметр.ЗначениеПараметра);
...
Процедура КомандаВыгрузитьРезультатВФайлНаСервере(Команда)
	...
	Ответ = Неопределено;
	ПоказатьВопрос(Новый ОписаниеОповещения("КомандаВыгрузитьРезультатВФайлНаСервереЗавершение", ЭтаФорма),
		"Выгрузить таблицу в файл на сервере?", РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет, "Выгрузка");
...
Процедура ПутьКФайлуВыгрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбора = Новый ДиалогВыбораФайла(Режим);
	ДиалогВыбора.ПолноеИмяФайла = Объект.Заголовок;
	Фильтр = "Файл csv (*.csv)|*.csv";
	ДиалогВыбора.Фильтр = Фильтр;
	ДиалогВыбора.Заголовок = "Укажите файл для сохранения результата сравнения";

	ДиалогВыбора.Показать(Новый ОписаниеОповещения("ПутьКФайлуВыгрузкиНачалоВыбораЗавершение", ЭтаФорма,
		Новый Структура("ДиалогВыбора", ДиалогВыбора)));
...
Процедура КомандаВыгрузитьРезультатВФайлНаКлиенте(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("КомандаВыгрузитьРезультатВФайлНаКлиентеЗавершениеВопрос", ЭтаФорма),
		"Выгрузить таблицу в файл на клиенте?", РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет, "Выгрузка");
