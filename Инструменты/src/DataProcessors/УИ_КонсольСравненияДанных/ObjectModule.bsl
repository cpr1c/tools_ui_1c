Перем ЧислоРеквизитов, КодПриведенияРеквизитаКТипуЧисло;
...
Процедура СравнитьДанныеНаСервере(ТекстОшибок = "") Экспорт
	...
	Если ЧислоСтолбцовВКлюче = 1 Тогда
		ТЗ_А_Сгруппированная.Свернуть(ИмяКлючаА, ИмяКолонкиКоличествоСтрокИсточникаДанных);
	ИначеЕсли ЧислоСтолбцовВКлюче = 2 Тогда
		ТЗ_А_Сгруппированная.Свернуть(ИмяКлючаА + "," + ИмяКлючаА2, ИмяКолонкиКоличествоСтрокИсточникаДанных);
	Иначе
		ТЗ_А_Сгруппированная.Свернуть(ИмяКлючаА + "," + ИмяКлючаА2 + "," + ИмяКлючаА3, ИмяКолонкиКоличествоСтрокИсточникаДанных);
	КонецЕсли;
	...
	Если ЧислоСтолбцовВКлюче = 1 Тогда
		ТЗ_Б_Сгруппированная.Свернуть(ИмяКлючаБ, ИмяКолонкиКоличествоСтрокИсточникаДанных);
	ИначеЕсли ЧислоСтолбцовВКлюче = 2 Тогда
		ТЗ_Б_Сгруппированная.Свернуть(ИмяКлючаБ + "," + ИмяКлючаБ2, ИмяКолонкиКоличествоСтрокИсточникаДанных);
	Иначе
		ТЗ_Б_Сгруппированная.Свернуть(ИмяКлючаБ + "," + ИмяКлючаБ2 + "," + ИмяКлючаБ3, ИмяКолонкиКоличествоСтрокИсточникаДанных);
	КонецЕсли;
	...
	Если СообщениеОНаличииНесколькихСтрокПоОдномуКлючу Тогда
		Сообщить("Обнаружены записи, в которых ключу соответствует более одной строки. Такие записи подсвечены красным цветом, настройки отбора на них не распространяются.");
	КонецЕсли;
	
КонецПроцедуры

Функция ПрочитатьДанныеИПолучитьТЗ(ИдентификаторБазы, ТекстОшибки = "", Подключение = Неопределено)
...
Функция ВыполнитьЗапрос1С8ИПолучитьТЗ(ИдентификаторБазы, ТекстОшибок = "", Подключение = Неопределено)
	...
	Если ТЗ <> Неопределено Тогда
		...
		ЧислоКолонокВТЗ = ТЗ.Колонки.Количество();
...
Функция ПроверитьЗаполнениеРеквизитов(ТекстОшибок = "")
...
Функция СохранитьНастройкиВБазуНаСервере(НастройкаСсылка) Экспорт
	
	ОперацияЗавершенаУспешно = Истина;
	
	Попытка
		НастройкаОбъект = НастройкаСсылка.ПолучитьОбъект();
		Данные = ПолучитьДанныеВВидеСтруктурыНаСервере(); 
...
Процедура ОткрытьНастройкиИзБазыНаСервере(НастройкаСсылка) Экспорт
...
Функция ПолучитьДанныеВВидеСтруктурыНаСервере() Экспорт
...
Функция ВыгрузитьРезультатВФайлНаСервере(ДляКлиента = Ложь) Экспорт
	...
	Сообщить(Формат(ТекущаяДата(), "ДЛФ=DT") + " Выгрузка в файл " + ПутьКВременномуФайлу + " начата");
	...
	Иначе
		...
		Сообщить(Формат(ТекущаяДата(), "ДЛФ=DT") + " Выгрузка в файл " + ПутьКВременномуФайлу + " завершена (число строк: " + СчетчикСтрок + ")");
		Возврат Неопределено;
