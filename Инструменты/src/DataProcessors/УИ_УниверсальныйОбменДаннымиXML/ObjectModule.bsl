	...
	Если Реквизит = "ИспользоватьОтбор" Тогда
		...
		Для Каждого Строка Из Родитель.Строки Цикл
			...
			Если Строка[Реквизит] = 0 
				И Строка.Отбор <> Неопределено Тогда //УИ
...
Процедура ЗагрузитьПВД(ПравилаОбмена, НоваяСтрока)
	...
	Если ПустаяСтрока(НоваяСтрока.ИмяМетаданных) Тогда
		НоваяСтрока.ИмяМетаданных = ?(ЗначениеЗаполнено(НоваяСтрока.ИмяОбъектаДляЗапросаРегистра),НоваяСтрока.ИмяОбъектаДляЗапросаРегистра,НоваяСтрока.ИмяОбъектаДляЗапроса);
	КонецЕсли;	
...
//	КонецЕсли;
	...
	Если Правило.Отбор<>Неопределено Тогда
		...
		УИ_ОбщегоНазначенияКлиентСервер.УстановитьЗначениеПараметраСКД(КомпоновщикНастроек, "ДатаОкончания",     ДатаОкончания);
