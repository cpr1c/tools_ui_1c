...
Процедура ПриСозданииНаСервереИнициализацияДанных(НастройкиФормы)
	УчитыватьПрикладныеПравила = УИ_ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкиФормы, "УчитыватьПрикладныеПравила");
	ОбластьПоискаДублей        = УИ_ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкиФормы, "ОбластьПоискаДублей");
...
Функция НайтиИУдалитьДубли()
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		...
		ПараметрыПроцедуры.Вставить("ЗаписьБезПроверок",ЗаписьБезПроверок);
		ПараметрыПроцедуры.Вставить("ПривилегированнаяЗапись",БезПроверокПравДоступа);
