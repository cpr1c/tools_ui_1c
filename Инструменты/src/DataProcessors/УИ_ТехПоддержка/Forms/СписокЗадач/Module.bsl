...
&НаКлиенте
Процедура СписокЗадачВыбор(Элемент, ВыбраннаяСтрока, Поле,
...
Процедура СоздатьНовуюЗадачу(Команда)
	...
	Ответ = УИ_КоннекторHTTP.PostJson(БазовыйАдрес, СтруктураТела, Новый Структура("Заголовки, Аутентификация", Заголовки, Аутентификация));
...
Процедура СоздатьНовуюЗадачуНаГитхабе(Команда)
	НачатьЗапускПриложения(УИ_ОбщегоНазначенияКлиент.ПустоеОписаниеОповещенияДляЗапускаПриложения(), "https://github.com/cpr1c/tools_ui_1c/issues/new");
КонецПроцедуры
