...
&НаКлиенте
Процедура РезультатПоискаВыбор(Элемент, ВыбраннаяСтрока, Поле,
...
Процедура ВыполнитьПоискСсылокНаСервере()
	Если Не ЗначениеЗаполнено(Объект.ИсходныйОбъект) Тогда
		УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не выбран объект, на который необходимо найти ссылки", , "Объект.ИсходныйОбъект");
		Возврат;
	КонецЕсли;
	...
	// 2 - metadata object
		...
		ПредставлениеНайденного = ПредставлениеНайденногоОбъекта(БазовыйТипЧислом, СтрокаНайденнного.Метаданные, СтрокаНайденнного.Данные)
			+ " (" + СтрокаНайденнного.Метаданные.ПолноеИмя() + ")";
...
Функция ПредставлениеНайденногоОбъекта(БазовыйТипЧислом, МетаданныеОбъекта,
		НайденныйОбъект)

	Представление = СокрЛП(НайденныйОбъект);
	Если БазовыйТипЧислом = 2 ИЛИ БазовыйТипЧислом = 3 ИЛИ БазовыйТипЧислом = 8
			ИЛИ БазовыйТипЧислом = 9 ИЛИ БазовыйТипЧислом = 10 ИЛИ БазовыйТипЧислом = 11
			ИЛИ БазовыйТипЧислом = 12 ИЛИ БазовыйТипЧислом = 14 Тогда

	ИначеЕсли БазовыйТипЧислом = 4 ИЛИ БазовыйТипЧислом = 5
			ИЛИ БазовыйТипЧислом = 6 ИЛИ БазовыйТипЧислом = 7 Тогда

		Представление = "";
		Если МетаданныеОбъекта.InformationRegisterPeriodicity <> Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда
		...
		Если МетаданныеОбъекта.WriteMode = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору Тогда
			...
			Представление = ?(СтрДлина(Представление) = 0, "", Представление + "; ")
				+ Строка(НайденныйОбъект.Регистратор);
		...
		Для Каждого Измерение Из МетаданныеОбъекта.Измерения Цикл
			...
			Представление = ?(СтрДлина(Представление) = 0, "", Представление + "; ")
				+ Строка(НайденныйОбъект[Измерение.Имя]);
	...
	Иначе
		Для Каждого ВнешнийИсточник Из Метаданные.ВнешниеИсточникиДанных Цикл
			...
			Если ВнешнийИсточник.Таблицы.Содержит(МетаданныеОбъекта) Тогда
				...
				Если МетаданныеОбъекта.TableDataType = Метаданные.СвойстваОбъектов.ТипДанныхТаблицыВнешнегоИсточникаДанных.ОбъектныеДанные Тогда
