...
Функция ПолучитьОписаниеВидаОбъектов(Имя, ТекИБ = Неопределено) Экспорт
	...
	ОписаниеТипа.Вставить("Коллекция", ?(ТекИБ = Неопределено, Вычислить(ОписаниеТипаКэш.Коллекция), ТекИБ.Вычислить(
		ОписаниеТипаКэш.Коллекция)));
	ОписаниеТипа.Вставить("МетаКоллекция", ?(ТекИБ = Неопределено, Метаданные[ОписаниеТипаКэш.Коллекция],
...
Функция СсылкаСуществует(Значение) Экспорт
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "//[ИмяТаблицы]", СтрШаблонВнутр("%1.%2", ОписаниеВида.Имя,
		ОписаниеВида.ИмяТипа));
...
Функция ПолучитьИмяПредопределенныхДанных(Значение) Экспорт
	...
	Если ЗначениеЗаполнено(Значение) Тогда
		...
		Исключение
			Это83 = Ложь;
			ТекстОшибки = СтрШаблонВнутр("Ошибка получения имени предопределенных данных ""%1"": %2%3", СокрЛП(Значение),
		...
		Если Не Это83 Тогда
			Попытка
				Выполнить (Текст82);
			Исключение
				ТекстОшибки = СтрШаблонВнутр("Ошибка получения имени предопределенных данных ""%1"": %2%3", СокрЛП(
					Значение), Символы.ПС, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ОписаниеТиповВСтруктуру(ОписаниеТипов) Экспорт
	...
	СтруктураОписаниеТипов.Вставить("КвалификаторыСтроки", Новый Структура("Длина, ДопустимаяДлина", 10, "Переменная"));
	СтруктураОписаниеТипов.Вставить("КвалификаторыДаты", Новый Структура("ЧастиДаты", "ДатаВремя"));
	СтруктураОписаниеТипов.Вставить("КвалификаторыДвоичныхДанных", Новый Структура("Длина, ДопустимаяДлина", 0,
		"Переменная"));
...
Функция ОписаниеТиповИзСтруктуры(СтруктураОписаниеТипов) Экспорт
	...
	КвалификаторыЧисла = Новый КвалификаторыЧисла(СтруктураОписаниеТипов.КвалификаторыЧисла.Разрядность,
		СтруктураОписаниеТипов.КвалификаторыЧисла.РазрядностьДробнойЧасти, Вычислить("ДопустимыйЗнак."
		+ СтруктураОписаниеТипов.КвалификаторыЧисла.ДопустимыйЗнак));

	КвалификаторыСтроки = Новый КвалификаторыСтроки(СтруктураОписаниеТипов.КвалификаторыСтроки.Длина, Вычислить(
		"ДопустимаяДлина." + СтруктураОписаниеТипов.КвалификаторыСтроки.ДопустимаяДлина));

	КвалификаторыДаты = Новый КвалификаторыДаты(Вычислить("ЧастиДаты."
...
Функция СвойствоИерархическогоОбъектаИспользуется(Объект, МетаСвойство, МетаОбъект = Неопределено)
	...
	Если СтрокаНачиснаетсяСВнутр(МетаОбъект.ПолноеИмя(), "Справочник.") И МетаОбъект.ВидИерархии
		= Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов Или СтрокаНачиснаетсяСВнутр(
		МетаОбъект.ПолноеИмя(), "ПланВидовХарактеристик.") Тогда

		Если (Объект.ЭтоГруппа И МетаСвойство.Использование
			= Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляЭлемента) Или (Не Объект.ЭтоГруппа
...
//  ИсключаяСвойства	- Строка, Массив(Строка)	- Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура СвойствоОбъектаВСтруктуру(СтруктураОбъекта, Объект, ИмяСвойства, СписокСвойств = Неопределено,
...
//  ИсключаяСвойства    - Строка, Массив(Строка)  - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, ИмяСвойства, ПолучатьБитыеСсылки = Ложь,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено)
...
//  ИсключаяСвойства	- Строка, Массив(Строка)	- Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ТЧОбъектаВСтруктуру(СтруктураОбъекта, Объект, ИмяТЧ, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено)
...
//  ИсключаяСвойства    - Строка, Массив(Строка)  - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ТЧОбъектаИзСтруктуры(Объект, СтруктураОбъекта, ИмяТЧ, ПолучатьБитыеСсылки = Ложь,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено)
...
Функция ЗначенияСубконтоЗаписиВСтруктуру(Запись, ДтКт, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено)
	...
	Для Каждого ТекСтрокаВида Из ВидыСубконто Цикл
		Если Не ИспользоватьСвойство("Субконто" + ДтКт + Формат(й, "ЧДЦ=; ЧГ="), СписокСвойств, ИсключаяСвойства) Тогда
			...
			Продолжить;
		КонецЕсли;
		МассивСубконто.Добавить(Новый Структура("НомерСтроки, ВидСубконто, Значение", й, ЗначениеВСтруктуру(
			ТекСтрокаВида.ВидСубконто), ЗначениеВСтруктуру(ЗначенияСубконто[ТекСтрокаВида.ВидСубконто])));
		й = й + 1;
	КонецЦикла;
...
//  ИсключаяСвойства    - Строка, Массив(Строка)    - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ЗначенияСубконтоЗаписиИзСтруктуры(Запись, СтруктураЗаписи, ДтКт, ПолучатьБитыеСсылки = Ложь,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено)
...
Процедура ЗаполнитьИзмерениеРегистраБухгалтерии(Запись, СтруктураЗаписи, ОписаниеПоля, ДтКт,
	ПолучатьБитыеСсылки = Ложь, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено)

	Если Не (СтруктураЗаписи.Свойство(ОписаниеПоля.Имя + ДтКт) И ИспользоватьСвойство(ОписаниеПоля.Имя, СписокСвойств,
		ИсключаяСвойства)) Тогда
		Возврат;
	КонецЕсли;
...
//  ИсключаяСвойства    - Строка, Массив(Строка)        - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ЗаполнитьРесурсРегистраБухгалтерии(Запись, СтруктураЗаписи, ОписаниеПоля, ДтКт, СписокСвойств,
	ИсключаяСвойства)
...
Функция НайтиЗначениеПоОписанию(ОписаниеВида, МетаОбъект, ОписаниеЗначения)
	...
	Если ЗначениеЗаполнено(ОписаниеЗначения.ИмяПредопределенныхДанных) Тогда
		Если ВРег(ОписаниеВида.Имя) = ВРег("ПланОбмена") И ВРег(ОписаниеЗначения.ИмяПредопределенныхДанных) = ВРег(
			"ЭтотУзел") Тогда
	...
	Иначе
		...
		Если ОписаниеЗначения.Свойство("ИдПолный") И Найти(ОписаниеЗначения.ИдПолный,
			"00000000000000000000000000000000") = 0 Тогда
	...
	Если Не ПустаяСтрока(ОписаниеЗначения.Представление) И (ВРег(ОписаниеВида.Имя) = ВРег("Справочник") Или ВРег(
		ОписаниеВида.Имя) = ВРег("ПланВидовХарактеристик") Или ВРег(ОписаниеВида.Имя) = ВРег("ПланСчетов") Или ВРег(
		ОписаниеВида.Имя) = ВРег("ПланВидовРасчета") Или ВРег(ОписаниеВида.Имя) = ВРег("ПланОбмена") Или ВРег(
		ОписаниеВида.Имя) = ВРег("Задача")) Тогда
...
Функция ПрочитатьОписаниеОбъектаИзJSON(Знач ТекстОписанияОбъекта, Знач СвойстваСоЗначениемДата = Неопределено) Экспорт
	...
	Попытка
		Возврат ПрочитатьJSON(Чтение, Ложь, СвойстваСоЗначениемДата, ФорматДатыJSON.Microsoft,
			"ОбработчикЧтенияЗначенийJSON", ЭтотОбъект);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция СтруктураТипЗнч(СтруктураЗначения)
	...
	Если ТипЗнч(СтруктураЗначения) = Тип("Массив") Тогда
		...
		Если ТипЗнч(СтруктураЗначения[0]) = Тип("Структура") Тогда
			Если СтруктураЗначения[0].Свойство("Ключ") И СтруктураЗначения[0].Свойство("Значение") Тогда
				Возврат Тип("Соответствие");
			ИначеЕсли СтруктураЗначения[0].Свойство("Представление") И СтруктураЗначения[0].Свойство("Значение")
	...
	ИначеЕсли ТипЗнч(СтруктураЗначения) = Тип("Структура") Тогда
		...
		КонецЕсли;
		Если СтруктураЗначения.Свойство("Тип") И СтруктураЗначения.Свойство("Ид") И СтруктураЗначения.Свойство(
			"ИдПолный") Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Если СтруктураЗначения.Свойство("Типы") И СтруктураЗначения.Свойство("КвалификаторыСтроки")
			И СтруктураЗначения.Свойство("КвалификаторыЧисла") И СтруктураЗначения.Свойство("КвалификаторыДаты") Тогда
...
Функция СтруктураВСтруктуру(Значение)
	...
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	Если Не (ТипЗнч(Значение) = Тип("Структура") Или ТипЗнч(Значение) = Тип("ФиксированнаяСтруктура")) Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция МассивИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь)
	...
	ИначеЕсли ТипЗнч(СтруктураЗначения) = Тип("Структура") Тогда
		...
		Если ВРег(СтруктураЗначения.__Тип) = ВРег("Массив") Или ВРег(СтруктураЗначения.__Тип) = ВРег(
...
Функция СоответствиеВСтруктуру(Значение)
	...
	Если Не (ТипЗнч(Значение) = Тип("Соответствие") Или ТипЗнч(Значение) = Тип("ФиксированноеСоответствие")) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Для Каждого ТекЭлемент Из Значение Цикл
		ВремСоответствие.Элементы.Добавить(Новый Структура("Ключ,
														   |Значение", ЗначениеВСтруктуру(ТекЭлемент.Ключ),
...
Функция СоответствиеИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь)
	...
	ИначеЕсли ТипЗнч(СтруктураЗначения) = Тип("Структура") Тогда
		...
		Если ВРег(СтруктураЗначения.__Тип) = ВРег("Соответствие") Или ВРег(СтруктураЗначения.__Тип) = ВРег(
	...
	Для Каждого ТекЭлемент Из Элементы Цикл
		ВремСоответствие.Вставить(ЗначениеИзСтруктуры(ТекЭлемент.Ключ, ПолучатьБитыеСсылки), ЗначениеИзСтруктуры(
			ТекЭлемент.Значение, ПолучатьБитыеСсылки));
	КонецЦикла;
...
Функция СписокЗначенийИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь)
	...
	Для Каждого ТекЭлемент Из Элементы Цикл
		ВремСписок.Добавить(ЗначениеИзСтруктуры(ТекЭлемент.Значение, ПолучатьБитыеСсылки), ТекЭлемент.Представление,
			ТекЭлемент.Пометка);
	КонецЦикла;
...
//	СписокСвойств       - Массив(Строка), Структура - Список заполняемых полей строки
//
Процедура СтрокаЗначенийИзСтруктуры(СтрокаЗначений, СтруктураЗначений, ПолучатьБитыеСсылки = Ложь,
	...
	Для Каждого ТекСвойство Из СписокСвойств Цикл
		...
		ИначеЕсли ТипЗнч(ТекСвойство) = Тип("КолонкаТаблицыЗначений") Или ТипЗнч(ТекСвойство) = Тип(
			"КолонкаДереваЗначений") Тогда
...
//
Функция ТаблицаЗначенийИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено)
	...
	Для Каждого ТекЭлемент Из Колонки Цикл
		...
		Если ТипЗнч(ТекЭлемент.Значение) = Тип("Структура") Тогда
			ВремТаб.Колонки.Добавить(ТекЭлемент.Ключ, ЗначениеИзСтруктуры(ТекЭлемент.Значение.ТипЗначения),
				ТекЭлемент.Значение.Заголовок, ТекЭлемент.Значение.Ширина);
		Иначе
...
//
Процедура СтрокиДереваИзСтруктуры(СтрокиДерева, СтруктураСтрок, ПолучатьБитыеСсылки = Ложь, СписокСвойств = Неопределено)
...
//
Функция ДеревоЗначенийИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено)
	...
	Для Каждого ТекЭлемент Из Колонки Цикл
		...
		КонецЕсли;
		Если ТипЗнч(ТекЭлемент.Значение) = Тип("Структура") Тогда
			ВремДерево.Колонки.Добавить(ТекЭлемент.Ключ, ЗначениеИзСтруктуры(ТекЭлемент.Значение.ТипЗначения),
				ТекЭлемент.Значение.Заголовок, ТекЭлемент.Значение.Ширина);
		Иначе
...
Функция ЗначениеВСтруктуру(Значение) Экспорт
	...
	Если ТипЗнч(Значение) = Тип("Строка") Или ТипЗнч(Значение) = Тип("Число") Или ТипЗнч(Значение) = Тип("Булево")
		Или ТипЗнч(Значение) = Тип("Дата") Или ТипЗнч(Значение) = Тип("ХранилищеЗначения") Или ТипЗнч(Значение) = Тип(
		"УникальныйИдентификатор") Или ТипЗнч(Значение) = Тип("NULL") Или ТипЗнч(Значение) = Тип("Неопределено") Тогда
		Возврат Значение;
	КонецЕсли;
...
Функция ЗначениеИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь) Экспорт
	...
	Если ТипЗнч(СтруктураЗначения) = Тип("Строка") Или ТипЗнч(СтруктураЗначения) = Тип("Число") Или ТипЗнч(
		СтруктураЗначения) = Тип("Булево") Или ТипЗнч(СтруктураЗначения) = Тип("Дата") Или ТипЗнч(СтруктураЗначения)
		= Тип("ХранилищеЗначения") Или ТипЗнч(СтруктураЗначения) = Тип("УникальныйИдентификатор") Или ТипЗнч(
		СтруктураЗначения) = Тип("Неопределено") Или ТипЗнч(СтруктураЗначения) = Тип("NULL") Тогда
...
//  ИсключаяСвойства       - Строка, Массив(Строка)	    - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ЗаполнитьОбъектИзСтруктуры(Объект, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь,
	...
	Исключение
		ВызватьИсключение СтрШаблонВнутр("Ошибка получения описания вида объекта ""%1"": %2%3", ТипОбъекта, Символы.ПС,
	...
	ИначеЕсли ВРег(ОписаниеВида.Имя) = ВРег("ПланВидовХарактеристик") Тогда
		ПланВидовХарактеристикИзСтруктуры(Объект, СтруктураОбъекта, ПолучатьБитыеСсылки, СписокСвойств,
			ИсключаяСвойства);
	ИначеЕсли ВРег(ОписаниеВида.Имя) = ВРег("ПланСчетов") Тогда
...
//
Функция СоздатьОбъектИзСтруктуры(СтруктураОбъекта, УстановитьСсылку = Ложь, ПолучатьБитыеСсылки = Ложь,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
...
Функция СсылкаВСтруктуру(Значение) Экспорт
	...
	Иначе
		...
		Если Метаданные.Справочники.Содержит(МетаданныеЗначения) Или Метаданные.ПланыВидовХарактеристик.Содержит(
			МетаданныеЗначения) Или Метаданные.ПланыСчетов.Содержит(МетаданныеЗначения)
			Или Метаданные.ПланыВидовРасчета.Содержит(МетаданныеЗначения) Тогда
	...
	КодДополненияЗначения = КодДополненияЗначения + СокрЛП(ПравилоВыгрузки.ФункцияДополнения)
		+ "(ОписаниеЗначения, Значение);";

	Попытка
		Выполнить (КодДополненияЗначения);
	Исключение
		ТекстОшибки = "Ошибка дополнения значения: " + СокрЛП(Значение) + Символы.ПС + ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция СсылкаИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь) Экспорт
	...
	Если МетаОбъект = Неопределено Тогда
		ТекстОшибки = "Не найден тип " + ПравилоЗагрузки.ТипЗначения + " для значения: """ + СтруктураЗначения.Представление
	...
	КодПоискаСсылки = КодПоискаСсылки + СокрЛП(ПравилоЗагрузки.ФункцияПоиска)
		+ "(ОписаниеВида, МетаОбъект, СтруктураЗначения);";

	Попытка
		Выполнить (КодПоискаСсылки);
	Исключение
		ТекстОшибки = "Ошибка поиска ссылки по описанию: " + Символы.ПС + ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;

	Если Не ЗначениеЗаполнено(Результат) Тогда
		ПоказатьСообщение("Не найдено значение " + МетаОбъект.ПолноеИмя() + ": """ + СтруктураЗначения.Представление
			+ " (" + СтруктураЗначения.Ид + ")""");
...
//
//
Функция НаборЗаписейРегистраВСтруктуру(НаборЗаписей, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	ИначеЕсли ВРег(ОписаниеВида.Имя) = ВРег("РегистрБухгалтерии") Тогда
		СтруктураНабораЗаписей = НаборЗаписейРегистраБухгалтерииВСтруктуру(НаборЗаписей, СписокСвойств,
			ИсключаяСвойства);
...
//  ИсключаяСвойства	 - Строка, Массив(Строка)        - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ЗаполнитьНаборЗаписейРегистраИзСтруктуры(НаборЗаписей, СтруктураНабораЗаписей, ПолучатьБитыеСсылки,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Если ВРег(ОписаниеВида.Имя) = ВРег("РегистрСведений") Тогда
		НаборЗаписейРегистраСведенийИзСтруктуры(НаборЗаписей, СтруктураНабораЗаписей, ПолучатьБитыеСсылки, СписокСвойств,
			ИсключаяСвойства);
	ИначеЕсли ВРег(ОписаниеВида.Имя) = ВРег("РегистрНакопления") Тогда
		НаборЗаписейРегистраНакопленияИзСтруктуры(НаборЗаписей, СтруктураНабораЗаписей, ПолучатьБитыеСсылки,
			СписокСвойств, ИсключаяСвойства);
	ИначеЕсли ВРег(ОписаниеВида.Имя) = ВРег("РегистрБухгалтерии") Тогда
		НаборЗаписейРегистраБухгалтерииИзСтруктуры(НаборЗаписей, СтруктураНабораЗаписей, ПолучатьБитыеСсылки,
			СписокСвойств, ИсключаяСвойства);
	ИначеЕсли ВРег(ОписаниеВида.Имя) = ВРег("РегистрРасчета") Тогда
		НаборЗаписейРегистраРасчетаИзСтруктуры(НаборЗаписей, СтруктураНабораЗаписей, ПолучатьБитыеСсылки, СписокСвойств,
			ИсключаяСвойства);
	Иначе
		ПоказатьСообщение(СтрШаблонВнутр("Неизвестный тип объекта: %1", МетаОбъект.ПолноеИмя()));
	КонецЕсли;
...
//  ИсключаяСвойства	 - Строка, Массив(Строка)        - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Функция СоздатьНаборЗаписейРегистраИзСтруктуры(СтруктураНабораЗаписей, ПолучатьБитыеСсылки,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Конструктор) Тогда
		...
		ЗаполнитьНаборЗаписейРегистраИзСтруктуры(НаборЗаписей, СтруктураНабораЗаписей, ПолучатьБитыеСсылки,
			СписокСвойств, ИсключаяСвойства);
...
// Возвращаемое значение:
//
Процедура ДвиженияДокументаИзСтруктуры(Объект, СтруктураДвижений, ПолучатьБитыеСсылки = Ложь,
	СписокРегистров = Неопределено, ИсключаяРегистры = Неопределено) Экспорт
...
//  ИсключаяСвойства    - Строка, Массив(Строка)    - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура СправочникИзСтруктуры(Объект, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь, СписокСвойств = Неопределено,
	...
	Если МетаОбъект.Иерархический Тогда
		СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, "Родитель", ПолучатьБитыеСсылки, СписокСвойств,
			ИсключаяСвойства);
	КонецЕсли;

	Если МетаОбъект.Владельцы.Количество() > 0 Тогда
		СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, "Владелец", ПолучатьБитыеСсылки, СписокСвойств,
			ИсключаяСвойства);
	КонецЕсли;
	...
	Для Каждого ТекРеквизит Из МетаОбъект.Реквизиты Цикл
		Если МетаОбъект.Иерархический И МетаОбъект.ВидИерархии = МетаСвойства.ВидИерархии.ИерархияГруппИЭлементов Тогда
			...
			КонецЕсли;
		КонецЕсли;
		СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, ТекРеквизит.Имя, ПолучатьБитыеСсылки, СписокСвойств,
			ИсключаяСвойства);
	КонецЦикла;
	...
	Для Каждого ТекТЧ Из МетаОбъект.ТабличныеЧасти Цикл
		Если МетаОбъект.Иерархический И МетаОбъект.ВидИерархии = МетаСвойства.ВидИерархии.ИерархияГруппИЭлементов Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ТЧОбъектаИзСтруктуры(Объект, СтруктураОбъекта, ТекТЧ.Имя, ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
	КонецЦикла;
...
Функция ПланСчетовВСтруктуру(Объект, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Если ИспользоватьСвойство("ВидыСубконто", СписокСвойств, ИсключаяСвойства) И МетаОбъект.МаксКоличествоСубконто > 0 Тогда
		й = 1;
		Для Каждого ТекСтрока Из Объект.ВидыСубконто Цикл
			СтрокаСубконто = Новый Структура("НомерСтроки, ВидСубконто, ТолькоОбороты", й, ЗначениеВСтруктуру(
				ТекСтрока.ВидСубконто), ТекСтрока.ТолькоОбороты);
...
//  ИсключаяСвойства    - Строка, Массив(Строка)    - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ПланСчетовИзСтруктуры(Объект, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено) Экспорт
	...
	Для Каждого ТекПризнак Из МетаОбъект.ПризнакиУчета Цикл
		СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, ТекПризнак.Имя, ПолучатьБитыеСсылки, СписокСвойств,
			ИсключаяСвойства);
	КонецЦикла;

	Если СтруктураОбъекта.Свойство("ВидыСубконто") И ИспользоватьСвойство("ВидыСубконто", СписокСвойств,
		ИсключаяСвойства) И МетаОбъект.МаксКоличествоСубконто > 0 Тогда
...
//  ИсключаяСвойства    - Строка, Массив(Строка)    - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ПланВидовХарактеристикИзСтруктуры(Объект, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Для Каждого ТекРеквизит Из МетаОбъект.Реквизиты Цикл
		Если МетаОбъект.Иерархический Тогда
			Если (Объект.ЭтоГруппа И ТекРеквизит.Использование
				= Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляЭлемента) Или (Не Объект.ЭтоГруппа
	...
	Для Каждого ТекТЧ Из МетаОбъект.ТабличныеЧасти Цикл
		Если МетаОбъект.Иерархический Тогда
			Если (Объект.ЭтоГруппа И ТекТЧ.Использование
				= Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляЭлемента) Или (Не Объект.ЭтоГруппа
				И ТекТЧ.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляГруппы) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		ТЧОбъектаИзСтруктуры(Объект, СтруктураОбъекта, ТекТЧ.Имя, ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
...
Функция ПланВидовРасчетаВСтруктуру(Объект, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Если МетаОбъект.ИспользованиеПериодаДействия И ИспользоватьСвойство("ВытесняющиеВидыРасчета", СписокСвойств,
		ИсключаяСвойства) Тогда
...
//  ИсключаяСвойства    - Строка, Массив(Строка)    - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ПланВидовРасчетаИзСтруктуры(Объект, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
...
Функция ПланОбменаВСтруктуру(Объект, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Исключение
		Если ИспользоватьСвойство("ЭтотУзел", СписокСвойств, ИсключаяСвойства) Тогда
			Если Объект.Ссылка.Пустая() Тогда
				СтруктураОбъекта.Вставить("ЭтотУзел", ПланыОбмена[МетаОбъект.Имя].ЭтотУзел()
...
//  ИсключаяСвойства    - Строка, Массив(Строка)    - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ПланОбменаИзСтруктуры(Объект, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено) Экспорт
...
//  ИсключаяСвойства    - Строка, Массив(Строка)    - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ДокументИзСтруктуры(Объект, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено) Экспорт
	...
	Если ИспользоватьСвойство("Движения", СписокСвойств, ИсключаяСвойства) И СтруктураОбъекта.Свойство("Движения") Тогда
		ДвиженияДокументаИзСтруктуры(Объект, СтруктураОбъекта.Движения, ПолучатьБитыеСсылки, СписокСвойств,
			ИсключаяСвойства);
	КонецЕсли;
...
//  ИсключаяСвойства    - Строка, Массив(Строка)    - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура БизнесПроцессИзСтруктуры(Объект, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено) Экспорт
	...
	СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, "ВедущаяЗадача", ПолучатьБитыеСсылки, СписокСвойств,
		ИсключаяСвойства);
...
//  ИсключаяСвойства    - Строка, Массив(Строка)    - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ЗадачаИзСтруктуры(Объект, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено) Экспорт
	...
	СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, "БизнесПроцесс", ПолучатьБитыеСсылки, СписокСвойств,
		ИсключаяСвойства);
	СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, "Выполнена", , СписокСвойств, ИсключаяСвойства);
	СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, "ТочкаМаршрута", ПолучатьБитыеСсылки, СписокСвойств,
		ИсключаяСвойства);
...
//			<имя поля>     - Произвольный          - Значения измерений, ресурсов и реквизитов записи регистра
//
Функция НаборЗаписейРегистраСведенийВСтруктуру(НаборЗаписей, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено) Экспорт
	...
	Для Каждого ТекСтрока Из НаборЗаписей Цикл
		...
		Если МетаОбъект.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору Тогда
			...
			Если ИспользоватьСвойство("НомерСтроки", СписокСвойств, ИсключаяСвойства) Тогда
				СтруктураСтроки.Вставить("НомерСтроки", ?(ТекСтрока.НомерСтроки = 0, НомерСтроки,
...
//  ИсключаяСвойства	 - Строка, Массив(Строка)        - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура НаборЗаписейРегистраСведенийИзСтруктуры(НаборЗаписей, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Иначе
		...
		Для Каждого ТекПоле Из МетаОбъект.Измерения Цикл
			Если Не (СтруктураОбъекта.Отбор.Свойство(ТекПоле.Имя) И ИспользоватьСвойство(ТекПоле.Имя, СписокСвойств,
				ИсключаяСвойства)) Тогда
				Продолжить;
			КонецЕсли;
	...
	Для Каждого ТекЗапись Из СтруктураОбъекта.Записи Цикл
		...
		Для Каждого ТекПоле Из МетаОбъект.Измерения Цикл
			Если Не (ТекЗапись.Свойство(ТекПоле.Имя) И ИспользоватьСвойство(ТекПоле.Имя, СписокСвойств,
				ИсключаяСвойства)) Тогда
				Продолжить;
			КонецЕсли;
...
//			<имя поля>      - Произвольный         - Значения измерений, ресурсов и реквизитов записи регистра
//
Функция НаборЗаписейРегистраНакопленияВСтруктуру(НаборЗаписей, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено) Экспорт
	...
	Для Каждого ТекСтрока Из НаборЗаписей Цикл
		...
		Если МетаОбъект.ВидРегистра = Метаданные.СвойстваОбъектов.ВидРегистраНакопления.Остатки Тогда
			СтруктураСтроки.Вставить("ВидДвижения", ?(ТекСтрока.ВидДвижения = ВидДвиженияНакопления.Приход, "Приход",
				"Расход"));
		КонецЕсли;
...
//  ИсключаяСвойства	 - Строка, Массив(Строка)        - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура НаборЗаписейРегистраНакопленияИзСтруктуры(НаборЗаписей, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
...
//			<имя поля>      - Произвольный         - Значения измерений, ресурсов и реквизитов записи регистра
//
Функция НаборЗаписейРегистраБухгалтерииВСтруктуру(НаборЗаписей, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено) Экспорт
	...
	Для Каждого ТекСтрока Из НаборЗаписей Цикл
		...
		Если МетаОбъект.ДлинаУточненияПериода > 0 И ИспользоватьСвойство("УточнениеПериода", СписокСвойств,
			ИсключаяСвойства) Тогда
		...
		Если МетаОбъект.Корреспонденция Тогда
			...
			СтруктураСтроки.Вставить("СубконтоДт", ЗначенияСубконтоЗаписиВСтруктуру(ТекСтрока, "Дт", СписокСвойств,
				ИсключаяСвойства));
			СтруктураСтроки.Вставить("СубконтоКт", ЗначенияСубконтоЗаписиВСтруктуру(ТекСтрока, "Кт", СписокСвойств,
				ИсключаяСвойства));
		Иначе
			СтруктураСтроки.Вставить("ВидДвижения", ?(ТекСтрока.ВидДвижения = ВидДвиженияБухгалтерии.Дебет, "Дебет",
				"Кредит"));

			СтруктураСтроки.Вставить("Счет", ЗначениеВСтруктуру(ТекСтрока.Счет));

			СтруктураСтроки.Вставить("Субконто", ЗначенияСубконтоЗаписиВСтруктуру(ТекСтрока, "", СписокСвойств,
				ИсключаяСвойства));
		КонецЕсли;
...
//  ИсключаяСвойства	 - Строка, Массив(Строка)        - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура НаборЗаписейРегистраБухгалтерииИзСтруктуры(НаборЗаписей, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Для Каждого ТекЗапись Из СтруктураОбъекта.Записи Цикл
		...
		Если МетаОбъект.Корреспонденция Тогда
			...
			ЗначенияСубконтоЗаписиИзСтруктуры(НоваяЗапись, ТекЗапись, "Дт", ПолучатьБитыеСсылки, СписокСвойств,
				ИсключаяСвойства);
			ЗначенияСубконтоЗаписиИзСтруктуры(НоваяЗапись, ТекЗапись, "Кт", ПолучатьБитыеСсылки, СписокСвойств,
				ИсключаяСвойства);
		Иначе
			...
			ЗначенияСубконтоЗаписиИзСтруктуры(НоваяЗапись, ТекЗапись, "", ПолучатьБитыеСсылки, СписокСвойств,
				ИсключаяСвойства);
		КонецЕсли;

		Для Каждого ТекПоле Из МетаОбъект.Измерения Цикл
			Если МетаОбъект.Корреспонденция И Не ТекПоле.Балансовый Тогда
				ЗаполнитьИзмерениеРегистраБухгалтерии(НоваяЗапись, ТекЗапись, ТекПоле, "Дт", ПолучатьБитыеСсылки,
					СписокСвойств, ИсключаяСвойства);
				ЗаполнитьИзмерениеРегистраБухгалтерии(НоваяЗапись, ТекЗапись, ТекПоле, "Кт", ПолучатьБитыеСсылки,
					СписокСвойств, ИсключаяСвойства);
			Иначе
				ЗаполнитьИзмерениеРегистраБухгалтерии(НоваяЗапись, ТекЗапись, ТекПоле, "", ПолучатьБитыеСсылки,
					СписокСвойств, ИсключаяСвойства);
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого ТекПоле Из МетаОбъект.Ресурсы Цикл
			Если МетаОбъект.Корреспонденция И Не ТекПоле.Балансовый Тогда
				ЗаполнитьРесурсРегистраБухгалтерии(НоваяЗапись, ТекЗапись, ТекПоле, "Дт", СписокСвойств,
					ИсключаяСвойства);
				ЗаполнитьРесурсРегистраБухгалтерии(НоваяЗапись, ТекЗапись, ТекПоле, "Кт", СписокСвойств,
					ИсключаяСвойства);
			Иначе
				ЗаполнитьРесурсРегистраБухгалтерии(НоваяЗапись, ТекЗапись, ТекПоле, "", СписокСвойств, ИсключаяСвойства);
			КонецЕсли;
		КонецЦикла;
...
//			<имя поля>      - Произвольный         - Значения измерений, ресурсов и реквизитов записи регистра
//
Функция НаборЗаписейРегистраРасчетаВСтруктуру(НаборЗаписей, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено) Экспорт
	...
	Для Каждого ТекСтрока Из НаборЗаписей Цикл
		...
		Если МетаОбъект.ПериодДействия Тогда
			...
			Для Каждого ТекПериод Из ТекСтрока.ФактическийПериодДействия Цикл
				МассивПериодов.Добавить(Новый Структура("ДатаНачала, ДатаОкончания", ТекПериод.ДатаНачала,
					ТекПериод.ДатаОкончания));
			КонецЦикла;
...
//  ИсключаяСвойства	 - Строка, Массив(Строка)        - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура НаборЗаписейРегистраРасчетаИзСтруктуры(НаборЗаписей, СтруктураОбъекта, ПолучатьБитыеСсылки = Ложь,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
...
//
Функция ДанныеФормыВСтруктуру(Форма, ТолькоСохраняемые = Ложь, СписокСвойств = Неопределено,
	ИсключаяСвойства = Неопределено) Экспорт
	...
	Для Каждого ТекРеквизит Из РеквизитыФормы Цикл
		...
		Если ТипРеквизита = Тип("ДанныеФормыСтруктура") Или ТипРеквизита = Тип("ДанныеФормыСтруктураСКоллекцией")
			Или ТипРеквизита = Тип("ДанныеФормыКоллекция") Или ТипРеквизита = Тип("ДанныеФормыДерево") Тогда
...
//  ИсключаяСвойства	 - Строка, Массив(Строка)  - Список исключаемых свойств (если пустой, то допустимы все свойства)
//
Процедура ДанныеФормыИзСтруктуры(Форма, СтруктураДанных, ПроверятьИмяФормы = Ложь, СоздаватьРеквизиты = Ложь,
	СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт

	Если Не ТипЗнч(Форма) = Тип("ФормаКлиентскогоПриложения") Тогда
		ТекстОшибки = СтрШаблонВнутр("Попытка заполнения объекта типа ""%1"" данными для объекта типа ""%2""!", ТипЗнч(
	...
	Если Не ВРег(Лев(СтруктураДанных.__Тип, СтрДлина("ДанныеФормы"))) = ВРег("ДанныеФормы") Тогда
		ТекстОшибки = СтрШаблонВнутр("Тип данных ""%1"" структуры данных не является данными формы",
	...
	Если ПроверятьИмяФормы И ВРег(СтруктураДанных.__Тип) = ВРег(СтрШаблонВнутр("ДанныеФормы.%1", Форма.ИмяФормы)) Тогда
		ТекстОшибки = СтрШаблонВнутр("Тип данных ""%1"" структуры данных не является данными формы ""%2""!",
			СтруктураДанных.__Тип, Форма.ИмяФормы);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Для Каждого ТекРеквизит Из СтруктураДанных Цикл
		...
		Если СоздаватьРеквизиты И ИменаРеквизитовФормы.Получить(ТекРеквизит.Ключ) = Неопределено Тогда
			СоздаваемыеРеквизиты.Добавить(Новый РеквизитФормы(ТекРеквизит.Ключ, ЗначениеИзСтруктуры(
				ТекРеквизит.Значение.ТипЗначения), ТекРеквизит.Значение.Путь, ТекРеквизит.Значение.Заголовок,
...
Функция СКДВСтруктуру(Объект) Экспорт
	...
	СтруктураОбъекта.Вставить("МакетыЗаголовковГруппировок", КоллекцияСКДВМассив(Объект.МакетыЗаголовковГруппировок));
	...
	СтруктураОбъекта.Вставить("ВложенныеСхемыКомпоновкиДанных", КоллекцияСКДВМассив(
		Объект.ВложенныеСхемыКомпоновкиДанных));
...
Функция ПараметрСКДВСтруктуру(Объект)
	...
	СтруктураОбъекта.Вставить("Использование", ПеречислениеСКДВСтроку(Объект.Использование));
...
Функция ВыражениеУпорядочиванияСКДВСтруктуру(Объект)
	...
	СтруктураОбъекта.Вставить("Выражение", Объект.Выражение);
	СтруктураОбъекта.Вставить("ТипУпорядочивания", ПеречислениеСКДВСтроку(Объект.ТипУпорядочивания));
...
Функция ЗначениеПараметраНастроекСКДВСтруктуру(Объект)
	...
	СтруктураОбъекта.Вставить("ЗначенияВложенныхПараметров", КоллекцияСКДВМассив(Объект.ЗначенияВложенныхПараметров));
	СтруктураОбъекта.Вставить("ИдентификаторПользовательскойНастройки", Объект.ИдентификаторПользовательскойНастройки);
	СтруктураОбъекта.Вставить("ПредставлениеПользовательскойНастройки", Объект.ПредставлениеПользовательскойНастройки);
...
//
Процедура ЗначенияПараметровОбъектаСКДИзСтруктуры(ОбъектВладелец, Объект, СтруктураОбъекта,
	...
	Для Каждого ТекЭлемент Из СтруктураОбъекта[ИмяКоллекцииПараметров] Цикл
		...
		Иначе
			...
			КонецЕсли;
			ЗначенияПараметровОбъектаСКДИзСтруктуры(ОбъектВладелец, ТекПараметр, ТекЭлемент,
				"ЗначенияВложенныхПараметров");
...
Функция ПараметрОбластиРасшифровкаСКДВСтруктуру(Объект)
	...
	СтруктураОбъекта.Вставить("ОсновноеДействие", ПеречислениеСКДВСтроку(Объект.ОсновноеДействие));
...
Функция ПолеНабораДанныхПолеСКДВСтруктуру(Объект)
	...
	СтруктураОбъекта.Вставить("ОграничениеИспользования", ЗначениеСКДВСтруктуру(Объект.ОграничениеИспользования));
	СтруктураОбъекта.Вставить("ОграничениеИспользованияРеквизитов", ЗначениеСКДВСтруктуру(
		Объект.ОграничениеИспользованияРеквизитов));
...
Функция РольПоляНабораДанныхСКДВСтруктуру(Объект)
	...
	СтруктураОбъекта.Вставить("ТипБухгалтерскогоОстатка", ПеречислениеСКДВСтроку(Объект.ТипБухгалтерскогоОстатка));
...
Процедура РольПоляНабораДанныхСКДИзСтруктуры(Объект, СтруктураОбъекта)
	...
	Если СтруктураОбъекта.Свойство("ТипБухгалтерскогоОстатка") Тогда
		Объект.ТипБухгалтерскогоОстатка = ТипБухгалтерскогоОстаткаКомпоновкиДанных[СтруктураОбъекта.ТипБухгалтерскогоОстатка];
	КонецЕсли;
...
Функция ГруппаЭлементовОтбораСКДВСтруктуру(Объект)
	...
	СтруктураОбъекта.Вставить("Представление", Объект.Представление);
	СтруктураОбъекта.Вставить("ПредставлениеПользовательскойНастройки", Объект.ПредставлениеПользовательскойНастройки);
	СтруктураОбъекта.Вставить("Применение", ПеречислениеСКДВСтроку(Объект.Применение));
	СтруктураОбъекта.Вставить("РежимОтображения", ПеречислениеСКДВСтроку(Объект.РежимОтображения));
	// TODO: проверить родителя
	СтруктураОбъекта.Вставить("ТипГруппы", ПеречислениеСКДВСтроку(Объект.ТипГруппы));
...
Функция ГруппировкаСКДВСтруктуру(Объект)
	...
	// TODO: проверить родителя
	СтруктураОбъекта.Вставить("Состояние", ПеречислениеСКДВСтроку(Объект.Состояние));
...
Функция ПолеГруппировкиСКДВСтруктуру(Объект)
	...
	СтруктураОбъекта.Вставить("НачалоПериода", ?(ТипЗнч(Объект.НачалоПериода) = Тип("ПолеКомпоновкиДанных"),
		ЗначениеСКДВСтруктуру(Объект.НачалоПериода), Объект.НачалоПериода));
	СтруктураОбъекта.Вставить("КонецПериода", ?(ТипЗнч(Объект.КонецПериода) = Тип("ПолеКомпоновкиДанных"),
		ЗначениеСКДВСтруктуру(Объект.КонецПериода), Объект.КонецПериода));
	СтруктураОбъекта.Вставить("Поле", ЗначениеСКДВСтруктуру(Объект.Поле));
...
Процедура ПолеГруппировкиСКДИзСтруктуры(Объект, СтруктураОбъекта)
	...
	Если СтруктураОбъекта.Свойство("НачалоПериода") Тогда
		...
		Иначе
			ПоказатьСообщение(СтрШаблонВнутр("Неизвестный тип поля ""НачалоПериода"": %1",
	...
	Если СтруктураОбъекта.Свойство("КонецПериода") Тогда
		...
		Иначе
			ПоказатьСообщение(СтрШаблонВнутр("Неизвестный тип поля ""КонецПериода"": %1",
				СтруктураОбъекта.НачалоПериода));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗначенияПараметровВыводаГруппировкиСКДИзСтруктуры(Объект, СтруктураОбъекта)
	...
	Если Не (ТипЗнч(Объект) = Тип("ЗначенияПараметровВыводаГруппировкиКомпоновкиДанных") Или ТипЗнч(Объект) = Тип(
		"ЗначенияПараметровВыводаГруппировкиТаблицыКомпоновкиДанных") Или ТипЗнч(Объект) = Тип(
		"ЗначенияПараметровВыводаГруппировкиДиаграммыКомпоновкиДанных")) Тогда
		Возврат;
	КонецЕсли;
...
Функция ЭлементУсловногоОформленияСКДВСтруктуру(Объект)
	...
	СтруктураОбъекта.Вставить("ИспользоватьВГруппировке", ПеречислениеСКДВСтроку(Объект.ИспользоватьВГруппировке));
	СтруктураОбъекта.Вставить("ИспользоватьВЗаголовке", ПеречислениеСКДВСтроку(Объект.ИспользоватьВЗаголовке));
	СтруктураОбъекта.Вставить("ИспользоватьВЗаголовкеПолей", ПеречислениеСКДВСтроку(Объект.ИспользоватьВЗаголовкеПолей));
	СтруктураОбъекта.Вставить("ИспользоватьВИерархическойГруппировке", ПеречислениеСКДВСтроку(
		Объект.ИспользоватьВИерархическойГруппировке));
	СтруктураОбъекта.Вставить("ИспользоватьВОбщемИтоге", ПеречислениеСКДВСтроку(Объект.ИспользоватьВОбщемИтоге));
	СтруктураОбъекта.Вставить("ИспользоватьВОтборе", ПеречислениеСКДВСтроку(Объект.ИспользоватьВОтборе));
	СтруктураОбъекта.Вставить("ИспользоватьВПараметрах", ПеречислениеСКДВСтроку(Объект.ИспользоватьВПараметрах));
...
Процедура ЭлементУсловногоОформленияСКДИзСтруктуры(Объект, СтруктураОбъекта)
	...
	Если СтруктураОбъекта.Свойство("ИспользоватьВГруппировке") Тогда
		Объект.ИспользоватьВГруппировке = ИспользованиеУсловногоОформленияКомпоновкиДанных[СтруктураОбъекта.ИспользоватьВГруппировке];
	КонецЕсли;

	Если СтруктураОбъекта.Свойство("ИспользоватьВЗаголовке") Тогда
		Объект.ИспользоватьВЗаголовке = ИспользованиеУсловногоОформленияКомпоновкиДанных[СтруктураОбъекта.ИспользоватьВЗаголовке];
	КонецЕсли;

	Если СтруктураОбъекта.Свойство("ИспользоватьВЗаголовкеПолей") Тогда
		Объект.ИспользоватьВЗаголовкеПолей = ИспользованиеУсловногоОформленияКомпоновкиДанных[СтруктураОбъекта.ИспользоватьВЗаголовкеПолей];
	КонецЕсли;

	Если СтруктураОбъекта.Свойство("ИспользоватьВИерархическойГруппировке") Тогда
		Объект.ИспользоватьВИерархическойГруппировке = ИспользованиеУсловногоОформленияКомпоновкиДанных[СтруктураОбъекта.ИспользоватьВИерархическойГруппировке];
	КонецЕсли;

	Если СтруктураОбъекта.Свойство("ИспользоватьВОбщемИтоге") Тогда
		Объект.ИспользоватьВОбщемИтоге = ИспользованиеУсловногоОформленияКомпоновкиДанных[СтруктураОбъекта.ИспользоватьВОбщемИтоге];
	КонецЕсли;

	Если СтруктураОбъекта.Свойство("ИспользоватьВОтборе") Тогда
		Объект.ИспользоватьВОтборе = ИспользованиеУсловногоОформленияКомпоновкиДанных[СтруктураОбъекта.ИспользоватьВОтборе];
	КонецЕсли;

	Если СтруктураОбъекта.Свойство("ИспользоватьВПараметрах") Тогда
		Объект.ИспользоватьВПараметрах = ИспользованиеУсловногоОформленияКомпоновкиДанных[СтруктураОбъекта.ИспользоватьВПараметрах];
	КонецЕсли;
...
Функция ЗначениеСКДВСтруктуру(Объект)
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ЗначенияПараметровВыводаГруппировкиКомпоновкиДанных") Или ТипЗнч(Объект) = Тип(
		"ЗначенияПараметровВыводаГруппировкиТаблицыКомпоновкиДанных") Или ТипЗнч(Объект) = Тип(
		"ЗначенияПараметровВыводаГруппировкиДиаграммыКомпоновкиДанных") Тогда
...
Функция СтрРазделитьВнутр(Знач Стр, Разделитель = ",", ВключатьПустые = Истина) Экспорт
	...
	Исключение
		ПоказатьСообщение(СтрШаблонВнутр("Ошибка вызова метода СтрРазделить(): %1", ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке())));
	КонецПопытки;
...
Функция СтрШаблонВнутр(Знач Текст, Замена1 = Неопределено, Замена2 = Неопределено, Замена3 = Неопределено,
	Замена4 = Неопределено, Замена5 = Неопределено, Замена6 = Неопределено, Замена7 = Неопределено,
	Замена8 = Неопределено, Замена9 = Неопределено, Замена10 = Неопределено)
	Попытка
		Возврат Вычислить("СтрШаблон(Текст, Замена1, Замена2, Замена3,
						  |Замена4, Замена5, Замена6,
						  |Замена7, Замена8, Замена9, Замена10)");
	Исключение
		ПоказатьСообщение("Ошибка вызова метода СтрШаблон():" + Символы.ПС + ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке()));
	КонецПопытки;
