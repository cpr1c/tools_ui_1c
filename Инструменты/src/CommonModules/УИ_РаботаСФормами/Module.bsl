...
Процедура ФормаПриСозданииНаСервереСоздатьРеквизитыПараметровЗаписи(Форма, ГруппаФормы) Экспорт
	СтруктураПараметров=УИ_ОбщегоНазначенияКлиентСервер.СтруктураПараметровЗаписиПоУмолчанию();
	...
	Для Каждого КлючЗначение Из СтруктураПараметров Цикл
		ТипРеквизита=ТипЗнч(КлючЗначение.Значение);
			
		Если ТипРеквизита=Тип("Структура") Тогда
			ТипРеквизита=Тип("Произвольный");
		КонецЕсли;
	...
	Для Каждого ИмяСоздаваемогоЭлемента Из МассивДляСозданияЭлементов Цикл
		...
		ОписаниеЭлемента.СоздаватьЭлемент = МассивДляСозданияЭлементов.Найти(ПрефиксПараметра + ИмяСоздаваемогоЭлемента)
			<> Неопределено;
