...
Функция ВерсияПлатформыСтарше_8_3_14() Экспорт
	ВерсияБезСборки=ВерсияКонфигурацииБезНомераСборки(ТекущаяВерсияПлатформы1СПредприятие());
	
	Возврат СравнитьВерсииБезНомераСборки(ВерсияБезСборки,"8.3.14");
Конецфункции

Функция ПолеHTMLПостроеноНаWebkit() Экспорт
	Возврат ВерсияПлатформыСтарше_8_3_14() Или ЭтоLinux()
Конецфункции
...
Функция СериализоватьЗапросДляОтладки(ОбъектОтладки)
	...
	Если ОбъектОтладки.МенеджерВременныхТаблиц <> Неопределено Тогда
		СтруктураВременныхТаблиц = УИ_ОбщегоНазначенияВызовСервера.СтруктураВременныхТаблицМенеджераВременныхТаблиц(ОбъектОтладки.МенеджерВременныхТаблиц);
