		...
		Для Каждого Ссылка Из Удаляемые Цикл
			...
			Попытка
				Если УдалятьНепосредственно Тогда
					ОбработатьОбъектСПерехватомСообщений(Объект, "НепосредственноеУдаление", Неопределено,
						ПараметрыВыполнения);
				Иначе
...
Функция ЗаписатьОбъектВБазу(Объект, ПараметрыЗаписи, Знач Действие = "Запись", Знач РежимЗаписи = Неопределено) Экспорт
