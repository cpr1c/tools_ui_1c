...
Процедура ОткрытьКонсольОтладки(ТипОбъектаОтладки, ДанныеДляОтладки,
		УникальностьФормыКонсоли = Неопределено) Экспорт
...
Процедура ЗапуститьКонсольОтладкиПоСсылкеНаДанныеОтладки(СсылкаНаДанныеОтладки) Экспорт
	...
	ДанныеОтладки = УИ_ОбщегоНазначенияВызовСервера.СтруктураДанныхОбъектаОтладкиИзСправочникаДанныхОтладки(СсылкаНаДанныеОтладки);
	УИ_ОбщегоНазначенияКлиент.ОткрытьКонсольОтладки(ДанныеОтладки.ТипОбъектаОтладки, ДанныеОтладки.АдресОбъектаОтладки, СсылкаНаДанныеОтладки);
КонецПроцедуры
...
Процедура НачатьЗапускПриложенияЗавершениеПустое(КодВозврата,
...
Процедура ОткрытьФормуРедактированияТекста(Текст, ОписаниеОповещенияОЗакрытии,
		Заголовок = "") Экспорт
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Текст", Текст);
	ПараметрыФормы.Вставить("Заголовок", Заголовок);

	ОткрытьФорму("ОбщаяФорма.УИ_ФормаРедактированияТекста", ПараметрыФормы, , , , , ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
КонецПроцедуры

#Область ХранилищеЗначения

Процедура РедактироватьХранилищеЗначения(Форма,
	...
	Если ОписаниеОповещения = Неопределено Тогда
		...
		ПараметрыОписанияОповещения.Вставить("АдресВременногоХранилищаЗначенияИлиЗначение", АдресВременногоХранилищаЗначенияИлиЗначение);
		ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("РедактироватьПараметрыЗаписиЗавершение", ЭтотОбъект, ПараметрыОписанияОповещения);
	Иначе
	...
	ОткрытьФорму("ОбщаяФорма.УИ_ФормаХранилищаЗначения", ПараметрыФормы, Форма, Форма.УникальныйИдентификатор, , , ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);

КонецПроцедуры

Процедура РедактироватьХранилищеЗначенияЗавершение(Результат,
		ДополнительныеПараметры) Экспорт
...
Процедура РедактироватьПараметрыЗаписи(Форма) Экспорт
	...
	ОткрытьФорму("ОбщаяФорма.УИ_ПараметрыЗаписи", ПараметрыФормы, Форма, , , , ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

Процедура РедактироватьПараметрыЗаписиЗавершение(Результат,
		ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
