...
Процедура ДобавитьСвойстваФоновыхЗаданий(Знач МассивФоновыхЗаданий, Знач ТаблицаСвойствФоновыхЗаданий)
	...
	Пока Индекс >= 0 Цикл
		...
		Если РегламентноеЗадание = Неопределено
		   И УИ_СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(ФоновоеЗадание.Ключ) Тогда
...
Процедура ВызватьИсключениеЕслиНетПраваАдминистрирования() Экспорт
	...
	Если УИ_ОбщегоНазначения.РазделениеВключено()
		И УИ_ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
...
Процедура ОбновленнаяТаблицаРегламентныхЗаданий(Параметры, АдресХранилища) Экспорт
	...
	ПодсистемаРаботаВМоделиСервисаСуществует = УИ_ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса");
	...
	Если ИдентификаторРегламентногоЗадания = Неопределено Тогда
		...
		Для каждого Задание Из ТекущиеЗадания Цикл
			Если НЕ УИ_ОбщегоНазначения.РазделениеВключено()
...
Процедура ОбновитьСтрокуТаблицыРегламентныхЗаданий(Строка, Задание)
	...
	СвойстваПоследнегоФоновогоЗадания = УИ_РегламентныеЗаданияСлужебный
